...
bool Table::Update(RA_Processor &ra_Processor,DCE::Socket *pSocket)
{
	...
	{
		ra_Processor.RemoveRequest(&r_UpdateTable); // It's going to fall out of scope, so we don't want the processor to retry sending a deleted request
	...
	for(map<int,MapTable *>::iterator it=g_GlobalConfig.m_mapUsersTables.begin();it!=g_GlobalConfig.m_mapUsersTables.end();++it)
	{
		...
		if( m_mapUsers2ChangedRowList.find(PK_Users) != m_mapUsers2ChangedRowList.end() )
		{
			...
			for(ListChangedRow::iterator itCR=pListChangedRow->begin();itCR!=pListChangedRow->end();++itCR)
			{
				...
				// Go through all our fields.  If any are foreign keys to another table, that row in the other table must be checked in as well
				for(MapField::iterator it=m_mapField.begin();it!=m_mapField.end();++it)
				{
					Field *pField = (*it).second;
...
bool Table::ConfirmDependency(ChangedRow *pChangedRow,Field *pField_Referring,Field *pField_ReferredTo)
{
	...
	// We refer to a table that's not being checked in.  That's okay if the row we're referncing has not been modified.  If
	// it has changed, then we must check in that table too, otherwise we will be uploading a row to the server that references
...
#pragma warning("take into account return value of 'record frozen'")
				...
				{
					ra_Processor.RemoveRequest(&r_CommitRow); // It's going to fall out of scope, so we don't want the processor to retry sending a deleted request
...
void Table::AddRow(R_CommitRow *pR_CommitRow,sqlCVSprocessor *psqlCVSprocessor)
{
	...
	sSQL << m_psc_id_next << "," << psqlCVSprocessor->m_i_psc_batch << ")"; // batch # todo - hack
