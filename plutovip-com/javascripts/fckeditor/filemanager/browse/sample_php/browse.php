...
// Init var :
	...
	$IMAGES_BASE_URL = 'userimages/';
	$IMAGES_BASE_DIR = $_SERVER['DOCUMENT_ROOT'].'/pluto_home/'.$IMAGES_BASE_URL;
	$IMAGES_BASE_URL = 'http://10.0.0.150/pluto_home/userimages/';
...
function walk_dir($path) {
	if ($dir = opendir($path)) {
		while (false !== ($file = readdir($dir)))
		{
			if ($file[0]==".") continue;
			if (is_dir($path."/".$file))
				$retval = array_merge($retval,walk_dir($file));
