...
/*virtual*/ bool CPlutoVMCUtil::DrawImage(const char *Filename,int X,int Y,int Width,int Height)
{
	...
	CPAlbImageViewerBasic* iViewer = CPAlbImageViewerBasic::NewL(m_pContainer, rect);
	
	if(KErrNone != iViewer->LoadImageL(string(Filename).Des(), EColor4K))
	{
		MessageBox("Invalid image file", "Error");
	}
	else
	{
		iViewer->ScaleOptimumL();
		while(iViewer->IsBusy());

		CFbsBitmap *m_iBitmap = iViewer->Bitmap();
		m_pGC->BitBlt(rect.iTl, m_iBitmap);
		bResult = true;
	}
...
{
	if(
		((CPlutoBTAppUi *)CCoeEnv::Static()->AppUi())->m_pBDCommandProcessor
	)
	{
		BDCommand *pCommand = new BD_PC_SelectedFromList(m_uGridSelectedItem); 
		((CPlutoBTAppUi *)CCoeEnv::Static()->AppUi())->m_pBDCommandProcessor->AddCommand(pCommand);
	}
...
/*virtual*/ bool CPlutoVMCUtil::SetVariable()
{
	...
	{
		if(((CPlutoBTAppUi *)CCoeEnv::Static()->AppUi())->m_pBDCommandProcessor)
		{
...
{ 
	...
	BDCommandProcessor_Symbian_Base* pBDCommandProcessor_Symbian_Base = 
		((CPlutoBTAppUi *)CCoeEnv::Static()->AppUi())->m_pBDCommandProcessor_Symbian_Bluetooth;


	BD_WhatDoYouHave *pBD_WhatDoYouHave = new BD_WhatDoYouHave();
	pBDCommandProcessor_Symbian_Base->AddCommand(pBD_WhatDoYouHave);

	pBDCommandProcessor_Symbian_Base->ProcessCommands(pBDCommandProcessor_Symbian_Base);
}
//------------------------------------------------------------------------------------------------------------------
