...
//for windows ONLY, return code for getRemoteDeviceInfo from the registrywow
#ifndef _WIN32_WCE
typedef enum
{
    REM_DEV_INFO_SUCCESS, //success response
	REM_DEV_INFO_EOF,     //no more device found
...
typedef struct
{
	...
	BOOL SetLocalDeviceName(BD_NAME BdName);
...
#ifndef _WIN32_WCE    
	BOOL GetBTWVersionInfo(BT_CHAR *p_version_info, int nSize);
	BOOL GetDKVersionInfo(BT_CHAR *p_version_info, int nSize);
#endif
    ...
    BOOL ReadCOMPortAssociation(tBT_REM_ASSOC_REC *pBuffList, DWORD dwBuffSize, DWORD *pdw_RquiredSixze);

    BOOL Add_Printer(LPCTSTR PortName, BD_ADDR bd_Addr);
...
private:
	...
	BOOL    GetDLLVersion(LPCSTR sDLLFileName, LPTSTR lpVersionInfo);
    TCHAR   m_szKeyName[MAX_PATH];
