...
#pragma warning(disable: 4255 4145 4018)
...
Rasterizer::Rasterizer( Surface* renderTarget )
:   m_surface( 0 ),
...
void Rasterizer::SetBlending( uint8_t alpha )
{
    ...
    {
/*bugfix_6*/ if (m_alphaRaster.get() == 0) //old_code// if (!m_alphaRaster)
/*bugfix_6*/		m_alphaRaster.reset(new Internal::AlphaRaster2D( alpha )); //old_code// m_alphaRaster = new Internal::AlphaRaster2D( alpha );
        else
            m_alphaRaster->SetAlpha( alpha );

/*bugfix_6*/		m_raster = m_alphaRaster.get(); //old_code// m_raster = m_alphaRaster;
    }
}
...
{
/*bugfix_6*/	if (m_genericRaster.get() == 0)//old_code// if (!m_genericRaster)
/*bugfix_6*/		m_genericRaster.reset(new Internal::GenericRaster2D( shader ));//old_code//     m_genericRaster = new Internal::GenericRaster2D( shader );
		else
			m_genericRaster->SetPixelShader( shader );
    
/*bugfix_6*/     m_raster = m_genericRaster.get(); //old_code// m_raster = m_genericRaster;
}
...
void Rasterizer::BlitRotated( int destX, int destY, float angle, const Surface* source )
{
    ...
    int diag = int(sqrtf(tw*tw+th*th) + 1.0f);
    ...
    int c = int(cosf( -angle ) * 1024.0f);
    int s = int(sinf( -angle ) * 1024.0f);
    int w = source->GetWidth() << 9;
    int h = source->GetHeight() << 9;
    ...
    T[1][0] = s;
    T[1][1] = c;
    T[2][0] = -c * destX - s * destY + w;
    T[2][1] =  s * destX - c * destY + h;
...
//J Elmore 08/18/2004
void Rasterizer::Blit( int destX, int destY, const Surface* source, const Surface* AlphaSurf, const Rect* srcRect_ )
{
    // Prepare blit source and destination rectangles
    Rect srcRect;
    if (srcRect_)
    ...
    // Drawing
	m_raster->BlitAlpha( *m_surface->m_buffer, destRect, *source->m_buffer, *AlphaSurf->m_buffer, srcRect );
...
void Rasterizer::DrawCircle( int x, int y, int radius, Pixel color )
{
    if (!m_clipper.IsEmpty())
    {
        ...
        {
            // Drawing
            m_raster->Circle( *m_surface->m_buffer, Point(x,y), radius, color, m_clipper );
            return;
        }
    }
    
    // Drawing
    m_raster->Circle( *m_surface->m_buffer, Point(x,y), radius, color );
}
...
{
    DrawHLine( x1, y1, x2-x1, color );		/*DrawHLine( x1, y1, x2-x1, color )*/// fixed by Benjamin Kharitonov (19 June 2005)
    DrawVLine( x1, y1+1, y2-y1-2, color );	/*DrawVLine( x1, y1, y2-y1, color );*/// fixed by Benjamin Kharitonov (19 June 2005)
    DrawVLine( x2-1, y1+1, y2-y1-2, color );/*DrawVLine( x2, y1, y2-y1, color );*/// fixed by Benjamin Kharitonov (19 June 2005)
    DrawHLine( x1, y2-1, x2-x1, color );	/*DrawHLine( x1, y2, x2-x1, color );*/// fixed by Benjamin Kharitonov (19 June 2005)
}
...
// Added 5/9/2003 by Frank W. Zammetti
void Rasterizer::DrawPoly( const Point points[], int pointCount, Pixel color )
{
...
void Rasterizer::FillPoly( const Point vertex_vector[], int vertex_cnt, Pixel color )
{
    // Useful declarations
    int i;
    const Point* current_vertex;
...
{
    // Return the pixel
    return *m_surface->m_buffer->GetPixels( x, y );
}
...
void Rasterizer::SetPixel( int x, int y, Pixel color )
{
    ...
    // Drawing
    m_raster->SetPixel( *m_surface->m_buffer, Point(x,y), color );
}
