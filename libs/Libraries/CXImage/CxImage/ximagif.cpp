...
bool CxImageGIF::Decode(CxFile *fp)
{
	...
	int iImage = 0;
	info.nNumFrames=get_num_frames(fp);
...
////////////////////////////////////////////////////////////////////////////////
bool CxImageGIF::Encode(CxFile * fp, CxImage ** pImages, int pagecount)
{
  try{
...
#if (CXIMAGE_GIF_COMPRESSION == 0)
	compressNONE(InitCodeSize+1, fp);
#endif
#if (CXIMAGE_GIF_COMPRESSION == 1)
	compressRLE(InitCodeSize+1, fp);
#endif
#if (CXIMAGE_GIF_COMPRESSION == 2)
...
//	EncodeLoopExtension(fp);
	...
	for (long y=0;y<head.biHeight;y+=cellh){
		for (long x=0;x<head.biWidth;x+=cellw){
			...
			if (IsTransparent()){
				tmp.SetTransIndex(0);
				tmp.SetPaletteIndex(0,GetTransColor());
			}
			...
			for (unsigned long j=0;j<h;j++){
				for (unsigned long k=0;k<w;k++){
					i=(BYTE)(1+k+cellw*j);
					tmp.SetPaletteIndex(i,GetPixelColor(x+k,head.biHeight-y-h+j));
...
////////////////////////////////////////////////////////////////////////////////
int CxImageGIF::get_num_frames(CxFile *fp)
{
	struct_image image;
	struct_TabCol TabCol;
