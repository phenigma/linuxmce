...
{	if(image==0 || hThread || !image->HasSelection()) pCmdUI->Enable(0);}
...
void CDemoDoc::OnTransformEllipse() 
{
	...
	CxImage *image2 = new CxImage(*image,false,false,false);
...
//		break;
	...
	case ID_CXIMAGE_CONTRAST:
		pDoc->image->Light((long)pDoc->m_fp[0],120);
		break;
	...
	case ID_CXIMAGE_EDGE:
		{
			long kernel[]={-1,-1,-1,-1,8,-1,-1,-1,-1};
		pDoc->image->Filter(kernel,3,0,0);
		break;
		}
...
{
	SubmitUndo();
	image->AlphaPaletteEnable(!image->IsAlphaPaletteEnabled());
	UpdateAllViews(NULL);	
}
//////////////////////////////////////////////////////////////////////////////
