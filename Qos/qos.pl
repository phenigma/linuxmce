...
print "--> Building Shaper\n";
  ...
  system($cmd);
  $cmd="tc class add dev $DEVICE parent 1:1 classid 1:70 htb rate 0"."kbit ceil $BAND"."kbit prio 7";
  system($cmd);
print "--> Securing shaper\n";
  $cmd="tc qdisc add dev $DEVICE parent 1:10 handle 10: sfq perturb 10";
  system($cmd);
  $cmd="tc qdisc add dev $DEVICE parent 1:11 handle 11: sfq perturb 10";
  system($cmd);
...
print "-> Seting Firewall\n";
  ...
  $cmd="iptables -t mangle -A PREROUTING -i $DEVICE --syn -m length --length 40:68 -j MARK --set-mark 1";
  system($cmd);
  $cmd="iptables -t mangle -A PREROUTING -i $DEVICE --tcp-flags ALL SYN,ACK -m length --length 40:68 -j MARK --set-mark 1";
  system($cmd);
  $cmd="iptables -t mangle -A PREROUTING -i $DEVICE --tcp-flags ALL ACK -m length --length 40:100 -j MARK --set-mark 1";
  system($cmd);
  $cmd="iptables -t mangle -A PREROUTING -i $DEVICE --tcp-flags ALL RST -j MARK --set-mark 1";
  system($cmd);
  $cmd="iptables -t mangle -A PREROUTING -i $DEVICE --tcp-flags ALL ACK,RST -j MARK --set-mark 1";
  system($cmd);
  $cmd="iptables -t mangle -A PREROUTING -i $DEVICE --tcp-flags ALL ACK,FIN -j MARK --set-mark 1";
  system($cmd);
