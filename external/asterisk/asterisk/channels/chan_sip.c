...
static struct ast_channel *sip_new(struct sip_pvt *i, int state, char *title)
{
	...
	if (tmp) {
		...
		ast_log(LOG_WARNING, "Channel capabilities: JOINT=%x, SIMPLE=%x, GLOBAL=%x\n", i->jointcapability, i->capability, global_capability);
		
		if (i->jointcapability)
			tmp->nativeformats = i->jointcapability; //ast_codec_choose(&i->prefs, i->jointcapability, 1);
		else if (i->capability)
			tmp->nativeformats = i->capability; //ast__codec_choose(&i->prefs, i->capability, 1);
		else
		...
		if (title) {
			...
			ast_log(LOG_WARNING, "Channel %s has native formats %x\n", tmp->name, tmp->nativeformats);
		}
		else
...
static struct ast_channel *sip_request(const char *type, int format, void *data, int *cause)
{
	...
	oldformat = format;
	format &= ((AST_FORMAT_MAX_AUDIO << 1) - 1) | ((AST_FORMAT_MAX_VIDEO << 1) - 1);
	if (!format) {
...
int load_module()
{
	...
	if (!res) {
		...
		if (ast_channel_register_ex(channeltype, tdesc, ((AST_FORMAT_MAX_AUDIO << 1) - 1) | ((AST_FORMAT_MAX_VIDEO << 1) - 1), sip_request, sip_devicestate)) {
