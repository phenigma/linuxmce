<?php
  ...
  *
  * $Id: phpagi.php,v 1.1 2005/03/02 22:12:19 gregmac Exp $
  *
  ...
  class AGI
  {
    ...
    */
    function AGI($config=NULL, $optconfig=array())
    {
      ...
      if(!isset($this->config['festival']['tempdir'])) $this->config['festival']['tempdir'] = AST_TMP_DIR;
      ...
      // make sure festival temp folder exists
      $this->make_folder($this->config['festival']['tempdir']);
    ...
    */
    ...
    {
      $message = str_replace("\n", '\n', addslashes($message));
      return $this->evaluate("VERBOSE \"$message\" $level");
    }
    ...
    */
    function parse_callerid($callerid=NULL)
    {
      ...
      {
        $callerid = explode($callerid{0}, $callerid);
        return array('name'=>$callerid[1], 'number'=>trim($callerid[2], '<> '));
      }
      return array('name'=>'', 'number'=>trim($callerid[2], '<> '));
    }
    ...
    */
    function text2wav($text, $escape_digits='', $frequency=8000)
    {
      ...
      $fname = $this->config['festival']['tempdir'] . DIRECTORY_SEPARATOR;
