...
B<pthsem> - GNU Portable Threads

=head1 VERSION

pthsem PTH_VERSION_STR
based on GNU Pth
...
C<PTH_UNTIL_COUNT> other values can be used. If the flag
C<PTH_UNTIL_DECREMENT> is used, the semaphore value is decremented (so
...
The data-type for the semaphore is names C<pth_sem_t> and it has an
initializer like C<pth_cond_t>.


=over 4

=item int B<pth_sem_init>(pth_sem_t *sem);

This dynamically initializes a semaphore variable of type `C<pth_sem_t>'.
Alternatively one can also use static initialization via `C<pth_sem_t
semaphore = PTH_SEM_INIT>'.

=item int B<pth_sem_dec>(pth_sem_t *sem);

waits, until the value of C<sem> is >= 1 and decrement it.

=item int B<pth_sem_dec_value>(pth_sem_t *sem, unsigned value);

waits, until the value of C<sem> is >= C<value> and subtracts C<value>.

=item int B<pth_sem_inc>(pth_sem_t *sem, int notify);

increments C<sem>. The scheduler is started, if C<notify> is not null.

=item int B<pth_sem_inc_value>(pth_sem_t *sem, unsigned value, int notify);

adds value to C<sem>. The scheduler is started, if C<notify> is not null.

=item int B<pth_sem_set_value>(pth_sem_t *sem, unsigned value);

sets the value of C<sem> to C<value>.

=item int B<pth_sem_get_value>(pth_sem_t *sem, unsigned *value);

stores the value of C<sem> in *C<value>.
