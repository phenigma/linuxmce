...
AC_ARG_WITH(cards,
  ...
  [                          rme96, rme9652, hdsp, hdspm, trident, ymfpci, ]
  [                          ad1889, als4000, cmipci, ens1370, ens1371, ]
  [                          es1938, es1968, maestro3, fm801, fm801-tea575x, ]
  [                          ice1712, ice1724, intel8x0, intel8x0m, ]
  [                          sonicvibes, via82xx, via82xx-modem, vx222, ]
  [                          hda-intel, pdplus, asihpi, pcxhr, darla20, ]
  [                          gina20, layla20, darla24, gina24, layla24, mona, ]
  [                          mia, echo3g, indigo, indigoio, indigodj, riptide, ]
  [                          powermac, sa11xx-uda1341, armaaci, pxa2xx-ac97, ]
  [                          s3c2410, au1x00, usb-audio, usb-usx2y, vxpocket, ]
...
for card in $SELECTED_CARDS; do
  ...
  if test "$card" = "all" -o "$card" = "portman2x4"; then
    if ( test "$CONFIG_PARPORT" = "y" -o "$CONFIG_PARPORT" = "m" ); then
      CONFIG_SND_RAWMIDI="m"
  ...
  if test "$card" = "all" -o "$card" = "pcsp"; then
    if ( test "$CONFIG_X86" = "y" -o "$CONFIG_X86" = "m" ) &&
  ...
  if test "$card" = "all" -o "$card" = "powermac"; then
    if ( test "$CONFIG_PPC" = "y" -o "$CONFIG_PPC" = "m" ) &&
  ...
  if test "$card" = "all" -o "$card" = "au1x00"; then
    if ( test "$CONFIG_MIPS" = "y" -o "$CONFIG_MIPS" = "m" ) &&
  ...
  if test "$card" = "all" -o "$card" = "usb-usx2y"; then
    if ( test "$CONFIG_USB" = "y" -o "$CONFIG_USB" = "m" ) &&
...
AC_ARG_WITH(card_options,
  ...
  [                          seq-rtctimer-default, generic-driver, sb16-csp, ]
...
for card in $SELECTED_OPTIONS; do
  ...
  if test "$card" = "all" -o "$card" = "generic-driver"; then
      CONFIG_SND_GENERIC_DRIVER="y"
      probed=1
  fi
...
if test -n "$CONFIG_SND_GENERIC_DRIVER"; then
  AC_DEFINE(CONFIG_SND_GENERIC_DRIVER)
fi
...
AC_SUBST(CONFIG_SND_GENERIC_DRIVER)
