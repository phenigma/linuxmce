...
static int alsa_read(snd_pcm_t *handle,char *buf,int nsamples)
{
	int err;
...
static int alsa_write(snd_pcm_t *handle, char *buf,int nsamples)
{
	int err;
	...
	}else if (err!=nsamples) ms_warning("Only %i samples written.",err);
	return err;
}
...
void alsa_write_process(MSFilter *obj){
	...
	while ((im=ms_queue_get(obj->inputs[0]))!=NULL){
		...
		alsa_write(ad->handle,im->b_rptr,samples);
		freemsg(im);
	}
}
