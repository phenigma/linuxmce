...
static int v4l_start(MSFilter *f, void *arg)
{
	...
	if (s->fd<0)
	{
		ms_error("MSV4l: cannot open video device: %s.",strerror(errno));
		return -1;
	}else{
		err=v4l_configure(s);
		if (err<0) 
...
static int v4l_stop(MSFilter *f, void *arg){
	V4lState *s=(V4lState*)f->data;
	if (s->fd>=0){
		if (s->mmapdbuf!=NULL) munmap(s->mmapdbuf,s->msize);
		s->mmapdbuf=NULL;
		close(s->fd);
		s->fd=-1;
		s->frame_count=-1;
...
static int v4l_configure(V4lState *s)
{
	...
	if (err<0){
		...
		if (vmbuf.size>0){
			/* do the mmap */
			s->msize=vmbuf.size;
			s->mmapdbuf=mmap((void*)s,vmbuf.size,PROT_READ,MAP_SHARED,s->fd,0);
			if (s->mmapdbuf==(void*)-1) {
				ms_error("Could not mmap.");
				s->mmapdbuf=NULL;
				return -1;
			}else {
				/* initialize the mediastreamer buffers */
				ms_message("Using %i-frames mmap'd buffer.",vmbuf.frames);
				for(i=0;i<vmbuf.frames;i++){
					mblk_t *buf=allocb_with_buf(s->mmapdbuf+vmbuf.offsets[i],vmbuf.offsets[1],0,NULL);
					buf->b_wptr+=vmbuf.offsets[1];
					s->frames[i]=buf;
				}
				s->frame_ind=0;
