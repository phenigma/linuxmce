...
int linphone_call_terminated(LinphoneCore *lc, eXosip_event_t *ev)
{
	...
	if (lc->call!=NULL){
		if (lc->call->cid!=ev->cid){
			...
			g_message("call %i terminated, this was not current call.",ev->cid);
			return 0;
		}
	}
	
	g_message("Current call terminated...");
	if (lc->ringstream!=NULL) {
		ring_stop(lc->ringstream);
		lc->ringstream=NULL;
	}
...
SupportLevel linphone_payload_is_supported(sdp_payload_t *payload,RtpProfile *local_profile,RtpProfile *dialog_profile)
{
	int localpt;
	SupportLevel ret;
...
void linphone_registration_faillure(LinphoneCore *lc,eXosip_event_t *ev)
{
	...
	switch(ev->status_code){
		...
		case 407:
			...
			break;
		default:
			if (strlen(ev->reason_phrase)==0) msg=g_strdup_printf(_("Registration on %s failed (timeout)."),ev->req_uri);
			else msg=ev->reason_phrase;
			lc->vtable.display_status(lc,msg);
			if ( msg != ev->reason_phrase ) g_free(msg);
	}
