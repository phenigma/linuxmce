...
# DO NOT EDIT
    ...
    if test "$?" = "0"; then
        ...
        if false; then :
			...
			c "Pinnacle Systems PCTV Sat receiver" \
			d "PixelView RemoteMaster RC2000/RC3000" \
			e "REALmagic (bundled with Hollywood Plus DVD card)" \
			f "Slink-e (receive only)" \
			g "Silitek SM-1000" \
			h "Tekram Irmate 210 (16x50 UART compatible serial port)" \
			i "UIRT2 (receive only, UIR mode)" \
			j "UIRT2 (receive and transmit)" \
			k "X10 MouseRemote RF Receiver (Serial)" \
			l "X10 MouseRemote RF Receiver (PS/2)" \
			m "X10 MP3 Anywhere RF Receiver" 2> $TEMP
            if test "$?" = "0"; then
                {
                set `cat $TEMP`
                if false; then :
                ...
                elif test "$1" = "d"; then LIRC_DRIVER=remotemaster;	DRIVER_PARAMETER=tty1; DRIVER_PARAM_TYPE=tty;
                elif test "$1" = "e"; then LIRC_DRIVER=realmagic;	DRIVER_PARAMETER=tty1; DRIVER_PARAM_TYPE=tty;
                elif test "$1" = "f"; then LIRC_DRIVER=slinke;		DRIVER_PARAMETER=tty3; DRIVER_PARAM_TYPE=tty;
                elif test "$1" = "g"; then LIRC_DRIVER=silitek;		DRIVER_PARAMETER=tty1; DRIVER_PARAM_TYPE=tty;
                elif test "$1" = "h"; then LIRC_DRIVER=tekram;		DRIVER_PARAMETER=com1; DRIVER_PARAM_TYPE=com;
                elif test "$1" = "i"; then LIRC_DRIVER=uirt2;		DRIVER_PARAMETER=tty1; DRIVER_PARAM_TYPE=tty;
                elif test "$1" = "j"; then LIRC_DRIVER=uirt2_raw;	DRIVER_PARAMETER=tty1; DRIVER_PARAM_TYPE=tty;
                elif test "$1" = "k"; then LIRC_DRIVER=mouseremote;	DRIVER_PARAMETER=tty1; DRIVER_PARAM_TYPE=tty;
                elif test "$1" = "l"; then LIRC_DRIVER=mouseremote_ps2;	DRIVER_PARAMETER=none; DRIVER_PARAM_TYPE=none;
                elif test "$1" = "m"; then LIRC_DRIVER=mp3anywhere;	DRIVER_PARAMETER=tty1; DRIVER_PARAM_TYPE=tty;
                fi
                }
                else
                    return;
			...
			8 "Iguanaworks USB IR Transceiver" \
			9 "PCMAK USB receiver" \
			0 "Remotec Multimedia PC Remote BW6130" \
			a "Sasem OnAir Remocon-V" \
			b "Sound Blaster Extigy USB sound card (exaudio)" \
			c "Sound Blaster Extigy/Audigy 2 NX (ALSA snd-usb-audio)" \
			d "Soundgraph iMON MultiMedian IR/VFD" \
			e "Soundgraph iMON Knob" \
			f "Soundgraph iMON PAD IR/VFD" \
			g "Soundgraph iMON RSC" \
			h "Streamzap PC Remote" \
			i "USB-UIRT" \
			j "Windows Media Center Remotes (old version, MicroSoft USB ID)" \
			k "Windows Media Center Remotes (new version, Philips et al.)" 2> $TEMP
            if test "$?" = "0"; then
                {
                set `cat $TEMP`
                if false; then :
                ...
                elif test "$1" = "8"; then LIRC_DRIVER=iguanaIR;	DRIVER_PARAMETER=none; DRIVER_PARAM_TYPE=none;
                elif test "$1" = "9"; then LIRC_DRIVER=pcmak_usb;	DRIVER_PARAMETER=ttyUSB1; DRIVER_PARAM_TYPE=ttyUSB;
                elif test "$1" = "0"; then LIRC_DRIVER=bw6130;		DRIVER_PARAMETER=none; DRIVER_PARAM_TYPE=none;
                elif test "$1" = "a"; then LIRC_DRIVER=sasem;		DRIVER_PARAMETER=none; DRIVER_PARAM_TYPE=none;
                elif test "$1" = "b"; then LIRC_DRIVER=exaudio;		DRIVER_PARAMETER=none; DRIVER_PARAM_TYPE=none;
                elif test "$1" = "c"; then LIRC_DRIVER=alsa_usb;	DRIVER_PARAMETER=none; DRIVER_PARAM_TYPE=none;
                elif test "$1" = "d"; then LIRC_DRIVER=imon;		DRIVER_PARAMETER=none; DRIVER_PARAM_TYPE=none;
                elif test "$1" = "e"; then LIRC_DRIVER=imon_knob;	DRIVER_PARAMETER=none; DRIVER_PARAM_TYPE=none;
                elif test "$1" = "f"; then LIRC_DRIVER=imon_pad;	DRIVER_PARAMETER=none; DRIVER_PARAM_TYPE=none;
                elif test "$1" = "g"; then LIRC_DRIVER=imon_rsc;	DRIVER_PARAMETER=none; DRIVER_PARAM_TYPE=none;
                elif test "$1" = "h"; then LIRC_DRIVER=streamzap;	DRIVER_PARAMETER=none; DRIVER_PARAM_TYPE=none;
                elif test "$1" = "i"; then LIRC_DRIVER=usb_uirt_raw;	DRIVER_PARAMETER=ttyUSB1; DRIVER_PARAM_TYPE=ttyUSB;
                elif test "$1" = "j"; then LIRC_DRIVER=mceusb;		DRIVER_PARAMETER=none; DRIVER_PARAM_TYPE=none;
                elif test "$1" = "k"; then LIRC_DRIVER=mceusb2;		DRIVER_PARAMETER=none; DRIVER_PARAM_TYPE=none;
                fi
                }
                else
                    return;
