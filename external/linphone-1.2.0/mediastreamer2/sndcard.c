...
gchar *snd_card_get_identifier(SndCard *obj)
{
...
gint sys_audio_manager_init(SndCardManager *manager, gint index)
{
	...
	very often... */
	manager->cards[index]=SND_CARD(hpux_snd_card_new("/dev/audio","/dev/audio"));
	return 1;
}
...
SndCard * snd_card_manager_get_card_with_string(SndCardManager *manager,char *cardname,int *index)
{
	int i;
	for (i=0;i<MAX_SND_CARDS;i++){
		...
		if (strncmp(card_name,cardname,strlen(card_name))==0){
			*index=i;
