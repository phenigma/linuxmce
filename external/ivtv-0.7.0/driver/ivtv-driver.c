				...
				const struct pci_device_id *pci_id)
{
	...
	case TUNER_PHILIPS_FM1236_MK3:
                cfg = TDA9887_PORT1_ACTIVE|TDA9887_PORT2_ACTIVE|TDA9887_TOP(top);
                ivtv_call_i2c_clients(itv, TDA9887_SET_CONFIG, &cfg);
		itv->set_fm_high_sensitivity = 1;
		break;

	case TUNER_PHILIPS_FQ1216AME_MK4:
		if (itv->tuner_std == V4L2_STD_SECAM_L ||
		    itv->tuner_std == V4L2_STD_SECAM_LC)
			top = -2;
		/* fall through */
	case TUNER_PHILIPS_FQ1236A_MK4:
                cfg = TDA9887_PORT1_ACTIVE|TDA9887_PORT2_INACTIVE|TDA9887_TOP(top);
                ivtv_call_i2c_clients(itv, TDA9887_SET_CONFIG, &cfg);
		itv->set_fm_high_sensitivity = 1;
		break;

	case TUNER_PANASONIC_VP27:
