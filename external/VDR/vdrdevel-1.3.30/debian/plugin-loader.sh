...
getplugins ()
{
    ...
    version=`LD_ASSUME_KERNEL=2.4.1 /usr/bin/vdr -V -L/usr/bin/vdr 2>/dev/null | sed -e 's/.*(\(.*\)).*/\1/'`
    ...
    if [ "$PLUGIN_CHECK_STARTABLE" == "yes" ]; then
        ...
        for (( i=${#installed_plugins[@]}, i-- ; i >= 0 ; i-- )); do
            if ! LD_ASSUME_KERNEL=2.4.1 /usr/bin/vdr -V -L $PLUGIN_DIR -P ${installed_plugins[$i]} \
