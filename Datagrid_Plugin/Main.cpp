...
extern "C" {
	...
	{ 
		...
		if( bIsRuntimePlugin )
			return Datagrid_Plugin::PK_DeviceTemplate_get_static();
		else
			return 0;
	}
}
...
extern "C" {
	class Command_Impl *RegisterAsPlugIn(class Router *pRouter,int PK_Device,string sLogger)
	{
		...
		{
			g_pPlutoLogger = new ServerLogger(PK_Device, Datagrid_Plugin::PK_DeviceTemplate_get_static(), "localhost");
...
int main(int argc, char* argv[]) 
{
	...
	cout << "Datagrid_Plugin, v." << VERSION << endl
	...
	if (bError)
	{
		cout << "A Pluto DCE Device.  See www.plutohome.com/dce for details." << endl
			<< "Usage: Datagrid_Plugin [-r Router's IP] [-d My Device ID] [-l dcerouter|stdout|null|filename]" << endl
	...
	{
		if( sLogger=="dcerouter" )
			g_pPlutoLogger = new ServerLogger(PK_Device, Datagrid_Plugin::PK_DeviceTemplate_get_static(), sRouter_IP);
