...
extern "C" {
	class Command_Impl *RegisterAsPlugIn(class Router *pRouter,int PK_Device,string sLogger)
	{
		...
		{
			g_pPlutoLogger = new ServerLogger(PK_Device, Infrared_Plugin::PK_DeviceTemplate_get_static(), "localhost");
...
int main(int argc, char* argv[]) 
{
	...
	if (bError)
	{
		cout << "A Pluto DCE Device.  See www.plutohome.com/dce for details." << endl
			<< "Usage: Infrared_Plugin [-r Router's IP] [-d My Device ID] [-l dcerouter|stdout|null|filename]" << endl
	...
	{
		if( sLogger=="dcerouter" )
			g_pPlutoLogger = new ServerLogger(PK_Device, Infrared_Plugin::PK_DeviceTemplate_get_static(), sRouter_IP);
	...
	try
	{
		...
		if ( pInfrared_Plugin->Connect(pInfrared_Plugin->PK_DeviceTemplate_get()) ) 
		{
			...
			pthread_join(pInfrared_Plugin->m_RequestHandlerThread, NULL);
