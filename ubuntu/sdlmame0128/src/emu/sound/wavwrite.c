...
wav_file *wav_open(const char *filename, int sample_rate, int channels)
{
	...
	foo = fwrite("RIFF", 1, 4, wav->file);

	/* write the total size */
	temp32 = 0;
	wav->total_offs = ftell(wav->file);
	foo = fwrite(&temp32, 1, 4, wav->file);

	/* write the 'WAVE' type */
	foo = fwrite("WAVE", 1, 4, wav->file);

	/* write the 'fmt ' tag */
	foo = fwrite("fmt ", 1, 4, wav->file);
	...
	foo = fwrite(&temp16, 1, 2, wav->file);
...
void wav_add_data_16(wav_file *wav, INT16 *data, int samples)
{
	...
	foo = fwrite(data, 2, samples, wav->file);
	fflush(wav->file);
}
...
void wav_add_data_32(wav_file *wav, INT32 *data, int samples, int shift)
{
	...
	/* write and flush */
	foo = fwrite(temp, 2, samples, wav->file);
	fflush(wav->file);
...
void wav_add_data_16lr(wav_file *wav, INT16 *left, INT16 *right, int samples)
{
	...
	/* write and flush */
	foo = fwrite(temp, 4, samples, wav->file);
	fflush(wav->file);
