...
GList *find_friend(GList *fl, const gchar *friend, LinphoneFriend **lf){
	...
	if (err<0){
		g_warning("Invalid friend to search sip uri: %s",friend);
		osip_from_free(tmpf);
		return NULL;
	}
	dummy.url=tmpf;
...
void __linphone_friend_do_subscribe(LinphoneFriend *fr){
	...
	if (err<0){
		g_warning("Could not subscribe to %s.",friend);
	}
	osip_free(friend);
}
...
gint linphone_friend_set_sip_addr(LinphoneFriend *lf, const gchar *addr){
	...
	if (err<0) {
		g_warning("Invalid friend sip uri: %s",addr);
		osip_from_free(fr);
		return -1;
	}
...
{
	g_return_if_fail(lf->lc==NULL);
	g_return_if_fail(lf->url!=NULL);
	linphone_friend_apply(lf,lc);
	lc->friends=g_list_insert_sorted(lc->friends,(gpointer)lf,friend_compare);
	return ;
}

void linphone_core_remove_friend(LinphoneCore *lc, LinphoneFriend* fl){
	GList *el=g_list_find(lc->friends,(gpointer)fl);
	if (el!=NULL){
		lc->friends=g_list_remove_link(lc->friends,el);
...
LinphoneSubscribePolicy __policy_str_to_enum(const char* pol){
	...
	g_warning("Unrecognized subscribe policy: %s",pol);
	return LinphoneSPWait;
}
