 ...
 *
 * $Date: 2013-04-09 18:13:28 +0200 (Tue, 09 Apr 2013) $
 * $Revision: 4554 $
 */
...
SCCP_FILE_VERSION(__FILE__, "$Revision: 4554 $")
...
static void __sccp_indicate_remote_device(sccp_device_t * device, sccp_channel_t * c, uint8_t state, uint8_t debug, char *file, int line, const char *pretty_function)
{
	...
	SCCP_LIST_TRAVERSE(&c->line->devices, linedevice, list) {
		...
		if (!device || !linedevice)
			break;

		if (!linedevice->device) {
			pbx_log(LOG_NOTICE, "Strange to meet a linedevice (%p) here without a valid device connected to it !", linedevice);
			break;
		}
