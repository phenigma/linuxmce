...
# DO NOT EDIT
    ...
    if test "$?" = "0"; then
        ...
        if false; then :
			...
			2 "Receiver/transmitter using PortAudio (EXPERIMENTAL)" \
            ...
            if test "$?" = "0"; then
                ...
                elif test "$1" = "2"; then LIRC_DRIVER=caraca;		DRIVER_PARAMETER=none; DRIVER_PARAM_TYPE=none;
                ...
                elif test "$1" = "8"; then LIRC_DRIVER=nslu2;		DRIVER_PARAMETER=void; DRIVER_PARAM_TYPE=none;
			...
			i "Hauppauge DVB-s card (ver. 2.1)" \
			j "Hercules Smart TV Stereo (card=100)" \
			k "I-O Data Co. GV-BCTV5/PCI (card=81)" \
			l "Jetway TV/Capture JW-TV878-FBK, Kworld KW-TV878RF (card=78)" \
			m "KNC ONE TV Station (-/SE/PRO/RDS)" \
			n "Lenco MXTV-9578 CP (card=50)" \
			o "Miro PCTV serial port receiver" \
			p "Phoebe Tv Master + FM (card=22)" \
			q "Pinnacle Systems PCTV Sat receiver" \
			r "Pixelview PlayTV MPEG2" \
			s "PixelView PlayTV PAK (card=50)" \
			t "Pixelview PlayTV pro (card=37)" \
			u "Pixelview PlayTV (bt878) (Prolink PV-BT878P+, card=16)" \
			v "Prolink Pixelview PV-BT878P+ (Rev.4C,8E, card=70)" \
			w "Prolink PV-BT878P+4E (card=50)" \
			x "Prolink PV-BT878P+9B (PlayTV Pro rev.9B FM+NICAM) (card=72)" \
			y "ProVideo PV951 (card=42)" \
			z "Technisat MediaFocus I" \
			A "Tekram M230 Mach64 (and others bt829 based)" \
			B "TriTan Technology TView95 CPH03x (card=1)" \
			C "TView99 CPH063 (card=38)" \
			D "Typhoon TView RDS / FM Stereo (card=53)" \
			E "Winfast PVR2000 (Linux kernel >=2.6.11 required)" \
			F "Winfast TV2000/XP (card=34)" \
			G "WinView 601 (card=17)" 2> $TEMP
            if test "$?" = "0"; then
                {
                set `cat $TEMP`
                if false; then :
                ...
                elif test "$1" = "h"; then LIRC_DRIVER=hauppauge;	DRIVER_PARAMETER=none; DRIVER_PARAM_TYPE=none;
                elif test "$1" = "i"; then LIRC_DRIVER=hauppauge_dvb;	DRIVER_PARAMETER=none; DRIVER_PARAM_TYPE=none;
                elif test "$1" = "j"; then LIRC_DRIVER=hercules_smarttv_stereo;	DRIVER_PARAMETER=none; DRIVER_PARAM_TYPE=none;
                elif test "$1" = "k"; then LIRC_DRIVER=gvbctv5pci;	DRIVER_PARAMETER=none; DRIVER_PARAM_TYPE=none;
                elif test "$1" = "l"; then LIRC_DRIVER=kworld;		DRIVER_PARAMETER=none; DRIVER_PARAM_TYPE=none;
                elif test "$1" = "m"; then LIRC_DRIVER=knc_one;		DRIVER_PARAMETER=none; DRIVER_PARAM_TYPE=none;
                elif test "$1" = "n"; then LIRC_DRIVER=pixelview_pak;	DRIVER_PARAMETER=none; DRIVER_PARAM_TYPE=none;
                elif test "$1" = "o"; then LIRC_DRIVER=pctv;		DRIVER_PARAMETER=tty1; DRIVER_PARAM_TYPE=tty;
                elif test "$1" = "p"; then LIRC_DRIVER=cph06x;		DRIVER_PARAMETER=none; DRIVER_PARAM_TYPE=none;
                elif test "$1" = "q"; then LIRC_DRIVER=pctv;		DRIVER_PARAMETER=tty1; DRIVER_PARAM_TYPE=tty;
                ...
                elif test "$1" = "u"; then LIRC_DRIVER=pixelview_bt878;	DRIVER_PARAMETER=none; DRIVER_PARAM_TYPE=none;
                elif test "$1" = "v"; then LIRC_DRIVER=pixelview_pro;	DRIVER_PARAMETER=none; DRIVER_PARAM_TYPE=none;
                elif test "$1" = "w"; then LIRC_DRIVER=pixelview_pak;	DRIVER_PARAMETER=none; DRIVER_PARAM_TYPE=none;
                elif test "$1" = "x"; then LIRC_DRIVER=pixelview_pro;	DRIVER_PARAMETER=none; DRIVER_PARAM_TYPE=none;
                elif test "$1" = "y"; then LIRC_DRIVER=provideo;	DRIVER_PARAMETER=none; DRIVER_PARAM_TYPE=none;
                elif test "$1" = "z"; then LIRC_DRIVER=mediafocusI;	DRIVER_PARAMETER=none; DRIVER_PARAM_TYPE=none;
                elif test "$1" = "A"; then LIRC_DRIVER=tekram_bt829;	DRIVER_PARAMETER=none; DRIVER_PARAM_TYPE=none;
                elif test "$1" = "B"; then LIRC_DRIVER=cph03x;		DRIVER_PARAMETER=none; DRIVER_PARAM_TYPE=none;
                elif test "$1" = "C"; then LIRC_DRIVER=cph06x;		DRIVER_PARAMETER=none; DRIVER_PARAM_TYPE=none;
                elif test "$1" = "D"; then LIRC_DRIVER=knc_one;		DRIVER_PARAMETER=none; DRIVER_PARAM_TYPE=none;
                elif test "$1" = "E"; then LIRC_DRIVER=leadtek_pvr2000;	DRIVER_PARAMETER=none; DRIVER_PARAM_TYPE=none;
                elif test "$1" = "F"; then LIRC_DRIVER=leadtek_0010;	DRIVER_PARAMETER=none; DRIVER_PARAM_TYPE=none;
                elif test "$1" = "G"; then LIRC_DRIVER=leadtek_0007;	DRIVER_PARAMETER=none; DRIVER_PARAM_TYPE=none;
                fi
                }
                else
                    return;
			...
			3 "SIR IrDA (built-in IR ports)" \
			4 "Tekram Irmate 210 (16x50 UART compatible serial port)" \
			5 "Winbond 8769L CIR port (e.g. Acer Aspire 6530G)" 2> $TEMP
            if test "$?" = "0"; then
                {
                set `cat $TEMP`
                if false; then :
                ...
                elif test "$1" = "3"; then LIRC_DRIVER=sir;		DRIVER_PARAMETER=com3; DRIVER_PARAM_TYPE=com;
                elif test "$1" = "4"; then LIRC_DRIVER=tekram;		DRIVER_PARAMETER=com1; DRIVER_PARAM_TYPE=com;
                elif test "$1" = "5"; then LIRC_DRIVER=wpc8769l;	DRIVER_PARAMETER=none; DRIVER_PARAM_TYPE=none;
                fi
                }
                else
                    return;
			...
			6 "Aureal ATWF@83-W001 ESKY.CC remote" \
			7 "Awox RF/IR Remote (userspace)" \
			8 "Creative USB IR Receiver (SB0540)" \
			9 "CommandIR Multi-IR Transceiver (userspace)" \
			0 "DFC USB InfraRed Remote Control (userspace)" \
			a "Dign HV5 HTPC IR/VFD Module" \
			b "DViCO USB Remote" \
			c "FTDI FT232-based IR Receiver" \
			d "Home Electronics Tira USB device" \
			e "Home Electronics Tira USB device (timing mode)" \
			f "Igor Cesko's USB IR Receiver" \
			g "Iguanaworks USB IR Transceiver" \
			h "IRLink VS USB IR Receiver" \
			i "PCMAK USB receiver" \
			j "Philips SRM 7500 RF remote" \
			k "Remotec Multimedia PC Remote BW6130" \
			l "Samsung USB IR Receiver" \
			m "Sasem OnAir Remocon-V" \
			n "Snapstream Firefly" \
			o "Sound Blaster Extigy USB sound card (exaudio)" \
			p "Sound Blaster Extigy/Audigy 2 NX (ALSA snd-usb-audio)" \
			q "Soundgraph iMON 2.4G DT & LT" \
			r "Soundgraph iMON MultiMedian IR/VFD" \
			s "Soundgraph iMON IR/LCD" \
			t "Soundgraph iMON Knob" \
			u "Soundgraph iMON PAD IR/VFD" \
			v "Soundgraph iMON RSC" \
            ...
            if test "$?" = "0"; then
                ...
                elif test "$1" = "6"; then LIRC_DRIVER=atwf83;		DRIVER_PARAMETER=none; DRIVER_PARAM_TYPE=none;
                elif test "$1" = "7"; then LIRC_DRIVER=awlibusb;	DRIVER_PARAMETER=none; DRIVER_PARAM_TYPE=none;
                elif test "$1" = "8"; then LIRC_DRIVER=sb0540;		DRIVER_PARAMETER=none; DRIVER_PARAM_TYPE=none;
                elif test "$1" = "9"; then LIRC_DRIVER=commandir;	DRIVER_PARAMETER=none; DRIVER_PARAM_TYPE=none;
                elif test "$1" = "0"; then LIRC_DRIVER=dfclibusb;	DRIVER_PARAMETER=none; DRIVER_PARAM_TYPE=none;
                elif test "$1" = "a"; then LIRC_DRIVER=sasem;		DRIVER_PARAMETER=none; DRIVER_PARAM_TYPE=none;
                elif test "$1" = "b"; then LIRC_DRIVER=dvico;		DRIVER_PARAMETER=none; DRIVER_PARAM_TYPE=none;
                elif test "$1" = "c"; then LIRC_DRIVER=ftdi;		DRIVER_PARAMETER=none; DRIVER_PARAM_TYPE=none;
                elif test "$1" = "d"; then LIRC_DRIVER=tira;		DRIVER_PARAMETER=ttyUSB1; DRIVER_PARAM_TYPE=ttyUSB;
                elif test "$1" = "e"; then LIRC_DRIVER=tira_raw;	DRIVER_PARAMETER=ttyUSB1; DRIVER_PARAM_TYPE=ttyUSB;
                elif test "$1" = "f"; then LIRC_DRIVER=igorplugusb;	DRIVER_PARAMETER=none; DRIVER_PARAM_TYPE=none;
                elif test "$1" = "g"; then LIRC_DRIVER=iguanaIR;	DRIVER_PARAMETER=none; DRIVER_PARAM_TYPE=none;
                elif test "$1" = "h"; then LIRC_DRIVER=irlink;		DRIVER_PARAMETER=ttyUSB1; DRIVER_PARAM_TYPE=ttyUSB;
                elif test "$1" = "i"; then LIRC_DRIVER=pcmak_usb;	DRIVER_PARAMETER=ttyUSB1; DRIVER_PARAM_TYPE=ttyUSB;
                elif test "$1" = "j"; then LIRC_DRIVER=srm7500libusb;	DRIVER_PARAMETER=none; DRIVER_PARAM_TYPE=none;
                elif test "$1" = "k"; then LIRC_DRIVER=bw6130;		DRIVER_PARAMETER=none; DRIVER_PARAM_TYPE=none;
                elif test "$1" = "l"; then LIRC_DRIVER=samsung;		DRIVER_PARAMETER=none; DRIVER_PARAM_TYPE=none;
                elif test "$1" = "m"; then LIRC_DRIVER=sasem;		DRIVER_PARAMETER=none; DRIVER_PARAM_TYPE=none;
                elif test "$1" = "n"; then LIRC_DRIVER=atiusb;		DRIVER_PARAMETER=none; DRIVER_PARAM_TYPE=none;
                elif test "$1" = "o"; then LIRC_DRIVER=exaudio;		DRIVER_PARAMETER=none; DRIVER_PARAM_TYPE=none;
                elif test "$1" = "p"; then LIRC_DRIVER=alsa_usb;	DRIVER_PARAMETER=none; DRIVER_PARAM_TYPE=none;
                elif test "$1" = "q"; then LIRC_DRIVER=imon_24g;	DRIVER_PARAMETER=none; DRIVER_PARAM_TYPE=none;
                elif test "$1" = "r"; then LIRC_DRIVER=imon;		DRIVER_PARAMETER=none; DRIVER_PARAM_TYPE=none;
                elif test "$1" = "s"; then LIRC_DRIVER=imon_lcd;	DRIVER_PARAMETER=none; DRIVER_PARAM_TYPE=none;
                elif test "$1" = "t"; then LIRC_DRIVER=imon_knob;	DRIVER_PARAMETER=none; DRIVER_PARAM_TYPE=none;
                elif test "$1" = "u"; then LIRC_DRIVER=imon_pad;	DRIVER_PARAMETER=none; DRIVER_PARAM_TYPE=none;
                elif test "$1" = "v"; then LIRC_DRIVER=imon_rsc;	DRIVER_PARAMETER=none; DRIVER_PARAM_TYPE=none;
                elif test "$1" = "w"; then LIRC_DRIVER=ttusbir;		DRIVER_PARAMETER=none; DRIVER_PARAM_TYPE=none;
			...
			6 "Linux input layer (/dev/input/eventX)" \
			7 "User-space I2C driver" 2> $TEMP
            if test "$?" = "0"; then
                {
                set `cat $TEMP`
                if false; then :
                ...
                elif test "$1" = "7"; then LIRC_DRIVER=i2cuser;		DRIVER_PARAMETER=none; DRIVER_PARAM_TYPE=none;
                fi
                }
                else
                    return;
