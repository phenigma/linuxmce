...
class cSwReceiver : public cReceiver {
private:
  cChannel * channel;
protected:
...
cSwReceiver::cSwReceiver(cChannel * Channel) : 
#if VDRVERSNUM < 10500
     cReceiver(0                      , 100, Channel->Vpid()) {
#else
     cReceiver(Channel->GetChannelID(), 100, Channel->Vpid()) {
#endif
  channel = Channel;
  }
...
void cScanner::Action(void) {
  ...
  int           thisChannel      = 0, lastChannel = 0;

  const char *  country   = country_to_short_name(Wirbelscan.CountryIndex);
  const char *  satellite = satellite_to_short_name(Wirbelscan.SatIndex);
  cString       channelname, shortname;
  cSwReceiver * SwReceiver;
  ...
  switch (type) {
     case DVB_TERR:
       ...
       if (invAuto) {
         ...
         if (Wirbelscan.DVBT_Inversion == 0) {
     ...
     case DVB_CABLE:
       ...
       if (invAuto) {
         ...
         if (Wirbelscan.DVBC_Inversion == 0) {
            dlog(1, "INVERSION_AUTO not supported, trying INVERSION_OFF.");
            caps_inversion = INVERSION_OFF;
            }
         else {
       ...
       caps_fec = FEC_NONE;
       if (crAuto) {
         dlog(1, "CODERATE_AUTO");
         dvbc_symbolrate_min = dvbc_symbolrate_max = 0;
         }
       else {
         dlog(1, "CODERATE_AUTO not supported.");
         switch(Wirbelscan.DVBC_Symbolrate) {
            case 0: // auto
               dvbc_symbolrate_min = 0;
               dvbc_symbolrate_max = 1;
               break;
            case 1 ... 15:
               dvbc_symbolrate_min = dvbc_symbolrate_max = Wirbelscan.DVBC_Symbolrate - 1;
               break;
            default:// all
               dvbc_symbolrate_min = 0;
...
*/
        SetSatTransponderDataFromDVB(aChannel,
                                    ...
                                    27500, FEC_5_6, 9, 6, 0);
       #if VDRVERSNUM < 10500
       if ((dev = cDevice::GetDevice(aChannel, 100))    == NULL) {
       #else
...
//         SetSatTransponderDataFromVDR(aChannel, cSource::FromString(sat_list[this_channellist].source_id), 11112, eHorizontal, 27500, eCoderate56, eSatModulationQpsk, eDvbs, eRolloff35);
     ...
     case DVB_ATSC:
       {
       int atsc = 1 + Wirbelscan.ATSC_type;
     ...
     case PVRINPUT_FM:
       ...
       #if VDRVERSNUM>10713
       aChannel->Parse("N24;wirbelscan:250000:TV:V:0:301:300:305:0:1:0:8432:62");       
       #elif defined (PLUGINPARAMPATCHVERSNUM)     
       aChannel->Parse("N24;wirbelscan:250000:PVRINPUT|TV:P:0:301:300:305:0:8436:0:0:62");
       #else
       dlog(0, "WARNING: YOUR VDR CORE MISSES THE IPTV PATCH - FUTURE VERSIONS OF PVRINPUT\n"
               "         WILL NO LONGER WORK WITHOUT THIS PATCH");
       aChannel->Parse("N24;wirbelscan:250000:C0:C:0:301:300:305:A1:8436:0:0:62");
       #endif
    ...
    for (channel = channel_min; channel <= channel_max; channel++) {
      for (offs = freq_offset_min; offs <= freq_offset_max; offs++)
        for (sr_parm = dvbc_symbolrate_min; sr_parm <= dvbc_symbolrate_max; sr_parm++) {
          ...
          switch (type) {
             ...
             case DVB_SAT:
               ...
               if (sat_list[this_channellist].items[channel].modulation_system == SYS_DVBS2) {
                 if (!(caps_s2) || (DVB_API_VERSION < 5) || (Wirbelscan.enable_s2 == 0)) {
                   dlog(1, "%d: skipped ()",
                        sat_list[this_channellist].items[channel].intermediate_frequency,
                        (Wirbelscan.enable_s2 == 0)?"disabled":"no driver support");
                   thisChannel++;
                   continue;
                   }
                 }
          ...
          ++thisChannel;
          lProgress = (int) (thisChannel * 100) / lastChannel;
          lTransponder = *PrintTransponder(aChannel);
          if (MenuScanning) {
            MenuScanning->SetProgress(lProgress, type, (lastChannel - thisChannel));
          ...
          switch (type) {
             ...
             case DVB_TERR:
               ...
               StateMachine = new cStateMachine(dev, aChannel);
             ...
             case PVRINPUT:
               ...
               cCondWait::SleepMs(1000);
               lStrength = PvrHasLock(2500, videodev);
dlog(0, "lStrength = %d\n", lStrength);
