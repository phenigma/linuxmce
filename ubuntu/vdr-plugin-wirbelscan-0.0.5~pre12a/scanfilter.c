...
void resetLists() {
  ;
  NewChannels.Load(false, false);
  NewTransponders.Load(false, false);
  ScannedTransponders.Load(false, false);
  }
...
bool is_known_initial_transponder(cChannel * newChannel, bool auto_allowed, cChannels * list) {
  ...
  for (cChannel * channel = list->First(); channel; channel = list->Next(channel)) {
    ...
    if (newChannel->IsPlug()) {
      ...
      if (!is_nearly_same_frequency(channel, newChannel, 50)) {
        continue;
        }
      if ((sscanf(channel->PluginParam(), "%a[^|]|%a[^|]|%a[^|]|%a[^:\n]", &PluginId, &Input1, &optArg1, &optArg2) < 2) ||
          (strcasecmp(PluginId, "PVRINPUT"))) {
        continue;
        }
      sscanf(newChannel->PluginParam(), "%a[^|]|%a[^|]|%a[^|]|%a[^:\n]", &PluginId, &Input2, &optArg1, &optArg2);
      if (!strcasecmp(Input1, Input2)) {
        return (true);
        }
      }
#endif
...
bool is_nearly_same_frequency(const cChannel * chan_a, const cChannel * chan_b, uint delta) {
  ...
  if (diff < delta) {
    dlog(3, "f1 = %u is same TP as f2 = %u", f1, f2);
    return (true);
    }
  return (false);
  }
           ...
           for (int n = 0; n < NumFrequencies; n++) {
             ...
             if (SetSatTransponderDataFromDVB(transponder, Source, Frequencies[n], Polarization, SymbolRate, CodeRate, ModulationType, System, RollOff)) {
               if (!is_known_initial_transponder(transponder, true)) {
                 if (System && !Wirbelscan.enable_s2) {
...
int AddChannels() {
  ...
  for (cChannel * Channel = NewChannels.First(); Channel; Channel = NewChannels.Next(Channel)) {
    ...
    if (Channel->Ca() && ! (Wirbelscan.scanflags & SCAN_SCRAMBLED)) {
      dlog(3,"      skipped (no scrambled channels) %s", *PrintChannel(Channel));
      continue;
      }
    if (! Channel->Ca() && ! (Wirbelscan.scanflags & SCAN_FTA)) {
      dlog(3,"      skipped (no FTA channels) %s", *PrintChannel(Channel));
      continue;
      }
    if (! Channel->Vpid() && (Channel->Apid(0) || Channel->Dpid(0)) && ! (Wirbelscan.scanflags & SCAN_RADIO)) {
      dlog(3,"      skipped (no radio channels) %s", *PrintChannel(Channel));
      continue;
      }
    if (Channel->Vpid() && ! (Wirbelscan.scanflags & SCAN_TV)) {
      dlog(3,"      skipped (no tv channels) %s", *PrintChannel(Channel));
      continue;
      }
    #if VDRVERSNUM > 10701
    if ((Channel->Vtype() > 2) && ! (Wirbelscan.scanflags & SCAN_HD)) {
...
public:
  cEitParser(cSchedules *Schedules, int Source, u_char Tid, const u_char *Data, bool OnlyRunningStatus = false);
  };

cEitParser::cEitParser(cSchedules *Schedules, int Source, u_char Tid, const u_char *Data, bool OnlyRunningStatus)
:SI::EIT(Data, false)
{
  if (!CheckCRCAndParse())
     return;

//  dlog(2, "%s onid=%d, tsid=%d, sid=%d", __FUNCTION__, getOriginalNetworkId(), getTransportStreamId(), getServiceId());

  tChannelID channelID(Source, getOriginalNetworkId(), getTransportStreamId(), getServiceId());
  cChannel *channel = Channels.GetByChannelID(channelID, true);
  if (!channel) {
     //dlog(2, "no channel. returning..");
     return; // only collect data for known channels
     }
//  dlog(2, "channel %s", *PrintChannel(channel));
  bool Empty = true;
  bool Modified = false;
...
void cEitScanner::Process(u_short Pid, u_char Tid, const u_char *Data, int Length) {
  ...
  if (Schedules)
     cEitParser EitParser(Schedules, Source(), Tid, Data);
  else {
     ...
     cSchedules *Schedules = (cSchedules *)cSchedules::Schedules(SchedulesLock);
     if (Schedules)
        cEitParser EitParser(Schedules, Source(), Tid, Data, true);
     }
}
