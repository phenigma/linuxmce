...
fe_delivery_system_t SatSystems(eSatSystems ss) {
  ...
  switch(ss) {
    ...
    default      : dlog(0, "%s, unknown system %u",
                     __FUNCTION__, ss);
                   return SYS_DVBS;
    }
#else
...
bool SetSatTransponderDataFromDVB(cChannel * channel, int Source, int Frequency, char Polarization, int Srate, int CoderateH, int Modulation, int System, int RollOff) {
  ...
  if ((System == 6)        || // SYS_DVBS2
      ...
      (Modulation > 8)) {     // PSK_8 .. DQPSK
        dlog(1, "%s: unsupported by VDR version", __FUNCTION__);
        return false;
        }
...
bool SetTerrTransponderDataFromDVB(cChannel * channel, int Source, int Frequency, int Bandwidth, int Modulation, int Hierarchy, int CodeRateH, int CodeRateL, int Guard, int Transmission, int Inversion) {
 ...
 switch (Bandwidth) {
    ...
    default:              Bandwidth=8000000; break; /* oops.., what to do with auto?? */
    }
 #endif
...
void SetPids(cChannel * channel, int Vpid, int Ppid, int Vtype, int *Apids, int *Atypes, char ALangs[][MAXLANGCODE2], int *Dpids, int *Dtypes, char DLangs[][MAXLANGCODE2], int *Spids, char SLangs[][MAXLANGCODE2], int Tpid) {
 channel->SetPids(Vpid, Ppid,
                  ...
                  DLangs,
                  #if VDRVERSNUM >= 10510
                  Spids, SLangs, /* vdr 1.5.10 added subtitling */
                  #endif
                  Tpid);
}
...
bool GetTerrCapabilities (int cardIndex, bool *CodeRate, bool *Modulation, bool *Inversion, bool *Bandwidth, bool *Hierarchy,
  ...
  if (cap == 0)
     return false;
  *CodeRate         = cap & TerrCodeRates(eTerrCoderateAuto);
  *Modulation       = cap & TerrConstellations(eModulationAuto);
  *Inversion        = cap & CableTerrInversions(eInversionAuto); 
  *Bandwidth        = cap & BANDWIDTH_AUTO; // was: TerrBandwidths(eBandwidthAuto);
  *Hierarchy        = cap & TerrHierarchies(eHierarchyAuto);
  *TransmissionMode = cap & TerrGuardIntervals(eGuardintervalAuto);
  *GuardInterval    = cap & TerrTransmissionModes(eTransmissionmodeAuto);
  return true; 
}
...
bool GetCableCapabilities(int cardIndex, bool *CodeRate, bool *Modulation, bool *Inversion) {
  ...
  if (cap < 0)
     return false;
  *CodeRate         = cap & CableSatCodeRates(eCoderateAuto);
  *Modulation       = cap & CableModulations(eModulationQamAuto);
  *Inversion        = cap & CableTerrInversions(eInversionAuto);
  return true; 
}
...
bool GetAtscCapabilities (int cardIndex, bool *CodeRate, bool *Modulation, bool *Inversion, bool *VSB, bool * QAM) {
  ...
  *Modulation       = cap & CableModulations(eModulationQamAuto);
  *Inversion        = cap & CableTerrInversions(eInversionAuto);
  *VSB              = cap & AtscModulations(eVsb8);
  *QAM              = cap & AtscModulations(eQam256);
  return true; 
}
...
bool GetSatCapabilities  (int cardIndex, bool *CodeRate, bool *Modulation, bool *RollOff, bool *DvbS2) {
  ...
  if (cap < 0)
     return false;
  *CodeRate         = cap & CableSatCodeRates(eCoderateAuto);
  *Modulation       = false; // remove or fixable? What means auto here -> QAM_AUTO or what?
  *RollOff          = cap & SatRollOffs(eRolloffAuto);            // check this!
  *DvbS2            = cap & SatSystems(eDvbs2);                   // check this!
  return true; 
}
