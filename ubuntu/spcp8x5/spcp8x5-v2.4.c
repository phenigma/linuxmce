			   ...
			   enum spcp8x5_type type)
{
	...
	if (ret < 0)
		dbg("Get MSR = 0x%p failed (error = %d)",
			status_buffer, ret);

	dbg("0xc0:0x22:0:6  %d - 0x%p ", ret, status_buffer);
				 ...
				 u16 index, enum spcp8x5_type type)
{
	...
	dbg("value = %#x , index = %#x\n", value, index);
	if (ret < 0)
		dbg("RTSCTS usb_control_msg(enable flowctrl) = %d\n", ret);
}
...
static void spcp8x5_close(struct usb_serial_port *port, struct file *filp)
{
	...
	if (port->write_urb != NULL) {
		...
		if (result)
			dbg( "usb_unlink_urb(write_urb) = %d\n", result);
	}
	result = usb_unlink_urb(port->read_urb);
	if (result)
		dbg("usb_unlink_urb(read_urb) = %d\n", result);
}
...
static void spcp8x5_read_bulk_callback(struct urb *urb)
{
	...
	if (urb->status) {
		...
		if (urb->status == -EPROTO) {
			...
			if (result)
				dbg( "failed submitting read urb %d\n",
					result);
			return;
		}
...
static void spcp8x5_send(struct usb_serial_port *port)
{
	...
	if (result) {
		dbg("failed submitting write urb, error %d\n",
			result);
...
static void spcp8x5_write_bulk_callback(struct urb *urb)
{
	...
	default:
		...
		result = usb_submit_urb(port->write_urb);
		if (result)
			dbg( "failed resubmitting write urb %d\n", result);
		else
			return;
	}
...
static int spcp8x5_write(struct usb_serial_port *port, int from_user,
