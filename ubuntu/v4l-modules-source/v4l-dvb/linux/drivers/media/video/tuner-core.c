...
static int tuner_legacy_probe(struct i2c_adapter *adap)
{
	...
	/* The FMD1216ME(X) Hybrid Tuners start-up with the TDA9887
	 * demodulator disabled. Here we enable it prior to probing.
	 */
	if (adap->id == I2C_HW_B_CX2388x)
	{
		struct cx88_core *core = adap->dev.driver_data;
		
		if (core->board.tuner_type == TUNER_PHILIPS_FMD1216ME_MK3 ||
			core->board.tuner_type == TUNER_PHILIPS_FMD1216MEX_MK3) {
			u8 pll_data[] = {
			...
			if (rc != 1)
				printk("%s: FMD1216ME(X) init failed\n", __func__);
		}
	}
