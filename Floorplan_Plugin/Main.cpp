...
#define  Floorplan_Plugin_VERSION "testing"
...
extern "C" {
	class Command_Impl *RegisterAsPlugIn(class Router *pRouter,int PK_Device,string sLogger)
	{
		...
		Floorplan_Plugin *pFloorplan_Plugin = new Floorplan_Plugin(PK_Device, "localhost",true,false,pRouter);
		if( pFloorplan_Plugin->m_bQuit )
		{
			delete pFloorplan_Plugin;
			return NULL;
		}
		return pFloorplan_Plugin;
	}
}
//<-dceag-plug-e->
...
int main(int argc, char* argv[]) 
{
	...
	try
	{
		Floorplan_Plugin *pFloorplan_Plugin = new Floorplan_Plugin(PK_Device, sRouter_IP);	
		if ( pFloorplan_Plugin->Connect() ) 
		{
			g_pPlutoLogger->Write(LV_STATUS, "Connect OK");
			pFloorplan_Plugin->CreateChildren();
			pthread_join(pFloorplan_Plugin->m_RequestHandlerThread, NULL);
