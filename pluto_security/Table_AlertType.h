...
public:
	...
	Table_AlertType(Database_pluto_security *pDatabase):database(pDatabase), m_Mutex("AlertType")
	{
...
void EntryDelay_set(short int val);
void ExitDelay_set(short int val);
void Description_set(string val);
void Define_set(string val);

		
		bool EntryDelay_isNull();
bool ExitDelay_isNull();
bool Define_isNull();

			
		void EntryDelay_setNull(bool val);
...
void Define_setNull(bool val);
		...
		void SetupSerialization(int iSC_Version) {
			StartSerializeList() + m_PK_AlertType+ m_EntryDelay+ m_ExitDelay+ m_Description+ m_Define;
		}
	private:
		void SetDefaultValues();
		
		string PK_AlertType_asSQL();
string EntryDelay_asSQL();
