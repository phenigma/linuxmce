...
#include "Table_psc_security_tables.h"



void Database_pluto_security::CreateTable_psc_security_tables()
{
	tblpsc_security_tables = new Table_psc_security_tables(this);
}
...
Table_psc_security_tables::~Table_psc_security_tables()
{
	...
	{
		Row_psc_security_tables *pRow = (Row_psc_security_tables *) (*it).second;
		delete pRow;
	}
...
void Row_psc_security_tables::Delete()
{
	PLUTO_SAFETY_LOCK(M, table->m_Mutex);
	Row_psc_security_tables *pRow = this; // Needed so we will have only 1 version of get_primary_fields_assign_from_row
	
	if (!is_deleted)
		if (is_added)	
		{	
			vector<TableRow*>::iterator i;	
			for (i = table->addedRows.begin(); (i!=table->addedRows.end()) && ( (Row_psc_security_tables *) *i != this); i++);
		...
		{
			SingleLongKey key(pRow->m_PK_psc_security_tables);
...
void Row_psc_security_tables::Reload()
{
	Row_psc_security_tables *pRow = this; // Needed so we will have only 1 version of get_primary_fields_assign_from_row
	...
	{
		SingleLongKey key(pRow->m_PK_psc_security_tables);
		Row_psc_security_tables *pRow = table->FetchRow(key);
...
Row_psc_security_tables::Row_psc_security_tables(Table_psc_security_tables *pTable):table(pTable)
{
	SetDefaultValues();
}

void Row_psc_security_tables::SetDefaultValues()
{
	m_PK_psc_security_tables = 0;
is_null[0] = false;
m_Tablename = "";
is_null[1] = false;
is_null[2] = true;
...
long int Row_psc_security_tables::PK_psc_security_tables_get(){PLUTO_SAFETY_LOCK(M, table->m_Mutex);

return m_PK_psc_security_tables;}
string Row_psc_security_tables::Tablename_get(){PLUTO_SAFETY_LOCK(M, table->m_Mutex);

return m_Tablename;}
string Row_psc_security_tables::filter_get(){PLUTO_SAFETY_LOCK(M, table->m_Mutex);
...
long int Row_psc_security_tables::last_psc_batch_get(){PLUTO_SAFETY_LOCK(M, table->m_Mutex);

return m_last_psc_batch;}

		
void Row_psc_security_tables::PK_psc_security_tables_set(long int val){PLUTO_SAFETY_LOCK(M, table->m_Mutex);

m_PK_psc_security_tables = val; is_modified=true; is_null[0]=false;}
void Row_psc_security_tables::Tablename_set(string val){PLUTO_SAFETY_LOCK(M, table->m_Mutex);

m_Tablename = val; is_modified=true; is_null[1]=false;}
void Row_psc_security_tables::filter_set(string val){PLUTO_SAFETY_LOCK(M, table->m_Mutex);
...
char buf[32];
sprintf(buf, "%li", m_PK_psc_security_tables);

return buf;
}

string Row_psc_security_tables::Tablename_asSQL()
{
PLUTO_SAFETY_LOCK(M, table->m_Mutex);
...
string Row_psc_security_tables::filter_asSQL()
{
PLUTO_SAFETY_LOCK(M, table->m_Mutex);
...
string Row_psc_security_tables::frozen_asSQL()
{
PLUTO_SAFETY_LOCK(M, table->m_Mutex);
...
string Row_psc_security_tables::last_psc_batch_asSQL()
{
PLUTO_SAFETY_LOCK(M, table->m_Mutex);
...
Table_psc_security_tables::Key::Key(long int in_PK_psc_security_tables)
{
			pk_PK_psc_security_tables = in_PK_psc_security_tables;
	
}

Table_psc_security_tables::Key::Key(Row_psc_security_tables *pRow)
{
			PLUTO_SAFETY_LOCK(M, pRow->table->m_Mutex);

			pk_PK_psc_security_tables = pRow->m_PK_psc_security_tables;
	
}		

bool Table_psc_security_tables::Key_Less::operator()(const Table_psc_security_tables::Key &key1, const Table_psc_security_tables::Key &key2) const
{
			if (key1.pk_PK_psc_security_tables!=key2.pk_PK_psc_security_tables)
return key1.pk_PK_psc_security_tables<key2.pk_PK_psc_security_tables;
else
return false;	
}	

bool Table_psc_security_tables::Commit()
{
...
//insert added
	while (!addedRows.empty())
	{
		...
		Row_psc_security_tables *pRow = (Row_psc_security_tables *)*i;
...
//update modified
	...
	{
		Row_psc_security_tables* pRow = (Row_psc_security_tables*) (*i).second;	
		SingleLongKey key(pRow->m_PK_psc_security_tables);

		char tmp_PK_psc_security_tables[32];
sprintf(tmp_PK_psc_security_tables, "%li", key.pk);


string condition;
condition = condition + "PK_psc_security_tables=" + tmp_PK_psc_security_tables;
...
condition = condition + "PK_psc_security_tables=" + tmp_PK_psc_security_tables;
		...
		pRow = (Row_psc_security_tables*) (*i).second;;
		delete pRow;
...
bool Table_psc_security_tables::GetRows(string where_statement,vector<class Row_psc_security_tables*> *rows)
{
	PLUTO_SAFETY_LOCK(M, m_Mutex);
	...
	{	
		...
		Row_psc_security_tables *pRow = new Row_psc_security_tables(this);
		
		if (row[0] == NULL)
{
pRow->is_null[0]=true;
pRow->m_PK_psc_security_tables = 0;
}
else
{
pRow->is_null[0]=false;
sscanf(row[0], "%li", &(pRow->m_PK_psc_security_tables));
}
...
Row_psc_security_tables* Table_psc_security_tables::AddRow()
{
	PLUTO_SAFETY_LOCK(M, m_Mutex);
...
Row_psc_security_tables* Table_psc_security_tables::GetRow(long int in_PK_psc_security_tables)
{
	PLUTO_SAFETY_LOCK(M, m_Mutex);

	SingleLongKey row_key(in_PK_psc_security_tables);
	...
	//row is cached
	if (i!=cachedRows.end())
		return (Row_psc_security_tables*) (*i).second;
	//we have to fetch row
	Row_psc_security_tables* pRow = FetchRow(row_key);
...
Row_psc_security_tables* Table_psc_security_tables::FetchRow(SingleLongKey &key)
{
	PLUTO_SAFETY_LOCK(M, m_Mutex);
