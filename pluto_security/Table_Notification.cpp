// If using the thread logger, these generated classes create lots of activity
#ifdef NO_SQL_THREAD_LOG
#undef THREAD_LOG
#endif

#ifdef WIN32
#include <winsock.h>
#endif

#include <iostream>
#include <string>
#include <vector>
#include <map>

#include <mysql.h>

using namespace std;
#include "PlutoUtils/StringUtils.h"
#include "Table_Notification.h"
#include "Table_Alert.h"



void Database_pluto_security::CreateTable_Notification()
{
	tblNotification = new Table_Notification(this);
}

void Database_pluto_security::DeleteTable_Notification()
{
	delete tblNotification;
}

Table_Notification::~Table_Notification()
{
	map<SingleLongKey, class TableRow*, SingleLongKey_Less>::iterator it;
	for(it=cachedRows.begin();it!=cachedRows.end();++it)
	{
		Row_Notification *pRow = (Row_Notification *) (*it).second;
		delete pRow;
	}

	for(it=deleted_cachedRows.begin();it!=deleted_cachedRows.end();++it)
	{
		Row_Notification *pRow = (Row_Notification *) (*it).second;
		delete pRow;
	}

	size_t i;
	for(i=0;i<addedRows.size();++i)
		delete addedRows[i];
	for(i=0;i<deleted_addedRows.size();++i)
		delete deleted_addedRows[i];
}


void Row_Notification::Delete()
{
	PLUTO_SAFETY_LOCK(M, table->m_Mutex);
	Row_Notification *pRow = this; // Needed so we will have only 1 version of get_primary_fields_assign_from_row
	
	if (!is_deleted)
		if (is_added)	
		{	
			vector<TableRow*>::iterator i;	
			for (i = table->addedRows.begin(); (i!=table->addedRows.end()) && ( (Row_Notification *) *i != this); i++);
			
			if (i!=	table->addedRows.end())
				table->addedRows.erase(i);
		
			table->deleted_addedRows.push_back(this);
			is_deleted = true;	
		}
		else
		{
			SingleLongKey key(pRow->m_PK_Notification);
			map<SingleLongKey, TableRow*, SingleLongKey_Less>::iterator i = table->cachedRows.find(key);
			if (i!=table->cachedRows.end())
				table->cachedRows.erase(i);
						
			table->deleted_cachedRows[key] = this;
			is_deleted = true;	
		}	
}

void Row_Notification::Reload()
{
	Row_Notification *pRow = this; // Needed so we will have only 1 version of get_primary_fields_assign_from_row

	PLUTO_SAFETY_LOCK(M, table->m_Mutex);
	
	
	if (!is_added)
	{
		SingleLongKey key(pRow->m_PK_Notification);
		Row_Notification *pRow = table->FetchRow(key);
		
		if (pRow!=NULL)
		{
			*this = *pRow;	
			
			delete pRow;		
		}	
	}	
	
}

Row_Notification::Row_Notification(Table_Notification *pTable):table(pTable)
{
	SetDefaultValues();
}

void Row_Notification::SetDefaultValues()
{
	m_PK_Notification = 0;
is_null[0] = false;
m_FK_Alert = 0;
is_null[1] = false;
m_NotificationTime = "0000-00-00 00:00:00";
is_null[2] = false;
is_null[3] = true;
is_null[4] = true;


	is_added=false;
	is_deleted=false;
	is_modified=false;
}

long int Row_Notification::PK_Notification_get(){PLUTO_SAFETY_LOCK(M, table->m_Mutex);

return m_PK_Notification;}
long int Row_Notification::FK_Alert_get(){PLUTO_SAFETY_LOCK(M, table->m_Mutex);

return m_FK_Alert;}
string Row_Notification::NotificationTime_get(){PLUTO_SAFETY_LOCK(M, table->m_Mutex);

return m_NotificationTime;}
string Row_Notification::Info_get(){PLUTO_SAFETY_LOCK(M, table->m_Mutex);

return m_Info;}
string Row_Notification::Result_get(){PLUTO_SAFETY_LOCK(M, table->m_Mutex);

return m_Result;}

		
void Row_Notification::PK_Notification_set(long int val){PLUTO_SAFETY_LOCK(M, table->m_Mutex);

m_PK_Notification = val; is_modified=true; is_null[0]=false;}
void Row_Notification::FK_Alert_set(long int val){PLUTO_SAFETY_LOCK(M, table->m_Mutex);

m_FK_Alert = val; is_modified=true; is_null[1]=false;}
void Row_Notification::NotificationTime_set(string val){PLUTO_SAFETY_LOCK(M, table->m_Mutex);

m_NotificationTime = val; is_modified=true; is_null[2]=false;}
void Row_Notification::Info_set(string val){PLUTO_SAFETY_LOCK(M, table->m_Mutex);

m_Info = val; is_modified=true; is_null[3]=false;}
void Row_Notification::Result_set(string val){PLUTO_SAFETY_LOCK(M, table->m_Mutex);

m_Result = val; is_modified=true; is_null[4]=false;}

		
bool Row_Notification::Info_isNull() {PLUTO_SAFETY_LOCK(M, table->m_Mutex);

return is_null[3];}
bool Row_Notification::Result_isNull() {PLUTO_SAFETY_LOCK(M, table->m_Mutex);

return is_null[4];}

			
void Row_Notification::Info_setNull(bool val){PLUTO_SAFETY_LOCK(M, table->m_Mutex);

is_null[3]=val;}
void Row_Notification::Result_setNull(bool val){PLUTO_SAFETY_LOCK(M, table->m_Mutex);

is_null[4]=val;}
	

string Row_Notification::PK_Notification_asSQL()
{
PLUTO_SAFETY_LOCK(M, table->m_Mutex);

if (is_null[0])
return "NULL";

char buf[32];
sprintf(buf, "%li", m_PK_Notification);

return buf;
}

string Row_Notification::FK_Alert_asSQL()
{
PLUTO_SAFETY_LOCK(M, table->m_Mutex);

if (is_null[1])
return "NULL";

char buf[32];
sprintf(buf, "%li", m_FK_Alert);

return buf;
}

string Row_Notification::NotificationTime_asSQL()
{
PLUTO_SAFETY_LOCK(M, table->m_Mutex);

if (is_null[2])
return "NULL";

char *buf = new char[39];
mysql_real_escape_string(table->database->db_handle, buf, m_NotificationTime.c_str(), (unsigned long) m_NotificationTime.size());
string s=string()+"\""+buf+"\"";
delete buf;
return s;
}

string Row_Notification::Info_asSQL()
{
PLUTO_SAFETY_LOCK(M, table->m_Mutex);

if (is_null[3])
return "NULL";

char *buf = new char[81];
mysql_real_escape_string(table->database->db_handle, buf, m_Info.c_str(), (unsigned long) m_Info.size());
string s=string()+"\""+buf+"\"";
delete buf;
return s;
}

string Row_Notification::Result_asSQL()
{
PLUTO_SAFETY_LOCK(M, table->m_Mutex);

if (is_null[4])
return "NULL";

char *buf = new char[81];
mysql_real_escape_string(table->database->db_handle, buf, m_Result.c_str(), (unsigned long) m_Result.size());
string s=string()+"\""+buf+"\"";
delete buf;
return s;
}




Table_Notification::Key::Key(long int in_PK_Notification)
{
			pk_PK_Notification = in_PK_Notification;
	
}

Table_Notification::Key::Key(Row_Notification *pRow)
{
			PLUTO_SAFETY_LOCK(M, pRow->table->m_Mutex);

			pk_PK_Notification = pRow->m_PK_Notification;
	
}		

bool Table_Notification::Key_Less::operator()(const Table_Notification::Key &key1, const Table_Notification::Key &key2) const
{
			if (key1.pk_PK_Notification!=key2.pk_PK_Notification)
return key1.pk_PK_Notification<key2.pk_PK_Notification;
else
return false;	
}	

bool Table_Notification::Commit()
{
	PLUTO_SAFETY_LOCK(M, m_Mutex);

//insert added
	while (!addedRows.empty())
	{
		vector<TableRow*>::iterator i = addedRows.begin();
	
		Row_Notification *pRow = (Row_Notification *)*i;
	
		
string values_list_comma_separated;
values_list_comma_separated = values_list_comma_separated + pRow->PK_Notification_asSQL()+", "+pRow->FK_Alert_asSQL()+", "+pRow->NotificationTime_asSQL()+", "+pRow->Info_asSQL()+", "+pRow->Result_asSQL();

	
		string query = "insert into Notification (PK_Notification, FK_Alert, NotificationTime, Info, Result) values ("+
			values_list_comma_separated+")";
			
		if (mysql_query(database->db_handle, query.c_str()))
		{	
			cerr << "Cannot perform query: [" << query << "]" << endl;
			return false;
		}
	
		if (mysql_affected_rows(database->db_handle)!=0)
		{
			
			
			long int id	= (long int) mysql_insert_id(database->db_handle);
		
			if (id!=0)
pRow->m_PK_Notification=id;
	
			
			addedRows.erase(i);
			SingleLongKey key(pRow->m_PK_Notification);	
			cachedRows[key] = pRow;
					
			
			pRow->is_added = false;	
			pRow->is_modified = false;	
		}	
				
	}	


//update modified
	

	for (map<SingleLongKey, class TableRow*, SingleLongKey_Less>::iterator i = cachedRows.begin(); i!= cachedRows.end(); i++)
		if	(((*i).second)->is_modified_get())
	{
		Row_Notification* pRow = (Row_Notification*) (*i).second;	
		SingleLongKey key(pRow->m_PK_Notification);

		char tmp_PK_Notification[32];
sprintf(tmp_PK_Notification, "%li", key.pk);


string condition;
condition = condition + "PK_Notification=" + tmp_PK_Notification;
	
			
		
string update_values_list;
update_values_list = update_values_list + "PK_Notification="+pRow->PK_Notification_asSQL()+", FK_Alert="+pRow->FK_Alert_asSQL()+", NotificationTime="+pRow->NotificationTime_asSQL()+", Info="+pRow->Info_asSQL()+", Result="+pRow->Result_asSQL();

	
		string query = "update Notification set " + update_values_list + " where " + condition;
			
		if (mysql_query(database->db_handle, query.c_str()))
		{	
			cerr << "Cannot perform query: [" << query << "]" << endl;
			return false;
		}
	
		pRow->is_modified = false;	
	}	
	

//delete deleted added
	while (!deleted_addedRows.empty())
	{	
		vector<TableRow*>::iterator i = deleted_addedRows.begin();
		Row_Notification* pRow = (Row_Notification*) (*i);
		delete pRow;
		deleted_addedRows.erase(i);
	}	


//delete deleted cached
	
	while (!deleted_cachedRows.empty())
	{	
		map<SingleLongKey, class TableRow*, SingleLongKey_Less>::iterator i = deleted_cachedRows.begin();
	
		SingleLongKey key = (*i).first;
		Row_Notification* pRow = (Row_Notification*) (*i).second;	

		char tmp_PK_Notification[32];
sprintf(tmp_PK_Notification, "%li", key.pk);


string condition;
condition = condition + "PK_Notification=" + tmp_PK_Notification;

	
		string query = "delete from Notification where " + condition;
		
		if (mysql_query(database->db_handle, query.c_str()))
		{	
			cerr << "Cannot perform query: [" << query << "]" << endl;
			return false;
		}	
		
		pRow = (Row_Notification*) (*i).second;;
		delete pRow;
		deleted_cachedRows.erase(key);
	}
	
	return true;
}

bool Table_Notification::GetRows(string where_statement,vector<class Row_Notification*> *rows)
{
	PLUTO_SAFETY_LOCK(M, m_Mutex);

	string query;
	if( StringUtils::StartsWith(where_statement,"where ",true) || StringUtils::StartsWith(where_statement,"join ",true) )
		query = "select * from Notification " + where_statement;
	else if( StringUtils::StartsWith(where_statement,"select ",true) )
		query = where_statement;
	else
		query = "select * from Notification where " + where_statement;
		
	if (mysql_query(database->db_handle, query.c_str()))
	{	
		cerr << "Cannot perform query: [" << query << "]" << endl;
		return false;
	}	

	MYSQL_RES *res = mysql_store_result(database->db_handle);
	
	if (!res)
	{
		cerr << "mysql_store_result returned NULL handler" << endl;
		return false;
	}	
	
	MYSQL_ROW row;
						
		
	while ((row = mysql_fetch_row(res)) != NULL)
	{	
		unsigned long *lengths = mysql_fetch_lengths(res);

		Row_Notification *pRow = new Row_Notification(this);
		
		if (row[0] == NULL)
{
pRow->is_null[0]=true;
pRow->m_PK_Notification = 0;
}
else
{
pRow->is_null[0]=false;
sscanf(row[0], "%li", &(pRow->m_PK_Notification));
}

if (row[1] == NULL)
{
pRow->is_null[1]=true;
pRow->m_FK_Alert = 0;
}
else
{
pRow->is_null[1]=false;
sscanf(row[1], "%li", &(pRow->m_FK_Alert));
}

if (row[2] == NULL)
{
pRow->is_null[2]=true;
pRow->m_NotificationTime = "";
}
else
{
pRow->is_null[2]=false;
pRow->m_NotificationTime = string(row[2],lengths[2]);
}

if (row[3] == NULL)
{
pRow->is_null[3]=true;
pRow->m_Info = "";
}
else
{
pRow->is_null[3]=false;
pRow->m_Info = string(row[3],lengths[3]);
}

if (row[4] == NULL)
{
pRow->is_null[4]=true;
pRow->m_Result = "";
}
else
{
pRow->is_null[4]=false;
pRow->m_Result = string(row[4],lengths[4]);
}



		//checking for duplicates

		SingleLongKey key(pRow->m_PK_Notification);
		
		map<SingleLongKey, class TableRow*, SingleLongKey_Less>::iterator i = cachedRows.find(key);
			
		if (i!=cachedRows.end())
		{
			delete pRow;
			pRow = (Row_Notification *)(*i).second;
		}

		rows->push_back(pRow);
		
		cachedRows[key] = pRow;
	}

	mysql_free_result(res);			
		
	return true;					
}

Row_Notification* Table_Notification::AddRow()
{
	PLUTO_SAFETY_LOCK(M, m_Mutex);

	Row_Notification *pRow = new Row_Notification(this);
	pRow->is_added=true;
	addedRows.push_back(pRow);
	return pRow;		
}



Row_Notification* Table_Notification::GetRow(long int in_PK_Notification)
{
	PLUTO_SAFETY_LOCK(M, m_Mutex);

	SingleLongKey row_key(in_PK_Notification);

	map<SingleLongKey, class TableRow*, SingleLongKey_Less>::iterator i;
	i = deleted_cachedRows.find(row_key);	
		
	//row was deleted	
	if (i!=deleted_cachedRows.end())
		return NULL;
	
	i = cachedRows.find(row_key);
	
	//row is cached
	if (i!=cachedRows.end())
		return (Row_Notification*) (*i).second;
	//we have to fetch row
	Row_Notification* pRow = FetchRow(row_key);

	if (pRow!=NULL)
		cachedRows[row_key] = pRow;
	return pRow;	
}



Row_Notification* Table_Notification::FetchRow(SingleLongKey &key)
{
	PLUTO_SAFETY_LOCK(M, m_Mutex);

	//defines the string query for the value of key
	char tmp_PK_Notification[32];
sprintf(tmp_PK_Notification, "%li", key.pk);


string condition;
condition = condition + "PK_Notification=" + tmp_PK_Notification;


	string query = "select * from Notification where " + condition;		

	if (mysql_query(database->db_handle, query.c_str()))
	{	
		cerr << "Cannot perform query: [" << query << "]" << endl;
		return NULL;
	}	

	MYSQL_RES *res = mysql_store_result(database->db_handle);
	
	if (!res)
	{
		cerr << "mysql_store_result returned NULL handler" << endl;
		return NULL;
	}	
	
	MYSQL_ROW row = mysql_fetch_row(res);
	
	if (!row)
	{
		//dataset is empty
		mysql_free_result(res);			
		return NULL;		
	}	
						
	unsigned long *lengths = mysql_fetch_lengths(res);

	Row_Notification *pRow = new Row_Notification(this);
		
	if (row[0] == NULL)
{
pRow->is_null[0]=true;
pRow->m_PK_Notification = 0;
}
else
{
pRow->is_null[0]=false;
sscanf(row[0], "%li", &(pRow->m_PK_Notification));
}

if (row[1] == NULL)
{
pRow->is_null[1]=true;
pRow->m_FK_Alert = 0;
}
else
{
pRow->is_null[1]=false;
sscanf(row[1], "%li", &(pRow->m_FK_Alert));
}

if (row[2] == NULL)
{
pRow->is_null[2]=true;
pRow->m_NotificationTime = "";
}
else
{
pRow->is_null[2]=false;
pRow->m_NotificationTime = string(row[2],lengths[2]);
}

if (row[3] == NULL)
{
pRow->is_null[3]=true;
pRow->m_Info = "";
}
else
{
pRow->is_null[3]=false;
pRow->m_Info = string(row[3],lengths[3]);
}

if (row[4] == NULL)
{
pRow->is_null[4]=true;
pRow->m_Result = "";
}
else
{
pRow->is_null[4]=false;
pRow->m_Result = string(row[4],lengths[4]);
}



	mysql_free_result(res);			
	
	return pRow;						
}


class Row_Alert* Row_Notification::FK_Alert_getrow()
{
PLUTO_SAFETY_LOCK(M, table->m_Mutex);

class Table_Alert *pTable = table->database->Alert_get();
return pTable->GetRow(m_FK_Alert);
}





