...
#define  Media_Grids_Plugin_VERSION "testing"
...
extern "C" {
	class Command_Impl *RegisterAsPlugIn(class Router *pRouter,int PK_Device,string sLogger)
	{
		...
		Media_Grids_Plugin *pMedia_Grids_Plugin = new Media_Grids_Plugin(PK_Device, "localhost",true,false,pRouter);
		if( pMedia_Grids_Plugin->m_bQuit )
		{
			delete pMedia_Grids_Plugin;
			return NULL;
		}
		return pMedia_Grids_Plugin;
	}
}
//<-dceag-plug-e->
...
int main(int argc, char* argv[]) 
{
	...
	if (bError)
	{
		cout << "Media_Grids_Plugin, v." << Media_Grids_Plugin_VERSION << endl
			<< "A Pluto DCE Device.  See www.plutohome.com/dce for details." << endl
			<< "Usage: Media_Grids_Plugin [-r Router's IP] [-d My Device ID] [-l dce_router|stdout|null|filename]" << endl
	...
	try
	{
		Media_Grids_Plugin *pMedia_Grids_Plugin = new Media_Grids_Plugin(PK_Device, sRouter_IP);	
		if ( pMedia_Grids_Plugin->Connect() ) 
		{
			g_pPlutoLogger->Write(LV_STATUS, "Connect OK");
			pMedia_Grids_Plugin->CreateChildren();
			pthread_join(pMedia_Grids_Plugin->m_RequestHandlerThread, NULL);
