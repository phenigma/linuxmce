...
Event_Impl *General_Info_Plugin_Event::CreateEvent(int PK_DeviceTemplate, ClientSocket *pOCClientSocket, int DeviceID)
{
	switch(PK_DeviceTemplate) {
		case 27:
			return (Event_Impl *) new General_Info_Plugin_Event(pOCClientSocket, DeviceID);
	};
...
Command_Impl  *General_Info_Plugin_Command::CreateCommand(int PK_DeviceTemplate, Command_Impl *pPrimaryDeviceCommand, DeviceData_Impl *pData, Event_Impl *pEvent)
{
	...
	{
		case 27:
			return (Command_Impl *) new General_Info_Plugin_Command(pPrimaryDeviceCommand, pData, pEvent, m_pRouter);
	};
