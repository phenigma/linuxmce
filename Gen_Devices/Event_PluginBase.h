...
{
public:
	Event_Plugin_Event(int DeviceID, string ServerAddress, bool bConnectEventHandler=true) : Event_Impl(DeviceID, ServerAddress, bConnectEventHandler) {};
...
class Event_Plugin_Data : public DeviceData_Impl
{
public:
	virtual ~Event_Plugin_Data() {};
...
class Event_Plugin_Command : public Command_Impl
{
public:
	Event_Plugin_Command(int DeviceID, string ServerAddress,bool bConnectEventHandler=true,bool bLocalMode=false,class Router *pRouter=NULL)
	: Command_Impl(DeviceID, ServerAddress, bLocalMode, pRouter)
	{
		if( m_bLocalMode )
			return;
		m_pData=NULL;
		m_pEvent = new Event_Plugin_Event(DeviceID, ServerAddress);
	...
	Event_Plugin_Command(Command_Impl *pPrimaryDeviceCommand, DeviceData_Impl *pData, Event_Impl *pEvent, Router *pRouter) : Command_Impl(pPrimaryDeviceCommand, pData, pEvent, pRouter) {};
	virtual ~Event_Plugin_Command() {};
	Event_Plugin_Event *GetEvents() { return (Event_Plugin_Event *) m_pEvent; };
	Event_Plugin_Data *GetData() { return (Event_Plugin_Data *) m_pData; };
	const char *GetClassName() { return "Event_Plugin_Command"; };
	virtual int PK_DeviceTemplate_get() { return 52; };
	static int PK_DeviceTemplate_get_static() { return 52; };
		...
		for(int s=-1;s<(int) pMessageOriginal->m_vectExtraMessages.size(); ++s)
		{
			...
			if (pMessage->m_dwPK_Device_To==m_dwPK_Device && pMessage->m_dwMessage_Type == MESSAGETYPE_COMMAND)
			{
				...
				case 263:
					...
					int iPK_EventHandler=atoi(pMessage->m_mapParameters[107].c_str());
