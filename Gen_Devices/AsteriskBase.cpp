...
Event_Impl *Asterisk_Event::CreateEvent(int PK_DeviceTemplate, ClientSocket *pOCClientSocket, int DeviceID)
{
	switch(PK_DeviceTemplate) {
		case 45:
			return (Event_Impl *) new Asterisk_Event(pOCClientSocket, DeviceID);
	};
...
Command_Impl  *Asterisk_Command::CreateCommand(int PK_DeviceTemplate, Command_Impl *pPrimaryDeviceCommand, DeviceData_Impl *pData, Event_Impl *pEvent)
{
	...
	{
		case 45:
			return (Command_Impl *) new Asterisk_Command(pPrimaryDeviceCommand, pData, pEvent, m_pRouter);
	};
