...
Event_Impl *Orbiter_Event::CreateEvent(int PK_DeviceTemplate, ClientSocket *pOCClientSocket, int DeviceID)
{
	switch(PK_DeviceTemplate) {
		case 8:
			return (Event_Impl *) new Orbiter_Event(pOCClientSocket, DeviceID);
	};
...
Command_Impl  *Orbiter_Command::CreateCommand(int PK_DeviceTemplate, Command_Impl *pPrimaryDeviceCommand, DeviceData_Impl *pData, Event_Impl *pEvent)
{
	...
	{
		case 8:
			return (Command_Impl *) new Orbiter_Command(pPrimaryDeviceCommand, pData, pEvent, m_pRouter);
	};
