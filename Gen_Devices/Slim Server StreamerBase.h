#ifndef Slim Server StreamerBase_h
#define Slim Server StreamerBase_h
...
class Slim Server Streamer_Event : public Event_Impl
{
public:
	Slim Server Streamer_Event(int DeviceID, string ServerAddress, bool bConnectEventHandler=true) : Event_Impl(DeviceID, ServerAddress, bConnectEventHandler) {};
	Slim Server Streamer_Event(class ClientSocket *pOCClientSocket, int DeviceID) : Event_Impl(pOCClientSocket, DeviceID) {};
	//Events
...
class Slim Server Streamer_Data : public DeviceData_Impl
{
public:
	virtual ~Slim Server Streamer_Data() {};
...
class Slim Server Streamer_Command : public Command_Impl
{
public:
	Slim Server Streamer_Command(int DeviceID, string ServerAddress,bool bConnectEventHandler=true,bool bLocalMode=false,class Router *pRouter=NULL)
	: Command_Impl(DeviceID, ServerAddress, bLocalMode, pRouter)
	{
		if( m_bLocalMode )
			return;
		m_pData=NULL;
		m_pEvent = new Slim Server Streamer_Event(DeviceID, ServerAddress);
		int Size; char *pConfig = m_pEvent->GetConfig(Size);
		if( !pConfig )
			throw "Cannot get configuration data";
		m_pData = new Slim Server Streamer_Data();
		if( Size )
	...
	Slim Server Streamer_Command(Command_Impl *pPrimaryDeviceCommand, DeviceData_Impl *pData, Event_Impl *pEvent, Router *pRouter) : Command_Impl(pPrimaryDeviceCommand, pData, pEvent, pRouter) {};
	virtual ~Slim Server Streamer_Command() {};
	Slim Server Streamer_Event *GetEvents() { return (Slim Server Streamer_Event *) m_pEvent; };
	Slim Server Streamer_Data *GetData() { return (Slim Server Streamer_Data *) m_pData; };
	const char *GetClassName() { return "Slim Server Streamer_Command"; };
	static int PK_DeviceTemplate_get() { return 58; };
