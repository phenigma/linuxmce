#include "Media_Grids_PluginBase.h"
#include "DeviceData_Impl.h"
#include "Logger.h"

using namespace DCE;
DeviceData_Impl *Media_Grids_Plugin_Data::CreateData(DeviceData_Impl *Parent,char *pDataBlock,unsigned long AllocatedSize,char *CurrentPosition)
{
...
Event_Impl *Media_Grids_Plugin_Event::CreateEvent(int PK_DeviceTemplate, ClientSocket *pOCClientSocket, int DeviceID)
{
	g_pPlutoLogger->Write(LV_CRITICAL, "Got CreateEvent for unknown type %d.", PK_DeviceTemplate);
	return NULL;
}
Command_Impl  *Media_Grids_Plugin_Command::CreateCommand(int PK_DeviceTemplate, Command_Impl *pPrimaryDeviceCommand, DeviceData_Impl *pData, Event_Impl *pEvent)
{
