...
Event_Impl *MythTV_PlugIn_Event::CreateEvent(int PK_DeviceTemplate, ClientSocket *pOCClientSocket, int DeviceID)
{
	switch(PK_DeviceTemplate) {
		case 36:
			return (Event_Impl *) new MythTV_PlugIn_Event(pOCClientSocket, DeviceID);
	};
...
Command_Impl  *MythTV_PlugIn_Command::CreateCommand(int PK_DeviceTemplate, Command_Impl *pPrimaryDeviceCommand, DeviceData_Impl *pData, Event_Impl *pEvent)
{
	...
	{
		case 36:
			return (Command_Impl *) new MythTV_PlugIn_Command(pPrimaryDeviceCommand, pData, pEvent, m_pRouter);
	};
