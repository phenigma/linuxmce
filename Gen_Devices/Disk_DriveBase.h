...
{
public:
	Disk_Drive_Event(int DeviceID, string ServerAddress, bool bConnectEventHandler=true) : Event_Impl(DeviceID, ServerAddress, bConnectEventHandler) {};
	...
	{
		SendMessage(new Message(m_DeviceID, DEVICEID_EVENTMANAGER, PRIORITY_NORMAL, MESSAGETYPE_EVENT, 3,2,3,StringUtils::itos(iFK_MediaType).c_str(),4,sMRL.c_str()));
	}
...
public:
	...
	string Get_Drive() { return m_Parameters[6];}
...
public:
	...
	: Command_Impl(DeviceID, ServerAddress, bLocalMode, pRouter)
	{
		if( m_bLocalMode )
			return;
		m_pData=NULL;
		m_pEvent = new Disk_Drive_Event(DeviceID, ServerAddress);
			...
			if (pMessage->m_DeviceIDTo==m_DeviceID && pMessage->m_MessageType == MESSAGETYPE_COMMAND)
			{
				...
				case 54:
					...
					string sFilename=pMessage->m_Parameters[13];
						...
						{
							Message *pMessageOut=new Message(m_DeviceID,pMessage->m_DeviceIDFrom,PRIORITY_NORMAL,MESSAGETYPE_REPLY,0,0);
						pMessageOut->m_Parameters[59]=sMediaURL;
							SendMessage(pMessageOut);
						}
						else if( pMessage->m_eExpectedResponse==ER_DeliveryConfirmation || pMessage->m_eExpectedResponse==ER_ReplyString )
							SendString(sCMD_Result);
