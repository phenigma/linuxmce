...
Event_Impl *Infrared_Plugin_Event::CreateEvent(int PK_DeviceTemplate, ClientSocket *pOCClientSocket, int DeviceID)
{
	switch(PK_DeviceTemplate) {
		case 39:
			return (Event_Impl *) new Infrared_Plugin_Event(pOCClientSocket, DeviceID);
	};
...
Command_Impl  *Infrared_Plugin_Command::CreateCommand(int PK_DeviceTemplate, Command_Impl *pPrimaryDeviceCommand, DeviceData_Impl *pData, Event_Impl *pEvent)
{
	...
	{
		case 39:
			return (Command_Impl *) new Infrared_Plugin_Command(pPrimaryDeviceCommand, pData, pEvent, m_pRouter);
	};
