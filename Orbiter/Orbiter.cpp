...

{
	...
	m_CaptureKeyboard_bOnOff = false;
	m_CaptureKeyboard_bReset = false;
	m_CaptureKeyboard_bTypePin = false;
	m_CaptureKeyboard_bDataGrid = false;
	m_CaptureKeyboard_iPK_Variable = 0;
	m_CaptureKeyboard_sText = "";
	m_CaptureKeyboard_InternalBuffer = "";
	m_CaptureKeyboard_pText = NULL;
}
...
//      LocalHandleWebWindow(pObj, rectTotal);
    ...
    case DESIGNOBJTYPE_Pronto_File_CONST:
        ...
        if (pObj->m_pCCF)
        {
            pObj->m_pCCF->RenderPronto(XOffset, YOffset);
        }
        break;
#endif
...
//------------------------------------------------------------------------
/*virtual*/ void Orbiter::HighlightObject(class DesignObj_Orbiter *pObj)
{
...
*/
/*
    else if( pObj->m_iBaseObjectID==DESIGNOBJ_BUTUSER_CONST )
    {
        int PK_Users=0;
        DesignObjZoneList::iterator iZone;
        for(iZone=pObj->m_ZoneList.begin();!PK_Users && iZone!=pObj->m_ZoneList.end();++iZone)
        {
            ...
            {
                if( (*iAction)->m_PK_Command==COMMAND_Set_Variable_CONST )
                {
                    int Value=0;
                    bool bIsMatch=false;
                    ...
                    {
                        if( (*iap).first==COMMAND_Set_Variable_CONST && atoi(((*iap).second).c_str())==VARIABLE_PK_USERS_CONST )
                            bIsMatch=true;
                        else if( (*iap).first==COMMANDPARAMETER_Value_To_Assign_CONST )
                            Value=atoi(((*iap).second).c_str());
                    }
                    if( bIsMatch )
                        PK_Users=Value;
                }
    ...
    else if( pObj->m_iBaseObjectID==DESIGNOBJ_BUTLOCATIONS_CONST )
    {
        string ArrayID="";
        DesignObjZoneList::iterator iZone;
        for(iZone=pObj->m_ZoneList.begin();ArrayID.length()==0 && iZone!=pObj->m_ZoneList.end();++iZone)
        {
            DesignObjCommandList::iterator iAction;
            for(iAction=(*iZone)->m_Commands.begin();ArrayID.length()==0 && iAction!=(*iZone)->m_Commands.end();++iAction)
            {
                if( (*iAction)->m_PK_Command==COMMAND_Set_Variable_CONST )
                {
                    string sValue="";
                    ...
                    for(iap=(*iAction)->m_ParameterList.begin();iap!=(*iAction)->m_ParameterList.end();++iap)
                    {
                        if( (*iap).first==COMMAND_Set_Variable_CONST && atoi(((*iap).second).c_str())==VARIABLE_ARRAY_ID_CONST )
                            bIsMatch=true;
                        else if( (*iap).first==COMMANDPARAMETER_Value_To_Assign_CONST )
                        {
                            sValue=(*iap).second;
        ...
        {
            string::size_type pos = 0;
            int PK_Location = atoi(StringUtils::Tokenize(ArrayID,",",pos).c_str());
            int PK_Room = atoi(StringUtils::Tokenize(ArrayID,",",pos).c_str());
            int PK_EntGroup = atoi(StringUtils::Tokenize(ArrayID,",",pos).c_str());
            m_mapLocationIcons[PK_Location]=pObj->m_pGraphic;
            m_vectLocations.push_back(new OrbiterLocationInfo(PK_Location,PK_Room,PK_EntGroup));
        }
    }
    */
}
...
#ifndef PHONEKEYS
    if(m_CaptureKeyboard_bOnOff)
    {
        if(m_CaptureKeyboard_iPK_Variable)
        {
            if(
			...
			else if(PK_Button == BUTTON_Enter_CONST)
			{
				...
				//CaptureKeyboard_UpdateVariableAndText(m_CaptureKeyboard_iPK_Variable, 
				//	m_CaptureKeyboard_InternalBuffer);
			}
        }

        if(m_CaptureKeyboard_bDataGrid)
        {
            if(
                (PK_Button >= BUTTON_a_CONST && PK_Button <= BUTTON_z_CONST) ||
                (PK_Button >= BUTTON_A_CONST && PK_Button <= BUTTON_Z_CONST) ||
            ...
            )
            {
                ...
                if(m_vectObjs_GridsOnScreen.size() > 0)
                {
                    ...
                    for(int i = 0; i < pDataGrid->m_pDataGridTable->m_RowCount; i++)
                    {
                        ...
                        if(0 == Text.find(m_CaptureKeyboard_InternalBuffer)) //we have a match, Text starts with m_CaptureKeyboard_InternalBuffer
                        {
...
#else if
    if(m_CaptureKeyboard_bOnOff && m_CaptureKeyboard_iPK_Variable)
    {
...
DesignObj_Orbiter *Orbiter::FindObject(string PK_DesignObj,class DesignObj_Orbiter *pDesignObj_Orbiter)
{
    ...
    PLUTO_SAFETY_LOCK(vm,m_VariableMutex)
    DesignObj_OrbiterMap::iterator oi = m_mapObj_All.find(PK_DesignObj);
...
string Orbiter::SubstituteVariables(string Input, DesignObj_Orbiter *pObj, int X, int Y)
{
    ...
    for(;;)
    {
        ...
        if( Variable=="M" )
            ...
            Output += StringUtils::itos(m_pScreenHistory_Current->m_iPK_Users);
        else if( Variable=="X" )
...
{
    //on/off parameter
    m_CaptureKeyboard_bOnOff = sOnOff == "0" ? false : true;

    //normal/pin paramter
    m_CaptureKeyboard_bTypePin = sType != "2";

    //reset parameter
    m_CaptureKeyboard_bReset = sReset != "0";

    //data grid
    m_CaptureKeyboard_bDataGrid = bDataGrid;

    //set var
    m_CaptureKeyboard_iPK_Variable = iPK_Variable;
    ...
    m_CaptureKeyboard_pText = FindText(pObj, iPK_Text);

    if(iPK_Variable)
        m_CaptureKeyboard_pText->m_sText = m_mapVariable[iPK_Variable];

    //build the text string
    m_CaptureKeyboard_sText = "";
    if(NULL == m_CaptureKeyboard_pText)
    {
    ...
    {
        m_CaptureKeyboard_sText += StringUtils::itos(m_CaptureKeyboard_pText->m_rPosition.X) + ",";
        m_CaptureKeyboard_sText += StringUtils::itos(m_CaptureKeyboard_pText->m_rPosition.Y) + ",";
        m_CaptureKeyboard_sText += StringUtils::itos(m_CaptureKeyboard_pText->m_rPosition.Width) + ",";
        m_CaptureKeyboard_sText += StringUtils::itos(m_CaptureKeyboard_pText->m_rPosition.Height);
    }
...
{
    string OldValue = m_mapVariable[m_CaptureKeyboard_iPK_Variable];
    string NewValue = "";

    if(OldValue.size())
        NewValue = OldValue.substr(0, OldValue.size() - 1);

    m_CaptureKeyboard_InternalBuffer = NewValue;
    m_mapVariable[m_CaptureKeyboard_iPK_Variable] = NewValue;

    if(NULL != m_CaptureKeyboard_pText)
    {
        m_CaptureKeyboard_pText->m_sText = NewValue;
...
void Orbiter::CaptureKeyboard_UpdateVariableAndText(int iVariable, string sVariable) //for capture keyboard cmd
{
	...
	/*
	if(m_CaptureKeyboard_pText->m_pObject)
	{
		NeedToRender render(this,"Capture update var and text");
		m_CaptureKeyboard_pText->m_sText = sVariable;
		m_vectObjs_NeedRedraw.push_back(m_CaptureKeyboard_pText->m_pObject);
	}*/
}
