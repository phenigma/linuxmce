...
cat <<-EOM >$R/etc/apt/sources.list
	#deb http://10.10.42.99/raspbian-jessie/  ./
	...
	#deb-src http://mirrordirector.raspbian.org/raspbian/ ${RELEASE}  main contrib non-free
	EOM
...
DEBIAN_FRONTEND=noninteractive chroot $R apt-get -y dist-upgrade

# Install the RPi PPA
DEBIAN_FRONTEND=noninteractive chroot $R apt-get -y install software-properties-common raspbian-archive-keyring
...
#DEBIAN_FRONTEND=noninteractive FLASH_KERNEL_SKIP="true" chroot $R apt-get -y install linux-image-rpi2-rpfv linux-headers-rpi2-rpfv

DEBIAN_FRONTEND=noninteractive FLASH_KERNEL_SKIP="true" chroot $R apt-get -y install wpasupplicant firmware-linux-free openssh-server nano cifs-utils pastebinit parted sudo-ldap
