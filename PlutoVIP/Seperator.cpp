// Seperator.cpp : implementation file
//
...
BEGIN_MESSAGE_MAP(CSeperator, CStatic)
	//{{AFX_MSG_MAP(CSeperator)
	ON_WM_CREATE()
	ON_WM_PAINT()
...
/////////////////////////////////////////////////////////////////////////////
// CSeperator message handlers

void CSeperator::PreSubclassWindow() 
{
...
int CSeperator::OnCreate(LPCREATESTRUCT lpCreateStruct) 
{
	// make sure there wasn't any border styles specified, 
	// we don't support them
...
LONG CSeperator::OnSetText(UINT wParam, LONG lParam)
{
...
LONG CSeperator::OnGetText(UINT wParam, LONG lParam)
{
	if(wParam>0)
	{
		wParam=wParam>(UINT)m_sText.GetLength()+1 ? 
			m_sText.GetLength()+1 : wParam;
		VERIFY(lstrcpyn((LPTSTR)lParam,m_sText,wParam-1)!=NULL);
	}
	return (LONG)wParam;
}

LONG CSeperator::OnGetTextLength(UINT wParam, LONG lParam)
{
	UNREFERENCED_PARAMETER(wParam);
