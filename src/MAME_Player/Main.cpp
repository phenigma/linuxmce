...
extern "C" {
	...
	{ 
		...
		if( bIsRuntimePlugin )
			return MAME_Player::PK_DeviceTemplate_get_static();
		else
			return 0;
	}
}
...
extern "C" {
	class Command_Impl *RegisterAsPlugIn(class Router *pRouter,int PK_Device,Logger *pPlutoLogger)
	{
		...
		MAME_Player *pMAME_Player = new MAME_Player(PK_Device, "localhost",true,false,pRouter);
		if( pMAME_Player->m_bQuit_get()|| !pMAME_Player->GetConfig() )
		{
			delete pMAME_Player;
			return NULL;
		}
...
int main(int argc, char* argv[]) 
{
	...
	cout << "MAME_Player, v." << VERSION << endl
	...
	if (bError)
	{
		cout << "A Pluto DCE Device.  See www.plutohome.com/dce for details." << endl
			<< "Usage: MAME_Player [-r Router's IP] [-d My Device ID] [-l dcerouter|stdout|null|filename]" << endl
	...
	try
	{
		if( sLogger=="dcerouter" )
			LoggerWrapper::SetInstance(new ServerLogger(PK_Device, MAME_Player::PK_DeviceTemplate_get_static(), sRouter_IP));
	...
	try
	{
		MAME_Player *pMAME_Player = new MAME_Player(PK_Device, sRouter_IP,true,bLocalMode);
		if ( pMAME_Player->GetConfig() && pMAME_Player->Connect(pMAME_Player->PK_DeviceTemplate_get()) ) 
		{
			g_pCommand_Impl=pMAME_Player;
			g_pDeadlockHandler=DeadlockHandler;
			g_pSocketCrashHandler=SocketCrashHandler;
			LoggerWrapper::GetInstance()->Write(LV_STATUS, "Connect OK");
			pMAME_Player->CreateChildren();
			if( bLocalMode )
				pMAME_Player->RunLocalMode();
			else
			{
				if(pMAME_Player->m_RequestHandlerThread)
					pthread_join(pMAME_Player->m_RequestHandlerThread, NULL);  // This function will return when the device is shutting down
			}
		...
		{
			bAppError = true;
			if( pMAME_Player->m_pEvent && pMAME_Player->m_pEvent->m_pClientSocket && pMAME_Player->m_pEvent->m_pClientSocket->m_eLastError==ClientSocket::cs_err_CannotConnect )
			{
				bAppError = false;
				bReload = false;
