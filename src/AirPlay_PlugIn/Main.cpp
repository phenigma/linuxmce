...
extern "C" {
	...
	{ 
		...
		if( bIsRuntimePlugin )
			return AirPlay_PlugIn::PK_DeviceTemplate_get_static();
		else
			return 0;
	}
}
...
extern "C" {
	class Command_Impl *RegisterAsPlugIn(class Router *pRouter,int PK_Device,Logger *pPlutoLogger)
	{
		...
		AirPlay_PlugIn *pAirPlay_PlugIn = new AirPlay_PlugIn(PK_Device, "localhost",true,false,pRouter);
		if( pAirPlay_PlugIn->m_bQuit_get()|| !pAirPlay_PlugIn->GetConfig() )
		{
			delete pAirPlay_PlugIn;
			return NULL;
		}
		else
		{
			g_pCommand_Impl=pAirPlay_PlugIn;
			g_pDeadlockHandler=Plugin_DeadlockHandler;
			g_pSocketCrashHandler=Plugin_SocketCrashHandler;
		}
		return pAirPlay_PlugIn;
	}
}
//<-dceag-plug-e->
...
int main(int argc, char* argv[]) 
{
	...
	cout << "AirPlay_PlugIn, v." << VERSION << endl
	...
	if (bError)
	{
		cout << "A Pluto DCE Device.  See www.plutohome.com/dce for details." << endl
			<< "Usage: AirPlay_PlugIn [-r Router's IP] [-d My Device ID] [-l dcerouter|stdout|null|filename]" << endl
	...
	try
	{
		if( sLogger=="dcerouter" )
			LoggerWrapper::SetInstance(new ServerLogger(PK_Device, AirPlay_PlugIn::PK_DeviceTemplate_get_static(), sRouter_IP));
	...
	try
	{
		AirPlay_PlugIn *pAirPlay_PlugIn = new AirPlay_PlugIn(PK_Device, sRouter_IP,true,bLocalMode);
		if ( pAirPlay_PlugIn->GetConfig() && pAirPlay_PlugIn->Connect(pAirPlay_PlugIn->PK_DeviceTemplate_get()) ) 
		{
			g_pCommand_Impl=pAirPlay_PlugIn;
			g_pDeadlockHandler=DeadlockHandler;
			g_pSocketCrashHandler=SocketCrashHandler;
			LoggerWrapper::GetInstance()->Write(LV_STATUS, "Connect OK");
			pAirPlay_PlugIn->CreateChildren();
			if( bLocalMode )
				pAirPlay_PlugIn->RunLocalMode();
			else
			{
				if(pAirPlay_PlugIn->m_RequestHandlerThread)
					pthread_join(pAirPlay_PlugIn->m_RequestHandlerThread, NULL);  // This function will return when the device is shutting down
			}
		...
		{
			bAppError = true;
			if( pAirPlay_PlugIn->m_pEvent && pAirPlay_PlugIn->m_pEvent->m_pClientSocket && pAirPlay_PlugIn->m_pEvent->m_pClientSocket->m_eLastError==ClientSocket::cs_err_CannotConnect )
			{
				bAppError = false;
				bReload = false;
