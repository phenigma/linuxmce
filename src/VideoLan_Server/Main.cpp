...
extern "C" {
	...
	{ 
		...
		if( bIsRuntimePlugin )
			return VideoLan_Server::PK_DeviceTemplate_get_static();
		else
			return 0;
	}
}
...
extern "C" {
	class Command_Impl *RegisterAsPlugIn(class Router *pRouter,int PK_Device,Logger *pPlutoLogger)
	{
		...
		if( pVideoLan_Server->m_bQuit_get()|| !pVideoLan_Server->GetConfig() )
		{
...
int main(int argc, char* argv[]) 
{
	...
	cout << "VideoLan_Server, v." << VERSION << endl
	...
	if (bError)
	{
		cout << "A Pluto DCE Device.  See www.plutohome.com/dce for details." << endl
			<< "Usage: VideoLan_Server [-r Router's IP] [-d My Device ID] [-l dcerouter|stdout|null|filename]" << endl
	...
	try
	{
		if( sLogger=="dcerouter" )
			LoggerWrapper::SetInstance(new ServerLogger(PK_Device, VideoLan_Server::PK_DeviceTemplate_get_static(), sRouter_IP));
	...
	try
	{
		VideoLan_Server *pVideoLan_Server = new VideoLan_Server(PK_Device, sRouter_IP,true,bLocalMode);
		if ( pVideoLan_Server->GetConfig() && pVideoLan_Server->Connect(pVideoLan_Server->PK_DeviceTemplate_get()) ) 
		{
			...
			pVideoLan_Server->CreateChildren();
			if( bLocalMode )
		...
		{
			bAppError = true;
			if( pVideoLan_Server->m_pEvent && pVideoLan_Server->m_pEvent->m_pClientSocket && pVideoLan_Server->m_pEvent->m_pClientSocket->m_eLastError==ClientSocket::cs_err_CannotConnect )
			{
				bAppError = false;
				bReload = false;
