...
extern "C" {
	...
	{ 
		...
		if( bIsRuntimePlugin )
			return SMPTE_Fountain::PK_DeviceTemplate_get_static();
		else
			return 0;
	}
}
...
int main(int argc, char* argv[]) 
{
	...
	if (bError)
	{
		cout << "A Pluto DCE Device.  See www.plutohome.com/dce for details." << endl
			<< "Usage: SMPTE_Fountain [-r Router's IP] [-d My Device ID] [-l dcerouter|stdout|null|filename]" << endl
	...
	try
	{
		if( sLogger=="dcerouter" )
			LoggerWrapper::SetInstance(new ServerLogger(PK_Device, SMPTE_Fountain::PK_DeviceTemplate_get_static(), sRouter_IP));
	...
	try
	{
		SMPTE_Fountain *pSMPTE_Fountain = new SMPTE_Fountain(PK_Device, sRouter_IP,true,bLocalMode);
		if ( pSMPTE_Fountain->GetConfig() && pSMPTE_Fountain->Connect(pSMPTE_Fountain->PK_DeviceTemplate_get()) ) 
		{
			g_pCommand_Impl=pSMPTE_Fountain;
			g_pDeadlockHandler=DeadlockHandler;
			g_pSocketCrashHandler=SocketCrashHandler;
			LoggerWrapper::GetInstance()->Write(LV_STATUS, "Connect OK");
			pSMPTE_Fountain->CreateChildren();
			if( bLocalMode )
		...
		{
			bAppError = true;
			if( pSMPTE_Fountain->m_pEvent && pSMPTE_Fountain->m_pEvent->m_pClientSocket && pSMPTE_Fountain->m_pEvent->m_pClientSocket->m_eLastError==ClientSocket::cs_err_CannotConnect )
			{
				bAppError = false;
				bReload = false;
