...
extern "C" {
	...
	{ 
		...
		if( bIsRuntimePlugin )
			return VLC_Player::PK_DeviceTemplate_get_static();
		else
			return 0;
	}
}
...
extern "C" {
	class Command_Impl *RegisterAsPlugIn(class Router *pRouter,int PK_Device,Logger *pPlutoLogger)
	{
		...
		VLC_Player *pVLC_Player = new VLC_Player(PK_Device, "localhost",true,false,pRouter);
		if( pVLC_Player->m_bQuit_get()|| !pVLC_Player->GetConfig() )
		{
			delete pVLC_Player;
			return NULL;
		}
...
int main(int argc, char* argv[]) 
{
	...
	cout << "VLC_Player, v." << VERSION << endl
	...
	if (bError)
	{
		cout << "A Pluto DCE Device.  See www.plutohome.com/dce for details." << endl
			<< "Usage: VLC_Player [-r Router's IP] [-d My Device ID] [-l dcerouter|stdout|null|filename]" << endl
	...
	try
	{
		if( sLogger=="dcerouter" )
			LoggerWrapper::SetInstance(new ServerLogger(PK_Device, VLC_Player::PK_DeviceTemplate_get_static(), sRouter_IP));
	...
	try
	{
		VLC_Player *pVLC_Player = new VLC_Player(PK_Device, sRouter_IP,true,bLocalMode);
		if ( pVLC_Player->GetConfig() && pVLC_Player->Connect(pVLC_Player->PK_DeviceTemplate_get()) ) 
		{
			g_pCommand_Impl=pVLC_Player;
			g_pDeadlockHandler=DeadlockHandler;
			g_pSocketCrashHandler=SocketCrashHandler;
			LoggerWrapper::GetInstance()->Write(LV_STATUS, "Connect OK");
			pVLC_Player->CreateChildren();
			if( bLocalMode )
				pVLC_Player->RunLocalMode();
			else
			{
				if(pVLC_Player->m_RequestHandlerThread)
					pthread_join(pVLC_Player->m_RequestHandlerThread, NULL);  // This function will return when the device is shutting down
			}
		...
		{
			bAppError = true;
			if( pVLC_Player->m_pEvent && pVLC_Player->m_pEvent->m_pClientSocket && pVLC_Player->m_pEvent->m_pClientSocket->m_eLastError==ClientSocket::cs_err_CannotConnect )
			{
				bAppError = false;
				bReload = false;
