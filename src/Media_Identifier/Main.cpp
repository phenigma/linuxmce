...
extern "C" {
	...
	{ 
		...
		if( bIsRuntimePlugin )
			return Media_Identifier::PK_DeviceTemplate_get_static();
		else
			return 0;
	}
}
...
extern "C" {
	class Command_Impl *RegisterAsPlugIn(class Router *pRouter,int PK_Device,Logger *pPlutoLogger)
	{
		...
		Media_Identifier *pMedia_Identifier = new Media_Identifier(PK_Device, "localhost",true,false,pRouter);
		if( pMedia_Identifier->m_bQuit_get()|| !pMedia_Identifier->GetConfig() )
		{
			delete pMedia_Identifier;
			return NULL;
		}
		else
		{
			g_pCommand_Impl=pMedia_Identifier;
			g_pDeadlockHandler=Plugin_DeadlockHandler;
			g_pSocketCrashHandler=Plugin_SocketCrashHandler;
		}
		return pMedia_Identifier;
	}
}
//<-dceag-plug-e->
...
int main(int argc, char* argv[]) 
{
	...
	cout << "Media_Identifier, v." << VERSION << endl
	...
	if (bError)
	{
		cout << "A Pluto DCE Device.  See www.plutohome.com/dce for details." << endl
			<< "Usage: Media_Identifier [-r Router's IP] [-d My Device ID] [-l dcerouter|stdout|null|filename]" << endl
	...
	try
	{
		if( sLogger=="dcerouter" )
			LoggerWrapper::SetInstance(new ServerLogger(PK_Device, Media_Identifier::PK_DeviceTemplate_get_static(), sRouter_IP));
	...
	try
	{
		Media_Identifier *pMedia_Identifier = new Media_Identifier(PK_Device, sRouter_IP,true,bLocalMode);
		if ( pMedia_Identifier->GetConfig() && pMedia_Identifier->Connect(pMedia_Identifier->PK_DeviceTemplate_get()) ) 
		{
			g_pCommand_Impl=pMedia_Identifier;
			g_pDeadlockHandler=DeadlockHandler;
			g_pSocketCrashHandler=SocketCrashHandler;
			LoggerWrapper::GetInstance()->Write(LV_STATUS, "Connect OK");
			pMedia_Identifier->CreateChildren();
			if( bLocalMode )
				pMedia_Identifier->RunLocalMode();
			else
			{
				if(pMedia_Identifier->m_RequestHandlerThread)
					pthread_join(pMedia_Identifier->m_RequestHandlerThread, NULL);  // This function will return when the device is shutting down
			}
		...
		{
			bAppError = true;
			if( pMedia_Identifier->m_pEvent && pMedia_Identifier->m_pEvent->m_pClientSocket && pMedia_Identifier->m_pEvent->m_pClientSocket->m_eLastError==ClientSocket::cs_err_CannotConnect )
			{
				bAppError = false;
				bReload = false;
		...
		if( pMedia_Identifier->m_bReload )
			bReload=true;
