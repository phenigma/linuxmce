...
int main(int argc, char* argv[]) 
{
	...
	try
	{
		if( sLogger=="dcerouter" )
			LoggerWrapper::SetInstance(new ServerLogger(PK_Device, VideoLan_PlugIn::PK_DeviceTemplate_get_static(), sRouter_IP));
	...
	try
	{
		VideoLan_PlugIn *pVideoLan_PlugIn = new VideoLan_PlugIn(PK_Device, sRouter_IP,true,bLocalMode);
		...
		{
			bAppError = true;
			if( pVideoLan_PlugIn->m_pEvent && pVideoLan_PlugIn->m_pEvent->m_pClientSocket && pVideoLan_PlugIn->m_pEvent->m_pClientSocket->m_eLastError==ClientSocket::cs_err_CannotConnect )
			{
				bAppError = false;
				bReload = false;
