...
*/
/** @file Main.cpp
 *@brief The X10 CTX35 main-line program.

Usage: CTX35 [-r Router's IP] [-d My Device ID] [-l dcerouter|stdout|null|filename]
...
extern "C" {
	class Command_Impl *RegisterAsPlugIn(class Router *pRouter,int PK_Device,Logger *pPlutoLogger)
	{
		...
		CTX35 *pCTX35 = new CTX35(PK_Device, "localhost",true,false,pRouter);
		if( pCTX35->m_bQuit_get()|| !pCTX35->GetConfig() )
		{
			delete pCTX35;
			return NULL;
		}
...
int main(int argc, char* argv[])
{
	...
	if (bError)
	{
		cout << "A Pluto DCE Device.  See www.plutohome.com/dce for details." << endl
			<< "Usage: CTX35 [-r Router's IP] [-d My Device ID] [-l dcerouter|stdout|null|filename]" << endl
	...
	try
	{
		if( sLogger=="dcerouter" )
			LoggerWrapper::SetInstance(new ServerLogger(PK_Device, CTX35::PK_DeviceTemplate_get_static(), sRouter_IP));
	...
	try
	{
		CTX35 *pCTX35 = new CTX35(PK_Device, sRouter_IP,true,bLocalMode);
		if ( pCTX35->GetConfig() && pCTX35->Connect(pCTX35->PK_DeviceTemplate_get()) )
		{
			g_pCommand_Impl=pCTX35;
			g_pDeadlockHandler=DeadlockHandler;
			g_pSocketCrashHandler=SocketCrashHandler;
			LoggerWrapper::GetInstance()->Write(LV_STATUS, "Connect OK");
			pCTX35->CreateChildren();
			if( bLocalMode )
				pCTX35->RunLocalMode();
			else
				pthread_join(pCTX35->m_RequestHandlerThread, NULL);  // This function will return when the device is shutting down
		...
		{
			bAppError = true;
			if( pCTX35->m_pEvent && pCTX35->m_pEvent->m_pClientSocket && pCTX35->m_pEvent->m_pClientSocket->m_eLastError==ClientSocket::cs_err_CannotConnect )
			{
				bAppError = false;
				bReload = false;
