...
#define CTX35_PORT					"/dev/ttyS0"
#define CTX35_BPS					4800

#define CTX35_ERROR_SLEEP			5100
#define CTX35_NOREQUES_SLEEP		100 

#define CTX35_SEND_RETRY			5
#define CTX35_READ_TIMEOUT			15000 
#define CTX35_FUNC_TIMEOUT			30000
#define CTX35_MAX_DIM_LEVEL			22
			...
			unsigned char highByte, unsigned char lowByte)
{
	...
	if(pport->Read((char*)&resp, 1, CTX35_READ_TIMEOUT) == 0)
	{
		LoggerWrapper::GetInstance()->Write(LV_STATUS, "No response from CTX35 device.");
		return -1;
	}
	else
	{
		LoggerWrapper::GetInstance()->Write(LV_STATUS, "Got response: %x from CTX35.", resp);
		if(resp == CTX35_CLOCK_REQ && chksum != CTX35_CLOCK_REQ)
		{
			LoggerWrapper::GetInstance()->Write(LV_STATUS, "CTX35 requested clock SET UP", resp);
			SendClock(pport);
			return -1;
		}
		else
		{
			if(resp == CTX35_INTERFACE_CQ && chksum != CTX35_INTERFACE_CQ)
			{
	...
	sendbuff[0] = CTX35_ACK;
	pport->Write((char*)sendbuff, 1);
	if(pport->Read((char*)&resp, 1, CTX35_READ_TIMEOUT) == 0) {
		LoggerWrapper::GetInstance()->Write(LV_CRITICAL, "No response from CTX35 device.");
		return -1;
	} else {
		LoggerWrapper::GetInstance()->Write(LV_STATUS, "Got response: %x from CTX35.", resp);
	
		if(resp != CTX35_INTERFACE_READY) {
...
DevicePoll::SendClock(CSerialPort* pport) {
	...
	sbuff[0] = CTX35_SET_TIME;             /* CTX35 timer download code */
	sbuff[1] = tp->tm_sec ;
...
DevicePoll::SendAddress(CSerialPort* pport, const Message* pMesg) {
	...
	return SendPacket(pport,/*high*/CTX35_STANDARD_ADDRESS, /*low*/pMesg->getHouseCode() << 4 | pMesg->getDeviceCode());
}
...
DevicePoll::SendFunction(CSerialPort* pport, const Message* pMesg) {
	...
	unsigned char dim = (unsigned char)(pMesg->getDimmLevel() * CTX35_MAX_DIM_LEVEL * 1.0 / 100);
	if(dim > 0) {
		LoggerWrapper::GetInstance()->Write(LV_STATUS, "Using Dim Level: %d.", dim);
	}
	
	return SendPacket(pport,/*high*/(dim << 3) | CTX35_STANDARD_FUNCTION,/*low*/pMesg->getHouseCode() << 4 | pMesg->getFunctionCode());
...
DevicePoll::ReadIncoming(CSerialPort* pport) {
	...
	unsigned char resp;
	LoggerWrapper::GetInstance()->Write(LV_STATUS, "Processing incoming data from the CTX35.");
	if(pport->Read((char*)&resp, 1, CTX35_READ_TIMEOUT) != 0)
	{
		LoggerWrapper::GetInstance()->Write(LV_STATUS, "Got response: %x from CTX35.", resp);
		if(resp == CTX35_INTERFACE_CQ) {
			resp = CTX35_COMPUTER_READY;
			pport->Write((char*)&resp, 1);
			int nread = 0;
			do {
				nread = pport->Read((char*)&resp, 1, CTX35_READ_TIMEOUT);
			} while (nread > 0 && resp == CTX35_INTERFACE_CQ);
			if(nread > 0) {
				if(resp > 0 && resp < 128) {
					LoggerWrapper::GetInstance()->Write(LV_STATUS, "Reading %d bytes of DATA...",resp);

					char *buff = new char[resp];
					pport->Read(buff, resp, CTX35_READ_TIMEOUT);
					char mask=buff[0];
					string recv_msg="";
					char hex_buff[128];
					hex_buff[0]=0;
					...
					for (i=1;i<resp;i++) {
						...
						{ //an address
							...
							recv_msg+=addr;
							//get timestamp for computing the CTX35_DIM_TIMEOUT
							gettimeofday(&timestampLast, NULL);	
							LoggerWrapper::GetInstance()->Write(LV_STATUS, "Address received. Resetting CTX35_FUNC_TIMEOUT timer.");						
						}
						else if((timestampNow.tv_sec - timestampLast.tv_sec)*1000 + (timestampNow.tv_usec - timestampLast.tv_usec)/1000 < CTX35_FUNC_TIMEOUT)
						{ 
							//This is a function and the CTX35_FUNC_TIMEOUT has not been reached
							...
							switch(func) {
								case CTX35_FUNC_ALL_U_OFF : message+=" ALL UNITS OFF"; break;
								case CTX35_FUNC_ALL_L_ON  : message+=" ALL LIGHTS ON"; break;
								case CTX35_FUNC_ON        : message+=" ON"; break;
								case CTX35_FUNC_OFF       : message+=" OFF"; break;
								case CTX35_FUNC_DIM       : 
									message+=" DIM"; 
									i++;
									data=(buff[i]*100)/210; 
									break;
								case CTX35_FUNC_BRIGHT    : 
									message+=" BRIGHT"; 
									i++;
									data=(buff[i]*100)/210; 
									break;
								case CTX35_FUNC_ALL_L_OFF : message+=" ALL LIGHTS OFF"; break;
								case CTX35_FUNC_EXT       : message+=" EXTENDED"; break;
								case CTX35_FUNC_HREQ      : message+=" HAIL REQUEST"; break;
								case CTX35_FUNC_HACK      : message+=" HAIL ACK"; break;
								case CTX35_FUNC_PSD1      : message+=" PRESET DIM 1"; break;
								case CTX35_FUNC_PSD2      : message+=" PRESET DIM 2"; break;
								case CTX35_FUNC_EXTDT     : message+=" EXTENDED DATA"; break;
								case CTX35_FUNC_STATON    : message+=" STATUS ON"; break;
								case CTX35_FUNC_STATOFF   : message+=" STATUS OFF"; break;
								case CTX35_FUNC_STATREQ   : message+=" STATUS REQUEST"; break;
							}
							for(std::list<std::string>::iterator it = used_addresses.begin(); it != used_addresses.end(); it++)
							{
								...
								if (child->m_dwPK_DeviceCategory == DEVICECATEGORY_Lighting_Device_CONST) {//device_status.find(*it) != device_status.end()) {
									...
									if(child->m_dwPK_DeviceCategory == DEVICECATEGORY_Security_Device_CONST)
									{
										...
										pMessageOut->m_mapParameters[EVENTPARAMETER_Tripped_CONST]=(func==CTX35_FUNC_ON?"1":"0");
										real_cm11a->m_pEvent->SendMessage(pMessageOut);
									} else {
							...
							//CTX35_FUNC_TIMEOUT has been reached. The timeout timer will be resed on the next address received.
...
void* DevicePoll::_Run() {
	...
	while(!isStopRequested()) {// will be changed with thread end request
		try {
			CSerialPort serprt(serport_.c_str(), CTX35_BPS, epbsN81);

			// get the msg from queue
			mq_.Lock();
			if(msgqueue_.size() > 0) {
				LoggerWrapper::GetInstance()->Write(LV_STATUS, "We have data to send for CTX35.");
				Message msg = msgqueue_.front();
				msgqueue_.pop_front();
				mq_.Unlock();

				int sendretry = 0;
				for(; sendretry < CTX35_SEND_RETRY; sendretry++) {
				...
				if(sendretry < CTX35_SEND_RETRY) { // adress sent seccessfully
				...
				Sleep(CTX35_NOREQUES_SLEEP);
			}
		} catch(...) {
			LoggerWrapper::GetInstance()->Write(LV_CRITICAL, "Exception occured on CTX35's serial port: %s. Sleeping for %0.1f second.", 
													serport_.c_str(), 1.0 * CTX35_ERROR_SLEEP / 1000);
			Sleep(CTX35_ERROR_SLEEP);
		}
	}
