...
extern "C" {
	...
	{ 
		...
		if( bIsRuntimePlugin )
			return Game_PlugIn::PK_DeviceTemplate_get_static();
		else
			return 0;
	}
}
...
extern "C" {
	class Command_Impl *RegisterAsPlugIn(class Router *pRouter,int PK_Device,Logger *pPlutoLogger)
	{
		...
		Game_PlugIn *pGame_PlugIn = new Game_PlugIn(PK_Device, "localhost",true,false,pRouter);
		if( pGame_PlugIn->m_bQuit_get()|| !pGame_PlugIn->GetConfig() )
		{
			delete pGame_PlugIn;
			return NULL;
		}
...
int main(int argc, char* argv[]) 
{
	...
	cout << "Game_PlugIn, v." << VERSION << endl
	...
	if (bError)
	{
		cout << "A Pluto DCE Device.  See www.plutohome.com/dce for details." << endl
			<< "Usage: Game_PlugIn [-r Router's IP] [-d My Device ID] [-l dcerouter|stdout|null|filename]" << endl
	...
	try
	{
		if( sLogger=="dcerouter" )
			LoggerWrapper::SetInstance(new ServerLogger(PK_Device, Game_PlugIn::PK_DeviceTemplate_get_static(), sRouter_IP));
	...
	try
	{
		Game_PlugIn *pGame_PlugIn = new Game_PlugIn(PK_Device, sRouter_IP,true,bLocalMode);
		if ( pGame_PlugIn->GetConfig() && pGame_PlugIn->Connect(pGame_PlugIn->PK_DeviceTemplate_get()) ) 
		{
			g_pCommand_Impl=pGame_PlugIn;
			g_pDeadlockHandler=DeadlockHandler;
			g_pSocketCrashHandler=SocketCrashHandler;
			LoggerWrapper::GetInstance()->Write(LV_STATUS, "Connect OK");
			pGame_PlugIn->CreateChildren();
			if( bLocalMode )
				pGame_PlugIn->RunLocalMode();
			else
			{
				if(pGame_PlugIn->m_RequestHandlerThread)
					pthread_join(pGame_PlugIn->m_RequestHandlerThread, NULL);  // This function will return when the device is shutting down
			}
		...
		{
			bAppError = true;
			if( pGame_PlugIn->m_pEvent && pGame_PlugIn->m_pEvent->m_pClientSocket && pGame_PlugIn->m_pEvent->m_pClientSocket->m_eLastError==ClientSocket::cs_err_CannotConnect )
			{
				bAppError = false;
				bReload = false;
