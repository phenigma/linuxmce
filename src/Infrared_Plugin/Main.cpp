...
int main(int argc, char* argv[]) 
{
	...
	try
	{
		if( sLogger=="dcerouter" )
			LoggerWrapper::SetInstance(new ServerLogger(PK_Device, Infrared_Plugin::PK_DeviceTemplate_get_static(), sRouter_IP));
	...
	try
	{
		...
		if ( pInfrared_Plugin->GetConfig() && pInfrared_Plugin->Connect(pInfrared_Plugin->PK_DeviceTemplate_get()) ) 
		{
			...
			{
				if(pInfrared_Plugin->m_RequestHandlerThread)
		...
		{
			bAppError = true;
			if( pInfrared_Plugin->m_pEvent && pInfrared_Plugin->m_pEvent->m_pClientSocket && pInfrared_Plugin->m_pEvent->m_pClientSocket->m_eLastError==ClientSocket::cs_err_CannotConnect )
			{
				bAppError = false;
				bReload = false;
