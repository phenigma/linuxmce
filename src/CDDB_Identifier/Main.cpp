...
extern "C" {
	...
	{ 
		...
		if( bIsRuntimePlugin )
			return CDDB_Identifier::PK_DeviceTemplate_get_static();
		else
			return 0;
	}
}
...
int main(int argc, char* argv[]) 
{
	...
	cout << "CDDB_Identifier, v." << VERSION << endl
	...
	if (bError)
	{
		cout << "A Pluto DCE Device.  See www.plutohome.com/dce for details." << endl
			<< "Usage: CDDB_Identifier [-r Router's IP] [-d My Device ID] [-l dcerouter|stdout|null|filename]" << endl
	...
	try
	{
		if( sLogger=="dcerouter" )
			LoggerWrapper::SetInstance(new ServerLogger(PK_Device, CDDB_Identifier::PK_DeviceTemplate_get_static(), sRouter_IP));
	...
	try
	{
		...
		if ( pCDDB_Identifier->GetConfig() && pCDDB_Identifier->Connect(pCDDB_Identifier->PK_DeviceTemplate_get()) ) 
		{
			...
			pCDDB_Identifier->CreateChildren();
			if( bLocalMode )
