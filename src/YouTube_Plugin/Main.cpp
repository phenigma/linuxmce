...
extern "C" {
	...
	{ 
		...
		if( bIsRuntimePlugin )
			return YouTube_Plugin::PK_DeviceTemplate_get_static();
		else
			return 0;
	}
}
...
extern "C" {
	class Command_Impl *RegisterAsPlugIn(class Router *pRouter,int PK_Device,Logger *pPlutoLogger)
	{
		...
		YouTube_Plugin *pYouTube_Plugin = new YouTube_Plugin(PK_Device, "localhost",true,false,pRouter);
		if( pYouTube_Plugin->m_bQuit_get()|| !pYouTube_Plugin->GetConfig() )
		{
			delete pYouTube_Plugin;
			return NULL;
		}
		else
		{
			g_pCommand_Impl=pYouTube_Plugin;
			g_pDeadlockHandler=Plugin_DeadlockHandler;
			g_pSocketCrashHandler=Plugin_SocketCrashHandler;
		}
		return pYouTube_Plugin;
	}
}
//<-dceag-plug-e->
...
int main(int argc, char* argv[]) 
{
	...
	cout << "YouTube_Plugin, v." << VERSION << endl
	...
	if (bError)
	{
		cout << "A Pluto DCE Device.  See www.plutohome.com/dce for details." << endl
			<< "Usage: YouTube_Plugin [-r Router's IP] [-d My Device ID] [-l dcerouter|stdout|null|filename]" << endl
	...
	try
	{
		if( sLogger=="dcerouter" )
			LoggerWrapper::SetInstance(new ServerLogger(PK_Device, YouTube_Plugin::PK_DeviceTemplate_get_static(), sRouter_IP));
	...
	try
	{
		YouTube_Plugin *pYouTube_Plugin = new YouTube_Plugin(PK_Device, sRouter_IP,true,bLocalMode);
		if ( pYouTube_Plugin->GetConfig() && pYouTube_Plugin->Connect(pYouTube_Plugin->PK_DeviceTemplate_get()) ) 
		{
			g_pCommand_Impl=pYouTube_Plugin;
			g_pDeadlockHandler=DeadlockHandler;
			g_pSocketCrashHandler=SocketCrashHandler;
			LoggerWrapper::GetInstance()->Write(LV_STATUS, "Connect OK");
			pYouTube_Plugin->CreateChildren();
			if( bLocalMode )
				pYouTube_Plugin->RunLocalMode();
			else
			{
				if(pYouTube_Plugin->m_RequestHandlerThread)
					pthread_join(pYouTube_Plugin->m_RequestHandlerThread, NULL);  // This function will return when the device is shutting down
			}
		...
		{
			bAppError = true;
			if( pYouTube_Plugin->m_pEvent && pYouTube_Plugin->m_pEvent->m_pClientSocket && pYouTube_Plugin->m_pEvent->m_pClientSocket->m_eLastError==ClientSocket::cs_err_CannotConnect )
			{
				bAppError = false;
				bReload = false;
