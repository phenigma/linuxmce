...
extern "C" {
	...
	{ 
		...
		if( bIsRuntimePlugin )
			return Tongdy_RS485::PK_DeviceTemplate_get_static();
		else
			return 0;
	}
}
...
extern "C" {
	class Command_Impl *RegisterAsPlugIn(class Router *pRouter,int PK_Device,Logger *pPlutoLogger)
	{
		...
		Tongdy_RS485 *pTongdy_RS485 = new Tongdy_RS485(PK_Device, "localhost",true,false,pRouter);
		if( pTongdy_RS485->m_bQuit_get()|| !pTongdy_RS485->GetConfig() )
		{
			delete pTongdy_RS485;
			return NULL;
		}
		else
		{
			g_pCommand_Impl=pTongdy_RS485;
			g_pDeadlockHandler=Plugin_DeadlockHandler;
			g_pSocketCrashHandler=Plugin_SocketCrashHandler;
		}
		return pTongdy_RS485;
	}
}
//<-dceag-plug-e->
...
int main(int argc, char* argv[]) 
{
	...
	cout << "Tongdy_RS485, v." << VERSION << endl
	...
	if (bError)
	{
		cout << "A Pluto DCE Device.  See www.plutohome.com/dce for details." << endl
			<< "Usage: Tongdy_RS485 [-r Router's IP] [-d My Device ID] [-l dcerouter|stdout|null|filename]" << endl
	...
	try
	{
		if( sLogger=="dcerouter" )
			LoggerWrapper::SetInstance(new ServerLogger(PK_Device, Tongdy_RS485::PK_DeviceTemplate_get_static(), sRouter_IP));
	...
	try
	{
		Tongdy_RS485 *pTongdy_RS485 = new Tongdy_RS485(PK_Device, sRouter_IP,true,bLocalMode);
		if ( pTongdy_RS485->GetConfig() && pTongdy_RS485->Connect(pTongdy_RS485->PK_DeviceTemplate_get()) ) 
		{
			g_pCommand_Impl=pTongdy_RS485;
			g_pDeadlockHandler=DeadlockHandler;
			g_pSocketCrashHandler=SocketCrashHandler;
			LoggerWrapper::GetInstance()->Write(LV_STATUS, "Connect OK");
			pTongdy_RS485->CreateChildren();
			if( bLocalMode )
				pTongdy_RS485->RunLocalMode();
			else
			{
				if(pTongdy_RS485->m_RequestHandlerThread)
					pthread_join(pTongdy_RS485->m_RequestHandlerThread, NULL);  // This function will return when the device is shutting down
			}
		...
		{
			bAppError = true;
			if( pTongdy_RS485->m_pEvent && pTongdy_RS485->m_pEvent->m_pClientSocket && pTongdy_RS485->m_pEvent->m_pClientSocket->m_eLastError==ClientSocket::cs_err_CannotConnect )
			{
				bAppError = false;
				bReload = false;
