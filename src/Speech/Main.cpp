...
int main(int argc, char* argv[]) 
{
	...
	try
	{
		if( sLogger=="dcerouter" )
			LoggerWrapper::SetInstance(new ServerLogger(PK_Device, Speech::PK_DeviceTemplate_get_static(), sRouter_IP));
	...
	try
	{
		...
		if ( pSpeech->GetConfig() && pSpeech->Connect(pSpeech->PK_DeviceTemplate_get()) ) 
		{
			g_pCommand_Impl=pSpeech;
		...
		{
			bAppError = true;
			if( pSpeech->m_pEvent && pSpeech->m_pEvent->m_pClientSocket && pSpeech->m_pEvent->m_pClientSocket->m_eLastError==ClientSocket::cs_err_CannotConnect )
			{
				bAppError = false;
				bReload = false;
