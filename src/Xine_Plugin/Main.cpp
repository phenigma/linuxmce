...
int main(int argc, char* argv[]) 
{
	...
	try
	{
		if( sLogger=="dcerouter" )
			LoggerWrapper::SetInstance(new ServerLogger(PK_Device, Xine_Plugin::PK_DeviceTemplate_get_static(), sRouter_IP));
	...
	try
	{
		Xine_Plugin *pXine_Plugin = new Xine_Plugin(PK_Device, sRouter_IP,true,bLocalMode);
		if ( pXine_Plugin->GetConfig() && pXine_Plugin->Connect(pXine_Plugin->PK_DeviceTemplate_get()) ) 
		{
			...
			if( bLocalMode )
				pXine_Plugin->RunLocalMode();
			else
			{
				if(pXine_Plugin->m_RequestHandlerThread)
					pthread_join(pXine_Plugin->m_RequestHandlerThread, NULL);  // This function will return when the device is shutting down
			}
		...
		{
			bAppError = true;
			if( pXine_Plugin->m_pEvent && pXine_Plugin->m_pEvent->m_pClientSocket && pXine_Plugin->m_pEvent->m_pClientSocket->m_eLastError==ClientSocket::cs_err_CannotConnect )
			{
				bAppError = false;
				bReload = false;
