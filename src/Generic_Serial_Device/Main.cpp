...
extern "C" {
	...
	{ 
		...
		if( bIsRuntimePlugin )
			return Generic_Serial_Device::PK_DeviceTemplate_get_static();
		else
			return 0;
	}
}
...
extern "C" {
	class Command_Impl *RegisterAsPlugIn(class Router *pRouter,int PK_Device,string sLogger)
	{
		...
		{
			g_pPlutoLogger = new ServerLogger(PK_Device, Generic_Serial_Device::PK_DeviceTemplate_get_static(), "localhost");
...
int main(int argc, char* argv[]) 
{
	...
	cout << "Generic_Serial_Device, v." << VERSION << endl
	...
	if (bError)
	{
		cout << "A Pluto DCE Device.  See www.plutohome.com/dce for details." << endl
			<< "Usage: Generic_Serial_Device [-r Router's IP] [-d My Device ID] [-l dcerouter|stdout|null|filename]" << endl
