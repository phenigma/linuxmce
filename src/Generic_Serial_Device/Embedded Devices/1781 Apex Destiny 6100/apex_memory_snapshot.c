...
char* get_wrapped_command_string( char* command_string );

char* get_location_content_command_string( int byte_count, int offset );

int store_mem_contents_to_file( char* filename );



int main( )
{
	char* file_before = "apex_mem_before";
	char* file_after = "apex_mem_after";

	store_mem_contents_to_file( file_before );
	
	printf( "\n\nPress any key after the panel's memmory has been changed to aquire the \"after\" snapshot\n\n" );
	getchar();
...
int store_mem_contents_to_file( char* filename )
{
	char* command_start = "/usr/pluto/bin/TestSerialPort -p /dev/ttyUSB3 -P N81 -b 1200 -t \"";
	char* command_end = "\\r\\n\" -i 1";
	...
	while ( read_count != max_read_count )
	{
		...
		sprintf( command, "%s%s%s | awk '{print substr($0,7,68)}' >> %s", 
				command_start, 
