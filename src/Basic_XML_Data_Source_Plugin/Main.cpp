...
extern "C" {
	...
	{ 
		...
		if( bIsRuntimePlugin )
			return Basic_XML_Data_Source_Plugin::PK_DeviceTemplate_get_static();
		else
			return 0;
	}
}
...
extern "C" {
	class Command_Impl *RegisterAsPlugIn(class Router *pRouter,int PK_Device,Logger *pPlutoLogger)
	{
		...
		Basic_XML_Data_Source_Plugin *pBasic_XML_Data_Source_Plugin = new Basic_XML_Data_Source_Plugin(PK_Device, "localhost",true,false,pRouter);
		if( pBasic_XML_Data_Source_Plugin->m_bQuit_get()|| !pBasic_XML_Data_Source_Plugin->GetConfig() )
		{
			delete pBasic_XML_Data_Source_Plugin;
			return NULL;
		}
		else
		{
			g_pCommand_Impl=pBasic_XML_Data_Source_Plugin;
			g_pDeadlockHandler=Plugin_DeadlockHandler;
			g_pSocketCrashHandler=Plugin_SocketCrashHandler;
		}
		return pBasic_XML_Data_Source_Plugin;
	}
}
//<-dceag-plug-e->
...
int main(int argc, char* argv[]) 
{
	...
	cout << "Basic_XML_Data_Source_Plugin, v." << VERSION << endl
	...
	if (bError)
	{
		cout << "A Pluto DCE Device.  See www.plutohome.com/dce for details." << endl
			<< "Usage: Basic_XML_Data_Source_Plugin [-r Router's IP] [-d My Device ID] [-l dcerouter|stdout|null|filename]" << endl
	...
	try
	{
		if( sLogger=="dcerouter" )
			LoggerWrapper::SetInstance(new ServerLogger(PK_Device, Basic_XML_Data_Source_Plugin::PK_DeviceTemplate_get_static(), sRouter_IP));
	...
	try
	{
		Basic_XML_Data_Source_Plugin *pBasic_XML_Data_Source_Plugin = new Basic_XML_Data_Source_Plugin(PK_Device, sRouter_IP,true,bLocalMode);
		if ( pBasic_XML_Data_Source_Plugin->GetConfig() && pBasic_XML_Data_Source_Plugin->Connect(pBasic_XML_Data_Source_Plugin->PK_DeviceTemplate_get()) ) 
		{
			g_pCommand_Impl=pBasic_XML_Data_Source_Plugin;
			g_pDeadlockHandler=DeadlockHandler;
			g_pSocketCrashHandler=SocketCrashHandler;
			LoggerWrapper::GetInstance()->Write(LV_STATUS, "Connect OK");
			pBasic_XML_Data_Source_Plugin->CreateChildren();
			if( bLocalMode )
				pBasic_XML_Data_Source_Plugin->RunLocalMode();
			else
			{
				if(pBasic_XML_Data_Source_Plugin->m_RequestHandlerThread)
					pthread_join(pBasic_XML_Data_Source_Plugin->m_RequestHandlerThread, NULL);  // This function will return when the device is shutting down
			}
		...
		{
			bAppError = true;
			if( pBasic_XML_Data_Source_Plugin->m_pEvent && pBasic_XML_Data_Source_Plugin->m_pEvent->m_pClientSocket && pBasic_XML_Data_Source_Plugin->m_pEvent->m_pClientSocket->m_eLastError==ClientSocket::cs_err_CannotConnect )
			{
				bAppError = false;
				bReload = false;
		...
		if( pBasic_XML_Data_Source_Plugin->m_bReload )
			bReload=true;
