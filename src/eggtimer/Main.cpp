...
extern "C" {
	...
	{ 
		...
		if( bIsRuntimePlugin )
			return eggtimer::PK_DeviceTemplate_get_static();
		else
			return 0;
	}
}
...
extern "C" {
	class Command_Impl *RegisterAsPlugIn(class Router *pRouter,int PK_Device,Logger *pPlutoLogger)
	{
		...
		eggtimer *peggtimer = new eggtimer(PK_Device, "localhost",true,false,pRouter);
		if( peggtimer->m_bQuit_get()|| !peggtimer->GetConfig() )
		{
			delete peggtimer;
			return NULL;
		}
...
int main(int argc, char* argv[]) 
{
	...
	if (bError)
	{
		cout << "A Pluto DCE Device.  See www.plutohome.com/dce for details." << endl
			<< "Usage: eggtimer [-r Router's IP] [-d My Device ID] [-l dcerouter|stdout|null|filename]" << endl
	...
	try
	{
		if( sLogger=="dcerouter" )
			LoggerWrapper::SetInstance(new ServerLogger(PK_Device, eggtimer::PK_DeviceTemplate_get_static(), sRouter_IP));
	...
	try
	{
		eggtimer *peggtimer = new eggtimer(PK_Device, sRouter_IP,true,bLocalMode);
		if ( peggtimer->GetConfig() && peggtimer->Connect(peggtimer->PK_DeviceTemplate_get()) ) 
		{
			g_pCommand_Impl=peggtimer;
			g_pDeadlockHandler=DeadlockHandler;
			g_pSocketCrashHandler=SocketCrashHandler;
			LoggerWrapper::GetInstance()->Write(LV_STATUS, "Connect OK");
			peggtimer->CreateChildren();
			if( bLocalMode )
				peggtimer->RunLocalMode();
			else
			{
				if(peggtimer->m_RequestHandlerThread)
					pthread_join(peggtimer->m_RequestHandlerThread, NULL);  // This function will return when the device is shutting down
			}
		...
		{
			bAppError = true;
			if( peggtimer->m_pEvent && peggtimer->m_pEvent->m_pClientSocket && peggtimer->m_pEvent->m_pClientSocket->m_eLastError==ClientSocket::cs_err_CannotConnect )
			{
				bAppError = false;
				bReload = false;
