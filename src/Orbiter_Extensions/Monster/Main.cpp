...
extern "C" {
	...
	{ 
		...
		if( bIsRuntimePlugin )
			return MonsterScreenHandler::PK_DeviceTemplate_get_static();
		else
			return 0;
	}
}
...
extern "C" {
	class ScreenHandler *RegisterAsOrbiterExtension(class Orbiter *pOrbiter, map<int,int> *p_MapDesignObj, Logger *pPlutoLogger)
	{
		g_pPlutoLogger = pPlutoLogger;
		g_pPlutoLogger->Write(LV_STATUS, "Orbiter Monster extension started!");

		MonsterScreenHandler *pScreenHandler = new MonsterScreenHandler(pOrbiter, p_MapDesignObj);
...
int main(int argc, char* argv[]) 
{
	...
	try
	{
		Security_Plugin *pSecurity_Plugin = new Security_Plugin(PK_Device, sRouter_IP,true,bLocalMode);
		if ( pSecurity_Plugin->GetConfig() && pSecurity_Plugin->Connect(pSecurity_Plugin->PK_DeviceTemplate_get()) ) 
		{
			...
			if( bLocalMode )
				pSecurity_Plugin->RunLocalMode();
			else
				pthread_join(pSecurity_Plugin->m_RequestHandlerThread, NULL);  // This function will return when the device is shutting down
