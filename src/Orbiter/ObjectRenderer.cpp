...
/*virtual*/ bool ObjectRenderer::PostRenderActions(DesignObj_Orbiter *pObj_Screen, PlutoPoint point)
{
	...
	if(m_pObj_Owner->m_pFloorplanObject && m_pObj_Owner->m_pOrbiter->m_mapDevice_Selected.find(m_pObj_Owner->m_pFloorplanObject->PK_Device) != m_pObj_Owner->m_pOrbiter->m_mapDevice_Selected.end() )
	{
		...
		m_pObj_Owner->m_pOrbiter->Renderer()->UpdateRect(rect, NULL != m_pObj_Owner->m_pOrbiter->GetActivePopup() ? m_pObj_Owner->m_pOrbiter->GetActivePopup()->m_Position : PlutoPoint(0, 0));
	}
