...
#define ORBITER_SCHEMA		29	// Used to determine if cached screens can be used or if the schema has changed
...
public:
	...
	void SetupSerialization(int iSC_Version)
	{
		StartSerializeList() + m_mapVariable + m_Width + m_Height + m_rSpacing + m_AnimationStyle + m_sInitialScreen + m_sMainMenu + m_sSleepingMenu + m_sScreenSaveMenu + m_dwPK_Users_Default + m_iLocation_Initial + m_iUiVersion + m_sSkin
			...
			+ m_dwPK_Device_LocalOsdIRReceiver + m_dwPK_Device_LocalOsdVfdLcd + m_bIsOSD + m_bNewOrbiter + m_bUseAlphaBlending + m_bUseMask + m_mapDesignObj + m_mapPK_Screen_GoBackToScreen;
