...
{
	if( m_pMouseBehavior->m_pOrbiter->m_bPK_Device_NowPlaying_Audio_DiscreteVolume && m_pMouseBehavior->m_iTime_Last_Mouse_Up )
	{
...
{
	int NotchWidth = m_bTapAndRelease ? m_pObj->m_rPosition.Width/100 : m_pObj->m_rPosition.Width/15; // Allow for 7 repeat levels in each direction
