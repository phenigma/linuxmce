...
int ScreenHandler::GetCurrentScreen_PK_DesignObj()
{
	...
	if( !m_pOrbiter->m_pScreenHistory_Current || (pObj=m_pOrbiter->m_pScreenHistory_Current->GetObj())==NULL )
		return 0;

	return pObj->m_iBaseObjectID;
}
//-----------------------------------------------------------------------------------------------------
...
{
	string sFilter;
	int NumLines=0; // We only have 2 lines
	for( list< MediaFileAttributeDrillDown * >::iterator it=m_listMediaFileAttributeDrillDown.begin();it!=m_listMediaFileAttributeDrillDown.end();++it )
	{
		if( sFilter.empty()==false )
			sFilter += "\n";

		MediaFileAttributeDrillDown *pMediaFileAttributeDrillDown = *it;
		...
		NumLines++;
		if( NumLines>=2 )  // Can only fit 2 anyway
			break; 
	}
	bool bHide = NumLines==0;
...
void ScreenHandler::SCREEN_FileList_PlayLists(long PK_Screen)
{
	...
	if(m_pOrbiter->m_sSkin == AUDIO_STATION_SKIN)
	{
		...
		RegisterCallBack(cbObjectSelected, (ScreenHandlerCallBack) &ScreenHandler::FileList_PlayLists_ObjectSelected,	new ObjectInfoBackData());
		RegisterCallBack(cbDataGridRendering, (ScreenHandlerCallBack) &ScreenHandler::FileList_PlayLists_GridRendering,	new DatagridAcquiredBackData());
		RegisterCallBack(cbDataGridSelected, (ScreenHandlerCallBack) &ScreenHandler::FileList_PlayLists_DatagridSelected,	new DatagridCellBackData());
		SetAudioServerTabs(5556);
	}
}
//-----------------------------------------------------------------------------------------------------
bool ScreenHandler::FileList_PlayLists_ObjectSelected(CallBackData *pData)
{
	ObjectInfoBackData *pObjectInfoData = (ObjectInfoBackData *)pData;

	PLUTO_SAFETY_LOCK(vm, m_pOrbiter->m_ScreenMutex);
	if( pObjectInfoData->m_pObj->m_iBaseObjectID==5598 || pObjectInfoData->m_pObj->m_iBaseObjectID==5652 )  // play it or delete it
	{
		PLUTO_SAFETY_LOCK(vm, m_pOrbiter->m_VariableMutex);
		DesignObj_DataGrid *pObj = (DesignObj_DataGrid *) m_pOrbiter->FindObject( TOSTRING(5556) ".0.0." TOSTRING(5648) );

		int position = pObj->m_GridCurRow + pObj->m_iHighlightedRow_get();
		DataGridTable *pDataGridTable = pObj->m_pDataGridTable_Current_get();
		DataGridCell *pCell = pDataGridTable ? pDataGridTable->GetData(0,position) : NULL;

		if( pCell )
		{
			if( pObjectInfoData->m_pObj->m_iBaseObjectID==5652 )
			{
				string sLocationInfo = pObjectInfoData->m_pObj->GetVariableAssignment(VARIABLE_Array_ID_CONST);

				// It's 3 numbers with comma separatation and the last is the entertainment area
				string::size_type pos_comma1 = sLocationInfo.find(',');
				string::size_type pos_comma2 = pos_comma1!=string::npos ? sLocationInfo.find(',',pos_comma1+1) : string::npos;

				DCE::CMD_Load_Playlist CMD_Load_Playlist(m_pOrbiter->m_dwPK_Device,m_pOrbiter->m_dwPK_Device_MediaPlugIn,pos_comma2!=string::npos ? sLocationInfo.substr(pos_comma2+1) : "",atoi(pCell->GetValue()));
				m_pOrbiter->SendCommand(CMD_Load_Playlist);
			}
			else
			{
				DCE::CMD_Remove_playlist CMD_Remove_playlist(m_pOrbiter->m_dwPK_Device,m_pOrbiter->m_dwPK_Device_MediaPlugIn,atoi(pCell->GetValue()));
				m_pOrbiter->SendCommand(CMD_Remove_playlist);
			}
...
//-----------------------------------------------------------------------------------------------------
bool ScreenHandler::FileList_PlayLists_GridRendering(CallBackData *pData)
{
	...
	DesignObj_Orbiter *pObj_SelectedObjBar = m_pOrbiter->FindObject(TOSTRING(5556) ".0.0." TOSTRING(5598));  // the delete button
	...
	pObj_SelectedObjBar = m_pOrbiter->FindObject(TOSTRING(5556) ".0.0." TOSTRING(5653));  // the zones array
	if( pObj_SelectedObjBar && pObj_SelectedObjBar->m_bHidden!=bHidden )
		m_pOrbiter->CMD_Show_Object( pObj_SelectedObjBar->m_ObjectID,0,"","", bHidden ? "0" : "1" );
...
//-----------------------------------------------------------------------------------------------------
bool ScreenHandler::FileList_PlayLists_DatagridSelected(CallBackData *pData)
{
...
bool ScreenHandler::MediaBrowser_ObjectSelected(CallBackData *pData)
{
	...
	else if( pObjectInfoData->m_PK_DesignObj_SelectedObject == 5656 )
	{
	...
	else if( pObjectInfoData->m_PK_DesignObj_SelectedObject == 5652 )
	{
		string sLocationInfo = pObjectInfoData->m_pObj->GetVariableAssignment(VARIABLE_Array_ID_CONST);
		...
		string::size_type pos_comma2 = pos_comma1!=string::npos ? sLocationInfo.find(',',pos_comma1+1) : string::npos;
		DCE::CMD_MH_Play_Media CMD_MH_Play_Media(m_pOrbiter->m_dwPK_Device,m_pOrbiter->m_dwPK_Device_MediaPlugIn,
			0,mediaFileBrowserOptions.m_sSelectedFile,0,0,pos_comma2!=string::npos ? sLocationInfo.substr(pos_comma2+1) : "",false,0,m_pOrbiter->DATA_Get_Queue_Instead_of_Instant_Play(),false,false);
		m_pOrbiter->SendCommand(CMD_MH_Play_Media);
	}
...
/*
	...
	else if( pObjectInfoData->m_PK_DesignObj_SelectedObject == DESIGNOBJ_butFBSF_Delete_CONST || pObjectInfoData->m_PK_DesignObj_SelectedObject == 5657 )
	{
	...
	else if( (pObjectInfoData->m_PK_DesignObj_SelectedObject == DESIGNOBJ_butFileBrowserBack_CONST || pObjectInfoData->m_PK_DesignObj_SelectedObject == 5607) && GetCurrentScreen_PK_DesignObj()!=DESIGNOBJ_popKeywordSearch_CONST )
	{
		...
		ShowRightIconListGrid(); // We may have changed between list or icon view
	...
	if(m_pOrbiter->m_sSkin == AUDIO_STATION_SKIN)
	{
		...
		{
			mediaFileBrowserOptions.m_mapGridStyle[mediaFileBrowserOptions.m_PK_AttributeType_Sort] = omgs_BrowserStyle_List;
			bChangedSortFilter = true;
		}
		else if(pObjectInfoData->m_PK_DesignObj_SelectedObject == DESIGNOBJ_butViewIcons_CONST)
		{
			mediaFileBrowserOptions.m_mapGridStyle[mediaFileBrowserOptions.m_PK_AttributeType_Sort] = omgs_BrowserStyle_Icon;
			bChangedSortFilter = true;
		}
...
//-----------------------------------------------------------------------------------------------------
void ScreenHandler::ShowRightIconListGrid()
{
	if(mediaFileBrowserOptions.m_mapGridStyle_Current() == omgs_BrowserStyle_List)
	{
	...
	else if(mediaFileBrowserOptions.m_mapGridStyle_Current() == omgs_BrowserStyle_Icon)
	{
...
bool ScreenHandler::MediaBrowser_DatagridSelected(CallBackData *pData)
{
	...
	if(	pCellInfoData->m_pDesignObj_DataGrid->m_iBaseObjectID == DESIGNOBJ_dgFileList2_CONST || pCellInfoData->m_pDesignObj_DataGrid->m_iBaseObjectID == DESIGNOBJ_dgFileList2_Pics_CONST || pCellInfoData->m_pDesignObj_DataGrid->m_iBaseObjectID == 5604 )
	{
	...
	else if( pCellInfoData->m_nPK_Datagrid==DATAGRID_Media_Search_Auto_Compl_CONST )
	{
		...
		int PK_AttributeType = atoi(pCellInfoData->m_pDataGridCell->m_mapAttributes_Find("PK_AttributeType").c_str());
		string sName = pCellInfoData->m_pDataGridCell->m_mapAttributes_Find("Name");
		MediaFileAttributeDrillDown *pMediaFileAttributeDrillDown = new MediaFileAttributeDrillDown(atoi(pCellInfoData->m_sValue.c_str()),PK_AttributeType,sName.empty()==false ? sName : pCellInfoData->m_sText,0);
		mediaFileBrowserOptions.m_listPK_AttributeType_Sort_Prior.push_front(PK_AttributeType);
...
void ScreenHandler::SelectedMediaFile(string sFile,char cActionItem)
{
	...
	if( sTerms.empty() )
	{
		...
		{
			pObj_Play = m_pOrbiter->FindObject( TOSTRING(5654) ".0.0." TOSTRING(5652) );
			pObj_Close = m_pOrbiter->FindObject( TOSTRING(5654) ".0.0." TOSTRING(5656) );
			pObj_Delete = m_pOrbiter->FindObject( TOSTRING(5654) ".0.0." TOSTRING(5657) );
		}
		else
		{
	...
	m_pOrbiter->CMD_Goto_DesignObj(0,pObj_Play->TopMostObject()->m_ObjectID,"","",false,true);
	...
	if( sPicture.empty()==false )
	{
		...
		m_pOrbiter->CMD_Update_Object_Image(pObj_Play->TopMostObject()->m_ObjectID + "." TOSTRING(DESIGNOBJ_objCDCover_CONST),"jpg",
			pGraphicData,
			(int) size,"0");
	...
	DesignObj_Orbiter *pParentObject = dynamic_cast<DesignObj_Orbiter *>(pObj_Play->TopMostObject());
...
void ScreenHandler::SelectedAttributeCell(DataGridCell *pCell)
{
	...
	else
	{
		...
		case ATTRIBUTETYPE_Album_CONST:
			mediaFileBrowserOptions.m_PK_AttributeType_Sort = ATTRIBUTETYPE_Tracks_On_Album_SPECIAL;
			break;
...
//-----------------------------------------------------------------------------------------------------
void ScreenHandler::SCREEN_Music_Full_Screen_OSD(long PK_Screen)
{
	if(m_pOrbiter->m_sSkin == AUDIO_STATION_SKIN)
	{
		DesignObj_DataGrid *pObj = (DesignObj_DataGrid *) m_pOrbiter->FindObject( TOSTRING(5551) ".0.0." TOSTRING(5603) );
...
//-----------------------------------------------------------------------------------------------------
bool ScreenHandler::MusicFullScreen_ObjectSelected(CallBackData *pData)
{
	ObjectInfoBackData *pObjectInfoData = (ObjectInfoBackData *)pData;
	...
	else if( pObjectInfoData->m_pObj->m_iBaseObjectID==5660 )  // save playlist
	{
		string sName = m_pOrbiter->m_mapVariable_Find(VARIABLE_Seek_Value_CONST);
		if( sName.empty()==false )
		{
			DCE::CMD_Save_playlist CMD_Save_playlist(m_pOrbiter->m_dwPK_Device,m_pOrbiter->m_dwPK_Device_MediaPlugIn, 0, "", sName, true);
			m_pOrbiter->SendCommand(CMD_Save_playlist);
			m_pOrbiter->CMD_Go_back("","");
		}
	}
	else if( pObjectInfoData->m_pObj->m_iBaseObjectID==5661 )  // cancel save playlist
	{
		m_pOrbiter->CMD_Goto_Screen("",SCREEN_CurrentlyActiveRemote_CONST,0,false,false);
	}
	return false; // Keep processing it
}
//-----------------------------------------------------------------------------------------------------
bool ScreenHandler::MusicFullScreen_GridRendering(CallBackData *pData)
{
	...
	if( pCell )
	{
		...
		{
			string sValue;
			DCE::CMD_Get_Attributes_For_Media CMD_Get_Attributes_For_Media(m_pOrbiter->m_dwPK_Device, m_pOrbiter->m_dwPK_Device_MediaPlugIn, "!F" + sPK_File, "", &sValue);
...
//-----------------------------------------------------------------------------------------------------
bool ScreenHandler::MusicFullScreen_DatagridSelected(CallBackData *pData)
{
	DatagridCellBackData *pCellInfoData = (DatagridCellBackData *)pData;
	if( pCellInfoData->m_pDesignObj_DataGrid )
	{
...
void ScreenHandler::SCREEN_CurrentlyActiveRemote(long PK_Screen)
{
	...
	{
		m_pOrbiter->CMD_Goto_Screen("",SCREEN_Music_Full_Screen_OSD_CONST);
	}
	else
	{
...
void ScreenHandler::SetupAudioServer()
{
	...
	else
	{
		...
		if( mediaFileBrowserOptions.m_pObj_ListGrid )
		{
			mediaFileBrowserOptions.m_pObj_ListGrid->m_pObjUp = m_pOrbiter->FindObject( TOSTRING(DESIGNOBJ_mnuMenuAudioServer_CONST) ".0.0." TOSTRING(DESIGNOBJ_butScrollUp_MediaBrowser_CONST) );
			mediaFileBrowserOptions.m_pObj_ListGrid->m_pObjDown = m_pOrbiter->FindObject( TOSTRING(DESIGNOBJ_mnuMenuAudioServer_CONST) ".0.0." TOSTRING(DESIGNOBJ_butScrollDown_MediaBrowser_CONST) );
		}
		if( mediaFileBrowserOptions.m_pObj_PicGrid )
		{
			mediaFileBrowserOptions.m_pObj_PicGrid->m_pObjUp = m_pOrbiter->FindObject( TOSTRING(DESIGNOBJ_mnuMenuAudioServer_CONST) ".0.0." TOSTRING(DESIGNOBJ_butScrollUp_MediaBrowser_CONST) );
			mediaFileBrowserOptions.m_pObj_PicGrid->m_pObjDown = m_pOrbiter->FindObject( TOSTRING(DESIGNOBJ_mnuMenuAudioServer_CONST) ".0.0." TOSTRING(DESIGNOBJ_butScrollDown_MediaBrowser_CONST) );
		}
		...
		for(MemoryDataTable::iterator it=pDatagridAcquiredBackData->m_pDataGridTable->m_MemoryDataTable.begin();it!=pDatagridAcquiredBackData->m_pDataGridTable->m_MemoryDataTable.end();++it)
		{
			...
			if( itobj!=pDatagridAcquiredBackData->m_pObj->m_mapChildDgObjects.end() )
			{
				...
				for( iHao=pObj->m_ChildObjects.begin(  ); iHao != pObj->m_ChildObjects.end(  ); ++iHao )
				{
					...
					DesignObj_Orbiter *pDesignObj_Orbiter = (DesignObj_Orbiter *)( *iHao );
					if( pDesignObj_Orbiter->m_iBaseObjectID==5611 ) //TODO : remove hardcodings
					{
...
void ScreenHandler::AudioServer_PopulateDatagrid()
{
	...
	if( mediaFileBrowserOptions.m_mapGridStyle_Current() == omgs_BrowserStyle_Icon )
	...
	else
	{
		...
		int PK_DesignObj_ToShow = 0;  // See which of the various styles of the list cell we want to show based on the sort
		switch( mediaFileBrowserOptions.m_PK_AttributeType_Sort )
		{
		case ATTRIBUTETYPE_Tracks_On_Album_SPECIAL:
			PK_DesignObj_ToShow=5635;
			break;
		case ATTRIBUTETYPE_Album_CONST:
			PK_DesignObj_ToShow=5633;
			break;
		case ATTRIBUTETYPE_Title_CONST:
			PK_DesignObj_ToShow=5634;
			break;
		default:
			PK_DesignObj_ToShow=5632;
			break;
		}

		// Hide or show the correct children of the grid
		for( map< pair<int,int>, DesignObj_Orbiter *>::iterator it=pObj->m_mapChildDgObjects.begin();it!=pObj->m_mapChildDgObjects.end();++it )
		{
			DesignObj_Orbiter *pObj_Cell = it->second; // This is the cell.  Go through it's children
			for( DesignObj_DataList::iterator itli=pObj_Cell->m_ChildObjects.begin();itli!=pObj_Cell->m_ChildObjects.end();++itli)
			{
				DesignObj_Orbiter *pObj_Child = (DesignObj_Orbiter *) *itli;
				pObj_Child->m_bHidden = PK_DesignObj_ToShow!=pObj_Child->m_iBaseObjectID;
			}
		}
	}
	...
	if(m_pOrbiter->m_sSkin == AUDIO_STATION_SKIN && NULL != pObj)
	{
		...
		m_pOrbiter->CMD_Set_Graphic_To_Display( TOSTRING(DESIGNOBJ_mnuMenuAudioServer_CONST) ".0.0." TOSTRING(DESIGNOBJ_butSortBySong_CONST), mediaFileBrowserOptions.m_PK_AttributeType_Sort==ATTRIBUTETYPE_Title_CONST || mediaFileBrowserOptions.m_PK_AttributeType_Sort==ATTRIBUTETYPE_Tracks_On_Album_SPECIAL ?  "-1" : "0" );

		m_pOrbiter->CMD_Set_Graphic_To_Display( TOSTRING(DESIGNOBJ_mnuMenuAudioServer_CONST) ".0.0." TOSTRING(DESIGNOBJ_butViewList_CONST), mediaFileBrowserOptions.m_mapGridStyle_Current() == omgs_BrowserStyle_List ?  "-1" : "0" );
		m_pOrbiter->CMD_Set_Graphic_To_Display( TOSTRING(DESIGNOBJ_mnuMenuAudioServer_CONST) ".0.0." TOSTRING(DESIGNOBJ_butViewIcons_CONST), mediaFileBrowserOptions.m_mapGridStyle_Current() == omgs_BrowserStyle_Icon ?  "-1" : "0" );

		m_pOrbiter->CMD_Show_Object( TOSTRING(DESIGNOBJ_mnuMenuAudioServer_CONST) ".0.0." TOSTRING(DESIGNOBJ_butPlayAlbum_CONST), 0, "", "", mediaFileBrowserOptions.m_PK_AttributeType_Sort==ATTRIBUTETYPE_Title_CONST || mediaFileBrowserOptions.m_PK_AttributeType_Sort==ATTRIBUTETYPE_Tracks_On_Album_SPECIAL ? "1" : "0");
...
bool ScreenHandler::CurrentDisc_GridRendering(CallBackData *pData)
{
	...
	while(pos<sValue.size())
	{
		...
		if( sToken=="PICTURE" )
		{
			...
			if( pImage )
			{
				m_pOrbiter->CMD_Update_Object_Image(TOSTRING(5555) ".0.0." TOSTRING(DESIGNOBJ_objCDCover_CONST),"jpg", pImage, static_cast<int>(size), "0");
				bUpdatedPic=true;
				delete[] pImage;
			}
		}
			...
			if( pCell && pCell->GetValue() )
			{
				...
				else if( pObjectInfoData->m_pObj->m_iBaseObjectID==DESIGNOBJ_icoPlay_CONST && m_pOrbiter->m_pLocationInfo && pCell->m_Value ) // play requires a location to play at
				{
					...
					DCE::CMD_MH_Play_Media CMD_MH_Play_Media(m_pOrbiter->m_dwPK_Device,m_pOrbiter->m_dwPK_Device_MediaPlugIn,
						...
						0,0,StringUtils::itos(m_pOrbiter->m_pLocationInfo->PK_EntertainArea),false,0,false,false,false);
					m_pOrbiter->SendCommand(CMD_MH_Play_Media);
				}
