...
bool OrbiterLinux::PreprocessEvent(Orbiter::Event &event)
{
	...
	{
/*		
 *		THIS WON'T WORK. IT'S A DESIGN ISSUE WITH IRReceiverBase::GetConfig and the data deserialized from orbitergen
 *		
		g_pPlutoLogger->Write(LV_CRITICAL, "mumu 1, size %d", m_mapScanCodeToRemoteButton.size());
		
		map<int,string>::iterator it = m_mapScanCodeToRemoteButton.find(event.data.button.m_iPK_Button);
		if(it != m_mapScanCodeToRemoteButton.end())
		{
			g_pPlutoLogger->Write(LV_CRITICAL, "mumu 2");
			
			//TODO: maybe we should have a "default remote" or something like this
			for(VectDeviceData_Impl::iterator itr=m_pData->m_vectDeviceData_Impl_Children.begin();itr!=m_pData->m_vectDeviceData_Impl_Children.end();++itr)
			{
				g_pPlutoLogger->Write(LV_CRITICAL, "mumu 3");
				
				DeviceData_Impl *pDeviceData_Impl = *itr;
				if( pDeviceData_Impl->WithinCategory(DEVICECATEGORY_Remote_Controls_CONST) )
				{
					g_pPlutoLogger->Write(LV_CRITICAL, "Calling received code for remote %d, code %s", pDeviceData_Impl->m_dwPK_Device, it->second.c_str()); 
		            ReceivedCode(pDeviceData_Impl->m_dwPK_Device, it->second.c_str());
        		    return true;
				}
			}
		}
*/	
		bool bGotARemote = false;
        for(VectDeviceData_Impl::iterator itr=m_pData->m_vectDeviceData_Impl_Children.begin();itr!=m_pData->m_vectDeviceData_Impl_Children.end();++itr)
        {
   	    	g_pPlutoLogger->Write(LV_CRITICAL, "mumu 3");
        	DeviceData_Impl *pDeviceData_Impl = *itr;
		...
		//hardcoding for now, until we'll fix the design issue with irreceiverbase :(
		if(bGotARemote)
		{
	        switch (event.data.button.m_iPK_Button)
    	    {
        	    case 98:   event.data.button.m_iPK_Button = BUTTON_Up_Arrow_CONST; return true;
				...
				case 20:   event.data.button.m_iPK_Button = BUTTON_Mouse_8_CONST; return true;

				case 44:   event.data.button.m_iPK_Button = BUTTON_Mouse_3_CONST; return true;

				case 10:   event.data.button.m_iPK_Button = BUTTON_1_CONST; return true;
				case 11:   event.data.button.m_iPK_Button = BUTTON_2_CONST; return true;
				case 12:   event.data.button.m_iPK_Button = BUTTON_3_CONST; return true;
				case 13:   event.data.button.m_iPK_Button = BUTTON_4_CONST; return true;
				case 14:   event.data.button.m_iPK_Button = BUTTON_5_CONST; return true;
				case 15:   event.data.button.m_iPK_Button = BUTTON_6_CONST; return true;
				case 16:   event.data.button.m_iPK_Button = BUTTON_7_CONST; return true;
				case 17:   event.data.button.m_iPK_Button = BUTTON_8_CONST; return true;
				case 18:   event.data.button.m_iPK_Button = BUTTON_9_CONST; return true;
				case 19:   event.data.button.m_iPK_Button = BUTTON_0_CONST; return true;
			}
		}
	...
	g_pPlutoLogger->Write(LV_CRITICAL, "mumu 4");
