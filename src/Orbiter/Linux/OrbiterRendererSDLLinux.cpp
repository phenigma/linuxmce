...
void OrbiterRendererSDLLinux::reinitGraphics()
{
    ...
    g_pOrbiter->CallMaintenanceInMiliseconds( 3000, callback, NULL, pe_ALL, this );
}

void OrbiterRendererSDLLinux::setInputFocusToMe(void *)
{
    g_pOrbiter->CallMaintenanceInMiliseconds( 7000, (OrbiterCallBack)&OrbiterRendererSDLLinux::setInputFocusToMe, NULL, pe_ALL, this ); // do this every 7 seconds
}
...
bool OrbiterRendererSDLLinux::RenderDesktop( class DesignObj_Orbiter *pObj, PlutoRectangle rectTotal, PlutoPoint point )
{
    ...
    g_pOrbiter->GetButtonsInObject(pObj,vectButtonMaps);
...
/*virtual*/ void OrbiterRendererSDLLinux::ActivateExternalWindowAsync(void *)
{
    ...
    if (bIsWindowAvailable)
    {
        ...
        g_pOrbiter->CallMaintenanceInMiliseconds( bIsWindowAvailable ? 1000 : 200, (OrbiterCallBack)&OrbiterRendererSDLLinux::ActivateExternalWindowAsync, NULL, pe_ALL );
}
...
void OrbiterRendererSDLLinux::Initialize(GraphicType Type, int iPK_Room, int iPK_EntertainArea)
{
    ...
    g_pPlutoLogger->Write(LV_WARNING, "Orbiter UI Version is %d", g_pOrbiter->m_iUiVersion);

    if(g_pOrbiter->UsesUIVersion2())
	{
		g_pOrbiter->m_pMouseBehavior = new MouseBehavior_Linux();
	}
    m_pRecordHandler->enableRecording(g_pOrbiter, true);
