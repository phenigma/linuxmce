...
extern "C" {
	...
	{ 
		...
		if( bIsRuntimePlugin )
			return AirPlay_Streamer::PK_DeviceTemplate_get_static();
		else
			return 0;
	}
}
...
extern "C" {
	class Command_Impl *RegisterAsPlugIn(class Router *pRouter,int PK_Device,Logger *pPlutoLogger)
	{
		...
		AirPlay_Streamer *pAirPlay_Streamer = new AirPlay_Streamer(PK_Device, "localhost",true,false,pRouter);
		if( pAirPlay_Streamer->m_bQuit_get()|| !pAirPlay_Streamer->GetConfig() )
		{
			delete pAirPlay_Streamer;
			return NULL;
		}
		else
		{
			g_pCommand_Impl=pAirPlay_Streamer;
			g_pDeadlockHandler=Plugin_DeadlockHandler;
			g_pSocketCrashHandler=Plugin_SocketCrashHandler;
		}
		return pAirPlay_Streamer;
	}
}
//<-dceag-plug-e->
...
int main(int argc, char* argv[]) 
{
	...
	cout << "AirPlay_Streamer, v." << VERSION << endl
	...
	if (bError)
	{
		cout << "A Pluto DCE Device.  See www.plutohome.com/dce for details." << endl
			<< "Usage: AirPlay_Streamer [-r Router's IP] [-d My Device ID] [-l dcerouter|stdout|null|filename]" << endl
	...
	try
	{
		if( sLogger=="dcerouter" )
			LoggerWrapper::SetInstance(new ServerLogger(PK_Device, AirPlay_Streamer::PK_DeviceTemplate_get_static(), sRouter_IP));
	...
	try
	{
		AirPlay_Streamer *pAirPlay_Streamer = new AirPlay_Streamer(PK_Device, sRouter_IP,true,bLocalMode);
		if ( pAirPlay_Streamer->GetConfig() && pAirPlay_Streamer->Connect(pAirPlay_Streamer->PK_DeviceTemplate_get()) ) 
		{
			g_pCommand_Impl=pAirPlay_Streamer;
			g_pDeadlockHandler=DeadlockHandler;
			g_pSocketCrashHandler=SocketCrashHandler;
			LoggerWrapper::GetInstance()->Write(LV_STATUS, "Connect OK");
			pAirPlay_Streamer->CreateChildren();
			if( bLocalMode )
				pAirPlay_Streamer->RunLocalMode();
			else
			{
				if(pAirPlay_Streamer->m_RequestHandlerThread)
					pthread_join(pAirPlay_Streamer->m_RequestHandlerThread, NULL);  // This function will return when the device is shutting down
			}
		...
		{
			bAppError = true;
			if( pAirPlay_Streamer->m_pEvent && pAirPlay_Streamer->m_pEvent->m_pClientSocket && pAirPlay_Streamer->m_pEvent->m_pClientSocket->m_eLastError==ClientSocket::cs_err_CannotConnect )
			{
				bAppError = false;
				bReload = false;
		...
		if( pAirPlay_Streamer->m_bReload )
			bReload=true;
