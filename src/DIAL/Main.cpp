...
extern "C" {
	...
	{ 
		...
		if( bIsRuntimePlugin )
			return DIAL::PK_DeviceTemplate_get_static();
		else
			return 0;
	}
}
...
extern "C" {
	class Command_Impl *RegisterAsPlugIn(class Router *pRouter,int PK_Device,Logger *pPlutoLogger)
	{
		...
		DIAL *pDIAL = new DIAL(PK_Device, "localhost",true,false,pRouter);
		if( pDIAL->m_bQuit_get()|| !pDIAL->GetConfig() )
		{
			delete pDIAL;
			return NULL;
		}
...
int main(int argc, char* argv[]) 
{
	...
	if (bError)
	{
		cout << "A Pluto DCE Device.  See www.plutohome.com/dce for details." << endl
			<< "Usage: DIAL [-r Router's IP] [-d My Device ID] [-l dcerouter|stdout|null|filename]" << endl
	...
	try
	{
		if( sLogger=="dcerouter" )
			LoggerWrapper::SetInstance(new ServerLogger(PK_Device, DIAL::PK_DeviceTemplate_get_static(), sRouter_IP));
	...
	try
	{
		DIAL *pDIAL = new DIAL(PK_Device, sRouter_IP,true,bLocalMode);
		if ( pDIAL->GetConfig() && pDIAL->Connect(pDIAL->PK_DeviceTemplate_get()) ) 
		{
			g_pCommand_Impl=pDIAL;
			g_pDeadlockHandler=DeadlockHandler;
			g_pSocketCrashHandler=SocketCrashHandler;
			LoggerWrapper::GetInstance()->Write(LV_STATUS, "Connect OK");
			pDIAL->CreateChildren();
			if( bLocalMode )
				pDIAL->RunLocalMode();
			else
			{
				if(pDIAL->m_RequestHandlerThread)
					pthread_join(pDIAL->m_RequestHandlerThread, NULL);  // This function will return when the device is shutting down
			}
		...
		{
			bAppError = true;
			if( pDIAL->m_pEvent && pDIAL->m_pEvent->m_pClientSocket && pDIAL->m_pEvent->m_pClientSocket->m_eLastError==ClientSocket::cs_err_CannotConnect )
			{
				bAppError = false;
				bReload = false;
