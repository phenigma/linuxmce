...
extern "C" {
	...
	{ 
		...
		if( bIsRuntimePlugin )
			return CM15A::PK_DeviceTemplate_get_static();
		else
			return 0;
	}
}
...
extern "C" {
	class Command_Impl *RegisterAsPlugIn(class Router *pRouter,int PK_Device,Logger *pPlutoLogger)
	{
		...
		CM15A *pCM15A = new CM15A(PK_Device, "localhost",true,false,pRouter);
		if( pCM15A->m_bQuit_get()|| !pCM15A->GetConfig() )
		{
			delete pCM15A;
			return NULL;
		}
...
int main(int argc, char* argv[]) 
{
	...
	if (bError)
	{
		cout << "A Pluto DCE Device.  See www.plutohome.com/dce for details." << endl
			<< "Usage: CM15A [-r Router's IP] [-d My Device ID] [-l dcerouter|stdout|null|filename]" << endl
	...
	try
	{
		if( sLogger=="dcerouter" )
			LoggerWrapper::SetInstance(new ServerLogger(PK_Device, CM15A::PK_DeviceTemplate_get_static(), sRouter_IP));
	...
	try
	{
		CM15A *pCM15A = new CM15A(PK_Device, sRouter_IP,true,bLocalMode);
		if ( pCM15A->GetConfig() && pCM15A->Connect(pCM15A->PK_DeviceTemplate_get()) ) 
		{
			g_pCommand_Impl=pCM15A;
			g_pDeadlockHandler=DeadlockHandler;
			g_pSocketCrashHandler=SocketCrashHandler;
			LoggerWrapper::GetInstance()->Write(LV_STATUS, "Connect OK");
			pCM15A->CreateChildren();
			if( bLocalMode )
				pCM15A->RunLocalMode();
			else
			{
				if(pCM15A->m_RequestHandlerThread)
					pthread_join(pCM15A->m_RequestHandlerThread, NULL);  // This function will return when the device is shutting down
			}
		...
		{
			bAppError = true;
			if( pCM15A->m_pEvent && pCM15A->m_pEvent->m_pClientSocket && pCM15A->m_pEvent->m_pClientSocket->m_eLastError==ClientSocket::cs_err_CannotConnect )
			{
				bAppError = false;
				bReload = false;
