...
extern "C" {
	...
	{ 
		...
		if( bIsRuntimePlugin )
			return USB_Game_Pad::PK_DeviceTemplate_get_static();
		else
			return 0;
	}
}
...
extern "C" {
	class Command_Impl *RegisterAsPlugIn(class Router *pRouter,int PK_Device,Logger *pPlutoLogger)
	{
		...
		USB_Game_Pad *pUSB_Game_Pad = new USB_Game_Pad(PK_Device, "localhost",true,false,pRouter);
		if( pUSB_Game_Pad->m_bQuit_get()|| !pUSB_Game_Pad->GetConfig() )
		{
			delete pUSB_Game_Pad;
			return NULL;
		}
		else
		{
...
int main(int argc, char* argv[]) 
{
	...
	cout << "USB_Game_Pad, v." << VERSION << endl
	...
	if (bError)
	{
		cout << "A Pluto DCE Device.  See www.plutohome.com/dce for details." << endl
			<< "Usage: USB_Game_Pad [-r Router's IP] [-d My Device ID] [-l dcerouter|stdout|null|filename]" << endl
	...
	try
	{
		if( sLogger=="dcerouter" )
			LoggerWrapper::SetInstance(new ServerLogger(PK_Device, USB_Game_Pad::PK_DeviceTemplate_get_static(), sRouter_IP));
	...
	try
	{
		...
		if ( pUSB_Game_Pad->GetConfig() && pUSB_Game_Pad->Connect(pUSB_Game_Pad->PK_DeviceTemplate_get()) ) 
		{
			...
			pUSB_Game_Pad->CreateChildren();
			if( bLocalMode )
				pUSB_Game_Pad->RunLocalMode();
			else
			{
				if(pUSB_Game_Pad->m_RequestHandlerThread)
					pthread_join(pUSB_Game_Pad->m_RequestHandlerThread, NULL);  // This function will return when the device is shutting down
			}
		...
		{
			bAppError = true;
			if( pUSB_Game_Pad->m_pEvent && pUSB_Game_Pad->m_pEvent->m_pClientSocket && pUSB_Game_Pad->m_pEvent->m_pClientSocket->m_eLastError==ClientSocket::cs_err_CannotConnect )
			{
				bAppError = false;
				bReload = false;
