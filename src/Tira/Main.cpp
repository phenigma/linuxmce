...
extern "C" {
	...
	{ 
		...
		if( bIsRuntimePlugin )
			return Tira::PK_DeviceTemplate_get_static();
		else
			return 0;
	}
}
...
extern "C" {
	class Command_Impl *RegisterAsPlugIn(class Router *pRouter,int PK_Device,Logger *pPlutoLogger)
	{
		...
		Tira *pTira = new Tira(PK_Device, "localhost",true,false,pRouter);
		if( pTira->m_bQuit_get()|| !pTira->GetConfig() )
		{
			delete pTira;
			return NULL;
		}
...
int main(int argc, char* argv[]) 
{
	...
	if (bError)
	{
		cout << "A Pluto DCE Device.  See www.plutohome.com/dce for details." << endl
			<< "Usage: Tira [-r Router's IP] [-d My Device ID] [-l dcerouter|stdout|null|filename]" << endl
	...
	try
	{
		if( sLogger=="dcerouter" )
			LoggerWrapper::SetInstance(new ServerLogger(PK_Device, Tira::PK_DeviceTemplate_get_static(), sRouter_IP));
	...
	try
	{
		Tira *pTira = new Tira(PK_Device, sRouter_IP,true,bLocalMode);
		if ( pTira->GetConfig() && pTira->Connect(pTira->PK_DeviceTemplate_get()) ) 
		{
			g_pCommand_Impl=pTira;
			g_pDeadlockHandler=DeadlockHandler;
			g_pSocketCrashHandler=SocketCrashHandler;
			LoggerWrapper::GetInstance()->Write(LV_STATUS, "Connect OK");
			pTira->CreateChildren();
			if( bLocalMode )
				pTira->RunLocalMode();
			else
			{
				if(pTira->m_RequestHandlerThread)
					pthread_join(pTira->m_RequestHandlerThread, NULL);  // This function will return when the device is shutting down
			}
		...
		{
			bAppError = true;
			if( pTira->m_pEvent && pTira->m_pEvent->m_pClientSocket && pTira->m_pEvent->m_pClientSocket->m_eLastError==ClientSocket::cs_err_CannotConnect )
			{
				bAppError = false;
				bReload = false;
