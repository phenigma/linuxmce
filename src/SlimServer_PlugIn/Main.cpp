...
extern "C" {
	...
	{ 
		...
		if( bIsRuntimePlugin )
			return SlimServer_PlugIn::PK_DeviceTemplate_get_static();
		else
			return 0;
	}
}
...
extern "C" {
	class Command_Impl *RegisterAsPlugIn(class Router *pRouter,int PK_Device,Logger *pPlutoLogger)
	{
		...
		SlimServer_PlugIn *pSlimServer_PlugIn = new SlimServer_PlugIn(PK_Device, "localhost",true,false,pRouter);
		if( pSlimServer_PlugIn->m_bQuit_get()|| !pSlimServer_PlugIn->GetConfig() )
		{
			delete pSlimServer_PlugIn;
			return NULL;
		}
		else
		{
			g_pCommand_Impl=pSlimServer_PlugIn;
			g_pDeadlockHandler=Plugin_DeadlockHandler;
			g_pSocketCrashHandler=Plugin_SocketCrashHandler;
		}
		return pSlimServer_PlugIn;
	}
}
//<-dceag-plug-e->
...
int main(int argc, char* argv[]) 
{
	...
	cout << "SlimServer_PlugIn, v." << VERSION << endl
	...
	if (bError)
	{
		cout << "A Pluto DCE Device.  See www.plutohome.com/dce for details." << endl
			<< "Usage: SlimServer_PlugIn [-r Router's IP] [-d My Device ID] [-l dcerouter|stdout|null|filename]" << endl
	...
	try
	{
		if( sLogger=="dcerouter" )
			LoggerWrapper::SetInstance(new ServerLogger(PK_Device, SlimServer_PlugIn::PK_DeviceTemplate_get_static(), sRouter_IP));
	...
	try
	{
		SlimServer_PlugIn *pSlimServer_PlugIn = new SlimServer_PlugIn(PK_Device, sRouter_IP,true,bLocalMode);
		if ( pSlimServer_PlugIn->GetConfig() && pSlimServer_PlugIn->Connect(pSlimServer_PlugIn->PK_DeviceTemplate_get()) ) 
		{
			g_pCommand_Impl=pSlimServer_PlugIn;
			g_pDeadlockHandler=DeadlockHandler;
			g_pSocketCrashHandler=SocketCrashHandler;
			LoggerWrapper::GetInstance()->Write(LV_STATUS, "Connect OK");
			pSlimServer_PlugIn->CreateChildren();
			if( bLocalMode )
				pSlimServer_PlugIn->RunLocalMode();
			else
			{
				if(pSlimServer_PlugIn->m_RequestHandlerThread)
					pthread_join(pSlimServer_PlugIn->m_RequestHandlerThread, NULL);  // This function will return when the device is shutting down
			}
		...
		{
			bAppError = true;
			if( pSlimServer_PlugIn->m_pEvent && pSlimServer_PlugIn->m_pEvent->m_pClientSocket && pSlimServer_PlugIn->m_pEvent->m_pClientSocket->m_eLastError==ClientSocket::cs_err_CannotConnect )
			{
				bAppError = false;
				bReload = false;
		...
		if( pSlimServer_PlugIn->m_bReload )
			bReload=true;
