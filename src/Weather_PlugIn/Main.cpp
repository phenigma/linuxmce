...
extern "C" {
	...
	{ 
		...
		if( bIsRuntimePlugin )
			return Weather_PlugIn::PK_DeviceTemplate_get_static();
		else
			return 0;
	}
}
...
extern "C" {
	class Command_Impl *RegisterAsPlugIn(class Router *pRouter,int PK_Device,Logger *pPlutoLogger)
	{
		...
		Weather_PlugIn *pWeather_PlugIn = new Weather_PlugIn(PK_Device, "localhost",true,false,pRouter);
		if( pWeather_PlugIn->m_bQuit_get()|| !pWeather_PlugIn->GetConfig() )
		{
			delete pWeather_PlugIn;
			return NULL;
		}
		else
		{
			g_pCommand_Impl=pWeather_PlugIn;
			g_pDeadlockHandler=Plugin_DeadlockHandler;
			g_pSocketCrashHandler=Plugin_SocketCrashHandler;
		}
		return pWeather_PlugIn;
	}
}
//<-dceag-plug-e->
...
int main(int argc, char* argv[]) 
{
	...
	cout << "Weather_PlugIn, v." << VERSION << endl
	...
	if (bError)
	{
		cout << "A Pluto DCE Device.  See www.plutohome.com/dce for details." << endl
			<< "Usage: Weather_PlugIn [-r Router's IP] [-d My Device ID] [-l dcerouter|stdout|null|filename]" << endl
	...
	try
	{
		if( sLogger=="dcerouter" )
			LoggerWrapper::SetInstance(new ServerLogger(PK_Device, Weather_PlugIn::PK_DeviceTemplate_get_static(), sRouter_IP));
	...
	try
	{
		Weather_PlugIn *pWeather_PlugIn = new Weather_PlugIn(PK_Device, sRouter_IP,true,bLocalMode);
		if ( pWeather_PlugIn->GetConfig() && pWeather_PlugIn->Connect(pWeather_PlugIn->PK_DeviceTemplate_get()) ) 
		{
			g_pCommand_Impl=pWeather_PlugIn;
			g_pDeadlockHandler=DeadlockHandler;
			g_pSocketCrashHandler=SocketCrashHandler;
			LoggerWrapper::GetInstance()->Write(LV_STATUS, "Connect OK");
			pWeather_PlugIn->CreateChildren();
			if( bLocalMode )
				pWeather_PlugIn->RunLocalMode();
			else
			{
				if(pWeather_PlugIn->m_RequestHandlerThread)
					pthread_join(pWeather_PlugIn->m_RequestHandlerThread, NULL);  // This function will return when the device is shutting down
			}
		...
		{
			bAppError = true;
			if( pWeather_PlugIn->m_pEvent && pWeather_PlugIn->m_pEvent->m_pClientSocket && pWeather_PlugIn->m_pEvent->m_pClientSocket->m_eLastError==ClientSocket::cs_err_CannotConnect )
			{
				bAppError = false;
				bReload = false;
