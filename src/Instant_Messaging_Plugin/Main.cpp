...
extern "C" {
	...
	{ 
		...
		if( bIsRuntimePlugin )
			return Instant_Messaging_Plugin::PK_DeviceTemplate_get_static();
		else
			return 0;
	}
}
...
extern "C" {
	class Command_Impl *RegisterAsPlugIn(class Router *pRouter,int PK_Device,Logger *pPlutoLogger)
	{
		...
		Instant_Messaging_Plugin *pInstant_Messaging_Plugin = new Instant_Messaging_Plugin(PK_Device, "localhost",true,false,pRouter);
		if( pInstant_Messaging_Plugin->m_bQuit_get()|| !pInstant_Messaging_Plugin->GetConfig() )
		{
			delete pInstant_Messaging_Plugin;
			return NULL;
		}
		else
		{
			g_pCommand_Impl=pInstant_Messaging_Plugin;
			g_pDeadlockHandler=Plugin_DeadlockHandler;
			g_pSocketCrashHandler=Plugin_SocketCrashHandler;
		}
		return pInstant_Messaging_Plugin;
	}
}
//<-dceag-plug-e->
...
int main(int argc, char* argv[]) 
{
	...
	cout << "Instant_Messaging_Plugin, v." << VERSION << endl
	...
	if (bError)
	{
		cout << "A LinuxMCE DCE Device.  See wiki.linuxmce.org/index.php/DCE for details." << endl
			<< "Usage: Instant_Messaging_Plugin [-r Router's IP] [-d My Device ID] [-l dcerouter|stdout|null|filename]" << endl
	...
	try
	{
		if( sLogger=="dcerouter" )
			LoggerWrapper::SetInstance(new ServerLogger(PK_Device, Instant_Messaging_Plugin::PK_DeviceTemplate_get_static(), sRouter_IP));
	...
	try
	{
		Instant_Messaging_Plugin *pInstant_Messaging_Plugin = new Instant_Messaging_Plugin(PK_Device, sRouter_IP,true,bLocalMode);
		if ( pInstant_Messaging_Plugin->GetConfig() && pInstant_Messaging_Plugin->Connect(pInstant_Messaging_Plugin->PK_DeviceTemplate_get()) ) 
		{
			g_pCommand_Impl=pInstant_Messaging_Plugin;
			g_pDeadlockHandler=DeadlockHandler;
			g_pSocketCrashHandler=SocketCrashHandler;
			LoggerWrapper::GetInstance()->Write(LV_STATUS, "Connect OK");
			pInstant_Messaging_Plugin->CreateChildren();
			if( bLocalMode )
				pInstant_Messaging_Plugin->RunLocalMode();
			else
			{
				if(pInstant_Messaging_Plugin->m_RequestHandlerThread)
					pthread_join(pInstant_Messaging_Plugin->m_RequestHandlerThread, NULL);  // This function will return when the device is shutting down
			}
		...
		{
			bAppError = true;
			if( pInstant_Messaging_Plugin->m_pEvent && pInstant_Messaging_Plugin->m_pEvent->m_pClientSocket && pInstant_Messaging_Plugin->m_pEvent->m_pClientSocket->m_eLastError==ClientSocket::cs_err_CannotConnect )
			{
				bAppError = false;
				bReload = false;
		...
		if( pInstant_Messaging_Plugin->m_bReload )
			bReload=true;
