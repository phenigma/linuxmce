...
extern "C" {
	...
	{ 
		...
		if( bIsRuntimePlugin )
			return Pandora_Player::PK_DeviceTemplate_get_static();
		else
			return 0;
	}
}
...
extern "C" {
	class Command_Impl *RegisterAsPlugIn(class Router *pRouter,int PK_Device,Logger *pPlutoLogger)
	{
		...
		Pandora_Player *pPandora_Player = new Pandora_Player(PK_Device, "localhost",true,false,pRouter);
		if( pPandora_Player->m_bQuit_get()|| !pPandora_Player->GetConfig() )
		{
			delete pPandora_Player;
			return NULL;
		}
		else
		{
			g_pCommand_Impl=pPandora_Player;
			g_pDeadlockHandler=Plugin_DeadlockHandler;
			g_pSocketCrashHandler=Plugin_SocketCrashHandler;
		}
		return pPandora_Player;
	}
}
//<-dceag-plug-e->
...
int main(int argc, char* argv[]) 
{
	...
	cout << "Pandora_Player, v." << VERSION << endl
	...
	if (bError)
	{
		cout << "A Pluto DCE Device.  See www.plutohome.com/dce for details." << endl
			<< "Usage: Pandora_Player [-r Router's IP] [-d My Device ID] [-l dcerouter|stdout|null|filename]" << endl
	...
	try
	{
		if( sLogger=="dcerouter" )
			LoggerWrapper::SetInstance(new ServerLogger(PK_Device, Pandora_Player::PK_DeviceTemplate_get_static(), sRouter_IP));
	...
	try
	{
		Pandora_Player *pPandora_Player = new Pandora_Player(PK_Device, sRouter_IP,true,bLocalMode);
		if ( pPandora_Player->GetConfig() && pPandora_Player->Connect(pPandora_Player->PK_DeviceTemplate_get()) ) 
		{
			g_pCommand_Impl=pPandora_Player;
			g_pDeadlockHandler=DeadlockHandler;
			g_pSocketCrashHandler=SocketCrashHandler;
			LoggerWrapper::GetInstance()->Write(LV_STATUS, "Connect OK");
			pPandora_Player->CreateChildren();
			if( bLocalMode )
				pPandora_Player->RunLocalMode();
			else
			{
				if(pPandora_Player->m_RequestHandlerThread)
					pthread_join(pPandora_Player->m_RequestHandlerThread, NULL);  // This function will return when the device is shutting down
			}
		...
		{
			bAppError = true;
			if( pPandora_Player->m_pEvent && pPandora_Player->m_pEvent->m_pClientSocket && pPandora_Player->m_pEvent->m_pClientSocket->m_eLastError==ClientSocket::cs_err_CannotConnect )
			{
				bAppError = false;
				bReload = false;
