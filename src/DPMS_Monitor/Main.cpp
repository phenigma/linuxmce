...
extern "C" {
	...
	{ 
		...
		if( bIsRuntimePlugin )
			return DPMS_Monitor::PK_DeviceTemplate_get_static();
		else
			return 0;
	}
}
...
extern "C" {
	class Command_Impl *RegisterAsPlugIn(class Router *pRouter,int PK_Device,Logger *pPlutoLogger)
	{
		...
		DPMS_Monitor *pDPMS_Monitor = new DPMS_Monitor(PK_Device, "localhost",true,false,pRouter);
		if( pDPMS_Monitor->m_bQuit_get()|| !pDPMS_Monitor->GetConfig() )
		{
			delete pDPMS_Monitor;
			return NULL;
		}
		else
		{
			g_pCommand_Impl=pDPMS_Monitor;
			g_pDeadlockHandler=Plugin_DeadlockHandler;
			g_pSocketCrashHandler=Plugin_SocketCrashHandler;
		}
		return pDPMS_Monitor;
	}
}
//<-dceag-plug-e->
...
int main(int argc, char* argv[]) 
{
	...
	cout << "DPMS_Monitor, v." << VERSION << endl
	...
	if (bError)
	{
		cout << "A Pluto DCE Device.  See www.plutohome.com/dce for details." << endl
			<< "Usage: DPMS_Monitor [-r Router's IP] [-d My Device ID] [-l dcerouter|stdout|null|filename]" << endl
	...
	try
	{
		if( sLogger=="dcerouter" )
			LoggerWrapper::SetInstance(new ServerLogger(PK_Device, DPMS_Monitor::PK_DeviceTemplate_get_static(), sRouter_IP));
	...
	try
	{
		DPMS_Monitor *pDPMS_Monitor = new DPMS_Monitor(PK_Device, sRouter_IP,true,bLocalMode);
		if ( pDPMS_Monitor->GetConfig() && pDPMS_Monitor->Connect(pDPMS_Monitor->PK_DeviceTemplate_get()) ) 
		{
			g_pCommand_Impl=pDPMS_Monitor;
			g_pDeadlockHandler=DeadlockHandler;
			g_pSocketCrashHandler=SocketCrashHandler;
			LoggerWrapper::GetInstance()->Write(LV_STATUS, "Connect OK");
			pDPMS_Monitor->CreateChildren();
			if( bLocalMode )
				pDPMS_Monitor->RunLocalMode();
			else
			{
				if(pDPMS_Monitor->m_RequestHandlerThread)
					pthread_join(pDPMS_Monitor->m_RequestHandlerThread, NULL);  // This function will return when the device is shutting down
			}
		...
		{
			bAppError = true;
			if( pDPMS_Monitor->m_pEvent && pDPMS_Monitor->m_pEvent->m_pClientSocket && pDPMS_Monitor->m_pEvent->m_pClientSocket->m_eLastError==ClientSocket::cs_err_CannotConnect )
			{
				bAppError = false;
				bReload = false;
