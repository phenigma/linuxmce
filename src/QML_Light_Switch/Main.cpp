...
extern "C" {
	...
	{ 
		...
		if( bIsRuntimePlugin )
			return QML_Light_Switch::PK_DeviceTemplate_get_static();
		else
			return 0;
	}
}
...
extern "C" {
	class Command_Impl *RegisterAsPlugIn(class Router *pRouter,int PK_Device,Logger *pPlutoLogger)
	{
		...
		QML_Light_Switch *pQML_Light_Switch = new QML_Light_Switch(PK_Device, "localhost",true,false,pRouter);
		if( pQML_Light_Switch->m_bQuit_get()|| !pQML_Light_Switch->GetConfig() )
		{
			delete pQML_Light_Switch;
			return NULL;
		}
		else
		{
			g_pCommand_Impl=pQML_Light_Switch;
			g_pDeadlockHandler=Plugin_DeadlockHandler;
			g_pSocketCrashHandler=Plugin_SocketCrashHandler;
		}
		return pQML_Light_Switch;
	}
}
//<-dceag-plug-e->
...
int main(int argc, char* argv[]) 
{
	...
	cout << "QML_Light_Switch, v." << VERSION << endl
	...
	try
	{
		if( sLogger=="dcerouter" )
			LoggerWrapper::SetInstance(new ServerLogger(PK_Device, QML_Light_Switch::PK_DeviceTemplate_get_static(), sRouter_IP));
	...
	try
		...
		if ( pQML_Light_Switch->GetConfig() && pQML_Light_Switch->Connect(pQML_Light_Switch->PK_DeviceTemplate_get()) ) 
		{
			...
			pQML_Light_Switch->CreateChildren();
			if( bLocalMode )
				pQML_Light_Switch->RunLocalMode();
			else
			{
				if(pQML_Light_Switch->m_RequestHandlerThread)
					pthread_join(pQML_Light_Switch->m_RequestHandlerThread, NULL);  // This function will return when the device is shutting down
			}
		...
		{
			bAppError = true;
			if( pQML_Light_Switch->m_pEvent && pQML_Light_Switch->m_pEvent->m_pClientSocket && pQML_Light_Switch->m_pEvent->m_pClientSocket->m_eLastError==ClientSocket::cs_err_CannotConnect )
			{
				bAppError = false;
				bReload = false;
		...
		if( pQML_Light_Switch->m_bReload )
			bReload=true;
