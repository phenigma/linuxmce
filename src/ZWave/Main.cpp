...
extern "C" {
	...
	{ 
		...
		if( bIsRuntimePlugin )
			return ZWave::PK_DeviceTemplate_get_static();
		else
			return 0;
	}
}
...
extern "C" {
	class Command_Impl *RegisterAsPlugIn(class Router *pRouter,int PK_Device,Logger *pPlutoLogger)
	{
		...
		if( pZWave->m_bQuit_get()|| !pZWave->GetConfig() )
		{
			delete pZWave;
			return NULL;
		}
...
int main(int argc, char* argv[]) 
{
	...
	try
	{
		if( sLogger=="dcerouter" )
			LoggerWrapper::SetInstance(new ServerLogger(PK_Device, ZWave::PK_DeviceTemplate_get_static(), sRouter_IP));
