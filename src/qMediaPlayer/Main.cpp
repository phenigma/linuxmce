...
extern "C" {
	...
	{ 
		...
		if( bIsRuntimePlugin )
			return qMediaPlayer::PK_DeviceTemplate_get_static();
		else
			return 0;
	}
}
...
extern "C" {
	class Command_Impl *RegisterAsPlugIn(class Router *pRouter,int PK_Device,Logger *pPlutoLogger)
	{
		...
		qMediaPlayer *pqMediaPlayer = new qMediaPlayer(PK_Device, "localhost",true,false,pRouter);
		if( pqMediaPlayer->m_bQuit_get()|| !pqMediaPlayer->GetConfig() )
		{
			delete pqMediaPlayer;
			return NULL;
		}
		else
		{
			g_pCommand_Impl=pqMediaPlayer;
...
int main(int argc, char* argv[]) 
{
	...
	cout << "qMediaPlayer, v." << VERSION << endl
	...
	if (bError)
	{
		cout << "A Pluto DCE Device.  See www.plutohome.com/dce for details." << endl
			<< "Usage: qMediaPlayer [-r Router's IP] [-d My Device ID] [-l dcerouter|stdout|null|filename]" << endl
	...
	try
	{
		if( sLogger=="dcerouter" )
			LoggerWrapper::SetInstance(new ServerLogger(PK_Device, qMediaPlayer::PK_DeviceTemplate_get_static(), sRouter_IP));
	...
	try
	{
		qMediaPlayer *pqMediaPlayer = new qMediaPlayer(PK_Device, sRouter_IP,true,bLocalMode);
		if ( pqMediaPlayer->GetConfig() && pqMediaPlayer->Connect(pqMediaPlayer->PK_DeviceTemplate_get()) ) 
		{
			g_pCommand_Impl=pqMediaPlayer;
			g_pDeadlockHandler=DeadlockHandler;
			g_pSocketCrashHandler=SocketCrashHandler;
			LoggerWrapper::GetInstance()->Write(LV_STATUS, "Connect OK");
			pqMediaPlayer->CreateChildren();
			if( bLocalMode )
				pqMediaPlayer->RunLocalMode();
			else
			{
				if(pqMediaPlayer->m_RequestHandlerThread)
					pthread_join(pqMediaPlayer->m_RequestHandlerThread, NULL);  // This function will return when the device is shutting down
			}
		...
		{
			bAppError = true;
			if( pqMediaPlayer->m_pEvent && pqMediaPlayer->m_pEvent->m_pClientSocket && pqMediaPlayer->m_pEvent->m_pClientSocket->m_eLastError==ClientSocket::cs_err_CannotConnect )
			{
				bAppError = false;
				bReload = false;
