...
extern "C" {
	...
	{ 
		...
		if( bIsRuntimePlugin )
			return Climate_Plugin::PK_DeviceTemplate_get_static();
		else
			return 0;
	}
}
...
int main(int argc, char* argv[]) 
{
	...
	cout << "Climate_Plugin, v." << VERSION << endl
	...
	if (bError)
	{
		cout << "A Pluto DCE Device.  See www.plutohome.com/dce for details." << endl
			<< "Usage: Climate_Plugin [-r Router's IP] [-d My Device ID] [-l dcerouter|stdout|null|filename]" << endl
	...
	try
	{
		if( sLogger=="dcerouter" )
			LoggerWrapper::SetInstance(new ServerLogger(PK_Device, Climate_Plugin::PK_DeviceTemplate_get_static(), sRouter_IP));
	...
	try
	{
		...
		{
			bAppError = true;
			if( pClimate_Plugin->m_pEvent && pClimate_Plugin->m_pEvent->m_pClientSocket && pClimate_Plugin->m_pEvent->m_pClientSocket->m_eLastError==ClientSocket::cs_err_CannotConnect )
			{
				bAppError = false;
				bReload = false;
