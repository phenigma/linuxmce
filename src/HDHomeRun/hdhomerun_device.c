...
int hdhomerun_device_get_tuner_program(struct hdhomerun_device_t *hd, uint16_t *pprogram_number, char **poptions)
{
	*poptions = NULL;
	...
	char *ptr;
	*pprogram_number = (uint16_t)strtol(program_str, &ptr, 0);
	if (ptr == program_str) {
		return 0;
	}
	if (*ptr == 0) {
...
int hdhomerun_device_set_tuner_program(struct hdhomerun_device_t *hd, uint16_t program_number, const char *options)
{
	char name[32], value[128];
	sprintf(name, "/tuner%u/program", hd->tuner);
	if (options) {
		snprintf(value, sizeof(value), "%u %s", program_number, options);
