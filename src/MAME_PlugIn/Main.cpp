...
extern "C" {
	...
	{ 
		...
		if( bIsRuntimePlugin )
			return MAME_PlugIn::PK_DeviceTemplate_get_static();
		else
			return 0;
	}
}
...
extern "C" {
	class Command_Impl *RegisterAsPlugIn(class Router *pRouter,int PK_Device,Logger *pPlutoLogger)
	{
		...
		MAME_PlugIn *pMAME_PlugIn = new MAME_PlugIn(PK_Device, "localhost",true,false,pRouter);
		if( pMAME_PlugIn->m_bQuit_get()|| !pMAME_PlugIn->GetConfig() )
		{
			delete pMAME_PlugIn;
			return NULL;
		}
...
int main(int argc, char* argv[]) 
{
	...
	cout << "MAME_PlugIn, v." << VERSION << endl
	...
	if (bError)
	{
		cout << "A Pluto DCE Device.  See www.plutohome.com/dce for details." << endl
			<< "Usage: MAME_PlugIn [-r Router's IP] [-d My Device ID] [-l dcerouter|stdout|null|filename]" << endl
	...
	try
	{
		if( sLogger=="dcerouter" )
			LoggerWrapper::SetInstance(new ServerLogger(PK_Device, MAME_PlugIn::PK_DeviceTemplate_get_static(), sRouter_IP));
	...
	try
	{
		MAME_PlugIn *pMAME_PlugIn = new MAME_PlugIn(PK_Device, sRouter_IP,true,bLocalMode);
		if ( pMAME_PlugIn->GetConfig() && pMAME_PlugIn->Connect(pMAME_PlugIn->PK_DeviceTemplate_get()) ) 
		{
			g_pCommand_Impl=pMAME_PlugIn;
			g_pDeadlockHandler=DeadlockHandler;
			g_pSocketCrashHandler=SocketCrashHandler;
			LoggerWrapper::GetInstance()->Write(LV_STATUS, "Connect OK");
			pMAME_PlugIn->CreateChildren();
			if( bLocalMode )
				pMAME_PlugIn->RunLocalMode();
			else
			{
				if(pMAME_PlugIn->m_RequestHandlerThread)
					pthread_join(pMAME_PlugIn->m_RequestHandlerThread, NULL);  // This function will return when the device is shutting down
			}
		...
		{
			bAppError = true;
			if( pMAME_PlugIn->m_pEvent && pMAME_PlugIn->m_pEvent->m_pClientSocket && pMAME_PlugIn->m_pEvent->m_pClientSocket->m_eLastError==ClientSocket::cs_err_CannotConnect )
			{
				bAppError = false;
				bReload = false;
