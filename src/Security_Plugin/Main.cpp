...
int main(int argc, char* argv[]) 
{
	...
	try
	{
		if( sLogger=="dcerouter" )
			LoggerWrapper::SetInstance(new ServerLogger(PK_Device, Security_Plugin::PK_DeviceTemplate_get_static(), sRouter_IP));
	...
	try
	{
		...
		if ( pSecurity_Plugin->GetConfig() && pSecurity_Plugin->Connect(pSecurity_Plugin->PK_DeviceTemplate_get()) ) 
		{
			...
			{
				if(pSecurity_Plugin->m_RequestHandlerThread)
		...
		{
			bAppError = true;
			if( pSecurity_Plugin->m_pEvent && pSecurity_Plugin->m_pEvent->m_pClientSocket && pSecurity_Plugin->m_pEvent->m_pClientSocket->m_eLastError==ClientSocket::cs_err_CannotConnect )
			{
				bAppError = false;
				bReload = false;
