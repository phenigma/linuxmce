...
#define  VERSION "2.0.0.45.11052424001"
const char *g_szCompile_Date="Tue May 24 17:12:16 2011";
int g_SvnRevision=24001;
...
extern "C" {
	...
	{ 
		...
		if( bIsRuntimePlugin )
			return Rain8Net::PK_DeviceTemplate_get_static();
		else
			return 0;
	}
}
...
extern "C" {
	class Command_Impl *RegisterAsPlugIn(class Router *pRouter,int PK_Device,Logger *pPlutoLogger)
	{
		...
		Rain8Net *pRain8Net = new Rain8Net(PK_Device, "localhost",true,false,pRouter);
		if( pRain8Net->m_bQuit_get()|| !pRain8Net->GetConfig() )
		{
			delete pRain8Net;
			return NULL;
		}
...
int main(int argc, char* argv[]) 
{
	...
	if (bError)
	{
		cout << "A Pluto DCE Device.  See www.plutohome.com/dce for details." << endl
			<< "Usage: Rain8Net [-r Router's IP] [-d My Device ID] [-l dcerouter|stdout|null|filename]" << endl
	...
	try
	{
		if( sLogger=="dcerouter" )
			LoggerWrapper::SetInstance(new ServerLogger(PK_Device, Rain8Net::PK_DeviceTemplate_get_static(), sRouter_IP));
	...
	try
	{
		Rain8Net *pRain8Net = new Rain8Net(PK_Device, sRouter_IP,true,bLocalMode);
		if ( pRain8Net->GetConfig() && pRain8Net->Connect(pRain8Net->PK_DeviceTemplate_get()) ) 
		{
			g_pCommand_Impl=pRain8Net;
			g_pDeadlockHandler=DeadlockHandler;
			g_pSocketCrashHandler=SocketCrashHandler;
			LoggerWrapper::GetInstance()->Write(LV_STATUS, "Connect OK");
			pRain8Net->CreateChildren();
			if( bLocalMode )
				pRain8Net->RunLocalMode();
			else
			{
				if(pRain8Net->m_RequestHandlerThread)
					pthread_join(pRain8Net->m_RequestHandlerThread, NULL);  // This function will return when the device is shutting down
			}
		...
		{
			bAppError = true;
			if( pRain8Net->m_pEvent && pRain8Net->m_pEvent->m_pClientSocket && pRain8Net->m_pEvent->m_pClientSocket->m_eLastError==ClientSocket::cs_err_CannotConnect )
			{
				bAppError = false;
				bReload = false;
