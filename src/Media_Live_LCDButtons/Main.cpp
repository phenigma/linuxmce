...
extern "C" {
	...
	{ 
		...
		if( bIsRuntimePlugin )
			return Media_Live_LCDButtons::PK_DeviceTemplate_get_static();
		else
			return 0;
	}
}
...
extern "C" {
	class Command_Impl *RegisterAsPlugIn(class Router *pRouter,int PK_Device,Logger *pPlutoLogger)
	{
		...
		Media_Live_LCDButtons *pMedia_Live_LCDButtons = new Media_Live_LCDButtons(PK_Device, "localhost",true,false,pRouter);
		if( pMedia_Live_LCDButtons->m_bQuit_get()|| !pMedia_Live_LCDButtons->GetConfig() )
		{
			delete pMedia_Live_LCDButtons;
			return NULL;
		}
		else
		{
			g_pCommand_Impl=pMedia_Live_LCDButtons;
			g_pDeadlockHandler=Plugin_DeadlockHandler;
			g_pSocketCrashHandler=Plugin_SocketCrashHandler;
		}
		return pMedia_Live_LCDButtons;
	}
}
//<-dceag-plug-e->
...
int main(int argc, char* argv[]) 
{
	...
	cout << "Media_Live_LCDButtons, v." << VERSION << endl
	...
	if (bError)
	{
		cout << "A Pluto DCE Device.  See www.plutohome.com/dce for details." << endl
			<< "Usage: Media_Live_LCDButtons [-r Router's IP] [-d My Device ID] [-l dcerouter|stdout|null|filename]" << endl
	...
	try
	{
		if( sLogger=="dcerouter" )
			LoggerWrapper::SetInstance(new ServerLogger(PK_Device, Media_Live_LCDButtons::PK_DeviceTemplate_get_static(), sRouter_IP));
	...
	try
	{
		Media_Live_LCDButtons *pMedia_Live_LCDButtons = new Media_Live_LCDButtons(PK_Device, sRouter_IP,true,bLocalMode);
		if ( pMedia_Live_LCDButtons->GetConfig() && pMedia_Live_LCDButtons->Connect(pMedia_Live_LCDButtons->PK_DeviceTemplate_get()) ) 
		{
			g_pCommand_Impl=pMedia_Live_LCDButtons;
			g_pDeadlockHandler=DeadlockHandler;
			g_pSocketCrashHandler=SocketCrashHandler;
			LoggerWrapper::GetInstance()->Write(LV_STATUS, "Connect OK");
			pMedia_Live_LCDButtons->CreateChildren();
			if( bLocalMode )
				pMedia_Live_LCDButtons->RunLocalMode();
			else
			{
				if(pMedia_Live_LCDButtons->m_RequestHandlerThread)
					pthread_join(pMedia_Live_LCDButtons->m_RequestHandlerThread, NULL);  // This function will return when the device is shutting down
			}
		...
		{
			bAppError = true;
			if( pMedia_Live_LCDButtons->m_pEvent && pMedia_Live_LCDButtons->m_pEvent->m_pClientSocket && pMedia_Live_LCDButtons->m_pEvent->m_pClientSocket->m_eLastError==ClientSocket::cs_err_CannotConnect )
			{
				bAppError = false;
				bReload = false;
		...
		if( pMedia_Live_LCDButtons->m_bReload )
			bReload=true;
