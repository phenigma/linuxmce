...
Cloud_Interface::Cloud_Interface(int DeviceID, string ServerAddress,bool bConnectEventHandler,bool bLocalMode,class Router *pRouter)
    ...
    //<-dceag-const-e->
{
    qDebug() << Q_FUNC_INFO << "Construction complete";
}
...
bool Cloud_Interface::GetConfig()
{
    if( !Cloud_Interface_Command::GetConfig() )
        return false;
    //<-dceag-getconfig-e->
    m_configurationString =  QString::fromStdString(DATA_Get_Configuration());
    qDebug() << "Configuration " << m_configurationString;
...
//<-dceag-cmdch-e->
{
    QStringList commandList = QString::fromStdString(pMessage->ToString(false)).remove("-o ").remove("\"").split(" ");
    qDebug() << Q_FUNC_INFO << "Command List\n" << commandList.join("\n");
    sCMD_Result = "OK";
}
