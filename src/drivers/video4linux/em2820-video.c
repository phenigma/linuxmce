...
#if 0
/* Control in the saa7113 and not in the em2820 */
		.id = V4L2_CID_HUE,
		.type = V4L2_CTRL_TYPE_INTEGER,
		.name = "Hue",
		.minimum = 0x0,
...
#if 0
	em2820_write_regs_req(dev,0x00,0x0e,"\xC0",1);
	em2820_write_regs_req(dev,0x00,0x0f,"\x80",1);
	em2820_write_regs_req(dev,0x00,0x11,"\x51",1);
#endif
...
void em2820_config_i2c(struct em2820 *dev)
{
	...
	if (dev->decoder == EM2820_SAA7113) {
		...
		em2820_vdi.data = saa7113_i2c_init;
		em2820_vdi.len = sizeof(saa7113_i2c_init);
	}
#endif
		 ...
		 loff_t * f_pos)
{
	...
	if (dev->type == V4L2_BUF_TYPE_VIDEO_CAPTURE) {
		em2820_videodbg("V4l2_Buf_type_videocapture is set\n");
	}
	...
	case VIDIOC_QUERYCTRL:
		{
			...
			if (dev->decoder == EM2820_TVP5150) {
				em2820_i2c_call_clients(dev,cmd,arg);
				return 0;
			}
#endif
