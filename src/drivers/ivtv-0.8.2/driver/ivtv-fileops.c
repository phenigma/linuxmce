...
static int ivtv_check_digitizer_sync(struct ivtv *itv)
{
	...
	itv->video_dec_func(itv, VIDIOC_G_TUNER, &vt);
...
static int ivtv_reset_digitizer(struct ivtv *itv)
{
	...
	if (reset) {
		...
		ret = itv->video_dec_func(itv, VIDIOC_INT_RESET, &x);
	}
 ...
 */
	...
	height = itv->params.height;
	width = itv->params.width;
...
static int ivtv_read(struct file *filp, char *ubuf, size_t count, loff_t * pos)
{
	...
	do {
		...
		if (type != IVTV_DEC_STREAM_TYPE_VBI &&
			...
			IVTV_DEBUG_WARN("Check digitizer Sync\n");
				...
				struct list_head *full_list, int block)
{
	...
	for (;;) {
		...
		if ((((!test_bit(FW_RESET_LOCK, &itv->r_flags)) &&
		    ...
		    (ivtv_buf_fill_lock(st, 0, BUF_USED) >= st->buf_min &&
...
unsigned int ivtv_v4l2_dec_poll(struct file *filp, poll_table * wait)
{
	...
	if (ivtv_buf_fill_lock(st, 0, BUF_USED) <=
	    st->buf_min - itv->dma_cfg.dec_min_xfer) {
...
int ivtv_v4l2_open(struct inode *inode, struct file *filp)
{
	...
	if (item->type == IVTV_ENC_STREAM_TYPE_RAD) {
		...
		ivtv_call_i2c_clients(itv, AUDC_SET_RADIO, 0);
...
void ivtv_mute(struct ivtv *itv)
{
	...
	if (atomic_read(&itv->capturing))
        	ivtv_vapi(itv, CX2341X_ENC_MUTE_AUDIO, 1);
...
void ivtv_unmute(struct ivtv *itv)
{
	...
	if (atomic_read(&itv->capturing)) {
		ivtv_vapi(itv, CX2341X_ENC_MISC, 12);
		ivtv_vapi(itv, CX2341X_ENC_MUTE_AUDIO, 0);
	}
