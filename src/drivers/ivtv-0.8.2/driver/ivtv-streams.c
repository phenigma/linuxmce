		     ...
		     int buffers, int bufsize, int dma)
{
	...
	if (buffers == 0) {
		IVTV_DEBUG_INFO("Create %s%s stream\n",
...
static int ivtv_reg_dev(struct ivtv *itv, int streamtype, int minor, int reg_type)
{
	...
	switch (reg_type) {
		case VFL_TYPE_GRABBER:
			IVTV_INFO("Registered device video%d for %s\n", 
				s->v4l2dev->minor, ivtv_stream_name(streamtype));
			break;
		case VFL_TYPE_RADIO:
			IVTV_INFO("Registered device radio%d for %s\n", 
				s->v4l2dev->minor - MINOR_VFL_TYPE_RADIO_MIN,
			       	ivtv_stream_name(streamtype));
			break;
		case VFL_TYPE_VBI:
