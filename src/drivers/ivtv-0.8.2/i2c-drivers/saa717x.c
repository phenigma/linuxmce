...
/* ----------------------------------------------------------------------- */
/*
 * Now,This routine and  value is larger than 0xff,
 * happend problems cause of invalid endian,please fix! 20060928 K.Ohta
 */
...
static int saa717x_write(struct i2c_client *client, u32 reg, u32 value)
{
	...
	if (fw_addr) { /* Fix2 20061006 K.Ohta */
		mm1[4] = (value >> 16) & 0xff; // Apply data-tables.
		mm1[3] = (value >> 8) & 0xff;// Apply data-tables.
		mm1[2] = value & 0xff;// Apply data-tables.
	} else {
	        mm1[2] = value & 0xff;
	}
	//msg.len = fw_addr ? 6 : 3;
...
{
        int i=0; // Because imcompatible effect with gcc4.1.1 @ amd64
	while ((data[i+0] != 0) || (data[i+1] != 0)) { // Fix because writing register address 0x0000 while initialization 20061107
		saa717x_write(client, data[i+0], data[i+1]);
		i += 2;
	}
}
...
static u32 saa717x_read(struct i2c_client *client, u32 reg)
{
	...
	msgs[0].len = 2;
	msgs[0].buf = mm1;
       	//msgs[1].len = fw_addr ? 4 : 1;
        msgs[1].len = fw_addr ? 3 : 1; // Multibyte Registers contains *only 3bytes */
       	msgs[1].buf = mm2;
...
/* ----------------------------------------------------------------------- */
/*
 * Now,This table and  value is larger than 0xff,
 * happend problems cause of invalid endian,please fix! 20060928 K.Ohta
 */
...
static int saa717x_command(struct i2c_client *client, unsigned int cmd, void *arg)
{
	...
	case VIDIOC_INT_S_VIDEO_ROUTING: /* Rename from VIDIOC_S_INPUT,another arg. 20060706 K.Ohta Pls. Merge it! (-_-# */
	{
	        struct v4l2_routing *route = arg;
	        int inp = route->input;
		       ...
		       int                 kind)
{
	...
	if (id != 0xc2 && id != 0x32 && id != 0xf2) {
		v4l_dbg(1, debug, client, "saa717x not found (id=%02x)\n", id);
		kfree(client);
		return 0;
	}
	if (id == 0xc2) p = "saa7173";
	else if (id == 0x32) p = "saa7174A";
	else p = "saa7171";
	v4l_info(client, "%s found @ 0x%x (%s)\n", p, address << 1, adapter->name);
