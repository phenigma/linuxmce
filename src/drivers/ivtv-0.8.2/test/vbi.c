...
int main(int argc, char **argv)
{
	...
	while (1) {
		int ch = getopt(argc, argv, "d:t:h");

		if (ch == -1)
			break;
		switch (ch) {
		case 'd':
		        strncpy(device, optarg , 99 );
			device[99] = 0;
			if (strlen(optarg) > 99 ) {
				fprintf(stderr, "device too long [max 99 char], truncated to %s" , device);
			}
			if (optarg[0] >= '0' && optarg[0] <= '9')
				sprintf(device, "/dev/vbi%s", optarg);
			break;
		case 't':
			timeout = atol(optarg);
			break;
		default:
			fprintf(stderr, "usage: %s [-d <dev>] [-t <secs>] [-h]\n"
