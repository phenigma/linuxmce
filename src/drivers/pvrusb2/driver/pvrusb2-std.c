/*
 *
 *  $Id: pvrusb2-std.c 1882 2008-02-09 15:28:54Z isely $
 *
...
#ifndef V4L2_STD_NTSC_443
#define V4L2_STD_NTSC_443 0
#endif
#ifndef V4L2_STD_NTSC_M_KR
#define V4L2_STD_NTSC_M_KR 0
#endif
#ifndef V4L2_STD_SECAM_LC
#define V4L2_STD_SECAM_LC 0
#endif
					   ...
					   v4l2_std_id id)
{
	...
	*countptr = 0;
	...
	/* Prioritize NTSC choices over PAL if both are present (need this
	   for unusual cases of a device which commonly supports an NTSC
	   choice but might also handle a less common PAL choice. */
	cmsk = CSTD_NTSC;
	cmsk &= id;
	id &= ~cmsk;
	for (idmsk = 1; cmsk && (idx < std_cnt); idmsk <<= 1) {
		if (!(idmsk & cmsk)) continue;
		cmsk &= ~idmsk;
