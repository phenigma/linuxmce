/*
 *
 *  $Id: pvrusb2-sysfs-cd.c 2138 2009-01-15 07:51:54Z isely $
 *
...
struct pvr2_sysfs {
	...
	struct class_device_attribute attr_hdw_name;
	struct class_device_attribute attr_hdw_desc;
...
struct pvr2_sysfs_ctl_item {
	...
	struct class_device_attribute attr_def;
...
static ssize_t show_def(int id,struct class_device *class_dev,char *buf)
{
	struct pvr2_ctrl *cptr;
	struct pvr2_sysfs *sfp;
	int val;
	...
	cptr = pvr2_hdw_get_ctrl_by_index(sfp->channel.hdw,id);
	if (!cptr) return -EINVAL;
	ret = pvr2_ctrl_get_def(cptr,&val);
...
CREATE_SHOW_INSTANCE(show_def,ctl_id) \
...
struct pvr2_sysfs_func_set {
	...
	ssize_t (*show_def)(struct class_device *,char *);
...
#ifdef PVR2_ENABLE_OLD_MODULE_OWNER
	cip->attr_def.attr.owner = THIS_MODULE;
#endif
	cip->attr_def.attr.name = "def_val";
	cip->attr_def.attr.mode = S_IRUGO;
	cip->attr_def.show = fp->show_def;
...
static void pvr2_sysfs_add_debugifc(struct pvr2_sysfs *sfp)
{
	...
	if (ret < 0) {
		pvr2_trace(PVR2_TRACE_ERROR_LEGS,
			   "class_device_create_file error: %d",
			   ret);
	} else {
...
static ssize_t hdw_name_show(struct class_device *class_dev, char *buf)
{
	struct pvr2_sysfs *sfp;
	sfp = (struct pvr2_sysfs *)class_dev->class_data;
	if (!sfp) return -EINVAL;
...
static ssize_t hdw_desc_show(struct class_device *class_dev, char *buf)
{
	struct pvr2_sysfs *sfp;
	sfp = (struct pvr2_sysfs *)class_dev->class_data;
	if (!sfp) return -EINVAL;
			     ...
			     struct pvr2_sysfs_class *class_ptr)
{
	...
	snprintf(class_dev->class_id, BUS_ID_SIZE, "%s",
...
#ifdef PVR2_ENABLE_OLD_MODULE_OWNER
	sfp->attr_hdw_name.attr.owner = THIS_MODULE;
#endif
...
#ifdef PVR2_ENABLE_OLD_MODULE_OWNER
	sfp->attr_hdw_desc.attr.owner = THIS_MODULE;
#endif
