...
MODULE_PARM(ivtv_pci_latency, "i");
...
MODULE_PARM_DESC(ivtv_efw,
		 ...
		 "\t\t\tDefault: " CX2341X_FIRM_ENC_FILENAME);
MODULE_PARM_DESC(ivtv_dfw,
		 "Decoder firmware image\n"
		 "\t\t\tDefault: " CX2341X_FIRM_DEC_FILENAME);
...
static void ivtv_load_and_init_modules(struct ivtv *itv)
{
	...
	case IVTV_CARD_CX23416GYC:
		ctrl.value = (UPD64031A_SET_PARAM1<<UPD640XX_SET_COMMAND_SHIFT) |
			UPD64031A_VERTICAL_EXTERNAL | UPD64031A_LIN_GROFF |
			UPD64031A_LIN_GR_THROUGH | UPD64031A_LIN_3DYCS_COMPOSITE;
		break;
	case IVTV_CARD_TG5000TV:
		ctrl.value = (UPD64031A_SET_PARAM1<<UPD640XX_SET_COMMAND_SHIFT)
			|UPD64031A_VERTICAL_EXTERNAL;

		break;
	}
	if ((hw & IVTV_HW_UPD64031A) && ctrl.value != 0)
		ivtv_upd64031a(itv, VIDIOC_S_CTRL, &ctrl);

	ctrl.id = V4L2_CID_PRIVATE_BASE;
	ctrl.value = 0;
	switch (itv->card->type) {
	case IVTV_CARD_CX23416GYC:
		ctrl.value = (UPD64083_SET_PARAM1<<UPD640XX_SET_COMMAND_SHIFT) |
			(UPD64083_SET_COMP_YCSMODE | 0x00 | UPD64083_COMP_USE_EXT_Y_ADC);
		break;
	case IVTV_CARD_CX23416GYC_NOGR:
	case IVTV_CARD_VA2000MAX_SNT6:
		ctrl.value = (UPD64083_SET_PARAM1<<UPD640XX_SET_COMMAND_SHIFT) |
			UPD64083_TUN_USE_INT_Y_ADC;
		break;
	}
	if ((hw & IVTV_HW_UPD6408X)  && ctrl.value != 0)
		ivtv_upd64083(itv, VIDIOC_S_CTRL, &ctrl);
				...
				const struct pci_device_id *pci_id)
{
	...
	itv->gpio_dir = itv->reg_mem + IVTV_REG_GPIO_DIR_OFFSET;
	itv->gpio_out = itv->reg_mem + IVTV_REG_GPIO_OUT_OFFSET;
	itv->gpio_in  = itv->reg_mem + IVTV_REG_GPIO_IN_OFFSET;
	ivtv_gpio_init(itv);
	...
	itv->codec.framespergop = itv->is_60hz ? 15 : 12;
	...
	ivtv_v4l2_ioctls(itv, NULL, 0, VIDIOC_S_INPUT, &video_input);
	...
	ivtv_v4l2_ioctls(itv, NULL, 0, VIDIOC_S_STD, &itv->tuner_std);
        ...
        } else if (itv->options.tuner == TUNER_PANASONIC_VP27) {
		int cfg = TDA9887_INTERCARRIER;
