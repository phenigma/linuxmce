...
void ivtv_setup_v4l2_encode_stream(struct ivtv *itv, int type)
{
	...
	/* assign stream type */
	ivtv_vapi(itv, CX2341X_ENC_SET_STREAM_TYPE, 1, itv->codec.stream_type);

	/* assign output port */
	ivtv_vapi(itv, CX2341X_ENC_SET_OUTPUT_PORT, 1, 0);	/* 0 = Memory */

	/* assign framerate */
	ivtv_vapi(itv, CX2341X_ENC_SET_FRAME_RATE, 1, itv->is_50hz);

	/* assign frame size */
	ivtv_vapi(itv, CX2341X_ENC_SET_FRAME_SIZE, 2, itv->height, itv->width);

	/* assign aspect ratio */
	ivtv_vapi(itv, CX2341X_ENC_SET_ASPECT_RATIO, 1, itv->codec.aspect);
	...
	/* assign bitrates */
	ivtv_vapi(itv, CX2341X_ENC_SET_BIT_RATE, 5, itv->codec.bitrate_mode,
		  itv->codec.bitrate,	/* bps */
		  itv->codec.bitrate_peak / 400,	/* peak/400 */
		  0x00, 0x00, 0x70);	/* encoding buffer, ckennedy */

	/* assign gop properties */
	ivtv_vapi(itv, CX2341X_ENC_SET_GOP_PROPERTIES, 2,
		  itv->codec.framespergop, itv->codec.bframes);

	/* assign 3 2 pulldown */
	ivtv_vapi(itv, CX2341X_ENC_SET_3_2_PULLDOWN, 1, itv->codec.pulldown);

	/* assign gop closure */
	ivtv_vapi(itv, CX2341X_ENC_SET_GOP_CLOSURE, 1, itv->codec.gop_closure);

	/* assign audio properties */
	ivtv_vapi(itv, CX2341X_ENC_SET_AUDIO_PROPERTIES, 1,
		  itv->codec.audio_bitmask);
	ivtv_dualwatch_start_encoding(itv);

	/* assign dnr filter mode */
	ivtv_vapi(itv, CX2341X_ENC_SET_DNR_FILTER_MODE, 2,
	...
	ivtv_vapi(itv, CX2341X_ENC_SET_DNR_FILTER_PROPS, 2,
		  itv->codec.dnr_spatial, dnr_temporal);

	/* assign coring levels (luma_h, luma_l, chroma_h, chroma_l) */
	ivtv_vapi(itv, CX2341X_ENC_SET_CORING_LEVELS, 4, 0, 255, 0, 255);

	/* assign spatial filter type: luma_t: 1 = horiz_only, chroma_t: 1 = horiz_only */
	ivtv_vapi(itv, CX2341X_ENC_SET_SPATIAL_FILTER_TYPE, 2, 1, 1);

	/* assign frame drop rate */
	ivtv_vapi(itv, CX2341X_ENC_SET_FRAME_DROP_RATE, 1, 0);
}
...
int ivtv_passthrough_mode(struct ivtv *itv, int enable)
{
	...
	if (enable) {
		...
		fmt.type = V4L2_BUF_TYPE_SLICED_VBI_CAPTURE;
                vbi_setup_lcr(itv, itv->vbi_passthrough, itv->is_50hz, &fmt.fmt.sliced);
                itv->video_dec_func(itv, VIDIOC_S_FMT, &fmt);

                /* Start Passthrough Mode */
		x = ivtv_vapi(itv, CX2341X_ENC_START_CAPTURE, 2, 2, 11);
	...
	down(&dec_stream->mlock);
	x = ivtv_vapi(itv, CX2341X_ENC_STOP_CAPTURE, 3, 1, 1, 11);
	x = ivtv_vapi(itv, CX2341X_DEC_STOP_PLAYBACK, 3, 1, 0, 0);
	if (x)
		IVTV_DEBUG_WARN(
...
static void ivtv_vbi_setup(struct ivtv *itv, int mode)
{
		...
		!raw && itv->vbi_insert_mpeg) 
	{
		itv->codec.stream_type = 0;
        	...
        	/* assign stream type */
        	ivtv_vapi(itv, CX2341X_ENC_SET_STREAM_TYPE, 1, 
	...
	if (raw) {
        ...
        ivtv_vapi(itv, CX2341X_ENC_SET_NUM_VSYNC_LINES,
	...
	x = ivtv_api(itv, itv->enc_mbox, &itv->enc_msem, CX2341X_ENC_SET_VBI_CONFIG,
		     &result, 7, &data[0]);
	if (x)
...
{
	ivtv_vapi(itv, CX2341X_ENC_UNKNOWN, 1, 0);
...
int ivtv_start_v4l2_encode_stream(struct ivtv *itv, int type)
{
	...
	clear_bit(IVTV_F_S_OVERFLOW, &st->s_flags);
		...
		ivtv_api(itv, itv->enc_mbox, &itv->enc_msem,
	     		CX2341X_ENC_SET_DMA_BLOCK_SIZE, &result, 2, &data[0]);
		...
		ivtv_api(itv, itv->enc_mbox, &itv->enc_msem,
	 		CX2341X_ENC_SET_PGM_INDEX_INFO, &result, 2, &data[0]);
	...
	if (atomic_read(&itv->capturing) == 0) {
		ivtv_vapi(itv, CX2341X_ENC_MISC, 2, 3, 
				(itv->hw_flags & IVTV_HW_CX25840) ? 1 : 0);
	}
...
int ivtv_setup_v4l2_decode_stream(struct ivtv *itv, int type)
{
	...
	ivtv_vapi(itv, CX2341X_DEC_SET_AUDIO_MODE, 2, 0, 0);
	...
	data[0] = itv->dec_options.prebuffer;	/* 0 = no prebuffering,
						   1 = enabled, see docs */
	ivtv_vapi(itv, CX2341X_DEC_SET_PREBUFFERING, 1, itv->dec_options.prebuffer);

	/* extract from user packets */
	data[0] = itv->dec_vbi_pkt;
        ivtv_api(itv, itv->dec_mbox, &itv->dec_msem,
                     CX2341X_DEC_EXTRACT_VBI, &result, 2, &data[0]);
...
int ivtv_stop_capture(struct ivtv *itv, int type)
{
	...
	/* end_capture */
	x = ivtv_vapi(itv, CX2341X_ENC_STOP_CAPTURE, 3, stopmode,	
	...
	if (atomic_read(&itv->capturing) - 1 == 0) {
		...
		if (test_and_clear_bit(IVTV_F_I_DIG_RST, &itv->i_flags)) {
			/* type: 0 = refresh */
			x = ivtv_vapi(itv, CX2341X_ENC_SET_EVENT_NOTIFICATION, 4, 0, 0,	/* on/off: 0 = off */
			...
			ivtv_set_irq_mask(itv, IVTV_IRQ_ENC_VIM_RST);
                ...
                ivtv_vapi(itv, CX2341X_ENC_MISC, 2, 3, 0);
