...
{
	/* check enc firmware */
	if (ivtv_vapi(itv, CX2341X_ENC_PING_FW, 0)) {
...
{
	/* check dec firmware */
	if (ivtv_vapi(itv, CX2341X_DEC_PING_FW, 0)) {
...
retry:
	if (retries && request_firmware(&fw, fn, FWDEV(itv)) == 0) {
		int i;
		u32 *dst = (u32 *)mem;
		...
		release_firmware(fw);
		IVTV_INFO("loaded %s firmware (%ld bytes)\n", fn, size);
		return size;
	}
...
static int ivtv_enc_firmware_copy(struct ivtv *itv)
{
	...
	if (load_fw_direct(ivtv_efw ? ivtv_efw : CX2341X_FIRM_ENC_FILENAME,
			   (char *)(itv->enc_mem), itv, IVTV_FW_ENC_SIZE) !=
	    IVTV_FW_ENC_SIZE) {
...
static int ivtv_dec_firmware_copy(struct ivtv *itv)
{
	...
	if (load_fw_direct(ivtv_dfw ? ivtv_dfw : CX2341X_FIRM_DEC_FILENAME,
			   (char *)(itv->dec_mem), itv,IVTV_FW_DEC_SIZE) !=
	    IVTV_FW_DEC_SIZE) {
...
ivtv_init_mpeg_decoder(struct ivtv *itv)
{
	...
	if ((readbytes = load_fw_direct(IVTV_DECODE_INIT_MPEG_FILENAME, (char *)mem_offset, itv, IVTV_DECODE_INIT_MPEG_SIZE)) <= 0)
	{
	...
	ivtv_vapi(itv, CX2341X_DEC_STOP_PLAYBACK, 4, 0,
                  itv->dec_options.pts_low, itv->dec_options.pts_hi, 1);
}
