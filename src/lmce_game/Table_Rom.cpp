...
return m_PK_Rom;}
string Row_Rom::IK_Hash_get(){PLUTO_SAFETY_LOCK_ERRORSONLY(sl,table->database->m_DBMutex);
...
void Row_Rom::IK_Hash_set(string val){PLUTO_SAFETY_LOCK_ERRORSONLY(sl,table->database->m_DBMutex);

m_IK_Hash = val; is_modified=true; is_null[1]=false;}
...
db_wrapper_real_escape_string(table->database->m_pDB, buf, m_IK_Hash.c_str(), (unsigned long) min((size_t)120,m_IK_Hash.size()));
string s=string()+"\""+buf+"\"";
delete[] buf;
return s;
}
...
values_list_comma_separated = values_list_comma_separated + pRow->PK_Rom_asSQL()+", "+pRow->IK_Hash_asSQL();

	
		string query = "insert into Rom (`PK_Rom`, `IK_Hash`) values ("+
...
update_values_list = update_values_list + "`PK_Rom`="+pRow->PK_Rom_asSQL()+", `IK_Hash`="+pRow->IK_Hash_asSQL();
...
pRow->m_IK_Hash = string(row[1],lengths[1]);
}
...
void Row_Rom::Game_GameSystem_Rom_FK_Rom_getrows(vector <class Row_Game_GameSystem_Rom*> *rows)
{
PLUTO_SAFETY_LOCK_ERRORSONLY(sl,table->database->m_DBMutex);
