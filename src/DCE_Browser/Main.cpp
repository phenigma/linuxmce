...
extern "C" {
	...
	{ 
		...
		if( bIsRuntimePlugin )
			return DCE_Browser::PK_DeviceTemplate_get_static();
		else
			return 0;
	}
}
...
extern "C" {
	class Command_Impl *RegisterAsPlugIn(class Router *pRouter,int PK_Device,Logger *pPlutoLogger)
	{
		...
		DCE_Browser *pDCE_Browser = new DCE_Browser(PK_Device, "localhost",true,false,pRouter);
		if( pDCE_Browser->m_bQuit_get()|| !pDCE_Browser->GetConfig() )
		{
			delete pDCE_Browser;
			return NULL;
		}
...
int main(int argc, char* argv[]) 
{
	...
	cout << "DCE_Browser, v." << VERSION << endl
	...
	if (bError)
	{
		cout << "A Pluto DCE Device.  See www.plutohome.com/dce for details." << endl
			<< "Usage: DCE_Browser [-r Router's IP] [-d My Device ID] [-l dcerouter|stdout|null|filename]" << endl
	...
	try
	{
		if( sLogger=="dcerouter" )
			LoggerWrapper::SetInstance(new ServerLogger(PK_Device, DCE_Browser::PK_DeviceTemplate_get_static(), sRouter_IP));
	...
	try
	{
		DCE_Browser *pDCE_Browser = new DCE_Browser(PK_Device, sRouter_IP,true,bLocalMode);
		if ( pDCE_Browser->GetConfig() && pDCE_Browser->Connect(pDCE_Browser->PK_DeviceTemplate_get()) ) 
		{
			g_pCommand_Impl=pDCE_Browser;
			g_pDeadlockHandler=DeadlockHandler;
			g_pSocketCrashHandler=SocketCrashHandler;
			LoggerWrapper::GetInstance()->Write(LV_STATUS, "Connect OK");
			pDCE_Browser->CreateChildren();
			if( bLocalMode )
				pDCE_Browser->RunLocalMode();
			else
			{
				if(pDCE_Browser->m_RequestHandlerThread)
					pthread_join(pDCE_Browser->m_RequestHandlerThread, NULL);  // This function will return when the device is shutting down
			}
		...
		{
			bAppError = true;
			if( pDCE_Browser->m_pEvent && pDCE_Browser->m_pEvent->m_pClientSocket && pDCE_Browser->m_pEvent->m_pClientSocket->m_eLastError==ClientSocket::cs_err_CannotConnect )
			{
				bAppError = false;
				bReload = false;
