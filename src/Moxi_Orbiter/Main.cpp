...
extern "C" {
	...
	{ 
		...
		if( bIsRuntimePlugin )
			return Moxi_Orbiter::PK_DeviceTemplate_get_static();
		else
			return 0;
	}
}
...
extern "C" {
	class Command_Impl *RegisterAsPlugIn(class Router *pRouter,int PK_Device,Logger *pPlutoLogger)
	{
		...
		Moxi_Orbiter *pMoxi_Orbiter = new Moxi_Orbiter(PK_Device, DEVICETEMPLATE_Moxi_Orbiter_CONST, "localhost");
		if( pMoxi_Orbiter->m_bQuit_get()|| !pMoxi_Orbiter->GetConfig() )
		{
			delete pMoxi_Orbiter;
			return NULL;
		}
		else
		{
			g_pCommand_Impl=pMoxi_Orbiter;
			g_pDeadlockHandler=Plugin_DeadlockHandler;
			g_pSocketCrashHandler=Plugin_SocketCrashHandler;
		}
		return pMoxi_Orbiter;
	}
}
//<-dceag-plug-e->
...
int main(int argc, char* argv[]) 
{
	...
	cout << "Moxi_Orbiter, v." << VERSION << endl
	...
	if (bError)
	{
		cout << "A Pluto DCE Device.  See www.plutohome.com/dce for details." << endl
			<< "Usage: Moxi_Orbiter [-r Router's IP] [-d My Device ID] [-l dcerouter|stdout|null|filename]" << endl
	...
	try
	{
		if( sLogger=="dcerouter" )
			LoggerWrapper::SetInstance(new ServerLogger(PK_Device, Moxi_Orbiter::PK_DeviceTemplate_get_static(), sRouter_IP));
	...
	try
	{
		Moxi_Orbiter *pMoxi_Orbiter = new Moxi_Orbiter(PK_Device, DEVICETEMPLATE_Moxi_Orbiter_CONST, sRouter_IP);
		if ( pMoxi_Orbiter->GetConfig() && pMoxi_Orbiter->Connect(pMoxi_Orbiter->PK_DeviceTemplate_get()) ) 
		{
			g_pCommand_Impl=pMoxi_Orbiter;
			...
			pMoxi_Orbiter->Initialize(gtSDLGraphic);
			pMoxi_Orbiter->CreateChildren();

			if(Simulator::GetInstance()->m_bEnableGenerator)
				Simulator::GetInstance()->StartRandomEventGenerator();

			pMoxi_Orbiter->Renderer()->EventLoop();

			if( bLocalMode )
				pMoxi_Orbiter->RunLocalMode();
			else
			{
				if(pMoxi_Orbiter->m_RequestHandlerThread)
					pthread_join(pMoxi_Orbiter->m_RequestHandlerThread, NULL);  // This function will return when the device is shutting down
			}
