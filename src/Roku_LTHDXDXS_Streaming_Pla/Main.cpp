...
extern "C" {
	...
	{ 
		...
		if( bIsRuntimePlugin )
			return Roku_LTHDXDXS_Streaming_Pla::PK_DeviceTemplate_get_static();
		else
			return 0;
	}
}
...
extern "C" {
	class Command_Impl *RegisterAsPlugIn(class Router *pRouter,int PK_Device,Logger *pPlutoLogger)
	{
		...
		Roku_LTHDXDXS_Streaming_Pla *pRoku_LTHDXDXS_Streaming_Pla = new Roku_LTHDXDXS_Streaming_Pla(PK_Device, "localhost",true,false,pRouter);
		if( pRoku_LTHDXDXS_Streaming_Pla->m_bQuit_get()|| !pRoku_LTHDXDXS_Streaming_Pla->GetConfig() )
		{
			delete pRoku_LTHDXDXS_Streaming_Pla;
			return NULL;
		}
		else
		{
			g_pCommand_Impl=pRoku_LTHDXDXS_Streaming_Pla;
			g_pDeadlockHandler=Plugin_DeadlockHandler;
			g_pSocketCrashHandler=Plugin_SocketCrashHandler;
		}
		return pRoku_LTHDXDXS_Streaming_Pla;
	}
}
//<-dceag-plug-e->
...
int main(int argc, char* argv[]) 
{
	...
	cout << "Roku_LTHDXDXS_Streaming_Pla, v." << VERSION << endl
	...
	if (bError)
	{
		cout << "A Pluto DCE Device.  See www.plutohome.com/dce for details." << endl
			<< "Usage: Roku_LTHDXDXS_Streaming_Pla [-r Router's IP] [-d My Device ID] [-l dcerouter|stdout|null|filename]" << endl
	...
	try
	{
		if( sLogger=="dcerouter" )
			LoggerWrapper::SetInstance(new ServerLogger(PK_Device, Roku_LTHDXDXS_Streaming_Pla::PK_DeviceTemplate_get_static(), sRouter_IP));
	...
	try
	{
		Roku_LTHDXDXS_Streaming_Pla *pRoku_LTHDXDXS_Streaming_Pla = new Roku_LTHDXDXS_Streaming_Pla(PK_Device, sRouter_IP,true,bLocalMode);
		if ( pRoku_LTHDXDXS_Streaming_Pla->GetConfig() && pRoku_LTHDXDXS_Streaming_Pla->Connect(pRoku_LTHDXDXS_Streaming_Pla->PK_DeviceTemplate_get()) ) 
		{
			g_pCommand_Impl=pRoku_LTHDXDXS_Streaming_Pla;
			g_pDeadlockHandler=DeadlockHandler;
			g_pSocketCrashHandler=SocketCrashHandler;
			LoggerWrapper::GetInstance()->Write(LV_STATUS, "Connect OK");
			pRoku_LTHDXDXS_Streaming_Pla->CreateChildren();
			if( bLocalMode )
				pRoku_LTHDXDXS_Streaming_Pla->RunLocalMode();
			else
			{
				if(pRoku_LTHDXDXS_Streaming_Pla->m_RequestHandlerThread)
					pthread_join(pRoku_LTHDXDXS_Streaming_Pla->m_RequestHandlerThread, NULL);  // This function will return when the device is shutting down
			}
		...
		{
			bAppError = true;
			if( pRoku_LTHDXDXS_Streaming_Pla->m_pEvent && pRoku_LTHDXDXS_Streaming_Pla->m_pEvent->m_pClientSocket && pRoku_LTHDXDXS_Streaming_Pla->m_pEvent->m_pClientSocket->m_eLastError==ClientSocket::cs_err_CannotConnect )
			{
				bAppError = false;
				bReload = false;
		...
		if( pRoku_LTHDXDXS_Streaming_Pla->m_bReload )
			bReload=true;
