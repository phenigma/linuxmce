...
#define  VERSION "2.0.0.44.0805082200"
const char *g_szCompile_Date="Wed Nov 18 19:19:05 2009";
int g_SvnRevision=22481;
...
extern "C" {
	...
	{ 
		...
		if( bIsRuntimePlugin )
			return MPlayer_Player::PK_DeviceTemplate_get_static();
		else
			return 0;
	}
}
...
extern "C" {
	class Command_Impl *RegisterAsPlugIn(class Router *pRouter,int PK_Device,Logger *pPlutoLogger)
	{
		...
		MPlayer_Player *pMPlayer_Player = new MPlayer_Player(PK_Device, "localhost",true,false,pRouter);
		if( pMPlayer_Player->m_bQuit_get()|| !pMPlayer_Player->GetConfig() )
		{
			delete pMPlayer_Player;
			return NULL;
		}
		else
		{
			g_pCommand_Impl=pMPlayer_Player;
			g_pDeadlockHandler=Plugin_DeadlockHandler;
			g_pSocketCrashHandler=Plugin_SocketCrashHandler;
		}
		return pMPlayer_Player;
	}
}
//<-dceag-plug-e->
...
int main(int argc, char* argv[]) 
{
	...
	cout << "MPlayer_Player, v." << VERSION << endl
	...
	if (bError)
	{
		cout << "A Pluto DCE Device.  See www.plutohome.com/dce for details." << endl
			<< "Usage: MPlayer_Player [-r Router's IP] [-d My Device ID] [-l dcerouter|stdout|null|filename]" << endl
	...
	try
	{
		if( sLogger=="dcerouter" )
			LoggerWrapper::SetInstance(new ServerLogger(PK_Device, MPlayer_Player::PK_DeviceTemplate_get_static(), sRouter_IP));
	...
	try
	{
		MPlayer_Player *pMPlayer_Player = new MPlayer_Player(PK_Device, sRouter_IP,true,bLocalMode);
		if ( pMPlayer_Player->GetConfig() && pMPlayer_Player->Connect(pMPlayer_Player->PK_DeviceTemplate_get()) ) 
		{
			g_pCommand_Impl=pMPlayer_Player;
			g_pDeadlockHandler=DeadlockHandler;
			g_pSocketCrashHandler=SocketCrashHandler;
			LoggerWrapper::GetInstance()->Write(LV_STATUS, "Connect OK");
			pMPlayer_Player->CreateChildren();
			if( bLocalMode )
				pMPlayer_Player->RunLocalMode();
			else
			{
				if(pMPlayer_Player->m_RequestHandlerThread)
					pthread_join(pMPlayer_Player->m_RequestHandlerThread, NULL);  // This function will return when the device is shutting down
			}
		...
		{
			bAppError = true;
			if( pMPlayer_Player->m_pEvent && pMPlayer_Player->m_pEvent->m_pClientSocket && pMPlayer_Player->m_pEvent->m_pClientSocket->m_eLastError==ClientSocket::cs_err_CannotConnect )
			{
				bAppError = false;
				bReload = false;
