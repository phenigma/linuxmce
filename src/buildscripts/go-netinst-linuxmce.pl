...
`mkisofs -V "$version" -J -o "$build_dir/installation-cd.iso" -r -b isolinux/isolinux.bin -c isolinux/boot.cat -no-emul-boot -boot-load-size 4 -boot-info-table image-netinst/`;
