...
if ( defined $ARGV[1] && $ARGV[1] eq "cache" )
{
	...
	# Place monster cd boot logo
	`cp image-netinst/isolinux/monster.rle image-netinst/isolinux/splash.rle`;
...
`mkisofs -V "$version" -J -o "$build_dir/installation-cd-1-$version.monster.iso" -r -b isolinux/isolinux.bin -c isolinux/boot.cat -no-emul-boot -boot-load-size 4 -boot-info-table image-netinst/`;

print "Building iso image for bonuscd (cd2)";
`/bin/bash /home/installation-cd-kernel-2.6.12/build-bonuscd.sh monster $version`;
if (-e "message.txt")
{
