...
int main(int argc, char* argv[]) 
{
	...
	if (bError)
	{
		cout << "A Pluto DCE Device.  See www.plutohome.com/dce for details." << endl
			<< "Usage: MythTV_PlugIn [-r Router's IP] [-d My Device ID] [-l dcerouter|stdout|null|filename]" << endl
	...
	try
	{
		if( sLogger=="dcerouter" )
			LoggerWrapper::SetInstance(new ServerLogger(PK_Device, MythTV_PlugIn::PK_DeviceTemplate_get_static(), sRouter_IP));
	...
	try
	{
		...
		if ( pMythTV_PlugIn->GetConfig() && pMythTV_PlugIn->Connect(pMythTV_PlugIn->PK_DeviceTemplate_get()) ) 
		{
			...
			{
				if(pMythTV_PlugIn->m_RequestHandlerThread)
		...
		{
			bAppError = true;
			if( pMythTV_PlugIn->m_pEvent && pMythTV_PlugIn->m_pEvent->m_pClientSocket && pMythTV_PlugIn->m_pEvent->m_pClientSocket->m_eLastError==ClientSocket::cs_err_CannotConnect )
			{
				bAppError = false;
				bReload = false;
