...
extern "C" {
	class Command_Impl *RegisterAsPlugIn(class Router *pRouter,int PK_Device,string sLogger)
	{
		...
		{
			g_pPlutoLogger = new ServerLogger(PK_Device, MythTV_PlugIn::PK_DeviceTemplate_get_static(), "localhost");
...
int main(int argc, char* argv[]) 
{
	...
	{
		if( sLogger=="dcerouter" )
			g_pPlutoLogger = new ServerLogger(PK_Device, MythTV_PlugIn::PK_DeviceTemplate_get_static(), sRouter_IP);
	...
	try
	{
		...
		if ( pMythTV_PlugIn->Connect(pMythTV_PlugIn->PK_DeviceTemplate_get()) ) 
		{
			...
			pthread_join(pMythTV_PlugIn->m_RequestHandlerThread, NULL);
