...
bool MythBackEnd_Socket::Connect( int PK_DeviceTemplate,string sExtraInfo )
{
	...
	{
		LoggerWrapper::GetInstance()->Write(LV_CRITICAL,"MythBackEnd_Socket::Connect failed");
		m_bConnected=false;
		return false;
	}
	...
	string sResponse;
	if( !SendMythString("MYTH_PROTO_VERSION " TOSTRING(MYTH_PROTOCOL),&sResponse) )
	{
		LoggerWrapper::GetInstance()->Write(LV_CRITICAL,"MythBackEnd_Socket::Connect couldn't send MYTH_PROTO_VERSION");
		m_bConnected=false;
		return false;
	}
	...
	{
		LoggerWrapper::GetInstance()->Write(LV_CRITICAL,"MythBackEnd_Socket::Connect sent MYTH_PROTO_VERSION: got %s", sResponse.c_str());
		m_bConnected=false;
		return false;
	}

	if( !SendMythStringGetOk("ANN Playback mythtv_plugin 01") )
	{
		LoggerWrapper::GetInstance()->Write(LV_CRITICAL,"MythBackEnd_Socket::Connect couldn't send ANN");
		m_bConnected=false;
		return false;
	}
