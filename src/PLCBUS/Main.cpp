...
extern "C" {
	...
	{ 
		...
		if( bIsRuntimePlugin )
			return PLCBUS::PK_DeviceTemplate_get_static();
		else
			return 0;
	}
}
...
extern "C" {
	class Command_Impl *RegisterAsPlugIn(class Router *pRouter,int PK_Device,Logger *pPlutoLogger)
	{
		...
		PLCBUS *pPLCBUS = new PLCBUS(PK_Device, "localhost",true,false,pRouter);
		if( pPLCBUS->m_bQuit_get()|| !pPLCBUS->GetConfig() )
		{
			delete pPLCBUS;
			return NULL;
		}
...
int main(int argc, char* argv[]) 
{
	...
	if (bError)
	{
		cout << "A Pluto DCE Device.  See www.plutohome.com/dce for details." << endl
			<< "Usage: PLCBUS [-r Router's IP] [-d My Device ID] [-l dcerouter|stdout|null|filename]" << endl
	...
	try
	{
		if( sLogger=="dcerouter" )
			LoggerWrapper::SetInstance(new ServerLogger(PK_Device, PLCBUS::PK_DeviceTemplate_get_static(), sRouter_IP));
	...
	try
	{
		PLCBUS *pPLCBUS = new PLCBUS(PK_Device, sRouter_IP,true,bLocalMode);
		if ( pPLCBUS->GetConfig() && pPLCBUS->Connect(pPLCBUS->PK_DeviceTemplate_get()) ) 
		{
			g_pCommand_Impl=pPLCBUS;
			g_pDeadlockHandler=DeadlockHandler;
			g_pSocketCrashHandler=SocketCrashHandler;
			LoggerWrapper::GetInstance()->Write(LV_STATUS, "Connect OK");
			pPLCBUS->CreateChildren();
			if( bLocalMode )
				pPLCBUS->RunLocalMode();
			else
			{
				if(pPLCBUS->m_RequestHandlerThread)
					pthread_join(pPLCBUS->m_RequestHandlerThread, NULL);  // This function will return when the device is shutting down
			}
		...
		{
			bAppError = true;
			if( pPLCBUS->m_pEvent && pPLCBUS->m_pEvent->m_pClientSocket && pPLCBUS->m_pEvent->m_pClientSocket->m_eLastError==ClientSocket::cs_err_CannotConnect )
			{
				bAppError = false;
				bReload = false;
