...
const char *g_szCompile_Date="Thu Nov 12 11:31:52 2009";
int g_SvnRevision=22455;
...
extern "C" {
	...
	{ 
		...
		if( bIsRuntimePlugin )
			return Picture_Plugin::PK_DeviceTemplate_get_static();
		else
			return 0;
	}
}
...
extern "C" {
	class Command_Impl *RegisterAsPlugIn(class Router *pRouter,int PK_Device,Logger *pPlutoLogger)
	{
		...
		Picture_Plugin *pPicture_Plugin = new Picture_Plugin(PK_Device, "localhost",true,false,pRouter);
		if( pPicture_Plugin->m_bQuit_get()|| !pPicture_Plugin->GetConfig() )
		{
			delete pPicture_Plugin;
			return NULL;
		}
		else
		{
			g_pCommand_Impl=pPicture_Plugin;
			g_pDeadlockHandler=Plugin_DeadlockHandler;
			g_pSocketCrashHandler=Plugin_SocketCrashHandler;
		}
		return pPicture_Plugin;
	}
}
//<-dceag-plug-e->
...
int main(int argc, char* argv[]) 
{
	...
	cout << "Picture_Plugin, v." << VERSION << endl
	...
	if (bError)
	{
		cout << "A Pluto DCE Device.  See www.plutohome.com/dce for details." << endl
			<< "Usage: Picture_Plugin [-r Router's IP] [-d My Device ID] [-l dcerouter|stdout|null|filename]" << endl
	...
	try
	{
		if( sLogger=="dcerouter" )
			LoggerWrapper::SetInstance(new ServerLogger(PK_Device, Picture_Plugin::PK_DeviceTemplate_get_static(), sRouter_IP));
	...
	try
	{
		Picture_Plugin *pPicture_Plugin = new Picture_Plugin(PK_Device, sRouter_IP,true,bLocalMode);
		if ( pPicture_Plugin->GetConfig() && pPicture_Plugin->Connect(pPicture_Plugin->PK_DeviceTemplate_get()) ) 
		{
			g_pCommand_Impl=pPicture_Plugin;
			g_pDeadlockHandler=DeadlockHandler;
			g_pSocketCrashHandler=SocketCrashHandler;
			LoggerWrapper::GetInstance()->Write(LV_STATUS, "Connect OK");
			pPicture_Plugin->CreateChildren();
			if( bLocalMode )
				pPicture_Plugin->RunLocalMode();
			else
			{
				if(pPicture_Plugin->m_RequestHandlerThread)
					pthread_join(pPicture_Plugin->m_RequestHandlerThread, NULL);  // This function will return when the device is shutting down
			}
		...
		{
			bAppError = true;
			if( pPicture_Plugin->m_pEvent && pPicture_Plugin->m_pEvent->m_pClientSocket && pPicture_Plugin->m_pEvent->m_pClientSocket->m_eLastError==ClientSocket::cs_err_CannotConnect )
			{
				bAppError = false;
				bReload = false;
