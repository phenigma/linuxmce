...
g_pPlutoLogger->Write(LV_STATUS,"MediaAttributes::AddAttributeToStream %p %d",pRow_Attribute,(pRow_Attribute ? pRow_Attribute->PK_Attribute_get() : 0));
	if( File )
	{
		if( File>0 && File<=pMediaStream->m_dequeMediaFile.size() )
		{
			...
			if( Title_Track )
			{
				...
				m_pMediaAttributes_LowLevel->GetMediaSection(&pMediaFile->m_dequeMediaSection,Section)->m_mapPK_Attribute[PK_AttributeType] = pRow_Attribute->PK_Attribute_get();
		}
	}
	else if( Title_Track ) // For DVD's Track is the title
	{
		...
		m_pMediaAttributes_LowLevel->GetMediaSection(&pMediaStream->m_dequeMediaSection,Section)->m_mapPK_Attribute[PK_AttributeType] = pRow_Attribute->PK_Attribute_get();
}
