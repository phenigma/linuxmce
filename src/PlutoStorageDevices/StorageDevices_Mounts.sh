...
AutoMaster_StorageDevices="/mnt/device /etc/auto.PlutoStorageDevices --timeout=1,--ghost"
...
for Device in $ExternalStorageDevices; do
	...
	Fstab_ExternalStorageDevices="$Fstab_ExternalStorageDevices\n$Device_ID -fstype=nfs,tcp,soft,timeo=3,retrans=1,retry=0 ${Device_Host}:${Device_MountPoint}"
