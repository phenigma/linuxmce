...
Detect() {
	...
	availPath=($(diff_Funk availPath[@] unknownPath[@]))
        availPath=($(diff_Funk availPath[@] mountedPath[@]))
        availPath=($(diff_Funk availPath[@] mountedDevPath[@]))
        availPath=$(echo "${availPath[@]}" | sort -u)

        for Path in $availPath; do
                for path_alias in $(udevadm info --query=symlink --name="/dev/disk/by-path/${Path}" | awk '{ print $4 }' | sed 's,disk/by-label,,g'); do
                        for alias_mounted in $(mount | grep -wo "$path_alias"); do
