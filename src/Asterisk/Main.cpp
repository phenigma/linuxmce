...
int main(int argc, char* argv[]) 
{
	...
	try
	{
		if( sLogger=="dcerouter" )
			LoggerWrapper::SetInstance(new ServerLogger(PK_Device, Asterisk::PK_DeviceTemplate_get_static(), sRouter_IP));
	...
	try
	{
		...
		if ( pAsterisk->GetConfig() && pAsterisk->Connect(pAsterisk->PK_DeviceTemplate_get()) ) 
		{
			g_pCommand_Impl=pAsterisk;
		...
		{
			bAppError = true;
			if( pAsterisk->m_pEvent && pAsterisk->m_pEvent->m_pClientSocket && pAsterisk->m_pEvent->m_pClientSocket->m_eLastError==ClientSocket::cs_err_CannotConnect )
			{
				bAppError = false;
				bReload = false;
