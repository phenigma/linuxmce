...
# LinuxMCE create script for VoipWA service
...
my $DECLARED_NUMBER;
my $DECLARED_HOST = "voice.hgait.net.au";
my $DECLARED_PREFIX = "";
my $LOCAL_PREFIX1 = "";
my $LOCAL_PREFIX2 = "";
...
{
    chomp;
    if($_ =~ /[<]option value[=]\"([^\"]+)\"[>]IAX2\/voipwa[<]\/option[>]/)
    {
        $OUT_ROUTE=$1;
    }
}
...
print STDERR "curl -d '$OUT_DATA' '$OUT_URL' > /dev/null\n";
