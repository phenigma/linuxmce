...
$TRUNK_VARS{'peerdetails'}.="incominglimit=1\n";
$TRUNK_VARS{'peerdetails'}.="nat=never\n";
...
{
    chomp;
    if($_ =~ /[<]option value[=]\"([^\"]+)\"[>]SIP\/spa3102[<]\/option[>]/)
    {
        $OUT_ROUTE=$1;
    }
}
