...
$TRUNK_VARS{'register'}="$DECLARED_USERNAME@$DECLARED_HOST/1und1";
...
{
    chomp;
    if($_ =~ /[<]option value[=]\"([^\"]+)\"[>]SIP\/1und1[<]\/option[>]/)
    {
        $OUT_ROUTE=$1;
    }
}
