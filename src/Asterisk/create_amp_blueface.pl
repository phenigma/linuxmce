...
my $DECLARED_NUMBER;
my $DECLARED_HOST = "sip.blueface.ie";
...
$TRUNK_VARS{'peerdetails'}.="insecure=invite\n";
...
$TRUNK_VARS{'userconfig'}.="insecure=invite\n";
...
{
    chomp;
    if($_ =~ /[<]option value[=]\"([^\"]+)\"[>]SIP\/blueface[<]\/option[>]/)
    {
        $OUT_ROUTE=$1;
    }
}
