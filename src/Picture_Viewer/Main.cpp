...
extern "C" {
	...
	{ 
		...
		if( bIsRuntimePlugin )
			return Picture_Viewer::PK_DeviceTemplate_get_static();
		else
			return 0;
	}
}
...
extern "C" {
	class Command_Impl *RegisterAsPlugIn(class Router *pRouter,int PK_Device,Logger *pPlutoLogger)
	{
		...
		Picture_Viewer *pPicture_Viewer = new Picture_Viewer(PK_Device, "localhost",true,false,pRouter);
		if( pPicture_Viewer->m_bQuit_get()|| !pPicture_Viewer->GetConfig() )
		{
			delete pPicture_Viewer;
			return NULL;
		}
		else
		{
			g_pCommand_Impl=pPicture_Viewer;
			g_pDeadlockHandler=Plugin_DeadlockHandler;
			g_pSocketCrashHandler=Plugin_SocketCrashHandler;
		}
		return pPicture_Viewer;
	}
}
//<-dceag-plug-e->
...
int main(int argc, char* argv[]) 
{
	...
	cout << "Picture_Viewer, v." << VERSION << endl
	...
	if (bError)
	{
		cout << "A Pluto DCE Device.  See www.plutohome.com/dce for details." << endl
			<< "Usage: Picture_Viewer [-r Router's IP] [-d My Device ID] [-l dcerouter|stdout|null|filename]" << endl
	...
	try
	{
		if( sLogger=="dcerouter" )
			LoggerWrapper::SetInstance(new ServerLogger(PK_Device, Picture_Viewer::PK_DeviceTemplate_get_static(), sRouter_IP));
	...
	try
	{
		Picture_Viewer *pPicture_Viewer = new Picture_Viewer(PK_Device, sRouter_IP,true,bLocalMode);
		if ( pPicture_Viewer->GetConfig() && pPicture_Viewer->Connect(pPicture_Viewer->PK_DeviceTemplate_get()) ) 
		{
			g_pCommand_Impl=pPicture_Viewer;
			g_pDeadlockHandler=DeadlockHandler;
			g_pSocketCrashHandler=SocketCrashHandler;
			LoggerWrapper::GetInstance()->Write(LV_STATUS, "Connect OK");
			pPicture_Viewer->CreateChildren();
			if( bLocalMode )
				pPicture_Viewer->RunLocalMode();
			else
			{
				if(pPicture_Viewer->m_RequestHandlerThread)
					pthread_join(pPicture_Viewer->m_RequestHandlerThread, NULL);  // This function will return when the device is shutting down
			}
		...
		{
			bAppError = true;
			if( pPicture_Viewer->m_pEvent && pPicture_Viewer->m_pEvent->m_pClientSocket && pPicture_Viewer->m_pEvent->m_pClientSocket->m_eLastError==ClientSocket::cs_err_CannotConnect )
			{
				bAppError = false;
				bReload = false;
