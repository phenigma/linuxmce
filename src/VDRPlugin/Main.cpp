...
#define  VERSION "2.0.0.44.0709282200"
const char *g_szCompile_Date="Sat Sep 29 01:55:21 2007";
int g_SvnRevision=18079;
...
extern "C" {
	...
	{ 
		...
		if( bIsRuntimePlugin )
			return VDRPlugin::PK_DeviceTemplate_get_static();
		else
			return 0;
	}
}
...
int main(int argc, char* argv[]) 
{
	...
	if (bError)
	{
		cout << "A Pluto DCE Device.  See www.plutohome.com/dce for details." << endl
			<< "Usage: VDRPlugin [-r Router's IP] [-d My Device ID] [-l dcerouter|stdout|null|filename]" << endl
	...
	try
	{
		if( sLogger=="dcerouter" )
			LoggerWrapper::SetInstance(new ServerLogger(PK_Device, VDRPlugin::PK_DeviceTemplate_get_static(), sRouter_IP));
	...
	try
	{
		VDRPlugin *pVDRPlugin = new VDRPlugin(PK_Device, sRouter_IP,true,bLocalMode);
		if ( pVDRPlugin->GetConfig() && pVDRPlugin->Connect(pVDRPlugin->PK_DeviceTemplate_get()) ) 
		{
			...
			pVDRPlugin->CreateChildren();
			if( bLocalMode )
				pVDRPlugin->RunLocalMode();
			else
			{
				if(pVDRPlugin->m_RequestHandlerThread)
					pthread_join(pVDRPlugin->m_RequestHandlerThread, NULL);  // This function will return when the device is shutting down
			}
		...
		{
			bAppError = true;
			if( pVDRPlugin->m_pEvent && pVDRPlugin->m_pEvent->m_pClientSocket && pVDRPlugin->m_pEvent->m_pClientSocket->m_eLastError==ClientSocket::cs_err_CannotConnect )
			{
				bAppError = false;
				bReload = false;
