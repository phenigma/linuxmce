...
//
typedef LRESULT (FAR WINAPI *SENDMESSAGE)(HWND,UINT,WPARAM,LPARAM);
...
{
    pData->fnSendMessage(pData->hwnd, WM_KEYDOWN, VK_DOWN, 1 | 0x50 << 16 | 1 << 24);
    pData->fnSendMessage(pData->hwnd, WM_KEYUP,   VK_DOWN, 1 | 0x50 << 16 | 1 << 24 | 1 << 30 | 1 << 31);
...
{	
	...
	__try {
        ...
        SENDMESSAGE fnSendMessage = (SENDMESSAGE)GetProcAddress(hUser32, fUnicode ? "PostMessageW" : "PostMessageA");

		// Initialize INJDATA and then 
		// copy it to the remote process
		INJDATA DataLocal = {
			hWnd,
			(SENDMESSAGE) GetProcAddress(hUser32, fUnicode ? "PostMessageW" : "PostMessageA"),
            0
		};
		...
		const int cbCodeSize = (const int)((LPBYTE) AfterThreadFunc - (LPBYTE) ThreadFunc);
