...
int CAppBarDlg::OnCreate(LPCREATESTRUCT lpCreateStruct) 
{
	...
	if (pOptions)
    {
	    ...
	    pOptions->uSide = m_Side; //ABE_TOP;

        // Calculate the Height and the width
        DWORD dwUnits;
        dwUnits = GetDialogBaseUnits ();

		//changing appbar size
	    pOptions->cyHeight = (CY_DEFHEIGHT*HIWORD (dwUnits)) / 3 + 30;
        pOptions->cxWidth = (CX_DEFWIDTH*LOWORD (dwUnits)) / 6 + 30;
	...
	m_pAppBar->Register ();
	m_pAppBar->SetSide (m_Side/*ABE_TOP*/);
...
void CAppBarDlg::SetDesktopSide(int Side)
{
	m_Side=Side;
}
