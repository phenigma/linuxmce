...
#define  VERSION "2.0.0.46.13122028462"
const char *g_szCompile_Date="Sat Dec 21 04:05:46 2013";
int g_SvnRevision=28462;
...
extern "C" {
	...
	{ 
		...
		if( bIsRuntimePlugin )
			return VDR::PK_DeviceTemplate_get_static();
		else
			return 0;
	}
}
...
int main(int argc, char* argv[]) 
{
	...
	if (bError)
	{
		cout << "A Pluto DCE Device.  See www.plutohome.com/dce for details." << endl
			<< "Usage: VDR [-r Router's IP] [-d My Device ID] [-l dcerouter|stdout|null|filename]" << endl
	...
	try
	{
		if( sLogger=="dcerouter" )
			LoggerWrapper::SetInstance(new ServerLogger(PK_Device, VDR::PK_DeviceTemplate_get_static(), sRouter_IP));
	...
	try
	{
		VDR *pVDR = new VDR(PK_Device, sRouter_IP,true,bLocalMode);
		if ( pVDR->GetConfig() && pVDR->Connect(pVDR->PK_DeviceTemplate_get()) ) 
		{
			g_pCommand_Impl=pVDR;
		...
		{
			bAppError = true;
			if( pVDR->m_pEvent && pVDR->m_pEvent->m_pClientSocket && pVDR->m_pEvent->m_pClientSocket->m_eLastError==ClientSocket::cs_err_CannotConnect )
			{
				bAppError = false;
				bReload = false;
