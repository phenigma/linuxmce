//<-dceag-incl-b->
#include "Generic_Analog_Camera.h"
...
extern "C" {
	...
	{ 
		...
		if( bIsRuntimePlugin )
			return Generic_Analog_Camera::PK_DeviceTemplate_get_static();
		else
			return 0;
	}
}
...
extern "C" {
	class Command_Impl *RegisterAsPlugIn(class Router *pRouter,int PK_Device,string sLogger)
	{
		...
		{
			g_pPlutoLogger = new ServerLogger(PK_Device, Generic_Analog_Camera::PK_DeviceTemplate_get_static(), "localhost");
		...
		Generic_Analog_Camera *pGeneric_Analog_Camera = new Generic_Analog_Camera(PK_Device, "localhost",true,false,pRouter);
		if( pGeneric_Analog_Camera->m_bQuit )
		{
			delete pGeneric_Analog_Camera;
			return NULL;
		}
		else
		{
			g_pCommand_Impl=pGeneric_Analog_Camera;
			g_pDeadlockHandler=Plugin_DeadlockHandler;
			g_pSocketCrashHandler=Plugin_SocketCrashHandler;
		}
		return pGeneric_Analog_Camera;
	}
}
//<-dceag-plug-e->
...
int main(int argc, char* argv[]) 
{
	...
	cout << "Generic_Analog_Camera, v." << VERSION << endl
	...
	if (bError)
	{
		cout << "A Pluto DCE Device.  See www.plutohome.com/dce for details." << endl
			<< "Usage: Generic_Analog_Camera [-r Router's IP] [-d My Device ID] [-l dcerouter|stdout|null|filename]" << endl
	...
	{
		if( sLogger=="dcerouter" )
			g_pPlutoLogger = new ServerLogger(PK_Device, Generic_Analog_Camera::PK_DeviceTemplate_get_static(), sRouter_IP);
	...
	try
	{
		Generic_Analog_Camera *pGeneric_Analog_Camera = new Generic_Analog_Camera(PK_Device, sRouter_IP);	
		if ( pGeneric_Analog_Camera->Connect(pGeneric_Analog_Camera->PK_DeviceTemplate_get()) ) 
		{
			g_pCommand_Impl=pGeneric_Analog_Camera;
			g_pDeadlockHandler=DeadlockHandler;
			g_pSocketCrashHandler=SocketCrashHandler;
			g_pPlutoLogger->Write(LV_STATUS, "Connect OK");
			pGeneric_Analog_Camera->CreateChildren();
			pthread_join(pGeneric_Analog_Camera->m_RequestHandlerThread, NULL);  // This function will return when the device is shutting down
		...
		if( pGeneric_Analog_Camera->m_bReload )
			bReload=true;
