...
extern "C" {
	...
	{ 
		...
		if( bIsRuntimePlugin )
			return AEt_EMC2000::PK_DeviceTemplate_get_static();
		else
			return 0;
	}
}
...
extern "C" {
	class Command_Impl *RegisterAsPlugIn(class Router *pRouter,int PK_Device,Logger *pPlutoLogger)
	{
		...
		AEt_EMC2000 *pAEt_EMC2000 = new AEt_EMC2000(PK_Device, "localhost",true,false,pRouter);
		if( pAEt_EMC2000->m_bQuit_get()|| !pAEt_EMC2000->GetConfig() )
		{
			delete pAEt_EMC2000;
			return NULL;
		}
...
int main(int argc, char* argv[]) 
{
	...
	cout << "AEt_EMC2000, v." << VERSION << endl
	...
	if (bError)
	{
		cout << "A Pluto DCE Device.  See www.plutohome.com/dce for details." << endl
			<< "Usage: AEt_EMC2000 [-r Router's IP] [-d My Device ID] [-l dcerouter|stdout|null|filename]" << endl
	...
	try
	{
		if( sLogger=="dcerouter" )
			LoggerWrapper::SetInstance(new ServerLogger(PK_Device, AEt_EMC2000::PK_DeviceTemplate_get_static(), sRouter_IP));
	...
	try
	{
		AEt_EMC2000 *pAEt_EMC2000 = new AEt_EMC2000(PK_Device, sRouter_IP,true,bLocalMode);
		if ( pAEt_EMC2000->GetConfig() && pAEt_EMC2000->Connect(pAEt_EMC2000->PK_DeviceTemplate_get()) ) 
		{
			g_pCommand_Impl=pAEt_EMC2000;
			g_pDeadlockHandler=DeadlockHandler;
			g_pSocketCrashHandler=SocketCrashHandler;
			LoggerWrapper::GetInstance()->Write(LV_STATUS, "Connect OK");
			pAEt_EMC2000->CreateChildren();
			if( bLocalMode )
				pAEt_EMC2000->RunLocalMode();
			else
			{
				if(pAEt_EMC2000->m_RequestHandlerThread)
					pthread_join(pAEt_EMC2000->m_RequestHandlerThread, NULL);  // This function will return when the device is shutting down
			}
		...
		{
			bAppError = true;
			if( pAEt_EMC2000->m_pEvent && pAEt_EMC2000->m_pEvent->m_pClientSocket && pAEt_EMC2000->m_pEvent->m_pClientSocket->m_eLastError==ClientSocket::cs_err_CannotConnect )
			{
				bAppError = false;
				bReload = false;
