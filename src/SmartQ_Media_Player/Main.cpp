...
extern "C" {
	...
	{ 
		...
		if( bIsRuntimePlugin )
			return SmartQ_Media_Player::PK_DeviceTemplate_get_static();
		else
			return 0;
	}
}
...
extern "C" {
	class Command_Impl *RegisterAsPlugIn(class Router *pRouter,int PK_Device,Logger *pPlutoLogger)
	{
		...
		SmartQ_Media_Player *pSmartQ_Media_Player = new SmartQ_Media_Player(PK_Device, "localhost",true,false,pRouter);
		if( pSmartQ_Media_Player->m_bQuit_get()|| !pSmartQ_Media_Player->GetConfig() )
		{
			delete pSmartQ_Media_Player;
			return NULL;
		}
		else
		{
			g_pCommand_Impl=pSmartQ_Media_Player;
			g_pDeadlockHandler=Plugin_DeadlockHandler;
			g_pSocketCrashHandler=Plugin_SocketCrashHandler;
		}
		return pSmartQ_Media_Player;
	}
}
//<-dceag-plug-e->
...
int main(int argc, char* argv[]) 
{
	...
	cout << "SmartQ_Media_Player, v." << VERSION << endl
	...
	if (bError)
	{
		cout << "A Pluto DCE Device.  See www.plutohome.com/dce for details." << endl
			<< "Usage: SmartQ_Media_Player [-r Router's IP] [-d My Device ID] [-l dcerouter|stdout|null|filename]" << endl
	...
	try
	{
		if( sLogger=="dcerouter" )
			LoggerWrapper::SetInstance(new ServerLogger(PK_Device, SmartQ_Media_Player::PK_DeviceTemplate_get_static(), sRouter_IP));
	...
	try
	{
		SmartQ_Media_Player *pSmartQ_Media_Player = new SmartQ_Media_Player(PK_Device, sRouter_IP,true,bLocalMode);
		if ( pSmartQ_Media_Player->GetConfig() && pSmartQ_Media_Player->Connect(pSmartQ_Media_Player->PK_DeviceTemplate_get()) ) 
		{
			g_pCommand_Impl=pSmartQ_Media_Player;
			g_pDeadlockHandler=DeadlockHandler;
			g_pSocketCrashHandler=SocketCrashHandler;
			LoggerWrapper::GetInstance()->Write(LV_STATUS, "Connect OK");
			pSmartQ_Media_Player->CreateChildren();
			if( bLocalMode )
				pSmartQ_Media_Player->RunLocalMode();
			else
			{
				if(pSmartQ_Media_Player->m_RequestHandlerThread)
					pthread_join(pSmartQ_Media_Player->m_RequestHandlerThread, NULL);  // This function will return when the device is shutting down
			}
		...
		{
			bAppError = true;
			if( pSmartQ_Media_Player->m_pEvent && pSmartQ_Media_Player->m_pEvent->m_pClientSocket && pSmartQ_Media_Player->m_pEvent->m_pClientSocket->m_eLastError==ClientSocket::cs_err_CannotConnect )
			{
				bAppError = false;
				bReload = false;
		...
		if( pSmartQ_Media_Player->m_bReload )
			bReload=true;
