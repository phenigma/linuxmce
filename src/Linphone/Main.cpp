...
extern "C" {
	...
	{ 
		...
		if( bIsRuntimePlugin )
			return Linphone::PK_DeviceTemplate_get_static();
		else
			return 0;
	}
}
...
int main(int argc, char* argv[]) 
{
	...
	if (bError)
	{
		cout << "A Pluto DCE Device.  See www.plutohome.com/dce for details." << endl
			<< "Usage: Linphone [-r Router's IP] [-d My Device ID] [-l dcerouter|stdout|null|filename]" << endl
	...
	try
	{
		if( sLogger=="dcerouter" )
			LoggerWrapper::SetInstance(new ServerLogger(PK_Device, Linphone::PK_DeviceTemplate_get_static(), sRouter_IP));
	...
	try
	{
		...
		{
			bAppError = true;
			if( pLinphone->m_pEvent && pLinphone->m_pEvent->m_pClientSocket && pLinphone->m_pEvent->m_pClientSocket->m_eLastError==ClientSocket::cs_err_CannotConnect )
			{
				bAppError = false;
				bReload = false;
