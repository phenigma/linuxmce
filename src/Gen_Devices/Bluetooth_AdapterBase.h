#ifndef Bluetooth_AdapterBase_h
#define Bluetooth_AdapterBase_h
...
class Bluetooth_Adapter_Event : public Event_Impl
{
public:
	...
	* @brief Constructors
	*/
	Bluetooth_Adapter_Event(int DeviceID, string ServerAddress, bool bConnectEventHandler=true) :
		Event_Impl(DeviceID, DEVICETEMPLATE_Bluetooth_Adapter_CONST, ServerAddress, bConnectEventHandler, SOCKET_TIMEOUT) {};
	Bluetooth_Adapter_Event(class ClientSocket *pOCClientSocket, int DeviceID) : Event_Impl(pOCClientSocket, DeviceID) {};
...
class Bluetooth_Adapter_Data : public DeviceData_Impl
{
public:
	...
	*/
	virtual ~Bluetooth_Adapter_Data() {};
	...
	*/
	virtual int GetPK_DeviceList() { return DEVICETEMPLATE_Bluetooth_Adapter_CONST; } ;

	/**
	* @brief Returns the description of the device
	*/
	virtual const char *GetDeviceDescription() { return "Bluetooth_Adapter"; } ;
...
class Bluetooth_Adapter_Command : public Command_Impl
{
public:
	Bluetooth_Adapter_Command(int DeviceID, string ServerAddress,bool bConnectEventHandler=true,bool bLocalMode=false,class Router *pRouter=NULL)
	...
	{
		m_pData=NULL;
		m_pEvent = new Bluetooth_Adapter_Event(m_dwPK_Device, m_sHostName, !m_bLocalMode);
		...
		if( m_bLocalMode )
		{
			m_pData = new Bluetooth_Adapter_Data();
			return true;
		}
		...
		m_pcRequestSocket = new Event_Impl(m_dwPK_Device, DEVICETEMPLATE_Bluetooth_Adapter_CONST,m_sHostName);
		if( m_iInstanceID )
		{
	...
	Bluetooth_Adapter_Command(Command_Impl *pPrimaryDeviceCommand, DeviceData_Impl *pData, Event_Impl *pEvent, Router *pRouter) : Command_Impl(pPrimaryDeviceCommand, pData, pEvent, pRouter) {};
	virtual ~Bluetooth_Adapter_Command() {};
	Bluetooth_Adapter_Event *GetEvents() { return (Bluetooth_Adapter_Event *) m_pEvent; };
	Bluetooth_Adapter_Data *GetData() { return (Bluetooth_Adapter_Data *) m_pData; };
	const char *GetClassName() { return "Bluetooth_Adapter_Command"; };
	virtual int PK_DeviceTemplate_get() { return DEVICETEMPLATE_Bluetooth_Adapter_CONST; };
	static int PK_DeviceTemplate_get_static() { return DEVICETEMPLATE_Bluetooth_Adapter_CONST; };
	...
	virtual void CMD_Bluetooth_Pair_Device(string sMac_address,string &sCMD_Result,class Message *pMessage) {};
	virtual void CMD_Bluetooth_Connect_Device(string sMac_address,string &sCMD_Result,class Message *pMessage) {};
	virtual void CMD_Bluetooth_Disconnect_Device(string sMac_address,string &sCMD_Result,class Message *pMessage) {};
	...
	virtual ReceivedMessageResult ReceivedMessage(class Message *pMessageOriginal)
	{
		...
		for(int s=-1;s<(int) pMessageOriginal->m_vectExtraMessages.size(); ++s)
		{
			...
			if (pMessage->m_dwPK_Device_To==m_dwPK_Device && pMessage->m_dwMessage_Type == MESSAGETYPE_COMMAND)
			{
				...
				{
				case COMMAND_Bluetooth_Pair_Device_CONST:
					{
						string sCMD_Result="OK";
						string sMac_address=pMessage->m_mapParameters[COMMANDPARAMETER_Mac_address_CONST];
						CMD_Bluetooth_Pair_Device(sMac_address.c_str(),sCMD_Result,pMessage);
					...
					iHandled++;
					continue;
				case COMMAND_Bluetooth_Connect_Device_CONST:
					{
						string sCMD_Result="OK";
						string sMac_address=pMessage->m_mapParameters[COMMANDPARAMETER_Mac_address_CONST];
						CMD_Bluetooth_Connect_Device(sMac_address.c_str(),sCMD_Result,pMessage);
						if( pMessage->m_eExpectedResponse==ER_ReplyMessage && !pMessage->m_bRespondedToMessage )
						{
							pMessage->m_bRespondedToMessage=true;
							Message *pMessageOut=new Message(m_dwPK_Device,pMessage->m_dwPK_Device_From,PRIORITY_NORMAL,MESSAGETYPE_REPLY,0,0);
					...
					iHandled++;
					continue;
				case COMMAND_Bluetooth_Disconnect_Device_CONST:
					{
						string sCMD_Result="OK";
						string sMac_address=pMessage->m_mapParameters[COMMANDPARAMETER_Mac_address_CONST];
						CMD_Bluetooth_Disconnect_Device(sMac_address.c_str(),sCMD_Result,pMessage);
						if( pMessage->m_eExpectedResponse==ER_ReplyMessage && !pMessage->m_bRespondedToMessage )
						{
							pMessage->m_bRespondedToMessage=true;
							Message *pMessageOut=new Message(m_dwPK_Device,pMessage->m_dwPK_Device_From,PRIORITY_NORMAL,MESSAGETYPE_REPLY,0,0);
