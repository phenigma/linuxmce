...
class eggtimer_Event : public Event_Impl
{
public:
	...
	* @brief Constructors
	*/
	eggtimer_Event(int DeviceID, string ServerAddress, bool bConnectEventHandler=true) :
		Event_Impl(DeviceID, DEVICETEMPLATE_Egg_Timer_CONST, ServerAddress, bConnectEventHandler, SOCKET_TIMEOUT) {};
	eggtimer_Event(class ClientSocket *pOCClientSocket, int DeviceID) : Event_Impl(pOCClientSocket, DeviceID) {};
...
class eggtimer_Data : public DeviceData_Impl
{
public:
	...
	*/
	virtual int GetPK_DeviceList() { return DEVICETEMPLATE_Egg_Timer_CONST; } ;

	/**
	* @brief Returns the description of the device
	*/
	virtual const char *GetDeviceDescription() { return "eggtimer"; } ;
...
class eggtimer_Command : public Command_Impl
{
public:
	eggtimer_Command(int DeviceID, string ServerAddress,bool bConnectEventHandler=true,bool bLocalMode=false,class Router *pRouter=NULL)
	...
	{
		m_pData=NULL;
		m_pEvent = new eggtimer_Event(m_dwPK_Device, m_sHostName, !m_bLocalMode);
		...
		m_pcRequestSocket = new Event_Impl(m_dwPK_Device, DEVICETEMPLATE_Egg_Timer_CONST,m_sHostName);
		if( m_iInstanceID )
		{
	...
	eggtimer_Command(Command_Impl *pPrimaryDeviceCommand, DeviceData_Impl *pData, Event_Impl *pEvent, Router *pRouter) : Command_Impl(pPrimaryDeviceCommand, pData, pEvent, pRouter) {};
	virtual ~eggtimer_Command() {};
	eggtimer_Event *GetEvents() { return (eggtimer_Event *) m_pEvent; };
	eggtimer_Data *GetData() { return (eggtimer_Data *) m_pData; };
	const char *GetClassName() { return "eggtimer_Command"; };
	virtual int PK_DeviceTemplate_get() { return DEVICETEMPLATE_Egg_Timer_CONST; };
	static int PK_DeviceTemplate_get_static() { return DEVICETEMPLATE_Egg_Timer_CONST; };
	...
	virtual ReceivedMessageResult ReceivedMessage(class Message *pMessageOriginal)
	{
		...
		for(int s=-1;s<(int) pMessageOriginal->m_vectExtraMessages.size(); ++s)
		{
			...
			if (pMessage->m_dwPK_Device_To==m_dwPK_Device && pMessage->m_dwMessage_Type == MESSAGETYPE_COMMAND)
			{
				...
				{
				case COMMAND_Start_Egg_Timer_CONST:
					{
					...
					iHandled++;
					continue;
				case COMMAND_Cancel_Egg_Timer_CONST:
					{
						string sCMD_Result="OK";
						int iDeviceToLink=atoi(pMessage->m_mapParameters[COMMANDPARAMETER_DeviceToLink_CONST].c_str());
						bool bSendOFF=(pMessage->m_mapParameters[COMMANDPARAMETER_SendOFF_CONST]=="1" ? true : false);
						CMD_Cancel_Egg_Timer(iDeviceToLink,bSendOFF,sCMD_Result,pMessage);
						if( pMessage->m_eExpectedResponse==ER_ReplyMessage && !pMessage->m_bRespondedToMessage )
						{
							pMessage->m_bRespondedToMessage=true;
							Message *pMessageOut=new Message(m_dwPK_Device,pMessage->m_dwPK_Device_From,PRIORITY_NORMAL,MESSAGETYPE_REPLY,0,0);
