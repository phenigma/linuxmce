#ifndef Advanced_IP_CameraBase_h
#define Advanced_IP_CameraBase_h
...
class Advanced_IP_Camera_Event : public Event_Impl
{
public:
	...
	* @brief Constructors
	*/
	Advanced_IP_Camera_Event(int DeviceID, string ServerAddress, bool bConnectEventHandler=true) :
		Event_Impl(DeviceID, DEVICETEMPLATE_Advanced_IP_Camera_CONST, ServerAddress, bConnectEventHandler, SOCKET_TIMEOUT) {};
	Advanced_IP_Camera_Event(class ClientSocket *pOCClientSocket, int DeviceID) : Event_Impl(pOCClientSocket, DeviceID) {};
...
class Advanced_IP_Camera_Data : public DeviceData_Impl
{
public:
	...
	*/
	virtual ~Advanced_IP_Camera_Data() {};
	...
	*/
	virtual int GetPK_DeviceList() { return DEVICETEMPLATE_Advanced_IP_Camera_CONST; } ;

	/**
	* @brief Returns the description of the device
	*/
	virtual const char *GetDeviceDescription() { return "Advanced_IP_Camera"; } ;
	...
	*/
	...
	{
		if( m_bRunningWithoutDeviceData )
			return m_pEvent_Impl->GetDeviceDataFromDatabase(m_dwPK_Device,DEVICEDATA_AuthUser_CONST);
		else
			return m_mapParameters[DEVICEDATA_AuthUser_CONST];
	}
	...
	bool Get_Video_Support()
	{
		if( m_bRunningWithoutDeviceData )
			return (m_pEvent_Impl->GetDeviceDataFromDatabase(m_dwPK_Device,DEVICEDATA_Video_Support_CONST)=="1" ? true : false);
		else
			return (m_mapParameters[DEVICEDATA_Video_Support_CONST]=="1" ? true : false);
	}
...
class Advanced_IP_Camera_Command : public Command_Impl
{
public:
	Advanced_IP_Camera_Command(int DeviceID, string ServerAddress,bool bConnectEventHandler=true,bool bLocalMode=false,class Router *pRouter=NULL)
	...
	{
		m_pData=NULL;
		m_pEvent = new Advanced_IP_Camera_Event(m_dwPK_Device, m_sHostName, !m_bLocalMode);
		...
		if( m_bLocalMode )
		{
			m_pData = new Advanced_IP_Camera_Data();
			return true;
		}
		...
		m_pcRequestSocket = new Event_Impl(m_dwPK_Device, DEVICETEMPLATE_Advanced_IP_Camera_CONST,m_sHostName);
		if( m_iInstanceID )
		{
	...
	Advanced_IP_Camera_Command(Command_Impl *pPrimaryDeviceCommand, DeviceData_Impl *pData, Event_Impl *pEvent, Router *pRouter) : Command_Impl(pPrimaryDeviceCommand, pData, pEvent, pRouter) {};
	virtual ~Advanced_IP_Camera_Command() {};
	Advanced_IP_Camera_Event *GetEvents() { return (Advanced_IP_Camera_Event *) m_pEvent; };
	Advanced_IP_Camera_Data *GetData() { return (Advanced_IP_Camera_Data *) m_pData; };
	const char *GetClassName() { return "Advanced_IP_Camera_Command"; };
	virtual int PK_DeviceTemplate_get() { return DEVICETEMPLATE_Advanced_IP_Camera_CONST; };
	static int PK_DeviceTemplate_get_static() { return DEVICETEMPLATE_Advanced_IP_Camera_CONST; };
	...
	bool DATA_Get_Video_Support() { return GetData()->Get_Video_Support(); }
	//Event accessors
	...
	virtual void CMD_Zoom_In(string &sCMD_Result,class Message *pMessage) {};
	virtual void CMD_Zoom_Out(string &sCMD_Result,class Message *pMessage) {};
		...
		for(int s=-1;s<(int) pMessageOriginal->m_vectExtraMessages.size(); ++s)
		{
			...
			if (pMessage->m_dwPK_Device_To==m_dwPK_Device && pMessage->m_dwMessage_Type == MESSAGETYPE_COMMAND)
			{
				...
				case COMMAND_Move_Right_CONST:
					...
					iHandled++;
					continue;
				case COMMAND_Zoom_In_CONST:
					{
						string sCMD_Result="OK";
						CMD_Zoom_In(sCMD_Result,pMessage);
						if( pMessage->m_eExpectedResponse==ER_ReplyMessage && !pMessage->m_bRespondedToMessage )
						{
							pMessage->m_bRespondedToMessage=true;
							Message *pMessageOut=new Message(m_dwPK_Device,pMessage->m_dwPK_Device_From,PRIORITY_NORMAL,MESSAGETYPE_REPLY,0,0);
					...
					iHandled++;
					continue;
				case COMMAND_Zoom_Out_CONST:
					{
						string sCMD_Result="OK";
						CMD_Zoom_Out(sCMD_Result,pMessage);
						if( pMessage->m_eExpectedResponse==ER_ReplyMessage && !pMessage->m_bRespondedToMessage )
						{
							pMessage->m_bRespondedToMessage=true;
							Message *pMessageOut=new Message(m_dwPK_Device,pMessage->m_dwPK_Device_From,PRIORITY_NORMAL,MESSAGETYPE_REPLY,0,0);
