#ifndef OMX_PlayerBase_h
#define OMX_PlayerBase_h
...
class OMX_Player_Event : public Event_Impl
{
public:
	...
	* @brief Constructors
	*/
	OMX_Player_Event(int DeviceID, string ServerAddress, bool bConnectEventHandler=true) :
		Event_Impl(DeviceID, DEVICETEMPLATE_OMX_Player_CONST, ServerAddress, bConnectEventHandler, SOCKET_TIMEOUT) {};
	OMX_Player_Event(class ClientSocket *pOCClientSocket, int DeviceID) : Event_Impl(pOCClientSocket, DeviceID) {};
	...
	*/
	...
	virtual void Subtitle_Track_Changed(string sID,int iStream_ID,string sName,string sFormat,string sLanguage)
	{
		SendMessage(new Message(m_dwPK_Device, DEVICEID_EVENTMANAGER, PRIORITY_NORMAL, MESSAGETYPE_EVENT, 
			EVENT_Subtitle_Track_Changed_CONST,
			5 /* number of parameter's pairs (id, value) */,
			EVENTPARAMETER_ID_CONST, sID.c_str(),
			EVENTPARAMETER_Stream_ID_CONST, StringUtils::itos(iStream_ID).c_str(),
			EVENTPARAMETER_Name_CONST, sName.c_str(),
			EVENTPARAMETER_Format_CONST, sFormat.c_str(),
			EVENTPARAMETER_Language_CONST, sLanguage.c_str()));
	}

	virtual void Audio_Track_Changed(string sID,int iStream_ID,string sName,string sFormat,string sLanguage)
	{
		SendMessage(new Message(m_dwPK_Device, DEVICEID_EVENTMANAGER, PRIORITY_NORMAL, MESSAGETYPE_EVENT, 
			EVENT_Audio_Track_Changed_CONST,
			5 /* number of parameter's pairs (id, value) */,
			EVENTPARAMETER_ID_CONST, sID.c_str(),
			EVENTPARAMETER_Stream_ID_CONST, StringUtils::itos(iStream_ID).c_str(),
			EVENTPARAMETER_Name_CONST, sName.c_str(),
	...
	virtual void Video_Track_Changed(string sID,int iStream_ID,string sName,string sFormat,string sLanguage)
	{
		SendMessage(new Message(m_dwPK_Device, DEVICEID_EVENTMANAGER, PRIORITY_NORMAL, MESSAGETYPE_EVENT, 
			EVENT_Video_Track_Changed_CONST,
			5 /* number of parameter's pairs (id, value) */,
			EVENTPARAMETER_ID_CONST, sID.c_str(),
			EVENTPARAMETER_Stream_ID_CONST, StringUtils::itos(iStream_ID).c_str(),
			EVENTPARAMETER_Name_CONST, sName.c_str(),
...
class OMX_Player_Data : public DeviceData_Impl
{
public:
	...
	*/
	virtual int GetPK_DeviceList() { return DEVICETEMPLATE_OMX_Player_CONST; } ;

	/**
	* @brief Returns the description of the device
	*/
	virtual const char *GetDeviceDescription() { return "OMX_Player"; } ;
...
class OMX_Player_Command : public Command_Impl
{
public:
	OMX_Player_Command(int DeviceID, string ServerAddress,bool bConnectEventHandler=true,bool bLocalMode=false,class Router *pRouter=NULL)
	...
	{
		m_pData=NULL;
		m_pEvent = new OMX_Player_Event(m_dwPK_Device, m_sHostName, !m_bLocalMode);
		...
		m_pcRequestSocket = new Event_Impl(m_dwPK_Device, DEVICETEMPLATE_OMX_Player_CONST,m_sHostName);
		if( m_iInstanceID )
		{
	...
	OMX_Player_Command(Command_Impl *pPrimaryDeviceCommand, DeviceData_Impl *pData, Event_Impl *pEvent, Router *pRouter) : Command_Impl(pPrimaryDeviceCommand, pData, pEvent, pRouter) {};
	virtual ~OMX_Player_Command() {};
	OMX_Player_Event *GetEvents() { return (OMX_Player_Event *) m_pEvent; };
	OMX_Player_Data *GetData() { return (OMX_Player_Data *) m_pData; };
	const char *GetClassName() { return "OMX_Player_Command"; };
	virtual int PK_DeviceTemplate_get() { return DEVICETEMPLATE_OMX_Player_CONST; };
	static int PK_DeviceTemplate_get_static() { return DEVICETEMPLATE_OMX_Player_CONST; };
	...
	void EVENT_Subtitle_Track_Changed(string sID,int iStream_ID,string sName,string sFormat,string sLanguage) { GetEvents()->Subtitle_Track_Changed(sID.c_str(),iStream_ID,sName.c_str(),sFormat.c_str(),sLanguage.c_str()); }
	void EVENT_Audio_Track_Changed(string sID,int iStream_ID,string sName,string sFormat,string sLanguage) { GetEvents()->Audio_Track_Changed(sID.c_str(),iStream_ID,sName.c_str(),sFormat.c_str(),sLanguage.c_str()); }
	void EVENT_Video_Track_Changed(string sID,int iStream_ID,string sName,string sFormat,string sLanguage) { GetEvents()->Video_Track_Changed(sID.c_str(),iStream_ID,sName.c_str(),sFormat.c_str(),sLanguage.c_str()); }
