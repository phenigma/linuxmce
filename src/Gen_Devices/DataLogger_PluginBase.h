#ifndef DataLogger_PluginBase_h
#define DataLogger_PluginBase_h
...
class DataLogger_Plugin_Event : public Event_Impl
{
public:
	...
	* @brief Constructors
	*/
	DataLogger_Plugin_Event(int DeviceID, string ServerAddress, bool bConnectEventHandler=true) :
		Event_Impl(DeviceID, DEVICETEMPLATE_DataLogger_Plugin_CONST, ServerAddress, bConnectEventHandler, SOCKET_TIMEOUT_PLUGIN) {};
	DataLogger_Plugin_Event(class ClientSocket *pOCClientSocket, int DeviceID) : Event_Impl(pOCClientSocket, DeviceID) {};
...
class DataLogger_Plugin_Data : public DeviceData_Impl
{
public:
	...
	*/
	virtual ~DataLogger_Plugin_Data() {};
	...
	*/
	virtual int GetPK_DeviceList() { return DEVICETEMPLATE_DataLogger_Plugin_CONST; } ;

	/**
	* @brief Returns the description of the device
	*/
	virtual const char *GetDeviceDescription() { return "DataLogger_Plugin"; } ;
...
class DataLogger_Plugin_Command : public Command_Impl
{
public:
	DataLogger_Plugin_Command(int DeviceID, string ServerAddress,bool bConnectEventHandler=true,bool bLocalMode=false,class Router *pRouter=NULL)
	...
	{
		m_pData=NULL;
		m_pEvent = new DataLogger_Plugin_Event(m_dwPK_Device, m_sHostName, !m_bLocalMode);
		...
		if( m_bLocalMode )
		{
			m_pData = new DataLogger_Plugin_Data();
			return true;
		}
		...
		m_pcRequestSocket = new Event_Impl(m_dwPK_Device, DEVICETEMPLATE_DataLogger_Plugin_CONST,m_sHostName);
		if( m_iInstanceID )
		{
	...
	DataLogger_Plugin_Command(Command_Impl *pPrimaryDeviceCommand, DeviceData_Impl *pData, Event_Impl *pEvent, Router *pRouter) : Command_Impl(pPrimaryDeviceCommand, pData, pEvent, pRouter) {};
	virtual ~DataLogger_Plugin_Command() {};
	DataLogger_Plugin_Event *GetEvents() { return (DataLogger_Plugin_Event *) m_pEvent; };
	DataLogger_Plugin_Data *GetData() { return (DataLogger_Plugin_Data *) m_pData; };
	const char *GetClassName() { return "DataLogger_Plugin_Command"; };
	virtual int PK_DeviceTemplate_get() { return DEVICETEMPLATE_DataLogger_Plugin_CONST; };
	static int PK_DeviceTemplate_get_static() { return DEVICETEMPLATE_DataLogger_Plugin_CONST; };
