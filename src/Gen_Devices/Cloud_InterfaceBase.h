#ifndef Cloud_InterfaceBase_h
#define Cloud_InterfaceBase_h
...
class Cloud_Interface_Event : public Event_Impl
{
public:
	...
	* @brief Constructors
	*/
	Cloud_Interface_Event(int DeviceID, string ServerAddress, bool bConnectEventHandler=true) :
		Event_Impl(DeviceID, DEVICETEMPLATE_Cloud_Interface_CONST, ServerAddress, bConnectEventHandler, SOCKET_TIMEOUT) {};
	Cloud_Interface_Event(class ClientSocket *pOCClientSocket, int DeviceID) : Event_Impl(pOCClientSocket, DeviceID) {};
	...
	*/
	...
	virtual void UserPassword_Mismatch(int iPK_Device,string sComment)
	{
		SendMessage(new Message(m_dwPK_Device, DEVICEID_EVENTMANAGER, PRIORITY_NORMAL, MESSAGETYPE_EVENT, 
			EVENT_UserPassword_Mismatch_CONST,
			2 /* number of parameter's pairs (id, value) */,
			EVENTPARAMETER_PK_Device_CONST, StringUtils::itos(iPK_Device).c_str(),
			EVENTPARAMETER_Comment_CONST, sComment.c_str()));
	}
...
class Cloud_Interface_Data : public DeviceData_Impl
{
public:
	...
	*/
	virtual int GetPK_DeviceList() { return DEVICETEMPLATE_Cloud_Interface_CONST; } ;

	/**
	* @brief Returns the description of the device
	*/
	virtual const char *GetDeviceDescription() { return "Cloud_Interface"; } ;
...
class Cloud_Interface_Command : public Command_Impl
{
public:
	Cloud_Interface_Command(int DeviceID, string ServerAddress,bool bConnectEventHandler=true,bool bLocalMode=false,class Router *pRouter=NULL)
	...
	{
		m_pData=NULL;
		m_pEvent = new Cloud_Interface_Event(m_dwPK_Device, m_sHostName, !m_bLocalMode);
		...
		if( m_bLocalMode )
		{
			m_pData = new Cloud_Interface_Data();
			return true;
		}
		...
		m_pcRequestSocket = new Event_Impl(m_dwPK_Device, DEVICETEMPLATE_Cloud_Interface_CONST,m_sHostName);
		if( m_iInstanceID )
		{
	...
	Cloud_Interface_Command(Command_Impl *pPrimaryDeviceCommand, DeviceData_Impl *pData, Event_Impl *pEvent, Router *pRouter) : Command_Impl(pPrimaryDeviceCommand, pData, pEvent, pRouter) {};
	virtual ~Cloud_Interface_Command() {};
	Cloud_Interface_Event *GetEvents() { return (Cloud_Interface_Event *) m_pEvent; };
	Cloud_Interface_Data *GetData() { return (Cloud_Interface_Data *) m_pData; };
	const char *GetClassName() { return "Cloud_Interface_Command"; };
	virtual int PK_DeviceTemplate_get() { return DEVICETEMPLATE_Cloud_Interface_CONST; };
	static int PK_DeviceTemplate_get_static() { return DEVICETEMPLATE_Cloud_Interface_CONST; };
	...
	void EVENT_UserPassword_Mismatch(int iPK_Device,string sComment) { GetEvents()->UserPassword_Mismatch(iPK_Device,sComment.c_str()); }
