...
*/
#ifndef Plug_And_Play_PluginBase_h
#define Plug_And_Play_PluginBase_h
...
class Plug_And_Play_Plugin_Event : public Event_Impl
{
public:
	...
	* @brief Constructors
	*/
	Plug_And_Play_Plugin_Event(int DeviceID, string ServerAddress, bool bConnectEventHandler=true) :
		Event_Impl(DeviceID, DEVICETEMPLATE_Plug_And_Play_Plugin_CONST, ServerAddress, bConnectEventHandler, SOCKET_TIMEOUT_PLUGIN) {};
	Plug_And_Play_Plugin_Event(class ClientSocket *pOCClientSocket, int DeviceID) : Event_Impl(pOCClientSocket, DeviceID) {};
...
class Plug_And_Play_Plugin_Data : public DeviceData_Impl
{
public:
	...
	*/
	virtual ~Plug_And_Play_Plugin_Data() {};
	...
	*/
	virtual int GetPK_DeviceList() { return DEVICETEMPLATE_Plug_And_Play_Plugin_CONST; } ;

	/**
	* @brief Returns the description of the device
	*/
	virtual const char *GetDeviceDescription() { return "Plug_And_Play_Plugin"; } ;
...
class Plug_And_Play_Plugin_Command : public Command_Impl
{
public:
	Plug_And_Play_Plugin_Command(int DeviceID, string ServerAddress,bool bConnectEventHandler=true,bool bLocalMode=false,class Router *pRouter=NULL)
	...
	{
		m_pData=NULL;
		m_pEvent = new Plug_And_Play_Plugin_Event(m_dwPK_Device, m_sHostName, !m_bLocalMode);
		...
		if( m_bLocalMode )
		{
			m_pData = new Plug_And_Play_Plugin_Data();
			return true;
		}
		...
		m_pcRequestSocket = new Event_Impl(m_dwPK_Device, DEVICETEMPLATE_Plug_And_Play_Plugin_CONST,m_sHostName);
		if( m_iInstanceID )
		{
	...
	Plug_And_Play_Plugin_Command(Command_Impl *pPrimaryDeviceCommand, DeviceData_Impl *pData, Event_Impl *pEvent, Router *pRouter) : Command_Impl(pPrimaryDeviceCommand, pData, pEvent, pRouter) {};
	virtual ~Plug_And_Play_Plugin_Command() {};
	Plug_And_Play_Plugin_Event *GetEvents() { return (Plug_And_Play_Plugin_Event *) m_pEvent; };
	Plug_And_Play_Plugin_Data *GetData() { return (Plug_And_Play_Plugin_Data *) m_pData; };
	const char *GetClassName() { return "Plug_And_Play_Plugin_Command"; };
	virtual int PK_DeviceTemplate_get() { return DEVICETEMPLATE_Plug_And_Play_Plugin_CONST; };
	static int PK_DeviceTemplate_get_static() { return DEVICETEMPLATE_Plug_And_Play_Plugin_CONST; };
	...
	virtual void CMD_Pre_Pnp_Script_Done(string sData_String,bool bTrueFalse,int iPK_PnpQueue,string &sCMD_Result,class Message *pMessage) {};
		...
		for(int s=-1;s<(int) pMessageOriginal->m_vectExtraMessages.size(); ++s)
		{
			...
			if (pMessage->m_dwPK_Device_To==m_dwPK_Device && pMessage->m_dwMessage_Type == MESSAGETYPE_COMMAND)
			{
				...
				case COMMAND_Choose_Pnp_Device_Template_CONST:
					...
					iHandled++;
					continue;
				case COMMAND_Set_Pnp_Options_CONST:
					{
						string sCMD_Result="OK";
						...
						int iPK_PnpQueue=atoi(pMessage->m_mapParameters[COMMANDPARAMETER_PK_PnpQueue_CONST].c_str());
						CMD_Set_Pnp_Options(sValue_To_Assign.c_str(),iPK_DeviceData,iPK_PnpQueue,sCMD_Result,pMessage);
						if( pMessage->m_eExpectedResponse==ER_ReplyMessage && !pMessage->m_bRespondedToMessage )
						{
							pMessage->m_bRespondedToMessage=true;
							Message *pMessageOut=new Message(m_dwPK_Device,pMessage->m_dwPK_Device_From,PRIORITY_NORMAL,MESSAGETYPE_REPLY,0,0);
					...
					iHandled++;
					continue;
				case COMMAND_Ignore_PNP_Device_CONST:
					{
						string sCMD_Result="OK";
						int iPK_PnpQueue=atoi(pMessage->m_mapParameters[COMMANDPARAMETER_PK_PnpQueue_CONST].c_str());
						bool bAlways=(pMessage->m_mapParameters[COMMANDPARAMETER_Always_CONST]=="1" ? true : false);
						CMD_Ignore_PNP_Device(iPK_PnpQueue,bAlways,sCMD_Result,pMessage);
						if( pMessage->m_eExpectedResponse==ER_ReplyMessage && !pMessage->m_bRespondedToMessage )
						{
							pMessage->m_bRespondedToMessage=true;
							Message *pMessageOut=new Message(m_dwPK_Device,pMessage->m_dwPK_Device_From,PRIORITY_NORMAL,MESSAGETYPE_REPLY,0,0);
				...
				case COMMAND_PNP_Detection_Script_Finished_CONST:
					...
					iHandled++;
					continue;
				case COMMAND_Pre_Pnp_Script_Done_CONST:
					{
						string sCMD_Result="OK";
						string sData_String=pMessage->m_mapParameters[COMMANDPARAMETER_Data_String_CONST];
						bool bTrueFalse=(pMessage->m_mapParameters[COMMANDPARAMETER_TrueFalse_CONST]=="1" ? true : false);
						int iPK_PnpQueue=atoi(pMessage->m_mapParameters[COMMANDPARAMETER_PK_PnpQueue_CONST].c_str());
						CMD_Pre_Pnp_Script_Done(sData_String.c_str(),bTrueFalse,iPK_PnpQueue,sCMD_Result,pMessage);
						if( pMessage->m_eExpectedResponse==ER_ReplyMessage && !pMessage->m_bRespondedToMessage )
						{
							pMessage->m_bRespondedToMessage=true;
							Message *pMessageOut=new Message(m_dwPK_Device,pMessage->m_dwPK_Device_From,PRIORITY_NORMAL,MESSAGETYPE_REPLY,0,0);
