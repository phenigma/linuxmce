#ifndef Data_Provider_Catalog_PluginBase_h
#define Data_Provider_Catalog_PluginBase_h
...
class Data_Provider_Catalog_Plugin_Event : public Event_Impl
{
public:
	...
	* @brief Constructors
	*/
	Data_Provider_Catalog_Plugin_Event(int DeviceID, string ServerAddress, bool bConnectEventHandler=true) :
		Event_Impl(DeviceID, DEVICETEMPLATE_Data_Provider_Catalog_Plugin_CONST, ServerAddress, bConnectEventHandler, SOCKET_TIMEOUT_PLUGIN) {};
	Data_Provider_Catalog_Plugin_Event(class ClientSocket *pOCClientSocket, int DeviceID) : Event_Impl(pOCClientSocket, DeviceID) {};
...
class Data_Provider_Catalog_Plugin_Data : public DeviceData_Impl
{
public:
	...
	*/
	virtual ~Data_Provider_Catalog_Plugin_Data() {};
	...
	*/
	virtual int GetPK_DeviceList() { return DEVICETEMPLATE_Data_Provider_Catalog_Plugin_CONST; } ;

	/**
	* @brief Returns the description of the device
	*/
	virtual const char *GetDeviceDescription() { return "Data_Provider_Catalog_Plugin"; } ;
...
class Data_Provider_Catalog_Plugin_Command : public Command_Impl
{
public:
	Data_Provider_Catalog_Plugin_Command(int DeviceID, string ServerAddress,bool bConnectEventHandler=true,bool bLocalMode=false,class Router *pRouter=NULL)
	...
	{
		m_pData=NULL;
		m_pEvent = new Data_Provider_Catalog_Plugin_Event(m_dwPK_Device, m_sHostName, !m_bLocalMode);
		...
		if( m_bLocalMode )
		{
			m_pData = new Data_Provider_Catalog_Plugin_Data();
			return true;
		}
		...
		m_pcRequestSocket = new Event_Impl(m_dwPK_Device, DEVICETEMPLATE_Data_Provider_Catalog_Plugin_CONST,m_sHostName);
		if( m_iInstanceID )
		{
	...
	Data_Provider_Catalog_Plugin_Command(Command_Impl *pPrimaryDeviceCommand, DeviceData_Impl *pData, Event_Impl *pEvent, Router *pRouter) : Command_Impl(pPrimaryDeviceCommand, pData, pEvent, pRouter) {};
	virtual ~Data_Provider_Catalog_Plugin_Command() {};
	Data_Provider_Catalog_Plugin_Event *GetEvents() { return (Data_Provider_Catalog_Plugin_Event *) m_pEvent; };
	Data_Provider_Catalog_Plugin_Data *GetData() { return (Data_Provider_Catalog_Plugin_Data *) m_pData; };
	const char *GetClassName() { return "Data_Provider_Catalog_Plugin_Command"; };
	virtual int PK_DeviceTemplate_get() { return DEVICETEMPLATE_Data_Provider_Catalog_Plugin_CONST; };
	static int PK_DeviceTemplate_get_static() { return DEVICETEMPLATE_Data_Provider_Catalog_Plugin_CONST; };
	...
	virtual void CMD_Register_Data_Provider(int iPK_Device,string sID,string sFormat_Input,string sFormat_Output,string &sCMD_Result,class Message *pMessage) {};
	virtual void CMD_Request_Data_Provider(string sID,string sParameters,bool bMultiple,string sFormat_Input,string sFormat_Output,string sCache,int *iValue,string *sData_String,string &sCMD_Result,class Message *pMessage) {};
	...
	virtual ReceivedMessageResult ReceivedMessage(class Message *pMessageOriginal)
	{
		...
		for(int s=-1;s<(int) pMessageOriginal->m_vectExtraMessages.size(); ++s)
		{
			...
			if (pMessage->m_dwPK_Device_To==m_dwPK_Device && pMessage->m_dwMessage_Type == MESSAGETYPE_COMMAND)
			{
				...
				{
				case COMMAND_Register_Data_Provider_CONST:
					{
						...
						string sFormat_Input=pMessage->m_mapParameters[COMMANDPARAMETER_Format_Input_CONST];
						string sFormat_Output=pMessage->m_mapParameters[COMMANDPARAMETER_Format_Output_CONST];
						CMD_Register_Data_Provider(iPK_Device,sID.c_str(),sFormat_Input.c_str(),sFormat_Output.c_str(),sCMD_Result,pMessage);
					...
					iHandled++;
					continue;
				case COMMAND_Request_Data_Provider_CONST:
					{
						string sCMD_Result="OK";
						string sID=pMessage->m_mapParameters[COMMANDPARAMETER_ID_CONST];
						...
						string sFormat_Input=pMessage->m_mapParameters[COMMANDPARAMETER_Format_Input_CONST];
						string sFormat_Output=pMessage->m_mapParameters[COMMANDPARAMETER_Format_Output_CONST];
						string sCache=pMessage->m_mapParameters[COMMANDPARAMETER_Cache_CONST];
						...
						CMD_Request_Data_Provider(sID.c_str(),sParameters.c_str(),bMultiple,sFormat_Input.c_str(),sFormat_Output.c_str(),sCache.c_str(),&iValue,&sData_String,sCMD_Result,pMessage);
						if( pMessage->m_eExpectedResponse==ER_ReplyMessage && !pMessage->m_bRespondedToMessage )
						{
							pMessage->m_bRespondedToMessage=true;
							Message *pMessageOut=new Message(m_dwPK_Device,pMessage->m_dwPK_Device_From,PRIORITY_NORMAL,MESSAGETYPE_REPLY,0,0);
