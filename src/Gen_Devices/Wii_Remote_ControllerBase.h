#ifndef Wii_Remote_ControllerBase_h
#define Wii_Remote_ControllerBase_h
...
class Wii_Remote_Controller_Event : public Event_Impl
{
public:
	...
	* @brief Constructors
	*/
	Wii_Remote_Controller_Event(int DeviceID, string ServerAddress, bool bConnectEventHandler=true) :
		Event_Impl(DeviceID, DEVICETEMPLATE_Wii_Remote_Controller_CONST, ServerAddress, bConnectEventHandler, SOCKET_TIMEOUT) {};
	Wii_Remote_Controller_Event(class ClientSocket *pOCClientSocket, int DeviceID) : Event_Impl(pOCClientSocket, DeviceID) {};
...
class Wii_Remote_Controller_Data : public DeviceData_Impl
{
public:
	...
	*/
	virtual ~Wii_Remote_Controller_Data() {};
	...
	*/
	virtual int GetPK_DeviceList() { return DEVICETEMPLATE_Wii_Remote_Controller_CONST; } ;

	/**
	* @brief Returns the description of the device
	*/
	virtual const char *GetDeviceDescription() { return "Wii_Remote_Controller"; } ;
...
class Wii_Remote_Controller_Command : public Command_Impl
{
public:
	Wii_Remote_Controller_Command(int DeviceID, string ServerAddress,bool bConnectEventHandler=true,bool bLocalMode=false,class Router *pRouter=NULL)
	...
	{
		m_pData=NULL;
		m_pEvent = new Wii_Remote_Controller_Event(m_dwPK_Device, m_sHostName, !m_bLocalMode);
		...
		if( m_bLocalMode )
		{
			m_pData = new Wii_Remote_Controller_Data();
			return true;
		}
		...
		m_pcRequestSocket = new Event_Impl(m_dwPK_Device, DEVICETEMPLATE_Wii_Remote_Controller_CONST,m_sHostName);
		if( m_iInstanceID )
		{
	...
	Wii_Remote_Controller_Command(Command_Impl *pPrimaryDeviceCommand, DeviceData_Impl *pData, Event_Impl *pEvent, Router *pRouter) : Command_Impl(pPrimaryDeviceCommand, pData, pEvent, pRouter) {};
	virtual ~Wii_Remote_Controller_Command() {};
	Wii_Remote_Controller_Event *GetEvents() { return (Wii_Remote_Controller_Event *) m_pEvent; };
	Wii_Remote_Controller_Data *GetData() { return (Wii_Remote_Controller_Data *) m_pData; };
	const char *GetClassName() { return "Wii_Remote_Controller_Command"; };
	virtual int PK_DeviceTemplate_get() { return DEVICETEMPLATE_Wii_Remote_Controller_CONST; };
	static int PK_DeviceTemplate_get_static() { return DEVICETEMPLATE_Wii_Remote_Controller_CONST; };
	...
	virtual void CMD_Connect_to_Device(string sMac_address,string &sCMD_Result,class Message *pMessage) {};
		...
		for(int s=-1;s<(int) pMessageOriginal->m_vectExtraMessages.size(); ++s)
		{
			...
			if (pMessage->m_dwPK_Device_To==m_dwPK_Device && pMessage->m_dwMessage_Type == MESSAGETYPE_COMMAND)
			{
				...
				case COMMAND_Set_Screen_Type_CONST:
					...
					iHandled++;
					continue;
				case COMMAND_Connect_to_Device_CONST:
					{
						string sCMD_Result="OK";
						string sMac_address=pMessage->m_mapParameters[COMMANDPARAMETER_Mac_address_CONST];
						CMD_Connect_to_Device(sMac_address.c_str(),sCMD_Result,pMessage);
						if( pMessage->m_eExpectedResponse==ER_ReplyMessage && !pMessage->m_bRespondedToMessage )
						{
							pMessage->m_bRespondedToMessage=true;
							Message *pMessageOut=new Message(m_dwPK_Device,pMessage->m_dwPK_Device_From,PRIORITY_NORMAL,MESSAGETYPE_REPLY,0,0);
