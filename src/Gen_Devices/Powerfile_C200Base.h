...
{
public:
	Powerfile_C200_Event(int DeviceID, string ServerAddress, bool bConnectEventHandler=true) : Event_Impl(DeviceID,1737, ServerAddress, bConnectEventHandler) {};
...
public:
	...
	virtual bool GetConfig()
	{
		...
		m_pcRequestSocket = new Event_Impl(m_dwPK_Device, 1737,m_sHostName);
		if( m_iInstanceID )
		{
	...
	virtual int PK_DeviceTemplate_get() { return 1737; };
	static int PK_DeviceTemplate_get_static() { return 1737; };
		...
		for(int s=-1;s<(int) pMessageOriginal->m_vectExtraMessages.size(); ++s)
		{
			...
			if (pMessage->m_dwPK_Device_To==m_dwPK_Device && pMessage->m_dwMessage_Type == MESSAGETYPE_COMMAND)
			{
				...
				case 701:
					...
					int iDrive_Number=atoi(pMessage->m_mapParameters[152].c_str());
						CMD_Load_from_Slot_into_Drive(iSlot_Number,iDrive_Number,sCMD_Result,pMessage);
						if( pMessage->m_eExpectedResponse==ER_ReplyMessage && !pMessage->m_bRespondedToMessage )
						{
							pMessage->m_bRespondedToMessage=true;
							Message *pMessageOut=new Message(m_dwPK_Device,pMessage->m_dwPK_Device_From,PRIORITY_NORMAL,MESSAGETYPE_REPLY,0,0);
				...
				case 702:
					...
					int iSlot_Number=atoi(pMessage->m_mapParameters[151].c_str());
					int iDrive_Number=atoi(pMessage->m_mapParameters[152].c_str());
						CMD_Unload_from_Drive_into_Slot(iSlot_Number,iDrive_Number,sCMD_Result,pMessage);
						if( pMessage->m_eExpectedResponse==ER_ReplyMessage && !pMessage->m_bRespondedToMessage )
						{
							pMessage->m_bRespondedToMessage=true;
							Message *pMessageOut=new Message(m_dwPK_Device,pMessage->m_dwPK_Device_From,PRIORITY_NORMAL,MESSAGETYPE_REPLY,0,0);
				...
				case 703:
					...
					string sJukebox_Status=pMessage->m_mapParameters[153];
						...
						if( pMessage->m_eExpectedResponse==ER_ReplyMessage && !pMessage->m_bRespondedToMessage )
						{
							pMessage->m_bRespondedToMessage=true;
							Message *pMessageOut=new Message(m_dwPK_Device,pMessage->m_dwPK_Device_From,PRIORITY_NORMAL,MESSAGETYPE_REPLY,0,0);
						pMessageOut->m_mapParameters[153]=sJukebox_Status;
							pMessageOut->m_mapParameters[0]=sCMD_Result;
							SendMessage(pMessageOut);
						}
						else if( (pMessage->m_eExpectedResponse==ER_DeliveryConfirmation || pMessage->m_eExpectedResponse==ER_ReplyString) && !pMessage->m_bRespondedToMessage )
				...
				case 739:
					{
						string sCMD_Result="OK";
					string sBulk_rip_status=pMessage->m_mapParameters[158];
						CMD_Get_Bulk_Ripping_Status(&sBulk_rip_status,sCMD_Result,pMessage);
						if( pMessage->m_eExpectedResponse==ER_ReplyMessage && !pMessage->m_bRespondedToMessage )
						{
							pMessage->m_bRespondedToMessage=true;
							Message *pMessageOut=new Message(m_dwPK_Device,pMessage->m_dwPK_Device_From,PRIORITY_NORMAL,MESSAGETYPE_REPLY,0,0);
						pMessageOut->m_mapParameters[158]=sBulk_rip_status;
							pMessageOut->m_mapParameters[0]=sCMD_Result;
							SendMessage(pMessageOut);
						}
						else if( (pMessage->m_eExpectedResponse==ER_DeliveryConfirmation || pMessage->m_eExpectedResponse==ER_ReplyString) && !pMessage->m_bRespondedToMessage )
