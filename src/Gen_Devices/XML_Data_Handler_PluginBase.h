#ifndef XML_Data_Handler_PluginBase_h
#define XML_Data_Handler_PluginBase_h
...
class XML_Data_Handler_Plugin_Event : public Event_Impl
{
public:
	...
	* @brief Constructors
	*/
	XML_Data_Handler_Plugin_Event(int DeviceID, string ServerAddress, bool bConnectEventHandler=true) :
		Event_Impl(DeviceID, DEVICETEMPLATE_XML_Data_Handler_Plugin_CONST, ServerAddress, bConnectEventHandler, SOCKET_TIMEOUT_PLUGIN) {};
	XML_Data_Handler_Plugin_Event(class ClientSocket *pOCClientSocket, int DeviceID) : Event_Impl(pOCClientSocket, DeviceID) {};
...
class XML_Data_Handler_Plugin_Data : public DeviceData_Impl
{
public:
	...
	*/
	virtual ~XML_Data_Handler_Plugin_Data() {};
	...
	*/
	virtual int GetPK_DeviceList() { return DEVICETEMPLATE_XML_Data_Handler_Plugin_CONST; } ;

	/**
	* @brief Returns the description of the device
	*/
	virtual const char *GetDeviceDescription() { return "XML_Data_Handler_Plugin"; } ;
...
class XML_Data_Handler_Plugin_Command : public Command_Impl
{
public:
	XML_Data_Handler_Plugin_Command(int DeviceID, string ServerAddress,bool bConnectEventHandler=true,bool bLocalMode=false,class Router *pRouter=NULL)
	...
	{
		m_pData=NULL;
		m_pEvent = new XML_Data_Handler_Plugin_Event(m_dwPK_Device, m_sHostName, !m_bLocalMode);
		...
		if( m_bLocalMode )
		{
			m_pData = new XML_Data_Handler_Plugin_Data();
			return true;
		}
		...
		m_pcRequestSocket = new Event_Impl(m_dwPK_Device, DEVICETEMPLATE_XML_Data_Handler_Plugin_CONST,m_sHostName);
		if( m_iInstanceID )
		{
	...
	XML_Data_Handler_Plugin_Command(Command_Impl *pPrimaryDeviceCommand, DeviceData_Impl *pData, Event_Impl *pEvent, Router *pRouter) : Command_Impl(pPrimaryDeviceCommand, pData, pEvent, pRouter) {};
	virtual ~XML_Data_Handler_Plugin_Command() {};
	XML_Data_Handler_Plugin_Event *GetEvents() { return (XML_Data_Handler_Plugin_Event *) m_pEvent; };
	XML_Data_Handler_Plugin_Data *GetData() { return (XML_Data_Handler_Plugin_Data *) m_pData; };
	const char *GetClassName() { return "XML_Data_Handler_Plugin_Command"; };
	virtual int PK_DeviceTemplate_get() { return DEVICETEMPLATE_XML_Data_Handler_Plugin_CONST; };
	static int PK_DeviceTemplate_get_static() { return DEVICETEMPLATE_XML_Data_Handler_Plugin_CONST; };
	...
	virtual void CMD_Request_XML_Data(string sDataGrid_ID,string sParameters,string *sData_String,string &sCMD_Result,class Message *pMessage) {};
	...
	virtual ReceivedMessageResult ReceivedMessage(class Message *pMessageOriginal)
	{
		...
		for(int s=-1;s<(int) pMessageOriginal->m_vectExtraMessages.size(); ++s)
		{
			...
			if (pMessage->m_dwPK_Device_To==m_dwPK_Device && pMessage->m_dwMessage_Type == MESSAGETYPE_COMMAND)
			{
				...
				{
				case COMMAND_Request_XML_Data_CONST:
					{
						...
						CMD_Request_XML_Data(sDataGrid_ID.c_str(),sParameters.c_str(),&sData_String,sCMD_Result,pMessage);
