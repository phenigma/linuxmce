...
public:
	...
	virtual int GetPK_DeviceList() { return 39; } ;
...
public:
	...
	: Command_Impl(DeviceID, ServerAddress, bLocalMode, pRouter)
	{
		if( m_bLocalMode )
			return;
		m_pData=NULL;
		m_pEvent = new Infrared_Plugin_Event(DeviceID, ServerAddress);
		...
		m_pcRequestSocket = new Event_Impl(DeviceID, 39,ServerAddress);
	};
	...
	virtual int PK_DeviceTemplate_get() { return 39; };
	static int PK_DeviceTemplate_get_static() { return 39; };
	...
	virtual void CMD_Add_GC100(int iPK_Device,string &sCMD_Result,class Message *pMessage) {};
				...
				case 276:
					{
						string sCMD_Result="OK";
					int iPK_Device=atoi(pMessage->m_mapParameters[2].c_str());
						CMD_Add_GC100(iPK_Device,sCMD_Result,pMessage);
						if( pMessage->m_eExpectedResponse==ER_ReplyMessage )
						{
							Message *pMessageOut=new Message(m_dwPK_Device,pMessage->m_dwPK_Device_From,PRIORITY_NORMAL,MESSAGETYPE_REPLY,0,0);
							SendMessage(pMessageOut);
