...
class CM11A_Event : public Event_Impl
{
public:
	CM11A_Event(int DeviceID, string ServerAddress, bool bConnectEventHandler=true) : Event_Impl(DeviceID,48, ServerAddress, bConnectEventHandler) {};
	CM11A_Event(class ClientSocket *pOCClientSocket, int DeviceID) : Event_Impl(pOCClientSocket, DeviceID) {};
	//Events
...
class CM11A_Data : public DeviceData_Impl
{
public:
	virtual ~CM11A_Data() {};
	class DeviceData_Impl *CreateData(DeviceData_Impl *Parent,char *pDataBlock,unsigned long AllocatedSize,char *CurrentPosition);
	virtual int GetPK_DeviceList() { return 48; } ;
	virtual const char *GetDeviceDescription() { return "CM11A"; } ;
...
class CM11A_Command : public Command_Impl
{
public:
	CM11A_Command(int DeviceID, string ServerAddress,bool bConnectEventHandler=true,bool bLocalMode=false,class Router *pRouter=NULL)
	...
	virtual bool GetConfig()
	{
		...
		m_pData=NULL;
		m_pEvent = new CM11A_Event(m_dwPK_Device, m_sHostName, !m_bLocalMode);
		...
		m_pcRequestSocket = new Event_Impl(m_dwPK_Device, 48,m_sHostName);
		if( m_iInstanceID )
		{
	...
	CM11A_Command(Command_Impl *pPrimaryDeviceCommand, DeviceData_Impl *pData, Event_Impl *pEvent, Router *pRouter) : Command_Impl(pPrimaryDeviceCommand, pData, pEvent, pRouter) {};
	virtual ~CM11A_Command() {};
