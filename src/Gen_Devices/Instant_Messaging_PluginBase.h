#ifndef Instant_Messaging_PluginBase_h
#define Instant_Messaging_PluginBase_h
...
class Instant_Messaging_Plugin_Event : public Event_Impl
{
public:
	...
	* @brief Constructors
	*/
	Instant_Messaging_Plugin_Event(int DeviceID, string ServerAddress, bool bConnectEventHandler=true) :
		Event_Impl(DeviceID, DEVICETEMPLATE_Instant_Messaging_Plugin_CONST, ServerAddress, bConnectEventHandler, SOCKET_TIMEOUT_PLUGIN) {};
	Instant_Messaging_Plugin_Event(class ClientSocket *pOCClientSocket, int DeviceID) : Event_Impl(pOCClientSocket, DeviceID) {};
	...
	*/
	...
	virtual void Instant_Message_Recieved(string sText,int iPK_Users,string sIM_User_ID)
	{
		SendMessage(new Message(m_dwPK_Device, DEVICEID_EVENTMANAGER, PRIORITY_NORMAL, MESSAGETYPE_EVENT, 
			EVENT_Instant_Message_Recieved_CONST,
			...
			EVENTPARAMETER_IM_User_ID_CONST, sIM_User_ID.c_str()));
	}
...
class Instant_Messaging_Plugin_Data : public DeviceData_Impl
{
public:
	...
	*/
	virtual ~Instant_Messaging_Plugin_Data() {};
	...
	*/
	virtual int GetPK_DeviceList() { return DEVICETEMPLATE_Instant_Messaging_Plugin_CONST; } ;

	/**
	* @brief Returns the description of the device
	*/
	virtual const char *GetDeviceDescription() { return "Instant_Messaging_Plugin"; } ;
	...
	*/
	...
	bool Get_Online()
	{
		if( m_bRunningWithoutDeviceData )
			return (m_pEvent_Impl->GetDeviceDataFromDatabase(m_dwPK_Device,DEVICEDATA_Online_CONST)=="1" ? true : false);
		else
			return (m_mapParameters[DEVICEDATA_Online_CONST]=="1" ? true : false);
	}
...
class Instant_Messaging_Plugin_Command : public Command_Impl
{
public:
	Instant_Messaging_Plugin_Command(int DeviceID, string ServerAddress,bool bConnectEventHandler=true,bool bLocalMode=false,class Router *pRouter=NULL)
	...
	{
		m_pData=NULL;
		m_pEvent = new Instant_Messaging_Plugin_Event(m_dwPK_Device, m_sHostName, !m_bLocalMode);
		...
		if( m_bLocalMode )
		{
			m_pData = new Instant_Messaging_Plugin_Data();
			return true;
		}
		...
		m_pcRequestSocket = new Event_Impl(m_dwPK_Device, DEVICETEMPLATE_Instant_Messaging_Plugin_CONST,m_sHostName);
		if( m_iInstanceID )
		{
	...
	Instant_Messaging_Plugin_Command(Command_Impl *pPrimaryDeviceCommand, DeviceData_Impl *pData, Event_Impl *pEvent, Router *pRouter) : Command_Impl(pPrimaryDeviceCommand, pData, pEvent, pRouter) {};
	virtual ~Instant_Messaging_Plugin_Command() {};
	Instant_Messaging_Plugin_Event *GetEvents() { return (Instant_Messaging_Plugin_Event *) m_pEvent; };
	Instant_Messaging_Plugin_Data *GetData() { return (Instant_Messaging_Plugin_Data *) m_pData; };
	const char *GetClassName() { return "Instant_Messaging_Plugin_Command"; };
	virtual int PK_DeviceTemplate_get() { return DEVICETEMPLATE_Instant_Messaging_Plugin_CONST; };
	static int PK_DeviceTemplate_get_static() { return DEVICETEMPLATE_Instant_Messaging_Plugin_CONST; };
	virtual void ReceivedCommandForChild(DeviceData_Impl *pDeviceData_Impl,string &sCMD_Result,Message *pMessage) { };
	virtual void ReceivedUnknownCommand(string &sCMD_Result,Message *pMessage) { };
	Command_Impl *CreateCommand(int PK_DeviceTemplate, Command_Impl *pPrimaryDeviceCommand, DeviceData_Impl *pData, Event_Impl *pEvent);
	//Data accessors
	bool DATA_Get_Online() { return GetData()->Get_Online(); }
	//Event accessors
	void EVENT_Instant_Message_Recieved(string sText,int iPK_Users,string sIM_User_ID) { GetEvents()->Instant_Message_Recieved(sText.c_str(),iPK_Users,sIM_User_ID.c_str()); }
	//Commands - Override these to handle commands from the server
	virtual void CMD_Send_IM(string sText,int iPK_Users,string sIM_User,string &sCMD_Result,class Message *pMessage) {};
				...
				case COMMAND_Send_IM_CONST:
					{
						...
						string sIM_User=pMessage->m_mapParameters[COMMANDPARAMETER_IM_User_CONST];
						CMD_Send_IM(sText.c_str(),iPK_Users,sIM_User.c_str(),sCMD_Result,pMessage);
