...
class Tira_Event : public Event_Impl
{
public:
	Tira_Event(int DeviceID, string ServerAddress, bool bConnectEventHandler=true) : Event_Impl(DeviceID,1717, ServerAddress, bConnectEventHandler) {};
	Tira_Event(class ClientSocket *pOCClientSocket, int DeviceID) : Event_Impl(pOCClientSocket, DeviceID) {};
	//Events
...
class Tira_Data : public DeviceData_Impl
{
public:
	virtual ~Tira_Data() {};
	...
	int Get_Repeat() { if( m_bRunningWithoutDeviceData )  return atoi(m_pEvent_Impl->GetDeviceDataFromDatabase(m_dwPK_Device,121).c_str()); else return atoi(m_mapParameters[121].c_str());}
};
...
class Tira_Command : public Command_Impl
{
public:
	Tira_Command(int DeviceID, string ServerAddress,bool bConnectEventHandler=true,bool bLocalMode=false,class Router *pRouter=NULL)
	...
	virtual bool GetConfig()
	{
		...
		m_pcRequestSocket = new Event_Impl(m_dwPK_Device, 1717,m_sHostName);
		if( m_iInstanceID )
		{
	...
	Tira_Command(Command_Impl *pPrimaryDeviceCommand, DeviceData_Impl *pData, Event_Impl *pEvent, Router *pRouter) : Command_Impl(pPrimaryDeviceCommand, pData, pEvent, pRouter) {};
	virtual ~Tira_Command() {};
	Tira_Event *GetEvents() { return (Tira_Event *) m_pEvent; };
	Tira_Data *GetData() { return (Tira_Data *) m_pData; };
	const char *GetClassName() { return "Tira_Command"; };
	virtual int PK_DeviceTemplate_get() { return 1717; };
	static int PK_DeviceTemplate_get_static() { return 1717; };
