...
{
public:
	File_Grids_Plugin_Event(int DeviceID, string ServerAddress, bool bConnectEventHandler=true) : Event_Impl(DeviceID,10, ServerAddress, bConnectEventHandler, SOCKET_TIMEOUT_PLUGIN) {};
...
public:
	...
	{
		m_pData=NULL;
		m_pEvent = new File_Grids_Plugin_Event(m_dwPK_Device, m_sHostName, !m_bLocalMode);
		...
		m_pcRequestSocket = new Event_Impl(m_dwPK_Device, 10,m_sHostName);
		if( m_iInstanceID )
		{
	...
	virtual int PK_DeviceTemplate_get() { return 10; };
	static int PK_DeviceTemplate_get_static() { return 10; };
