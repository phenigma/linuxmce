...
class EIB_Event : public Event_Impl
{
public:
	EIB_Event(int DeviceID, string ServerAddress, bool bConnectEventHandler=true) : Event_Impl(DeviceID,49, ServerAddress, bConnectEventHandler) {};
	EIB_Event(class ClientSocket *pOCClientSocket, int DeviceID) : Event_Impl(pOCClientSocket, DeviceID) {};
	//Events
...
class EIB_Data : public DeviceData_Impl
{
public:
	virtual ~EIB_Data() {};
...
class EIB_Command : public Command_Impl
{
public:
	EIB_Command(int DeviceID, string ServerAddress,bool bConnectEventHandler=true,bool bLocalMode=false,class Router *pRouter=NULL)
	...
	virtual bool GetConfig()
	{
		...
		m_pData=NULL;
		m_pEvent = new EIB_Event(m_dwPK_Device, m_sHostName, !m_bLocalMode);
		...
		m_pcRequestSocket = new Event_Impl(m_dwPK_Device, 49,m_sHostName);
		if( m_iInstanceID )
		{
	...
	EIB_Command(Command_Impl *pPrimaryDeviceCommand, DeviceData_Impl *pData, Event_Impl *pEvent, Router *pRouter) : Command_Impl(pPrimaryDeviceCommand, pData, pEvent, pRouter) {};
	virtual ~EIB_Command() {};
	EIB_Event *GetEvents() { return (EIB_Event *) m_pEvent; };
	EIB_Data *GetData() { return (EIB_Data *) m_pData; };
	const char *GetClassName() { return "EIB_Command"; };
	virtual int PK_DeviceTemplate_get() { return 49; };
	static int PK_DeviceTemplate_get_static() { return 49; };
