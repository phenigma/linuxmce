...
{
public:
	Xine_Plugin_Event(int DeviceID, string ServerAddress, bool bConnectEventHandler=true) : Event_Impl(DeviceID,6, ServerAddress, bConnectEventHandler, SOCKET_TIMEOUT_PLUGIN) {};
...
public:
	...
	{
		m_pData=NULL;
		m_pEvent = new Xine_Plugin_Event(m_dwPK_Device, m_sHostName, !m_bLocalMode);
		...
		m_pcRequestSocket = new Event_Impl(m_dwPK_Device, 6,m_sHostName);
		if( m_iInstanceID )
		{
