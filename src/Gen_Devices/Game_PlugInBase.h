#ifndef Game_PlugInBase_h
#define Game_PlugInBase_h
...
class Game_PlugIn_Event : public Event_Impl
{
public:
	...
	* @brief Constructors
	*/
	Game_PlugIn_Event(int DeviceID, string ServerAddress, bool bConnectEventHandler=true) :
		Event_Impl(DeviceID, DEVICETEMPLATE_Game_PlugIn_CONST, ServerAddress, bConnectEventHandler, SOCKET_TIMEOUT_PLUGIN) {};
	Game_PlugIn_Event(class ClientSocket *pOCClientSocket, int DeviceID) : Event_Impl(pOCClientSocket, DeviceID) {};
...
class Game_PlugIn_Data : public DeviceData_Impl
{
public:
	...
	*/
	virtual int GetPK_DeviceList() { return DEVICETEMPLATE_Game_PlugIn_CONST; } ;

	/**
	* @brief Returns the description of the device
	*/
	virtual const char *GetDeviceDescription() { return "Game_PlugIn"; } ;
...
class Game_PlugIn_Command : public Command_Impl
{
public:
	Game_PlugIn_Command(int DeviceID, string ServerAddress,bool bConnectEventHandler=true,bool bLocalMode=false,class Router *pRouter=NULL)
	...
	{
		m_pData=NULL;
		m_pEvent = new Game_PlugIn_Event(m_dwPK_Device, m_sHostName, !m_bLocalMode);
		...
		m_pcRequestSocket = new Event_Impl(m_dwPK_Device, DEVICETEMPLATE_Game_PlugIn_CONST,m_sHostName);
		if( m_iInstanceID )
		{
	...
	Game_PlugIn_Command(Command_Impl *pPrimaryDeviceCommand, DeviceData_Impl *pData, Event_Impl *pEvent, Router *pRouter) : Command_Impl(pPrimaryDeviceCommand, pData, pEvent, pRouter) {};
	virtual ~Game_PlugIn_Command() {};
	Game_PlugIn_Event *GetEvents() { return (Game_PlugIn_Event *) m_pEvent; };
	Game_PlugIn_Data *GetData() { return (Game_PlugIn_Data *) m_pData; };
	const char *GetClassName() { return "Game_PlugIn_Command"; };
	virtual int PK_DeviceTemplate_get() { return DEVICETEMPLATE_Game_PlugIn_CONST; };
	static int PK_DeviceTemplate_get_static() { return DEVICETEMPLATE_Game_PlugIn_CONST; };
	...
	virtual void CMD_Bind_Keypad(string sPK_DesignObj,string sPK_EntertainArea,string &sCMD_Result,class Message *pMessage) {};
	...
	virtual ReceivedMessageResult ReceivedMessage(class Message *pMessageOriginal)
	{
		...
		for(int s=-1;s<(int) pMessageOriginal->m_vectExtraMessages.size(); ++s)
		{
			...
			if (pMessage->m_dwPK_Device_To==m_dwPK_Device && pMessage->m_dwMessage_Type == MESSAGETYPE_COMMAND)
			{
				...
				{
				case COMMAND_Bind_Keypad_CONST:
					{
						...
						CMD_Bind_Keypad(sPK_DesignObj.c_str(),sPK_EntertainArea.c_str(),sCMD_Result,pMessage);
