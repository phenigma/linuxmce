#ifndef Linuxmce_NVRBase_h
#define Linuxmce_NVRBase_h
...
class Linuxmce_NVR_Event : public Event_Impl
{
public:
	...
	* @brief Constructors
	*/
	Linuxmce_NVR_Event(int DeviceID, string ServerAddress, bool bConnectEventHandler=true) :
		Event_Impl(DeviceID, DEVICETEMPLATE_Linuxmce_NVR_CONST, ServerAddress, bConnectEventHandler, SOCKET_TIMEOUT) {};
	Linuxmce_NVR_Event(class ClientSocket *pOCClientSocket, int DeviceID) : Event_Impl(pOCClientSocket, DeviceID) {};
...
class Linuxmce_NVR_Data : public DeviceData_Impl
{
public:
	...
	*/
	virtual int GetPK_DeviceList() { return DEVICETEMPLATE_Linuxmce_NVR_CONST; } ;

	/**
	* @brief Returns the description of the device
	*/
	virtual const char *GetDeviceDescription() { return "Linuxmce_NVR"; } ;
	...
	*/
	...
	{
		SetParm(DEVICEDATA_Listen_Port_CONST,StringUtils::itos(Value).c_str());
	}
	string Get_EmailSettings()
	{
		if( m_bRunningWithoutDeviceData )
			return m_pEvent_Impl->GetDeviceDataFromDatabase(m_dwPK_Device,DEVICEDATA_EmailSettings_CONST);
		else
			return m_mapParameters[DEVICEDATA_EmailSettings_CONST];
	}

	void Set_EmailSettings(string Value)
	{
		SetParm(DEVICEDATA_EmailSettings_CONST,Value.c_str());
	}
};
...
class Linuxmce_NVR_Command : public Command_Impl
{
public:
	...
	{
		m_pData=NULL;
		m_pEvent = new Linuxmce_NVR_Event(m_dwPK_Device, m_sHostName, !m_bLocalMode);
		...
		m_pcRequestSocket = new Event_Impl(m_dwPK_Device, DEVICETEMPLATE_Linuxmce_NVR_CONST,m_sHostName);
		if( m_iInstanceID )
		{
	...
	Linuxmce_NVR_Command(Command_Impl *pPrimaryDeviceCommand, DeviceData_Impl *pData, Event_Impl *pEvent, Router *pRouter) : Command_Impl(pPrimaryDeviceCommand, pData, pEvent, pRouter) {};
	virtual ~Linuxmce_NVR_Command() {};
	Linuxmce_NVR_Event *GetEvents() { return (Linuxmce_NVR_Event *) m_pEvent; };
	Linuxmce_NVR_Data *GetData() { return (Linuxmce_NVR_Data *) m_pData; };
	const char *GetClassName() { return "Linuxmce_NVR_Command"; };
	virtual int PK_DeviceTemplate_get() { return DEVICETEMPLATE_Linuxmce_NVR_CONST; };
	static int PK_DeviceTemplate_get_static() { return DEVICETEMPLATE_Linuxmce_NVR_CONST; };
	...
	void DATA_Set_Listen_Port(int Value,bool bUpdateDatabase=false) { GetData()->Set_Listen_Port(Value); if( bUpdateDatabase ) SetDeviceDataInDB(m_dwPK_Device,119,Value); }
	string DATA_Get_EmailSettings() { return GetData()->Get_EmailSettings(); }
	void DATA_Set_EmailSettings(string Value,bool bUpdateDatabase=false) { GetData()->Set_EmailSettings(Value); if( bUpdateDatabase ) SetDeviceDataInDB(m_dwPK_Device,304,Value); }
	//Event accessors
	...
	virtual ReceivedMessageResult ReceivedMessage(class Message *pMessageOriginal)
	{
		...
		for(int s=-1;s<(int) pMessageOriginal->m_vectExtraMessages.size(); ++s)
		{
			...
			if (pMessage->m_dwPK_Device_To==m_dwPK_Device && pMessage->m_dwMessage_Type == MESSAGETYPE_COMMAND)
			{
				...
				{
				case COMMAND_Get_Capture_Video_Frame_CONST:
					{
						string sCMD_Result="OK";
						int iCameraID=atoi(pMessage->m_mapParameters[COMMANDPARAMETER_CameraID_CONST].c_str());
