...
touch /var/log/pluto/DownloadPackages.log
exec 6>&1
exec >>/var/log/pluto/DownloadPackages.log 2>&1
...
if [[ "$ThisVersion" != "" ]] ;then
	...
	PermanentPkgs=" pluto-kernel-upgrade pluto-avwizard-sounds pluto-fuse-smb " # keep the leading and trailing spaces
	RemovedList="0==1"
