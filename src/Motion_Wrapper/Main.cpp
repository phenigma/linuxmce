...
int main(int argc, char* argv[]) 
{
	...
	try
	{
		if( sLogger=="dcerouter" )
			LoggerWrapper::SetInstance(new ServerLogger(PK_Device, Motion_Wrapper::PK_DeviceTemplate_get_static(), sRouter_IP));
	...
	try
	{
		...
		if ( pMotion_Wrapper->GetConfig() && pMotion_Wrapper->Connect(pMotion_Wrapper->PK_DeviceTemplate_get()) ) 
		{
			...
			{
				if(pMotion_Wrapper->m_RequestHandlerThread)
		...
		{
			bAppError = true;
			if( pMotion_Wrapper->m_pEvent && pMotion_Wrapper->m_pEvent->m_pClientSocket && pMotion_Wrapper->m_pEvent->m_pClientSocket->m_eLastError==ClientSocket::cs_err_CannotConnect )
			{
				bAppError = false;
				bReload = false;
