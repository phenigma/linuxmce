...
extern "C" {
	...
	{ 
		...
		if( bIsRuntimePlugin )
			return LIRC_DCE::PK_DeviceTemplate_get_static();
		else
			return 0;
	}
}
...
int main(int argc, char* argv[]) 
{
	...
	if (bError)
	{
		cout << "A Pluto DCE Device.  See www.plutohome.com/dce for details." << endl
			<< "Usage: LIRC_DCE [-r Router's IP] [-d My Device ID] [-l dcerouter|stdout|null|filename]" << endl
	...
	try
	{
		if( sLogger=="dcerouter" )
			LoggerWrapper::SetInstance(new ServerLogger(PK_Device, LIRC_DCE::PK_DeviceTemplate_get_static(), sRouter_IP));
	...
	try
	{
		...
		if ( pLIRC_DCE->GetConfig() && pLIRC_DCE->Connect(pLIRC_DCE->PK_DeviceTemplate_get()) ) 
		{
			...
			pLIRC_DCE->CreateChildren();
			if( bLocalMode )
				pLIRC_DCE->RunLocalMode();
			else
			{
				if(pLIRC_DCE->m_RequestHandlerThread)
					pthread_join(pLIRC_DCE->m_RequestHandlerThread, NULL);  // This function will return when the device is shutting down
			}
		...
		{
			bAppError = true;
			if( pLIRC_DCE->m_pEvent && pLIRC_DCE->m_pEvent->m_pClientSocket && pLIRC_DCE->m_pEvent->m_pClientSocket->m_eLastError==ClientSocket::cs_err_CannotConnect )
			{
				bAppError = false;
				bReload = false;
