...
extern "C" {
	...
	{ 
		...
		if( bIsRuntimePlugin )
			return Advanced_IP_Camera::PK_DeviceTemplate_get_static();
		else
			return 0;
	}
}
...
extern "C" {
	class Command_Impl *RegisterAsPlugIn(class Router *pRouter,int PK_Device,Logger *pPlutoLogger)
	{
		...
		Advanced_IP_Camera *pAdvanced_IP_Camera = new Advanced_IP_Camera(PK_Device, "localhost",true,false,pRouter);
		if( pAdvanced_IP_Camera->m_bQuit_get()|| !pAdvanced_IP_Camera->GetConfig() )
		{
			delete pAdvanced_IP_Camera;
			return NULL;
		}
		else
		{
			g_pCommand_Impl=pAdvanced_IP_Camera;
			g_pDeadlockHandler=Plugin_DeadlockHandler;
			g_pSocketCrashHandler=Plugin_SocketCrashHandler;
		}
		return pAdvanced_IP_Camera;
	}
}
//<-dceag-plug-e->
...
int main(int argc, char* argv[]) 
{
	...
	cout << "Advanced_IP_Camera, v." << VERSION << endl
	...
	if (bError)
	{
		cout << "A Pluto DCE Device.  See www.plutohome.com/dce for details." << endl
			<< "Usage: Advanced_IP_Camera [-r Router's IP] [-d My Device ID] [-l dcerouter|stdout|null|filename]" << endl
	...
	try
	{
		if( sLogger=="dcerouter" )
			LoggerWrapper::SetInstance(new ServerLogger(PK_Device, Advanced_IP_Camera::PK_DeviceTemplate_get_static(), sRouter_IP));
	...
	try
	{
		Advanced_IP_Camera *pAdvanced_IP_Camera = new Advanced_IP_Camera(PK_Device, sRouter_IP,true,bLocalMode);
		if ( pAdvanced_IP_Camera->GetConfig() && pAdvanced_IP_Camera->Connect(pAdvanced_IP_Camera->PK_DeviceTemplate_get()) ) 
		{
			g_pCommand_Impl=pAdvanced_IP_Camera;
			g_pDeadlockHandler=DeadlockHandler;
			g_pSocketCrashHandler=SocketCrashHandler;
			LoggerWrapper::GetInstance()->Write(LV_STATUS, "Connect OK");
			pAdvanced_IP_Camera->CreateChildren();
			if( bLocalMode )
				pAdvanced_IP_Camera->RunLocalMode();
			else
			{
				if(pAdvanced_IP_Camera->m_RequestHandlerThread)
					pthread_join(pAdvanced_IP_Camera->m_RequestHandlerThread, NULL);  // This function will return when the device is shutting down
			}
		...
		{
			bAppError = true;
			if( pAdvanced_IP_Camera->m_pEvent && pAdvanced_IP_Camera->m_pEvent->m_pClientSocket && pAdvanced_IP_Camera->m_pEvent->m_pClientSocket->m_eLastError==ClientSocket::cs_err_CannotConnect )
			{
				bAppError = false;
				bReload = false;
		...
		if( pAdvanced_IP_Camera->m_bReload )
			bReload=true;
