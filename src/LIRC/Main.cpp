...
extern "C" {
	...
	{ 
		...
		if( bIsRuntimePlugin )
			return LIRC::PK_DeviceTemplate_get_static();
		else
			return 0;
	}
}
...
extern "C" {
	class Command_Impl *RegisterAsPlugIn(class Router *pRouter,int PK_Device,string sLogger)
	{
		...
		{
			g_pPlutoLogger = new ServerLogger(PK_Device, LIRC::PK_DeviceTemplate_get_static(), "localhost");
		...
		LIRC *pLIRC = new LIRC(PK_Device, "localhost",true,false,pRouter);
		if( pLIRC->m_bQuit )
		{
			delete pLIRC;
			return NULL;
...
int main(int argc, char* argv[]) 
{
	...
	if (bError)
	{
		cout << "A Pluto DCE Device.  See www.plutohome.com/dce for details." << endl
			<< "Usage: LIRC [-r Router's IP] [-d My Device ID] [-l dcerouter|stdout|null|filename]" << endl
	...
	{
		if( sLogger=="dcerouter" )
			g_pPlutoLogger = new ServerLogger(PK_Device, LIRC::PK_DeviceTemplate_get_static(), sRouter_IP);
	...
	try
	{
		LIRC *pLIRC = new LIRC(PK_Device, sRouter_IP);	
		if ( pLIRC->Connect(pLIRC->PK_DeviceTemplate_get()) ) 
		{
			g_pCommand_Impl=pLIRC;
			g_pDeadlockHandler=DeadlockHandler;
			g_pSocketCrashHandler=SocketCrashHandler;
			g_pPlutoLogger->Write(LV_STATUS, "Connect OK");
			pLIRC->CreateChildren();
			pthread_join(pLIRC->m_RequestHandlerThread, NULL);  // This function will return when the device is shutting down
