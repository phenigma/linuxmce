...
set +e
	...
	parted -s "$TargetHdd" -- mkpart primary ext2 0 500MB
	parted -s "$TargetHdd" -- mkpart extended 500MB -1s
	parted -s "$TargetHdd" -- mkpart logical linux-swap 500MB 1500MB
	parted -s "$TargetHdd" -- mkpart logical ext2 1500MB 6500MB
	parted -s "$TargetHdd" -- mkpart logical ext2 6500MB 11500MB
	sleep 1
...
FormatPartitions()
{
	...
	mkswap "$TargetHdd"5
	echo y|mkfs.ext3 "$TargetHdd"6
	echo y|mkfs.ext3 "$TargetHdd"7
