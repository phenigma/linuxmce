...
extern "C" {
	...
	{ 
		...
		if( bIsRuntimePlugin )
			return Insteon_PLM_DCE::PK_DeviceTemplate_get_static();
		else
			return 0;
	}
}
...
extern "C" {
	class Command_Impl *RegisterAsPlugIn(class Router *pRouter,int PK_Device,Logger *pPlutoLogger)
	{
		...
		Insteon_PLM_DCE *pInsteon_PLM_DCE = new Insteon_PLM_DCE(PK_Device, "localhost",true,false,pRouter);
		if( pInsteon_PLM_DCE->m_bQuit_get()|| !pInsteon_PLM_DCE->GetConfig() )
		{
			delete pInsteon_PLM_DCE;
			return NULL;
		}
		else
		{
			g_pCommand_Impl=pInsteon_PLM_DCE;
			g_pDeadlockHandler=Plugin_DeadlockHandler;
			g_pSocketCrashHandler=Plugin_SocketCrashHandler;
		}
		return pInsteon_PLM_DCE;
	}
}
//<-dceag-plug-e->
...
int main(int argc, char* argv[]) 
{
	...
	cout << "Insteon_PLM_DCE, v." << VERSION << endl
	...
	if (bError)
	{
		cout << "A Pluto DCE Device.  See www.plutohome.com/dce for details." << endl
			<< "Usage: Insteon_PLM_DCE [-r Router's IP] [-d My Device ID] [-l dcerouter|stdout|null|filename]" << endl
	...
	try
	{
		if( sLogger=="dcerouter" )
			LoggerWrapper::SetInstance(new ServerLogger(PK_Device, Insteon_PLM_DCE::PK_DeviceTemplate_get_static(), sRouter_IP));
	...
	try
	{
		Insteon_PLM_DCE *pInsteon_PLM_DCE = new Insteon_PLM_DCE(PK_Device, sRouter_IP,true,bLocalMode);
		if ( pInsteon_PLM_DCE->GetConfig() && pInsteon_PLM_DCE->Connect(pInsteon_PLM_DCE->PK_DeviceTemplate_get()) ) 
		{
			g_pCommand_Impl=pInsteon_PLM_DCE;
			g_pDeadlockHandler=DeadlockHandler;
			g_pSocketCrashHandler=SocketCrashHandler;
			LoggerWrapper::GetInstance()->Write(LV_STATUS, "Connect OK");
			pInsteon_PLM_DCE->CreateChildren();
			if( bLocalMode )
				pInsteon_PLM_DCE->RunLocalMode();
			else
			{
				if(pInsteon_PLM_DCE->m_RequestHandlerThread)
					pthread_join(pInsteon_PLM_DCE->m_RequestHandlerThread, NULL);  // This function will return when the device is shutting down
			}
		...
		{
			bAppError = true;
			if( pInsteon_PLM_DCE->m_pEvent && pInsteon_PLM_DCE->m_pEvent->m_pClientSocket && pInsteon_PLM_DCE->m_pEvent->m_pClientSocket->m_eLastError==ClientSocket::cs_err_CannotConnect )
			{
				bAppError = false;
				bReload = false;
