...
extern "C" {
	class Command_Impl *RegisterAsPlugIn(class Router *pRouter,int PK_Device,string sLogger)
	{
		...
		{
			g_pPlutoLogger = new ServerLogger(PK_Device, Event_Plugin::PK_DeviceTemplate_get_static(), "localhost");
...
int main(int argc, char* argv[]) 
{
	...
	{
		if( sLogger=="dcerouter" )
			g_pPlutoLogger = new ServerLogger(PK_Device, Event_Plugin::PK_DeviceTemplate_get_static(), sRouter_IP);
	...
	try
	{
		...
		if ( pEvent_Plugin->Connect(pEvent_Plugin->PK_DeviceTemplate_get()) ) 
		{
			...
			pthread_join(pEvent_Plugin->m_RequestHandlerThread, NULL);
