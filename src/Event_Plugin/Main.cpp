...
extern "C" {
	...
	{ 
		...
		if( bIsRuntimePlugin )
			return Event_Plugin::PK_DeviceTemplate_get_static();
		else
			return 0;
	}
}
...
int main(int argc, char* argv[]) 
{
	...
	cout << "Event_Plugin, v." << VERSION << endl
	...
	if (bError)
	{
		cout << "A Pluto DCE Device.  See www.plutohome.com/dce for details." << endl
			<< "Usage: Event_Plugin [-r Router's IP] [-d My Device ID] [-l dcerouter|stdout|null|filename]" << endl
	...
	try
	{
		if( sLogger=="dcerouter" )
			LoggerWrapper::SetInstance(new ServerLogger(PK_Device, Event_Plugin::PK_DeviceTemplate_get_static(), sRouter_IP));
	...
	try
	{
		...
		if ( pEvent_Plugin->GetConfig() && pEvent_Plugin->Connect(pEvent_Plugin->PK_DeviceTemplate_get()) ) 
		{
			...
			{
				if(pEvent_Plugin->m_RequestHandlerThread)
		...
		{
			bAppError = true;
			if( pEvent_Plugin->m_pEvent && pEvent_Plugin->m_pEvent->m_pClientSocket && pEvent_Plugin->m_pEvent->m_pClientSocket->m_eLastError==ClientSocket::cs_err_CannotConnect )
			{
				bAppError = false;
				bReload = false;
