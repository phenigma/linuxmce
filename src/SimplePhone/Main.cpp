...
#define  VERSION "2.0.0.45.13012427122"
const char *g_szCompile_Date="Thu Jan 24 20:27:58 2013";
int g_SvnRevision=27122;
...
extern "C" {
	...
	{ 
		...
		if( bIsRuntimePlugin )
			return SimplePhone::PK_DeviceTemplate_get_static();
		else
			return 0;
	}
}
...
int main(int argc, char* argv[]) 
{
	...
	cout << "SimplePhone, v." << VERSION << endl
	...
	if (bError)
	{
		cout << "A Pluto DCE Device.  See www.plutohome.com/dce for details." << endl
			<< "Usage: SimplePhone [-r Router's IP] [-d My Device ID] [-l dcerouter|stdout|null|filename]" << endl
	...
	try
	{
		if( sLogger=="dcerouter" )
			LoggerWrapper::SetInstance(new ServerLogger(PK_Device, SimplePhone::PK_DeviceTemplate_get_static(), sRouter_IP));
	...
	try
	{
		SimplePhone *pSimplePhone = new SimplePhone(PK_Device, sRouter_IP,true,bLocalMode);
		if ( pSimplePhone->GetConfig() && pSimplePhone->Connect(pSimplePhone->PK_DeviceTemplate_get()) ) 
		{
			g_pCommand_Impl=pSimplePhone;
			g_pDeadlockHandler=DeadlockHandler;
			g_pSocketCrashHandler=SocketCrashHandler;
			LoggerWrapper::GetInstance()->Write(LV_STATUS, "Connect OK");
			pSimplePhone->CreateChildren();
			if( bLocalMode )
				pSimplePhone->RunLocalMode();
			else
			{
				if(pSimplePhone->m_RequestHandlerThread)
					pthread_join(pSimplePhone->m_RequestHandlerThread, NULL);  // This function will return when the device is shutting down
			}
		...
		{
			bAppError = true;
			if( pSimplePhone->m_pEvent && pSimplePhone->m_pEvent->m_pClientSocket && pSimplePhone->m_pEvent->m_pClientSocket->m_eLastError==ClientSocket::cs_err_CannotConnect )
			{
				bAppError = false;
				bReload = false;
