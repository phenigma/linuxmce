...
/* Double Talk Detector hangover in taps. 
 * Small Value (0ms = No DTD) is good for Single-Talk Echo cancellation,
 * Large Value (30ms) is good for Double-Talk Echo cancellation.
 */
const int Thold = 30 * WIDEB * 8;
...
class AEC {
  ...
  IIR1 Fxx, Fdd;                  // pre-whitening Highpass
  ...
  // Different ageing Near-End Far-End Ratio DTD
  REAL dfast, xfast;    
  REAL dslow, xslow;

  // H-infinity max
  double max_max_u;               // max(|x[0]|, .. |x[L-1]|)
  // optimize: less calculations for max()
  REAL max_u[NLMS_LEN / DTD_LEN];
  int c_max_u;
  int i_max_u;
