...
extern "C" {
	...
	{ 
		...
		if( bIsRuntimePlugin )
			return Pandora_Plugin::PK_DeviceTemplate_get_static();
		else
			return 0;
	}
}
...
extern "C" {
	class Command_Impl *RegisterAsPlugIn(class Router *pRouter,int PK_Device,Logger *pPlutoLogger)
	{
		...
		Pandora_Plugin *pPandora_Plugin = new Pandora_Plugin(PK_Device, "localhost",true,false,pRouter);
		if( pPandora_Plugin->m_bQuit_get()|| !pPandora_Plugin->GetConfig() )
		{
			delete pPandora_Plugin;
			return NULL;
		}
		else
		{
			g_pCommand_Impl=pPandora_Plugin;
			g_pDeadlockHandler=Plugin_DeadlockHandler;
			g_pSocketCrashHandler=Plugin_SocketCrashHandler;
		}
		return pPandora_Plugin;
	}
}
//<-dceag-plug-e->
...
int main(int argc, char* argv[]) 
{
	...
	cout << "Pandora_Plugin, v." << VERSION << endl
	...
	if (bError)
	{
		cout << "A Pluto DCE Device.  See www.plutohome.com/dce for details." << endl
			<< "Usage: Pandora_Plugin [-r Router's IP] [-d My Device ID] [-l dcerouter|stdout|null|filename]" << endl
	...
	try
	{
		if( sLogger=="dcerouter" )
			LoggerWrapper::SetInstance(new ServerLogger(PK_Device, Pandora_Plugin::PK_DeviceTemplate_get_static(), sRouter_IP));
	...
	try
	{
		Pandora_Plugin *pPandora_Plugin = new Pandora_Plugin(PK_Device, sRouter_IP,true,bLocalMode);
		if ( pPandora_Plugin->GetConfig() && pPandora_Plugin->Connect(pPandora_Plugin->PK_DeviceTemplate_get()) ) 
		{
			g_pCommand_Impl=pPandora_Plugin;
			g_pDeadlockHandler=DeadlockHandler;
			g_pSocketCrashHandler=SocketCrashHandler;
			LoggerWrapper::GetInstance()->Write(LV_STATUS, "Connect OK");
			pPandora_Plugin->CreateChildren();
			if( bLocalMode )
				pPandora_Plugin->RunLocalMode();
			else
			{
				if(pPandora_Plugin->m_RequestHandlerThread)
					pthread_join(pPandora_Plugin->m_RequestHandlerThread, NULL);  // This function will return when the device is shutting down
			}
		...
		{
			bAppError = true;
			if( pPandora_Plugin->m_pEvent && pPandora_Plugin->m_pEvent->m_pClientSocket && pPandora_Plugin->m_pEvent->m_pClientSocket->m_eLastError==ClientSocket::cs_err_CannotConnect )
			{
				bAppError = false;
				bReload = false;
