...
int main(int argc, char* argv[]) 
{
	...
	if (bError)
	{
		cout << "A Pluto DCE Device.  See www.plutohome.com/dce for details." << endl
			<< "Usage: Bluetooth_Dongle [-r Router's IP] [-d My Device ID] [-l dcerouter|stdout|null|filename]" << endl
	...
	try
	{
		Bluetooth_Dongle *pBluetooth_Dongle = new Bluetooth_Dongle(PK_Device, sRouter_IP,true,bLocalMode);
		...
		{
			bAppError = true;
			if( pBluetooth_Dongle->m_pEvent && pBluetooth_Dongle->m_pEvent->m_pClientSocket && pBluetooth_Dongle->m_pEvent->m_pClientSocket->m_eLastError==ClientSocket::cs_err_CannotConnect )
			{
				bAppError = false;
				bReload = false;
