...
extern "C" {
	...
	{ 
		...
		if( bIsRuntimePlugin )
			return JSON_Plugin::PK_DeviceTemplate_get_static();
		else
			return 0;
	}
}
...
extern "C" {
	class Command_Impl *RegisterAsPlugIn(class Router *pRouter,int PK_Device,Logger *pPlutoLogger)
	{
		...
		JSON_Plugin *pJSON_Plugin = new JSON_Plugin(PK_Device, "localhost",true,false,pRouter);
		if( pJSON_Plugin->m_bQuit_get()|| !pJSON_Plugin->GetConfig() )
		{
			delete pJSON_Plugin;
			return NULL;
		}
...
int main(int argc, char* argv[]) 
{
	...
	cout << "JSON_Plugin, v." << VERSION << endl
	...
	if (bError)
	{
		cout << "A Pluto DCE Device.  See www.plutohome.com/dce for details." << endl
			<< "Usage: JSON_Plugin [-r Router's IP] [-d My Device ID] [-l dcerouter|stdout|null|filename]" << endl
	...
	try
	{
		if( sLogger=="dcerouter" )
			LoggerWrapper::SetInstance(new ServerLogger(PK_Device, JSON_Plugin::PK_DeviceTemplate_get_static(), sRouter_IP));
	...
	try
	{
		JSON_Plugin *pJSON_Plugin = new JSON_Plugin(PK_Device, sRouter_IP,true,bLocalMode);
		if ( pJSON_Plugin->GetConfig() && pJSON_Plugin->Connect(pJSON_Plugin->PK_DeviceTemplate_get()) ) 
		{
			g_pCommand_Impl=pJSON_Plugin;
			g_pDeadlockHandler=DeadlockHandler;
			g_pSocketCrashHandler=SocketCrashHandler;
			LoggerWrapper::GetInstance()->Write(LV_STATUS, "Connect OK");
			pJSON_Plugin->CreateChildren();
			if( bLocalMode )
				pJSON_Plugin->RunLocalMode();
			else
			{
				if(pJSON_Plugin->m_RequestHandlerThread)
					pthread_join(pJSON_Plugin->m_RequestHandlerThread, NULL);  // This function will return when the device is shutting down
			}
		...
		{
			bAppError = true;
			if( pJSON_Plugin->m_pEvent && pJSON_Plugin->m_pEvent->m_pClientSocket && pJSON_Plugin->m_pEvent->m_pClientSocket->m_eLastError==ClientSocket::cs_err_CannotConnect )
			{
				bAppError = false;
				bReload = false;
