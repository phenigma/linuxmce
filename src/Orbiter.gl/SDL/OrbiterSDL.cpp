...
#include "../OpenGL/Layers/GL2DEffectLayersCompose.h"
...
void *Orbiter_OpenGLThread(void *p)
{
	...
	while(!pOrbiter->m_bQuit)
	{
		...
		if (!pOrbiter->m_Desktop->HasEffects()) {
...
/*virtual*/ void OrbiterSDL::RenderScreen( bool bRenderGraphicsOnly )
{
	...
	else //EnableOpenGL
	{
		...
		if(m_pObj_SelectedLastScreen)
		{
			...
			if(m_Desktop)
			{
				GLEffect2D::LayersCompose::GetInstance()->ConfigureNextScreen(m_spAfterGraphic.get());
				GL2DEffect* Transit = m_Desktop->
					CreateEffect(
					GLEffect2D::DesktopOffScreen,
					GL2DEffectFactory::GetEffectCode(m_pObj_SelectedLastScreen->m_FK_Effect_Selected_WithChange),
						3500
...
void OrbiterSDL::SelectObject( class DesignObj_Orbiter *pObj, PlutoPoint point )
{
	...
	GL2DEffect * Effect = m_Desktop->CreateEffect(
		GLEffect2D::DesktopOffScreen,
		GL2D_EFFECT_SELECT_AREA, 
		Simulator::GetInstance()->m_iMilisecondsHighLight);
