...
#include "Tests/ExtensionManager.h"
...
{
	GLEffect2D::LayersCompose::GetInstance()->Paint();
	SDL_GL_SwapBuffers();
}
...
//    pOrbiter->InitializeAfterSetVideoMode();
	...
	*/
	...
	glOrtho(0, pOrbiter->m_iImageWidth, 0, 
		pOrbiter->m_iImageHeight, 0, pOrbiter->m_iImageWidth*4);
		*/
	
	gluPerspective(90.0f, (float)pOrbiter->m_iImageWidth/pOrbiter->m_iImageHeight,
		0, pOrbiter->m_iImageWidth*4);
	...
	glScalef(1, -1.0f, 1);
	glTranslatef((GLfloat)-pOrbiter->m_iImageWidth/2, (GLfloat)-pOrbiter->m_iImageHeight/2, 
		(GLfloat)-pOrbiter->m_iImageHeight/2);
...
int OrbiterGL3D::BuildOrbiterGL(Orbiter * pOrbiterGL)
{
	...
	GLEffect2D::LayersCompose::GetInstance()->CreateLayer(GLEffect2D::DesktopOffScreen);
	GLEffect2D::LayersCompose::GetInstance()->CreateLayer(GLEffect2D::DesktopOnScreen);

	Screen3D = (TBasicWindow *) GLEffect2D::LayersCompose::GetInstance()->CreateWidget(
	...
	HighLighArea = (TBasicWindow *) GLEffect2D::LayersCompose::GetInstance()->CreateWidget(
		GLEffect2D::DesktopOffScreen, 
		BASICWINDOW, 
		0, 0, 
		(int)0,
	...
	SelectedArea= (TBasicWindow *) GLEffect2D::LayersCompose::GetInstance()->CreateWidget(
		GLEffect2D::DesktopOffScreen, 
		BASICWINDOW, 
		0, 0, 
		(int)0,
	...
	Commons3D::Instance().BuildCommons(Screen3D, HighLighArea, SelectedArea);
	return 1;
}

bool OrbiterGL3D::HasEffects()
{
	return GLEffect2D::LayersCompose::GetInstance()->HasEffects();
}

GL2DEffect* OrbiterGL3D::CreateEffect(int LayerLevel, int IDEffect, int TimeForComplete)
{
	return GLEffect2D::LayersCompose::GetInstance()->CreateEffect(LayerLevel, IDEffect, TimeForComplete);
}