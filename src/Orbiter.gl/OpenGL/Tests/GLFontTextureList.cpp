		...
		)
{
	...
	for(int i = 1; i<256; i++)
	{
		...
		Letters[i].Convert(RenderedText);
...
/*virtual*/ int GLFontTextureList::TextOut(int X, int Y, char* Text)
{
	int Result = 0;
...
*/
	...
	for(int i = 0; i<Length; i++)
	{
		...
		// Point 1
		MB.SetTexture2D(0.0f, 0.0f);
		MB.AddVertexFloat((float)X, (float)Y, 480);

		// Point 2
		MB.SetTexture2D(0.0f, PixelMaxV);
		MB.AddVertexFloat((float)X, (float)Y+PixelHgt, 480);

		// Point 3
		MB.SetTexture2D(PixelMaxU, 0.0f);
		MB.AddVertexFloat((float)X+PixelLen, (float)Y, 480);

		// Point 4
		MB.SetTexture2D(PixelMaxU, PixelMaxV);
		MB.AddVertexFloat((float)X+PixelLen, (float)Y+PixelHgt, 480);
