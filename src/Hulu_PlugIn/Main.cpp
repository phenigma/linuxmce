...
extern "C" {
	...
	{ 
		...
		if( bIsRuntimePlugin )
			return Hulu_PlugIn::PK_DeviceTemplate_get_static();
		else
			return 0;
	}
}
...
extern "C" {
	class Command_Impl *RegisterAsPlugIn(class Router *pRouter,int PK_Device,Logger *pPlutoLogger)
	{
		...
		Hulu_PlugIn *pHulu_PlugIn = new Hulu_PlugIn(PK_Device, "localhost",true,false,pRouter);
		if( pHulu_PlugIn->m_bQuit_get()|| !pHulu_PlugIn->GetConfig() )
		{
			delete pHulu_PlugIn;
			return NULL;
		}
...
int main(int argc, char* argv[]) 
{
	...
	cout << "Hulu_PlugIn, v." << VERSION << endl
	...
	if (bError)
	{
		cout << "A Pluto DCE Device.  See www.plutohome.com/dce for details." << endl
			<< "Usage: Hulu_PlugIn [-r Router's IP] [-d My Device ID] [-l dcerouter|stdout|null|filename]" << endl
	...
	try
	{
		if( sLogger=="dcerouter" )
			LoggerWrapper::SetInstance(new ServerLogger(PK_Device, Hulu_PlugIn::PK_DeviceTemplate_get_static(), sRouter_IP));
	...
	try
	{
		Hulu_PlugIn *pHulu_PlugIn = new Hulu_PlugIn(PK_Device, sRouter_IP,true,bLocalMode);
		if ( pHulu_PlugIn->GetConfig() && pHulu_PlugIn->Connect(pHulu_PlugIn->PK_DeviceTemplate_get()) ) 
		{
			g_pCommand_Impl=pHulu_PlugIn;
			g_pDeadlockHandler=DeadlockHandler;
			g_pSocketCrashHandler=SocketCrashHandler;
			LoggerWrapper::GetInstance()->Write(LV_STATUS, "Connect OK");
			pHulu_PlugIn->CreateChildren();
			if( bLocalMode )
				pHulu_PlugIn->RunLocalMode();
			else
			{
				if(pHulu_PlugIn->m_RequestHandlerThread)
					pthread_join(pHulu_PlugIn->m_RequestHandlerThread, NULL);  // This function will return when the device is shutting down
			}
		...
		{
			bAppError = true;
			if( pHulu_PlugIn->m_pEvent && pHulu_PlugIn->m_pEvent->m_pClientSocket && pHulu_PlugIn->m_pEvent->m_pClientSocket->m_eLastError==ClientSocket::cs_err_CannotConnect )
			{
				bAppError = false;
				bReload = false;
