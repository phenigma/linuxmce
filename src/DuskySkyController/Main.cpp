...
extern "C" {
	...
	{ 
		...
		if( bIsRuntimePlugin )
			return DuskySkyController::PK_DeviceTemplate_get_static();
		else
			return 0;
	}
}
...
extern "C" {
	class Command_Impl *RegisterAsPlugIn(class Router *pRouter,int PK_Device,Logger *pPlutoLogger)
	{
		...
		DuskySkyController *pDuskySkyController = new DuskySkyController(PK_Device, "localhost",true,false,pRouter);
		if( pDuskySkyController->m_bQuit_get()|| !pDuskySkyController->GetConfig() )
		{
			delete pDuskySkyController;
			return NULL;
		}
		else
		{
			g_pCommand_Impl=pDuskySkyController;
			g_pDeadlockHandler=Plugin_DeadlockHandler;
			g_pSocketCrashHandler=Plugin_SocketCrashHandler;
		}
		return pDuskySkyController;
	}
}
//<-dceag-plug-e->
...
int main(int argc, char* argv[]) 
{
	...
	cout << "DuskySkyController, v." << VERSION << endl
	...
	if (bError)
	{
		cout << "A Pluto DCE Device.  See www.plutohome.com/dce for details." << endl
			<< "Usage: DuskySkyController [-r Router's IP] [-d My Device ID] [-l dcerouter|stdout|null|filename]" << endl
	...
	try
	{
		if( sLogger=="dcerouter" )
			LoggerWrapper::SetInstance(new ServerLogger(PK_Device, DuskySkyController::PK_DeviceTemplate_get_static(), sRouter_IP));
	...
	try
	{
		DuskySkyController *pDuskySkyController = new DuskySkyController(PK_Device, sRouter_IP,true,bLocalMode);
		if ( pDuskySkyController->GetConfig() && pDuskySkyController->Connect(pDuskySkyController->PK_DeviceTemplate_get()) ) 
		{
			g_pCommand_Impl=pDuskySkyController;
			g_pDeadlockHandler=DeadlockHandler;
			g_pSocketCrashHandler=SocketCrashHandler;
			LoggerWrapper::GetInstance()->Write(LV_STATUS, "Connect OK");
			pDuskySkyController->CreateChildren();
			if( bLocalMode )
				pDuskySkyController->RunLocalMode();
			else
			{
				if(pDuskySkyController->m_RequestHandlerThread)
					pthread_join(pDuskySkyController->m_RequestHandlerThread, NULL);  // This function will return when the device is shutting down
			}
		...
		{
			bAppError = true;
			if( pDuskySkyController->m_pEvent && pDuskySkyController->m_pEvent->m_pClientSocket && pDuskySkyController->m_pEvent->m_pClientSocket->m_eLastError==ClientSocket::cs_err_CannotConnect )
			{
				bAppError = false;
				bReload = false;
		...
		if( pDuskySkyController->m_bReload )
			bReload=true;
