...
extern "C" {
	...
	{ 
		...
		if( bIsRuntimePlugin )
			return Shoutcast_Radio_Plugin::PK_DeviceTemplate_get_static();
		else
			return 0;
	}
}
...
extern "C" {
	class Command_Impl *RegisterAsPlugIn(class Router *pRouter,int PK_Device,Logger *pPlutoLogger)
	{
		...
		Shoutcast_Radio_Plugin *pShoutcast_Radio_Plugin = new Shoutcast_Radio_Plugin(PK_Device, "localhost",true,false,pRouter);
		if( pShoutcast_Radio_Plugin->m_bQuit_get()|| !pShoutcast_Radio_Plugin->GetConfig() )
		{
			delete pShoutcast_Radio_Plugin;
			return NULL;
		}
		else
		{
			g_pCommand_Impl=pShoutcast_Radio_Plugin;
			g_pDeadlockHandler=Plugin_DeadlockHandler;
			g_pSocketCrashHandler=Plugin_SocketCrashHandler;
		}
		return pShoutcast_Radio_Plugin;
	}
}
//<-dceag-plug-e->
...
int main(int argc, char* argv[]) 
{
	...
	cout << "Shoutcast_Radio_Plugin, v." << VERSION << endl
	...
	if (bError)
	{
		cout << "A Pluto DCE Device.  See www.plutohome.com/dce for details." << endl
			<< "Usage: Shoutcast_Radio_Plugin [-r Router's IP] [-d My Device ID] [-l dcerouter|stdout|null|filename]" << endl
	...
	try
	{
		if( sLogger=="dcerouter" )
			LoggerWrapper::SetInstance(new ServerLogger(PK_Device, Shoutcast_Radio_Plugin::PK_DeviceTemplate_get_static(), sRouter_IP));
	...
	try
	{
		Shoutcast_Radio_Plugin *pShoutcast_Radio_Plugin = new Shoutcast_Radio_Plugin(PK_Device, sRouter_IP,true,bLocalMode);
		if ( pShoutcast_Radio_Plugin->GetConfig() && pShoutcast_Radio_Plugin->Connect(pShoutcast_Radio_Plugin->PK_DeviceTemplate_get()) ) 
		{
			g_pCommand_Impl=pShoutcast_Radio_Plugin;
			g_pDeadlockHandler=DeadlockHandler;
			g_pSocketCrashHandler=SocketCrashHandler;
			LoggerWrapper::GetInstance()->Write(LV_STATUS, "Connect OK");
			pShoutcast_Radio_Plugin->CreateChildren();
			if( bLocalMode )
				pShoutcast_Radio_Plugin->RunLocalMode();
			else
			{
				if(pShoutcast_Radio_Plugin->m_RequestHandlerThread)
					pthread_join(pShoutcast_Radio_Plugin->m_RequestHandlerThread, NULL);  // This function will return when the device is shutting down
			}
		...
		{
			bAppError = true;
			if( pShoutcast_Radio_Plugin->m_pEvent && pShoutcast_Radio_Plugin->m_pEvent->m_pClientSocket && pShoutcast_Radio_Plugin->m_pEvent->m_pClientSocket->m_eLastError==ClientSocket::cs_err_CannotConnect )
			{
				bAppError = false;
				bReload = false;
		...
		if( pShoutcast_Radio_Plugin->m_bReload )
			bReload=true;
