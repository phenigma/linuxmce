...
extern "C" {
	...
	{ 
		...
		if( bIsRuntimePlugin )
			return Mailx::PK_DeviceTemplate_get_static();
		else
			return 0;
	}
}
...
extern "C" {
	class Command_Impl *RegisterAsPlugIn(class Router *pRouter,int PK_Device,Logger *pPlutoLogger)
	{
		...
		Mailx *pMailx = new Mailx(PK_Device, "localhost",true,false,pRouter);
		if( pMailx->m_bQuit_get()|| !pMailx->GetConfig() )
		{
			delete pMailx;
			return NULL;
		}
...
int main(int argc, char* argv[]) 
{
	...
	if (bError)
	{
		cout << "A Pluto DCE Device.  See www.plutohome.com/dce for details." << endl
			<< "Usage: Mailx [-r Router's IP] [-d My Device ID] [-l dcerouter|stdout|null|filename]" << endl
	...
	try
	{
		if( sLogger=="dcerouter" )
			LoggerWrapper::SetInstance(new ServerLogger(PK_Device, Mailx::PK_DeviceTemplate_get_static(), sRouter_IP));
	...
	try
	{
		Mailx *pMailx = new Mailx(PK_Device, sRouter_IP,true,bLocalMode);
		if ( pMailx->GetConfig() && pMailx->Connect(pMailx->PK_DeviceTemplate_get()) ) 
		{
			g_pCommand_Impl=pMailx;
			g_pDeadlockHandler=DeadlockHandler;
			g_pSocketCrashHandler=SocketCrashHandler;
			LoggerWrapper::GetInstance()->Write(LV_STATUS, "Connect OK");
			pMailx->CreateChildren();
			if( bLocalMode )
				pMailx->RunLocalMode();
			else
			{
				if(pMailx->m_RequestHandlerThread)
					pthread_join(pMailx->m_RequestHandlerThread, NULL);  // This function will return when the device is shutting down
			}
		...
		{
			bAppError = true;
			if( pMailx->m_pEvent && pMailx->m_pEvent->m_pClientSocket && pMailx->m_pEvent->m_pClientSocket->m_eLastError==ClientSocket::cs_err_CannotConnect )
			{
				bAppError = false;
				bReload = false;
