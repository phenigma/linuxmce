...
extern "C" {
	...
	{ 
		...
		if( bIsRuntimePlugin )
			return Weather::PK_DeviceTemplate_get_static();
		else
			return 0;
	}
}
...
extern "C" {
	class Command_Impl *RegisterAsPlugIn(class Router *pRouter,int PK_Device,Logger *pPlutoLogger)
	{
		...
		Weather *pWeather = new Weather(PK_Device, "localhost",true,false,pRouter);
		if( pWeather->m_bQuit_get()|| !pWeather->GetConfig() )
		{
			delete pWeather;
			return NULL;
		}
...
int main(int argc, char* argv[]) 
{
	...
	if (bError)
	{
		cout << "A Pluto DCE Device.  See www.plutohome.com/dce for details." << endl
			<< "Usage: Weather [-r Router's IP] [-d My Device ID] [-l dcerouter|stdout|null|filename]" << endl
	...
	try
	{
		if( sLogger=="dcerouter" )
			LoggerWrapper::SetInstance(new ServerLogger(PK_Device, Weather::PK_DeviceTemplate_get_static(), sRouter_IP));
	...
	try
	{
		Weather *pWeather = new Weather(PK_Device, sRouter_IP,true,bLocalMode);
		if ( pWeather->GetConfig() && pWeather->Connect(pWeather->PK_DeviceTemplate_get()) ) 
		{
			g_pCommand_Impl=pWeather;
			g_pDeadlockHandler=DeadlockHandler;
			g_pSocketCrashHandler=SocketCrashHandler;
			LoggerWrapper::GetInstance()->Write(LV_STATUS, "Connect OK");
			pWeather->CreateChildren();
			if( bLocalMode )
				pWeather->RunLocalMode();
			else
			{
				if(pWeather->m_RequestHandlerThread)
					pthread_join(pWeather->m_RequestHandlerThread, NULL);  // This function will return when the device is shutting down
			}
		...
		{
			bAppError = true;
			if( pWeather->m_pEvent && pWeather->m_pEvent->m_pClientSocket && pWeather->m_pEvent->m_pClientSocket->m_eLastError==ClientSocket::cs_err_CannotConnect )
			{
				bAppError = false;
				bReload = false;
