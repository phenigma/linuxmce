...
extern "C" {
	...
	{ 
		...
		if( bIsRuntimePlugin )
			return Moxi_PVR::PK_DeviceTemplate_get_static();
		else
			return 0;
	}
}
...
extern "C" {
	class Command_Impl *RegisterAsPlugIn(class Router *pRouter,int PK_Device,Logger *pPlutoLogger)
	{
		...
		Moxi_PVR *pMoxi_PVR = new Moxi_PVR(PK_Device, "localhost",true,false,pRouter);
		if( pMoxi_PVR->m_bQuit_get()|| !pMoxi_PVR->GetConfig() )
		{
			delete pMoxi_PVR;
			return NULL;
		}
...
int main(int argc, char* argv[]) 
{
	...
	if (bError)
	{
		cout << "A Pluto DCE Device.  See www.plutohome.com/dce for details." << endl
			<< "Usage: Moxi_PVR [-r Router's IP] [-d My Device ID] [-l dcerouter|stdout|null|filename]" << endl
	...
	try
	{
		if( sLogger=="dcerouter" )
			g_pPlutoLogger = new ServerLogger(PK_Device, Moxi_PVR::PK_DeviceTemplate_get_static(), sRouter_IP);
	...
	try
	{
		Moxi_PVR *pMoxi_PVR = new Moxi_PVR(PK_Device, sRouter_IP,true,bLocalMode);
		if ( pMoxi_PVR->GetConfig() && pMoxi_PVR->Connect(pMoxi_PVR->PK_DeviceTemplate_get()) ) 
		{
			g_pCommand_Impl=pMoxi_PVR;
			g_pDeadlockHandler=DeadlockHandler;
			g_pSocketCrashHandler=SocketCrashHandler;
			g_pPlutoLogger->Write(LV_STATUS, "Connect OK");
			pMoxi_PVR->CreateChildren();
			if( bLocalMode )
				pMoxi_PVR->RunLocalMode();
			else
			{
				if(pMoxi_PVR->m_RequestHandlerThread)
					pthread_join(pMoxi_PVR->m_RequestHandlerThread, NULL);  // This function will return when the device is shutting down
			}
