...
{
	pthread_mutex_init(&IniStore->CriticalSection, NULL);
	return TRUE;
}

void DeInitIniFile(struct IniStore *IniStore)
{
	pthread_mutex_destroy (&IniStore->CriticalSection);
}


char ReadIniFile(char *Filename, struct IniStore *IniStore)
{
	pthread_mutex_lock (&IniStore->CriticalSection);

	FILE *f = fopen(Filename, "r"); //r+,w+
	if (f == NULL)
	{
		IniStore->Count = 0;
		IniStore->Data = NULL;
		pthread_mutex_unlock(&IniStore->CriticalSection);
		return FALSE;
	}
