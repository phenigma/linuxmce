...
int main(int argc, char* argv[]) 
{
	...
	try
	{
		if( sLogger=="dcerouter" )
			LoggerWrapper::SetInstance(new ServerLogger(PK_Device, Telecom_Plugin::PK_DeviceTemplate_get_static(), sRouter_IP));
	...
	try
	{
		Telecom_Plugin *pTelecom_Plugin = new Telecom_Plugin(PK_Device, sRouter_IP,true,bLocalMode);
		if ( pTelecom_Plugin->GetConfig() && pTelecom_Plugin->Connect(pTelecom_Plugin->PK_DeviceTemplate_get()) ) 
		{
			...
			if( bLocalMode )
				pTelecom_Plugin->RunLocalMode();
			else
			{
				if(pTelecom_Plugin->m_RequestHandlerThread)
					pthread_join(pTelecom_Plugin->m_RequestHandlerThread, NULL);  // This function will return when the device is shutting down
			}
		...
		{
			bAppError = true;
			if( pTelecom_Plugin->m_pEvent && pTelecom_Plugin->m_pEvent->m_pClientSocket && pTelecom_Plugin->m_pEvent->m_pClientSocket->m_eLastError==ClientSocket::cs_err_CannotConnect )
			{
				bAppError = false;
				bReload = false;
