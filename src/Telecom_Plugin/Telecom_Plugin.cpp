...
{
	///SELECT CommandGroup.Description,FK_CommandParameter,IK_CommandParameter FROM CommandGroup JOIN CommandGroup_Command ON CommandGroup_Command.FK_CommandGroup=PK_CommandGroup JOIN CommandGroup_Command_CommandParameter ON FK_CommandGroup_Command=PK_CommandGroup_Command JOIN CommandGroup_Room ON CommandGroup_Room.FK_CommandGroup=PK_CommandGroup JOIN Device ON CommandGroup_Room.FK_Room=Device.FK_Room WHERE FK_Array=4 AND AutoGenerated=0 AND Device.PK_Device=XXX ORDER BY PK_CommandGroup,FK_CommandParameter;
	...
	sql_buff += " JOIN Device ON CommandGroup_Room.FK_Room=Device.FK_Room";
	sql_buff += " WHERE AutoGenerated=0";
	sql_buff += " AND FK_Array="+StringUtils::itos(ARRAY_Communication_Scenarios_CONST);
	sql_buff += " AND PK_Device="+StringUtils::itos(pMessage->m_dwPK_Device_From);
	sql_buff += " ORDER BY PK_CommandGroup,FK_CommandParameter";

	g_pPlutoLogger->Write(LV_STATUS, "RecentCalls request received for GridID: %s with Params: %s",GridID.c_str(),Parms.c_str());
	DataGridTable *pDataGrid = new DataGridTable();
	DataGridCell *pCell;
	int Row = 0;
