...
*/
//<-dceag-incl-b->
#include "Generic_PC_Workstation.h"
...
extern "C" {
	...
	{ 
		...
		if( bIsRuntimePlugin )
			return Generic_PC_Workstation::PK_DeviceTemplate_get_static();
		else
			return 0;
	}
}
...
extern "C" {
	class Command_Impl *RegisterAsPlugIn(class Router *pRouter,int PK_Device,Logger *pPlutoLogger)
	{
		...
		Generic_PC_Workstation *pGeneric_PC_Workstation = new Generic_PC_Workstation(PK_Device, "localhost",true,false,pRouter);
		if( pGeneric_PC_Workstation->m_bQuit_get()|| !pGeneric_PC_Workstation->GetConfig() )
		{
			delete pGeneric_PC_Workstation;
			return NULL;
		}
		else
		{
			g_pCommand_Impl=pGeneric_PC_Workstation;
			g_pDeadlockHandler=Plugin_DeadlockHandler;
			g_pSocketCrashHandler=Plugin_SocketCrashHandler;
		}
		return pGeneric_PC_Workstation;
	}
}
//<-dceag-plug-e->
...
int main(int argc, char* argv[]) 
{
	...
	cout << "Generic_PC_Workstation, v." << VERSION << endl
	...
	if (bError)
	{
		cout << "A Pluto DCE Device.  See www.plutohome.com/dce for details." << endl
			<< "Usage: Generic_PC_Workstation [-r Router's IP] [-d My Device ID] [-l dcerouter|stdout|null|filename]" << endl
	...
	try
	{
		if( sLogger=="dcerouter" )
			LoggerWrapper::SetInstance(new ServerLogger(PK_Device, Generic_PC_Workstation::PK_DeviceTemplate_get_static(), sRouter_IP));
	...
	try
	{
		Generic_PC_Workstation *pGeneric_PC_Workstation = new Generic_PC_Workstation(PK_Device, sRouter_IP,true,bLocalMode);
		if ( pGeneric_PC_Workstation->GetConfig() && pGeneric_PC_Workstation->Connect(pGeneric_PC_Workstation->PK_DeviceTemplate_get()) ) 
		{
			g_pCommand_Impl=pGeneric_PC_Workstation;
			g_pDeadlockHandler=DeadlockHandler;
			g_pSocketCrashHandler=SocketCrashHandler;
			LoggerWrapper::GetInstance()->Write(LV_STATUS, "Connect OK");
			pGeneric_PC_Workstation->CreateChildren();
			if( bLocalMode )
				pGeneric_PC_Workstation->RunLocalMode();
			else
			{
				if(pGeneric_PC_Workstation->m_RequestHandlerThread)
					pthread_join(pGeneric_PC_Workstation->m_RequestHandlerThread, NULL);  // This function will return when the device is shutting down
			}
		...
		{
			bAppError = true;
			if( pGeneric_PC_Workstation->m_pEvent && pGeneric_PC_Workstation->m_pEvent->m_pClientSocket && pGeneric_PC_Workstation->m_pEvent->m_pClientSocket->m_eLastError==ClientSocket::cs_err_CannotConnect )
			{
				bAppError = false;
				bReload = false;
		...
		if( pGeneric_PC_Workstation->m_bReload )
			bReload=true;
