...
extern "C" {
	...
	{ 
		...
		if( bIsRuntimePlugin )
			return Clock_Screen_Saver::PK_DeviceTemplate_get_static();
		else
			return 0;
	}
}
...
extern "C" {
	class Command_Impl *RegisterAsPlugIn(class Router *pRouter,int PK_Device,Logger *pPlutoLogger)
	{
		...
		Clock_Screen_Saver *pClock_Screen_Saver = new Clock_Screen_Saver(PK_Device, "localhost",true,false,pRouter);
		if( pClock_Screen_Saver->m_bQuit_get()|| !pClock_Screen_Saver->GetConfig() )
		{
			delete pClock_Screen_Saver;
			return NULL;
		}
		else
		{
			g_pCommand_Impl=pClock_Screen_Saver;
			g_pDeadlockHandler=Plugin_DeadlockHandler;
			g_pSocketCrashHandler=Plugin_SocketCrashHandler;
		}
		return pClock_Screen_Saver;
	}
}
//<-dceag-plug-e->
...
int main(int argc, char* argv[]) 
{
	...
	cout << "Clock_Screen_Saver, v." << VERSION << endl
	...
	if (bError)
	{
		cout << "A Pluto DCE Device.  See www.plutohome.com/dce for details." << endl
			<< "Usage: Clock_Screen_Saver [-r Router's IP] [-d My Device ID] [-l dcerouter|stdout|null|filename]" << endl
	...
	try
	{
		if( sLogger=="dcerouter" )
			LoggerWrapper::SetInstance(new ServerLogger(PK_Device, Clock_Screen_Saver::PK_DeviceTemplate_get_static(), sRouter_IP));
	...
	try
	{
		Clock_Screen_Saver *pClock_Screen_Saver = new Clock_Screen_Saver(PK_Device, sRouter_IP,true,bLocalMode);
		if ( pClock_Screen_Saver->GetConfig() && pClock_Screen_Saver->Connect(pClock_Screen_Saver->PK_DeviceTemplate_get()) ) 
		{
			g_pCommand_Impl=pClock_Screen_Saver;
			g_pDeadlockHandler=DeadlockHandler;
			g_pSocketCrashHandler=SocketCrashHandler;
			LoggerWrapper::GetInstance()->Write(LV_STATUS, "Connect OK");
			pClock_Screen_Saver->CreateChildren();
			if( bLocalMode )
				pClock_Screen_Saver->RunLocalMode();
			else
			{
				if(pClock_Screen_Saver->m_RequestHandlerThread)
					pthread_join(pClock_Screen_Saver->m_RequestHandlerThread, NULL);  // This function will return when the device is shutting down
			}
		...
		{
			bAppError = true;
			if( pClock_Screen_Saver->m_pEvent && pClock_Screen_Saver->m_pEvent->m_pClientSocket && pClock_Screen_Saver->m_pEvent->m_pClientSocket->m_eLastError==ClientSocket::cs_err_CannotConnect )
			{
				bAppError = false;
				bReload = false;
		...
		if( pClock_Screen_Saver->m_bReload )
			bReload=true;
