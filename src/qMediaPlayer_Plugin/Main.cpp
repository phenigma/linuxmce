...
extern "C" {
	...
	{ 
		...
		if( bIsRuntimePlugin )
			return qMediaPlayer_Plugin::PK_DeviceTemplate_get_static();
		else
			return 0;
	}
}
...
extern "C" {
	class Command_Impl *RegisterAsPlugIn(class Router *pRouter,int PK_Device,Logger *pPlutoLogger)
	{
		...
		qMediaPlayer_Plugin *pqMediaPlayer_Plugin = new qMediaPlayer_Plugin(PK_Device, "localhost",true,false,pRouter);
		if( pqMediaPlayer_Plugin->m_bQuit_get()|| !pqMediaPlayer_Plugin->GetConfig() )
		{
			delete pqMediaPlayer_Plugin;
			return NULL;
		}
		else
		{
			g_pCommand_Impl=pqMediaPlayer_Plugin;
			g_pDeadlockHandler=Plugin_DeadlockHandler;
			g_pSocketCrashHandler=Plugin_SocketCrashHandler;
		}
		return pqMediaPlayer_Plugin;
	}
}
//<-dceag-plug-e->
...
int main(int argc, char* argv[]) 
{
	...
	cout << "qMediaPlayer_Plugin, v." << VERSION << endl
	...
	if (bError)
	{
		cout << "A Pluto DCE Device.  See www.plutohome.com/dce for details." << endl
			<< "Usage: qMediaPlayer_Plugin [-r Router's IP] [-d My Device ID] [-l dcerouter|stdout|null|filename]" << endl
	...
	try
	{
		if( sLogger=="dcerouter" )
			LoggerWrapper::SetInstance(new ServerLogger(PK_Device, qMediaPlayer_Plugin::PK_DeviceTemplate_get_static(), sRouter_IP));
	...
	try
	{
		qMediaPlayer_Plugin *pqMediaPlayer_Plugin = new qMediaPlayer_Plugin(PK_Device, sRouter_IP,true,bLocalMode);
		if ( pqMediaPlayer_Plugin->GetConfig() && pqMediaPlayer_Plugin->Connect(pqMediaPlayer_Plugin->PK_DeviceTemplate_get()) ) 
		{
			g_pCommand_Impl=pqMediaPlayer_Plugin;
			g_pDeadlockHandler=DeadlockHandler;
			g_pSocketCrashHandler=SocketCrashHandler;
			LoggerWrapper::GetInstance()->Write(LV_STATUS, "Connect OK");
			pqMediaPlayer_Plugin->CreateChildren();
			if( bLocalMode )
				pqMediaPlayer_Plugin->RunLocalMode();
			else
			{
				if(pqMediaPlayer_Plugin->m_RequestHandlerThread)
					pthread_join(pqMediaPlayer_Plugin->m_RequestHandlerThread, NULL);  // This function will return when the device is shutting down
			}
		...
		{
			bAppError = true;
			if( pqMediaPlayer_Plugin->m_pEvent && pqMediaPlayer_Plugin->m_pEvent->m_pClientSocket && pqMediaPlayer_Plugin->m_pEvent->m_pClientSocket->m_eLastError==ClientSocket::cs_err_CannotConnect )
			{
				bAppError = false;
				bReload = false;
		...
		if( pqMediaPlayer_Plugin->m_bReload )
			bReload=true;
