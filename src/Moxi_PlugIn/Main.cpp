...
extern "C" {
	...
	{ 
		...
		if( bIsRuntimePlugin )
			return Moxi_PlugIn::PK_DeviceTemplate_get_static();
		else
			return 0;
	}
}
...
extern "C" {
	class Command_Impl *RegisterAsPlugIn(class Router *pRouter,int PK_Device,Logger *pPlutoLogger)
	{
		...
		Moxi_PlugIn *pMoxi_PlugIn = new Moxi_PlugIn(PK_Device, "localhost",true,false,pRouter);
		if( pMoxi_PlugIn->m_bQuit_get()|| !pMoxi_PlugIn->GetConfig() )
		{
			delete pMoxi_PlugIn;
			return NULL;
		}
...
int main(int argc, char* argv[]) 
{
	...
	cout << "Moxi_PlugIn, v." << VERSION << endl
	...
	if (bError)
	{
		cout << "A Pluto DCE Device.  See www.plutohome.com/dce for details." << endl
			<< "Usage: Moxi_PlugIn [-r Router's IP] [-d My Device ID] [-l dcerouter|stdout|null|filename]" << endl
	...
	try
	{
		if( sLogger=="dcerouter" )
			LoggerWrapper::SetInstance(new ServerLogger(PK_Device, Moxi_PlugIn::PK_DeviceTemplate_get_static(), sRouter_IP));
	...
	try
	{
		Moxi_PlugIn *pMoxi_PlugIn = new Moxi_PlugIn(PK_Device, sRouter_IP,true,bLocalMode);
		if ( pMoxi_PlugIn->GetConfig() && pMoxi_PlugIn->Connect(pMoxi_PlugIn->PK_DeviceTemplate_get()) ) 
		{
			g_pCommand_Impl=pMoxi_PlugIn;
			g_pDeadlockHandler=DeadlockHandler;
			g_pSocketCrashHandler=SocketCrashHandler;
			LoggerWrapper::GetInstance()->Write(LV_STATUS, "Connect OK");
			pMoxi_PlugIn->CreateChildren();
			if( bLocalMode )
				pMoxi_PlugIn->RunLocalMode();
			else
			{
				if(pMoxi_PlugIn->m_RequestHandlerThread)
					pthread_join(pMoxi_PlugIn->m_RequestHandlerThread, NULL);  // This function will return when the device is shutting down
			}
