...
extern "C" {
	class Command_Impl *RegisterAsPlugIn(class Router *pRouter,int PK_Device,Logger *pPlutoLogger)
	{
		...
		Hulu_Player *pHulu_Player = new Hulu_Player(PK_Device, "localhost",true,false,pRouter);
...
int main(int argc, char* argv[]) 
{
	...
	if (bError)
	{
		cout << "A Pluto DCE Device.  See www.plutohome.com/dce for details." << endl
			<< "Usage: Hulu_Player [-r Router's IP] [-d My Device ID] [-l dcerouter|stdout|null|filename]" << endl
	...
	try
	{
		if( sLogger=="dcerouter" )
			LoggerWrapper::SetInstance(new ServerLogger(PK_Device, Hulu_Player::PK_DeviceTemplate_get_static(), sRouter_IP));
	...
	try
	{
		Hulu_Player *pHulu_Player = new Hulu_Player(PK_Device, sRouter_IP,true,bLocalMode);
		if ( pHulu_Player->GetConfig() && pHulu_Player->Connect(pHulu_Player->PK_DeviceTemplate_get()) ) 
		{
			g_pCommand_Impl=pHulu_Player;
		...
		{
			bAppError = true;
			if( pHulu_Player->m_pEvent && pHulu_Player->m_pEvent->m_pClientSocket && pHulu_Player->m_pEvent->m_pClientSocket->m_eLastError==ClientSocket::cs_err_CannotConnect )
			{
				bAppError = false;
				bReload = false;
