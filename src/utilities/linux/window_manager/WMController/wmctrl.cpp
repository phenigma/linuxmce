...
void window_set_title (Display *disp, Window win, /* {{{ */
    ...
    if (mode == 'T' || mode == 'N') {
        ...
        if (title_local) {
            XChangeProperty(disp, win, XA_WM_NAME, XA_STRING, 8, PropModeReplace,
                    reinterpret_cast<const unsigned char*>(title_local), strlen(title_local));
        }
        else {
        ...
        XChangeProperty(disp, win, XInternAtom(disp, "_NET_WM_NAME", False),
                ...
                reinterpret_cast<const unsigned char*>(title_utf8), strlen(title_utf8));
    }
...
gboolean wm_supports (Display *disp, const gchar *prop) {/*{{{*/
    ...
    for (i = 0; i < int(size / sizeof(Atom)); i++) {
        if (list[i] == xa_prop) {
            g_free(list);
            return TRUE;
        }
...
int action_window (Display *disp, Window win, char mode, char *param) {/*{{{*/
    ...
    switch (mode) {
        ...
        case 'e':
            ...
            return window_move_resize(disp, win, param);

        case 'b':
            /* change state of a window => -r -b */
            return window_state(disp, win, param);

        case 't':
            /* move the window to the specified desktop => -r -t */
            return window_to_desktop(disp, win, atoi(param));
        ...
        case 'N': case 'I': case 'T':
            window_set_title(disp, win, param, mode);
            return EXIT_SUCCESS;
...
int action_window_pid (Display *disp, char mode, char *param_window, char *param) {/*{{{*/
    ...
    if (sscanf(param_window, "0x%lx", &wid) != 1 &&
            sscanf(param_window, "0X%lx", &wid) != 1 &&
            sscanf(param_window, "%lu", &wid) != 1) {
        fputs("Cannot convert argument to number.\n", stderr);
        return EXIT_FAILURE;
    }

    return action_window(disp, (Window)wid, mode, param);
}/*}}}*/
...
int action_window_str (Display *disp, char mode, char *param_window, char *param, bool show_class, bool full_window_title_match) {/*{{{*/
    ...
    if (strcmp(SELECT_WINDOW_MAGIC, param_window) == 0) {
        activate = Select_Window(disp);
        if (activate) {
            return action_window(disp, activate, mode, param);
        }
        else {
            return EXIT_FAILURE;
        }
    }
    if (strcmp(ACTIVE_WINDOW_MAGIC, param_window) == 0) {
        ...
        for (i = 0; i < int(client_list_size / sizeof(Window)); i++) {
 			gchar *match_utf8;
 			if (show_class) {
            ...
            if (match_utf8) {
                ...
                if (envir_utf8) {
                    match = g_strdup(param_window);
                    match_cf = g_utf8_casefold(param_window, -1);
                }
                else {
                    if (! (match = g_locale_to_utf8(param_window, -1, NULL, NULL, NULL))) {
                        match = g_strdup(param_window);
                    }
                ...
                if ((full_window_title_match && strcmp(match_utf8, match) == 0) ||
                        (!full_window_title_match && strstr(match_utf8_cf, match_cf))) {
...
int list_desktops (Display *disp, bool wa_desktop_titles_invalid_utf8) {/*{{{*/
    ...
    if (wa_desktop_titles_invalid_utf8 ||
    ...
    if (list) {
        id = 0;
        names[id++] = list;
        for (i = 0; i < int(desktop_list_size); i++) {
            if (list[i] == '\0') {
                if (id >= int(*num_desktops)) {
                    break;
                }
                names[id++] = list + i + 1;
            }
    ...
    if (desktop_geometry && desktop_geometry_size > 0) {
        if (desktop_geometry_size == 2 * sizeof(*desktop_geometry)) {
            ...
            for (i = 0; i < int(*num_desktops); i++) {
                if (i < int(desktop_geometry_size / sizeof(*desktop_geometry) / 2)) {
                    desktop_geometry_str[i] = g_strdup_printf("%lux%lu",
    ...
    if (desktop_viewport && desktop_viewport_size > 0) {
        if (desktop_viewport_size == 2 * sizeof(*desktop_viewport)) {
            ...
            for (i = 0; i < int(*num_desktops); i++) {
                if (i < int(desktop_viewport_size / sizeof(*desktop_viewport) / 2)) {
                    desktop_viewport_str[i] = g_strdup_printf("%lu,%lu",
    ...
    if (desktop_workarea && desktop_workarea_size > 0) {
        if (desktop_workarea_size == 4 * sizeof(*desktop_workarea)) {
            ...
            /* seperate values for each of desktops */
            for (i = 0; i < int(*num_desktops); i++) {
                if (i < int(desktop_workarea_size / sizeof(*desktop_workarea) / 4)) {
                    desktop_workarea_str[i] = g_strdup_printf("%lu,%lu %lux%lu",
...
int list_windows (Display *disp, bool show_pid, bool show_geometry, bool show_class, list<WinInfo> &listWinInfo) {/*{{{*/
    ...
    for (i = 0; i < int(client_list_size / sizeof(Window)); i++) {
        ...
        listWinInfo.push_back(itemWinInfo);
    }
    g_free(client_list);
