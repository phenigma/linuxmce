...
extern "C" {
	...
	{ 
		...
		if( bIsRuntimePlugin )
			return Wii_Remote_Controller::PK_DeviceTemplate_get_static();
		else
			return 0;
	}
}
...
extern "C" {
	class Command_Impl *RegisterAsPlugIn(class Router *pRouter,int PK_Device,Logger *pPlutoLogger)
	{
		...
		Wii_Remote_Controller *pWii_Remote_Controller = new Wii_Remote_Controller(PK_Device, "localhost",true,false,pRouter);
		if( pWii_Remote_Controller->m_bQuit_get()|| !pWii_Remote_Controller->GetConfig() )
		{
			delete pWii_Remote_Controller;
			return NULL;
		}
		else
		{
			g_pCommand_Impl=pWii_Remote_Controller;
			g_pDeadlockHandler=Plugin_DeadlockHandler;
			g_pSocketCrashHandler=Plugin_SocketCrashHandler;
		}
		return pWii_Remote_Controller;
	}
}
//<-dceag-plug-e->
...
int main(int argc, char* argv[]) 
{
	...
	cout << "Wii_Remote_Controller, v." << VERSION << endl
	...
	if (bError)
	{
		cout << "A Pluto DCE Device.  See www.plutohome.com/dce for details." << endl
			<< "Usage: Wii_Remote_Controller [-r Router's IP] [-d My Device ID] [-l dcerouter|stdout|null|filename]" << endl
	...
	try
	{
		if( sLogger=="dcerouter" )
			LoggerWrapper::SetInstance(new ServerLogger(PK_Device, Wii_Remote_Controller::PK_DeviceTemplate_get_static(), sRouter_IP));
	...
	try
	{
		Wii_Remote_Controller *pWii_Remote_Controller = new Wii_Remote_Controller(PK_Device, sRouter_IP,true,bLocalMode);
		if ( pWii_Remote_Controller->GetConfig() && pWii_Remote_Controller->Connect(pWii_Remote_Controller->PK_DeviceTemplate_get()) ) 
		{
			g_pCommand_Impl=pWii_Remote_Controller;
			g_pDeadlockHandler=DeadlockHandler;
			g_pSocketCrashHandler=SocketCrashHandler;
			LoggerWrapper::GetInstance()->Write(LV_STATUS, "Connect OK");
			pWii_Remote_Controller->CreateChildren();
			if( bLocalMode )
				pWii_Remote_Controller->RunLocalMode();
			else
			{
				if(pWii_Remote_Controller->m_RequestHandlerThread)
					pthread_join(pWii_Remote_Controller->m_RequestHandlerThread, NULL);  // This function will return when the device is shutting down
			}
		...
		{
			bAppError = true;
			if( pWii_Remote_Controller->m_pEvent && pWii_Remote_Controller->m_pEvent->m_pClientSocket && pWii_Remote_Controller->m_pEvent->m_pClientSocket->m_eLastError==ClientSocket::cs_err_CannotConnect )
			{
				bAppError = false;
				bReload = false;
		...
		if( pWii_Remote_Controller->m_bReload )
			bReload=true;
