...
extern "C" {
	...
	{ 
		...
		if( bIsRuntimePlugin )
			return Windows_Share_Scanner::PK_DeviceTemplate_get_static();
		else
			return 0;
	}
}
...
int main(int argc, char* argv[]) 
{
	...
	cout << "Windows_Share_Scanner, v." << VERSION << endl
	...
	if (bError)
	{
		cout << "A Pluto DCE Device.  See www.plutohome.com/dce for details." << endl
			<< "Usage: Windows_Share_Scanner [-r Router's IP] [-d My Device ID] [-l dcerouter|stdout|null|filename]" << endl
	...
	try
	{
		if( sLogger=="dcerouter" )
			LoggerWrapper::SetInstance(new ServerLogger(PK_Device, Windows_Share_Scanner::PK_DeviceTemplate_get_static(), sRouter_IP));
	...
	try
	{
		Windows_Share_Scanner *pWindows_Share_Scanner = new Windows_Share_Scanner(PK_Device, sRouter_IP,true,bLocalMode);
		if ( pWindows_Share_Scanner->GetConfig() && pWindows_Share_Scanner->Connect(pWindows_Share_Scanner->PK_DeviceTemplate_get()) ) 
		{
			g_pCommand_Impl=pWindows_Share_Scanner;
			g_pDeadlockHandler=DeadlockHandler;
			g_pSocketCrashHandler=SocketCrashHandler;
			LoggerWrapper::GetInstance()->Write(LV_STATUS, "Connect OK");
			pWindows_Share_Scanner->CreateChildren();
			if( bLocalMode )
