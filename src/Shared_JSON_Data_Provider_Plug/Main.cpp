...
extern "C" {
	...
	{ 
		...
		if( bIsRuntimePlugin )
			return Shared_JSON_Data_Provider_Plug::PK_DeviceTemplate_get_static();
		else
			return 0;
	}
}
...
extern "C" {
	class Command_Impl *RegisterAsPlugIn(class Router *pRouter,int PK_Device,Logger *pPlutoLogger)
	{
		...
		Shared_JSON_Data_Provider_Plug *pShared_JSON_Data_Provider_Plug = new Shared_JSON_Data_Provider_Plug(PK_Device, "localhost",true,false,pRouter);
		if( pShared_JSON_Data_Provider_Plug->m_bQuit_get()|| !pShared_JSON_Data_Provider_Plug->GetConfig() )
		{
			delete pShared_JSON_Data_Provider_Plug;
			return NULL;
		}
		else
		{
			g_pCommand_Impl=pShared_JSON_Data_Provider_Plug;
			g_pDeadlockHandler=Plugin_DeadlockHandler;
			g_pSocketCrashHandler=Plugin_SocketCrashHandler;
		}
		return pShared_JSON_Data_Provider_Plug;
	}
}
//<-dceag-plug-e->
...
int main(int argc, char* argv[]) 
{
	...
	cout << "Shared_JSON_Data_Provider_Plug, v." << VERSION << endl
	...
	if (bError)
	{
		cout << "A Pluto DCE Device.  See www.plutohome.com/dce for details." << endl
			<< "Usage: Shared_JSON_Data_Provider_Plug [-r Router's IP] [-d My Device ID] [-l dcerouter|stdout|null|filename]" << endl
	...
	try
	{
		if( sLogger=="dcerouter" )
			LoggerWrapper::SetInstance(new ServerLogger(PK_Device, Shared_JSON_Data_Provider_Plug::PK_DeviceTemplate_get_static(), sRouter_IP));
	...
	try
	{
		Shared_JSON_Data_Provider_Plug *pShared_JSON_Data_Provider_Plug = new Shared_JSON_Data_Provider_Plug(PK_Device, sRouter_IP,true,bLocalMode);
		if ( pShared_JSON_Data_Provider_Plug->GetConfig() && pShared_JSON_Data_Provider_Plug->Connect(pShared_JSON_Data_Provider_Plug->PK_DeviceTemplate_get()) ) 
		{
			g_pCommand_Impl=pShared_JSON_Data_Provider_Plug;
			g_pDeadlockHandler=DeadlockHandler;
			g_pSocketCrashHandler=SocketCrashHandler;
			LoggerWrapper::GetInstance()->Write(LV_STATUS, "Connect OK");
			pShared_JSON_Data_Provider_Plug->CreateChildren();
			if( bLocalMode )
				pShared_JSON_Data_Provider_Plug->RunLocalMode();
			else
			{
				if(pShared_JSON_Data_Provider_Plug->m_RequestHandlerThread)
					pthread_join(pShared_JSON_Data_Provider_Plug->m_RequestHandlerThread, NULL);  // This function will return when the device is shutting down
			}
		...
		{
			bAppError = true;
			if( pShared_JSON_Data_Provider_Plug->m_pEvent && pShared_JSON_Data_Provider_Plug->m_pEvent->m_pClientSocket && pShared_JSON_Data_Provider_Plug->m_pEvent->m_pClientSocket->m_eLastError==ClientSocket::cs_err_CannotConnect )
			{
				bAppError = false;
				bReload = false;
		...
		if( pShared_JSON_Data_Provider_Plug->m_bReload )
			bReload=true;
