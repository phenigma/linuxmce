...
extern "C" {
	...
	{ 
		...
		if( bIsRuntimePlugin )
			return Powerfile_C200::PK_DeviceTemplate_get_static();
		else
			return 0;
	}
}
...
int main(int argc, char* argv[]) 
{
	...
	cout << "Powerfile_C200, v." << VERSION << endl
	...
	if (bError)
	{
		cout << "A Pluto DCE Device.  See www.plutohome.com/dce for details." << endl
			<< "Usage: Powerfile_C200 [-r Router's IP] [-d My Device ID] [-l dcerouter|stdout|null|filename]" << endl
	...
	try
	{
		if( sLogger=="dcerouter" )
			LoggerWrapper::SetInstance(new ServerLogger(PK_Device, Powerfile_C200::PK_DeviceTemplate_get_static(), sRouter_IP));
	...
	try
	{
		Powerfile_C200 *pPowerfile_C200 = new Powerfile_C200(PK_Device, sRouter_IP,true,bLocalMode);
		if ( pPowerfile_C200->GetConfig() && pPowerfile_C200->Connect(pPowerfile_C200->PK_DeviceTemplate_get()) ) 
		{
			g_pCommand_Impl=pPowerfile_C200;
			g_pDeadlockHandler=DeadlockHandler;
			g_pSocketCrashHandler=SocketCrashHandler;
			LoggerWrapper::GetInstance()->Write(LV_STATUS, "Connect OK");
			pPowerfile_C200->CreateChildren();
			if( bLocalMode )
		...
		{
			bAppError = true;
			if( pPowerfile_C200->m_pEvent && pPowerfile_C200->m_pEvent->m_pClientSocket && pPowerfile_C200->m_pEvent->m_pClientSocket->m_eLastError==ClientSocket::cs_err_CannotConnect )
			{
				bAppError = false;
				bReload = false;
