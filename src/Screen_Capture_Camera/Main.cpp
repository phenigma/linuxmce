...
extern "C" {
	...
	{ 
		...
		if( bIsRuntimePlugin )
			return Screen_Capture_Camera::PK_DeviceTemplate_get_static();
		else
			return 0;
	}
}
...
extern "C" {
	class Command_Impl *RegisterAsPlugIn(class Router *pRouter,int PK_Device,Logger *pPlutoLogger)
	{
		...
		Screen_Capture_Camera *pScreen_Capture_Camera = new Screen_Capture_Camera(PK_Device, "localhost",true,false,pRouter);
		if( pScreen_Capture_Camera->m_bQuit_get()|| !pScreen_Capture_Camera->GetConfig() )
		{
			delete pScreen_Capture_Camera;
			return NULL;
		}
		else
		{
			g_pCommand_Impl=pScreen_Capture_Camera;
			g_pDeadlockHandler=Plugin_DeadlockHandler;
			g_pSocketCrashHandler=Plugin_SocketCrashHandler;
		}
		return pScreen_Capture_Camera;
	}
}
//<-dceag-plug-e->
...
int main(int argc, char* argv[]) 
{
	...
	cout << "Screen_Capture_Camera, v." << VERSION << endl
	...
	if (bError)
	{
		cout << "A Pluto DCE Device.  See www.plutohome.com/dce for details." << endl
			<< "Usage: Screen_Capture_Camera [-r Router's IP] [-d My Device ID] [-l dcerouter|stdout|null|filename]" << endl
	...
	try
	{
		if( sLogger=="dcerouter" )
			LoggerWrapper::SetInstance(new ServerLogger(PK_Device, Screen_Capture_Camera::PK_DeviceTemplate_get_static(), sRouter_IP));
	...
	try
	{
		Screen_Capture_Camera *pScreen_Capture_Camera = new Screen_Capture_Camera(PK_Device, sRouter_IP,true,bLocalMode);
		if ( pScreen_Capture_Camera->GetConfig() && pScreen_Capture_Camera->Connect(pScreen_Capture_Camera->PK_DeviceTemplate_get()) ) 
		{
			g_pCommand_Impl=pScreen_Capture_Camera;
			g_pDeadlockHandler=DeadlockHandler;
			g_pSocketCrashHandler=SocketCrashHandler;
			LoggerWrapper::GetInstance()->Write(LV_STATUS, "Connect OK");
			pScreen_Capture_Camera->CreateChildren();
			if( bLocalMode )
				pScreen_Capture_Camera->RunLocalMode();
			else
			{
				if(pScreen_Capture_Camera->m_RequestHandlerThread)
					pthread_join(pScreen_Capture_Camera->m_RequestHandlerThread, NULL);  // This function will return when the device is shutting down
			}
		...
		{
			bAppError = true;
			if( pScreen_Capture_Camera->m_pEvent && pScreen_Capture_Camera->m_pEvent->m_pClientSocket && pScreen_Capture_Camera->m_pEvent->m_pClientSocket->m_eLastError==ClientSocket::cs_err_CannotConnect )
			{
				bAppError = false;
				bReload = false;
		...
		if( pScreen_Capture_Camera->m_bReload )
			bReload=true;
