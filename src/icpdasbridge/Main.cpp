...
extern "C" {
	...
	{ 
		...
		if( bIsRuntimePlugin )
			return icpdasbridge::PK_DeviceTemplate_get_static();
		else
			return 0;
	}
}
...
extern "C" {
	class Command_Impl *RegisterAsPlugIn(class Router *pRouter,int PK_Device,Logger *pPlutoLogger)
	{
		...
		icpdasbridge *picpdasbridge = new icpdasbridge(PK_Device, "localhost",true,false,pRouter);
		if( picpdasbridge->m_bQuit_get()|| !picpdasbridge->GetConfig() )
		{
			delete picpdasbridge;
			return NULL;
		}
		else
		{
			g_pCommand_Impl=picpdasbridge;
			g_pDeadlockHandler=Plugin_DeadlockHandler;
			g_pSocketCrashHandler=Plugin_SocketCrashHandler;
		}
		return picpdasbridge;
	}
}
//<-dceag-plug-e->
...
int main(int argc, char* argv[]) 
{
	...
	cout << "icpdasbridge, v." << VERSION << endl
	...
	if (bError)
	{
		cout << "A Pluto DCE Device.  See www.plutohome.com/dce for details." << endl
			<< "Usage: icpdasbridge [-r Router's IP] [-d My Device ID] [-l dcerouter|stdout|null|filename]" << endl
	...
	try
	{
		if( sLogger=="dcerouter" )
			LoggerWrapper::SetInstance(new ServerLogger(PK_Device, icpdasbridge::PK_DeviceTemplate_get_static(), sRouter_IP));
	...
	try
	{
		icpdasbridge *picpdasbridge = new icpdasbridge(PK_Device, sRouter_IP,true,bLocalMode);
		if ( picpdasbridge->GetConfig() && picpdasbridge->Connect(picpdasbridge->PK_DeviceTemplate_get()) ) 
		{
			g_pCommand_Impl=picpdasbridge;
			g_pDeadlockHandler=DeadlockHandler;
			g_pSocketCrashHandler=SocketCrashHandler;
			LoggerWrapper::GetInstance()->Write(LV_STATUS, "Connect OK");
			picpdasbridge->CreateChildren();
			if( bLocalMode )
				picpdasbridge->RunLocalMode();
			else
			{
				if(picpdasbridge->m_RequestHandlerThread)
					pthread_join(picpdasbridge->m_RequestHandlerThread, NULL);  // This function will return when the device is shutting down
			}
		...
		{
			bAppError = true;
			if( picpdasbridge->m_pEvent && picpdasbridge->m_pEvent->m_pClientSocket && picpdasbridge->m_pEvent->m_pClientSocket->m_eLastError==ClientSocket::cs_err_CannotConnect )
			{
				bAppError = false;
				bReload = false;
