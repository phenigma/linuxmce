...
extern "C" {
	...
	{ 
		...
		if( bIsRuntimePlugin )
			return Generic_VFDLCD::PK_DeviceTemplate_get_static();
		else
			return 0;
	}
}
...
extern "C" {
	class Command_Impl *RegisterAsPlugIn(class Router *pRouter,int PK_Device,Logger *pPlutoLogger)
	{
		...
		Generic_VFDLCD *pGeneric_VFDLCD = new Generic_VFDLCD(PK_Device, "localhost",true,false,pRouter);
		if( pGeneric_VFDLCD->m_bQuit_get()|| !pGeneric_VFDLCD->GetConfig() )
		{
			delete pGeneric_VFDLCD;
			return NULL;
		}
		else
		{
...
int main(int argc, char* argv[]) 
{
	...
	cout << "Generic_VFDLCD, v." << VERSION << endl
	...
	try
	{
		if( sLogger=="dcerouter" )
			LoggerWrapper::SetInstance(new ServerLogger(PK_Device, Generic_VFDLCD::PK_DeviceTemplate_get_static(), sRouter_IP));
