...
extern "C" {
	...
	{ 
		...
		if( bIsRuntimePlugin )
			return Mac_Workstation::PK_DeviceTemplate_get_static();
		else
			return 0;
	}
}
...
extern "C" {
	class Command_Impl *RegisterAsPlugIn(class Router *pRouter,int PK_Device,Logger *pPlutoLogger)
	{
		...
		Mac_Workstation *pMac_Workstation = new Mac_Workstation(PK_Device, "localhost",true,false,pRouter);
		if( pMac_Workstation->m_bQuit_get()|| !pMac_Workstation->GetConfig() )
		{
			delete pMac_Workstation;
			return NULL;
		}
		else
		{
			g_pCommand_Impl=pMac_Workstation;
			g_pDeadlockHandler=Plugin_DeadlockHandler;
			g_pSocketCrashHandler=Plugin_SocketCrashHandler;
		}
		return pMac_Workstation;
	}
}
//<-dceag-plug-e->
...
int main(int argc, char* argv[]) 
{
	...
	cout << "Mac_Workstation, v." << VERSION << endl
	...
	if (bError)
	{
		cout << "A Pluto DCE Device.  See www.plutohome.com/dce for details." << endl
			<< "Usage: Mac_Workstation [-r Router's IP] [-d My Device ID] [-l dcerouter|stdout|null|filename]" << endl
	...
	try
	{
		if( sLogger=="dcerouter" )
			LoggerWrapper::SetInstance(new ServerLogger(PK_Device, Mac_Workstation::PK_DeviceTemplate_get_static(), sRouter_IP));
	...
	try
	{
		Mac_Workstation *pMac_Workstation = new Mac_Workstation(PK_Device, sRouter_IP,true,bLocalMode);
		if ( pMac_Workstation->GetConfig() && pMac_Workstation->Connect(pMac_Workstation->PK_DeviceTemplate_get()) ) 
		{
			g_pCommand_Impl=pMac_Workstation;
			g_pDeadlockHandler=DeadlockHandler;
			g_pSocketCrashHandler=SocketCrashHandler;
			LoggerWrapper::GetInstance()->Write(LV_STATUS, "Connect OK");
			pMac_Workstation->CreateChildren();
			if( bLocalMode )
				pMac_Workstation->RunLocalMode();
			else
			{
				if(pMac_Workstation->m_RequestHandlerThread)
					pthread_join(pMac_Workstation->m_RequestHandlerThread, NULL);  // This function will return when the device is shutting down
			}
		...
		{
			bAppError = true;
			if( pMac_Workstation->m_pEvent && pMac_Workstation->m_pEvent->m_pClientSocket && pMac_Workstation->m_pEvent->m_pClientSocket->m_eLastError==ClientSocket::cs_err_CannotConnect )
			{
				bAppError = false;
				bReload = false;
