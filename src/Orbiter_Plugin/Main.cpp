...
extern "C" {
	...
	{ 
		...
		if( bIsRuntimePlugin )
			return Orbiter_Plugin::PK_DeviceTemplate_get_static();
		else
			return 0;
	}
}
...
extern "C" {
	class Command_Impl *RegisterAsPlugIn(class Router *pRouter,int PK_Device,Logger *pPlutoLogger)
	{
		...
		if( pOrbiter_Plugin->m_bQuit_get()|| !pOrbiter_Plugin->GetConfig() )
		{
...
int main(int argc, char* argv[]) 
{
	...
	if (bError)
	{
		cout << "A Pluto DCE Device.  See www.plutohome.com/dce for details." << endl
			<< "Usage: Orbiter_Plugin [-r Router's IP] [-d My Device ID] [-l dcerouter|stdout|null|filename]" << endl
	...
	try
	{
		if( sLogger=="dcerouter" )
			LoggerWrapper::SetInstance(new ServerLogger(PK_Device, Orbiter_Plugin::PK_DeviceTemplate_get_static(), sRouter_IP));
	...
	try
	{
		Orbiter_Plugin *pOrbiter_Plugin = new Orbiter_Plugin(PK_Device, sRouter_IP,true,bLocalMode);
		if ( pOrbiter_Plugin->GetConfig() && pOrbiter_Plugin->Connect(pOrbiter_Plugin->PK_DeviceTemplate_get()) ) 
		{
			...
			pOrbiter_Plugin->CreateChildren();
			if( bLocalMode )
				pOrbiter_Plugin->RunLocalMode();
			else
			{
				if(pOrbiter_Plugin->m_RequestHandlerThread)
					pthread_join(pOrbiter_Plugin->m_RequestHandlerThread, NULL);  // This function will return when the device is shutting down
			}
		...
		{
			bAppError = true;
			if( pOrbiter_Plugin->m_pEvent && pOrbiter_Plugin->m_pEvent->m_pClientSocket && pOrbiter_Plugin->m_pEvent->m_pClientSocket->m_eLastError==ClientSocket::cs_err_CannotConnect )
			{
				bAppError = false;
				bReload = false;
