...
extern "C" {
	...
	{ 
		...
		if( bIsRuntimePlugin )
			return VistaICM2::PK_DeviceTemplate_get_static();
		else
			return 0;
	}
}
...
extern "C" {
	class Command_Impl *RegisterAsPlugIn(class Router *pRouter,int PK_Device,Logger *pPlutoLogger)
	{
		...
		VistaICM2 *pVistaICM2 = new VistaICM2(PK_Device, "localhost",true,false,pRouter);
		if( pVistaICM2->m_bQuit_get()|| !pVistaICM2->GetConfig() )
		{
			delete pVistaICM2;
			return NULL;
		}
...
int main(int argc, char* argv[]) 
{
	...
	if (bError)
	{
		cout << "A Pluto DCE Device.  See www.plutohome.com/dce for details." << endl
			<< "Usage: VistaICM2 [-r Router's IP] [-d My Device ID] [-l dcerouter|stdout|null|filename]" << endl
	...
	try
	{
		if( sLogger=="dcerouter" )
			LoggerWrapper::SetInstance(new ServerLogger(PK_Device, VistaICM2::PK_DeviceTemplate_get_static(), sRouter_IP));
	...
	try
	{
		VistaICM2 *pVistaICM2 = new VistaICM2(PK_Device, sRouter_IP,true,bLocalMode);
		if ( pVistaICM2->GetConfig() && pVistaICM2->Connect(pVistaICM2->PK_DeviceTemplate_get()) ) 
		{
			g_pCommand_Impl=pVistaICM2;
			g_pDeadlockHandler=DeadlockHandler;
			g_pSocketCrashHandler=SocketCrashHandler;
			LoggerWrapper::GetInstance()->Write(LV_STATUS, "Connect OK");
			pVistaICM2->CreateChildren();
			if( bLocalMode )
				pVistaICM2->RunLocalMode();
			else
			{
				if(pVistaICM2->m_RequestHandlerThread)
					pthread_join(pVistaICM2->m_RequestHandlerThread, NULL);  // This function will return when the device is shutting down
			}
		...
		{
			bAppError = true;
			if( pVistaICM2->m_pEvent && pVistaICM2->m_pEvent->m_pClientSocket && pVistaICM2->m_pEvent->m_pClientSocket->m_eLastError==ClientSocket::cs_err_CannotConnect )
			{
				bAppError = false;
				bReload = false;
