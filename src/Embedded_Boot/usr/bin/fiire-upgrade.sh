...
echo " `date` ==fu== Free disk space available = `df -h | grep "tmpfs" | tr -s ' ' | cut -d ' ' -f 4` " >> $upgrade_log
echo " `date` ==fu== Free memory available = `cat /proc/meminfo | grep "MemFree" | tr -s ' ' | cut -d " " -f 2` " >> $upgrade_log
