...
int main(int argc, char* argv[]) 
{
	...
	try
	{
		if( sLogger=="dcerouter" )
			LoggerWrapper::SetInstance(new ServerLogger(PK_Device, Datagrid_Plugin::PK_DeviceTemplate_get_static(), sRouter_IP));
	...
	try
	{
		...
		if ( pDatagrid_Plugin->GetConfig() && pDatagrid_Plugin->Connect(pDatagrid_Plugin->PK_DeviceTemplate_get()) ) 
		{
			...
			{
				if(pDatagrid_Plugin->m_RequestHandlerThread)
		...
		{
			bAppError = true;
			if( pDatagrid_Plugin->m_pEvent && pDatagrid_Plugin->m_pEvent->m_pClientSocket && pDatagrid_Plugin->m_pEvent->m_pClientSocket->m_eLastError==ClientSocket::cs_err_CannotConnect )
			{
				bAppError = false;
				bReload = false;
