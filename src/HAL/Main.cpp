...
extern "C" {
	...
	{ 
		...
		if( bIsRuntimePlugin )
			return HAL::PK_DeviceTemplate_get_static();
		else
			return 0;
	}
}
...
int main(int argc, char* argv[]) 
{
	...
	if (bError)
	{
		cout << "A Pluto DCE Device.  See www.plutohome.com/dce for details." << endl
			<< "Usage: HAL [-r Router's IP] [-d My Device ID] [-l dcerouter|stdout|null|filename]" << endl
	...
	try
	{
		if( sLogger=="dcerouter" )
			LoggerWrapper::SetInstance(new ServerLogger(PK_Device, HAL::PK_DeviceTemplate_get_static(), sRouter_IP));
	...
	try
	{
		HAL *pHAL = new HAL(PK_Device, sRouter_IP,true,bLocalMode);
		if ( pHAL->GetConfig() && pHAL->Connect(pHAL->PK_DeviceTemplate_get()) ) 
		{
			g_pCommand_Impl=pHAL;
			...
			{
				if(pHAL->m_RequestHandlerThread)
					pthread_join(pHAL->m_RequestHandlerThread, NULL);  // This function will return when the device is shutting down
			}
		...
		{
			bAppError = true;
			if( pHAL->m_pEvent && pHAL->m_pEvent->m_pClientSocket && pHAL->m_pEvent->m_pClientSocket->m_eLastError==ClientSocket::cs_err_CannotConnect )
			{
				bAppError = false;
				bReload = false;
