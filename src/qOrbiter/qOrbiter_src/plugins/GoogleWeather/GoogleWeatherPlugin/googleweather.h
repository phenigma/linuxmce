#ifndef GOOGLEWEATHER_H
#define GOOGLEWEATHER_H
...
class GoogleWeather : public QAbstractListModel
{
...
public:
    ...
    GoogleWeather() {}
    explicit GoogleWeather(gWeatherItem* prototype, QObject *parent = 0);
    ...
    void setPostalCode(QString p) {postalCode = p; getWeather(postalCode); emit postalCodeChanged();}
...
public slots:
    void getWeather(const QString& n);
    void recieveReply(QNetworkReply* );
    void exitApp() {emit close();}
    void setLocation(QString l) {location = l; getWeather(location); emit locationChanged();}
...
QML_DECLARE_TYPE(GoogleWeather)

#endif // GOOGLEWEATHER_H
