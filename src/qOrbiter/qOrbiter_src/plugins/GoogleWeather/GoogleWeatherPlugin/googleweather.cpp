...
QVariant GoogleWeather::data(const QModelIndex &index, int role) const
{
...
QVariant GoogleWeather::headerData(int section, Qt::Orientation orientation, int role) const
{
}

int GoogleWeather::rowCount(const QModelIndex &parent) const
{
    return m_list.count();
}

void GoogleWeather::getWeather(const QString &n)
{
    if (!m_list.isEmpty())
        m_list.clear();

    location = n;
    QString url = "http://www.google.com/ig/api?weather=";

    dataManager = new QNetworkAccessManager();
    QNetworkRequest weatherRequest(QUrl(url.append(location)));
...
void GoogleWeather::recieveReply(QNetworkReply* )
{
...
void GoogleWeather::processCurrent()
{
...
void GoogleWeather::processModelItems()
{
...
bool GoogleWeather::appendRow(gWeatherItem *item)
{
...
void GoogleWeather::sortModel(int column, Qt::SortOrder order)
{
}
