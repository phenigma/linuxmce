...
namespace Tribune
{
	...
	public:
		...
		string m_sSqlCVSHost;	/**< The host name or IP for the Tribune listener */
		...
		int m_iSqlCVSPort;	/**< The port of the Tribune */
		...
		GlobalConfig()
		{
			...
			m_sDBName=dceConfig.ReadString("Tribune_MySqlDatabase","pluto_main");
			m_iDBPort=dceConfig.ReadInteger("Tribune_MySqlPort",3306);
			m_iSqlCVSPort=dceConfig.ReadInteger("Tribune_Port",3485);
			m_sSqlCVSHost=dceConfig.ReadString("Tribune_Host","localhost");
			m_sUsers=dceConfig.ReadString("Tribune_Users","");
			m_pDatabase=NULL;
			m_pRepository=NULL;
