...
extern "C" {
	...
	{ 
		...
		if( bIsRuntimePlugin )
			return AirPlay_Audio_Player::PK_DeviceTemplate_get_static();
		else
			return 0;
	}
}
...
extern "C" {
	class Command_Impl *RegisterAsPlugIn(class Router *pRouter,int PK_Device,Logger *pPlutoLogger)
	{
		...
		AirPlay_Audio_Player *pAirPlay_Audio_Player = new AirPlay_Audio_Player(PK_Device, "localhost",true,false,pRouter);
		if( pAirPlay_Audio_Player->m_bQuit_get()|| !pAirPlay_Audio_Player->GetConfig() )
		{
			delete pAirPlay_Audio_Player;
			return NULL;
		}
		else
		{
			g_pCommand_Impl=pAirPlay_Audio_Player;
			g_pDeadlockHandler=Plugin_DeadlockHandler;
			g_pSocketCrashHandler=Plugin_SocketCrashHandler;
		}
		return pAirPlay_Audio_Player;
	}
}
//<-dceag-plug-e->
...
int main(int argc, char* argv[]) 
{
	...
	cout << "AirPlay_Audio_Player, v." << VERSION << endl
	...
	if (bError)
	{
		cout << "A Pluto DCE Device.  See www.plutohome.com/dce for details." << endl
			<< "Usage: AirPlay_Audio_Player [-r Router's IP] [-d My Device ID] [-l dcerouter|stdout|null|filename]" << endl
	...
	try
	{
		if( sLogger=="dcerouter" )
			LoggerWrapper::SetInstance(new ServerLogger(PK_Device, AirPlay_Audio_Player::PK_DeviceTemplate_get_static(), sRouter_IP));
	...
	try
	{
		AirPlay_Audio_Player *pAirPlay_Audio_Player = new AirPlay_Audio_Player(PK_Device, sRouter_IP,true,bLocalMode);
		if ( pAirPlay_Audio_Player->GetConfig() && pAirPlay_Audio_Player->Connect(pAirPlay_Audio_Player->PK_DeviceTemplate_get()) ) 
		{
			g_pCommand_Impl=pAirPlay_Audio_Player;
			g_pDeadlockHandler=DeadlockHandler;
			g_pSocketCrashHandler=SocketCrashHandler;
			LoggerWrapper::GetInstance()->Write(LV_STATUS, "Connect OK");
			pAirPlay_Audio_Player->CreateChildren();
			if( bLocalMode )
				pAirPlay_Audio_Player->RunLocalMode();
			else
			{
				if(pAirPlay_Audio_Player->m_RequestHandlerThread)
					pthread_join(pAirPlay_Audio_Player->m_RequestHandlerThread, NULL);  // This function will return when the device is shutting down
			}
		...
		{
			bAppError = true;
			if( pAirPlay_Audio_Player->m_pEvent && pAirPlay_Audio_Player->m_pEvent->m_pClientSocket && pAirPlay_Audio_Player->m_pEvent->m_pClientSocket->m_eLastError==ClientSocket::cs_err_CannotConnect )
			{
				bAppError = false;
				bReload = false;
		...
		if( pAirPlay_Audio_Player->m_bReload )
			bReload=true;
