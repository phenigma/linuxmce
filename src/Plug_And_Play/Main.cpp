...
extern "C" {
	...
	{ 
		...
		if( bIsRuntimePlugin )
			return Plug_And_Play::PK_DeviceTemplate_get_static();
		else
			return 0;
	}
}
...
extern "C" {
	class Command_Impl *RegisterAsPlugIn(class Router *pRouter,int PK_Device,Logger *pPlutoLogger)
	{
		...
		Plug_And_Play *pPlug_And_Play = new Plug_And_Play(PK_Device, "localhost",true,false,pRouter);
		if( pPlug_And_Play->m_bQuit || !pPlug_And_Play->GetConfig() )
		{
			delete pPlug_And_Play;
			return NULL;
		}
		else
		{
			g_pCommand_Impl=pPlug_And_Play;
			g_pDeadlockHandler=Plugin_DeadlockHandler;
			g_pSocketCrashHandler=Plugin_SocketCrashHandler;
		}
		return pPlug_And_Play;
	}
}
//<-dceag-plug-e->
...
int main(int argc, char* argv[]) 
{
	...
	cout << "Plug_And_Play, v." << VERSION << endl
	...
	if (bError)
	{
		cout << "A Pluto DCE Device.  See www.plutohome.com/dce for details." << endl
			<< "Usage: Plug_And_Play [-r Router's IP] [-d My Device ID] [-l dcerouter|stdout|null|filename]" << endl
	...
	try
	{
		if( sLogger=="dcerouter" )
			g_pPlutoLogger = new ServerLogger(PK_Device, Plug_And_Play::PK_DeviceTemplate_get_static(), sRouter_IP);
	...
	try
	{
		Plug_And_Play *pPlug_And_Play = new Plug_And_Play(PK_Device, sRouter_IP,true,bLocalMode);
		if ( pPlug_And_Play->GetConfig() && pPlug_And_Play->Connect(pPlug_And_Play->PK_DeviceTemplate_get()) ) 
		{
			g_pCommand_Impl=pPlug_And_Play;
			g_pDeadlockHandler=DeadlockHandler;
			g_pSocketCrashHandler=SocketCrashHandler;
			g_pPlutoLogger->Write(LV_STATUS, "Connect OK");
			pPlug_And_Play->CreateChildren();
			if( bLocalMode )
				pPlug_And_Play->RunLocalMode();
			else
				pthread_join(pPlug_And_Play->m_RequestHandlerThread, NULL);  // This function will return when the device is shutting down
