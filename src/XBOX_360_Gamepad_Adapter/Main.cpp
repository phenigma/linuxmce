...
extern "C" {
	...
	{ 
		...
		if( bIsRuntimePlugin )
			return XBOX_360_Gamepad_Adapter::PK_DeviceTemplate_get_static();
		else
			return 0;
	}
}
...
extern "C" {
	class Command_Impl *RegisterAsPlugIn(class Router *pRouter,int PK_Device,Logger *pPlutoLogger)
	{
		...
		XBOX_360_Gamepad_Adapter *pXBOX_360_Gamepad_Adapter = new XBOX_360_Gamepad_Adapter(PK_Device, "localhost",true,false,pRouter);
		if( pXBOX_360_Gamepad_Adapter->m_bQuit_get()|| !pXBOX_360_Gamepad_Adapter->GetConfig() )
		{
			delete pXBOX_360_Gamepad_Adapter;
			return NULL;
		}
		else
		{
			g_pCommand_Impl=pXBOX_360_Gamepad_Adapter;
			g_pDeadlockHandler=Plugin_DeadlockHandler;
			g_pSocketCrashHandler=Plugin_SocketCrashHandler;
		}
		return pXBOX_360_Gamepad_Adapter;
	}
}
//<-dceag-plug-e->
...
int main(int argc, char* argv[]) 
{
	...
	cout << "XBOX_360_Gamepad_Adapter, v." << VERSION << endl
	...
	if (bError)
	{
		cout << "A Pluto DCE Device.  See www.plutohome.com/dce for details." << endl
			<< "Usage: XBOX_360_Gamepad_Adapter [-r Router's IP] [-d My Device ID] [-l dcerouter|stdout|null|filename]" << endl
	...
	try
	{
		if( sLogger=="dcerouter" )
			LoggerWrapper::SetInstance(new ServerLogger(PK_Device, XBOX_360_Gamepad_Adapter::PK_DeviceTemplate_get_static(), sRouter_IP));
	...
	try
	{
		XBOX_360_Gamepad_Adapter *pXBOX_360_Gamepad_Adapter = new XBOX_360_Gamepad_Adapter(PK_Device, sRouter_IP,true,bLocalMode);
		if ( pXBOX_360_Gamepad_Adapter->GetConfig() && pXBOX_360_Gamepad_Adapter->Connect(pXBOX_360_Gamepad_Adapter->PK_DeviceTemplate_get()) ) 
		{
			g_pCommand_Impl=pXBOX_360_Gamepad_Adapter;
			g_pDeadlockHandler=DeadlockHandler;
			g_pSocketCrashHandler=SocketCrashHandler;
			LoggerWrapper::GetInstance()->Write(LV_STATUS, "Connect OK");
			pXBOX_360_Gamepad_Adapter->CreateChildren();
			if( bLocalMode )
				pXBOX_360_Gamepad_Adapter->RunLocalMode();
			else
			{
				if(pXBOX_360_Gamepad_Adapter->m_RequestHandlerThread)
					pthread_join(pXBOX_360_Gamepad_Adapter->m_RequestHandlerThread, NULL);  // This function will return when the device is shutting down
			}
		...
		{
			bAppError = true;
			if( pXBOX_360_Gamepad_Adapter->m_pEvent && pXBOX_360_Gamepad_Adapter->m_pEvent->m_pClientSocket && pXBOX_360_Gamepad_Adapter->m_pEvent->m_pClientSocket->m_eLastError==ClientSocket::cs_err_CannotConnect )
			{
				bAppError = false;
				bReload = false;
		...
		if( pXBOX_360_Gamepad_Adapter->m_bReload )
			bReload=true;
