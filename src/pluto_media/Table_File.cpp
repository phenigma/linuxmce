...
m_EK_Users_Private = val; is_modified=true; is_null[9]=false;}
...
values_list_comma_separated = values_list_comma_separated + pRow->PK_File_asSQL()+", "+pRow->EK_MediaType_asSQL()+", "+pRow->FK_MediaSubType_asSQL()+", "+pRow->FK_FileFormat_asSQL()+", "+pRow->DateAdded_asSQL()+", "+pRow->Path_asSQL()+", "+pRow->Filename_asSQL()+", "+pRow->Missing_asSQL()+", "+pRow->IsDirectory_asSQL()+", "+pRow->EK_Users_Private_asSQL()+", "+pRow->EK_Device_asSQL()+", "+pRow->ModificationDate_asSQL()+", "+pRow->AttrCount_asSQL()+", "+pRow->AttrDate_asSQL()+", "+pRow->psc_id_asSQL()+", "+pRow->psc_batch_asSQL()+", "+pRow->psc_user_asSQL()+", "+pRow->psc_frozen_asSQL()+", "+pRow->psc_restrict_asSQL();

	
		string query = "insert into File (`PK_File`, `EK_MediaType`, `FK_MediaSubType`, `FK_FileFormat`, `DateAdded`, `Path`, `Filename`, `Missing`, `IsDirectory`, `EK_Users_Private`, `EK_Device`, `ModificationDate`, `AttrCount`, `AttrDate`, `psc_id`, `psc_batch`, `psc_user`, `psc_frozen`, `psc_restrict`) values ("+
		...
		{	
			...
			if( g_pPlutoLogger )
				g_pPlutoLogger->Write(LV_CRITICAL,"Table_File::Commit Cannot perform query [%s] %s",query.c_str(),database->m_sLastMySqlError.c_str());
		...
		g_pPlutoLogger->Write(LV_CRITICAL,"PK_File is auto increment but has no value %s",database->m_sLastMySqlError.c_str());	
...
update_values_list = update_values_list + "`PK_File`="+pRow->PK_File_asSQL()+", `EK_MediaType`="+pRow->EK_MediaType_asSQL()+", `FK_MediaSubType`="+pRow->FK_MediaSubType_asSQL()+", `FK_FileFormat`="+pRow->FK_FileFormat_asSQL()+", `DateAdded`="+pRow->DateAdded_asSQL()+", `Path`="+pRow->Path_asSQL()+", `Filename`="+pRow->Filename_asSQL()+", `Missing`="+pRow->Missing_asSQL()+", `IsDirectory`="+pRow->IsDirectory_asSQL()+", `EK_Users_Private`="+pRow->EK_Users_Private_asSQL()+", `EK_Device`="+pRow->EK_Device_asSQL()+", `ModificationDate`="+pRow->ModificationDate_asSQL()+", `AttrCount`="+pRow->AttrCount_asSQL()+", `AttrDate`="+pRow->AttrDate_asSQL()+", `psc_id`="+pRow->psc_id_asSQL()+", `psc_batch`="+pRow->psc_batch_asSQL()+", `psc_user`="+pRow->psc_user_asSQL()+", `psc_frozen`="+pRow->psc_frozen_asSQL()+", `psc_restrict`="+pRow->psc_restrict_asSQL();
		...
		{	
			...
			cerr << "Cannot perform query: [" << query << "] " << database->m_sLastMySqlError << endl;
			if( g_pPlutoLogger )
				g_pPlutoLogger->Write(LV_CRITICAL,"Table_File::Commit Cannot perform update query [%s] %s",query.c_str(),database->m_sLastMySqlError.c_str());
...
condition = condition + "`PK_File`=" + tmp_PK_File;
		...
		{	
			...
			cerr << "Cannot perform query: [" << query << "] " << database->m_sLastMySqlError << endl;
			if( g_pPlutoLogger )
				g_pPlutoLogger->Write(LV_CRITICAL,"Table_File::Commit Cannot perform delete query [%s] %s",query.c_str(),database->m_sLastMySqlError.c_str());
			return false;
		}	
...
bool Table_File::GetRows(string where_statement,vector<class Row_File*> *rows)
{
	...
	{	
		...
		cerr << "Cannot perform query: [" << query << "] " << database->m_sLastMySqlError << endl;
		if( g_pPlutoLogger )
			g_pPlutoLogger->Write(LV_CRITICAL,"Table_File::GetRows Cannot perform query [%s] %s",query.c_str(),database->m_sLastMySqlError.c_str());
		return false;
	}	
	...
	if (!res)
	{
		...
		if( g_pPlutoLogger )
			g_pPlutoLogger->Write(LV_CRITICAL,"Table_File::GetRows mysql_store_result returned NULL handler");
		database->m_sLastMySqlError = mysql_error(database->m_pMySQL);
		return false;
	}	
...
sscanf(row[9], "%li", &(pRow->m_EK_Users_Private));
}
...
sscanf(row[10], "%li", &(pRow->m_EK_Device));
}
...
pRow->m_ModificationDate = string(row[11],lengths[11]);
}
...
sscanf(row[12], "%li", &(pRow->m_AttrCount));
}
...
pRow->m_AttrDate = string(row[13],lengths[13]);
}
...
condition = condition + "`PK_File`=" + tmp_PK_File;
	...
	{	
		...
		cerr << "Cannot perform query: [" << query << "] " << database->m_sLastMySqlError << endl;
		if( g_pPlutoLogger )
			g_pPlutoLogger->Write(LV_CRITICAL,"Table_File::FetchRow Cannot perform query [%s] %s",query.c_str(),database->m_sLastMySqlError.c_str());
		return NULL;
	}	
	...
	if (!res)
	{
		cerr << "mysql_store_result returned NULL handler" << endl;
		if( g_pPlutoLogger )
			g_pPlutoLogger->Write(LV_CRITICAL,"Table_File::FetchRow mysql_store_result returned NULL handler");
		database->m_sLastMySqlError = mysql_error(database->m_pMySQL);
		return NULL;
	}	
