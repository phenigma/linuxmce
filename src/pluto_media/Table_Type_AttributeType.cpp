...
Table_Type_AttributeType::~Table_Type_AttributeType()
{
	...
	{
		Row_Type_AttributeType *pRow = (Row_Type_AttributeType *) (*it).second;
		delete pRow;
	}
...
void Row_Type_AttributeType::Delete()
{
	...
	if (!is_deleted)
		...
		{	
			...
			for (i = table->addedRows.begin(); (i!=table->addedRows.end()) && ( (Row_Type_AttributeType *) *i != this); i++);
		...
		{
			DoubleLongKey key(pRow->m_FK_Type,pRow->m_FK_AttributeType);
...
return m_psc_mod;}
long int Row_Type_AttributeType::psc_restrict_get(){PLUTO_SAFETY_LOCK_ERRORSONLY(sl,table->database->m_MySqlMutex);
...
void Row_Type_AttributeType::psc_restrict_set(long int val){PLUTO_SAFETY_LOCK_ERRORSONLY(sl,table->database->m_MySqlMutex);
...
return is_null[7];}
bool Row_Type_AttributeType::psc_restrict_isNull() {PLUTO_SAFETY_LOCK_ERRORSONLY(sl,table->database->m_MySqlMutex);
...
void Row_Type_AttributeType::psc_restrict_setNull(bool val){PLUTO_SAFETY_LOCK_ERRORSONLY(sl,table->database->m_MySqlMutex);
is_null[9]=val;
is_modified=true;
}
...
string Row_Type_AttributeType::psc_restrict_asSQL()
{
PLUTO_SAFETY_LOCK_ERRORSONLY(sl,table->database->m_MySqlMutex);
...
bool Table_Type_AttributeType::Commit(bool bDeleteFailedModifiedRow,bool bDeleteFailedInsertRow)
{
...
values_list_comma_separated = values_list_comma_separated + pRow->FK_Type_asSQL()+", "+pRow->FK_AttributeType_asSQL()+", "+pRow->Identifier_asSQL()+", "+pRow->CombineAsOne_asSQL()+", "+pRow->psc_id_asSQL()+", "+pRow->psc_batch_asSQL()+", "+pRow->psc_user_asSQL()+", "+pRow->psc_frozen_asSQL()+", "+pRow->psc_restrict_asSQL();

	
		string query = "insert into Type_AttributeType (`FK_Type`, `FK_AttributeType`, `Identifier`, `CombineAsOne`, `psc_id`, `psc_batch`, `psc_user`, `psc_frozen`, `psc_restrict`) values ("+
...
Row_Type_AttributeType* Table_Type_AttributeType::GetRow(long int in_FK_Type, long int in_FK_AttributeType)
{
	...
	//row is cached
	if (i!=cachedRows.end())
		return (Row_Type_AttributeType*) (*i).second;
