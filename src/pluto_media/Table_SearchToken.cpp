...
values_list_comma_separated = values_list_comma_separated + pRow->PK_SearchToken_asSQL()+", "+pRow->Token_asSQL()+", "+pRow->psc_id_asSQL()+", "+pRow->psc_batch_asSQL()+", "+pRow->psc_user_asSQL()+", "+pRow->psc_frozen_asSQL()+", "+pRow->psc_restrict_asSQL();
		...
		{	
			...
			cerr << "Cannot perform query: [" << query << "] " << database->m_sLastMySqlError << endl;
				LoggerWrapper::GetInstance()->Write(LV_CRITICAL,"Table_SearchToken::Commit Cannot perform query [%s] %s",query.c_str(),database->m_sLastMySqlError.c_str());
...
else 	LoggerWrapper::GetInstance()->Write(LV_CRITICAL,"PK_SearchToken is auto increment but has no value %s",database->m_sLastMySqlError.c_str());	
...
update_values_list = update_values_list + "`PK_SearchToken`="+pRow->PK_SearchToken_asSQL()+", `Token`="+pRow->Token_asSQL()+", `psc_id`="+pRow->psc_id_asSQL()+", `psc_batch`="+pRow->psc_batch_asSQL()+", `psc_user`="+pRow->psc_user_asSQL()+", `psc_frozen`="+pRow->psc_frozen_asSQL()+", `psc_restrict`="+pRow->psc_restrict_asSQL();
		...
		{	
			database->m_sLastMySqlError = mysql_error(database->m_pMySQL);
			cerr << "Cannot perform query: [" << query << "] " << database->m_sLastMySqlError << endl;
				LoggerWrapper::GetInstance()->Write(LV_CRITICAL,"Table_SearchToken::Commit Cannot perform update query [%s] %s",query.c_str(),database->m_sLastMySqlError.c_str());
...
condition = condition + "`PK_SearchToken`=" + tmp_PK_SearchToken;
		...
		{	
			database->m_sLastMySqlError = mysql_error(database->m_pMySQL);
			cerr << "Cannot perform query: [" << query << "] " << database->m_sLastMySqlError << endl;
				LoggerWrapper::GetInstance()->Write(LV_CRITICAL,"Table_SearchToken::Commit Cannot perform delete query [%s] %s",query.c_str(),database->m_sLastMySqlError.c_str());
			return false;
		}	
...
bool Table_SearchToken::GetRows(string where_statement,vector<class Row_SearchToken*> *rows)
{
	...
	{	
		database->m_sLastMySqlError = mysql_error(database->m_pMySQL);
		cerr << "Cannot perform query: [" << query << "] " << database->m_sLastMySqlError << endl;
		LoggerWrapper::GetInstance()->Write(LV_CRITICAL,"Table_SearchToken::GetRows Cannot perform query [%s] %s",query.c_str(),database->m_sLastMySqlError.c_str());
		return false;
	}	
...
condition = condition + "`PK_SearchToken`=" + tmp_PK_SearchToken;
	...
	{	
		database->m_sLastMySqlError = mysql_error(database->m_pMySQL);
		cerr << "Cannot perform query: [" << query << "] " << database->m_sLastMySqlError << endl;
		LoggerWrapper::GetInstance()->Write(LV_CRITICAL,"Table_SearchToken::FetchRow Cannot perform query [%s] %s",query.c_str(),database->m_sLastMySqlError.c_str());
		return NULL;
	}	
