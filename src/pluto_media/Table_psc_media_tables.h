...
class DECLSPECIFIER Table_psc_media_tables : public TableBase , SingleLongKeyBase
{
private:
...
public:
	Table_psc_media_tables(Database_pluto_media *pDatabase):database(pDatabase)
	{
	};
...
class DECLSPECIFIER Row_psc_media_tables : public TableRow, public SerializeClass
	{
...
void last_psc_batch_set(long int val);
		...
		void SetupSerialization(int iSC_Version) {
			StartSerializeList() + m_PK_psc_media_tables+ m_Tablename+ m_filter+ m_frozen+ m_anonymous+ m_last_psc_id+ m_last_psc_batch;
		}
	private:
