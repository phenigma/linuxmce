...
values_list_comma_separated = values_list_comma_separated + pRow->PK_Bookmark_asSQL()+", "+pRow->FK_File_asSQL()+", "+pRow->FK_Disc_asSQL()+", "+pRow->FK_Playlist_asSQL()+", "+pRow->FK_MediaProvider_asSQL()+", "+pRow->EK_MediaType_asSQL()+", "+pRow->FK_Picture_asSQL()+", "+pRow->EK_Users_asSQL()+", "+pRow->Description_asSQL()+", "+pRow->Position_asSQL()+", "+pRow->IsAutoResume_asSQL()+", "+pRow->psc_id_asSQL()+", "+pRow->psc_batch_asSQL()+", "+pRow->psc_user_asSQL()+", "+pRow->psc_frozen_asSQL()+", "+pRow->psc_restrict_asSQL();
		...
		{	
			...
			if( g_pPlutoLogger )
				g_pPlutoLogger->Write(LV_CRITICAL,"Table_Bookmark::Commit Cannot perform query [%s] %s",query.c_str(),database->m_sLastMySqlError.c_str());
		...
		g_pPlutoLogger->Write(LV_CRITICAL,"PK_Bookmark is auto increment but has no value %s",database->m_sLastMySqlError.c_str());	
...
update_values_list = update_values_list + "`PK_Bookmark`="+pRow->PK_Bookmark_asSQL()+", `FK_File`="+pRow->FK_File_asSQL()+", `FK_Disc`="+pRow->FK_Disc_asSQL()+", `FK_Playlist`="+pRow->FK_Playlist_asSQL()+", `FK_MediaProvider`="+pRow->FK_MediaProvider_asSQL()+", `EK_MediaType`="+pRow->EK_MediaType_asSQL()+", `FK_Picture`="+pRow->FK_Picture_asSQL()+", `EK_Users`="+pRow->EK_Users_asSQL()+", `Description`="+pRow->Description_asSQL()+", `Position`="+pRow->Position_asSQL()+", `IsAutoResume`="+pRow->IsAutoResume_asSQL()+", `psc_id`="+pRow->psc_id_asSQL()+", `psc_batch`="+pRow->psc_batch_asSQL()+", `psc_user`="+pRow->psc_user_asSQL()+", `psc_frozen`="+pRow->psc_frozen_asSQL()+", `psc_restrict`="+pRow->psc_restrict_asSQL();
		...
		{	
			...
			cerr << "Cannot perform query: [" << query << "] " << database->m_sLastMySqlError << endl;
			if( g_pPlutoLogger )
				g_pPlutoLogger->Write(LV_CRITICAL,"Table_Bookmark::Commit Cannot perform update query [%s] %s",query.c_str(),database->m_sLastMySqlError.c_str());
...
condition = condition + "`PK_Bookmark`=" + tmp_PK_Bookmark;
		...
		{	
			...
			cerr << "Cannot perform query: [" << query << "] " << database->m_sLastMySqlError << endl;
			if( g_pPlutoLogger )
				g_pPlutoLogger->Write(LV_CRITICAL,"Table_Bookmark::Commit Cannot perform delete query [%s] %s",query.c_str(),database->m_sLastMySqlError.c_str());
			return false;
		}	
...
bool Table_Bookmark::GetRows(string where_statement,vector<class Row_Bookmark*> *rows)
{
	...
	{	
		...
		cerr << "Cannot perform query: [" << query << "] " << database->m_sLastMySqlError << endl;
		if( g_pPlutoLogger )
			g_pPlutoLogger->Write(LV_CRITICAL,"Table_Bookmark::GetRows Cannot perform query [%s] %s",query.c_str(),database->m_sLastMySqlError.c_str());
		return false;
	}	
	...
	if (!res)
	{
		...
		if( g_pPlutoLogger )
			g_pPlutoLogger->Write(LV_CRITICAL,"Table_Bookmark::GetRows mysql_store_result returned NULL handler");
		database->m_sLastMySqlError = mysql_error(database->m_pMySQL);
		return false;
	}	
...
condition = condition + "`PK_Bookmark`=" + tmp_PK_Bookmark;
	...
	{	
		...
		cerr << "Cannot perform query: [" << query << "] " << database->m_sLastMySqlError << endl;
		if( g_pPlutoLogger )
			g_pPlutoLogger->Write(LV_CRITICAL,"Table_Bookmark::FetchRow Cannot perform query [%s] %s",query.c_str(),database->m_sLastMySqlError.c_str());
		return NULL;
	}	
	...
	if (!res)
	{
		cerr << "mysql_store_result returned NULL handler" << endl;
		if( g_pPlutoLogger )
			g_pPlutoLogger->Write(LV_CRITICAL,"Table_Bookmark::FetchRow mysql_store_result returned NULL handler");
		database->m_sLastMySqlError = mysql_error(database->m_pMySQL);
		return NULL;
	}	
