...
class DECLSPECIFIER Table_Type : public TableBase , SingleLongKeyBase
{
private:
...
public:
	Table_Type(Database_pluto_media *pDatabase):database(pDatabase)
	{
	};
	~Table_Type();
...
class DECLSPECIFIER Row_Type : public TableRow, public SerializeClass
	{
...
void Type_Extension_FK_Type_getrows(vector <class Row_Type_Extension*> *rows);
		...
		void SetupSerialization(int iSC_Version) {
			StartSerializeList() + m_PK_Type+ m_Description+ m_Define+ m_psc_id+ m_psc_batch+ m_psc_user+ m_psc_frozen+ m_psc_mod+ m_psc_restrict;
		}
	private:
