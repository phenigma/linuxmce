...
values_list_comma_separated = values_list_comma_separated + pRow->PK_AttributeType_asSQL()+", "+pRow->Description_asSQL()+", "+pRow->Define_asSQL()+", "+pRow->PicPriority_asSQL()+", "+pRow->psc_id_asSQL()+", "+pRow->psc_batch_asSQL()+", "+pRow->psc_user_asSQL()+", "+pRow->psc_frozen_asSQL()+", "+pRow->psc_restrict_asSQL();
		...
		{	
			...
			LoggerWrapper::GetInstance()->Write(LV_CRITICAL,"Table_AttributeType::Commit Cannot perform query [%s] %s reconnect: %d result2: %d",query.c_str(),database->m_sLastMySqlError.c_str(),(int) bResult, iResult2);
		...
		LoggerWrapper::GetInstance()->Write(LV_CRITICAL,"PK_AttributeType is auto increment but has no value %s",database->m_sLastMySqlError.c_str());	
...
update_values_list = update_values_list + "`PK_AttributeType`="+pRow->PK_AttributeType_asSQL()+", `Description`="+pRow->Description_asSQL()+", `Define`="+pRow->Define_asSQL()+", `PicPriority`="+pRow->PicPriority_asSQL()+", `psc_id`="+pRow->psc_id_asSQL()+", `psc_batch`="+pRow->psc_batch_asSQL()+", `psc_user`="+pRow->psc_user_asSQL()+", `psc_frozen`="+pRow->psc_frozen_asSQL()+", `psc_restrict`="+pRow->psc_restrict_asSQL();
		...
		{	
			...
			LoggerWrapper::GetInstance()->Write(LV_CRITICAL,"Table_AttributeType::Commit Cannot perform update query [%s] %s reconnect: %d result2: %d",query.c_str(),database->m_sLastMySqlError.c_str(),(int) bResult, iResult2);
			if( iResult2!=0 )  // We can keep going if the time it worked
			{
...
condition = condition + "`PK_AttributeType`=" + tmp_PK_AttributeType;
		...
		{	
			...
			LoggerWrapper::GetInstance()->Write(LV_CRITICAL,"Table_AttributeType::Commit Cannot perform delete query [%s] %s reconnect: %d result2: %d",query.c_str(),database->m_sLastMySqlError.c_str(),(int) bResult, iResult2);
			if( iResult2!=0 )  // We can keep going if the time it worked
				return false;
		}	
...
bool Table_AttributeType::GetRows(string where_statement,vector<class Row_AttributeType*> *rows)
{
	...
	{	
		...
		LoggerWrapper::GetInstance()->Write(LV_CRITICAL,"Table_AttributeType::GetRows Cannot perform query [%s] %s reconnect: %d result2: %d",query.c_str(),database->m_sLastMySqlError.c_str(),(int) bResult, iResult2);
		if( iResult2!=0 )  // We can keep going if the time it worked
			return false;
	}	
	...
	if (!res)
	{
		...
		LoggerWrapper::GetInstance()->Write(LV_CRITICAL,"Table_AttributeType::GetRows mysql_store_result returned NULL handler");
		database->m_sLastMySqlError = mysql_error(database->m_pMySQL);
		return false;
	}	
...
condition = condition + "`PK_AttributeType`=" + tmp_PK_AttributeType;
	...
	{	
		...
		LoggerWrapper::GetInstance()->Write(LV_CRITICAL,"Table_AttributeType::FetchRow Cannot perform query [%s] %s reconnect: %d result2: %d",query.c_str(),database->m_sLastMySqlError.c_str(),(int) bResult, iResult2);
		if( iResult2!=0 )  // We can keep going if the time it worked
			return NULL;
	}	
	...
	{
		cerr << "mysql_store_result returned NULL handler" << endl;
		LoggerWrapper::GetInstance()->Write(LV_CRITICAL,"Table_AttributeType::FetchRow mysql_store_result returned NULL handler");
		database->m_sLastMySqlError = mysql_error(database->m_pMySQL);
		return NULL;
	}	
