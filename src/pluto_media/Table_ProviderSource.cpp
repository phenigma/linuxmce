...
long int Row_ProviderSource::PK_ProviderSource_get(){PLUTO_SAFETY_LOCK_ERRORSONLY(sl,table->database->m_MySqlMutex);
...
return m_EK_Country;}
long int Row_ProviderSource::EK_MediaType_get(){PLUTO_SAFETY_LOCK_ERRORSONLY(sl,table->database->m_MySqlMutex);

return m_EK_MediaType;}
string Row_ProviderSource::ProviderOrganization_get(){PLUTO_SAFETY_LOCK_ERRORSONLY(sl,table->database->m_MySqlMutex);

return m_ProviderOrganization;}
long int Row_ProviderSource::EK_Package_get(){PLUTO_SAFETY_LOCK_ERRORSONLY(sl,table->database->m_MySqlMutex);

return m_EK_Package;}
string Row_ProviderSource::FillCommandLine_get(){PLUTO_SAFETY_LOCK_ERRORSONLY(sl,table->database->m_MySqlMutex);

return m_FillCommandLine;}
short int Row_ProviderSource::FillHours_get(){PLUTO_SAFETY_LOCK_ERRORSONLY(sl,table->database->m_MySqlMutex);

return m_FillHours;}
string Row_ProviderSource::ProviderCommandLine_get(){PLUTO_SAFETY_LOCK_ERRORSONLY(sl,table->database->m_MySqlMutex);

return m_ProviderCommandLine;}
string Row_ProviderSource::DeviceCommandLine_get(){PLUTO_SAFETY_LOCK_ERRORSONLY(sl,table->database->m_MySqlMutex);

return m_DeviceCommandLine;}
string Row_ProviderSource::PackageCommandLine_get(){PLUTO_SAFETY_LOCK_ERRORSONLY(sl,table->database->m_MySqlMutex);

return m_PackageCommandLine;}
string Row_ProviderSource::LineupCommandLine_get(){PLUTO_SAFETY_LOCK_ERRORSONLY(sl,table->database->m_MySqlMutex);

return m_LineupCommandLine;}
long int Row_ProviderSource::psc_id_get(){PLUTO_SAFETY_LOCK_ERRORSONLY(sl,table->database->m_MySqlMutex);

return m_psc_id;}
long int Row_ProviderSource::psc_batch_get(){PLUTO_SAFETY_LOCK_ERRORSONLY(sl,table->database->m_MySqlMutex);

return m_psc_batch;}
long int Row_ProviderSource::psc_user_get(){PLUTO_SAFETY_LOCK_ERRORSONLY(sl,table->database->m_MySqlMutex);

return m_psc_user;}
short int Row_ProviderSource::psc_frozen_get(){PLUTO_SAFETY_LOCK_ERRORSONLY(sl,table->database->m_MySqlMutex);

return m_psc_frozen;}
string Row_ProviderSource::psc_mod_get(){PLUTO_SAFETY_LOCK_ERRORSONLY(sl,table->database->m_MySqlMutex);

return m_psc_mod;}
long int Row_ProviderSource::psc_restrict_get(){PLUTO_SAFETY_LOCK_ERRORSONLY(sl,table->database->m_MySqlMutex);

return m_psc_restrict;}

		
void Row_ProviderSource::PK_ProviderSource_set(long int val){PLUTO_SAFETY_LOCK_ERRORSONLY(sl,table->database->m_MySqlMutex);
...
char *buf = new char[3];
mysql_real_escape_string(table->database->m_pMySQL, buf, m_ProviderOrganization.c_str(), (unsigned long) min((size_t)1,m_ProviderOrganization.size()));
string s=string()+"\""+buf+"\"";
delete[] buf;
return s;
}
...
mysql_real_escape_string(table->database->m_pMySQL, buf, m_FillCommandLine.c_str(), (unsigned long) min((size_t)255,m_FillCommandLine.size()));
string s=string()+"\""+buf+"\"";
delete[] buf;
return s;
}
...
char *buf = new char[511];
mysql_real_escape_string(table->database->m_pMySQL, buf, m_ProviderCommandLine.c_str(), (unsigned long) min((size_t)255,m_ProviderCommandLine.size()));
string s=string()+"\""+buf+"\"";
delete[] buf;
return s;
}
...
char *buf = new char[511];
mysql_real_escape_string(table->database->m_pMySQL, buf, m_DeviceCommandLine.c_str(), (unsigned long) min((size_t)255,m_DeviceCommandLine.size()));
string s=string()+"\""+buf+"\"";
delete[] buf;
return s;
}
...
char *buf = new char[511];
mysql_real_escape_string(table->database->m_pMySQL, buf, m_PackageCommandLine.c_str(), (unsigned long) min((size_t)255,m_PackageCommandLine.size()));
string s=string()+"\""+buf+"\"";
delete[] buf;
return s;
}
...
char *buf = new char[511];
mysql_real_escape_string(table->database->m_pMySQL, buf, m_LineupCommandLine.c_str(), (unsigned long) min((size_t)255,m_LineupCommandLine.size()));
string s=string()+"\""+buf+"\"";
delete[] buf;
return s;
}
...
values_list_comma_separated = values_list_comma_separated + pRow->PK_ProviderSource_asSQL()+", "+pRow->Description_asSQL()+", "+pRow->Comments_asSQL()+", "+pRow->UserNamePassword_asSQL()+", "+pRow->EK_Country_asSQL()+", "+pRow->EK_MediaType_asSQL()+", "+pRow->ProviderOrganization_asSQL()+", "+pRow->EK_Package_asSQL()+", "+pRow->FillCommandLine_asSQL()+", "+pRow->FillHours_asSQL()+", "+pRow->ProviderCommandLine_asSQL()+", "+pRow->DeviceCommandLine_asSQL()+", "+pRow->PackageCommandLine_asSQL()+", "+pRow->LineupCommandLine_asSQL()+", "+pRow->psc_id_asSQL()+", "+pRow->psc_batch_asSQL()+", "+pRow->psc_user_asSQL()+", "+pRow->psc_frozen_asSQL()+", "+pRow->psc_restrict_asSQL();
		...
		{	
			...
			LoggerWrapper::GetInstance()->Write(LV_CRITICAL,"Table_ProviderSource::Commit Cannot perform query [%s] %s reconnect: %d result2: %d",query.c_str(),database->m_sLastMySqlError.c_str(),(int) bResult, iResult2);
		...
		LoggerWrapper::GetInstance()->Write(LV_CRITICAL,"PK_ProviderSource is auto increment but has no value %s",database->m_sLastMySqlError.c_str());	
...
update_values_list = update_values_list + "`PK_ProviderSource`="+pRow->PK_ProviderSource_asSQL()+", `Description`="+pRow->Description_asSQL()+", `Comments`="+pRow->Comments_asSQL()+", `UserNamePassword`="+pRow->UserNamePassword_asSQL()+", `EK_Country`="+pRow->EK_Country_asSQL()+", `EK_MediaType`="+pRow->EK_MediaType_asSQL()+", `ProviderOrganization`="+pRow->ProviderOrganization_asSQL()+", `EK_Package`="+pRow->EK_Package_asSQL()+", `FillCommandLine`="+pRow->FillCommandLine_asSQL()+", `FillHours`="+pRow->FillHours_asSQL()+", `ProviderCommandLine`="+pRow->ProviderCommandLine_asSQL()+", `DeviceCommandLine`="+pRow->DeviceCommandLine_asSQL()+", `PackageCommandLine`="+pRow->PackageCommandLine_asSQL()+", `LineupCommandLine`="+pRow->LineupCommandLine_asSQL()+", `psc_id`="+pRow->psc_id_asSQL()+", `psc_batch`="+pRow->psc_batch_asSQL()+", `psc_user`="+pRow->psc_user_asSQL()+", `psc_frozen`="+pRow->psc_frozen_asSQL()+", `psc_restrict`="+pRow->psc_restrict_asSQL();
		...
		{	
			...
			LoggerWrapper::GetInstance()->Write(LV_CRITICAL,"Table_ProviderSource::Commit Cannot perform update query [%s] %s reconnect: %d result2: %d",query.c_str(),database->m_sLastMySqlError.c_str(),(int) bResult, iResult2);
			if( iResult2!=0 )  // We can keep going if the time it worked
			{
...
condition = condition + "`PK_ProviderSource`=" + tmp_PK_ProviderSource;
		...
		{	
			...
			LoggerWrapper::GetInstance()->Write(LV_CRITICAL,"Table_ProviderSource::Commit Cannot perform delete query [%s] %s reconnect: %d result2: %d",query.c_str(),database->m_sLastMySqlError.c_str(),(int) bResult, iResult2);
			if( iResult2!=0 )  // We can keep going if the time it worked
				return false;
		}	
...
bool Table_ProviderSource::GetRows(string where_statement,vector<class Row_ProviderSource*> *rows)
{
	...
	{	
		...
		LoggerWrapper::GetInstance()->Write(LV_CRITICAL,"Table_ProviderSource::GetRows Cannot perform query [%s] %s reconnect: %d result2: %d",query.c_str(),database->m_sLastMySqlError.c_str(),(int) bResult, iResult2);
		if( iResult2!=0 )  // We can keep going if the time it worked
			return false;
	}	
	...
	if (!res)
	{
		...
		LoggerWrapper::GetInstance()->Write(LV_CRITICAL,"Table_ProviderSource::GetRows mysql_store_result returned NULL handler");
		database->m_sLastMySqlError = mysql_error(database->m_pMySQL);
		return false;
	}	
...
condition = condition + "`PK_ProviderSource`=" + tmp_PK_ProviderSource;
	...
	{	
		...
		LoggerWrapper::GetInstance()->Write(LV_CRITICAL,"Table_ProviderSource::FetchRow Cannot perform query [%s] %s reconnect: %d result2: %d",query.c_str(),database->m_sLastMySqlError.c_str(),(int) bResult, iResult2);
		if( iResult2!=0 )  // We can keep going if the time it worked
			return NULL;
	}	
	...
	{
		cerr << "mysql_store_result returned NULL handler" << endl;
		LoggerWrapper::GetInstance()->Write(LV_CRITICAL,"Table_ProviderSource::FetchRow mysql_store_result returned NULL handler");
		database->m_sLastMySqlError = mysql_error(database->m_pMySQL);
		return NULL;
	}	
