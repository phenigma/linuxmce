...
return m_Slot;}
short int Row_Disc::Offline_get(){PLUTO_SAFETY_LOCK_ERRORSONLY(sl,table->database->m_MySqlMutex);
...
void Row_Disc::Offline_set(short int val){PLUTO_SAFETY_LOCK_ERRORSONLY(sl,table->database->m_MySqlMutex);

m_Offline = val; is_modified=true; is_null[7]=false;}
void Row_Disc::EK_Users_Private_set(long int val){PLUTO_SAFETY_LOCK_ERRORSONLY(sl,table->database->m_MySqlMutex);

m_EK_Users_Private = val; is_modified=true; is_null[8]=false;}
...
values_list_comma_separated = values_list_comma_separated + pRow->PK_Disc_asSQL()+", "+pRow->ID_asSQL()+", "+pRow->EK_MediaType_asSQL()+", "+pRow->FK_MediaSubType_asSQL()+", "+pRow->FK_FileFormat_asSQL()+", "+pRow->EK_Device_asSQL()+", "+pRow->Slot_asSQL()+", "+pRow->Offline_asSQL()+", "+pRow->EK_Users_Private_asSQL()+", "+pRow->psc_id_asSQL()+", "+pRow->psc_batch_asSQL()+", "+pRow->psc_user_asSQL()+", "+pRow->psc_frozen_asSQL()+", "+pRow->psc_restrict_asSQL();

	
		string query = "insert into Disc (`PK_Disc`, `ID`, `EK_MediaType`, `FK_MediaSubType`, `FK_FileFormat`, `EK_Device`, `Slot`, `Offline`, `EK_Users_Private`, `psc_id`, `psc_batch`, `psc_user`, `psc_frozen`, `psc_restrict`) values ("+
...
update_values_list = update_values_list + "`PK_Disc`="+pRow->PK_Disc_asSQL()+", `ID`="+pRow->ID_asSQL()+", `EK_MediaType`="+pRow->EK_MediaType_asSQL()+", `FK_MediaSubType`="+pRow->FK_MediaSubType_asSQL()+", `FK_FileFormat`="+pRow->FK_FileFormat_asSQL()+", `EK_Device`="+pRow->EK_Device_asSQL()+", `Slot`="+pRow->Slot_asSQL()+", `Offline`="+pRow->Offline_asSQL()+", `EK_Users_Private`="+pRow->EK_Users_Private_asSQL()+", `psc_id`="+pRow->psc_id_asSQL()+", `psc_batch`="+pRow->psc_batch_asSQL()+", `psc_user`="+pRow->psc_user_asSQL()+", `psc_frozen`="+pRow->psc_frozen_asSQL()+", `psc_restrict`="+pRow->psc_restrict_asSQL();
...
sscanf(row[7], "%hi", &(pRow->m_Offline));
}
...
sscanf(row[8], "%li", &(pRow->m_EK_Users_Private));
}
