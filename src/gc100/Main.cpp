...
int main(int argc, char* argv[]) 
{
	...
	if (bError)
	{
		cout << "A Pluto DCE Device.  See www.plutohome.com/dce for details." << endl
			<< "Usage: gc100 [-r Router's IP] [-d My Device ID] [-l dcerouter|stdout|null|filename]" << endl
	...
	try
	{
		if( sLogger=="dcerouter" )
			LoggerWrapper::SetInstance(new ServerLogger(PK_Device, gc100::PK_DeviceTemplate_get_static(), sRouter_IP));
	...
	try
	{
		gc100 *pgc100 = new gc100(PK_Device, sRouter_IP,true,bLocalMode);
		if ( pgc100->GetConfig() && pgc100->Connect(pgc100->PK_DeviceTemplate_get()) ) 
		{
			...
			{
				if(pgc100->m_RequestHandlerThread)
					pthread_join(pgc100->m_RequestHandlerThread, NULL);  // This function will return when the device is shutting down
			}
		...
		{
			bAppError = true;
			if( pgc100->m_pEvent && pgc100->m_pEvent->m_pClientSocket && pgc100->m_pEvent->m_pClientSocket->m_eLastError==ClientSocket::cs_err_CannotConnect )
			{
				bAppError = false;
				bReload = false;
