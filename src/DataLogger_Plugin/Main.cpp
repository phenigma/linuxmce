...
extern "C" {
	...
	{ 
		...
		if( bIsRuntimePlugin )
			return DataLogger_Plugin::PK_DeviceTemplate_get_static();
		else
			return 0;
	}
}
...
extern "C" {
	class Command_Impl *RegisterAsPlugIn(class Router *pRouter,int PK_Device,Logger *pPlutoLogger)
	{
		...
		DataLogger_Plugin *pDataLogger_Plugin = new DataLogger_Plugin(PK_Device, "localhost",true,false,pRouter);
		if( pDataLogger_Plugin->m_bQuit_get()|| !pDataLogger_Plugin->GetConfig() )
		{
			delete pDataLogger_Plugin;
			return NULL;
		}
		else
		{
			g_pCommand_Impl=pDataLogger_Plugin;
			g_pDeadlockHandler=Plugin_DeadlockHandler;
			g_pSocketCrashHandler=Plugin_SocketCrashHandler;
		}
		return pDataLogger_Plugin;
	}
}
//<-dceag-plug-e->
...
int main(int argc, char* argv[]) 
{
	...
	cout << "DataLogger_Plugin, v." << VERSION << endl
	...
	if (bError)
	{
		cout << "A Pluto DCE Device.  See www.plutohome.com/dce for details." << endl
			<< "Usage: DataLogger_Plugin [-r Router's IP] [-d My Device ID] [-l dcerouter|stdout|null|filename]" << endl
	...
	try
	{
		if( sLogger=="dcerouter" )
			LoggerWrapper::SetInstance(new ServerLogger(PK_Device, DataLogger_Plugin::PK_DeviceTemplate_get_static(), sRouter_IP));
	...
	try
	{
		DataLogger_Plugin *pDataLogger_Plugin = new DataLogger_Plugin(PK_Device, sRouter_IP,true,bLocalMode);
		if ( pDataLogger_Plugin->GetConfig() && pDataLogger_Plugin->Connect(pDataLogger_Plugin->PK_DeviceTemplate_get()) ) 
		{
			g_pCommand_Impl=pDataLogger_Plugin;
			g_pDeadlockHandler=DeadlockHandler;
			g_pSocketCrashHandler=SocketCrashHandler;
			LoggerWrapper::GetInstance()->Write(LV_STATUS, "Connect OK");
			pDataLogger_Plugin->CreateChildren();
			if( bLocalMode )
				pDataLogger_Plugin->RunLocalMode();
			else
			{
				if(pDataLogger_Plugin->m_RequestHandlerThread)
					pthread_join(pDataLogger_Plugin->m_RequestHandlerThread, NULL);  // This function will return when the device is shutting down
			}
		...
		{
			bAppError = true;
			if( pDataLogger_Plugin->m_pEvent && pDataLogger_Plugin->m_pEvent->m_pClientSocket && pDataLogger_Plugin->m_pEvent->m_pClientSocket->m_eLastError==ClientSocket::cs_err_CannotConnect )
			{
				bAppError = false;
				bReload = false;
		...
		if( pDataLogger_Plugin->m_bReload )
			bReload=true;
