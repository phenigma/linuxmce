...
open(FILE,"> /tftpboot/SEPDefault7921.cnf.xml");
print FILE $str_SEPDefault;
close(FILE);

`ln -sf /tftpboot/SEPDefault7921.cnf.xml /tftpboot/SEP$Device_MAC.cnf.xml`;

### Update Cisco 7921 Orbiter
#my $ORB_ID;
#my $ORB_CNT;
#my $DB_ROW;
#my $DB_PL_HANDLE = DBI->connect(&read_pluto_cred()) or die "Can't connect to database: $DBI::errstr\n";
#my $DB_SQL = "select count(PK_Device) from Device where FK_DeviceTemplate=2193";
...
#    $DB_STATEMENT->finish();
#}
#$DB_SQL = "select PK_Device from Device where FK_DeviceTemplate=2193 and FK_Device_ControlledVia=$Device_ID";
