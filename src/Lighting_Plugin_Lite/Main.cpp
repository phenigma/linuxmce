...
extern "C" {
	...
	{ 
		...
		if( bIsRuntimePlugin )
			return Lighting_Plugin::PK_DeviceTemplate_get_static();
		else
			return 0;
	}
}
...
int main(int argc, char* argv[]) 
{
	...
	cout << "Lighting_Plugin, v." << VERSION << endl
	...
	if (bError)
	{
		cout << "A Pluto DCE Device.  See www.plutohome.com/dce for details." << endl
			<< "Usage: Lighting_Plugin [-r Router's IP] [-d My Device ID] [-l dcerouter|stdout|null|filename]" << endl
	...
	try
	{
		if( sLogger=="dcerouter" )
			LoggerWrapper::SetInstance(new ServerLogger(PK_Device, Lighting_Plugin::PK_DeviceTemplate_get_static(), sRouter_IP));
	...
	try
	{
		Lighting_Plugin *pLighting_Plugin = new Lighting_Plugin(PK_Device, sRouter_IP,true,bLocalMode);
		if ( pLighting_Plugin->GetConfig() && pLighting_Plugin->Connect(pLighting_Plugin->PK_DeviceTemplate_get()) ) 
		{
			...
			pLighting_Plugin->CreateChildren();
			if( bLocalMode )
				pLighting_Plugin->RunLocalMode();
			else
			{
				if(pLighting_Plugin->m_RequestHandlerThread)
					pthread_join(pLighting_Plugin->m_RequestHandlerThread, NULL);  // This function will return when the device is shutting down
			}
		...
		{
			bAppError = true;
			if( pLighting_Plugin->m_pEvent && pLighting_Plugin->m_pEvent->m_pClientSocket && pLighting_Plugin->m_pEvent->m_pClientSocket->m_eLastError==ClientSocket::cs_err_CannotConnect )
			{
				bAppError = false;
				bReload = false;
