...
extern "C" {
	...
	{ 
		...
		if( bIsRuntimePlugin )
			return LMCE_Launch_Manager::PK_DeviceTemplate_get_static();
		else
			return 0;
	}
}
...
extern "C" {
	class Command_Impl *RegisterAsPlugIn(class Router *pRouter,int PK_Device,Logger *pPlutoLogger)
	{
		...
		LMCE_Launch_Manager *pLMCE_Launch_Manager = new LMCE_Launch_Manager(PK_Device, "localhost",true,false,pRouter);
		if( pLMCE_Launch_Manager->m_bQuit_get()|| !pLMCE_Launch_Manager->GetConfig() )
		{
			delete pLMCE_Launch_Manager;
			return NULL;
		}
		else
		{
			g_pCommand_Impl=pLMCE_Launch_Manager;
			g_pDeadlockHandler=Plugin_DeadlockHandler;
			g_pSocketCrashHandler=Plugin_SocketCrashHandler;
		}
		return pLMCE_Launch_Manager;
	}
}
//<-dceag-plug-e->
...
int main(int argc, char* argv[]) 
{
	...
	if (bError)
	{
		cout << "A Pluto DCE Device.  See www.plutohome.com/dce for details." << endl
			<< "Usage: LMCE_Launch_Manager [-r Router's IP] [-d My Device ID] [-l dcerouter|stdout|null|filename]" << endl
	...
	try
	{
		if( sLogger=="dcerouter" )
			LoggerWrapper::SetInstance(new ServerLogger(PK_Device, LMCE_Launch_Manager::PK_DeviceTemplate_get_static(), sRouter_IP));
	...
	try
	{
		LMCE_Launch_Manager *pLMCE_Launch_Manager = new LMCE_Launch_Manager(PK_Device, sRouter_IP,true,bLocalMode);
		if ( pLMCE_Launch_Manager->GetConfig() && pLMCE_Launch_Manager->Connect(pLMCE_Launch_Manager->PK_DeviceTemplate_get()) ) 
		{
			g_pCommand_Impl=pLMCE_Launch_Manager;
			g_pDeadlockHandler=DeadlockHandler;
			g_pSocketCrashHandler=SocketCrashHandler;
			LoggerWrapper::GetInstance()->Write(LV_STATUS, "Connect OK");
			pLMCE_Launch_Manager->CreateChildren();
			if( bLocalMode )
				pLMCE_Launch_Manager->RunLocalMode();
			else
			{
				if(pLMCE_Launch_Manager->m_RequestHandlerThread)
					pthread_join(pLMCE_Launch_Manager->m_RequestHandlerThread, NULL);  // This function will return when the device is shutting down
			}
		...
		{
			bAppError = true;
			if( pLMCE_Launch_Manager->m_pEvent && pLMCE_Launch_Manager->m_pEvent->m_pClientSocket && pLMCE_Launch_Manager->m_pEvent->m_pClientSocket->m_eLastError==ClientSocket::cs_err_CannotConnect )
			{
				bAppError = false;
				bReload = false;
		...
		if( pLMCE_Launch_Manager->m_bReload )
			bReload=true;
