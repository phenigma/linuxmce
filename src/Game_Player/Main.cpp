...
extern "C" {
	...
	{ 
		...
		if( bIsRuntimePlugin )
			return Game_Player::PK_DeviceTemplate_get_static();
		else
			return 0;
	}
}
...
extern "C" {
	class Command_Impl *RegisterAsPlugIn(class Router *pRouter,int PK_Device,Logger *pPlutoLogger)
	{
		...
		Game_Player *pGame_Player = new Game_Player(PK_Device, "localhost",true,false,pRouter);
		if( pGame_Player->m_bQuit_get()|| !pGame_Player->GetConfig() )
		{
			delete pGame_Player;
			return NULL;
		}
...
int main(int argc, char* argv[]) 
{
	...
	cout << "Game_Player, v." << VERSION << endl
	...
	if (bError)
	{
		cout << "A Pluto DCE Device.  See www.plutohome.com/dce for details." << endl
			<< "Usage: Game_Player [-r Router's IP] [-d My Device ID] [-l dcerouter|stdout|null|filename]" << endl
	...
	try
	{
		if( sLogger=="dcerouter" )
			LoggerWrapper::SetInstance(new ServerLogger(PK_Device, Game_Player::PK_DeviceTemplate_get_static(), sRouter_IP));
	...
	try
	{
		Game_Player *pGame_Player = new Game_Player(PK_Device, sRouter_IP,true,bLocalMode);
		if ( pGame_Player->GetConfig() && pGame_Player->Connect(pGame_Player->PK_DeviceTemplate_get()) ) 
		{
			g_pCommand_Impl=pGame_Player;
			g_pDeadlockHandler=DeadlockHandler;
			g_pSocketCrashHandler=SocketCrashHandler;
			LoggerWrapper::GetInstance()->Write(LV_STATUS, "Connect OK");
			pGame_Player->CreateChildren();
			if( bLocalMode )
				pGame_Player->RunLocalMode();
			else
			{
				if(pGame_Player->m_RequestHandlerThread)
					pthread_join(pGame_Player->m_RequestHandlerThread, NULL);  // This function will return when the device is shutting down
			}
		...
		{
			bAppError = true;
			if( pGame_Player->m_pEvent && pGame_Player->m_pEvent->m_pClientSocket && pGame_Player->m_pEvent->m_pClientSocket->m_eLastError==ClientSocket::cs_err_CannotConnect )
			{
				bAppError = false;
				bReload = false;
