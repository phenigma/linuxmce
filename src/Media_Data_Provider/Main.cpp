...
extern "C" {
	...
	{ 
		...
		if( bIsRuntimePlugin )
			return Media_Data_Provider::PK_DeviceTemplate_get_static();
		else
			return 0;
	}
}
...
extern "C" {
	class Command_Impl *RegisterAsPlugIn(class Router *pRouter,int PK_Device,Logger *pPlutoLogger)
	{
		...
		Media_Data_Provider *pMedia_Data_Provider = new Media_Data_Provider(PK_Device, "localhost",true,false,pRouter);
		if( pMedia_Data_Provider->m_bQuit_get()|| !pMedia_Data_Provider->GetConfig() )
		{
			delete pMedia_Data_Provider;
			return NULL;
		}
		else
		{
			g_pCommand_Impl=pMedia_Data_Provider;
			g_pDeadlockHandler=Plugin_DeadlockHandler;
			g_pSocketCrashHandler=Plugin_SocketCrashHandler;
		}
		return pMedia_Data_Provider;
	}
}
//<-dceag-plug-e->
...
int main(int argc, char* argv[]) 
{
	...
	cout << "Media_Data_Provider, v." << VERSION << endl
	...
	if (bError)
	{
		cout << "A Pluto DCE Device.  See www.plutohome.com/dce for details." << endl
			<< "Usage: Media_Data_Provider [-r Router's IP] [-d My Device ID] [-l dcerouter|stdout|null|filename]" << endl
	...
	try
	{
		if( sLogger=="dcerouter" )
			LoggerWrapper::SetInstance(new ServerLogger(PK_Device, Media_Data_Provider::PK_DeviceTemplate_get_static(), sRouter_IP));
	...
	try
	{
		Media_Data_Provider *pMedia_Data_Provider = new Media_Data_Provider(PK_Device, sRouter_IP,true,bLocalMode);
		if ( pMedia_Data_Provider->GetConfig() && pMedia_Data_Provider->Connect(pMedia_Data_Provider->PK_DeviceTemplate_get()) ) 
		{
			g_pCommand_Impl=pMedia_Data_Provider;
			g_pDeadlockHandler=DeadlockHandler;
			g_pSocketCrashHandler=SocketCrashHandler;
			LoggerWrapper::GetInstance()->Write(LV_STATUS, "Connect OK");
			pMedia_Data_Provider->CreateChildren();
			if( bLocalMode )
				pMedia_Data_Provider->RunLocalMode();
			else
			{
				if(pMedia_Data_Provider->m_RequestHandlerThread)
					pthread_join(pMedia_Data_Provider->m_RequestHandlerThread, NULL);  // This function will return when the device is shutting down
			}
		...
		{
			bAppError = true;
			if( pMedia_Data_Provider->m_pEvent && pMedia_Data_Provider->m_pEvent->m_pClientSocket && pMedia_Data_Provider->m_pEvent->m_pClientSocket->m_eLastError==ClientSocket::cs_err_CannotConnect )
			{
				bAppError = false;
				bReload = false;
		...
		if( pMedia_Data_Provider->m_bReload )
			bReload=true;
