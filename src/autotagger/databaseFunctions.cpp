...
void databaseFunctions::updateSynopsis(QString synop, QString FileID, int attributeEnum)
{
    ...
    chkSynop.prepare("SELECT * FROM LongAttribute WHERE FK_AttributeType = 37 AND FK_File = (:fileNum) ");
    chkSynop.bindValue("(:fileNum)", FileID);
    chkSynop.exec();

    QSqlRecord synRec = chkSynop.record();
    QString synopResp = synRec.field("FK_File").value().toString();
   // qDebug () << chkSynop.boundValues ();
	...
	insert_synopsis.prepare("INSERT INTO LongAttribute (PK_LongAttribute, FK_AttributeType, FK_File, Text) VALUES (:longAttribute, :attribType , :fileNo, :data)");
	insert_synopsis.bindValue("(:longAttribute)", "");
	insert_synopsis.bindValue("(:attribType)", "37");
	insert_synopsis.bindValue("(:fileNo)", FileID);
	...
	update_synopsis.prepare("UPDATE LongAttribute WHERE FK_File = (:fkFile) AND FK_AttributeType=37 SET Text = (:data)");
	update_synopsis.bindValue("(:fileNo)", FileID);
	update_synopsis.bindValue("(:date)", synop);
