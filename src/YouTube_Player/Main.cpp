...
extern "C" {
	...
	{ 
		...
		if( bIsRuntimePlugin )
			return YouTube_Player::PK_DeviceTemplate_get_static();
		else
			return 0;
	}
}
...
extern "C" {
	class Command_Impl *RegisterAsPlugIn(class Router *pRouter,int PK_Device,Logger *pPlutoLogger)
	{
		...
		YouTube_Player *pYouTube_Player = new YouTube_Player(PK_Device, "localhost",true,false,pRouter);
		if( pYouTube_Player->m_bQuit_get()|| !pYouTube_Player->GetConfig() )
		{
			delete pYouTube_Player;
			return NULL;
		}
		else
		{
			g_pCommand_Impl=pYouTube_Player;
			g_pDeadlockHandler=Plugin_DeadlockHandler;
			g_pSocketCrashHandler=Plugin_SocketCrashHandler;
		}
		return pYouTube_Player;
	}
}
//<-dceag-plug-e->
...
int main(int argc, char* argv[]) 
{
	...
	cout << "YouTube_Player, v." << VERSION << endl
	...
	if (bError)
	{
		cout << "A Pluto DCE Device.  See www.plutohome.com/dce for details." << endl
			<< "Usage: YouTube_Player [-r Router's IP] [-d My Device ID] [-l dcerouter|stdout|null|filename]" << endl
	...
	try
	{
		if( sLogger=="dcerouter" )
			LoggerWrapper::SetInstance(new ServerLogger(PK_Device, YouTube_Player::PK_DeviceTemplate_get_static(), sRouter_IP));
	...
	try
	{
		YouTube_Player *pYouTube_Player = new YouTube_Player(PK_Device, sRouter_IP,true,bLocalMode);
		if ( pYouTube_Player->GetConfig() && pYouTube_Player->Connect(pYouTube_Player->PK_DeviceTemplate_get()) ) 
		{
			g_pCommand_Impl=pYouTube_Player;
			g_pDeadlockHandler=DeadlockHandler;
			g_pSocketCrashHandler=SocketCrashHandler;
			LoggerWrapper::GetInstance()->Write(LV_STATUS, "Connect OK");
			pYouTube_Player->CreateChildren();
			if( bLocalMode )
				pYouTube_Player->RunLocalMode();
			else
			{
				if(pYouTube_Player->m_RequestHandlerThread)
					pthread_join(pYouTube_Player->m_RequestHandlerThread, NULL);  // This function will return when the device is shutting down
			}
		...
		{
			bAppError = true;
			if( pYouTube_Player->m_pEvent && pYouTube_Player->m_pEvent->m_pClientSocket && pYouTube_Player->m_pEvent->m_pClientSocket->m_eLastError==ClientSocket::cs_err_CannotConnect )
			{
				bAppError = false;
				bReload = false;
