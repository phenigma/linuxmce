...
//<-dceag-incl-b->
#include "YouTube_Player/YouTube_Player.h"
...
int main(int argc, char* argv[]) 
{
  QApplication a(argc, argv);
	...
	YouTubeWindow *qYouTubeWindow = new YouTubeWindow(PK_Device);
	qYouTubeWindow->show();
    ...
    try
    {
        ...
        bAppError = qYouTubeWindow->setupLMCE(PK_Device);
            ...
            {
                    bAppError = true;
                    if( qYouTubeWindow->m_pBrowser->m_pEvent && qYouTubeWindow->m_pBrowser->m_pEvent->m_pClientSocket && qYouTubeWindow->m_pBrowser->m_pEvent->m_pClientSocket->m_eLastError==ClientSocket::cs_err_CannotConnect )
                    {
                            bAppError = false;
                            bReload = false;
            ...
            if( qYouTubeWindow->m_pBrowser->m_bReload )
                    bReload=true;
            delete qYouTubeWindow->m_pBrowser;
