...
extern "C" {
	...
	{ 
		...
		if( bIsRuntimePlugin )
			return Bluetooth_Adapter::PK_DeviceTemplate_get_static();
		else
			return 0;
	}
}
...
extern "C" {
	class Command_Impl *RegisterAsPlugIn(class Router *pRouter,int PK_Device,Logger *pPlutoLogger)
	{
		...
		Bluetooth_Adapter *pBluetooth_Adapter = new Bluetooth_Adapter(PK_Device, "localhost",true,false,pRouter);
		if( pBluetooth_Adapter->m_bQuit_get()|| !pBluetooth_Adapter->GetConfig() )
		{
			delete pBluetooth_Adapter;
			return NULL;
		}
		else
		{
			g_pCommand_Impl=pBluetooth_Adapter;
			g_pDeadlockHandler=Plugin_DeadlockHandler;
			g_pSocketCrashHandler=Plugin_SocketCrashHandler;
		}
		return pBluetooth_Adapter;
	}
}
//<-dceag-plug-e->
...
int main(int argc, char* argv[]) 
{
	...
	cout << "Bluetooth_Adapter, v." << VERSION << endl
	...
	if (bError)
	{
		cout << "A Pluto DCE Device.  See www.plutohome.com/dce for details." << endl
			<< "Usage: Bluetooth_Adapter [-r Router's IP] [-d My Device ID] [-l dcerouter|stdout|null|filename]" << endl
	...
	try
	{
		if( sLogger=="dcerouter" )
			LoggerWrapper::SetInstance(new ServerLogger(PK_Device, Bluetooth_Adapter::PK_DeviceTemplate_get_static(), sRouter_IP));
	...
	try
	{
		Bluetooth_Adapter *pBluetooth_Adapter = new Bluetooth_Adapter(PK_Device, sRouter_IP,true,bLocalMode);
		if ( pBluetooth_Adapter->GetConfig() && pBluetooth_Adapter->Connect(pBluetooth_Adapter->PK_DeviceTemplate_get()) ) 
		{
			g_pCommand_Impl=pBluetooth_Adapter;
			g_pDeadlockHandler=DeadlockHandler;
			g_pSocketCrashHandler=SocketCrashHandler;
			LoggerWrapper::GetInstance()->Write(LV_STATUS, "Connect OK");
			pBluetooth_Adapter->CreateChildren();
			if( bLocalMode )
				pBluetooth_Adapter->RunLocalMode();
			else
			{
				if(pBluetooth_Adapter->m_RequestHandlerThread)
					pthread_join(pBluetooth_Adapter->m_RequestHandlerThread, NULL);  // This function will return when the device is shutting down
			}
		...
		{
			bAppError = true;
			if( pBluetooth_Adapter->m_pEvent && pBluetooth_Adapter->m_pEvent->m_pClientSocket && pBluetooth_Adapter->m_pEvent->m_pClientSocket->m_eLastError==ClientSocket::cs_err_CannotConnect )
			{
				bAppError = false;
				bReload = false;
		...
		if( pBluetooth_Adapter->m_bReload )
			bReload=true;
