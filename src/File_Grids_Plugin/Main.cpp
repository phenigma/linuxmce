...
int main(int argc, char* argv[]) 
{
	...
	if (bError)
	{
		cout << "A Pluto DCE Device.  See www.plutohome.com/dce for details." << endl
			<< "Usage: File_Grids_Plugin [-r Router's IP] [-d My Device ID] [-l dcerouter|stdout|null|filename]" << endl
	...
	try
	{
		if( sLogger=="dcerouter" )
			LoggerWrapper::SetInstance(new ServerLogger(PK_Device, File_Grids_Plugin::PK_DeviceTemplate_get_static(), sRouter_IP));
	...
	try
	{
		File_Grids_Plugin *pFile_Grids_Plugin = new File_Grids_Plugin(PK_Device, sRouter_IP,true,bLocalMode);
		if ( pFile_Grids_Plugin->GetConfig() && pFile_Grids_Plugin->Connect(pFile_Grids_Plugin->PK_DeviceTemplate_get()) ) 
		{
			...
			if( bLocalMode )
				pFile_Grids_Plugin->RunLocalMode();
			else
			{
				if(pFile_Grids_Plugin->m_RequestHandlerThread)
					pthread_join(pFile_Grids_Plugin->m_RequestHandlerThread, NULL);  // This function will return when the device is shutting down
			}
		...
		{
			bAppError = true;
			if( pFile_Grids_Plugin->m_pEvent && pFile_Grids_Plugin->m_pEvent->m_pClientSocket && pFile_Grids_Plugin->m_pEvent->m_pClientSocket->m_eLastError==ClientSocket::cs_err_CannotConnect )
			{
				bAppError = false;
				bReload = false;
		...
		if( pFile_Grids_Plugin->m_bReload )
			bReload=true;
