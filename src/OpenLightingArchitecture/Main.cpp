...
extern "C" {
	...
	{ 
		...
		if( bIsRuntimePlugin )
			return OpenLightingArchitecture::PK_DeviceTemplate_get_static();
		else
			return 0;
	}
}
...
extern "C" {
	class Command_Impl *RegisterAsPlugIn(class Router *pRouter,int PK_Device,Logger *pPlutoLogger)
	{
		...
		OpenLightingArchitecture *pOpenLightingArchitecture = new OpenLightingArchitecture(PK_Device, "localhost",true,false,pRouter);
		if( pOpenLightingArchitecture->m_bQuit_get()|| !pOpenLightingArchitecture->GetConfig() )
		{
			delete pOpenLightingArchitecture;
			return NULL;
		}
		else
		{
			g_pCommand_Impl=pOpenLightingArchitecture;
			g_pDeadlockHandler=Plugin_DeadlockHandler;
			g_pSocketCrashHandler=Plugin_SocketCrashHandler;
		}
		return pOpenLightingArchitecture;
	}
}
//<-dceag-plug-e->
...
int main(int argc, char* argv[]) 
{
	...
	cout << "OpenLightingArchitecture, v." << VERSION << endl
	...
	if (bError)
	{
		cout << "A Pluto DCE Device.  See www.plutohome.com/dce for details." << endl
			<< "Usage: OpenLightingArchitecture [-r Router's IP] [-d My Device ID] [-l dcerouter|stdout|null|filename]" << endl
	...
	try
	{
		if( sLogger=="dcerouter" )
			LoggerWrapper::SetInstance(new ServerLogger(PK_Device, OpenLightingArchitecture::PK_DeviceTemplate_get_static(), sRouter_IP));
	...
	try
	{
		OpenLightingArchitecture *pOpenLightingArchitecture = new OpenLightingArchitecture(PK_Device, sRouter_IP,true,bLocalMode);
		if ( pOpenLightingArchitecture->GetConfig() && pOpenLightingArchitecture->Connect(pOpenLightingArchitecture->PK_DeviceTemplate_get()) ) 
		{
			g_pCommand_Impl=pOpenLightingArchitecture;
			g_pDeadlockHandler=DeadlockHandler;
			g_pSocketCrashHandler=SocketCrashHandler;
			LoggerWrapper::GetInstance()->Write(LV_STATUS, "Connect OK");
			pOpenLightingArchitecture->CreateChildren();
			if( bLocalMode )
				pOpenLightingArchitecture->RunLocalMode();
			else
			{
				if(pOpenLightingArchitecture->m_RequestHandlerThread)
					pthread_join(pOpenLightingArchitecture->m_RequestHandlerThread, NULL);  // This function will return when the device is shutting down
			}
		...
		{
			bAppError = true;
			if( pOpenLightingArchitecture->m_pEvent && pOpenLightingArchitecture->m_pEvent->m_pClientSocket && pOpenLightingArchitecture->m_pEvent->m_pClientSocket->m_eLastError==ClientSocket::cs_err_CannotConnect )
			{
				bAppError = false;
				bReload = false;
		...
		if( pOpenLightingArchitecture->m_bReload )
			bReload=true;
