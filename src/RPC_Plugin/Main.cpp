...
extern "C" {
	...
	{ 
		...
		if( bIsRuntimePlugin )
			return RPC_Plugin::PK_DeviceTemplate_get_static();
		else
			return 0;
	}
}
...
extern "C" {
	class Command_Impl *RegisterAsPlugIn(class Router *pRouter,int PK_Device,Logger *pPlutoLogger)
	{
		...
		RPC_Plugin *pRPC_Plugin = new RPC_Plugin(PK_Device, "localhost",true,false,pRouter);
		if( pRPC_Plugin->m_bQuit_get()|| !pRPC_Plugin->GetConfig() )
		{
			delete pRPC_Plugin;
			return NULL;
		}
...
int main(int argc, char* argv[]) 
{
	...
	cout << "RPC_Plugin, v." << VERSION << endl
	...
	if (bError)
	{
		cout << "A Pluto DCE Device.  See www.plutohome.com/dce for details." << endl
			<< "Usage: RPC_Plugin [-r Router's IP] [-d My Device ID] [-l dcerouter|stdout|null|filename]" << endl
	...
	try
	{
		if( sLogger=="dcerouter" )
			LoggerWrapper::SetInstance(new ServerLogger(PK_Device, RPC_Plugin::PK_DeviceTemplate_get_static(), sRouter_IP));
	...
	try
	{
		RPC_Plugin *pRPC_Plugin = new RPC_Plugin(PK_Device, sRouter_IP,true,bLocalMode);
		if ( pRPC_Plugin->GetConfig() && pRPC_Plugin->Connect(pRPC_Plugin->PK_DeviceTemplate_get()) ) 
		{
			g_pCommand_Impl=pRPC_Plugin;
			g_pDeadlockHandler=DeadlockHandler;
			g_pSocketCrashHandler=SocketCrashHandler;
			LoggerWrapper::GetInstance()->Write(LV_STATUS, "Connect OK");
			pRPC_Plugin->CreateChildren();
			if( bLocalMode )
				pRPC_Plugin->RunLocalMode();
			else
			{
				if(pRPC_Plugin->m_RequestHandlerThread)
					pthread_join(pRPC_Plugin->m_RequestHandlerThread, NULL);  // This function will return when the device is shutting down
			}
		...
		{
			bAppError = true;
			if( pRPC_Plugin->m_pEvent && pRPC_Plugin->m_pEvent->m_pClientSocket && pRPC_Plugin->m_pEvent->m_pClientSocket->m_eLastError==ClientSocket::cs_err_CannotConnect )
			{
				bAppError = false;
				bReload = false;
