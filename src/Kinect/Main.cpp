...
extern "C" {
	...
	{ 
		...
		if( bIsRuntimePlugin )
			return Kinect::PK_DeviceTemplate_get_static();
		else
			return 0;
	}
}
...
extern "C" {
	class Command_Impl *RegisterAsPlugIn(class Router *pRouter,int PK_Device,Logger *pPlutoLogger)
	{
		...
		Kinect *pKinect = new Kinect(PK_Device, "localhost",true,false,pRouter);
		if( pKinect->m_bQuit_get()|| !pKinect->GetConfig() )
		{
			delete pKinect;
			return NULL;
		}
...
int main(int argc, char* argv[]) 
{
	...
	if (bError)
	{
		cout << "A Pluto DCE Device.  See www.plutohome.com/dce for details." << endl
			<< "Usage: Kinect [-r Router's IP] [-d My Device ID] [-l dcerouter|stdout|null|filename]" << endl
	...
	try
	{
		if( sLogger=="dcerouter" )
			LoggerWrapper::SetInstance(new ServerLogger(PK_Device, Kinect::PK_DeviceTemplate_get_static(), sRouter_IP));
	...
	try
	{
		Kinect *pKinect = new Kinect(PK_Device, sRouter_IP,true,bLocalMode);
		if ( pKinect->GetConfig() && pKinect->Connect(pKinect->PK_DeviceTemplate_get()) ) 
		{
			g_pCommand_Impl=pKinect;
			g_pDeadlockHandler=DeadlockHandler;
			g_pSocketCrashHandler=SocketCrashHandler;
			LoggerWrapper::GetInstance()->Write(LV_STATUS, "Connect OK");
			pKinect->CreateChildren();
			if( bLocalMode )
				pKinect->RunLocalMode();
			else
			{
				if(pKinect->m_RequestHandlerThread)
					pthread_join(pKinect->m_RequestHandlerThread, NULL);  // This function will return when the device is shutting down
			}
		...
		{
			bAppError = true;
			if( pKinect->m_pEvent && pKinect->m_pEvent->m_pClientSocket && pKinect->m_pEvent->m_pClientSocket->m_eLastError==ClientSocket::cs_err_CannotConnect )
			{
				bAppError = false;
				bReload = false;
