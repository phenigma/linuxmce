...
#ifndef __Table_EPG_H__
#define __Table_EPG_H__

#include "TableRow.h"
#include "Database_xxv.h"
#include "PlutoUtils/MultiThreadIncludes.h"
#include "Define_EPG.h"
...
public:
	Table_EPG(Database_xxv *pDatabase):database(pDatabase)
	{
	};
	~Table_EPG();

private:		
	friend class Row_EPG;
	struct Key
	{
		friend class Row_EPG;
...
public:				
	...
	bool GetRows(string where_statement,vector<class Row_EPG*> *rows);
	class Row_EPG* AddRow();
	Database_xxv *Database_xxv_get() { return database; }
	
		
	class Row_EPG* GetRow(u_int64_t in_eventid);
...
class DECLSPECIFIER Row_EPG : public TableRow, public SerializeClass
	{
		friend struct Table_EPG::Key;
		friend class Table_EPG;
	private:
		Table_EPG *table;
...
void vpstime_setNull(bool val);
		...
		Row_EPG(Table_EPG *pTable);
	
		bool IsDeleted(){return is_deleted;};
		bool IsModified(){return is_modified;};			
		class Table_EPG *Table_EPG_get() { return table; };
