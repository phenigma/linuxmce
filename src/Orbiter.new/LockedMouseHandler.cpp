	...
	: MouseHandler(pObj,sOptions,pMouseBehavior)
{
	...
	{
		DesignObj_Orbiter *pObj_First = m_pMouseBehavior->GetOrbiter()->FindObject(
		...
		if( pObj_First )
		{
			...
			PLUTO_SAFETY_LOCK( cm, m_pMouseBehavior->GetOrbiter()->m_ScreenMutex );  // Protect the highlighed object
			m_pObj_Highlighted=m_pMouseBehavior->GetOrbiter()->m_pObj_Highlighted=pObj_First;
...
void LockedMouseHandler::Start()
{
	...
	if( m_sOptions[0]=='H' )
		m_pMouseBehavior->SetMousePosition(m_pMouseBehavior->GetOrbiter()->m_iImageWidth/2,m_pMouseBehavior->GetOrbiter()->m_iImageHeight/2);
}
	...
	if( PK_Button==BUTTON_Mouse_1_CONST )
	{
		...
		else
		{
			...
			if( m_sOptions=="LS" )
				...
				m_pMouseBehavior->GetOrbiter()->CMD_Simulate_Keypress(StringUtils::ltos(BUTTON_Enter_CONST), "");
			return true; // Don't process any more
		}
	}
...
bool LockedMouseHandler::ButtonUp(int PK_Button)
{
	...
	if( PK_Button==BUTTON_Mouse_7_CONST && m_bTapAndRelease==false && m_bStartedMovement )
	{
		...
		if( m_sOptions[0]=='M' )
			...
			ActivatedSubMenu();
		else if( m_pMouseBehavior->GetOrbiter()->m_pObj_Highlighted && !m_pMouseBehavior->GetOrbiter()->m_pObj_Highlighted->IsHidden(  )  )
		{
			m_bActivatedObject = true;
			m_pMouseBehavior->GetOrbiter()->SelectedObject( m_pMouseBehavior->GetOrbiter()->m_pObj_Highlighted, smNavigation );
		}
		else
...
{
	if( m_PK_Direction_Last && PK_Direction!=m_PK_Direction_Last && m_pMouseBehavior->GetOrbiter()->m_pObj_Highlighted )
	{
		// The user is reversing direction.  Recenter so we don't switch back to easily, to give that 'pop' feel to the button
		int X = m_pMouseBehavior->GetOrbiter()->m_pObj_Highlighted->m_rPosition.X + m_pMouseBehavior->GetOrbiter()->m_pObj_Highlighted->m_pPopupPoint.X + m_pMouseBehavior->GetOrbiter()->m_pObj_Highlighted->m_rPosition.Width/2;
		int Y = m_pMouseBehavior->GetOrbiter()->m_pObj_Highlighted->m_rPosition.Y + m_pMouseBehavior->GetOrbiter()->m_pObj_Highlighted->m_pPopupPoint.Y + m_pMouseBehavior->GetOrbiter()->m_pObj_Highlighted->m_rPosition.Height/2;
...
//X,Y,m_pMouseBehavior->m_pSamples[0].X,m_pMouseBehavior->m_pSamples[0].Y,m_pMouseBehavior->m_pStartMovement.X,m_pMouseBehavior->m_pStartMovement.Y,(int) m_pMouseBehavior->m_cLockedAxes,(int) m_pMouseBehavior->m_cLocked_Axis_Current,(int) m_pMouseBehavior->m_iLockedPosition);
	...
	PLUTO_SAFETY_LOCK( cm, m_pMouseBehavior->GetOrbiter()->m_ScreenMutex );  // Protect the highlighed object
//	if( m_pMouseBehavior->GetOrbiter()->m_pObj_Highlighted && 
//		( (m_pMouseBehavior->GetOrbiter()->m_pObj_Highlighted->m_rPosition+m_pMouseBehavior->GetOrbiter()->m_pObj_Highlighted->m_pPopupPoint).Contains(X,Y)==false || m_pMouseBehavior->GetOrbiter()->m_pObj_Highlighted->m_ChildObjects.size()) )
	if( !m_pMouseBehavior->GetOrbiter()->m_pObj_Highlighted || 
		(m_pMouseBehavior->GetOrbiter()->m_pObj_Highlighted->m_rPosition+m_pMouseBehavior->GetOrbiter()->m_pObj_Highlighted->m_pPopupPoint).Contains(X,Y)==false )
	{
		...
		if( pObj_ToHighlight && pObj_ToHighlight!=m_pMouseBehavior->GetOrbiter()->m_pObj_Highlighted )
		{
			if( m_sOptions[0]=='H' )
			{
				NeedToRender render( m_pMouseBehavior->GetOrbiter(), "LockedMouseHandler::Move" );
				m_bActivatedObject = true;
				m_pMouseBehavior->GetOrbiter()->SelectedObject( pObj_ToHighlight, smNavigation );
			}
			else
			{
				m_pObj_Highlighted = m_pMouseBehavior->GetOrbiter()->m_pObj_Highlighted = pObj_ToHighlight;
				m_pMouseBehavior->HighlightObject(m_pObj_Highlighted);
			}
		}
	}
...
void LockedMouseHandler::Notch(int PK_Direction,int iRepeat)
{
	...
	if( m_pObj->m_iBaseObjectID!=DESIGNOBJ_popMainMenu_CONST )
		return; 
NeedToRender render( m_pMouseBehavior->GetOrbiter(), "LockedMouseHandler::Notch" );
	PLUTO_SAFETY_LOCK( cm, m_pMouseBehavior->GetOrbiter()->m_ScreenMutex );  // Protect the highlighed object
	for(int i=0;i<iRepeat;++i)
		m_pMouseBehavior->GetOrbiter()->HighlightNextObject(PK_Direction);
...
void LockedMouseHandler::ActivatedMainMenuPad()
{
	...
	string sSubMenu;
	switch( m_pMouseBehavior->GetOrbiter()->m_pObj_Highlighted->m_iBaseObjectID )
	{
	case DESIGNOBJ_butCurrentMedia_CONST:
		sSubMenu = StringUtils::itos(m_pMouseBehavior->GetOrbiter()->m_iPK_DesignObj_Remote_Popup);
		break;
	case 4954:  // climate
		sSubMenu = "4957.<%=L:0%>.0";
		break;
	...
	DesignObj_Orbiter *pObj = m_pMouseBehavior->GetOrbiter()->FindObject(sSubMenu);
	if( !pObj )
	{
	...
	PlutoPoint pt = m_pMouseBehavior->GetOrbiter()->m_pObj_Highlighted->m_rPosition.Location();
	pt.Y -= pObj->m_rPosition.Height;
	if( pt.X + pObj->m_rPosition.Width > m_pMouseBehavior->GetOrbiter()->m_Width )
		pt.X = m_pMouseBehavior->GetOrbiter()->m_Width-pObj->m_rPosition.Width;

	m_pMouseBehavior->GetOrbiter()->CMD_Show_Popup(pObj->m_ObjectID,pt.X,pt.Y,"","submenu",false,false);
...
bool LockedMouseHandler::SlowDrift(int &X,int &Y)
{
	if( m_pMouseBehavior->GetOrbiter()->m_pObj_Highlighted )
	{
		X = m_pMouseBehavior->GetOrbiter()->m_pObj_Highlighted->m_rPosition.X + m_pMouseBehavior->GetOrbiter()->m_pObj_Highlighted->m_pPopupPoint.X + m_pMouseBehavior->GetOrbiter()->m_pObj_Highlighted->m_rPosition.Width/2;
		Y = m_pMouseBehavior->GetOrbiter()->m_pObj_Highlighted->m_rPosition.Y + m_pMouseBehavior->GetOrbiter()->m_pObj_Highlighted->m_pPopupPoint.Y + m_pMouseBehavior->GetOrbiter()->m_pObj_Highlighted->m_rPosition.Height/2;
		m_pMouseBehavior->SetMousePosition(X,Y);
		return true;
	}
	return false;
...
bool LockedMouseHandler::MovedOutside(int PK_Direction)
{
	if( m_sOptions.size()>1 &&  m_sOptions[0]=='L' && m_sOptions[1]=='S' && PK_Direction==DIRECTION_Down_CONST )
	{
		...
		m_pMouseBehavior->GetOrbiter()->CMD_Remove_Popup("","submenu");
...
{
	NeedToRender render( m_pMouseBehavior->GetOrbiter(), "LockedMouseHandler::ActivatedSubMenu" );
	...
	DesignObj_Orbiter *pObj_Screen_Before = m_pMouseBehavior->GetOrbiter()->m_pScreenHistory_Current->GetObj();
	if( m_pMouseBehavior->GetOrbiter()->m_pObj_Highlighted && !m_pMouseBehavior->GetOrbiter()->m_pObj_Highlighted->IsHidden(  )  )
	{
		m_bActivatedObject = true;
		m_pMouseBehavior->GetOrbiter()->SelectedObject( m_pMouseBehavior->GetOrbiter()->m_pObj_Highlighted, smNavigation );
	}
	if( pObj_Screen_Before==m_pMouseBehavior->GetOrbiter()->m_pScreenHistory_Current->GetObj() && render.m_pScreenHistory_get()==NULL )  // See if the selection commands didn't already result in a change screen
	{
		m_pMouseBehavior->GetOrbiter()->CMD_Goto_Screen("",SCREEN_Main_CONST);
		...
		g_pPlutoLogger->Write(LV_CRITICAL,"LockedMouseHandler::ActivatedSubMenu already changed from %s to %s",
			pObj_Screen_Before->m_ObjectID.c_str(),m_pMouseBehavior->GetOrbiter()->m_pScreenHistory_Current->GetObj()->m_ObjectID.c_str());
