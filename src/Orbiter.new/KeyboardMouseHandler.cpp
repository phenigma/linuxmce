...
void KeyboardMouseHandler::Start()
{
	...
	if( !m_pObj )
		m_pMouseBehavior->SetMousePosition(m_pMouseBehavior->GetOrbiter()->m_Width/2,
			m_pMouseBehavior->GetOrbiter()->m_Height/2);
	else
	{
		if ( !m_pMouseBehavior->GetOrbiter()->m_pObj_Highlighted )
		{
			...
			if( m_pMouseBehavior->GetOrbiter()->m_pObj_Highlighted )
				m_pMouseBehavior->HighlightObject(m_pMouseBehavior->GetOrbiter()->m_pObj_Highlighted);
		}
		if( m_pMouseBehavior->GetOrbiter()->m_pObj_Highlighted )
		{
	...
	if( m_pMouseBehavior->GetOrbiter()->m_pObj_Highlighted && m_pMouseBehavior->GetOrbiter()->m_pObj_Highlighted->m_ObjectType==DESIGNOBJTYPE_Datagrid_CONST )
		m_spDatagridMouseHandlerHelper->Start( (DesignObj_DataGrid *) m_pMouseBehavior->GetOrbiter()->m_pObj_Highlighted);
}
...
NeedToRender render( m_pMouseBehavior->GetOrbiter(), "change k/b" );
PLUTO_SAFETY_LOCK(nd,m_pMouseBehavior->GetOrbiter()->m_NeedRedrawVarMutex);
m_pMouseBehavior->GetOrbiter()->m_vectObjs_NeedRedraw.push_back(m_pObj);
	}
}
...
bool KeyboardMouseHandler::ButtonDown(int PK_Button)
{
	if( PK_Button==BUTTON_Mouse_1_CONST )
	{
		if( m_pObj )
		{
			m_pMouseBehavior->GetOrbiter()->CMD_Simulate_Keypress(StringUtils::itos(BUTTON_Enter_CONST),"");
		}
		else
		{
			DCE::CMD_Simulate_Keypress CMD_Simulate_Keypress(m_pMouseBehavior->GetOrbiter()->m_dwPK_Device,m_pMouseBehavior->GetOrbiter()->m_dwPK_Device_NowPlaying,
				StringUtils::itos(BUTTON_Enter_CONST),"");
			m_pMouseBehavior->GetOrbiter()->SendCommand(CMD_Simulate_Keypress);
		}
	}
...
{
	if( !m_pObj && !m_spDatagridMouseHandlerHelper->m_dwPK_Direction_ScrollGrid && m_PK_Direction_Last && PK_Direction!=m_PK_Direction_Last && m_pMouseBehavior->GetOrbiter()->m_pObj_Highlighted )
	{
...
g_pPlutoLogger->Write(LV_FESTIVAL,"xxKeyboardMouseHandler::Move away from highlight %d at %d,%d",PK_Direction,X,Y);
		DesignObj_Orbiter *pObj_Before = m_pMouseBehavior->GetOrbiter()->m_pObj_Highlighted;
		...
		if( !m_pMouseBehavior->GetOrbiter()->HighlightNextObject(PK_Direction) )
		{
/*
		...
		if( m_pMouseBehavior->GetOrbiter()->m_pObj_Highlighted && 
			pObj_Before!=m_pMouseBehavior->GetOrbiter()->m_pObj_Highlighted )
		{
g_pPlutoLogger->Write(LV_FESTIVAL,"KeyboardMouseHandler::Move was %s now %s after direction %d",
					  pObj_Before->m_ObjectID.c_str(),m_pMouseBehavior->GetOrbiter()->m_pObj_Highlighted->m_ObjectID.c_str(),PK_Direction);
			m_pMouseBehavior->HighlightObject(m_pMouseBehavior->GetOrbiter()->m_pObj_Highlighted);

//			m_pMouseBehavior->PositionMouseAtObjectEdge(PK_Direction);
			PlutoRectangle rect = m_pMouseBehavior->GetHighlighedObjectCoordinates();

			if( m_pMouseBehavior->GetOrbiter()->m_pObj_Highlighted->m_ObjectType==DESIGNOBJTYPE_Datagrid_CONST )
				m_spDatagridMouseHandlerHelper->Start( (DesignObj_DataGrid *) m_pMouseBehavior->GetOrbiter()->m_pObj_Highlighted);
		}
	}
}
...
{
	/*
	int NotchSize = bHorizontal ? m_pMouseBehavior->GetOrbiter()->m_Width / m_pMouseBehavior->m_MouseSensitivity.NumNotchesForExternalApp : m_pMouseBehavior->GetOrbiter()->m_Height / m_pMouseBehavior->m_MouseSensitivity.NumNotchesForExternalApp;
	int Diff = bHorizontal ? X-m_pMouseBehavior->GetOrbiter()->m_Width/2 : Y-m_pMouseBehavior->GetOrbiter()->m_Height/2;
...
void KeyboardMouseHandler::MoveExternalApp(int X,int Y)
{
	...
	int NotchSize = (double) bHorizontal ? m_pMouseBehavior->GetOrbiter()->m_Width / 20 : m_pMouseBehavior->GetOrbiter()->m_Height / 20;
	int Notch;
	if( bHorizontal )
...
void KeyboardMouseHandler::IterateObject()
{
	switch( m_cDirection )
	{
		case 'U':
		{
			m_pMouseBehavior->GetOrbiter()->CMD_Simulate_Keypress(
...
{
	DesignObj_Orbiter *pObj = m_pMouseBehavior->GetOrbiter()->m_pObj_Highlighted;
	...
	m_pMouseBehavior->GetOrbiter()->Renderer()->HollowRectangle(
		...
		PlutoColor::White());
	m_pMouseBehavior->GetOrbiter()->Renderer()->UpdateRect(m_spDatagridMouseHandlerHelper->m_pObj_MediaBrowser_Alpha->m_rPosition + m_spDatagridMouseHandlerHelper->m_pObj_MediaBrowser_Alpha->m_pPopupPoint);
