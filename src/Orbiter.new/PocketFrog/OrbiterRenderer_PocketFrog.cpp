...
#endif
//-----------------------------------------------------------------------------------------------------
#define CHECK_STATUS() { if(GetOrbiter()->m_bQuit) return; }
//-----------------------------------------------------------------------------------------------------
OrbiterRenderer_PocketFrog *OrbiterRenderer_PocketFrog::m_pInstance = NULL; //the one and only
...
{
	OrbiterRenderer_PocketFrog *pRenderer = (OrbiterRenderer_PocketFrog *)p;
...
//-----------------------------------------------------------------------------------------------------
OrbiterRenderer_PocketFrog::OrbiterRenderer_PocketFrog(bool bFullScreen, bool bUseOpenGL, Orbiter *pOrbiter) : OrbiterRenderer(pOrbiter)	
{
#ifdef ENABLE_MOUSE_BEHAVIOR
	GetOrbiter()->m_pMouseBehavior = new MouseBehavior_Win32(GetOrbiter());
#endif
	...
	GetOrbiter()->m_bConnectionLost = false;
	GetOrbiter()->m_bReload = false;
	GetOrbiter()->m_bQuit = false;
...
//-----------------------------------------------------------------------------------------------------
/*virtual*/ OrbiterRenderer_PocketFrog::~OrbiterRenderer_PocketFrog()
{
	m_Desktop=NULL;
#ifndef WINCE
...
//-----------------------------------------------------------------------------------------------------
/*virtual*/ bool OrbiterRenderer_PocketFrog::GameInit()
{
	...
	if(pLogoSurface) 
	{
        if(pLogoSurface->m_height == GetOrbiter()->m_iImageHeight && pLogoSurface->m_width == GetOrbiter()->m_iImageWidth)
		    GetDisplay()->Blit( 0, 0, pLogoSurface );
        else //zoom
        {
            Rect dest;	
            dest.Set(0, 0, GetOrbiter()->m_iImageWidth, GetOrbiter()->m_iImageHeight);

            double ZoomX = GetOrbiter()->m_iImageWidth / double(pLogoSurface->GetWidth());
            double ZoomY = GetOrbiter()->m_iImageHeight / double(pLogoSurface->GetHeight());
	...
	{
		GetDisplay()->FillRect(0, 0, GetOrbiter()->m_iImageWidth, GetOrbiter()->m_iImageHeight, 0x00);
	}
	...
	if(GetOrbiter()->m_bQuit || GetOrbiter()->m_bReload)
    {
	...
	if (!GetOrbiter()->m_bLocalMode)
		GetOrbiter()->CreateChildren();
...
//-----------------------------------------------------------------------------------------------------
/*virtual*/ void OrbiterRenderer_PocketFrog::ShowWindowCustom()
{
#ifdef WINCE
...
//-----------------------------------------------------------------------------------------------------
/*virtual*/ void OrbiterRenderer_PocketFrog::GameEnd()
{
	
}
//-----------------------------------------------------------------------------------------------------
/*virtual*/ void OrbiterRenderer_PocketFrog::GameLoop()
{
}
//-----------------------------------------------------------------------------------------------------
/*virtual*/ void OrbiterRenderer_PocketFrog::StylusDown( Point p, MouseButton aMouseButton )
{
#ifdef DEBUG
	...
	GetOrbiter()->RegionDown(p.x, p.y);
	RecordMouseAction(p.x, p.y);
}
//-----------------------------------------------------------------------------------------------------
/*virtual*/ void OrbiterRenderer_PocketFrog::StylusUp( Point p, MouseButton aMouseButton )
{
#ifdef DEBUG
	...
	GetOrbiter()->RegionUp(p.x, p.y);
}
//-----------------------------------------------------------------------------------------------------
/*virtual*/ void OrbiterRenderer_PocketFrog::StylusMove( Point p )
{
#ifdef ENABLE_MOUSE_BEHAVIOR
	if(GetOrbiter()->UsesUIVersion2())
	{
		GetOrbiter()->m_pMouseBehavior->Move(p.x, p.y);
	}
#endif
}
//-----------------------------------------------------------------------------------------------------
/*virtual*/ void OrbiterRenderer_PocketFrog::StylusWheel( Point p, int delta)
{
#ifdef DEBUG
...
//-----------------------------------------------------------------------------------------------------
/*virtual*/ bool OrbiterRenderer_PocketFrog::PocketFrogButtonDown(int button)
{
	HandleKeyEvents(WM_KEYDOWN, button, 0L);
	return true;
}
//-----------------------------------------------------------------------------------------------------
/*virtual*/ bool OrbiterRenderer_PocketFrog::PocketFrogButtonUp(int button)
{
	HandleKeyEvents(WM_KEYUP, button, 0L);
	return true;
}
//-----------------------------------------------------------------------------------------------------
/*virtual*/ void OrbiterRenderer_PocketFrog::HandleKeyEvents(UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    if(uMsg == WM_KEYDOWN)
	{
		...
		g_pPlutoLogger->Write(LV_STATUS,"Key down %d bControlDown %d bAltDown %d bCapsLock %d bShiftDown %d",
			wParam,(int) GetOrbiter()->m_bControlDown,(int) GetOrbiter()->m_bAltDown,(int) GetOrbiter()->m_bCapsLock,(int) GetOrbiter()->m_bShiftDown);
#endif
	}
    ...
    if(!TranslateVirtualKeys2PlutoKeys(uMsg, wParam, lParam, orbiterEvent.data.button.m_iPK_Button, 
        GetOrbiter()->m_bShiftDown, GetOrbiter()->m_bAltDown, GetOrbiter()->m_bControlDown, GetOrbiter()->m_bCapsLock)
    )
    ...
    else if( wParam == VK_D && GetOrbiter()->m_bShiftDown && GetOrbiter()->m_bControlDown)
    {
        DumpLocks();
    }
	else if( wParam == VK_P && GetOrbiter()->m_bShiftDown && GetOrbiter()->m_bControlDown && uMsg == WM_KEYUP)
	{
#ifdef DEBUG
		GetOrbiter()->DumpScreenHistory();
#endif
	}
    else if( wParam == VK_P && GetOrbiter()->m_bControlDown && uMsg == WM_KEYUP)
    {
        static int Count = 0;
        GetOrbiter()->m_bControlDown=false;
...
//-----------------------------------------------------------------------------------------------------
/*virtual*/ void OrbiterRenderer_PocketFrog::SolidRectangle(int x, int y, int width, int height, PlutoColor color)
{
...
//-----------------------------------------------------------------------------------------------------
/*virtual*/ void OrbiterRenderer_PocketFrog::HollowRectangle(int x, int y, int width, int height, PlutoColor color)
{
	GetOrbiter()->ClipRectangle(x, y, width, height);
	...
	PlutoColor ColorToReplace, PlutoColor ReplacementColor)
{
    ...
    for (int j = 0; j < height; j++)
    {
        ...
        {
            if(i + x >= 0 && j + y >= 0 && i + x < GetOrbiter()->m_iImageWidth && j + y < GetOrbiter()->m_iImageHeight)
            {
...
//-----------------------------------------------------------------------------------------------------
/*virtual*/ void OrbiterRenderer_PocketFrog::RenderText(string &TextToDisplay,class DesignObjText *Text,class TextStyle *pTextStyle, PlutoPoint point)
{
    CHECK_STATUS();
    PLUTO_SAFETY_LOCK(cm, GetOrbiter()->m_ScreenMutex);
        ...
        {   
            ...
            for(it = vectTextPieces.begin(); it != vectTextPieces.end(); it++)
            {
                ...
                if(nPos == 0)
                {
                    ...
                    pPieceTextStyle = GetOrbiter()->m_mapTextStyle_Find(nTextStyle);
...
/*virtual*/ void OrbiterRenderer_PocketFrog::RenderText(HDC hdc,string &TextToDisplay,PlutoRectangle &rPosition,int iPK_HorizAlignment,int iPK_VertAlignment,string &sFont,PlutoColor &ForeColor,int iPixelHeight,bool bBold,bool bItalic,bool bUnderline, PlutoPoint point, RECT &rectCalcLocation)
{
	CHECK_STATUS();
	PLUTO_SAFETY_LOCK(cm, GetOrbiter()->m_ScreenMutex);
...
/*	MS' Gdi doesn't seem to work, we have to hack in a work around
	if( GetOrbiter()->m_iRotation )
	{
		//use this to rotate text
		lf.lfOrientation = (360 - GetOrbiter()->m_iRotation) * 10;
		lf.lfEscapement = (360 - GetOrbiter()->m_iRotation) * 10;
	}
*/
#ifdef WINCE
	...
	CalcTextRectangle(rectLocation,rPosition,GetOrbiter()->m_iRotation,iRealHeight,iPK_VertAlignment);
	...
	if( hdc_drawing!=hdc )
	{
		for(int x=0;x<(GetOrbiter()->m_iRotation==180 ? rPosition.Width : rPosition.Height);x++)
		{
			for(int y=0;y<(GetOrbiter()->m_iRotation==180 ? rPosition.Height: rPosition.Width);y++)
			{
				COLORREF c = ::GetPixel(hdc_drawing,x,y);
				if( c==132 || c==128 )
					continue;  // Unchanged
				if( GetOrbiter()->m_iRotation==90 )
					::SetPixel(hdc,rPosition.Right()-y,rPosition.Y+x,c);
				else if( GetOrbiter()->m_iRotation==180 )
					::SetPixel(hdc,rPosition.Right()-x,rPosition.Bottom()-y,c);
				else if( GetOrbiter()->m_iRotation==270 )
...
//-----------------------------------------------------------------------------------------------------
/*virtual*/ void OrbiterRenderer_PocketFrog::SaveBackgroundForDeselect(DesignObj_Orbiter *pObj, PlutoPoint point)
{
	CHECK_STATUS();
	PLUTO_SAFETY_LOCK(cm, GetOrbiter()->m_ScreenMutex);
	Rect srcRect;
    ...
    if(srcRect.right >= GetOrbiter()->m_iImageWidth)
        srcRect.right = GetOrbiter()->m_iImageWidth - 1;

    if(srcRect.bottom >= GetOrbiter()->m_iImageHeight)
        srcRect.bottom = GetOrbiter()->m_iImageHeight - 1;
...
//-----------------------------------------------------------------------------------------------------
/*virtual*/ PlutoGraphic *OrbiterRenderer_PocketFrog::CreateGraphic()
{
	return new PocketFrogGraphic();
}
//-----------------------------------------------------------------------------------------------------
/*virtual*/ void OrbiterRenderer_PocketFrog::RenderScreen( bool bRenderGraphicsOnly )
{
...
#ifndef WINCE
		Rect srcRect;
		srcRect.Set(0, 0, GetOrbiter()->m_iImageWidth, GetOrbiter()->m_iImageHeight);
		auto_ptr<Surface> spSurface(GetDisplay()->CreateSurface(GetOrbiter()->m_iImageWidth, GetOrbiter()->m_iImageHeight));
	...
	{
		PLUTO_SAFETY_LOCK(cm, GetOrbiter()->m_ScreenMutex);
		GetDisplay()->FillRect(0, 0, GetOrbiter()->m_iImageWidth, GetOrbiter()->m_iImageHeight, 0x0000);
	}
	...
	if (EnableOpenGL)
	{
		...
		{
			m_rectLastSelected = PlutoRectangle(GetOrbiter()->m_pObj_SelectedLastScreen->m_rPosition);
			m_rectLastSelected.Y = GetOrbiter()->m_iImageHeight - GetOrbiter()->m_pObj_SelectedLastScreen->m_rPosition.Y;
		}
		else
		{
			g_pPlutoLogger->Write(LV_CRITICAL, "No object selected? :o");
			m_rectLastSelected.X = 0;
			m_rectLastSelected.Y = GetOrbiter()->m_iImageHeight;
...
//				case DESIGNOBJ_mnuDvdSpeedControl_CONST:
			...
			{
				m_rectLastSelected = PlutoRectangle(GetOrbiter()->m_pObj_SelectedLastScreen->m_rPosition);
				m_rectLastSelected.Y = GetOrbiter()->m_pObj_SelectedLastScreen->m_rPosition.Y;

#ifdef DEBUG
				g_pPlutoLogger->Write(LV_WARNING, "Effect with change: %d", GetOrbiter()->m_pObj_SelectedLastScreen->m_FK_Effect_Selected_WithChange);
#endif
...
//-----------------------------------------------------------------------------------------------------
/*virtual*/ void OrbiterRenderer_PocketFrog::RedrawObjects()
{
	OrbiterRenderer::RedrawObjects();
}
//-----------------------------------------------------------------------------------------------------
/*virtual*/ void OrbiterRenderer_PocketFrog::RenderGraphic(class PlutoGraphic *pPlutoGraphic, PlutoRectangle rectTotal, 
...
//-----------------------------------------------------------------------------------------------------
/*virtual*/ void OrbiterRenderer_PocketFrog::BeginPaint()
{

}
//-----------------------------------------------------------------------------------------------------
/*virtual*/ void OrbiterRenderer_PocketFrog::EndPaint()
{
	if(EnableOpenGL)
	{
#ifndef DISABLE_OPENGL
...
//-----------------------------------------------------------------------------------------------------
/*virtual*/ void OrbiterRenderer_PocketFrog::UpdateRect(PlutoRectangle rect, PlutoPoint point)
{
...
//-----------------------------------------------------------------------------------------------------
/*virtual*/ void OrbiterRenderer_PocketFrog::OnQuit()
{
...
//-----------------------------------------------------------------------------------------------------
/*virtual*/ void OrbiterRenderer_PocketFrog::Initialize(GraphicType Type, int iPK_Room, int iPK_EntertainArea)
{
	CHECK_STATUS();
	OrbiterRenderer::Initialize(Type, iPK_Room, iPK_EntertainArea);
}
//-----------------------------------------------------------------------------------------------------
/*static*/ void OrbiterRenderer_PocketFrog::Cleanup()
{
	g_pPlutoLogger->Write(LV_STATUS, "OrbiterRenderer_PocketFrog: need to cleanup orbiter...");
	...
	if(NULL != m_pInstance)
	{
		...
		m_pInstance->GetOrbiter()->m_bQuit = true;	

		OrbiterRenderer_PocketFrog *pCopy = m_pInstance;
		m_pInstance = NULL;

		delete pCopy;
		pCopy = NULL;

		g_pPlutoLogger->Write(LV_STATUS, "OrbiterRenderer_PocketFrog: orbiter deleted.");
	}
}
//-----------------------------------------------------------------------------------------------------

//-----------------------------------------------------------------------------------------------------
/*static*/ OrbiterRenderer_PocketFrog *OrbiterRenderer_PocketFrog::GetInstance()
{
	if(!m_pInstance)
		g_pPlutoLogger->Write(LV_STATUS, "OrbiterRenderer_PocketFrog:GetInstance The instance to orbiter is NULL");
...
//-----------------------------------------------------------------------------------------------------
/*virtual*/ void OrbiterRenderer_PocketFrog::CMD_On(int iPK_Pipe,string sPK_Device_Pipes,string &sCMD_Result,Message *pMessage)
{
    /*
	CHECK_STATUS();
...
//-----------------------------------------------------------------------------------------------------
/*virtual*/ void OrbiterRenderer_PocketFrog::CMD_Off(int iPK_Pipe,string &sCMD_Result,Message *pMessage)
{
    /*
	CHECK_STATUS();
...
//-----------------------------------------------------------------------------------------------------
/*virtual*/ void OrbiterRenderer_PocketFrog::TryToUpdate()
{
	CHECK_STATUS();
	PLUTO_SAFETY_LOCK(cm, GetOrbiter()->m_ScreenMutex);
	UpdateScreen();
...
//-----------------------------------------------------------------------------------------------------
/*virtual*/ void OrbiterRenderer_PocketFrog::UpdateScreen()
{
	if (!EnableOpenGL)
	{
		GetDisplay()->Update();
...
//-----------------------------------------------------------------------------------------------------
/*virtual*/ void OrbiterRenderer_PocketFrog::ShowProgress(int nPercent)
{
	CHECK_STATUS();
	...
	int nProgressWidth = GetOrbiter()->m_iImageWidth < PROGRESS_OPTIMAL_SIZE ? GetOrbiter()->m_iImageWidth - PROGRESS_MAX : PROGRESS_OPTIMAL_SIZE;
	PlutoRectangle rect(
		(GetOrbiter()->m_iImageWidth - nProgressWidth) / 2, GetOrbiter()->m_iImageHeight - PROGRESS_BOTTOM_DISTANCE, 
...
//-----------------------------------------------------------------------------------------------------
bool OrbiterRenderer_PocketFrog::SelfUpdate()
{
...
//-----------------------------------------------------------------------------------------------------
int OrbiterRenderer_PocketFrog::GetWindowWidth() { return GetOrbiter()->m_iImageWidth;  } 
//-----------------------------------------------------------------------------------------------------
int OrbiterRenderer_PocketFrog::GetWindowHeight() { return GetOrbiter()->m_iImageHeight; } 
//-----------------------------------------------------------------------------------------------------
void OrbiterRenderer_PocketFrog::PingFailed()
{
	...
	GetOrbiter()->m_bQuit = true;
	GetOrbiter()->m_bReload = true;
	OnQuit();
}
//-----------------------------------------------------------------------------------------------------
/*virtual*/ void OrbiterRenderer_PocketFrog::AdjustWindowSize(int iOrbiterWidth, int iOrbiterHeight)
{
#ifndef WINCE
...
void OrbiterRenderer_PocketFrog::CalcTextRectangle(RECT &rectLocation,PlutoRectangle &rPosition,int iRotation,int iHeight,int iVertAlignment)
{
	...
	else if( GetOrbiter()->m_iRotation==90 || GetOrbiter()->m_iRotation==270 )
	{
		rectLocation.left = 0;
		rectLocation.right = rPosition.Height;
...
//-----------------------------------------------------------------------------------------------------
/*virtual*/ bool OrbiterRenderer_PocketFrog::OnReplaceHandler(string sIP)
{
    if(sIP != GetOrbiter()->m_sIPAddress)
    {
...
//-----------------------------------------------------------------------------------------------------
/*virtual*/ void OrbiterRenderer_PocketFrog::OnUnexpectedDisconnect()
{
...
//-----------------------------------------------------------------------------------------------------
/*virtual*/ int OrbiterRenderer_PocketFrog::PromptUser(string sPrompt,int iTimeoutSeconds,map<int,string> *p_mapPrompts)
{
    return PromptUserEx(sPrompt, p_mapPrompts);
}
//-----------------------------------------------------------------------------------------------------
/*virtual*/ bool OrbiterRenderer_PocketFrog::DisplayProgress(string sMessage, int nProgress)
{
...
{
    string sMessage = 
        FormatMutexMessage(GetOrbiter()->m_ScreenMutex) +
        FormatMutexMessage(GetOrbiter()->m_VariableMutex) +
        FormatMutexMessage(GetOrbiter()->m_DatagridMutex) +
        FormatMutexMessage(GetOrbiter()->m_MaintThreadMutex) +
        FormatMutexMessage(GetOrbiter()->m_NeedRedrawVarMutex);
...
//-----------------------------------------------------------------------------------------------------
void OrbiterRenderer_PocketFrog::OpenGL_RenderFrame(void *data) //callback
{
#ifndef WINCE
	if(m_Desktop && m_Desktop->EffectBuilder && (m_Desktop->EffectBuilder->HasEffects()))
	{
		GetOrbiter()->CallMaintenanceInMiliseconds(0, (OrbiterCallBack) &OrbiterRenderer_PocketFrog::OpenGL_RenderFrame, 
			NULL, pe_NO, true, this);
		OnIdle();
	}
#endif
}
...
void OrbiterRenderer_PocketFrog::SelectObject( class DesignObj_Orbiter *pObj, PlutoPoint point )
{
	...
	if(EnableOpenGL)
	{
		...
		Effect = m_Desktop->EffectBuilder->
			CreateEffect(
				m_Desktop->EffectBuilder->GetEffectCode(GetOrbiter()->m_pObj_SelectedLastScreen->m_FK_Effect_Selected_NoChange),
...
void OrbiterRenderer_PocketFrog::DoHighlightObjectOpenGL()
{
	...
	//m_pGraphicBeforeHighlight = GetBackground(GetOrbiter()->m_rectLastHighlight);
	...
	if (GetOrbiter()->m_pObj_SelectedLastScreen)
	 Effect = m_Desktop->EffectBuilder->
		CreateEffect(
			m_Desktop->EffectBuilder->GetEffectCode(GetOrbiter()->m_pObj_SelectedLastScreen->m_FK_Effect_Highlighted),
			Simulator::GetInstance()->m_iMilisecondsHighLight
		);
	if(!Effect)
		Effect = m_Desktop->EffectBuilder->
...
//-----------------------------------------------------------------------------------------------------
/*virtual*/ void OrbiterRenderer_PocketFrog::GetWindowPosition(PlutoPoint& point)
{
	RECT rect;
	GetWindowRect(&rect);
