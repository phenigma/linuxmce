...
void OrbiterRenderer::RenderScreen( bool bRenderGraphicsOnly )
{
	...
	if( !m_pOrbiter->m_pScreenHistory_Current || !m_pOrbiter->m_pScreenHistory_Current->GetObj() )
	{
		g_pPlutoLogger->Write( LV_CRITICAL, "Got attempt to render null screen: %s", m_pOrbiter->m_pScreenHistory_Current );
		return;
	}

	if( !bRenderGraphicsOnly )
	{
		CallBackData *pCallBackData = m_pOrbiter->m_pScreenHandler->m_mapCallBackData_Find(cbOnRenderScreen);
		if(pCallBackData)
		{
			...
			pRenderScreenCallBackData->m_pObj = m_pOrbiter->m_pScreenHistory_Current->GetObj();
		}

		m_pOrbiter->ExecuteScreenHandlerCallback(cbOnRenderScreen);
	}

#ifndef WINCE
	g_pPlutoLogger->Write( LV_STATUS, "Render screen: %s", m_pOrbiter->m_pScreenHistory_Current->GetObj()->m_ObjectID.c_str(  ) );
#endif
	...
	if (m_pOrbiter->m_pBackgroundImage)
	{
		RenderGraphic(m_pOrbiter->m_pBackgroundImage, PlutoRectangle(0, 0, m_pOrbiter->m_iImageWidth, m_pOrbiter->m_iImageHeight), false);
	}

	if ( m_pOrbiter->m_pScreenHistory_Current  )
	{
		m_pOrbiter->m_pScreenHistory_Current->GetObj()->RenderObject( m_pOrbiter->m_pScreenHistory_Current->GetObj());
	}

	for(list<class PlutoPopup*>::iterator it=m_pOrbiter->m_listPopups.begin();it!=m_pOrbiter->m_listPopups.end();++it)
		m_pOrbiter->RenderPopup(*it, (*it)->m_Position);

	if( m_pOrbiter->m_pScreenHistory_Current  )
	{
		for(list<class PlutoPopup*>::iterator it=m_pOrbiter->m_pScreenHistory_Current->GetObj()->m_listPopups.begin();it!=m_pOrbiter->m_pScreenHistory_Current->GetObj()->m_listPopups.end();++it)
			m_pOrbiter->RenderPopup(*it, (*it)->m_Position);
	}
	...
	if(  m_pOrbiter->m_pScreenHistory_Current   )
	{
		g_pPlutoLogger->Write( LV_CONTROLLER, "Render screen: %s took %d ms",
			m_pOrbiter->m_pScreenHistory_Current->m_pObj->m_ObjectID.c_str(  ), clkFinished-clkStart );
	}
#endif
...
#ifdef DEBUG
	g_pPlutoLogger->Write( LV_STATUS, "Render screen: %s finished", m_pOrbiter->m_pScreenHistory_Current->GetObj()->m_ObjectID.c_str(  ) );
#endif
}
...
{
	if(sbNoSelection != m_pOrbiter->m_nSelectionBehaviour)
		for(int i = 0; i < 4; i++)
			HollowRectangle(
...
/*virtual*/ void OrbiterRenderer::DoHighlightObject()
{
	...
	if(sbNoSelection == m_pOrbiter->m_nSelectionBehaviour || !m_pOrbiter->m_pObj_Highlighted->m_bOnScreen )
		return;
	UnHighlightObject();
	...
	m_pOrbiter->ExecuteCommandsInList( &m_pOrbiter->m_pObj_Highlighted->m_Action_HighlightList, m_pOrbiter->m_pObj_Highlighted, smHighlight, 0, 0 );

	if( m_pOrbiter->m_pObj_Highlighted->m_ObjectType==DESIGNOBJTYPE_Datagrid_CONST )
		m_pOrbiter->GetDataGridHighlightCellCoordinates((DesignObj_DataGrid *) m_pOrbiter->m_pObj_Highlighted,m_pOrbiter->m_rectLastHighlight);
	else
		m_pOrbiter->m_rectLastHighlight = m_pOrbiter->m_pObj_Highlighted->GetHighlightRegion();

	m_pOrbiter->m_rectLastHighlight.X += m_pOrbiter->m_pObj_Highlighted->m_pPopupPoint.X;
	m_pOrbiter->m_rectLastHighlight.Y += m_pOrbiter->m_pObj_Highlighted->m_pPopupPoint.Y;

	m_pOrbiter->m_rectLastHighlight.Width++;  // GetBackground always seems to be 1 pixel to little
	m_pOrbiter->m_rectLastHighlight.Height++;

	m_pOrbiter->m_pGraphicBeforeHighlight = GetBackground(m_pOrbiter->m_rectLastHighlight);

	PlutoGraphic *pPlutoGraphic = m_pOrbiter->m_pGraphicBeforeHighlight->GetHighlightedVersion();
	if(pPlutoGraphic)
	{
		RenderGraphic(pPlutoGraphic, m_pOrbiter->m_rectLastHighlight);
	...
	for(int i = 0; i < 4; i++)
        HollowRectangle(
			m_pOrbiter->m_rectLastHighlight.X + i, m_pOrbiter->m_rectLastHighlight.Y + i,
			m_pOrbiter->m_rectLastHighlight.Width - 2 * i - 2, m_pOrbiter->m_rectLastHighlight.Height - 2 * i - 2,
			i < 2 ? PlutoColor::Red() : PlutoColor::White()
		);

	UpdateRect(m_pOrbiter->m_rectLastHighlight);
}
...
/*virtual*/ void OrbiterRenderer::UnHighlightObject( bool bDeleteOnly )
{
	...
	if( !m_pOrbiter->m_pGraphicBeforeHighlight )
		return;

	if( !bDeleteOnly && m_pOrbiter->m_pObj_Highlighted)
	{
		RenderGraphic(m_pOrbiter->m_pGraphicBeforeHighlight, m_pOrbiter->m_rectLastHighlight);
		UpdateRect(m_pOrbiter->m_rectLastHighlight);
	}

	delete m_pOrbiter->m_pGraphicBeforeHighlight;
	m_pOrbiter->m_pGraphicBeforeHighlight=NULL;
}
...
int OrbiterRenderer::HandleNotOKStatus(string sStatus,string sRegenStatus,int iRegenPercent)
{
	...
	if( sStatus=="R" || sStatus=="r" )
		return MonitorRegen(m_pOrbiter->m_dwPK_Device);

	if( sStatus=="N" || ((sStatus=="D" || sStatus=="U") && !m_pOrbiter->m_dwPK_Device) )
	{
	...
	{
		PromptUser("Something went wrong and this orbiter's user interface wasn't created.  In Pluto Admin, go to Wizard, Devices, Orbiters and click Regen for Orbiter #" + StringUtils::itos(m_pOrbiter->m_dwPK_Device) + " and try later");
		m_pOrbiter->OnQuit();
		exit(0);
		return 0;
	}
	else if( sStatus=="D" || sStatus=="U" )
	{
		...
		int iResponse = prNo;
		if( sStatus=="D" )
			iResponse = PromptUser("Something went wrong. The device number, " + StringUtils::itos(m_pOrbiter->m_dwPK_Device) + ", doesn't seem to be an orbiter.  Reset the device number and try next time to determine it automatically?",0,&mapPrompts);
		else
			iResponse = PromptUser("Something went wrong.  The device number, " + StringUtils::itos(m_pOrbiter->m_dwPK_Device) + ", is not known to the Core.  Reset the device number and try next time to determine it automatically?",0,&mapPrompts);

		if( iResponse==prYes )
		{
			m_pOrbiter->m_dwPK_Device = 0;
...
bool OrbiterRenderer::RouterNeedsReload()
{
	...
	Event_Impl event_Impl(DEVICEID_MESSAGESEND, 0, m_pOrbiter->m_sIPAddress);
	string sStatus,sRegenStatus;
	int iRegenPercent;
	DCE::CMD_Get_Orbiter_Status_DT CMD_Get_Orbiter_Status_DT( m_pOrbiter->m_dwPK_Device, DEVICETEMPLATE_Orbiter_Plugin_CONST, BL_SameHouse,
		m_pOrbiter->m_dwPK_Device,&sStatus,&sRegenStatus,&iRegenPercent);
...
int OrbiterRenderer::DeviceIdInvalid()
{
	...
	if( m_pOrbiter->m_dwPK_Device )
	{
		PromptUser("Something went wrong.  The device number, " + StringUtils::itos(m_pOrbiter->m_dwPK_Device) + ", is reported as invalid.");
		m_pOrbiter->OnQuit();
		exit(0);
		return 0;
	}
	...
	int PK_Device = 0;
	if(m_pOrbiter->IsSelfInstallable())
	{
...
int OrbiterRenderer::PickOrbiterDeviceID()
{
	...
	m_pOrbiter->GetDevicesByCategory(DEVICECATEGORY_Orbiter_CONST,&mapDevices);
...
int OrbiterRenderer::SetupNewOrbiter()
{
	...
	int PK_Device=0;
	DCE::CMD_New_Orbiter_DT CMD_New_Orbiter_DT(m_pOrbiter->m_dwPK_Device, DEVICETEMPLATE_Orbiter_Plugin_CONST, BL_SameHouse, sType,
		PK_Users,m_pOrbiter->m_dwPK_DeviceTemplate,m_pOrbiter->m_sMacAddress,PK_Room,Width,Height,PK_Skin,PK_Language,PK_Size,&PK_Device);
	...
	Simulator::GetInstance()->m_sRouterIP = m_pOrbiter->m_sIPAddress;
...
int OrbiterRenderer::PromptFor(string sToken)
{
	...
	string sResults;
	DCE::CMD_Get_Orbiter_Options_DT CMD_Get_Orbiter_Options_DT(m_pOrbiter->m_dwPK_Device, DEVICETEMPLATE_Orbiter_Plugin_CONST, BL_SameHouse,
		sToken,&sResults);
...
bool OrbiterRenderer::RegenOrbiter()
{
	Event_Impl event_Impl(DEVICEID_MESSAGESEND, 0, m_pOrbiter->m_sIPAddress);
	string sResponse;
	DCE::CMD_Regen_Orbiter_DT CMD_Regen_Orbiter_DT( m_pOrbiter->m_dwPK_Device, DEVICETEMPLATE_Orbiter_Plugin_CONST, BL_SameHouse,
