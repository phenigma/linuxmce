...
{
NeedToRender render( m_pMouseBehavior->GetOrbiter(), "change light" );
...
bool LightMouseHandler::ButtonUp(int PK_Button)
{
	if( PK_Button==BUTTON_Mouse_2_CONST )
	{
		if( !m_bTapAndRelease )
		{
			DCE::CMD_Set_Level CMD_Set_Level(m_pMouseBehavior->GetOrbiter()->m_dwPK_Device,m_pMouseBehavior->GetOrbiter()->m_dwPK_Device_NowPlaying_Audio,StringUtils::itos(m_iCancelLevel));
			m_pMouseBehavior->GetOrbiter()->SendMessage(CMD_Set_Level.m_pMessage);
		}
		m_pMouseBehavior->Clear(true); // this will be deleted
		return false; // Keep processing
	}
...
void LightMouseHandler::Move(int X,int Y,int PK_Direction)
{
	...
	{
		NeedToRender render( m_pMouseBehavior->GetOrbiter(), "LightMouseHandler" );
		m_pMouseBehavior->GetOrbiter()->RenderObjectAsync(m_pObj);
		if( m_bTapAndRelease==false )
		{
			DCE::CMD_Set_Level CMD_Set_Level(m_pMouseBehavior->GetOrbiter()->m_dwPK_Device,m_pMouseBehavior->GetOrbiter()->m_dwPK_Device_LightingPlugIn,StringUtils::itos(Notch*10));
...
{
	NeedToRender render( m_pMouseBehavior->GetOrbiter(), "LightMouseHandler it" );
	m_pMouseBehavior->GetOrbiter()->RenderObjectAsync(m_pObj);
	...
	if( m_iCancelLevel < 0 )
		m_iCancelLevel = 0;
	DCE::CMD_Set_Level CMD_Set_Level(m_pMouseBehavior->GetOrbiter()->m_dwPK_Device,m_pMouseBehavior->GetOrbiter()->m_dwPK_Device_LightingPlugIn,(Parm>0 ? "+" : "") + StringUtils::itos(Parm*10));
	m_pMouseBehavior->m_pMouseGovernor->SendMessage(CMD_Set_Level.m_pMessage);
}
