...
void DatagridMouseHandlerHelper::Start(DesignObj_DataGrid *pObj_ScrollingGrid)
{
	...
	m_pObj_MediaBrowser_Down = m_pMouseBehavior->GetOrbiter()->FindObject(m_pMouseHandler->m_pObj->m_ObjectID + "." + StringUtils::itos(DESIGNOBJ_icoDownIndicator_CONST));
	m_pObj_MediaBrowser_Up = m_pMouseBehavior->GetOrbiter()->FindObject(m_pMouseHandler->m_pObj->m_ObjectID + "." + StringUtils::itos(DESIGNOBJ_icoUpIndicator_CONST));
	m_pObj_MediaBrowser_Alpha = m_pMouseBehavior->GetOrbiter()->FindObject(m_pMouseHandler->m_pObj->m_ObjectID + "." + StringUtils::itos(DESIGNOBJ_icoAlpha_CONST));
...
int k=2;
		...
		{
			m_pMouseBehavior->GetOrbiter()->RenderObjectAsync(m_pObj_ScrollingGrid);

//m_pMouseBehavior->GetOrbiter()->HighlightNextObject(PK_Direction);
			m_pMouseBehavior->GetOrbiter()->RedrawObjects();  // We may have scrolled past the end of a grid and need to re-render
		}
		return true;
	}
...
bool DatagridMouseHandlerHelper::MovedPastTopBottomOfDataGrid(DesignObj_DataGrid *pObj,int PK_Direction,int Y)
{
	...
	else
	{
		...
		PlutoRectangle r;
		m_pMouseBehavior->GetOrbiter()->GetGridCellDimensions( pObj,
			1, // col span
			1, // row span
			0, // column
	...
	m_pObj_ScrollingGrid = (DesignObj_DataGrid *) m_pMouseBehavior->GetOrbiter()->m_pObj_Highlighted;
...
g_pPlutoLogger->Write(LV_FESTIVAL,"DatagridMouseHandlerHelper::ScrollGrid direction changed from %d to %d",
		...
		else
		{
			...
			PlutoRectangle r;
			m_pMouseBehavior->GetOrbiter()->GetGridCellDimensions( m_pObj_ScrollingGrid,
				1, // col span
				1, // row span
				0, // column
...
g_pPlutoLogger->Write(LV_FESTIVAL,"DatagridMouseHandlerHelper::ScrollGrid Notch %d",Notch);
	NeedToRender render( m_pMouseBehavior->GetOrbiter(), "scroll" );
		m_pMouseBehavior->GetOrbiter()->RenderObjectAsync(m_pObj_ScrollingGrid);
		// We know the notch can't go in reverse since that would stop the scrolling
		if( Notch<=5 )
		{
			if( dwPK_Direction==DIRECTION_Up_CONST )
				m_pMouseBehavior->GetOrbiter()->CMD_Scroll_Grid("","",DIRECTION_Up_CONST);
			else
				m_pMouseBehavior->GetOrbiter()->CMD_Scroll_Grid("","",DIRECTION_Down_CONST);
		...
		if( dwPK_Direction==DIRECTION_Down_CONST && m_pObj_MediaBrowser_Down )
		{
			...
			m_pMouseBehavior->GetOrbiter()->RenderObjectAsync(m_pObj_MediaBrowser_Down);
		}
		else if( dwPK_Direction==DIRECTION_Up_CONST && m_pObj_MediaBrowser_Up )
		{
			...
			m_pMouseBehavior->GetOrbiter()->RenderObjectAsync(m_pObj_MediaBrowser_Up);
		}
