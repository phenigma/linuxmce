	...
	: MouseHandler(pObj,sOptions,pMouseBehavior)
{
	...
	string sResponse;
	DCE::CMD_Change_Playback_Speed CMD_Change_Playback_Speed(m_pMouseBehavior->GetOrbiter()->m_dwPK_Device,m_pMouseBehavior->GetOrbiter()->m_dwPK_Device_NowPlaying,0,0,true);
	m_pMouseBehavior->GetOrbiter()->SendCommand(CMD_Change_Playback_Speed,&sResponse);

	string sText,sMediaPosition;
	DCE::CMD_Report_Playback_Position CMD_Report_Playback_Position(m_pMouseBehavior->GetOrbiter()->m_dwPK_Device,m_pMouseBehavior->GetOrbiter()->m_dwPK_Device_NowPlaying,0,&sText,&sMediaPosition);
	m_bHasTimeline = m_pMouseBehavior->GetOrbiter()->SendCommand(CMD_Report_Playback_Position) && ParsePosition(sMediaPosition);

	DCE::CMD_Bind_to_Media_Remote CMD_Bind_to_Media_Remote(m_pMouseBehavior->GetOrbiter()->m_dwPK_Device,m_pMouseBehavior->GetOrbiter()->m_dwPK_Device_MediaPlugIn,0,"","1","", StringUtils::itos( m_pMouseBehavior->GetOrbiter()->m_pLocationInfo->PK_EntertainArea ),0,0);  // 0 for the screen means don't send us to the real remote if we're on the wrong one
	m_pMouseBehavior->GetOrbiter()->SendCommand(CMD_Bind_to_Media_Remote);
...
{
	DCE::CMD_Change_Playback_Speed CMD_Change_Playback_Speed(m_pMouseBehavior->GetOrbiter()->m_dwPK_Device,m_pMouseBehavior->GetOrbiter()->m_dwPK_Device_NowPlaying,0,1000,false);
	m_pMouseBehavior->GetOrbiter()->SendCommand(CMD_Change_Playback_Speed);
	DCE::CMD_Bind_to_Media_Remote CMD_Bind_to_Media_Remote(m_pMouseBehavior->GetOrbiter()->m_dwPK_Device,m_pMouseBehavior->GetOrbiter()->m_dwPK_Device_MediaPlugIn,0,"","0","", StringUtils::itos( m_pMouseBehavior->GetOrbiter()->m_pLocationInfo->PK_EntertainArea ),0,0);
	m_pMouseBehavior->GetOrbiter()->SendCommand(CMD_Bind_to_Media_Remote);
}
...
bool SpeedMouseHandler::ButtonDown(int PK_Button)
{
	if( PK_Button==BUTTON_Mouse_1_CONST || PK_Button==BUTTON_Mouse_6_CONST || PK_Button==BUTTON_Mouse_2_CONST )
	{
		...
		{
			DCE::CMD_Set_Media_Position CMD_Set_Media_Position(m_pMouseBehavior->GetOrbiter()->m_dwPK_Device,m_pMouseBehavior->GetOrbiter()->m_dwPK_Device_NowPlaying,0," dm_" + StringUtils::itos(m_iCancelLevel*1000));
			m_pMouseBehavior->GetOrbiter()->SendMessage(CMD_Set_Media_Position.m_pMessage);
		}
...
void SpeedMouseHandler::Move(int X,int Y,int PK_Direction)
{
	...
	if( m_bTapAndRelease || !m_bHasTimeline )  // holding button down, change speed, or this stream doesn't support absolute positioning anyway
	{
		...
		{
			DCE::CMD_Change_Playback_Speed CMD_Change_Playback_Speed(m_pMouseBehavior->GetOrbiter()->m_dwPK_Device,m_pMouseBehavior->GetOrbiter()->m_dwPK_Device_NowPlaying,0,Speed,true);
			m_pMouseBehavior->m_pMouseGovernor->SendMessage(CMD_Change_Playback_Speed.m_pMessage);
			m_iLastNotch=Notch;
NeedToRender render( m_pMouseBehavior->GetOrbiter(), "start speed" );
m_pMouseBehavior->GetOrbiter()->RenderObjectAsync(m_pObj);// Redraw even if the object was already in this state,  because maybe we're hiding this and something that
		}
	}
	else  // holding down, go to position
	{
		...
		{
			m_iLastNotch=Notch;
			DCE::CMD_Set_Media_Position CMD_Set_Media_Position(m_pMouseBehavior->GetOrbiter()->m_dwPK_Device,m_pMouseBehavior->GetOrbiter()->m_dwPK_Device_NowPlaying,0," POS:" + StringUtils::itos(m_iLastNotch*1000));
...
void SpeedMouseHandler::DrawInfo()
{
	...
	if( SeekPosition!=-1 )
	{
		...
		if( Offset<0 )
			Offset=0;
		m_pMouseBehavior->GetOrbiter()->Renderer()->SolidRectangle(
	...
	DesignObjText *pText = m_pMouseBehavior->GetOrbiter()->FindText(m_pObj,TEXT_Current_Speed_CONST);
	if( Speed!=-9999 )
	{
	...
	if( m_bHasTimeline )
	{
		DesignObjText *pText = m_pMouseBehavior->GetOrbiter()->FindText(m_pObj,TEXT_Current_Time_CONST);
		...
		pText = m_pMouseBehavior->GetOrbiter()->FindText(m_pObj,TEXT_Start_Time_CONST);
		if( pText )
			pText->m_sText = FormatTime(m_CurrentMedia_Start);

		pText = m_pMouseBehavior->GetOrbiter()->FindText(m_pObj,TEXT_Stop_Time_CONST);
		if( pText )
