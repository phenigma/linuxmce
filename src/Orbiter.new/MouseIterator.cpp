...
MouseIterator::MouseIterator(MouseBehavior *pMouseBehavior) : m_IteratorMutex("Iterator")
{
	...
	m_pOrbiter=m_pMouseBehavior->GetOrbiter();
	...
	case if_MediaTracks:
		{
			...
			PLUTO_SAFETY_LOCK( cm, m_pMouseBehavior->GetOrbiter()->m_ScreenMutex );  // Always lock this before datagrid to prevent a deadlock
			PLUTO_SAFETY_LOCK( dng, m_pMouseBehavior->GetOrbiter()->m_DatagridMutex );
