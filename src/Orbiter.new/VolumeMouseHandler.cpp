...
void VolumeMouseHandler::Start()
{
	...
	NeedToRender render( m_pMouseBehavior->GetOrbiter(), "start volume" );
	m_pMouseBehavior->GetOrbiter()->RenderObjectAsync(m_pObj);
m_pMouseBehavior->GetOrbiter()->m_bPK_Device_NowPlaying_Audio_DiscreteVolume=true;
	if( m_pMouseBehavior->GetOrbiter()->m_bPK_Device_NowPlaying_Audio_DiscreteVolume==false || m_pMouseBehavior->m_iTime_Last_Mouse_Up )
	{
	...
	else
	{
		...
		m_iCancelLevel = m_iLastNotch = atoi(m_pMouseBehavior->GetOrbiter()->GetEvents()->GetDeviceDataFromDatabase(m_pMouseBehavior->GetOrbiter()->m_dwPK_Device_NowPlaying_Audio,DEVICEDATA_Volume_Level_CONST).c_str());
...
{
NeedToRender render( m_pMouseBehavior->GetOrbiter(), "change to discrete volume" );
...
bool VolumeMouseHandler::ButtonUp(int PK_Button)
{
	if( PK_Button==BUTTON_Mouse_2_CONST )
	{
		if( !m_bTapAndRelease )
		{
			DCE::CMD_Set_Volume CMD_Set_Volume(m_pMouseBehavior->GetOrbiter()->m_dwPK_Device,m_pMouseBehavior->GetOrbiter()->m_dwPK_Device_NowPlaying_Audio,StringUtils::itos(m_iCancelLevel));
			m_pMouseBehavior->GetOrbiter()->SendMessage(CMD_Set_Volume.m_pMessage);
		}
		m_pMouseBehavior->Clear(true); // this will be deleted
		return false; // Keep processing
	}
...
void VolumeMouseHandler::Move(int X,int Y,int PK_Direction)
{
	...
	else
	{
		...
		if( Notch!=m_iLastNotch )
		{
			NeedToRender render( m_pMouseBehavior->GetOrbiter(), "start volume" );
			m_pMouseBehavior->GetOrbiter()->RenderObjectAsync(m_pObj);
			DCE::CMD_Set_Volume CMD_Set_Volume(m_pMouseBehavior->GetOrbiter()->m_dwPK_Device,m_pMouseBehavior->GetOrbiter()->m_dwPK_Device_NowPlaying_Audio,StringUtils::itos(Notch));
...
void VolumeMouseHandler::DoIteration(int Parm)
{
	...
	if( Parm<0 )
	{
		DCE::CMD_Vol_Down CMD_Vol_Down(m_pMouseBehavior->GetOrbiter()->m_dwPK_Device,m_pMouseBehavior->GetOrbiter()->m_dwPK_Device_NowPlaying_Audio,Parm*-1);
		m_pMouseBehavior->m_pMouseGovernor->SendMessage(CMD_Vol_Down.m_pMessage);
	}
	else if( Parm>0 )
	{
		DCE::CMD_Vol_Up CMD_Vol_Up(m_pMouseBehavior->GetOrbiter()->m_dwPK_Device,m_pMouseBehavior->GetOrbiter()->m_dwPK_Device_NowPlaying_Audio,Parm);
