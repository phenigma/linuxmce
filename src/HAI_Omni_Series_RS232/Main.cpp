...
extern "C" {
	...
	{ 
		...
		if( bIsRuntimePlugin )
			return HAI_Omni_Series_RS232::PK_DeviceTemplate_get_static();
		else
			return 0;
	}
}
...
extern "C" {
	class Command_Impl *RegisterAsPlugIn(class Router *pRouter,int PK_Device,Logger *pPlutoLogger)
	{
		...
		HAI_Omni_Series_RS232 *pHAI_Omni_Series_RS232 = new HAI_Omni_Series_RS232(PK_Device, "localhost",true,false,pRouter);
		if( pHAI_Omni_Series_RS232->m_bQuit_get()|| !pHAI_Omni_Series_RS232->GetConfig() )
		{
			delete pHAI_Omni_Series_RS232;
			return NULL;
		}
		else
		{
			g_pCommand_Impl=pHAI_Omni_Series_RS232;
			g_pDeadlockHandler=Plugin_DeadlockHandler;
			g_pSocketCrashHandler=Plugin_SocketCrashHandler;
		}
		return pHAI_Omni_Series_RS232;
	}
}
//<-dceag-plug-e->
...
int main(int argc, char* argv[]) 
{
	...
	cout << "HAI_Omni_Series_RS232, v." << VERSION << endl
	...
	if (bError)
	{
		cout << "A Pluto DCE Device.  See www.plutohome.com/dce for details." << endl
			<< "Usage: HAI_Omni_Series_RS232 [-r Router's IP] [-d My Device ID] [-l dcerouter|stdout|null|filename]" << endl
	...
	try
	{
		if( sLogger=="dcerouter" )
			LoggerWrapper::SetInstance(new ServerLogger(PK_Device, HAI_Omni_Series_RS232::PK_DeviceTemplate_get_static(), sRouter_IP));
	...
	try
	{
		HAI_Omni_Series_RS232 *pHAI_Omni_Series_RS232 = new HAI_Omni_Series_RS232(PK_Device, sRouter_IP,true,bLocalMode);
		if ( pHAI_Omni_Series_RS232->GetConfig() && pHAI_Omni_Series_RS232->Connect(pHAI_Omni_Series_RS232->PK_DeviceTemplate_get()) ) 
		{
			g_pCommand_Impl=pHAI_Omni_Series_RS232;
			g_pDeadlockHandler=DeadlockHandler;
			g_pSocketCrashHandler=SocketCrashHandler;
			LoggerWrapper::GetInstance()->Write(LV_STATUS, "Connect OK");
			pHAI_Omni_Series_RS232->CreateChildren();
			if( bLocalMode )
				pHAI_Omni_Series_RS232->RunLocalMode();
			else
			{
				if(pHAI_Omni_Series_RS232->m_RequestHandlerThread)
					pthread_join(pHAI_Omni_Series_RS232->m_RequestHandlerThread, NULL);  // This function will return when the device is shutting down
			}
		...
		{
			bAppError = true;
			if( pHAI_Omni_Series_RS232->m_pEvent && pHAI_Omni_Series_RS232->m_pEvent->m_pClientSocket && pHAI_Omni_Series_RS232->m_pEvent->m_pClientSocket->m_eLastError==ClientSocket::cs_err_CannotConnect )
			{
				bAppError = false;
				bReload = false;
		...
		if( pHAI_Omni_Series_RS232->m_bReload )
			bReload=true;
