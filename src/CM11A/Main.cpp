...
extern "C" {
	class Command_Impl *RegisterAsPlugIn(class Router *pRouter,int PK_Device,string sLogger)
	{
		...
		{
			g_pPlutoLogger = new ServerLogger(PK_Device, CM11A::PK_DeviceTemplate_get_static(), "localhost");
...
int main(int argc, char* argv[]) 
{
	...
	{
		if( sLogger=="dcerouter" )
			g_pPlutoLogger = new ServerLogger(PK_Device, CM11A::PK_DeviceTemplate_get_static(), sRouter_IP);
	...
	try
	{
		...
		if ( pCM11A->Connect(pCM11A->PK_DeviceTemplate_get()) ) 
		{
			g_pCommand_Impl=pCM11A;
