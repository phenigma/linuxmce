...
extern "C" {
	...
	{ 
		...
		if( bIsRuntimePlugin )
			return Plug_And_Play_Plugin::PK_DeviceTemplate_get_static();
		else
			return 0;
	}
}
...
int main(int argc, char* argv[]) 
{
	...
	cout << "Plug_And_Play_Plugin, v." << VERSION << endl
	...
	if (bError)
	{
		cout << "A Pluto DCE Device.  See www.plutohome.com/dce for details." << endl
			<< "Usage: Plug_And_Play_Plugin [-r Router's IP] [-d My Device ID] [-l dcerouter|stdout|null|filename]" << endl
	...
	try
	{
		if( sLogger=="dcerouter" )
			LoggerWrapper::SetInstance(new ServerLogger(PK_Device, Plug_And_Play_Plugin::PK_DeviceTemplate_get_static(), sRouter_IP));
	...
	try
	{
		Plug_And_Play_Plugin *pPlug_And_Play_Plugin = new Plug_And_Play_Plugin(PK_Device, sRouter_IP,true,bLocalMode);
		if ( pPlug_And_Play_Plugin->GetConfig() && pPlug_And_Play_Plugin->Connect(pPlug_And_Play_Plugin->PK_DeviceTemplate_get()) ) 
		{
			g_pCommand_Impl=pPlug_And_Play_Plugin;
			g_pDeadlockHandler=DeadlockHandler;
			g_pSocketCrashHandler=SocketCrashHandler;
			LoggerWrapper::GetInstance()->Write(LV_STATUS, "Connect OK");
			pPlug_And_Play_Plugin->CreateChildren();
			if( bLocalMode )
				pPlug_And_Play_Plugin->RunLocalMode();
			else
			{
				if(pPlug_And_Play_Plugin->m_RequestHandlerThread)
					pthread_join(pPlug_And_Play_Plugin->m_RequestHandlerThread, NULL);  // This function will return when the device is shutting down
			}
		...
		{
			bAppError = true;
			if( pPlug_And_Play_Plugin->m_pEvent && pPlug_And_Play_Plugin->m_pEvent->m_pClientSocket && pPlug_And_Play_Plugin->m_pEvent->m_pClientSocket->m_eLastError==ClientSocket::cs_err_CannotConnect )
			{
				bAppError = false;
				bReload = false;
		...
		if( pPlug_And_Play_Plugin->m_bReload )
			bReload=true;
