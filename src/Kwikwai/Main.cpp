...
extern "C" {
	...
	{ 
		...
		if( bIsRuntimePlugin )
			return Kwikwai::PK_DeviceTemplate_get_static();
		else
			return 0;
	}
}
...
extern "C" {
	class Command_Impl *RegisterAsPlugIn(class Router *pRouter,int PK_Device,Logger *pPlutoLogger)
	{
		...
		Kwikwai *pKwikwai = new Kwikwai(PK_Device, "localhost",true,false,pRouter);
		if( pKwikwai->m_bQuit_get()|| !pKwikwai->GetConfig() )
		{
			delete pKwikwai;
			return NULL;
		}
...
int main(int argc, char* argv[]) 
{
	...
	if (bError)
	{
		cout << "A Pluto DCE Device.  See www.plutohome.com/dce for details." << endl
			<< "Usage: Kwikwai [-r Router's IP] [-d My Device ID] [-l dcerouter|stdout|null|filename]" << endl
	...
	try
	{
		if( sLogger=="dcerouter" )
			LoggerWrapper::SetInstance(new ServerLogger(PK_Device, Kwikwai::PK_DeviceTemplate_get_static(), sRouter_IP));
	...
	try
	{
		Kwikwai *pKwikwai = new Kwikwai(PK_Device, sRouter_IP,true,bLocalMode);
		if ( pKwikwai->GetConfig() && pKwikwai->Connect(pKwikwai->PK_DeviceTemplate_get()) ) 
		{
			g_pCommand_Impl=pKwikwai;
			g_pDeadlockHandler=DeadlockHandler;
			g_pSocketCrashHandler=SocketCrashHandler;
			LoggerWrapper::GetInstance()->Write(LV_STATUS, "Connect OK");
			pKwikwai->CreateChildren();
			if( bLocalMode )
				pKwikwai->RunLocalMode();
			else
			{
				if(pKwikwai->m_RequestHandlerThread)
					pthread_join(pKwikwai->m_RequestHandlerThread, NULL);  // This function will return when the device is shutting down
			}
		...
		{
			bAppError = true;
			if( pKwikwai->m_pEvent && pKwikwai->m_pEvent->m_pClientSocket && pKwikwai->m_pEvent->m_pClientSocket->m_eLastError==ClientSocket::cs_err_CannotConnect )
			{
				bAppError = false;
				bReload = false;
