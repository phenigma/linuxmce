...
extern "C" {
	...
	{ 
		...
		if( bIsRuntimePlugin )
			return IRTrans_Ethernet::PK_DeviceTemplate_get_static();
		else
			return 0;
	}
}
...
extern "C" {
	class Command_Impl *RegisterAsPlugIn(class Router *pRouter,int PK_Device,Logger *pPlutoLogger)
	{
		...
		IRTrans_Ethernet *pIRTrans_Ethernet = new IRTrans_Ethernet(PK_Device, "localhost",true,false,pRouter);
		if( pIRTrans_Ethernet->m_bQuit_get()|| !pIRTrans_Ethernet->GetConfig() )
		{
			delete pIRTrans_Ethernet;
			return NULL;
		}
		else
		{
			g_pCommand_Impl=pIRTrans_Ethernet;
			g_pDeadlockHandler=Plugin_DeadlockHandler;
			g_pSocketCrashHandler=Plugin_SocketCrashHandler;
		}
		return pIRTrans_Ethernet;
	}
}
//<-dceag-plug-e->
...
int main(int argc, char* argv[]) 
{
	...
	cout << "IRTrans_Ethernet, v." << VERSION << endl
	...
	try
	{
		if( sLogger=="dcerouter" )
			LoggerWrapper::SetInstance(new ServerLogger(PK_Device, IRTrans_Ethernet::PK_DeviceTemplate_get_static(), sRouter_IP));
	...
	try
	{
		IRTrans_Ethernet *pIRTrans_Ethernet = new IRTrans_Ethernet(PK_Device, sRouter_IP,true,bLocalMode);
		if ( pIRTrans_Ethernet->GetConfig() && pIRTrans_Ethernet->Connect(pIRTrans_Ethernet->PK_DeviceTemplate_get()) ) 
		{
			g_pCommand_Impl=pIRTrans_Ethernet;
			g_pDeadlockHandler=DeadlockHandler;
			g_pSocketCrashHandler=SocketCrashHandler;
			LoggerWrapper::GetInstance()->Write(LV_STATUS, "Connect OK");
			pIRTrans_Ethernet->CreateChildren();
			if( bLocalMode )
				pIRTrans_Ethernet->RunLocalMode();
			else
			{
				if(pIRTrans_Ethernet->m_RequestHandlerThread)
					pthread_join(pIRTrans_Ethernet->m_RequestHandlerThread, NULL);  // This function will return when the device is shutting down
			}
		...
		{
			bAppError = true;
			if( pIRTrans_Ethernet->m_pEvent && pIRTrans_Ethernet->m_pEvent->m_pClientSocket && pIRTrans_Ethernet->m_pEvent->m_pClientSocket->m_eLastError==ClientSocket::cs_err_CannotConnect )
			{
				bAppError = false;
				bReload = false;
		...
		if( pIRTrans_Ethernet->m_bReload )
			bReload=true;
