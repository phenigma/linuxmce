...
extern "C" {
	...
	{ 
		...
		if( bIsRuntimePlugin )
			return XML_Data_Handler_Plugin::PK_DeviceTemplate_get_static();
		else
			return 0;
	}
}
...
extern "C" {
	class Command_Impl *RegisterAsPlugIn(class Router *pRouter,int PK_Device,Logger *pPlutoLogger)
	{
		...
		XML_Data_Handler_Plugin *pXML_Data_Handler_Plugin = new XML_Data_Handler_Plugin(PK_Device, "localhost",true,false,pRouter);
		if( pXML_Data_Handler_Plugin->m_bQuit_get()|| !pXML_Data_Handler_Plugin->GetConfig() )
		{
			delete pXML_Data_Handler_Plugin;
			return NULL;
		}
		else
		{
			g_pCommand_Impl=pXML_Data_Handler_Plugin;
			g_pDeadlockHandler=Plugin_DeadlockHandler;
			g_pSocketCrashHandler=Plugin_SocketCrashHandler;
		}
		return pXML_Data_Handler_Plugin;
	}
}
//<-dceag-plug-e->
...
int main(int argc, char* argv[]) 
{
	...
	cout << "XML_Data_Handler_Plugin, v." << VERSION << endl
	...
	if (bError)
	{
		cout << "A Pluto DCE Device.  See www.plutohome.com/dce for details." << endl
			<< "Usage: XML_Data_Handler_Plugin [-r Router's IP] [-d My Device ID] [-l dcerouter|stdout|null|filename]" << endl
	...
	try
	{
		if( sLogger=="dcerouter" )
			LoggerWrapper::SetInstance(new ServerLogger(PK_Device, XML_Data_Handler_Plugin::PK_DeviceTemplate_get_static(), sRouter_IP));
	...
	try
	{
		XML_Data_Handler_Plugin *pXML_Data_Handler_Plugin = new XML_Data_Handler_Plugin(PK_Device, sRouter_IP,true,bLocalMode);
		if ( pXML_Data_Handler_Plugin->GetConfig() && pXML_Data_Handler_Plugin->Connect(pXML_Data_Handler_Plugin->PK_DeviceTemplate_get()) ) 
		{
			g_pCommand_Impl=pXML_Data_Handler_Plugin;
			g_pDeadlockHandler=DeadlockHandler;
			g_pSocketCrashHandler=SocketCrashHandler;
			LoggerWrapper::GetInstance()->Write(LV_STATUS, "Connect OK");
			pXML_Data_Handler_Plugin->CreateChildren();
			if( bLocalMode )
				pXML_Data_Handler_Plugin->RunLocalMode();
			else
			{
				if(pXML_Data_Handler_Plugin->m_RequestHandlerThread)
					pthread_join(pXML_Data_Handler_Plugin->m_RequestHandlerThread, NULL);  // This function will return when the device is shutting down
			}
		...
		{
			bAppError = true;
			if( pXML_Data_Handler_Plugin->m_pEvent && pXML_Data_Handler_Plugin->m_pEvent->m_pClientSocket && pXML_Data_Handler_Plugin->m_pEvent->m_pClientSocket->m_eLastError==ClientSocket::cs_err_CannotConnect )
			{
				bAppError = false;
				bReload = false;
		...
		if( pXML_Data_Handler_Plugin->m_bReload )
			bReload=true;
