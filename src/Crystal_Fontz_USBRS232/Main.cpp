...
extern "C" {
	...
	{ 
		...
		if( bIsRuntimePlugin )
			return Crystal_Fontz_USBRS232::PK_DeviceTemplate_get_static();
		else
			return 0;
	}
}
...
int main(int argc, char* argv[]) 
{
	...
	cout << "Crystal_Fontz_USBRS232, v." << VERSION << endl
	...
	if (bError)
	{
		cout << "A Pluto DCE Device.  See www.plutohome.com/dce for details." << endl
			<< "Usage: Crystal_Fontz_USBRS232 [-r Router's IP] [-d My Device ID] [-l dcerouter|stdout|null|filename]" << endl
	...
	try
	{
		if( sLogger=="dcerouter" )
			LoggerWrapper::SetInstance(new ServerLogger(PK_Device, Crystal_Fontz_USBRS232::PK_DeviceTemplate_get_static(), sRouter_IP));
	...
	try
	{
		Crystal_Fontz_USBRS232 *pCrystal_Fontz_USBRS232 = new Crystal_Fontz_USBRS232(PK_Device, sRouter_IP,true,bLocalMode);
		if ( pCrystal_Fontz_USBRS232->GetConfig() && pCrystal_Fontz_USBRS232->Connect(pCrystal_Fontz_USBRS232->PK_DeviceTemplate_get()) ) 
		{
			g_pCommand_Impl=pCrystal_Fontz_USBRS232;
			g_pDeadlockHandler=DeadlockHandler;
			g_pSocketCrashHandler=SocketCrashHandler;
			LoggerWrapper::GetInstance()->Write(LV_STATUS, "Connect OK");
			pCrystal_Fontz_USBRS232->CreateChildren();
			if( bLocalMode )
				pCrystal_Fontz_USBRS232->RunLocalMode();
			else
			{
				if(pCrystal_Fontz_USBRS232->m_RequestHandlerThread)
					pthread_join(pCrystal_Fontz_USBRS232->m_RequestHandlerThread, NULL);  // This function will return when the device is shutting down
			}
		...
		{
			bAppError = true;
			if( pCrystal_Fontz_USBRS232->m_pEvent && pCrystal_Fontz_USBRS232->m_pEvent->m_pClientSocket && pCrystal_Fontz_USBRS232->m_pEvent->m_pClientSocket->m_eLastError==ClientSocket::cs_err_CannotConnect )
			{
				bAppError = false;
				bReload = false;
		...
		if( pCrystal_Fontz_USBRS232->m_bReload )
			bReload=true;
