...
extern "C" {
	...
	{ 
		...
		if( bIsRuntimePlugin )
			return CEC_Adaptor::PK_DeviceTemplate_get_static();
		else
			return 0;
	}
}
...
extern "C" {
	class Command_Impl *RegisterAsPlugIn(class Router *pRouter,int PK_Device,Logger *pPlutoLogger)
	{
		...
		CEC_Adaptor *pCEC_Adaptor = new CEC_Adaptor(PK_Device, "localhost",true,false,pRouter);
		if( pCEC_Adaptor->m_bQuit_get()|| !pCEC_Adaptor->GetConfig() )
		{
			delete pCEC_Adaptor;
			return NULL;
		}
...
int main(int argc, char* argv[]) 
{
	...
	cout << "CEC_Adaptor, v." << VERSION << endl
	...
	if (bError)
	{
		cout << "A Pluto DCE Device.  See www.plutohome.com/dce for details." << endl
			<< "Usage: CEC_Adaptor [-r Router's IP] [-d My Device ID] [-l dcerouter|stdout|null|filename]" << endl
	...
	try
	{
		if( sLogger=="dcerouter" )
			LoggerWrapper::SetInstance(new ServerLogger(PK_Device, CEC_Adaptor::PK_DeviceTemplate_get_static(), sRouter_IP));
	...
	try
	{
		...
		if ( pCEC_Adaptor->GetConfig() && pCEC_Adaptor->Connect(pCEC_Adaptor->PK_DeviceTemplate_get()) ) 
		{
			...
			pCEC_Adaptor->CreateChildren();
			if( bLocalMode )
				pCEC_Adaptor->RunLocalMode();
			else
			{
				if(pCEC_Adaptor->m_RequestHandlerThread)
					pthread_join(pCEC_Adaptor->m_RequestHandlerThread, NULL);  // This function will return when the device is shutting down
			}
		...
		{
			bAppError = true;
			if( pCEC_Adaptor->m_pEvent && pCEC_Adaptor->m_pEvent->m_pClientSocket && pCEC_Adaptor->m_pEvent->m_pClientSocket->m_eLastError==ClientSocket::cs_err_CannotConnect )
			{
				bAppError = false;
				bReload = false;
