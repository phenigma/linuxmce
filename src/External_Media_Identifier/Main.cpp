...
extern "C" {
	...
	{ 
		...
		if( bIsRuntimePlugin )
			return External_Media_Identifier::PK_DeviceTemplate_get_static();
		else
			return 0;
	}
}
...
extern "C" {
	class Command_Impl *RegisterAsPlugIn(class Router *pRouter,int PK_Device,Logger *pPlutoLogger)
	{
		...
		External_Media_Identifier *pExternal_Media_Identifier = new External_Media_Identifier(PK_Device, "localhost",true,false,pRouter);
		if( pExternal_Media_Identifier->m_bQuit_get()|| !pExternal_Media_Identifier->GetConfig() )
		{
			delete pExternal_Media_Identifier;
			return NULL;
		}
		else
		{
			g_pCommand_Impl=pExternal_Media_Identifier;
			g_pDeadlockHandler=Plugin_DeadlockHandler;
			g_pSocketCrashHandler=Plugin_SocketCrashHandler;
		}
		return pExternal_Media_Identifier;
	}
}
//<-dceag-plug-e->
...
int main(int argc, char* argv[]) 
{
	...
	cout << "External_Media_Identifier, v." << VERSION << endl
	...
	if (bError)
	{
		cout << "A Pluto DCE Device.  See www.plutohome.com/dce for details." << endl
			<< "Usage: External_Media_Identifier [-r Router's IP] [-d My Device ID] [-l dcerouter|stdout|null|filename]" << endl
	...
	try
	{
		if( sLogger=="dcerouter" )
			LoggerWrapper::SetInstance(new ServerLogger(PK_Device, External_Media_Identifier::PK_DeviceTemplate_get_static(), sRouter_IP));
	...
	try
	{
		External_Media_Identifier *pExternal_Media_Identifier = new External_Media_Identifier(PK_Device, sRouter_IP,true,bLocalMode);
		if ( pExternal_Media_Identifier->GetConfig() && pExternal_Media_Identifier->Connect(pExternal_Media_Identifier->PK_DeviceTemplate_get()) ) 
		{
			g_pCommand_Impl=pExternal_Media_Identifier;
			g_pDeadlockHandler=DeadlockHandler;
			g_pSocketCrashHandler=SocketCrashHandler;
			LoggerWrapper::GetInstance()->Write(LV_STATUS, "Connect OK");
			pExternal_Media_Identifier->CreateChildren();
			if( bLocalMode )
				pExternal_Media_Identifier->RunLocalMode();
			else
			{
				if(pExternal_Media_Identifier->m_RequestHandlerThread)
					pthread_join(pExternal_Media_Identifier->m_RequestHandlerThread, NULL);  // This function will return when the device is shutting down
			}
		...
		{
			bAppError = true;
			if( pExternal_Media_Identifier->m_pEvent && pExternal_Media_Identifier->m_pEvent->m_pClientSocket && pExternal_Media_Identifier->m_pEvent->m_pClientSocket->m_eLastError==ClientSocket::cs_err_CannotConnect )
			{
				bAppError = false;
				bReload = false;
		...
		if( pExternal_Media_Identifier->m_bReload )
			bReload=true;
