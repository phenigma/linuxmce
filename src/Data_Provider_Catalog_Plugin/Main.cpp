...
extern "C" {
	...
	{ 
		...
		if( bIsRuntimePlugin )
			return Data_Provider_Catalog_Plugin::PK_DeviceTemplate_get_static();
		else
			return 0;
	}
}
...
extern "C" {
	class Command_Impl *RegisterAsPlugIn(class Router *pRouter,int PK_Device,Logger *pPlutoLogger)
	{
		...
		Data_Provider_Catalog_Plugin *pData_Provider_Catalog_Plugin = new Data_Provider_Catalog_Plugin(PK_Device, "localhost",true,false,pRouter);
		if( pData_Provider_Catalog_Plugin->m_bQuit_get()|| !pData_Provider_Catalog_Plugin->GetConfig() )
		{
			delete pData_Provider_Catalog_Plugin;
			return NULL;
		}
		else
		{
			g_pCommand_Impl=pData_Provider_Catalog_Plugin;
			g_pDeadlockHandler=Plugin_DeadlockHandler;
			g_pSocketCrashHandler=Plugin_SocketCrashHandler;
		}
		return pData_Provider_Catalog_Plugin;
	}
}
//<-dceag-plug-e->
...
int main(int argc, char* argv[]) 
{
	...
	cout << "Data_Provider_Catalog_Plugin, v." << VERSION << endl
	...
	if (bError)
	{
		cout << "A Pluto DCE Device.  See www.plutohome.com/dce for details." << endl
			<< "Usage: Data_Provider_Catalog_Plugin [-r Router's IP] [-d My Device ID] [-l dcerouter|stdout|null|filename]" << endl
	...
	try
	{
		if( sLogger=="dcerouter" )
			LoggerWrapper::SetInstance(new ServerLogger(PK_Device, Data_Provider_Catalog_Plugin::PK_DeviceTemplate_get_static(), sRouter_IP));
	...
	try
	{
		Data_Provider_Catalog_Plugin *pData_Provider_Catalog_Plugin = new Data_Provider_Catalog_Plugin(PK_Device, sRouter_IP,true,bLocalMode);
		if ( pData_Provider_Catalog_Plugin->GetConfig() && pData_Provider_Catalog_Plugin->Connect(pData_Provider_Catalog_Plugin->PK_DeviceTemplate_get()) ) 
		{
			g_pCommand_Impl=pData_Provider_Catalog_Plugin;
			g_pDeadlockHandler=DeadlockHandler;
			g_pSocketCrashHandler=SocketCrashHandler;
			LoggerWrapper::GetInstance()->Write(LV_STATUS, "Connect OK");
			pData_Provider_Catalog_Plugin->CreateChildren();
			if( bLocalMode )
				pData_Provider_Catalog_Plugin->RunLocalMode();
			else
			{
				if(pData_Provider_Catalog_Plugin->m_RequestHandlerThread)
					pthread_join(pData_Provider_Catalog_Plugin->m_RequestHandlerThread, NULL);  // This function will return when the device is shutting down
			}
		...
		{
			bAppError = true;
			if( pData_Provider_Catalog_Plugin->m_pEvent && pData_Provider_Catalog_Plugin->m_pEvent->m_pClientSocket && pData_Provider_Catalog_Plugin->m_pEvent->m_pClientSocket->m_eLastError==ClientSocket::cs_err_CannotConnect )
			{
				bAppError = false;
				bReload = false;
		...
		if( pData_Provider_Catalog_Plugin->m_bReload )
			bReload=true;
