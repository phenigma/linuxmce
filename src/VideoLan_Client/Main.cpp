...
extern "C" {
	class Command_Impl *RegisterAsPlugIn(class Router *pRouter,int PK_Device,Logger *pPlutoLogger)
	{
		...
		if( pVideoLan_Client->m_bQuit_get()|| !pVideoLan_Client->GetConfig() )
		{
...
int main(int argc, char* argv[]) 
{
	...
	try
	{
		if( sLogger=="dcerouter" )
			LoggerWrapper::SetInstance(new ServerLogger(PK_Device, VideoLan_Client::PK_DeviceTemplate_get_static(), sRouter_IP));
	...
	try
	{
		VideoLan_Client *pVideoLan_Client = new VideoLan_Client(PK_Device, sRouter_IP,true,bLocalMode);
		if ( pVideoLan_Client->GetConfig() && pVideoLan_Client->Connect(pVideoLan_Client->PK_DeviceTemplate_get()) ) 
		{
			...
			if( bLocalMode )
				pVideoLan_Client->RunLocalMode();
			else
			{
				if(pVideoLan_Client->m_RequestHandlerThread)
		...
		{
			bAppError = true;
			if( pVideoLan_Client->m_pEvent && pVideoLan_Client->m_pEvent->m_pClientSocket && pVideoLan_Client->m_pEvent->m_pClientSocket->m_eLastError==ClientSocket::cs_err_CannotConnect )
			{
				bAppError = false;
				bReload = false;
