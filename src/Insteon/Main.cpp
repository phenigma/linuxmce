...
extern "C" {
	...
	{ 
		...
		if( bIsRuntimePlugin )
			return Insteon::PK_DeviceTemplate_get_static();
		else
			return 0;
	}
}
...
extern "C" {
	class Command_Impl *RegisterAsPlugIn(class Router *pRouter,int PK_Device,Logger *pPlutoLogger)
	{
		...
		Insteon *pInsteon = new Insteon(PK_Device, "localhost",true,false,pRouter);
		if( pInsteon->m_bQuit_get()|| !pInsteon->GetConfig() )
		{
			delete pInsteon;
			return NULL;
		}
...
int main(int argc, char* argv[]) 
{
	...
	if (bError)
	{
		cout << "A Pluto DCE Device.  See www.plutohome.com/dce for details." << endl
			<< "Usage: Insteon [-r Router's IP] [-d My Device ID] [-l dcerouter|stdout|null|filename]" << endl
	...
	try
	{
		if( sLogger=="dcerouter" )
			LoggerWrapper::SetInstance(new ServerLogger(PK_Device, Insteon::PK_DeviceTemplate_get_static(), sRouter_IP));
	...
	try
	{
		Insteon *pInsteon = new Insteon(PK_Device, sRouter_IP,true,bLocalMode);
		if ( pInsteon->GetConfig() && pInsteon->Connect(pInsteon->PK_DeviceTemplate_get()) ) 
		{
			g_pCommand_Impl=pInsteon;
			g_pDeadlockHandler=DeadlockHandler;
			g_pSocketCrashHandler=SocketCrashHandler;
			LoggerWrapper::GetInstance()->Write(LV_STATUS, "Connect OK");
			pInsteon->CreateChildren();
			if( bLocalMode )
				pInsteon->RunLocalMode();
			else
			{
				if(pInsteon->m_RequestHandlerThread)
					pthread_join(pInsteon->m_RequestHandlerThread, NULL);  // This function will return when the device is shutting down
			}
		...
		{
			bAppError = true;
			if( pInsteon->m_pEvent && pInsteon->m_pEvent->m_pClientSocket && pInsteon->m_pEvent->m_pClientSocket->m_eLastError==ClientSocket::cs_err_CannotConnect )
			{
				bAppError = false;
				bReload = false;
