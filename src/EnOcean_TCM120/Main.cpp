...
extern "C" {
	...
	{ 
		...
		if( bIsRuntimePlugin )
			return EnOcean_TCM120::PK_DeviceTemplate_get_static();
		else
			return 0;
	}
}
...
extern "C" {
	class Command_Impl *RegisterAsPlugIn(class Router *pRouter,int PK_Device,Logger *pPlutoLogger)
	{
		...
		EnOcean_TCM120 *pEnOcean_TCM120 = new EnOcean_TCM120(PK_Device, "localhost",true,false,pRouter);
		if( pEnOcean_TCM120->m_bQuit_get()|| !pEnOcean_TCM120->GetConfig() )
		{
			delete pEnOcean_TCM120;
			return NULL;
		}
		else
		{
			g_pCommand_Impl=pEnOcean_TCM120;
			g_pDeadlockHandler=Plugin_DeadlockHandler;
			g_pSocketCrashHandler=Plugin_SocketCrashHandler;
		}
		return pEnOcean_TCM120;
	}
}
//<-dceag-plug-e->
...
int main(int argc, char* argv[]) 
{
	...
	cout << "EnOcean_TCM120, v." << VERSION << endl
		<< "LinuxMCE EnOcean DCE Interface device" << endl << endl;
	...
	if (bError)
	{
		cout << "A Pluto DCE Device.  See www.plutohome.com/dce for details." << endl
			<< "Usage: EnOcean_TCM120 [-r Router's IP] [-d My Device ID] [-l dcerouter|stdout|null|filename]" << endl
	...
	try
	{
		if( sLogger=="dcerouter" )
			LoggerWrapper::SetInstance(new ServerLogger(PK_Device, EnOcean_TCM120::PK_DeviceTemplate_get_static(), sRouter_IP));
	...
	try
	{
		EnOcean_TCM120 *pEnOcean_TCM120 = new EnOcean_TCM120(PK_Device, sRouter_IP,true,bLocalMode);
		if ( pEnOcean_TCM120->GetConfig() && pEnOcean_TCM120->Connect(pEnOcean_TCM120->PK_DeviceTemplate_get()) ) 
		{
			g_pCommand_Impl=pEnOcean_TCM120;
			g_pDeadlockHandler=DeadlockHandler;
			g_pSocketCrashHandler=SocketCrashHandler;
			LoggerWrapper::GetInstance()->Write(LV_STATUS, "Connect OK");
			pEnOcean_TCM120->CreateChildren();
			if( bLocalMode )
				pEnOcean_TCM120->RunLocalMode();
			else
			{
				if(pEnOcean_TCM120->m_RequestHandlerThread)
					pthread_join(pEnOcean_TCM120->m_RequestHandlerThread, NULL);  // This function will return when the device is shutting down
			}
		...
		{
			bAppError = true;
			if( pEnOcean_TCM120->m_pEvent && pEnOcean_TCM120->m_pEvent->m_pClientSocket && pEnOcean_TCM120->m_pEvent->m_pClientSocket->m_eLastError==ClientSocket::cs_err_CannotConnect )
			{
				bAppError = false;
				bReload = false;
