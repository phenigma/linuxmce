...
values_list_comma_separated = values_list_comma_separated + pRow->PK_Software_asSQL()+", "+pRow->PackageName_asSQL()+", "+pRow->psc_id_asSQL()+", "+pRow->psc_batch_asSQL()+", "+pRow->psc_user_asSQL()+", "+pRow->psc_frozen_asSQL()+", "+pRow->psc_restrict_asSQL();
		...
		{	
			...
			cerr << "Cannot perform query: [" << query << "] " << database->m_sLastMySqlError << endl;
				LoggerWrapper::GetInstance()->Write(LV_CRITICAL,"Table_Software::Commit Cannot perform query [%s] %s",query.c_str(),database->m_sLastMySqlError.c_str());
...
else 	LoggerWrapper::GetInstance()->Write(LV_CRITICAL,"PK_Software is auto increment but has no value %s",database->m_sLastMySqlError.c_str());	
...
update_values_list = update_values_list + "`PK_Software`="+pRow->PK_Software_asSQL()+", `PackageName`="+pRow->PackageName_asSQL()+", `psc_id`="+pRow->psc_id_asSQL()+", `psc_batch`="+pRow->psc_batch_asSQL()+", `psc_user`="+pRow->psc_user_asSQL()+", `psc_frozen`="+pRow->psc_frozen_asSQL()+", `psc_restrict`="+pRow->psc_restrict_asSQL();
		...
		{	
			database->m_sLastMySqlError = mysql_error(database->m_pMySQL);
			cerr << "Cannot perform query: [" << query << "] " << database->m_sLastMySqlError << endl;
				LoggerWrapper::GetInstance()->Write(LV_CRITICAL,"Table_Software::Commit Cannot perform update query [%s] %s",query.c_str(),database->m_sLastMySqlError.c_str());
...
condition = condition + "`PK_Software`=" + tmp_PK_Software;
		...
		{	
			database->m_sLastMySqlError = mysql_error(database->m_pMySQL);
			cerr << "Cannot perform query: [" << query << "] " << database->m_sLastMySqlError << endl;
				LoggerWrapper::GetInstance()->Write(LV_CRITICAL,"Table_Software::Commit Cannot perform delete query [%s] %s",query.c_str(),database->m_sLastMySqlError.c_str());
			return false;
		}	
...
bool Table_Software::GetRows(string where_statement,vector<class Row_Software*> *rows)
{
	...
	{	
		database->m_sLastMySqlError = mysql_error(database->m_pMySQL);
		cerr << "Cannot perform query: [" << query << "] " << database->m_sLastMySqlError << endl;
		LoggerWrapper::GetInstance()->Write(LV_CRITICAL,"Table_Software::GetRows Cannot perform query [%s] %s",query.c_str(),database->m_sLastMySqlError.c_str());
		return false;
	}	
...
condition = condition + "`PK_Software`=" + tmp_PK_Software;
	...
	{	
		database->m_sLastMySqlError = mysql_error(database->m_pMySQL);
		cerr << "Cannot perform query: [" << query << "] " << database->m_sLastMySqlError << endl;
		LoggerWrapper::GetInstance()->Write(LV_CRITICAL,"Table_Software::FetchRow Cannot perform query [%s] %s",query.c_str(),database->m_sLastMySqlError.c_str());
		return NULL;
	}	
