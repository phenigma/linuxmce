#ifndef __Define_psc_local_bathdr_H__
#define __Define_psc_local_bathdr_H__


#define PSC_LOCAL_BATHDR_TABLE "psc_local_bathdr"
#define PSC_LOCAL_BATHDR_PK_PSC_LOCAL_BATHDR_FIELD "PK_psc_local_bathdr"
#define PSC_LOCAL_BATHDR_PK_PSC_LOCAL_BATHDR_TABLE_FIELD "psc_local_bathdr.PK_psc_local_bathdr"
#define PSC_LOCAL_BATHDR_IPADDRESS_FIELD "IPAddress"
#define PSC_LOCAL_BATHDR_IPADDRESS_TABLE_FIELD "psc_local_bathdr.IPAddress"
#define PSC_LOCAL_BATHDR_DATE_FIELD "date"
#define PSC_LOCAL_BATHDR_DATE_TABLE_FIELD "psc_local_bathdr.date"
#define PSC_LOCAL_BATHDR_COMMENTS_FIELD "comments"
#define PSC_LOCAL_BATHDR_COMMENTS_TABLE_FIELD "psc_local_bathdr.comments"


#endif

