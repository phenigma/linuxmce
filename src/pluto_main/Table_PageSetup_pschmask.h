...
#ifndef __Table_PageSetup_pschmask_H__
#define __Table_PageSetup_pschmask_H__

#include "TableRow.h"
#include "Database_pluto_main.h"
#include "PlutoUtils/MultiThreadIncludes.h"
#include "Define_PageSetup_pschmask.h"
...
class DECLSPECIFIER Table_PageSetup_pschmask : public TableBase , DoubleLongKeyBase
{
private:
...
public:
	Table_PageSetup_pschmask(Database_pluto_main *pDatabase):database(pDatabase)
	{
	};
	~Table_PageSetup_pschmask();

private:		
	friend class Row_PageSetup_pschmask;
	struct Key
	{
		friend class Row_PageSetup_pschmask;
		long int pk_psc_id;
long int pk_psc_batch;
		...
		Key(class Row_PageSetup_pschmask *pRow);
	};
	struct Key_Less
	{			
		bool operator()(const Table_PageSetup_pschmask::Key &key1, const Table_PageSetup_pschmask::Key &key2) const;
	};	
...
public:				
	...
	bool GetRows(string where_statement,vector<class Row_PageSetup_pschmask*> *rows);
	class Row_PageSetup_pschmask* AddRow();
	Database_pluto_main *Database_pluto_main_get() { return database; }
	
		
	class Row_PageSetup_pschmask* GetRow(long int in_psc_id, long int in_psc_batch);
	

private:	
	
		
	class Row_PageSetup_pschmask* FetchRow(DoubleLongKey &key);
		
			
};

class DECLSPECIFIER Row_PageSetup_pschmask : public TableRow, public SerializeClass
	{
		friend struct Table_PageSetup_pschmask::Key;
		friend class Table_PageSetup_pschmask;
	private:
		Table_PageSetup_pschmask *table;
		
		short int m_PK_PageSetup;
short int m_FK_PageSetup_Parent;
short int m_Website;
...
long int psc_restrict_get();
		...
		void PK_PageSetup_set(short int val);
void FK_PageSetup_Parent_set(short int val);
...
void pageURL_set(short int val);
...
void psc_restrict_setNull(bool val);
		...
		Row_PageSetup_pschmask(Table_PageSetup_pschmask *pTable);
	
		bool IsDeleted(){return is_deleted;};
		bool IsModified(){return is_modified;};			
		class Table_PageSetup_pschmask *Table_PageSetup_pschmask_get() { return table; };
