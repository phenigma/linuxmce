...
string Row_psc_document_bathdr::IPAddress_get(){PLUTO_SAFETY_LOCK(M, table->m_Mutex);

return m_IPAddress;}
string Row_psc_document_bathdr::date_get(){PLUTO_SAFETY_LOCK(M, table->m_Mutex);

return m_date;}
string Row_psc_document_bathdr::comments_get(){PLUTO_SAFETY_LOCK(M, table->m_Mutex);
...
void Row_psc_document_bathdr::IPAddress_set(string val){PLUTO_SAFETY_LOCK(M, table->m_Mutex);
...
void Row_psc_document_bathdr::comments_set(string val){PLUTO_SAFETY_LOCK(M, table->m_Mutex);

m_comments = val; is_modified=true; is_null[3]=false;}

		
bool Row_psc_document_bathdr::IPAddress_isNull() {PLUTO_SAFETY_LOCK(M, table->m_Mutex);

return is_null[1];}
bool Row_psc_document_bathdr::date_isNull() {PLUTO_SAFETY_LOCK(M, table->m_Mutex);

return is_null[2];}
bool Row_psc_document_bathdr::comments_isNull() {PLUTO_SAFETY_LOCK(M, table->m_Mutex);

return is_null[3];}

			
void Row_psc_document_bathdr::IPAddress_setNull(bool val){PLUTO_SAFETY_LOCK(M, table->m_Mutex);

is_null[1]=val;}
void Row_psc_document_bathdr::date_setNull(bool val){PLUTO_SAFETY_LOCK(M, table->m_Mutex);

is_null[2]=val;}
void Row_psc_document_bathdr::comments_setNull(bool val){PLUTO_SAFETY_LOCK(M, table->m_Mutex);
...
bool Table_psc_document_bathdr::GetRows(string where_statement,vector<class Row_psc_document_bathdr*> *rows)
{
	...
	if( StringUtils::StartsWith(where_statement,"where ",true) || StringUtils::StartsWith(where_statement,"join ",true) )
		query = "select * from psc_document_bathdr " + where_statement;
	else if( StringUtils::StartsWith(where_statement,"select ",true) )
		query = where_statement;
	else
		query = "select * from psc_document_bathdr where " + where_statement;
