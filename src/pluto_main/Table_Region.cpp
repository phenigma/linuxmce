...
values_list_comma_separated = values_list_comma_separated + pRow->PK_Region_asSQL()+", "+pRow->FK_Country_asSQL()+", "+pRow->Region_asSQL()+", "+pRow->Code_asSQL()+", "+pRow->ADM1Code_asSQL();
		...
		{	
			...
			cerr << "Cannot perform query: [" << query << "] " << database->m_sLastMySqlError << endl;
				LoggerWrapper::GetInstance()->Write(LV_CRITICAL,"Table_Region::Commit Cannot perform query [%s] %s",query.c_str(),database->m_sLastMySqlError.c_str());
		...
		if (mysql_affected_rows(database->m_pMySQL)!=0)
		{
			...
			if (id!=0)
		pRow->m_PK_Region=id;
else 	LoggerWrapper::GetInstance()->Write(LV_CRITICAL,"PK_Region is auto increment but has no value %s",database->m_sLastMySqlError.c_str());	
...
update_values_list = update_values_list + "`PK_Region`="+pRow->PK_Region_asSQL()+", `FK_Country`="+pRow->FK_Country_asSQL()+", `Region`="+pRow->Region_asSQL()+", `Code`="+pRow->Code_asSQL()+", `ADM1Code`="+pRow->ADM1Code_asSQL();
		...
		{	
			database->m_sLastMySqlError = mysql_error(database->m_pMySQL);
			cerr << "Cannot perform query: [" << query << "] " << database->m_sLastMySqlError << endl;
				LoggerWrapper::GetInstance()->Write(LV_CRITICAL,"Table_Region::Commit Cannot perform update query [%s] %s",query.c_str(),database->m_sLastMySqlError.c_str());
...
condition = condition + "`PK_Region`=" + tmp_PK_Region;
		...
		{	
			database->m_sLastMySqlError = mysql_error(database->m_pMySQL);
			cerr << "Cannot perform query: [" << query << "] " << database->m_sLastMySqlError << endl;
				LoggerWrapper::GetInstance()->Write(LV_CRITICAL,"Table_Region::Commit Cannot perform delete query [%s] %s",query.c_str(),database->m_sLastMySqlError.c_str());
			return false;
		}	
...
bool Table_Region::GetRows(string where_statement,vector<class Row_Region*> *rows)
{
	...
	{	
		database->m_sLastMySqlError = mysql_error(database->m_pMySQL);
		cerr << "Cannot perform query: [" << query << "] " << database->m_sLastMySqlError << endl;
		LoggerWrapper::GetInstance()->Write(LV_CRITICAL,"Table_Region::GetRows Cannot perform query [%s] %s",query.c_str(),database->m_sLastMySqlError.c_str());
		return false;
	}	
...
condition = condition + "`PK_Region`=" + tmp_PK_Region;
	...
	{	
		database->m_sLastMySqlError = mysql_error(database->m_pMySQL);
		cerr << "Cannot perform query: [" << query << "] " << database->m_sLastMySqlError << endl;
		LoggerWrapper::GetInstance()->Write(LV_CRITICAL,"Table_Region::FetchRow Cannot perform query [%s] %s",query.c_str(),database->m_sLastMySqlError.c_str());
		return NULL;
	}	
