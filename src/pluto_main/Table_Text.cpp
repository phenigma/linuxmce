...
return m_Define;}
short int Row_Text::AddToOrbiter_get(){PLUTO_SAFETY_LOCK(M, table->m_Mutex);
...
void Row_Text::AddToOrbiter_set(short int val){PLUTO_SAFETY_LOCK(M, table->m_Mutex);
...
bool Row_Text::AddToOrbiter_isNull() {PLUTO_SAFETY_LOCK(M, table->m_Mutex);
...
void Row_Text::AddToOrbiter_setNull(bool val){PLUTO_SAFETY_LOCK(M, table->m_Mutex);
is_null[4]=val;
is_modified=true;
}
