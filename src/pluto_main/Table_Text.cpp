...
char *buf = new char[81];
mysql_real_escape_string(table->database->m_pMySQL, buf, m_Define.c_str(), (unsigned long) min(40,m_Define.size()));
string s=string()+"\""+buf+"\"";
delete buf;
return s;
}
...
values_list_comma_separated = values_list_comma_separated + pRow->PK_Text_asSQL()+", "+pRow->FK_TextCategory_asSQL()+", "+pRow->Description_asSQL()+", "+pRow->Define_asSQL()+", "+pRow->AddToOrbiter_asSQL()+", "+pRow->psc_id_asSQL()+", "+pRow->psc_batch_asSQL()+", "+pRow->psc_user_asSQL()+", "+pRow->psc_frozen_asSQL();

	
		string query = "insert into Text (`PK_Text`, `FK_TextCategory`, `Description`, `Define`, `AddToOrbiter`, `psc_id`, `psc_batch`, `psc_user`, `psc_frozen`) values ("+
...
update_values_list = update_values_list + "`PK_Text`="+pRow->PK_Text_asSQL()+", `FK_TextCategory`="+pRow->FK_TextCategory_asSQL()+", `Description`="+pRow->Description_asSQL()+", `Define`="+pRow->Define_asSQL()+", `AddToOrbiter`="+pRow->AddToOrbiter_asSQL()+", `psc_id`="+pRow->psc_id_asSQL()+", `psc_batch`="+pRow->psc_batch_asSQL()+", `psc_user`="+pRow->psc_user_asSQL()+", `psc_frozen`="+pRow->psc_frozen_asSQL();
