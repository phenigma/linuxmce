...
#ifndef __Table_Package_Directory_pschist_H__
#define __Table_Package_Directory_pschist_H__

#include "TableRow.h"
#include "Database_pluto_main.h"
#include "PlutoUtils/MultiThreadIncludes.h"
#include "Define_Package_Directory_pschist.h"
...
class DECLSPECIFIER Table_Package_Directory_pschist : public TableBase , DoubleLongKeyBase
{
private:
...
public:
	Table_Package_Directory_pschist(Database_pluto_main *pDatabase):database(pDatabase)
	{
	};
	~Table_Package_Directory_pschist();

private:		
	friend class Row_Package_Directory_pschist;
	struct Key
	{
		friend class Row_Package_Directory_pschist;
		long int pk_psc_id;
long int pk_psc_batch;
		...
		Key(class Row_Package_Directory_pschist *pRow);
	};
	struct Key_Less
	{			
		bool operator()(const Table_Package_Directory_pschist::Key &key1, const Table_Package_Directory_pschist::Key &key2) const;
	};	
...
public:				
	...
	bool GetRows(string where_statement,vector<class Row_Package_Directory_pschist*> *rows);
	class Row_Package_Directory_pschist* AddRow();
	Database_pluto_main *Database_pluto_main_get() { return database; }
	
		
	class Row_Package_Directory_pschist* GetRow(long int in_psc_id, long int in_psc_batch);
	

private:	
	
		
	class Row_Package_Directory_pschist* FetchRow(DoubleLongKey &key);
		
			
};

class DECLSPECIFIER Row_Package_Directory_pschist : public TableRow, public SerializeClass
	{
		friend struct Table_Package_Directory_pschist::Key;
		friend class Table_Package_Directory_pschist;
	private:
		Table_Package_Directory_pschist *table;
...
void psc_restrict_setNull(bool val);
		...
		Row_Package_Directory_pschist(Table_Package_Directory_pschist *pTable);
	
		bool IsDeleted(){return is_deleted;};
		bool IsModified(){return is_modified;};			
		class Table_Package_Directory_pschist *Table_Package_Directory_pschist_get() { return table; };
...
class Row_Distro* FK_Distro_getrow();
		...
		void SetupSerialization(int iSC_Version) {
			StartSerializeList() + m_PK_Package_Directory+ m_FK_Package+ m_FK_Directory+ m_FK_OperatingSystem+ m_FK_Distro+ m_Path+ m_InputPath+ m_FlipSource+ m_GenerateDoxygen+ m_psc_id+ m_psc_batch+ m_psc_user+ m_psc_frozen+ m_psc_mod+ m_psc_toc+ m_psc_restrict;
		}
	private:
