...
Table_CriteriaParmNesting_D::~Table_CriteriaParmNesting_D()
{
	...
	{
		Row_CriteriaParmNesting_D *pRow = (Row_CriteriaParmNesting_D *) (*it).second;
		delete pRow;
	}
...
void Row_CriteriaParmNesting_D::Delete()
{
	...
	Row_CriteriaParmNesting_D *pRow = this; // Needed so we will have only 1 version of get_primary_fields_assign_from_row
	
	if (!is_deleted)
		if (is_added)	
		{	
			vector<TableRow*>::iterator i;	
			for (i = table->addedRows.begin(); (i!=table->addedRows.end()) && ( (Row_CriteriaParmNesting_D *) *i != this); i++);
		...
		{
			SingleLongKey key(pRow->m_PK_CriteriaParmNesting_D);
...
is_null[0] = false;
is_null[1] = true;
m_FK_CriteriaParmNesting_D_Parent = 0;
m_IsAnd = 0;
is_null[2] = false;
m_IsNot = 0;
is_null[3] = false;
...
return m_psc_mod;}
long int Row_CriteriaParmNesting_D::psc_restrict_get(){PLUTO_SAFETY_LOCK_ERRORSONLY(sl,table->database->m_MySqlMutex);
...
void Row_CriteriaParmNesting_D::psc_restrict_set(long int val){PLUTO_SAFETY_LOCK_ERRORSONLY(sl,table->database->m_MySqlMutex);
...
return is_null[7];}
bool Row_CriteriaParmNesting_D::psc_restrict_isNull() {PLUTO_SAFETY_LOCK_ERRORSONLY(sl,table->database->m_MySqlMutex);
...
void Row_CriteriaParmNesting_D::psc_restrict_setNull(bool val){PLUTO_SAFETY_LOCK_ERRORSONLY(sl,table->database->m_MySqlMutex);
is_null[9]=val;
is_modified=true;
}
...
string Row_CriteriaParmNesting_D::psc_restrict_asSQL()
{
PLUTO_SAFETY_LOCK_ERRORSONLY(sl,table->database->m_MySqlMutex);
...
bool Table_CriteriaParmNesting_D::Commit(bool bDeleteFailedModifiedRow,bool bDeleteFailedInsertRow)
{
...
//insert added
	while (!addedRows.empty())
	{
		...
		Row_CriteriaParmNesting_D *pRow = (Row_CriteriaParmNesting_D *)*i;
	
		
string values_list_comma_separated;
values_list_comma_separated = values_list_comma_separated + pRow->PK_CriteriaParmNesting_D_asSQL()+", "+pRow->FK_CriteriaParmNesting_D_Parent_asSQL()+", "+pRow->IsAnd_asSQL()+", "+pRow->IsNot_asSQL()+", "+pRow->psc_id_asSQL()+", "+pRow->psc_batch_asSQL()+", "+pRow->psc_user_asSQL()+", "+pRow->psc_frozen_asSQL()+", "+pRow->psc_restrict_asSQL();

	
		string query = "insert into CriteriaParmNesting_D (`PK_CriteriaParmNesting_D`, `FK_CriteriaParmNesting_D_Parent`, `IsAnd`, `IsNot`, `psc_id`, `psc_batch`, `psc_user`, `psc_frozen`, `psc_restrict`) values ("+
...
//update modified
	...
	{
		Row_CriteriaParmNesting_D* pRow = (Row_CriteriaParmNesting_D*) (*i).second;	
		SingleLongKey key(pRow->m_PK_CriteriaParmNesting_D);

		char tmp_PK_CriteriaParmNesting_D[32];
sprintf(tmp_PK_CriteriaParmNesting_D, "%li", key.pk);
...
condition = condition + "`PK_CriteriaParmNesting_D`=" + tmp_PK_CriteriaParmNesting_D;
		...
		pRow = (Row_CriteriaParmNesting_D*) (*i).second;;
		delete pRow;
...
bool Table_CriteriaParmNesting_D::GetRows(string where_statement,vector<class Row_CriteriaParmNesting_D*> *rows)
{
	...
	if( StringUtils::StartsWith(where_statement,"where ",true) || 
		...
		query = "select `CriteriaParmNesting_D`.* from CriteriaParmNesting_D";
...
Row_CriteriaParmNesting_D* Table_CriteriaParmNesting_D::GetRow(long int in_PK_CriteriaParmNesting_D)
{
	...
	SingleLongKey row_key(in_PK_CriteriaParmNesting_D);
	...
	//row is cached
	if (i!=cachedRows.end())
		return (Row_CriteriaParmNesting_D*) (*i).second;
...
Row_CriteriaParmNesting_D* Table_CriteriaParmNesting_D::FetchRow(SingleLongKey &key)
{
	PLUTO_SAFETY_LOCK_ERRORSONLY(sl,database->m_MySqlMutex);
