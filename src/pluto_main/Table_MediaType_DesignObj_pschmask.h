...
#ifndef __Table_MediaType_DesignObj_pschmask_H__
#define __Table_MediaType_DesignObj_pschmask_H__

#include "TableRow.h"
#include "Database_pluto_main.h"
#include "PlutoUtils/MultiThreadIncludes.h"
#include "Define_MediaType_DesignObj_pschmask.h"
...
class DECLSPECIFIER Table_MediaType_DesignObj_pschmask : public TableBase , DoubleLongKeyBase
{
private:
...
public:
	Table_MediaType_DesignObj_pschmask(Database_pluto_main *pDatabase):database(pDatabase)
	{
	};
	~Table_MediaType_DesignObj_pschmask();

private:		
	friend class Row_MediaType_DesignObj_pschmask;
	struct Key
	{
		friend class Row_MediaType_DesignObj_pschmask;
		long int pk_psc_id;
long int pk_psc_batch;
		...
		Key(class Row_MediaType_DesignObj_pschmask *pRow);
	};
	struct Key_Less
	{			
		bool operator()(const Table_MediaType_DesignObj_pschmask::Key &key1, const Table_MediaType_DesignObj_pschmask::Key &key2) const;
	};	
...
public:				
	...
	bool GetRows(string where_statement,vector<class Row_MediaType_DesignObj_pschmask*> *rows);
	class Row_MediaType_DesignObj_pschmask* AddRow();
	Database_pluto_main *Database_pluto_main_get() { return database; }
	
		
	class Row_MediaType_DesignObj_pschmask* GetRow(long int in_psc_id, long int in_psc_batch);
	

private:	
	
		
	class Row_MediaType_DesignObj_pschmask* FetchRow(DoubleLongKey &key);
		
			
};

class DECLSPECIFIER Row_MediaType_DesignObj_pschmask : public TableRow, public SerializeClass
	{
		friend struct Table_MediaType_DesignObj_pschmask::Key;
		friend class Table_MediaType_DesignObj_pschmask;
	private:
		Table_MediaType_DesignObj_pschmask *table;
		
		short int m_PK_MediaType_DesignObj;
...
long int m_psc_restrict;
	...
	public:
		short int PK_MediaType_DesignObj_get();
...
long int psc_restrict_get();
		...
		void PK_MediaType_DesignObj_set(short int val);
...
void psc_restrict_set(long int val);
		...
		bool PK_MediaType_DesignObj_isNull();
...
bool psc_restrict_isNull();
		...
		void PK_MediaType_DesignObj_setNull(bool val);
...
void psc_restrict_setNull(bool val);
		...
		Row_MediaType_DesignObj_pschmask(Table_MediaType_DesignObj_pschmask *pTable);
	
		bool IsDeleted(){return is_deleted;};
		bool IsModified(){return is_modified;};			
		class Table_MediaType_DesignObj_pschmask *Table_MediaType_DesignObj_pschmask_get() { return table; };
...
class Row_Skin* FK_Skin_getrow();
		...
		void SetupSerialization(int iSC_Version) {
			StartSerializeList() + m_PK_MediaType_DesignObj+ m_FK_MediaType+ m_FK_Screen+ m_FK_DesignObj_Popup+ m_FK_Screen_FileList+ m_FK_Screen_OSD+ m_FK_Screen_Alt+ m_FK_Screen_Alt_OSD+ m_FK_Screen_OSD_Speed+ m_FK_Screen_OSD_Track+ m_Description+ m_FK_Skin+ m_UIVersion+ m_psc_id+ m_psc_batch+ m_psc_user+ m_psc_frozen+ m_psc_mod+ m_psc_restrict;
		}
	private:
		void SetDefaultValues();
		
		string PK_MediaType_DesignObj_asSQL();
