...
class DECLSPECIFIER Table_psc_local_repset : public TableBase , SingleLongKeyBase
{
private:
...
public:
	Table_psc_local_repset(Database_pluto_main *pDatabase):database(pDatabase)
	{
	};
...
private:	
	...
	class Row_psc_local_repset* FetchRow(SingleLongKey &key);
		
			
};

class DECLSPECIFIER Row_psc_local_repset : public TableRow, public SerializeClass
	{
...
void Value_set(string val);
		...
		void SetupSerialization(int iSC_Version) {
			StartSerializeList() + m_PK_psc_local_repset+ m_Setting+ m_Value;
		}
	private:
