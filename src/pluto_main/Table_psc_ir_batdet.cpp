...
values_list_comma_separated = values_list_comma_separated + pRow->PK_psc_ir_batdet_asSQL()+", "+pRow->FK_psc_ir_bathdr_asSQL()+", "+pRow->Tablename_asSQL()+", "+pRow->New_asSQL()+", "+pRow->Deleted_asSQL()+", "+pRow->Modified_asSQL()+", "+pRow->FK_psc_ir_bathdr_orig_asSQL()+", "+pRow->FK_psc_ir_bathdr_auth_asSQL()+", "+pRow->FK_psc_ir_bathdr_unauth_asSQL();
		...
		{	
			...
			if( g_pPlutoLogger )
				g_pPlutoLogger->Write(LV_CRITICAL,"Table_psc_ir_batdet::Commit Cannot perform query [%s] %s",query.c_str(),database->m_sLastMySqlError.c_str());
		...
		g_pPlutoLogger->Write(LV_CRITICAL,"PK_psc_ir_batdet is auto increment but has no value %s",database->m_sLastMySqlError.c_str());	
...
update_values_list = update_values_list + "`PK_psc_ir_batdet`="+pRow->PK_psc_ir_batdet_asSQL()+", `FK_psc_ir_bathdr`="+pRow->FK_psc_ir_bathdr_asSQL()+", `Tablename`="+pRow->Tablename_asSQL()+", `New`="+pRow->New_asSQL()+", `Deleted`="+pRow->Deleted_asSQL()+", `Modified`="+pRow->Modified_asSQL()+", `FK_psc_ir_bathdr_orig`="+pRow->FK_psc_ir_bathdr_orig_asSQL()+", `FK_psc_ir_bathdr_auth`="+pRow->FK_psc_ir_bathdr_auth_asSQL()+", `FK_psc_ir_bathdr_unauth`="+pRow->FK_psc_ir_bathdr_unauth_asSQL();
		...
		{	
			...
			cerr << "Cannot perform query: [" << query << "] " << database->m_sLastMySqlError << endl;
			if( g_pPlutoLogger )
				g_pPlutoLogger->Write(LV_CRITICAL,"Table_psc_ir_batdet::Commit Cannot perform update query [%s] %s",query.c_str(),database->m_sLastMySqlError.c_str());
...
condition = condition + "`PK_psc_ir_batdet`=" + tmp_PK_psc_ir_batdet;
		...
		{	
			...
			cerr << "Cannot perform query: [" << query << "] " << database->m_sLastMySqlError << endl;
			if( g_pPlutoLogger )
				g_pPlutoLogger->Write(LV_CRITICAL,"Table_psc_ir_batdet::Commit Cannot perform delete query [%s] %s",query.c_str(),database->m_sLastMySqlError.c_str());
			return false;
		}	
...
bool Table_psc_ir_batdet::GetRows(string where_statement,vector<class Row_psc_ir_batdet*> *rows)
{
	...
	{	
		...
		cerr << "Cannot perform query: [" << query << "] " << database->m_sLastMySqlError << endl;
		if( g_pPlutoLogger )
			g_pPlutoLogger->Write(LV_CRITICAL,"Table_psc_ir_batdet::GetRows Cannot perform query [%s] %s",query.c_str(),database->m_sLastMySqlError.c_str());
		return false;
	}	
	...
	if (!res)
	{
		...
		if( g_pPlutoLogger )
			g_pPlutoLogger->Write(LV_CRITICAL,"Table_psc_ir_batdet::GetRows mysql_store_result returned NULL handler");
		database->m_sLastMySqlError = mysql_error(database->m_pMySQL);
		return false;
	}	
...
condition = condition + "`PK_psc_ir_batdet`=" + tmp_PK_psc_ir_batdet;
	...
	{	
		...
		cerr << "Cannot perform query: [" << query << "] " << database->m_sLastMySqlError << endl;
		if( g_pPlutoLogger )
			g_pPlutoLogger->Write(LV_CRITICAL,"Table_psc_ir_batdet::FetchRow Cannot perform query [%s] %s",query.c_str(),database->m_sLastMySqlError.c_str());
		return NULL;
	}	
	...
	if (!res)
	{
		cerr << "mysql_store_result returned NULL handler" << endl;
		if( g_pPlutoLogger )
			g_pPlutoLogger->Write(LV_CRITICAL,"Table_psc_ir_batdet::FetchRow mysql_store_result returned NULL handler");
		database->m_sLastMySqlError = mysql_error(database->m_pMySQL);
		return NULL;
	}	
