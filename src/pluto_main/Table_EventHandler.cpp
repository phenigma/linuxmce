...
long int Row_EventHandler::OncePerSeconds_get(){PLUTO_SAFETY_LOCK_ERRORSONLY(sl,table->database->m_MySqlMutex);
...
return m_psc_mod;}
long int Row_EventHandler::psc_restrict_get(){PLUTO_SAFETY_LOCK_ERRORSONLY(sl,table->database->m_MySqlMutex);
...
void Row_EventHandler::OncePerSeconds_set(long int val){PLUTO_SAFETY_LOCK_ERRORSONLY(sl,table->database->m_MySqlMutex);
...
return is_null[12];}
bool Row_EventHandler::OncePerSeconds_isNull() {PLUTO_SAFETY_LOCK_ERRORSONLY(sl,table->database->m_MySqlMutex);
...
void Row_EventHandler::OncePerSeconds_setNull(bool val){PLUTO_SAFETY_LOCK_ERRORSONLY(sl,table->database->m_MySqlMutex);
is_null[13]=val;
is_modified=true;
}
...
values_list_comma_separated = values_list_comma_separated + pRow->PK_EventHandler_asSQL()+", "+pRow->FK_Event_asSQL()+", "+pRow->TimedEvent_asSQL()+", "+pRow->Description_asSQL()+", "+pRow->FK_Criteria_asSQL()+", "+pRow->FK_Installation_asSQL()+", "+pRow->FK_CommandGroup_asSQL()+", "+pRow->UserCreated_asSQL()+", "+pRow->FK_CannedEvents_asSQL()+", "+pRow->Disabled_asSQL()+", "+pRow->FK_Template_asSQL()+", "+pRow->TemplateParm1_asSQL()+", "+pRow->TemplateParm2_asSQL()+", "+pRow->OncePerSeconds_asSQL()+", "+pRow->psc_id_asSQL()+", "+pRow->psc_batch_asSQL()+", "+pRow->psc_user_asSQL()+", "+pRow->psc_frozen_asSQL()+", "+pRow->psc_restrict_asSQL();
		...
		{	
			...
			LoggerWrapper::GetInstance()->Write(LV_CRITICAL,"Table_EventHandler::Commit Cannot perform query [%s] %s reconnect: %d result2: %d",query.c_str(),database->m_sLastMySqlError.c_str(),(int) bResult, iResult2);
		...
		LoggerWrapper::GetInstance()->Write(LV_CRITICAL,"PK_EventHandler is auto increment but has no value %s",database->m_sLastMySqlError.c_str());	
...
update_values_list = update_values_list + "`PK_EventHandler`="+pRow->PK_EventHandler_asSQL()+", `FK_Event`="+pRow->FK_Event_asSQL()+", `TimedEvent`="+pRow->TimedEvent_asSQL()+", `Description`="+pRow->Description_asSQL()+", `FK_Criteria`="+pRow->FK_Criteria_asSQL()+", `FK_Installation`="+pRow->FK_Installation_asSQL()+", `FK_CommandGroup`="+pRow->FK_CommandGroup_asSQL()+", `UserCreated`="+pRow->UserCreated_asSQL()+", `FK_CannedEvents`="+pRow->FK_CannedEvents_asSQL()+", `Disabled`="+pRow->Disabled_asSQL()+", `FK_Template`="+pRow->FK_Template_asSQL()+", `TemplateParm1`="+pRow->TemplateParm1_asSQL()+", `TemplateParm2`="+pRow->TemplateParm2_asSQL()+", `OncePerSeconds`="+pRow->OncePerSeconds_asSQL()+", `psc_id`="+pRow->psc_id_asSQL()+", `psc_batch`="+pRow->psc_batch_asSQL()+", `psc_user`="+pRow->psc_user_asSQL()+", `psc_frozen`="+pRow->psc_frozen_asSQL()+", `psc_restrict`="+pRow->psc_restrict_asSQL();
		...
		{	
			...
			LoggerWrapper::GetInstance()->Write(LV_CRITICAL,"Table_EventHandler::Commit Cannot perform update query [%s] %s reconnect: %d result2: %d",query.c_str(),database->m_sLastMySqlError.c_str(),(int) bResult, iResult2);
			if( iResult2!=0 )  // We can keep going if the time it worked
			{
...
condition = condition + "`PK_EventHandler`=" + tmp_PK_EventHandler;
		...
		{	
			...
			LoggerWrapper::GetInstance()->Write(LV_CRITICAL,"Table_EventHandler::Commit Cannot perform delete query [%s] %s reconnect: %d result2: %d",query.c_str(),database->m_sLastMySqlError.c_str(),(int) bResult, iResult2);
			if( iResult2!=0 )  // We can keep going if the time it worked
				return false;
		}	
...
bool Table_EventHandler::GetRows(string where_statement,vector<class Row_EventHandler*> *rows)
{
	...
	{	
		...
		LoggerWrapper::GetInstance()->Write(LV_CRITICAL,"Table_EventHandler::GetRows Cannot perform query [%s] %s reconnect: %d result2: %d",query.c_str(),database->m_sLastMySqlError.c_str(),(int) bResult, iResult2);
		if( iResult2!=0 )  // We can keep going if the time it worked
			return false;
	}	
	...
	if (!res)
	{
		...
		LoggerWrapper::GetInstance()->Write(LV_CRITICAL,"Table_EventHandler::GetRows mysql_store_result returned NULL handler");
		database->m_sLastMySqlError = mysql_error(database->m_pMySQL);
		return false;
	}	
...
condition = condition + "`PK_EventHandler`=" + tmp_PK_EventHandler;
	...
	{	
		...
		LoggerWrapper::GetInstance()->Write(LV_CRITICAL,"Table_EventHandler::FetchRow Cannot perform query [%s] %s reconnect: %d result2: %d",query.c_str(),database->m_sLastMySqlError.c_str(),(int) bResult, iResult2);
		if( iResult2!=0 )  // We can keep going if the time it worked
			return NULL;
	}	
	...
	{
		cerr << "mysql_store_result returned NULL handler" << endl;
		LoggerWrapper::GetInstance()->Write(LV_CRITICAL,"Table_EventHandler::FetchRow mysql_store_result returned NULL handler");
		database->m_sLastMySqlError = mysql_error(database->m_pMySQL);
		return NULL;
	}	
