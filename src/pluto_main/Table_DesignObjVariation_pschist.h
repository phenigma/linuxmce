...
#ifndef __Table_DesignObjVariation_pschist_H__
#define __Table_DesignObjVariation_pschist_H__

#include "TableRow.h"
#include "Database_pluto_main.h"
#include "PlutoUtils/MultiThreadIncludes.h"
#include "Define_DesignObjVariation_pschist.h"
...
class DECLSPECIFIER Table_DesignObjVariation_pschist : public TableBase , DoubleLongKeyBase
{
private:
...
public:
	Table_DesignObjVariation_pschist(Database_pluto_main *pDatabase):database(pDatabase)
	{
	};
	~Table_DesignObjVariation_pschist();

private:		
	friend class Row_DesignObjVariation_pschist;
	struct Key
	{
		friend class Row_DesignObjVariation_pschist;
		long int pk_psc_id;
long int pk_psc_batch;
		...
		Key(class Row_DesignObjVariation_pschist *pRow);
	};
	struct Key_Less
	{			
		bool operator()(const Table_DesignObjVariation_pschist::Key &key1, const Table_DesignObjVariation_pschist::Key &key2) const;
	};	
...
public:				
	...
	bool GetRows(string where_statement,vector<class Row_DesignObjVariation_pschist*> *rows);
	class Row_DesignObjVariation_pschist* AddRow();
	Database_pluto_main *Database_pluto_main_get() { return database; }
	
		
	class Row_DesignObjVariation_pschist* GetRow(long int in_psc_id, long int in_psc_batch);
	

private:	
	
		
	class Row_DesignObjVariation_pschist* FetchRow(DoubleLongKey &key);
		
			
};

class DECLSPECIFIER Row_DesignObjVariation_pschist : public TableRow, public SerializeClass
	{
		friend struct Table_DesignObjVariation_pschist::Key;
		friend class Table_DesignObjVariation_pschist;
	private:
		Table_DesignObjVariation_pschist *table;
...
void psc_restrict_setNull(bool val);
		...
		Row_DesignObjVariation_pschist(Table_DesignObjVariation_pschist *pTable);
	
		bool IsDeleted(){return is_deleted;};
		bool IsModified(){return is_modified;};			
		class Table_DesignObjVariation_pschist *Table_DesignObjVariation_pschist_get() { return table; };
