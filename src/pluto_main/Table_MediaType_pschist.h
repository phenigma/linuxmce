...
#ifndef __Table_MediaType_pschist_H__
#define __Table_MediaType_pschist_H__

#include "TableRow.h"
#include "Database_pluto_main.h"
#include "PlutoUtils/MultiThreadIncludes.h"
#include "Define_MediaType_pschist.h"
...
class DECLSPECIFIER Table_MediaType_pschist : public TableBase , DoubleLongKeyBase
{
private:
...
public:
	Table_MediaType_pschist(Database_pluto_main *pDatabase):database(pDatabase)
	{
	};
	~Table_MediaType_pschist();

private:		
	friend class Row_MediaType_pschist;
	struct Key
	{
		friend class Row_MediaType_pschist;
		long int pk_psc_id;
long int pk_psc_batch;
		...
		Key(class Row_MediaType_pschist *pRow);
	};
	struct Key_Less
	{			
		bool operator()(const Table_MediaType_pschist::Key &key1, const Table_MediaType_pschist::Key &key2) const;
	};	
...
public:				
	...
	bool GetRows(string where_statement,vector<class Row_MediaType_pschist*> *rows);
	class Row_MediaType_pschist* AddRow();
	Database_pluto_main *Database_pluto_main_get() { return database; }
	
		
	class Row_MediaType_pschist* GetRow(long int in_psc_id, long int in_psc_batch);
	

private:	
	
		
	class Row_MediaType_pschist* FetchRow(DoubleLongKey &key);
		
			
};

class DECLSPECIFIER Row_MediaType_pschist : public TableRow, public SerializeClass
	{
		friend struct Table_MediaType_pschist::Key;
		friend class Table_MediaType_pschist;
	private:
		Table_MediaType_pschist *table;
...
void psc_restrict_setNull(bool val);
		...
		Row_MediaType_pschist(Table_MediaType_pschist *pTable);
	
		bool IsDeleted(){return is_deleted;};
		bool IsModified(){return is_modified;};			
		class Table_MediaType_pschist *Table_MediaType_pschist_get() { return table; };
...
class Row_Pipe* FK_Pipe_getrow();
		...
		void SetupSerialization(int iSC_Version) {
			StartSerializeList() + m_PK_MediaType+ m_Define+ m_Description+ m_FK_DesignObj+ m_DCEAware+ m_Extensions+ m_Subdirectory+ m_IsExternalTransmission+ m_FK_Pipe+ m_CanBookmark+ m_EK_AttributeType_DefaultSort+ m_psc_id+ m_psc_batch+ m_psc_user+ m_psc_frozen+ m_psc_mod+ m_psc_toc+ m_psc_restrict;
		}
	private:
