...
return is_null[9];}
bool Row_Device_StartupScript::psc_mod_isNull() {PLUTO_SAFETY_LOCK_ERRORSONLY(sl,table->database->m_DBMutex);
...
void Row_Device_StartupScript::psc_mod_setNull(bool val){PLUTO_SAFETY_LOCK_ERRORSONLY(sl,table->database->m_DBMutex);
is_null[10]=val;
is_modified=true;
}
...
db_wrapper_real_escape_string(table->database->m_pDB, buf, m_Parameter.c_str(), (unsigned long) min((size_t)150,m_Parameter.size()));
string s=string()+"\""+buf+"\"";
delete[] buf;
return s;
}
