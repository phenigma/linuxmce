...
values_list_comma_separated = values_list_comma_separated + pRow->PK_psc_ir_repset_asSQL()+", "+pRow->Setting_asSQL()+", "+pRow->Value_asSQL();
		...
		{	
			...
			if( g_pPlutoLogger )
				g_pPlutoLogger->Write(LV_CRITICAL,"Table_psc_ir_repset::Commit Cannot perform query [%s] %s",query.c_str(),database->m_sLastMySqlError.c_str());
		...
		g_pPlutoLogger->Write(LV_CRITICAL,"PK_psc_ir_repset is auto increment but has no value %s",database->m_sLastMySqlError.c_str());	
...
update_values_list = update_values_list + "`PK_psc_ir_repset`="+pRow->PK_psc_ir_repset_asSQL()+", `Setting`="+pRow->Setting_asSQL()+", `Value`="+pRow->Value_asSQL();
		...
		{	
			...
			cerr << "Cannot perform query: [" << query << "] " << database->m_sLastMySqlError << endl;
			if( g_pPlutoLogger )
				g_pPlutoLogger->Write(LV_CRITICAL,"Table_psc_ir_repset::Commit Cannot perform update query [%s] %s",query.c_str(),database->m_sLastMySqlError.c_str());
...
condition = condition + "`PK_psc_ir_repset`=" + tmp_PK_psc_ir_repset;
		...
		{	
			...
			cerr << "Cannot perform query: [" << query << "] " << database->m_sLastMySqlError << endl;
			if( g_pPlutoLogger )
				g_pPlutoLogger->Write(LV_CRITICAL,"Table_psc_ir_repset::Commit Cannot perform delete query [%s] %s",query.c_str(),database->m_sLastMySqlError.c_str());
			return false;
		}	
...
bool Table_psc_ir_repset::GetRows(string where_statement,vector<class Row_psc_ir_repset*> *rows)
{
	...
	{	
		...
		cerr << "Cannot perform query: [" << query << "] " << database->m_sLastMySqlError << endl;
		if( g_pPlutoLogger )
			g_pPlutoLogger->Write(LV_CRITICAL,"Table_psc_ir_repset::GetRows Cannot perform query [%s] %s",query.c_str(),database->m_sLastMySqlError.c_str());
		return false;
	}	
	...
	if (!res)
	{
		...
		if( g_pPlutoLogger )
			g_pPlutoLogger->Write(LV_CRITICAL,"Table_psc_ir_repset::GetRows mysql_store_result returned NULL handler");
		database->m_sLastMySqlError = mysql_error(database->m_pMySQL);
		return false;
	}	
...
condition = condition + "`PK_psc_ir_repset`=" + tmp_PK_psc_ir_repset;
	...
	{	
		...
		cerr << "Cannot perform query: [" << query << "] " << database->m_sLastMySqlError << endl;
		if( g_pPlutoLogger )
			g_pPlutoLogger->Write(LV_CRITICAL,"Table_psc_ir_repset::FetchRow Cannot perform query [%s] %s",query.c_str(),database->m_sLastMySqlError.c_str());
		return NULL;
	}	
	...
	if (!res)
	{
		cerr << "mysql_store_result returned NULL handler" << endl;
		if( g_pPlutoLogger )
			g_pPlutoLogger->Write(LV_CRITICAL,"Table_psc_ir_repset::FetchRow mysql_store_result returned NULL handler");
		database->m_sLastMySqlError = mysql_error(database->m_pMySQL);
		return NULL;
	}	
