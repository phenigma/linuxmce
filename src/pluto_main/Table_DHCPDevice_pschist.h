...
#ifndef __Table_DHCPDevice_pschist_H__
#define __Table_DHCPDevice_pschist_H__

#include "TableRow.h"
#include "Database_pluto_main.h"
#include "PlutoUtils/MultiThreadIncludes.h"
#include "Define_DHCPDevice_pschist.h"
...
class DECLSPECIFIER Table_DHCPDevice_pschist : public TableBase , DoubleLongKeyBase
{
private:
...
public:
	Table_DHCPDevice_pschist(Database_pluto_main *pDatabase):database(pDatabase)
	{
	};
	~Table_DHCPDevice_pschist();

private:		
	friend class Row_DHCPDevice_pschist;
	struct Key
	{
		friend class Row_DHCPDevice_pschist;
		long int pk_psc_id;
long int pk_psc_batch;
		...
		Key(class Row_DHCPDevice_pschist *pRow);
	};
	struct Key_Less
	{			
		bool operator()(const Table_DHCPDevice_pschist::Key &key1, const Table_DHCPDevice_pschist::Key &key2) const;
	};	
...
public:				
	...
	bool GetRows(string where_statement,vector<class Row_DHCPDevice_pschist*> *rows);
	class Row_DHCPDevice_pschist* AddRow();
	Database_pluto_main *Database_pluto_main_get() { return database; }
	
		
	class Row_DHCPDevice_pschist* GetRow(long int in_psc_id, long int in_psc_batch);
	

private:	
	
		
	class Row_DHCPDevice_pschist* FetchRow(DoubleLongKey &key);
		
			
};

class DECLSPECIFIER Row_DHCPDevice_pschist : public TableRow, public SerializeClass
	{
		friend struct Table_DHCPDevice_pschist::Key;
		friend class Table_DHCPDevice_pschist;
	private:
		Table_DHCPDevice_pschist *table;
...
void psc_restrict_setNull(bool val);
		...
		Row_DHCPDevice_pschist(Table_DHCPDevice_pschist *pTable);
	
		bool IsDeleted(){return is_deleted;};
		bool IsModified(){return is_modified;};			
		class Table_DHCPDevice_pschist *Table_DHCPDevice_pschist_get() { return table; };
...
class Row_Manufacturer* FK_Manufacturer_getrow();
		...
		void SetupSerialization(int iSC_Version) {
			StartSerializeList() + m_PK_DHCPDevice+ m_FK_DeviceTemplate+ m_Mac_Range_Low+ m_Mac_Range_High+ m_VendorModelID+ m_SerialNumber+ m_Parms+ m_Category+ m_FK_PnpProtocol+ m_PnpDetectionScript+ m_FK_DeviceCategory+ m_FK_Manufacturer+ m_Description+ m_psc_id+ m_psc_batch+ m_psc_user+ m_psc_frozen+ m_psc_mod+ m_psc_toc+ m_psc_restrict;
		}
	private:
