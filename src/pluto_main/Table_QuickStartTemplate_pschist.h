...
#ifndef __Table_QuickStartTemplate_pschist_H__
#define __Table_QuickStartTemplate_pschist_H__

#include "TableRow.h"
#include "Database_pluto_main.h"
#include "PlutoUtils/MultiThreadIncludes.h"
#include "Define_QuickStartTemplate_pschist.h"
...
class DECLSPECIFIER Table_QuickStartTemplate_pschist : public TableBase , DoubleLongKeyBase
{
private:
...
public:
	Table_QuickStartTemplate_pschist(Database_pluto_main *pDatabase):database(pDatabase)
	{
	};
	~Table_QuickStartTemplate_pschist();

private:		
	friend class Row_QuickStartTemplate_pschist;
	struct Key
	{
		friend class Row_QuickStartTemplate_pschist;
		long int pk_psc_id;
long int pk_psc_batch;
		...
		Key(class Row_QuickStartTemplate_pschist *pRow);
	};
	struct Key_Less
	{			
		bool operator()(const Table_QuickStartTemplate_pschist::Key &key1, const Table_QuickStartTemplate_pschist::Key &key2) const;
	};	
...
public:				
	...
	bool GetRows(string where_statement,vector<class Row_QuickStartTemplate_pschist*> *rows);
	class Row_QuickStartTemplate_pschist* AddRow();
	Database_pluto_main *Database_pluto_main_get() { return database; }
	
		
	class Row_QuickStartTemplate_pschist* GetRow(long int in_psc_id, long int in_psc_batch);
	

private:	
	
		
	class Row_QuickStartTemplate_pschist* FetchRow(DoubleLongKey &key);
		
			
};

class DECLSPECIFIER Row_QuickStartTemplate_pschist : public TableRow, public SerializeClass
	{
		friend struct Table_QuickStartTemplate_pschist::Key;
		friend class Table_QuickStartTemplate_pschist;
	private:
		Table_QuickStartTemplate_pschist *table;
...
void psc_toc_setNull(bool val);
		...
		Row_QuickStartTemplate_pschist(Table_QuickStartTemplate_pschist *pTable);
	
		bool IsDeleted(){return is_deleted;};
		bool IsModified(){return is_modified;};			
		class Table_QuickStartTemplate_pschist *Table_QuickStartTemplate_pschist_get() { return table; };
...
class Row_DesignObj* FK_DesignObj_OSD_getrow();
		...
		void SetupSerialization(int iSC_Version) {
			StartSerializeList() + m_PK_QuickStartTemplate+ m_Description+ m_FK_QuickStartCategory+ m_FK_Package+ m_Binary+ m_Arguments+ m_Homepage+ m_Icon+ m_WindowClass+ m_FK_DesignObj+ m_FK_DesignObj_OSD+ m_psc_id+ m_psc_batch+ m_psc_user+ m_psc_frozen+ m_psc_mod+ m_psc_restrict+ m_psc_toc;
		}
	private:
