...
values_list_comma_separated = values_list_comma_separated + pRow->FK_Command_asSQL()+", "+pRow->FK_Pipe_asSQL()+", "+pRow->psc_id_asSQL()+", "+pRow->psc_batch_asSQL()+", "+pRow->psc_user_asSQL()+", "+pRow->psc_frozen_asSQL()+", "+pRow->psc_restrict_asSQL();
		...
		{	
			...
			if( g_pPlutoLogger )
				g_pPlutoLogger->Write(LV_CRITICAL,"Table_Command_Pipe::Commit Cannot perform query [%s] %s",query.c_str(),database->m_sLastMySqlError.c_str());
...
update_values_list = update_values_list + "`FK_Command`="+pRow->FK_Command_asSQL()+", `FK_Pipe`="+pRow->FK_Pipe_asSQL()+", `psc_id`="+pRow->psc_id_asSQL()+", `psc_batch`="+pRow->psc_batch_asSQL()+", `psc_user`="+pRow->psc_user_asSQL()+", `psc_frozen`="+pRow->psc_frozen_asSQL()+", `psc_restrict`="+pRow->psc_restrict_asSQL();
		...
		{	
			...
			cerr << "Cannot perform query: [" << query << "] " << database->m_sLastMySqlError << endl;
			if( g_pPlutoLogger )
				g_pPlutoLogger->Write(LV_CRITICAL,"Table_Command_Pipe::Commit Cannot perform update query [%s] %s",query.c_str(),database->m_sLastMySqlError.c_str());
...
condition = condition + "`FK_Command`=" + tmp_FK_Command+" AND "+"`FK_Pipe`=" + tmp_FK_Pipe;
		...
		{	
			...
			cerr << "Cannot perform query: [" << query << "] " << database->m_sLastMySqlError << endl;
			if( g_pPlutoLogger )
				g_pPlutoLogger->Write(LV_CRITICAL,"Table_Command_Pipe::Commit Cannot perform delete query [%s] %s",query.c_str(),database->m_sLastMySqlError.c_str());
			return false;
		}	
...
bool Table_Command_Pipe::GetRows(string where_statement,vector<class Row_Command_Pipe*> *rows)
{
	...
	{	
		...
		cerr << "Cannot perform query: [" << query << "] " << database->m_sLastMySqlError << endl;
		if( g_pPlutoLogger )
			g_pPlutoLogger->Write(LV_CRITICAL,"Table_Command_Pipe::GetRows Cannot perform query [%s] %s",query.c_str(),database->m_sLastMySqlError.c_str());
		return false;
	}	
	...
	if (!res)
	{
		...
		if( g_pPlutoLogger )
			g_pPlutoLogger->Write(LV_CRITICAL,"Table_Command_Pipe::GetRows mysql_store_result returned NULL handler");
		database->m_sLastMySqlError = mysql_error(database->m_pMySQL);
		return false;
	}	
...
condition = condition + "`FK_Command`=" + tmp_FK_Command+" AND "+"`FK_Pipe`=" + tmp_FK_Pipe;
	...
	{	
		...
		cerr << "Cannot perform query: [" << query << "] " << database->m_sLastMySqlError << endl;
		if( g_pPlutoLogger )
			g_pPlutoLogger->Write(LV_CRITICAL,"Table_Command_Pipe::FetchRow Cannot perform query [%s] %s",query.c_str(),database->m_sLastMySqlError.c_str());
		return NULL;
	}	
	...
	if (!res)
	{
		cerr << "mysql_store_result returned NULL handler" << endl;
		if( g_pPlutoLogger )
			g_pPlutoLogger->Write(LV_CRITICAL,"Table_Command_Pipe::FetchRow mysql_store_result returned NULL handler");
		database->m_sLastMySqlError = mysql_error(database->m_pMySQL);
		return NULL;
	}	
