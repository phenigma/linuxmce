...
values_list_comma_separated = values_list_comma_separated + pRow->PK_TimeZone_asSQL()+", "+pRow->ZoneName_asSQL()+", "+pRow->Coordinate_asSQL()+", "+pRow->TimeZoneOffset_asSQL()+", "+pRow->SummerTimeOffset_asSQL()+", "+pRow->Comments_asSQL()+", "+pRow->CountryCode_asSQL();
		...
		{	
			...
			cerr << "Cannot perform query: [" << query << "] " << database->m_sLastMySqlError << endl;
				LoggerWrapper::GetInstance()->Write(LV_CRITICAL,"Table_TimeZone::Commit Cannot perform query [%s] %s",query.c_str(),database->m_sLastMySqlError.c_str());
...
else 	LoggerWrapper::GetInstance()->Write(LV_CRITICAL,"PK_TimeZone is auto increment but has no value %s",database->m_sLastMySqlError.c_str());	
...
update_values_list = update_values_list + "`PK_TimeZone`="+pRow->PK_TimeZone_asSQL()+", `ZoneName`="+pRow->ZoneName_asSQL()+", `Coordinate`="+pRow->Coordinate_asSQL()+", `TimeZoneOffset`="+pRow->TimeZoneOffset_asSQL()+", `SummerTimeOffset`="+pRow->SummerTimeOffset_asSQL()+", `Comments`="+pRow->Comments_asSQL()+", `CountryCode`="+pRow->CountryCode_asSQL();
		...
		{	
			database->m_sLastMySqlError = mysql_error(database->m_pMySQL);
			cerr << "Cannot perform query: [" << query << "] " << database->m_sLastMySqlError << endl;
				LoggerWrapper::GetInstance()->Write(LV_CRITICAL,"Table_TimeZone::Commit Cannot perform update query [%s] %s",query.c_str(),database->m_sLastMySqlError.c_str());
...
condition = condition + "`PK_TimeZone`=" + tmp_PK_TimeZone;
		...
		{	
			database->m_sLastMySqlError = mysql_error(database->m_pMySQL);
			cerr << "Cannot perform query: [" << query << "] " << database->m_sLastMySqlError << endl;
				LoggerWrapper::GetInstance()->Write(LV_CRITICAL,"Table_TimeZone::Commit Cannot perform delete query [%s] %s",query.c_str(),database->m_sLastMySqlError.c_str());
			return false;
		}	
...
bool Table_TimeZone::GetRows(string where_statement,vector<class Row_TimeZone*> *rows)
{
	...
	{	
		database->m_sLastMySqlError = mysql_error(database->m_pMySQL);
		cerr << "Cannot perform query: [" << query << "] " << database->m_sLastMySqlError << endl;
		LoggerWrapper::GetInstance()->Write(LV_CRITICAL,"Table_TimeZone::GetRows Cannot perform query [%s] %s",query.c_str(),database->m_sLastMySqlError.c_str());
		return false;
	}	
...
condition = condition + "`PK_TimeZone`=" + tmp_PK_TimeZone;
	...
	{	
		database->m_sLastMySqlError = mysql_error(database->m_pMySQL);
		cerr << "Cannot perform query: [" << query << "] " << database->m_sLastMySqlError << endl;
		LoggerWrapper::GetInstance()->Write(LV_CRITICAL,"Table_TimeZone::FetchRow Cannot perform query [%s] %s",query.c_str(),database->m_sLastMySqlError.c_str());
		return NULL;
	}	
