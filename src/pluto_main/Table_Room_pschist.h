...
#ifndef __Table_Room_pschist_H__
#define __Table_Room_pschist_H__

#include "TableRow.h"
#include "Database_pluto_main.h"
#include "PlutoUtils/MultiThreadIncludes.h"
#include "Define_Room_pschist.h"
...
class DECLSPECIFIER Table_Room_pschist : public TableBase , DoubleLongKeyBase
{
private:
...
public:
	Table_Room_pschist(Database_pluto_main *pDatabase):database(pDatabase)
	{
	};
	~Table_Room_pschist();

private:		
	friend class Row_Room_pschist;
	struct Key
	{
		friend class Row_Room_pschist;
		long int pk_psc_id;
long int pk_psc_batch;
		...
		Key(class Row_Room_pschist *pRow);
	};
	struct Key_Less
	{			
		bool operator()(const Table_Room_pschist::Key &key1, const Table_Room_pschist::Key &key2) const;
	};	
...
public:				
	...
	bool GetRows(string where_statement,vector<class Row_Room_pschist*> *rows);
	class Row_Room_pschist* AddRow();
	Database_pluto_main *Database_pluto_main_get() { return database; }
	
		
	class Row_Room_pschist* GetRow(long int in_psc_id, long int in_psc_batch);
	

private:	
	
		
	class Row_Room_pschist* FetchRow(DoubleLongKey &key);
		
			
};

class DECLSPECIFIER Row_Room_pschist : public TableRow, public SerializeClass
	{
		friend struct Table_Room_pschist::Key;
		friend class Table_Room_pschist;
	private:
		Table_Room_pschist *table;
...
void psc_restrict_setNull(bool val);
		...
		Row_Room_pschist(Table_Room_pschist *pTable);
	
		bool IsDeleted(){return is_deleted;};
		bool IsModified(){return is_modified;};			
		class Table_Room_pschist *Table_Room_pschist_get() { return table; };
...
class Row_FloorplanObjectType* FK_FloorplanObjectType_getrow();
		...
		void SetupSerialization(int iSC_Version) {
			StartSerializeList() + m_PK_Room+ m_FK_Installation+ m_FK_RoomType+ m_Description+ m_FK_Icon+ m_ManuallyConfigureEA+ m_HideFromOrbiter+ m_FK_FloorplanObjectType+ m_FloorplanInfo+ m_psc_id+ m_psc_batch+ m_psc_user+ m_psc_frozen+ m_psc_mod+ m_psc_toc+ m_psc_restrict;
		}
	private:
