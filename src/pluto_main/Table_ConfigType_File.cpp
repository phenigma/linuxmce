...
mysql_real_escape_string(table->database->m_pMySQL, buf, m_InputFile.c_str(), (unsigned long) min(65535,m_InputFile.size()));
string s=string()+"\""+buf+"\"";
delete[] buf;
return s;
}
...
char *buf = new char[131071];
mysql_real_escape_string(table->database->m_pMySQL, buf, m_OutputFile.c_str(), (unsigned long) min(65535,m_OutputFile.size()));
string s=string()+"\""+buf+"\"";
delete[] buf;
return s;
}
