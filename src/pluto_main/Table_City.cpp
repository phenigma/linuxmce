...
values_list_comma_separated = values_list_comma_separated + pRow->PK_City_asSQL()+", "+pRow->FK_Country_asSQL()+", "+pRow->FK_Region_asSQL()+", "+pRow->City_asSQL()+", "+pRow->Latitude_asSQL()+", "+pRow->Longitude_asSQL()+", "+pRow->DmaId_asSQL()+", "+pRow->County_asSQL()+", "+pRow->Code_asSQL()+", "+pRow->FK_TimeZone_asSQL();
		...
		{	
			...
			if( g_pPlutoLogger )
				g_pPlutoLogger->Write(LV_CRITICAL,"Table_City::Commit Cannot perform query [%s] %s",query.c_str(),database->m_sLastMySqlError.c_str());
		...
		g_pPlutoLogger->Write(LV_CRITICAL,"PK_City is auto increment but has no value %s",database->m_sLastMySqlError.c_str());	
...
update_values_list = update_values_list + "`PK_City`="+pRow->PK_City_asSQL()+", `FK_Country`="+pRow->FK_Country_asSQL()+", `FK_Region`="+pRow->FK_Region_asSQL()+", `City`="+pRow->City_asSQL()+", `Latitude`="+pRow->Latitude_asSQL()+", `Longitude`="+pRow->Longitude_asSQL()+", `DmaId`="+pRow->DmaId_asSQL()+", `County`="+pRow->County_asSQL()+", `Code`="+pRow->Code_asSQL()+", `FK_TimeZone`="+pRow->FK_TimeZone_asSQL();
		...
		{	
			...
			cerr << "Cannot perform query: [" << query << "] " << database->m_sLastMySqlError << endl;
			if( g_pPlutoLogger )
				g_pPlutoLogger->Write(LV_CRITICAL,"Table_City::Commit Cannot perform update query [%s] %s",query.c_str(),database->m_sLastMySqlError.c_str());
...
condition = condition + "`PK_City`=" + tmp_PK_City;
		...
		{	
			...
			cerr << "Cannot perform query: [" << query << "] " << database->m_sLastMySqlError << endl;
			if( g_pPlutoLogger )
				g_pPlutoLogger->Write(LV_CRITICAL,"Table_City::Commit Cannot perform delete query [%s] %s",query.c_str(),database->m_sLastMySqlError.c_str());
			return false;
		}	
...
bool Table_City::GetRows(string where_statement,vector<class Row_City*> *rows)
{
	...
	{	
		...
		cerr << "Cannot perform query: [" << query << "] " << database->m_sLastMySqlError << endl;
		if( g_pPlutoLogger )
			g_pPlutoLogger->Write(LV_CRITICAL,"Table_City::GetRows Cannot perform query [%s] %s",query.c_str(),database->m_sLastMySqlError.c_str());
		return false;
	}	
	...
	if (!res)
	{
		...
		if( g_pPlutoLogger )
			g_pPlutoLogger->Write(LV_CRITICAL,"Table_City::GetRows mysql_store_result returned NULL handler");
		database->m_sLastMySqlError = mysql_error(database->m_pMySQL);
		return false;
	}	
...
condition = condition + "`PK_City`=" + tmp_PK_City;
	...
	{	
		...
		cerr << "Cannot perform query: [" << query << "] " << database->m_sLastMySqlError << endl;
		if( g_pPlutoLogger )
			g_pPlutoLogger->Write(LV_CRITICAL,"Table_City::FetchRow Cannot perform query [%s] %s",query.c_str(),database->m_sLastMySqlError.c_str());
		return NULL;
	}	
	...
	if (!res)
	{
		cerr << "mysql_store_result returned NULL handler" << endl;
		if( g_pPlutoLogger )
			g_pPlutoLogger->Write(LV_CRITICAL,"Table_City::FetchRow mysql_store_result returned NULL handler");
		database->m_sLastMySqlError = mysql_error(database->m_pMySQL);
		return NULL;
	}	
