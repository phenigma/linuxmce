...
return m_FK_Country;}
long int Row_Installation::FK_City_get(){PLUTO_SAFETY_LOCK_ERRORSONLY(sl,table->database->m_MySqlMutex);

return m_FK_City;}
long int Row_Installation::FK_PostalCode_get(){PLUTO_SAFETY_LOCK_ERRORSONLY(sl,table->database->m_MySqlMutex);
...
void Row_Installation::FK_City_set(long int val){PLUTO_SAFETY_LOCK_ERRORSONLY(sl,table->database->m_MySqlMutex);

m_FK_City = val; is_modified=true; is_null[8]=false;}
void Row_Installation::FK_PostalCode_set(long int val){PLUTO_SAFETY_LOCK_ERRORSONLY(sl,table->database->m_MySqlMutex);
...
return is_null[7];}
bool Row_Installation::FK_City_isNull() {PLUTO_SAFETY_LOCK_ERRORSONLY(sl,table->database->m_MySqlMutex);

return is_null[8];}
bool Row_Installation::FK_PostalCode_isNull() {PLUTO_SAFETY_LOCK_ERRORSONLY(sl,table->database->m_MySqlMutex);
...
void Row_Installation::FK_City_setNull(bool val){PLUTO_SAFETY_LOCK_ERRORSONLY(sl,table->database->m_MySqlMutex);
is_null[8]=val;
is_modified=true;
}
void Row_Installation::FK_PostalCode_setNull(bool val){PLUTO_SAFETY_LOCK_ERRORSONLY(sl,table->database->m_MySqlMutex);
is_null[9]=val;
is_modified=true;
}
...
mysql_real_escape_string(table->database->m_pMySQL, buf, m_Name.c_str(), (unsigned long) min((size_t)30,m_Name.size()));
string s=string()+"\""+buf+"\"";
delete[] buf;
return s;
}
...
mysql_real_escape_string(table->database->m_pMySQL, buf, m_Address.c_str(), (unsigned long) min((size_t)50,m_Address.size()));
string s=string()+"\""+buf+"\"";
delete[] buf;
return s;
}
...
char *buf = new char[41];
mysql_real_escape_string(table->database->m_pMySQL, buf, m_City.c_str(), (unsigned long) min((size_t)20,m_City.size()));
string s=string()+"\""+buf+"\"";
delete[] buf;
return s;
}
...
mysql_real_escape_string(table->database->m_pMySQL, buf, m_State.c_str(), (unsigned long) min((size_t)10,m_State.size()));
string s=string()+"\""+buf+"\"";
delete[] buf;
return s;
}
...
char *buf = new char[21];
mysql_real_escape_string(table->database->m_pMySQL, buf, m_Zip.c_str(), (unsigned long) min((size_t)10,m_Zip.size()));
string s=string()+"\""+buf+"\"";
delete[] buf;
return s;
}
...
char *buf = new char[101];
mysql_real_escape_string(table->database->m_pMySQL, buf, m_ActivationCode.c_str(), (unsigned long) min((size_t)50,m_ActivationCode.size()));
string s=string()+"\""+buf+"\"";
delete[] buf;
return s;
}
...
mysql_real_escape_string(table->database->m_pMySQL, buf, m_LastStatus.c_str(), (unsigned long) min((size_t)65535,m_LastStatus.size()));
string s=string()+"\""+buf+"\"";
delete[] buf;
return s;
}
...
mysql_real_escape_string(table->database->m_pMySQL, buf, m_LastAlive.c_str(), (unsigned long) min((size_t)19,m_LastAlive.size()));
string s=string()+"\""+buf+"\"";
delete[] buf;
return s;
}
...
values_list_comma_separated = values_list_comma_separated + pRow->PK_Installation_asSQL()+", "+pRow->Description_asSQL()+", "+pRow->Name_asSQL()+", "+pRow->Address_asSQL()+", "+pRow->City_asSQL()+", "+pRow->State_asSQL()+", "+pRow->Zip_asSQL()+", "+pRow->FK_Country_asSQL()+", "+pRow->FK_City_asSQL()+", "+pRow->FK_PostalCode_asSQL()+", "+pRow->ActivationCode_asSQL()+", "+pRow->LastStatus_asSQL()+", "+pRow->LastAlive_asSQL()+", "+pRow->isActive_asSQL()+", "+pRow->FK_Version_asSQL()+", "+pRow->isMonitored_asSQL()+", "+pRow->FK_RepositoryType_Source_asSQL()+", "+pRow->FK_RepositoryType_Binaries_asSQL()+", "+pRow->psc_id_asSQL()+", "+pRow->psc_batch_asSQL()+", "+pRow->psc_user_asSQL()+", "+pRow->psc_frozen_asSQL()+", "+pRow->psc_restrict_asSQL();
		...
		{	
			...
			LoggerWrapper::GetInstance()->Write(LV_CRITICAL,"Table_Installation::Commit Cannot perform query [%s] %s reconnect: %d result2: %d",query.c_str(),database->m_sLastMySqlError.c_str(),(int) bResult, iResult2);
		...
		LoggerWrapper::GetInstance()->Write(LV_CRITICAL,"PK_Installation is auto increment but has no value %s",database->m_sLastMySqlError.c_str());	
...
update_values_list = update_values_list + "`PK_Installation`="+pRow->PK_Installation_asSQL()+", `Description`="+pRow->Description_asSQL()+", `Name`="+pRow->Name_asSQL()+", `Address`="+pRow->Address_asSQL()+", `City`="+pRow->City_asSQL()+", `State`="+pRow->State_asSQL()+", `Zip`="+pRow->Zip_asSQL()+", `FK_Country`="+pRow->FK_Country_asSQL()+", `FK_City`="+pRow->FK_City_asSQL()+", `FK_PostalCode`="+pRow->FK_PostalCode_asSQL()+", `ActivationCode`="+pRow->ActivationCode_asSQL()+", `LastStatus`="+pRow->LastStatus_asSQL()+", `LastAlive`="+pRow->LastAlive_asSQL()+", `isActive`="+pRow->isActive_asSQL()+", `FK_Version`="+pRow->FK_Version_asSQL()+", `isMonitored`="+pRow->isMonitored_asSQL()+", `FK_RepositoryType_Source`="+pRow->FK_RepositoryType_Source_asSQL()+", `FK_RepositoryType_Binaries`="+pRow->FK_RepositoryType_Binaries_asSQL()+", `psc_id`="+pRow->psc_id_asSQL()+", `psc_batch`="+pRow->psc_batch_asSQL()+", `psc_user`="+pRow->psc_user_asSQL()+", `psc_frozen`="+pRow->psc_frozen_asSQL()+", `psc_restrict`="+pRow->psc_restrict_asSQL();
		...
		{	
			...
			LoggerWrapper::GetInstance()->Write(LV_CRITICAL,"Table_Installation::Commit Cannot perform update query [%s] %s reconnect: %d result2: %d",query.c_str(),database->m_sLastMySqlError.c_str(),(int) bResult, iResult2);
			if( iResult2!=0 )  // We can keep going if the time it worked
			{
...
condition = condition + "`PK_Installation`=" + tmp_PK_Installation;
		...
		{	
			...
			LoggerWrapper::GetInstance()->Write(LV_CRITICAL,"Table_Installation::Commit Cannot perform delete query [%s] %s reconnect: %d result2: %d",query.c_str(),database->m_sLastMySqlError.c_str(),(int) bResult, iResult2);
			if( iResult2!=0 )  // We can keep going if the time it worked
				return false;
		}	
...
bool Table_Installation::GetRows(string where_statement,vector<class Row_Installation*> *rows)
{
	...
	{	
		...
		LoggerWrapper::GetInstance()->Write(LV_CRITICAL,"Table_Installation::GetRows Cannot perform query [%s] %s reconnect: %d result2: %d",query.c_str(),database->m_sLastMySqlError.c_str(),(int) bResult, iResult2);
		if( iResult2!=0 )  // We can keep going if the time it worked
			return false;
	}	
	...
	if (!res)
	{
		...
		LoggerWrapper::GetInstance()->Write(LV_CRITICAL,"Table_Installation::GetRows mysql_store_result returned NULL handler");
		database->m_sLastMySqlError = mysql_error(database->m_pMySQL);
		return false;
	}	
...
condition = condition + "`PK_Installation`=" + tmp_PK_Installation;
	...
	{	
		...
		LoggerWrapper::GetInstance()->Write(LV_CRITICAL,"Table_Installation::FetchRow Cannot perform query [%s] %s reconnect: %d result2: %d",query.c_str(),database->m_sLastMySqlError.c_str(),(int) bResult, iResult2);
		if( iResult2!=0 )  // We can keep going if the time it worked
			return NULL;
	}	
	...
	{
		cerr << "mysql_store_result returned NULL handler" << endl;
		LoggerWrapper::GetInstance()->Write(LV_CRITICAL,"Table_Installation::FetchRow mysql_store_result returned NULL handler");
		database->m_sLastMySqlError = mysql_error(database->m_pMySQL);
		return NULL;
	}	
