...
mysql_real_escape_string(table->database->m_pMySQL, buf, m_IK_CommandParameter.c_str(), (unsigned long) min((size_t)65535,m_IK_CommandParameter.size()));
string s=string()+"\""+buf+"\"";
delete[] buf;
return s;
}
...
values_list_comma_separated = values_list_comma_separated + pRow->FK_CommandGroup_D_Command_asSQL()+", "+pRow->FK_CommandParameter_asSQL()+", "+pRow->IK_CommandParameter_asSQL()+", "+pRow->psc_id_asSQL()+", "+pRow->psc_batch_asSQL()+", "+pRow->psc_user_asSQL()+", "+pRow->psc_frozen_asSQL()+", "+pRow->psc_restrict_asSQL();
		...
		{	
			...
			LoggerWrapper::GetInstance()->Write(LV_CRITICAL,"Table_CommandGroup_D_Command_CommandParameter::Commit Cannot perform query [%s] %s reconnect: %d result2: %d",query.c_str(),database->m_sLastMySqlError.c_str(),(int) bResult, iResult2);
...
update_values_list = update_values_list + "`FK_CommandGroup_D_Command`="+pRow->FK_CommandGroup_D_Command_asSQL()+", `FK_CommandParameter`="+pRow->FK_CommandParameter_asSQL()+", `IK_CommandParameter`="+pRow->IK_CommandParameter_asSQL()+", `psc_id`="+pRow->psc_id_asSQL()+", `psc_batch`="+pRow->psc_batch_asSQL()+", `psc_user`="+pRow->psc_user_asSQL()+", `psc_frozen`="+pRow->psc_frozen_asSQL()+", `psc_restrict`="+pRow->psc_restrict_asSQL();
		...
		{	
			...
			LoggerWrapper::GetInstance()->Write(LV_CRITICAL,"Table_CommandGroup_D_Command_CommandParameter::Commit Cannot perform update query [%s] %s reconnect: %d result2: %d",query.c_str(),database->m_sLastMySqlError.c_str(),(int) bResult, iResult2);
			if( iResult2!=0 )  // We can keep going if the time it worked
			{
...
condition = condition + "`FK_CommandGroup_D_Command`=" + tmp_FK_CommandGroup_D_Command+" AND "+"`FK_CommandParameter`=" + tmp_FK_CommandParameter;
		...
		{	
			...
			LoggerWrapper::GetInstance()->Write(LV_CRITICAL,"Table_CommandGroup_D_Command_CommandParameter::Commit Cannot perform delete query [%s] %s reconnect: %d result2: %d",query.c_str(),database->m_sLastMySqlError.c_str(),(int) bResult, iResult2);
			if( iResult2!=0 )  // We can keep going if the time it worked
				return false;
		}	
...
bool Table_CommandGroup_D_Command_CommandParameter::GetRows(string where_statement,vector<class Row_CommandGroup_D_Command_CommandParameter*> *rows)
{
	...
	{	
		...
		LoggerWrapper::GetInstance()->Write(LV_CRITICAL,"Table_CommandGroup_D_Command_CommandParameter::GetRows Cannot perform query [%s] %s reconnect: %d result2: %d",query.c_str(),database->m_sLastMySqlError.c_str(),(int) bResult, iResult2);
		if( iResult2!=0 )  // We can keep going if the time it worked
			return false;
	}	
	...
	if (!res)
	{
		...
		LoggerWrapper::GetInstance()->Write(LV_CRITICAL,"Table_CommandGroup_D_Command_CommandParameter::GetRows mysql_store_result returned NULL handler");
		database->m_sLastMySqlError = mysql_error(database->m_pMySQL);
		return false;
	}	
...
condition = condition + "`FK_CommandGroup_D_Command`=" + tmp_FK_CommandGroup_D_Command+" AND "+"`FK_CommandParameter`=" + tmp_FK_CommandParameter;
	...
	{	
		...
		LoggerWrapper::GetInstance()->Write(LV_CRITICAL,"Table_CommandGroup_D_Command_CommandParameter::FetchRow Cannot perform query [%s] %s reconnect: %d result2: %d",query.c_str(),database->m_sLastMySqlError.c_str(),(int) bResult, iResult2);
		if( iResult2!=0 )  // We can keep going if the time it worked
			return NULL;
	}	
	...
	{
		cerr << "mysql_store_result returned NULL handler" << endl;
		LoggerWrapper::GetInstance()->Write(LV_CRITICAL,"Table_CommandGroup_D_Command_CommandParameter::FetchRow mysql_store_result returned NULL handler");
		database->m_sLastMySqlError = mysql_error(database->m_pMySQL);
		return NULL;
	}	
