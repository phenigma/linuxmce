...
#ifndef __Table_Orbiter_pschist_H__
#define __Table_Orbiter_pschist_H__

#include "TableRow.h"
#include "Database_pluto_main.h"
#include "PlutoUtils/MultiThreadIncludes.h"
#include "Define_Orbiter_pschist.h"
...
class DECLSPECIFIER Table_Orbiter_pschist : public TableBase , DoubleLongKeyBase
{
private:
...
public:
	Table_Orbiter_pschist(Database_pluto_main *pDatabase):database(pDatabase)
	{
	};
	~Table_Orbiter_pschist();

private:		
	friend class Row_Orbiter_pschist;
	struct Key
	{
		friend class Row_Orbiter_pschist;
		long int pk_psc_id;
long int pk_psc_batch;
		...
		Key(class Row_Orbiter_pschist *pRow);
	};
	struct Key_Less
	{			
		bool operator()(const Table_Orbiter_pschist::Key &key1, const Table_Orbiter_pschist::Key &key2) const;
	};	
...
public:				
	...
	bool GetRows(string where_statement,vector<class Row_Orbiter_pschist*> *rows);
	class Row_Orbiter_pschist* AddRow();
	Database_pluto_main *Database_pluto_main_get() { return database; }
	
		
	class Row_Orbiter_pschist* GetRow(long int in_psc_id, long int in_psc_batch);
	

private:	
	
		
	class Row_Orbiter_pschist* FetchRow(DoubleLongKey &key);
		
			
};

class DECLSPECIFIER Row_Orbiter_pschist : public TableRow, public SerializeClass
	{
		friend struct Table_Orbiter_pschist::Key;
		friend class Table_Orbiter_pschist;
	private:
		Table_Orbiter_pschist *table;
...
short int m_RegenStatus;
short int m_RegenPercent;
...
short int RegenStatus_get();
short int RegenPercent_get();
...
void RegenStatus_set(short int val);
void RegenPercent_set(short int val);
...
void FirstRegen_setNull(bool val);
		...
		Row_Orbiter_pschist(Table_Orbiter_pschist *pTable);
	
		bool IsDeleted(){return is_deleted;};
		bool IsModified(){return is_modified;};			
		class Table_Orbiter_pschist *Table_Orbiter_pschist_get() { return table; };
		...
		void SetupSerialization(int iSC_Version) {
			StartSerializeList() + m_PK_Orbiter+ m_FloorplanInfo+ m_Modification_LastGen+ m_Regen+ m_Size+ m_RegenInProgress+ m_RegenStatus+ m_RegenPercent+ m_ScenariosFloorplans+ m_psc_id+ m_psc_batch+ m_psc_user+ m_psc_frozen+ m_psc_mod+ m_psc_toc+ m_psc_restrict+ m_FirstRegen;
		}
	private:
