...
#ifndef __Table_EventHandler_pschist_H__
#define __Table_EventHandler_pschist_H__

#include "TableRow.h"
#include "Database_pluto_main.h"
#include "PlutoUtils/MultiThreadIncludes.h"
#include "Define_EventHandler_pschist.h"
...
class DECLSPECIFIER Table_EventHandler_pschist : public TableBase , DoubleLongKeyBase
{
private:
...
public:
	Table_EventHandler_pschist(Database_pluto_main *pDatabase):database(pDatabase)
	{
	};
	~Table_EventHandler_pschist();

private:		
	friend class Row_EventHandler_pschist;
	struct Key
	{
		friend class Row_EventHandler_pschist;
		long int pk_psc_id;
long int pk_psc_batch;
		...
		Key(class Row_EventHandler_pschist *pRow);
	};
	struct Key_Less
	{			
		bool operator()(const Table_EventHandler_pschist::Key &key1, const Table_EventHandler_pschist::Key &key2) const;
	};	
...
public:				
	...
	bool GetRows(string where_statement,vector<class Row_EventHandler_pschist*> *rows);
	class Row_EventHandler_pschist* AddRow();
	Database_pluto_main *Database_pluto_main_get() { return database; }
	
		
	class Row_EventHandler_pschist* GetRow(long int in_psc_id, long int in_psc_batch);
	

private:	
	
		
	class Row_EventHandler_pschist* FetchRow(DoubleLongKey &key);
		
			
};

class DECLSPECIFIER Row_EventHandler_pschist : public TableRow, public SerializeClass
	{
		friend struct Table_EventHandler_pschist::Key;
		friend class Table_EventHandler_pschist;
	private:
		Table_EventHandler_pschist *table;
...
void psc_restrict_setNull(bool val);
		...
		Row_EventHandler_pschist(Table_EventHandler_pschist *pTable);
	
		bool IsDeleted(){return is_deleted;};
		bool IsModified(){return is_modified;};			
		class Table_EventHandler_pschist *Table_EventHandler_pschist_get() { return table; };
...
class Row_Template* FK_Template_getrow();
		...
		void SetupSerialization(int iSC_Version) {
			StartSerializeList() + m_PK_EventHandler+ m_FK_Event+ m_TimedEvent+ m_Description+ m_FK_Criteria+ m_FK_Installation+ m_FK_CommandGroup+ m_UserCreated+ m_FK_CannedEvents+ m_Disabled+ m_FK_Template+ m_TemplateParm1+ m_TemplateParm2+ m_OncePerSeconds+ m_psc_id+ m_psc_batch+ m_psc_user+ m_psc_frozen+ m_psc_mod+ m_psc_toc+ m_psc_restrict;
		}
	private:
