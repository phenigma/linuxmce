...
mysql_real_escape_string(table->database->m_pMySQL, buf, m_URL.c_str(), (unsigned long) min(255,m_URL.size()));
string s=string()+"\""+buf+"\"";
delete[] buf;
return s;
}
...
mysql_real_escape_string(table->database->m_pMySQL, buf, m_Username.c_str(), (unsigned long) min(30,m_Username.size()));
string s=string()+"\""+buf+"\"";
delete[] buf;
return s;
}
...
char *buf = new char[61];
mysql_real_escape_string(table->database->m_pMySQL, buf, m_Password.c_str(), (unsigned long) min(30,m_Password.size()));
string s=string()+"\""+buf+"\"";
delete[] buf;
return s;
}
...
char *buf = new char[61];
mysql_real_escape_string(table->database->m_pMySQL, buf, m_Comments.c_str(), (unsigned long) min(30,m_Comments.size()));
string s=string()+"\""+buf+"\"";
delete[] buf;
return s;
}
