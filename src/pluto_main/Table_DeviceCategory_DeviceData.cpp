...
values_list_comma_separated = values_list_comma_separated + pRow->FK_DeviceCategory_asSQL()+", "+pRow->FK_DeviceData_asSQL()+", "+pRow->IK_DeviceData_asSQL()+", "+pRow->Required_asSQL()+", "+pRow->SetByDevice_asSQL()+", "+pRow->AllowedToModify_asSQL()+", "+pRow->Description_asSQL()+", "+pRow->ShowInWizard_asSQL()+", "+pRow->ShortDescription_asSQL()+", "+pRow->UseDeviceCategoryDefault_asSQL()+", "+pRow->psc_id_asSQL()+", "+pRow->psc_batch_asSQL()+", "+pRow->psc_user_asSQL()+", "+pRow->psc_frozen_asSQL()+", "+pRow->psc_restrict_asSQL();
		...
		{	
			...
			if( g_pPlutoLogger )
				g_pPlutoLogger->Write(LV_CRITICAL,"Table_DeviceCategory_DeviceData::Commit Cannot perform query [%s] %s",query.c_str(),database->m_sLastMySqlError.c_str());
...
update_values_list = update_values_list + "`FK_DeviceCategory`="+pRow->FK_DeviceCategory_asSQL()+", `FK_DeviceData`="+pRow->FK_DeviceData_asSQL()+", `IK_DeviceData`="+pRow->IK_DeviceData_asSQL()+", `Required`="+pRow->Required_asSQL()+", `SetByDevice`="+pRow->SetByDevice_asSQL()+", `AllowedToModify`="+pRow->AllowedToModify_asSQL()+", `Description`="+pRow->Description_asSQL()+", `ShowInWizard`="+pRow->ShowInWizard_asSQL()+", `ShortDescription`="+pRow->ShortDescription_asSQL()+", `UseDeviceCategoryDefault`="+pRow->UseDeviceCategoryDefault_asSQL()+", `psc_id`="+pRow->psc_id_asSQL()+", `psc_batch`="+pRow->psc_batch_asSQL()+", `psc_user`="+pRow->psc_user_asSQL()+", `psc_frozen`="+pRow->psc_frozen_asSQL()+", `psc_restrict`="+pRow->psc_restrict_asSQL();
		...
		{	
			...
			cerr << "Cannot perform query: [" << query << "] " << database->m_sLastMySqlError << endl;
			if( g_pPlutoLogger )
				g_pPlutoLogger->Write(LV_CRITICAL,"Table_DeviceCategory_DeviceData::Commit Cannot perform update query [%s] %s",query.c_str(),database->m_sLastMySqlError.c_str());
...
condition = condition + "`FK_DeviceCategory`=" + tmp_FK_DeviceCategory+" AND "+"`FK_DeviceData`=" + tmp_FK_DeviceData;
		...
		{	
			...
			cerr << "Cannot perform query: [" << query << "] " << database->m_sLastMySqlError << endl;
			if( g_pPlutoLogger )
				g_pPlutoLogger->Write(LV_CRITICAL,"Table_DeviceCategory_DeviceData::Commit Cannot perform delete query [%s] %s",query.c_str(),database->m_sLastMySqlError.c_str());
			return false;
		}	
...
bool Table_DeviceCategory_DeviceData::GetRows(string where_statement,vector<class Row_DeviceCategory_DeviceData*> *rows)
{
	...
	{	
		...
		cerr << "Cannot perform query: [" << query << "] " << database->m_sLastMySqlError << endl;
		if( g_pPlutoLogger )
			g_pPlutoLogger->Write(LV_CRITICAL,"Table_DeviceCategory_DeviceData::GetRows Cannot perform query [%s] %s",query.c_str(),database->m_sLastMySqlError.c_str());
		return false;
	}	
	...
	if (!res)
	{
		...
		if( g_pPlutoLogger )
			g_pPlutoLogger->Write(LV_CRITICAL,"Table_DeviceCategory_DeviceData::GetRows mysql_store_result returned NULL handler");
		database->m_sLastMySqlError = mysql_error(database->m_pMySQL);
		return false;
	}	
...
condition = condition + "`FK_DeviceCategory`=" + tmp_FK_DeviceCategory+" AND "+"`FK_DeviceData`=" + tmp_FK_DeviceData;
	...
	{	
		...
		cerr << "Cannot perform query: [" << query << "] " << database->m_sLastMySqlError << endl;
		if( g_pPlutoLogger )
			g_pPlutoLogger->Write(LV_CRITICAL,"Table_DeviceCategory_DeviceData::FetchRow Cannot perform query [%s] %s",query.c_str(),database->m_sLastMySqlError.c_str());
		return NULL;
	}	
	...
	if (!res)
	{
		cerr << "mysql_store_result returned NULL handler" << endl;
		if( g_pPlutoLogger )
			g_pPlutoLogger->Write(LV_CRITICAL,"Table_DeviceCategory_DeviceData::FetchRow mysql_store_result returned NULL handler");
		database->m_sLastMySqlError = mysql_error(database->m_pMySQL);
		return NULL;
	}	
