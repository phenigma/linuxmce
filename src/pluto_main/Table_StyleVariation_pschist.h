...
#ifndef __Table_StyleVariation_pschist_H__
#define __Table_StyleVariation_pschist_H__

#include "TableRow.h"
#include "Database_pluto_main.h"
#include "PlutoUtils/MultiThreadIncludes.h"
#include "Define_StyleVariation_pschist.h"
...
class DECLSPECIFIER Table_StyleVariation_pschist : public TableBase , DoubleLongKeyBase
{
private:
...
public:
	Table_StyleVariation_pschist(Database_pluto_main *pDatabase):database(pDatabase)
	{
	};
	~Table_StyleVariation_pschist();

private:		
	friend class Row_StyleVariation_pschist;
	struct Key
	{
		friend class Row_StyleVariation_pschist;
		long int pk_psc_id;
long int pk_psc_batch;
		...
		Key(class Row_StyleVariation_pschist *pRow);
	};
	struct Key_Less
	{			
		bool operator()(const Table_StyleVariation_pschist::Key &key1, const Table_StyleVariation_pschist::Key &key2) const;
	};	
...
public:				
	...
	bool GetRows(string where_statement,vector<class Row_StyleVariation_pschist*> *rows);
	class Row_StyleVariation_pschist* AddRow();
	Database_pluto_main *Database_pluto_main_get() { return database; }
	
		
	class Row_StyleVariation_pschist* GetRow(long int in_psc_id, long int in_psc_batch);
	

private:	
	
		
	class Row_StyleVariation_pschist* FetchRow(DoubleLongKey &key);
		
			
};

class DECLSPECIFIER Row_StyleVariation_pschist : public TableRow, public SerializeClass
	{
		friend struct Table_StyleVariation_pschist::Key;
		friend class Table_StyleVariation_pschist;
	private:
		Table_StyleVariation_pschist *table;
...
void psc_restrict_setNull(bool val);
		...
		Row_StyleVariation_pschist(Table_StyleVariation_pschist *pTable);
	
		bool IsDeleted(){return is_deleted;};
		bool IsModified(){return is_modified;};			
		class Table_StyleVariation_pschist *Table_StyleVariation_pschist_get() { return table; };
...
class Row_VertAlignment* FK_VertAlignment_getrow();
		...
		void SetupSerialization(int iSC_Version) {
			StartSerializeList() + m_PK_StyleVariation+ m_FK_Style+ m_FK_Skin+ m_FK_UI+ m_Font+ m_ForeColor+ m_PixelHeight+ m_Bold+ m_Italic+ m_Underline+ m_ShadowX+ m_ShadowY+ m_ShadowColor+ m_BorderStyle+ m_FK_HorizAlignment+ m_FK_VertAlignment+ m_BackColor+ m_psc_id+ m_psc_batch+ m_psc_user+ m_psc_frozen+ m_psc_mod+ m_psc_toc+ m_psc_restrict;
		}
	private:
