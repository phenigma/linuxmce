...
#ifndef __Table_Criteria_pschmask_H__
#define __Table_Criteria_pschmask_H__

#include "TableRow.h"
#include "Database_pluto_main.h"
#include "PlutoUtils/MultiThreadIncludes.h"
#include "Define_Criteria_pschmask.h"
...
class DECLSPECIFIER Table_Criteria_pschmask : public TableBase , DoubleLongKeyBase
{
private:
...
public:
	Table_Criteria_pschmask(Database_pluto_main *pDatabase):database(pDatabase)
	{
	};
	~Table_Criteria_pschmask();

private:		
	friend class Row_Criteria_pschmask;
	struct Key
	{
		friend class Row_Criteria_pschmask;
		long int pk_psc_id;
long int pk_psc_batch;
		...
		Key(class Row_Criteria_pschmask *pRow);
	};
	struct Key_Less
	{			
		bool operator()(const Table_Criteria_pschmask::Key &key1, const Table_Criteria_pschmask::Key &key2) const;
	};	
...
public:				
	...
	bool GetRows(string where_statement,vector<class Row_Criteria_pschmask*> *rows);
	class Row_Criteria_pschmask* AddRow();
	Database_pluto_main *Database_pluto_main_get() { return database; }
	
		
	class Row_Criteria_pschmask* GetRow(long int in_psc_id, long int in_psc_batch);
	

private:	
	
		
	class Row_Criteria_pschmask* FetchRow(DoubleLongKey &key);
		
			
};

class DECLSPECIFIER Row_Criteria_pschmask : public TableRow, public SerializeClass
	{
		friend struct Table_Criteria_pschmask::Key;
		friend class Table_Criteria_pschmask;
	private:
		Table_Criteria_pschmask *table;
		
		short int m_PK_Criteria;
...
long int psc_restrict_get();
		...
		void PK_Criteria_set(short int val);
...
void psc_restrict_setNull(bool val);
		...
		Row_Criteria_pschmask(Table_Criteria_pschmask *pTable);
	
		bool IsDeleted(){return is_deleted;};
		bool IsModified(){return is_modified;};			
		class Table_Criteria_pschmask *Table_Criteria_pschmask_get() { return table; };
