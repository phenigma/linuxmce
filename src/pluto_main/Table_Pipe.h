...
class DECLSPECIFIER Table_Pipe : public TableBase , SingleLongKeyBase
{
private:
...
public:
	Table_Pipe(Database_pluto_main *pDatabase):database(pDatabase)
	{
	};
	~Table_Pipe();
...
class DECLSPECIFIER Row_Pipe : public TableRow, public SerializeClass
	{
...
void Pipe_FK_Pipe_UsePath_getrows(vector <class Row_Pipe*> *rows);
		...
		void SetupSerialization(int iSC_Version) {
			StartSerializeList() + m_PK_Pipe+ m_Description+ m_FK_Pipe_UsePath+ m_psc_id+ m_psc_batch+ m_psc_user+ m_psc_frozen+ m_psc_mod+ m_psc_restrict;
		}
	private:
