...
return m_psc_id;}
long int Row_SetupStep::psc_batch_get(){PLUTO_SAFETY_LOCK(M, table->m_Mutex);

return m_psc_batch;}
long int Row_SetupStep::psc_user_get(){PLUTO_SAFETY_LOCK(M, table->m_Mutex);

return m_psc_user;}
short int Row_SetupStep::psc_frozen_get(){PLUTO_SAFETY_LOCK(M, table->m_Mutex);

return m_psc_frozen;}
string Row_SetupStep::psc_mod_get(){PLUTO_SAFETY_LOCK(M, table->m_Mutex);
...
bool Table_SetupStep::GetRows(string where_statement,vector<class Row_SetupStep*> *rows)
{
	...
	if( StringUtils::StartsWith(where_statement,"where ",true) || StringUtils::StartsWith(where_statement,"join ",true) )
		query = "select * from SetupStep " + where_statement;
	else if( StringUtils::StartsWith(where_statement,"select ",true) )
		query = where_statement;
	else
		query = "select * from SetupStep where " + where_statement;
