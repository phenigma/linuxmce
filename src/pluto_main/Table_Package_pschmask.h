...
#ifndef __Table_Package_pschmask_H__
#define __Table_Package_pschmask_H__

#include "TableRow.h"
#include "Database_pluto_main.h"
#include "PlutoUtils/MultiThreadIncludes.h"
#include "Define_Package_pschmask.h"
...
class DECLSPECIFIER Table_Package_pschmask : public TableBase , DoubleLongKeyBase
{
private:
...
public:
	Table_Package_pschmask(Database_pluto_main *pDatabase):database(pDatabase)
	{
	};
	~Table_Package_pschmask();

private:		
	friend class Row_Package_pschmask;
	struct Key
	{
		friend class Row_Package_pschmask;
		long int pk_psc_id;
long int pk_psc_batch;
		...
		Key(class Row_Package_pschmask *pRow);
	};
	struct Key_Less
	{			
		bool operator()(const Table_Package_pschmask::Key &key1, const Table_Package_pschmask::Key &key2) const;
	};	
...
public:				
	...
	bool GetRows(string where_statement,vector<class Row_Package_pschmask*> *rows);
	class Row_Package_pschmask* AddRow();
	Database_pluto_main *Database_pluto_main_get() { return database; }
	
		
	class Row_Package_pschmask* GetRow(long int in_psc_id, long int in_psc_batch);
	

private:	
	
		
	class Row_Package_pschmask* FetchRow(DoubleLongKey &key);
		
			
};

class DECLSPECIFIER Row_Package_pschmask : public TableRow, public SerializeClass
	{
		friend struct Table_Package_pschmask::Key;
		friend class Table_Package_pschmask;
	private:
		Table_Package_pschmask *table;
		
		short int m_PK_Package;
short int m_Description;
short int m_FK_PackageType;
short int m_FK_Package_Sourcecode;
...
short int m_HelpURL;
short int m_HelpURL_UsersManual;
short int m_HelpURL_ProgrammersGuide;
...
short int m_FK_Document_ProgrammersGuide;
short int m_FK_Document_UsersManual;

		bool is_null[22];
	
	public:
		short int PK_Package_get();
short int Description_get();
short int FK_PackageType_get();
short int FK_Package_Sourcecode_get();
...
short int HelpURL_UsersManual_get();
short int HelpURL_ProgrammersGuide_get();
...
short int FK_Document_ProgrammersGuide_get();
short int FK_Document_UsersManual_get();

		
		void PK_Package_set(short int val);
void Description_set(short int val);
void FK_PackageType_set(short int val);
void FK_Package_Sourcecode_set(short int val);
...
void HelpURL_UsersManual_set(short int val);
void HelpURL_ProgrammersGuide_set(short int val);
...
void FK_Document_ProgrammersGuide_set(short int val);
void FK_Document_UsersManual_set(short int val);
...
void FK_Document_UsersManual_setNull(bool val);
		...
		Row_Package_pschmask(Table_Package_pschmask *pTable);
	
		bool IsDeleted(){return is_deleted;};
		bool IsModified(){return is_modified;};			
		class Table_Package_pschmask *Table_Package_pschmask_get() { return table; };
