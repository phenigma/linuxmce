...
{
	if( tblpsc_local_schema )
		delete tblpsc_local_schema;
}

bool Database_pluto_main::Commit_psc_local_schema(bool bDeleteFailedModifiedRow,bool bDeleteFailedInsertRow)
{
	return tblpsc_local_schema->Commit(bDeleteFailedModifiedRow,bDeleteFailedInsertRow);
}
...
long int Row_psc_local_schema::PK_psc_local_schema_get(){PLUTO_SAFETY_LOCK_ERRORSONLY(sl,table->database->m_DBMutex);

return m_PK_psc_local_schema;}
string Row_psc_local_schema::Value_get(){PLUTO_SAFETY_LOCK_ERRORSONLY(sl,table->database->m_DBMutex);

return m_Value;}

		
void Row_psc_local_schema::PK_psc_local_schema_set(long int val){PLUTO_SAFETY_LOCK_ERRORSONLY(sl,table->database->m_DBMutex);

m_PK_psc_local_schema = val; is_modified=true; is_null[0]=false;}
void Row_psc_local_schema::Value_set(string val){PLUTO_SAFETY_LOCK_ERRORSONLY(sl,table->database->m_DBMutex);

m_Value = val; is_modified=true; is_null[1]=false;}

		
bool Row_psc_local_schema::Value_isNull() {PLUTO_SAFETY_LOCK_ERRORSONLY(sl,table->database->m_DBMutex);

return is_null[1];}

			
void Row_psc_local_schema::Value_setNull(bool val){PLUTO_SAFETY_LOCK_ERRORSONLY(sl,table->database->m_DBMutex);
is_null[1]=val;
is_modified=true;
}
...
bool Table_psc_local_schema::Commit(bool bDeleteFailedModifiedRow,bool bDeleteFailedInsertRow)
{
...
values_list_comma_separated = values_list_comma_separated + pRow->PK_psc_local_schema_asSQL()+", "+pRow->Value_asSQL();
		...
		{	
			...
			LoggerWrapper::GetInstance()->Write(LV_CRITICAL,"Table_psc_local_schema::Commit Cannot perform query [%s] %s reconnect: %d result2: %d",query.c_str(),database->m_sLastDBError.c_str(),(int) bResult, iResult2);
		...
		LoggerWrapper::GetInstance()->Write(LV_CRITICAL,"PK_psc_local_schema is auto increment but has no value %s",database->m_sLastDBError.c_str());	
...
update_values_list = update_values_list + "`PK_psc_local_schema`="+pRow->PK_psc_local_schema_asSQL()+", `Value`="+pRow->Value_asSQL();
		...
		{	
			...
			LoggerWrapper::GetInstance()->Write(LV_CRITICAL,"Table_psc_local_schema::Commit Cannot perform update query [%s] %s reconnect: %d result2: %d",query.c_str(),database->m_sLastDBError.c_str(),(int) bResult, iResult2);
			if( iResult2!=0 )  // We can keep going if the time it worked
			{
...
condition = condition + "`PK_psc_local_schema`=" + tmp_PK_psc_local_schema;
		...
		{	
			...
			LoggerWrapper::GetInstance()->Write(LV_CRITICAL,"Table_psc_local_schema::Commit Cannot perform delete query [%s] %s reconnect: %d result2: %d",query.c_str(),database->m_sLastDBError.c_str(),(int) bResult, iResult2);
			if( iResult2!=0 )  // We can keep going if the time it worked
				return false;
		}	
...
bool Table_psc_local_schema::GetRows(string where_statement,vector<class Row_psc_local_schema*> *rows)
{
	...
	if( StringUtils::StartsWith(where_statement,"where ",true) || 
		...
		query = "select `psc_local_schema`.* from psc_local_schema " + where_statement;
	else if( StringUtils::StartsWith(where_statement,"select ",true) )
		query = where_statement;
	else if( where_statement.size() )
		query = "select `psc_local_schema`.* from psc_local_schema where " + where_statement;
	else
		query = "select `psc_local_schema`.* from psc_local_schema";
	...
	{	
		...
		LoggerWrapper::GetInstance()->Write(LV_CRITICAL,"Table_psc_local_schema::GetRows Cannot perform query [%s] %s reconnect: %d result2: %d",query.c_str(),database->m_sLastDBError.c_str(),(int) bResult, iResult2);
		if( iResult2!=0 )  // We can keep going if the time it worked
			return false;
	}	
	...
	if (!res)
	{
		...
		LoggerWrapper::GetInstance()->Write(LV_CRITICAL,"Table_psc_local_schema::GetRows db_wrapper_store_result returned NULL handler");
		database->m_sLastDBError = db_wrapper_error(database->m_pDB);
		return false;
	}	
...
condition = condition + "`PK_psc_local_schema`=" + tmp_PK_psc_local_schema;
	...
	{	
		...
		LoggerWrapper::GetInstance()->Write(LV_CRITICAL,"Table_psc_local_schema::FetchRow Cannot perform query [%s] %s reconnect: %d result2: %d",query.c_str(),database->m_sLastDBError.c_str(),(int) bResult, iResult2);
		if( iResult2!=0 )  // We can keep going if the time it worked
			return NULL;
	}	
	...
	{
		cerr << "db_wrapper_store_result returned NULL handler" << endl;
		LoggerWrapper::GetInstance()->Write(LV_CRITICAL,"Table_psc_local_schema::FetchRow db_wrapper_store_result returned NULL handler");
		database->m_sLastDBError = db_wrapper_error(database->m_pDB);
		return NULL;
	}	
