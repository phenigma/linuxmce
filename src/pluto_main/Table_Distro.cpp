...
return m_psc_mod;}
long int Row_Distro::psc_restrict_get(){PLUTO_SAFETY_LOCK_ERRORSONLY(sl,table->database->m_MySqlMutex);
...
void Row_Distro::psc_restrict_set(long int val){PLUTO_SAFETY_LOCK_ERRORSONLY(sl,table->database->m_MySqlMutex);
...
return is_null[16];}
bool Row_Distro::psc_restrict_isNull() {PLUTO_SAFETY_LOCK_ERRORSONLY(sl,table->database->m_MySqlMutex);
...
void Row_Distro::psc_restrict_setNull(bool val){PLUTO_SAFETY_LOCK_ERRORSONLY(sl,table->database->m_MySqlMutex);
is_null[18]=val;
is_modified=true;
}
...
mysql_real_escape_string(table->database->m_pMySQL, buf, m_Installer.c_str(), (unsigned long) min((size_t)100,m_Installer.size()));
string s=string()+"\""+buf+"\"";
delete[] buf;
return s;
}
...
char *buf = new char[201];
mysql_real_escape_string(table->database->m_pMySQL, buf, m_KickStartCD.c_str(), (unsigned long) min((size_t)100,m_KickStartCD.size()));
string s=string()+"\""+buf+"\"";
delete[] buf;
return s;
}
...
char *buf = new char[201];
mysql_real_escape_string(table->database->m_pMySQL, buf, m_Binaries.c_str(), (unsigned long) min((size_t)100,m_Binaries.size()));
string s=string()+"\""+buf+"\"";
delete[] buf;
return s;
}
...
char *buf = new char[201];
mysql_real_escape_string(table->database->m_pMySQL, buf, m_SourceCode.c_str(), (unsigned long) min((size_t)100,m_SourceCode.size()));
string s=string()+"\""+buf+"\"";
delete[] buf;
return s;
}
...
char *buf = new char[201];
mysql_real_escape_string(table->database->m_pMySQL, buf, m_InstallerURL.c_str(), (unsigned long) min((size_t)100,m_InstallerURL.size()));
string s=string()+"\""+buf+"\"";
delete[] buf;
return s;
}
