...
values_list_comma_separated = values_list_comma_separated + pRow->PK_DeviceTemplate_MediaType_asSQL()+", "+pRow->FK_DeviceTemplate_asSQL()+", "+pRow->FK_MediaType_asSQL()+", "+pRow->CanPlayFromDiskDrive_asSQL()+", "+pRow->CanStoreOnServer_asSQL()+", "+pRow->CanSetPosition_asSQL()+", "+pRow->CanPlayInMultipleAreas_asSQL()+", "+pRow->StopOtherMediaInEntArea_asSQL()+", "+pRow->Extensions_asSQL()+", "+pRow->psc_id_asSQL()+", "+pRow->psc_batch_asSQL()+", "+pRow->psc_user_asSQL()+", "+pRow->psc_frozen_asSQL()+", "+pRow->psc_restrict_asSQL();
		...
		{	
			...
			LoggerWrapper::GetInstance()->Write(LV_CRITICAL,"Table_DeviceTemplate_MediaType::Commit Cannot perform query [%s] %s reconnect: %d result2: %d",query.c_str(),database->m_sLastMySqlError.c_str(),(int) bResult, iResult2);
		...
		LoggerWrapper::GetInstance()->Write(LV_CRITICAL,"PK_DeviceTemplate_MediaType is auto increment but has no value %s",database->m_sLastMySqlError.c_str());	
...
update_values_list = update_values_list + "`PK_DeviceTemplate_MediaType`="+pRow->PK_DeviceTemplate_MediaType_asSQL()+", `FK_DeviceTemplate`="+pRow->FK_DeviceTemplate_asSQL()+", `FK_MediaType`="+pRow->FK_MediaType_asSQL()+", `CanPlayFromDiskDrive`="+pRow->CanPlayFromDiskDrive_asSQL()+", `CanStoreOnServer`="+pRow->CanStoreOnServer_asSQL()+", `CanSetPosition`="+pRow->CanSetPosition_asSQL()+", `CanPlayInMultipleAreas`="+pRow->CanPlayInMultipleAreas_asSQL()+", `StopOtherMediaInEntArea`="+pRow->StopOtherMediaInEntArea_asSQL()+", `Extensions`="+pRow->Extensions_asSQL()+", `psc_id`="+pRow->psc_id_asSQL()+", `psc_batch`="+pRow->psc_batch_asSQL()+", `psc_user`="+pRow->psc_user_asSQL()+", `psc_frozen`="+pRow->psc_frozen_asSQL()+", `psc_restrict`="+pRow->psc_restrict_asSQL();
		...
		{	
			...
			LoggerWrapper::GetInstance()->Write(LV_CRITICAL,"Table_DeviceTemplate_MediaType::Commit Cannot perform update query [%s] %s reconnect: %d result2: %d",query.c_str(),database->m_sLastMySqlError.c_str(),(int) bResult, iResult2);
			if( iResult2!=0 )  // We can keep going if the time it worked
			{
...
condition = condition + "`PK_DeviceTemplate_MediaType`=" + tmp_PK_DeviceTemplate_MediaType;
		...
		{	
			...
			LoggerWrapper::GetInstance()->Write(LV_CRITICAL,"Table_DeviceTemplate_MediaType::Commit Cannot perform delete query [%s] %s reconnect: %d result2: %d",query.c_str(),database->m_sLastMySqlError.c_str(),(int) bResult, iResult2);
			if( iResult2!=0 )  // We can keep going if the time it worked
				return false;
		}	
...
bool Table_DeviceTemplate_MediaType::GetRows(string where_statement,vector<class Row_DeviceTemplate_MediaType*> *rows)
{
	...
	{	
		...
		LoggerWrapper::GetInstance()->Write(LV_CRITICAL,"Table_DeviceTemplate_MediaType::GetRows Cannot perform query [%s] %s reconnect: %d result2: %d",query.c_str(),database->m_sLastMySqlError.c_str(),(int) bResult, iResult2);
		if( iResult2!=0 )  // We can keep going if the time it worked
			return false;
	}	
	...
	if (!res)
	{
		...
		LoggerWrapper::GetInstance()->Write(LV_CRITICAL,"Table_DeviceTemplate_MediaType::GetRows mysql_store_result returned NULL handler");
		database->m_sLastMySqlError = mysql_error(database->m_pMySQL);
		return false;
	}	
...
condition = condition + "`PK_DeviceTemplate_MediaType`=" + tmp_PK_DeviceTemplate_MediaType;
	...
	{	
		...
		LoggerWrapper::GetInstance()->Write(LV_CRITICAL,"Table_DeviceTemplate_MediaType::FetchRow Cannot perform query [%s] %s reconnect: %d result2: %d",query.c_str(),database->m_sLastMySqlError.c_str(),(int) bResult, iResult2);
		if( iResult2!=0 )  // We can keep going if the time it worked
			return NULL;
	}	
	...
	{
		cerr << "mysql_store_result returned NULL handler" << endl;
		LoggerWrapper::GetInstance()->Write(LV_CRITICAL,"Table_DeviceTemplate_MediaType::FetchRow mysql_store_result returned NULL handler");
		database->m_sLastMySqlError = mysql_error(database->m_pMySQL);
		return NULL;
	}	
