...
values_list_comma_separated = values_list_comma_separated + pRow->FK_DeviceCommandGroup_asSQL()+", "+pRow->FK_DeviceCommandGroup_Parent_asSQL()+", "+pRow->psc_id_asSQL()+", "+pRow->psc_batch_asSQL()+", "+pRow->psc_user_asSQL()+", "+pRow->psc_frozen_asSQL()+", "+pRow->psc_restrict_asSQL();
		...
		{	
			...
			LoggerWrapper::GetInstance()->Write(LV_CRITICAL,"Table_DeviceCommandGroup_DeviceCommandGroup_Parent::Commit Cannot perform query [%s] %s reconnect: %d",query.c_str(),database->m_sLastMySqlError.c_str(),(int) bResult);
...
update_values_list = update_values_list + "`FK_DeviceCommandGroup`="+pRow->FK_DeviceCommandGroup_asSQL()+", `FK_DeviceCommandGroup_Parent`="+pRow->FK_DeviceCommandGroup_Parent_asSQL()+", `psc_id`="+pRow->psc_id_asSQL()+", `psc_batch`="+pRow->psc_batch_asSQL()+", `psc_user`="+pRow->psc_user_asSQL()+", `psc_frozen`="+pRow->psc_frozen_asSQL()+", `psc_restrict`="+pRow->psc_restrict_asSQL();
		...
		{	
			...
			cerr << "Cannot perform query: [" << query << "] " << database->m_sLastMySqlError << endl;
			bool bResult=database->MySQLConnect(true);
			LoggerWrapper::GetInstance()->Write(LV_CRITICAL,"Table_DeviceCommandGroup_DeviceCommandGroup_Parent::Commit Cannot perform update query [%s] %s reconnect: %d",query.c_str(),database->m_sLastMySqlError.c_str(),(int) bResult);
...
condition = condition + "`FK_DeviceCommandGroup`=" + tmp_FK_DeviceCommandGroup+" AND "+"`FK_DeviceCommandGroup_Parent`=" + tmp_FK_DeviceCommandGroup_Parent;
		...
		{	
			...
			cerr << "Cannot perform query: [" << query << "] " << database->m_sLastMySqlError << endl;
			bool bResult=database->MySQLConnect(true);
			LoggerWrapper::GetInstance()->Write(LV_CRITICAL,"Table_DeviceCommandGroup_DeviceCommandGroup_Parent::Commit Cannot perform delete query [%s] %s reconnect: %d",query.c_str(),database->m_sLastMySqlError.c_str(),(int) bResult);
			return false;
		}	
...
bool Table_DeviceCommandGroup_DeviceCommandGroup_Parent::GetRows(string where_statement,vector<class Row_DeviceCommandGroup_DeviceCommandGroup_Parent*> *rows)
{
	...
	{	
		...
		cerr << "Cannot perform query: [" << query << "] " << database->m_sLastMySqlError << endl;
		bool bResult=database->MySQLConnect(true);
		LoggerWrapper::GetInstance()->Write(LV_CRITICAL,"Table_DeviceCommandGroup_DeviceCommandGroup_Parent::GetRows Cannot perform query [%s] %s reconnect: %d",query.c_str(),database->m_sLastMySqlError.c_str(),(int) bResult);
		return false;
	}	
...
condition = condition + "`FK_DeviceCommandGroup`=" + tmp_FK_DeviceCommandGroup+" AND "+"`FK_DeviceCommandGroup_Parent`=" + tmp_FK_DeviceCommandGroup_Parent;
	...
	{	
		...
		cerr << "Cannot perform query: [" << query << "] " << database->m_sLastMySqlError << endl;
		bool bResult=database->MySQLConnect(true);
		LoggerWrapper::GetInstance()->Write(LV_CRITICAL,"Table_DeviceCommandGroup_DeviceCommandGroup_Parent::FetchRow Cannot perform query [%s] %s reconnect: %d",query.c_str(),database->m_sLastMySqlError.c_str(),(int) bResult);
		return NULL;
	}	
