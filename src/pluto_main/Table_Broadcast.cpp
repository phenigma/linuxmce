...
return is_null[6];}
bool Row_Broadcast::psc_mod_isNull() {PLUTO_SAFETY_LOCK_ERRORSONLY(sl,table->database->m_DBMutex);
...
void Row_Broadcast::psc_mod_setNull(bool val){PLUTO_SAFETY_LOCK_ERRORSONLY(sl,table->database->m_DBMutex);
is_null[7]=val;
is_modified=true;
}
...
db_wrapper_real_escape_string(table->database->m_pDB, buf, m_Extensions.c_str(), (unsigned long) min((size_t)60,m_Extensions.size()));
string s=string()+"\""+buf+"\"";
delete[] buf;
return s;
}
