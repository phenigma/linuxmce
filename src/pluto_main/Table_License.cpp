...
mysql_real_escape_string(table->database->m_pMySQL, buf, m_Summary.c_str(), (unsigned long) min(16777215,m_Summary.size()));
string s=string()+"\""+buf+"\"";
delete[] buf;
return s;
}
...
mysql_real_escape_string(table->database->m_pMySQL, buf, m_URL.c_str(), (unsigned long) min(100,m_URL.size()));
string s=string()+"\""+buf+"\"";
delete[] buf;
return s;
}
