...
class DECLSPECIFIER Table_CommandGroup : public TableBase , SingleLongKeyBase
{
private:
...
public:
	Table_CommandGroup(Database_pluto_main *pDatabase):database(pDatabase)
	{
	};
	~Table_CommandGroup();
...
class DECLSPECIFIER Row_CommandGroup : public TableRow, public SerializeClass
	{
...
void EventHandler_FK_CommandGroup_getrows(vector <class Row_EventHandler*> *rows);
		...
		void SetupSerialization(int iSC_Version) {
			StartSerializeList() + m_PK_CommandGroup+ m_FK_Array+ m_FK_Installation+ m_Description+ m_Hint+ m_CanTurnOff+ m_AlwaysShow+ m_CanBeHidden+ m_FK_Criteria_Orbiter+ m_FK_DesignObj+ m_FK_Template+ m_AltID+ m_FK_Icon+ m_psc_id+ m_psc_batch+ m_psc_user+ m_psc_frozen+ m_psc_mod+ m_psc_restrict;
		}
	private:
