...
class DECLSPECIFIER Table_CriteriaParmList : public TableBase , SingleLongKeyBase
{
private:
...
public:
	Table_CriteriaParmList(Database_pluto_main *pDatabase):database(pDatabase)
	{
	};
...
private:	
	...
	class Row_CriteriaParmList* FetchRow(SingleLongKey &key);
		
			
};

class DECLSPECIFIER Row_CriteriaParmList : public TableRow, public SerializeClass
	{
...
void CriteriaParm_D_FK_CriteriaParmList_getrows(vector <class Row_CriteriaParm_D*> *rows);
		...
		void SetupSerialization(int iSC_Version) {
			StartSerializeList() + m_PK_CriteriaParmList+ m_FK_ParameterType+ m_Description+ m_Define+ m_psc_id+ m_psc_batch+ m_psc_user+ m_psc_frozen+ m_psc_mod;
		}
	private:
