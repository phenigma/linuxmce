...
#ifndef __Table_Room_pschmask_H__
#define __Table_Room_pschmask_H__

#include "TableRow.h"
#include "Database_pluto_main.h"
#include "PlutoUtils/MultiThreadIncludes.h"
#include "Define_Room_pschmask.h"
...
class DECLSPECIFIER Table_Room_pschmask : public TableBase , DoubleLongKeyBase
{
private:
...
public:
	Table_Room_pschmask(Database_pluto_main *pDatabase):database(pDatabase)
	{
	};
	~Table_Room_pschmask();

private:		
	friend class Row_Room_pschmask;
	struct Key
	{
		friend class Row_Room_pschmask;
		long int pk_psc_id;
long int pk_psc_batch;
		...
		Key(class Row_Room_pschmask *pRow);
	};
	struct Key_Less
	{			
		bool operator()(const Table_Room_pschmask::Key &key1, const Table_Room_pschmask::Key &key2) const;
	};	
...
public:				
	...
	bool GetRows(string where_statement,vector<class Row_Room_pschmask*> *rows);
	class Row_Room_pschmask* AddRow();
	Database_pluto_main *Database_pluto_main_get() { return database; }
	
		
	class Row_Room_pschmask* GetRow(long int in_psc_id, long int in_psc_batch);
	

private:	
	
		
	class Row_Room_pschmask* FetchRow(DoubleLongKey &key);
		
			
};

class DECLSPECIFIER Row_Room_pschmask : public TableRow, public SerializeClass
	{
		friend struct Table_Room_pschmask::Key;
		friend class Table_Room_pschmask;
	private:
		Table_Room_pschmask *table;
		
		short int m_PK_Room;
short int m_FK_Installation;
short int m_FK_RoomType;
...
long int m_psc_restrict;
	...
	public:
		short int PK_Room_get();
short int FK_Installation_get();
short int FK_RoomType_get();
...
long int psc_restrict_get();
		...
		void PK_Room_set(short int val);
void FK_Installation_set(short int val);
void FK_RoomType_set(short int val);
...
void psc_restrict_setNull(bool val);
		...
		Row_Room_pschmask(Table_Room_pschmask *pTable);
	
		bool IsDeleted(){return is_deleted;};
		bool IsModified(){return is_modified;};			
		class Table_Room_pschmask *Table_Room_pschmask_get() { return table; };
