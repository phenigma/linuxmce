...
#ifndef __Table_Skin_pschist_H__
#define __Table_Skin_pschist_H__

#include "TableRow.h"
#include "Database_pluto_main.h"
#include "PlutoUtils/MultiThreadIncludes.h"
#include "Define_Skin_pschist.h"
...
class DECLSPECIFIER Table_Skin_pschist : public TableBase , DoubleLongKeyBase
{
private:
...
public:
	Table_Skin_pschist(Database_pluto_main *pDatabase):database(pDatabase)
	{
	};
	~Table_Skin_pschist();

private:		
	friend class Row_Skin_pschist;
	struct Key
	{
		friend class Row_Skin_pschist;
		long int pk_psc_id;
long int pk_psc_batch;
		...
		Key(class Row_Skin_pschist *pRow);
	};
	struct Key_Less
	{			
		bool operator()(const Table_Skin_pschist::Key &key1, const Table_Skin_pschist::Key &key2) const;
	};	
...
public:				
	...
	bool GetRows(string where_statement,vector<class Row_Skin_pschist*> *rows);
	class Row_Skin_pschist* AddRow();
	Database_pluto_main *Database_pluto_main_get() { return database; }
	
		
	class Row_Skin_pschist* GetRow(long int in_psc_id, long int in_psc_batch);
	

private:	
	
		
	class Row_Skin_pschist* FetchRow(DoubleLongKey &key);
		
			
};

class DECLSPECIFIER Row_Skin_pschist : public TableRow, public SerializeClass
	{
		friend struct Table_Skin_pschist::Key;
		friend class Table_Skin_pschist;
	private:
		Table_Skin_pschist *table;
...
void psc_restrict_setNull(bool val);
		...
		Row_Skin_pschist(Table_Skin_pschist *pTable);
	
		bool IsDeleted(){return is_deleted;};
		bool IsModified(){return is_modified;};			
		class Table_Skin_pschist *Table_Skin_pschist_get() { return table; };
