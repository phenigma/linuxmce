...
class DECLSPECIFIER Table_DataGrid : public TableBase , SingleLongKeyBase
{
private:
...
public:
	Table_DataGrid(Database_pluto_main *pDatabase):database(pDatabase)
	{
	};
	~Table_DataGrid();
...
private:	
	...
	class Row_DataGrid* FetchRow(SingleLongKey &key);
		
			
};

class DECLSPECIFIER Row_DataGrid : public TableRow, public SerializeClass
	{
...
void psc_restrict_setNull(bool val);
		...
		void SetupSerialization(int iSC_Version) {
			StartSerializeList() + m_PK_DataGrid+ m_Description+ m_Define+ m_Comments+ m_psc_id+ m_psc_batch+ m_psc_user+ m_psc_frozen+ m_psc_mod+ m_psc_restrict;
		}
	private:
