...
class DECLSPECIFIER Table_RoomType : public TableBase , SingleLongKeyBase
{
private:
...
public:
	Table_RoomType(Database_pluto_main *pDatabase):database(pDatabase)
	{
	};
	~Table_RoomType();
...
private:	
	...
	class Row_RoomType* FetchRow(SingleLongKey &key);
		
			
};

class DECLSPECIFIER Row_RoomType : public TableRow, public SerializeClass
	{
...
void Room_pschist_FK_RoomType_getrows(vector <class Row_Room_pschist*> *rows);
void Room_pschmask_FK_RoomType_getrows(vector <class Row_Room_pschmask*> *rows);
