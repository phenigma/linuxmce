...
#ifndef __Table_CriteriaParm_pschist_H__
#define __Table_CriteriaParm_pschist_H__

#include "TableRow.h"
#include "Database_pluto_main.h"
#include "PlutoUtils/MultiThreadIncludes.h"
#include "Define_CriteriaParm_pschist.h"
...
class DECLSPECIFIER Table_CriteriaParm_pschist : public TableBase , DoubleLongKeyBase
{
private:
...
public:
	Table_CriteriaParm_pschist(Database_pluto_main *pDatabase):database(pDatabase)
	{
	};
	~Table_CriteriaParm_pschist();

private:		
	friend class Row_CriteriaParm_pschist;
	struct Key
	{
		friend class Row_CriteriaParm_pschist;
		long int pk_psc_id;
long int pk_psc_batch;
		...
		Key(class Row_CriteriaParm_pschist *pRow);
	};
	struct Key_Less
	{			
		bool operator()(const Table_CriteriaParm_pschist::Key &key1, const Table_CriteriaParm_pschist::Key &key2) const;
	};	
...
public:				
	...
	bool GetRows(string where_statement,vector<class Row_CriteriaParm_pschist*> *rows);
	class Row_CriteriaParm_pschist* AddRow();
	Database_pluto_main *Database_pluto_main_get() { return database; }
	
		
	class Row_CriteriaParm_pschist* GetRow(long int in_psc_id, long int in_psc_batch);
	

private:	
	
		
	class Row_CriteriaParm_pschist* FetchRow(DoubleLongKey &key);
		
			
};

class DECLSPECIFIER Row_CriteriaParm_pschist : public TableRow, public SerializeClass
	{
		friend struct Table_CriteriaParm_pschist::Key;
		friend class Table_CriteriaParm_pschist;
	private:
		Table_CriteriaParm_pschist *table;
...
void psc_restrict_setNull(bool val);
		...
		Row_CriteriaParm_pschist(Table_CriteriaParm_pschist *pTable);
	
		bool IsDeleted(){return is_deleted;};
		bool IsModified(){return is_modified;};			
		class Table_CriteriaParm_pschist *Table_CriteriaParm_pschist_get() { return table; };
...
class Row_CannedEvents_CriteriaParmList* FK_CannedEvents_CriteriaParmList_getrow();
		...
		void SetupSerialization(int iSC_Version) {
			StartSerializeList() + m_PK_CriteriaParm+ m_FK_CriteriaParmNesting+ m_FK_CriteriaParmList+ m_Parm+ m_Operator+ m_Value+ m_FK_CannedEvents_CriteriaParmList+ m_psc_id+ m_psc_batch+ m_psc_user+ m_psc_frozen+ m_psc_mod+ m_psc_toc+ m_psc_restrict;
		}
	private:
