...
values_list_comma_separated = values_list_comma_separated + pRow->PK_PostalCode_asSQL()+", "+pRow->PostalCode_asSQL()+", "+pRow->Long_asSQL()+", "+pRow->Lat_asSQL()+", "+pRow->City_asSQL()+", "+pRow->State_asSQL()+", "+pRow->County_asSQL()+", "+pRow->FK_City_asSQL()+", "+pRow->FK_Country_asSQL();
		...
		{	
			...
			LoggerWrapper::GetInstance()->Write(LV_CRITICAL,"Table_PostalCode::Commit Cannot perform query [%s] %s reconnect: %d result2: %d",query.c_str(),database->m_sLastMySqlError.c_str(),(int) bResult, iResult2);
...
update_values_list = update_values_list + "`PK_PostalCode`="+pRow->PK_PostalCode_asSQL()+", `PostalCode`="+pRow->PostalCode_asSQL()+", `Long`="+pRow->Long_asSQL()+", `Lat`="+pRow->Lat_asSQL()+", `City`="+pRow->City_asSQL()+", `State`="+pRow->State_asSQL()+", `County`="+pRow->County_asSQL()+", `FK_City`="+pRow->FK_City_asSQL()+", `FK_Country`="+pRow->FK_Country_asSQL();
		...
		{	
			...
			LoggerWrapper::GetInstance()->Write(LV_CRITICAL,"Table_PostalCode::Commit Cannot perform update query [%s] %s reconnect: %d result2: %d",query.c_str(),database->m_sLastMySqlError.c_str(),(int) bResult, iResult2);
			if( iResult2!=0 )  // We can keep going if the time it worked
			{
...
condition = condition + "`PK_PostalCode`=" + tmp_PK_PostalCode;
		...
		{	
			...
			LoggerWrapper::GetInstance()->Write(LV_CRITICAL,"Table_PostalCode::Commit Cannot perform delete query [%s] %s reconnect: %d result2: %d",query.c_str(),database->m_sLastMySqlError.c_str(),(int) bResult, iResult2);
			if( iResult2!=0 )  // We can keep going if the time it worked
				return false;
		}	
...
bool Table_PostalCode::GetRows(string where_statement,vector<class Row_PostalCode*> *rows)
{
	...
	{	
		...
		LoggerWrapper::GetInstance()->Write(LV_CRITICAL,"Table_PostalCode::GetRows Cannot perform query [%s] %s reconnect: %d result2: %d",query.c_str(),database->m_sLastMySqlError.c_str(),(int) bResult, iResult2);
		if( iResult2!=0 )  // We can keep going if the time it worked
			return false;
	}	
...
condition = condition + "`PK_PostalCode`=" + tmp_PK_PostalCode;
	...
	{	
		...
		LoggerWrapper::GetInstance()->Write(LV_CRITICAL,"Table_PostalCode::FetchRow Cannot perform query [%s] %s reconnect: %d result2: %d",query.c_str(),database->m_sLastMySqlError.c_str(),(int) bResult, iResult2);
		if( iResult2!=0 )  // We can keep going if the time it worked
			return NULL;
	}	
