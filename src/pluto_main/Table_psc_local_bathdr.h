...
class DECLSPECIFIER Table_psc_local_bathdr : public TableBase , SingleLongKeyBase
{
private:
...
public:
	Table_psc_local_bathdr(Database_pluto_main *pDatabase):database(pDatabase)
	{
	};
...
class DECLSPECIFIER Row_psc_local_bathdr : public TableRow, public SerializeClass
	{
...
void psc_local_batuser_FK_psc_local_bathdr_getrows(vector <class Row_psc_local_batuser*> *rows);
		...
		void SetupSerialization(int iSC_Version) {
			StartSerializeList() + m_PK_psc_local_bathdr+ m_IPAddress+ m_date+ m_comments;
		}
	private:
