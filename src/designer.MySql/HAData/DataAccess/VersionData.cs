namespace HAData.DataAccess {
	...
	public class VersionData : MyDataSet {
		...
		public VersionData(MySqlConnection conn,MySqlTransaction trans) {
		...
		protected static void CreateParameters(MySqlParameterCollection Params, bool IsInsert) {
			Params.Add(new MySqlParameter(PK_VERSION_PARM, MySqlDbType.Int32,4));
			Params.Add(new MySqlParameter(VERSIONNAME_PARM, MySqlDbType.VarChar, 13));
			Params.Add(new MySqlParameter(DATE_PARM, MySqlDbType.DateTime,4));
			Params.Add(new MySqlParameter(DESCRIPTION_PARM, MySqlDbType.VarChar, 30));
			Params.Add(new MySqlParameter(REPOSITORY_PARM, MySqlDbType.Int32,4));
		...
		protected static void CreateCommands(MySqlConnection Conn, MySqlTransaction Trans, ref MySqlCommand LoadCommand, ref MySqlCommand InsertCommand, ref MySqlCommand UpdateCommand, ref MySqlCommand DeleteCommand) {
			if(LoadCommand == null) {
				...
				LoadCommand = new MySqlCommand("sp_Select_Version", Conn);
				LoadCommand.CommandType = CommandType.StoredProcedure;
				LoadCommand.Transaction = Trans;

				LoadCommand.Parameters.Add(new MySqlParameter(PK_VERSION_PARM, MySqlDbType.Int32,4));
			}

			if(InsertCommand == null) {
				// Create the command since it's null
				InsertCommand = new MySqlCommand("sp_Insert_Version", Conn);
			...
			if(UpdateCommand == null) {
				// Create the command since it's null
				UpdateCommand = new MySqlCommand("sp_Update_Version", Conn);
			...
			{
				DeleteCommand = new MySqlCommand("sp_Delete_Version", Conn);
				DeleteCommand.CommandType = CommandType.StoredProcedure;
				DeleteCommand.Transaction = Trans;

				DeleteCommand.Parameters.Add(PK_VERSION_PARM, MySqlDbType.Int32,4, PK_VERSION_FIELD);
		...
		protected static void CreateCommands(MySqlDataAdapter odbcda,MySqlConnection Conn, MySqlTransaction Trans, ref MySqlCommand LoadCommand, ref MySqlCommand InsertCommand, ref MySqlCommand UpdateCommand, ref MySqlCommand DeleteCommand) {
				LoadCommand = new MySqlCommand("SELECT PK_Version,VersionName,Date,Description,Repository FROM Version", Conn);
				LoadCommand.Transaction = Trans;
		...
		public static DataRowCollection LoadVersionWithWhere(ref MyDataSet ds, MySqlConnection conn, MySqlTransaction trans, string WhereClause) // marker:2
		{
		...
		public static DataRow LoadNoCacheVersion(ref MyDataSet ds, MySqlConnection conn, MySqlTransaction trans, System.Int32 PK_Version)
		{
			DataRow dr = null;
			if( ds==null )
			{
		...
		public static DataRow LoadVersion(ref MyDataSet ds, MySqlConnection conn, MySqlTransaction trans, System.Int32 PK_Version)  // marker:3
		{
			DataRow dr = null;
			if( ds==null )
			{
		...
		public VersionData LoadAll() {
			...
			// Create the command since it's null
			m_DSCommand.SelectCommand = new MySqlCommand("SELECT * FROM Version", m_Connection);
		...
		public static DataRowCollection LoadAll(ref MyDataSet ds, MySqlConnection conn, MySqlTransaction trans) {
			...
			if( conn==null )
				conn = HADataConfiguration.GetMySqlConnection();
			MySqlDataAdapter sqlda = new MySqlDataAdapter();
			MySqlCommand LoadCommand = new MySqlCommand("SELECT * FROM Version", conn);
		...
		public static bool UpdateVersion(ref MyDataSet ds, int CurUserID,MySqlConnection OdbcConn,MySqlTransaction Trans)
		{
	...
	public class VersionDataReader
	{
		...
		public VersionDataReader(MySqlDataReader d)
		{
			dr=d;
		}
		public VersionDataReader(MySqlCommand cmd)
		{
			dr = cmd.ExecuteReader();
		}

		public VersionDataReader(MySqlCommand cmd,bool Cache)
		{
			dr = cmd.ExecuteReader();
			bCache=Cache;
			if( bCache )
		...
		public VersionDataReader(string sSQL,MySqlConnection conn)
		{
			if( conn==null )
			{
				conn = HADataConfiguration.GetMySqlConnection();
		...
		public VersionDataReader(string sSQL,MySqlConnection conn,MySqlTransaction trans,bool Cache)
		{
			if( conn==null )
			{
				conn = HADataConfiguration.GetMySqlConnection();
	...
	public class VersionTable : DataTable
	{
		...
		{
			MySqlDataAdapter sqlda = new MySqlDataAdapter();
			MySqlCommand LoadCommand = new MySqlCommand("SELECT PK_Version,VersionName,Date,Description,Repository FROM Version", conn);
			LoadCommand.CommandType = CommandType.Text;
			if( trans!=null )
				LoadCommand.Transaction = trans;
