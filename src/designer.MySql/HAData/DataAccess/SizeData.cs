namespace HAData.DataAccess {
	...
	public class SizeData : MyDataSet {
		...
		public SizeData(MySqlConnection conn,MySqlTransaction trans) {
		...
		protected static void CreateParameters(MySqlParameterCollection Params, bool IsInsert) {
			Params.Add(new MySqlParameter(PK_SIZE_PARM, MySqlDbType.Int32,4));
			Params.Add(new MySqlParameter(DESCRIPTION_PARM, MySqlDbType.VarChar, 50));
			...
			Params.Add(new MySqlParameter(SCALEX_PARM, MySqlDbType.Int32,4));
			Params.Add(new MySqlParameter(SCALEY_PARM, MySqlDbType.Int32,4));
		...
		protected static void CreateCommands(MySqlConnection Conn, MySqlTransaction Trans, ref MySqlCommand LoadCommand, ref MySqlCommand InsertCommand, ref MySqlCommand UpdateCommand, ref MySqlCommand DeleteCommand) {
			if(LoadCommand == null) {
				...
				LoadCommand = new MySqlCommand("sp_Select_Size", Conn);
				LoadCommand.CommandType = CommandType.StoredProcedure;
				LoadCommand.Transaction = Trans;

				LoadCommand.Parameters.Add(new MySqlParameter(PK_SIZE_PARM, MySqlDbType.Int32,4));
			}

			if(InsertCommand == null) {
				// Create the command since it's null
				InsertCommand = new MySqlCommand("sp_Insert_Size", Conn);
			...
			if(UpdateCommand == null) {
				// Create the command since it's null
				UpdateCommand = new MySqlCommand("sp_Update_Size", Conn);
			...
			{
				DeleteCommand = new MySqlCommand("sp_Delete_Size", Conn);
				DeleteCommand.CommandType = CommandType.StoredProcedure;
				DeleteCommand.Transaction = Trans;

				DeleteCommand.Parameters.Add(PK_SIZE_PARM, MySqlDbType.Int32,4, PK_SIZE_FIELD);
		...
		protected static void CreateCommands(MySqlDataAdapter odbcda,MySqlConnection Conn, MySqlTransaction Trans, ref MySqlCommand LoadCommand, ref MySqlCommand InsertCommand, ref MySqlCommand UpdateCommand, ref MySqlCommand DeleteCommand) {
				LoadCommand = new MySqlCommand("SELECT PK_Size,Description,Width,Height,ScaleX,ScaleY FROM Size", Conn);
				LoadCommand.Transaction = Trans;
		...
		public static DataRowCollection LoadSizeWithWhere(ref MyDataSet ds, MySqlConnection conn, MySqlTransaction trans, string WhereClause) // marker:2
		{
			...
			string sSQL = "SELECT PK_Size, Description, Width, Height, ScaleX, ScaleY FROM Size WHERE " + WhereClause;
		...
		public static DataRow LoadNoCacheSize(ref MyDataSet ds, MySqlConnection conn, MySqlTransaction trans, System.Int32 PK_Size)
		{
			DataRow dr = null;
			if( ds==null )
			{
		...
		public static DataRow LoadSize(ref MyDataSet ds, MySqlConnection conn, MySqlTransaction trans, System.Int32 PK_Size)  // marker:3
		{
			DataRow dr = null;
			if( ds==null )
			{
		...
		public SizeData LoadAll() {
			...
			// Create the command since it's null
			m_DSCommand.SelectCommand = new MySqlCommand("SELECT * FROM Size", m_Connection);
		...
		public static DataRowCollection LoadAll(ref MyDataSet ds, MySqlConnection conn, MySqlTransaction trans) {
			...
			if( conn==null )
				conn = HADataConfiguration.GetMySqlConnection();
			MySqlDataAdapter sqlda = new MySqlDataAdapter();
			MySqlCommand LoadCommand = new MySqlCommand("SELECT * FROM Size", conn);
		...
		public static bool UpdateSize(ref MyDataSet ds, int CurUserID,MySqlConnection OdbcConn,MySqlTransaction Trans)
		{
	...
	public class SizeDataReader
	{
		...
		public SizeDataReader(MySqlDataReader d)
		{
			dr=d;
		}
		public SizeDataReader(MySqlCommand cmd)
		{
			dr = cmd.ExecuteReader();
		}

		public SizeDataReader(MySqlCommand cmd,bool Cache)
		{
			dr = cmd.ExecuteReader();
			bCache=Cache;
			if( bCache )
		...
		public SizeDataReader(string sSQL,MySqlConnection conn)
		{
			if( conn==null )
			{
				conn = HADataConfiguration.GetMySqlConnection();
		...
		public SizeDataReader(string sSQL,MySqlConnection conn,MySqlTransaction trans,bool Cache)
		{
			if( conn==null )
			{
				conn = HADataConfiguration.GetMySqlConnection();
	...
	public class SizeTable : DataTable
	{
		...
		{
			MySqlDataAdapter sqlda = new MySqlDataAdapter();
			MySqlCommand LoadCommand = new MySqlCommand("SELECT PK_Size,Description,Width,Height,ScaleX,ScaleY FROM Size", conn);
			LoadCommand.CommandType = CommandType.Text;
			if( trans!=null )
				LoadCommand.Transaction = trans;
