namespace HAData.DataAccess {
	...
	public class CriteriaListData : MyDataSet {
		...
		public CriteriaListData(MySqlConnection conn,MySqlTransaction trans) {
		...
		protected static void CreateParameters(MySqlParameterCollection Params, bool IsInsert) {
			Params.Add(new MySqlParameter(PK_CRITERIALIST_PARM, MySqlDbType.Int32,4));
		...
		protected static void CreateCommands(MySqlConnection Conn, MySqlTransaction Trans, ref MySqlCommand LoadCommand, ref MySqlCommand InsertCommand, ref MySqlCommand UpdateCommand, ref MySqlCommand DeleteCommand) {
			if(LoadCommand == null) {
				...
				LoadCommand = new MySqlCommand("sp_Select_CriteriaList", Conn);
				LoadCommand.CommandType = CommandType.StoredProcedure;
				LoadCommand.Transaction = Trans;

				LoadCommand.Parameters.Add(new MySqlParameter(PK_CRITERIALIST_PARM, MySqlDbType.Int32,4));
			}

			if(InsertCommand == null) {
				// Create the command since it's null
				InsertCommand = new MySqlCommand("sp_Insert_CriteriaList", Conn);
			...
			if(UpdateCommand == null) {
				// Create the command since it's null
				UpdateCommand = new MySqlCommand("sp_Update_CriteriaList", Conn);
			...
			{
				DeleteCommand = new MySqlCommand("sp_Delete_CriteriaList", Conn);
				DeleteCommand.CommandType = CommandType.StoredProcedure;
				DeleteCommand.Transaction = Trans;

				DeleteCommand.Parameters.Add(PK_CRITERIALIST_PARM, MySqlDbType.Int32,4, PK_CRITERIALIST_FIELD);
		...
		protected static void CreateCommands(MySqlDataAdapter odbcda,MySqlConnection Conn, MySqlTransaction Trans, ref MySqlCommand LoadCommand, ref MySqlCommand InsertCommand, ref MySqlCommand UpdateCommand, ref MySqlCommand DeleteCommand) {
				LoadCommand = new MySqlCommand("SELECT PK_CriteriaList,Description,Define FROM CriteriaList", Conn);
				LoadCommand.Transaction = Trans;
		...
		public static DataRowCollection LoadCriteriaListWithWhere(ref MyDataSet ds, MySqlConnection conn, MySqlTransaction trans, string WhereClause) // marker:2
		{
		...
		public static DataRow LoadNoCacheCriteriaList(ref MyDataSet ds, MySqlConnection conn, MySqlTransaction trans, System.Int32 PK_CriteriaList)
		{
			DataRow dr = null;
			if( ds==null )
			{
		...
		public static DataRow LoadCriteriaList(ref MyDataSet ds, MySqlConnection conn, MySqlTransaction trans, System.Int32 PK_CriteriaList)  // marker:3
		{
			DataRow dr = null;
			if( ds==null )
			{
		...
		public CriteriaListData LoadAll() {
			...
			// Create the command since it's null
			m_DSCommand.SelectCommand = new MySqlCommand("SELECT * FROM CriteriaList", m_Connection);
		...
		public static DataRowCollection LoadAll(ref MyDataSet ds, MySqlConnection conn, MySqlTransaction trans) {
			...
			if( conn==null )
				conn = HADataConfiguration.GetMySqlConnection();
			MySqlDataAdapter sqlda = new MySqlDataAdapter();
			MySqlCommand LoadCommand = new MySqlCommand("SELECT * FROM CriteriaList", conn);
		...
		public static bool UpdateCriteriaList(ref MyDataSet ds, int CurUserID,MySqlConnection OdbcConn,MySqlTransaction Trans)
		{
	...
	public class CriteriaListDataReader
	{
		...
		public CriteriaListDataReader(MySqlDataReader d)
		{
			dr=d;
		}
		public CriteriaListDataReader(MySqlCommand cmd)
		{
			dr = cmd.ExecuteReader();
		}

		public CriteriaListDataReader(MySqlCommand cmd,bool Cache)
		{
			dr = cmd.ExecuteReader();
			bCache=Cache;
			if( bCache )
		...
		public CriteriaListDataReader(string sSQL,MySqlConnection conn)
		{
			if( conn==null )
			{
				conn = HADataConfiguration.GetMySqlConnection();
		...
		public CriteriaListDataReader(string sSQL,MySqlConnection conn,MySqlTransaction trans,bool Cache)
		{
			if( conn==null )
			{
				conn = HADataConfiguration.GetMySqlConnection();
	...
	public class CriteriaListTable : DataTable
	{
		...
		{
			MySqlDataAdapter sqlda = new MySqlDataAdapter();
			MySqlCommand LoadCommand = new MySqlCommand("SELECT PK_CriteriaList,Description,Define FROM CriteriaList", conn);
			LoadCommand.CommandType = CommandType.Text;
			if( trans!=null )
				LoadCommand.Transaction = trans;
