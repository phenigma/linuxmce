namespace HAData.DataAccess {
	...
	public class UserModeData : MyDataSet {
		...
		public UserModeData(MySqlConnection conn,MySqlTransaction trans) {
		...
		protected static void CreateParameters(MySqlParameterCollection Params, bool IsInsert) {
			Params.Add(new MySqlParameter(PK_USERMODE_PARM, MySqlDbType.Int32,4));
		...
		protected static void CreateCommands(MySqlConnection Conn, MySqlTransaction Trans, ref MySqlCommand LoadCommand, ref MySqlCommand InsertCommand, ref MySqlCommand UpdateCommand, ref MySqlCommand DeleteCommand) {
			if(LoadCommand == null) {
				...
				LoadCommand = new MySqlCommand("sp_Select_UserMode", Conn);
				LoadCommand.CommandType = CommandType.StoredProcedure;
				LoadCommand.Transaction = Trans;

				LoadCommand.Parameters.Add(new MySqlParameter(PK_USERMODE_PARM, MySqlDbType.Int32,4));
			}

			if(InsertCommand == null) {
				// Create the command since it's null
				InsertCommand = new MySqlCommand("sp_Insert_UserMode", Conn);
			...
			if(UpdateCommand == null) {
				// Create the command since it's null
				UpdateCommand = new MySqlCommand("sp_Update_UserMode", Conn);
			...
			{
				DeleteCommand = new MySqlCommand("sp_Delete_UserMode", Conn);
				DeleteCommand.CommandType = CommandType.StoredProcedure;
				DeleteCommand.Transaction = Trans;

				DeleteCommand.Parameters.Add(PK_USERMODE_PARM, MySqlDbType.Int32,4, PK_USERMODE_FIELD);
		...
		protected static void CreateCommands(MySqlDataAdapter odbcda,MySqlConnection Conn, MySqlTransaction Trans, ref MySqlCommand LoadCommand, ref MySqlCommand InsertCommand, ref MySqlCommand UpdateCommand, ref MySqlCommand DeleteCommand) {
				LoadCommand = new MySqlCommand("SELECT PK_UserMode,Description,Define FROM UserMode", Conn);
				LoadCommand.Transaction = Trans;
		...
		public static DataRowCollection LoadUserModeWithWhere(ref MyDataSet ds, MySqlConnection conn, MySqlTransaction trans, string WhereClause) // marker:2
		{
			...
			string sSQL = "SELECT PK_UserMode, Description, Define FROM UserMode WHERE " + WhereClause;
		...
		public static DataRow LoadNoCacheUserMode(ref MyDataSet ds, MySqlConnection conn, MySqlTransaction trans, System.Int32 PK_UserMode)
		{
			DataRow dr = null;
			if( ds==null )
			{
		...
		public static DataRow LoadUserMode(ref MyDataSet ds, MySqlConnection conn, MySqlTransaction trans, System.Int32 PK_UserMode)  // marker:3
		{
			DataRow dr = null;
			if( ds==null )
			{
		...
		public UserModeData LoadAll() {
			...
			// Create the command since it's null
			m_DSCommand.SelectCommand = new MySqlCommand("SELECT * FROM UserMode", m_Connection);
		...
		public static DataRowCollection LoadAll(ref MyDataSet ds, MySqlConnection conn, MySqlTransaction trans) {
			...
			if( conn==null )
				conn = HADataConfiguration.GetMySqlConnection();
			MySqlDataAdapter sqlda = new MySqlDataAdapter();
			MySqlCommand LoadCommand = new MySqlCommand("SELECT * FROM UserMode", conn);
		...
		public static bool UpdateUserMode(ref MyDataSet ds, int CurUserID,MySqlConnection OdbcConn,MySqlTransaction Trans)
		{
	...
	public class UserModeDataReader
	{
		...
		public UserModeDataReader(MySqlDataReader d)
		{
			dr=d;
		}
		public UserModeDataReader(MySqlCommand cmd)
		{
			dr = cmd.ExecuteReader();
		}

		public UserModeDataReader(MySqlCommand cmd,bool Cache)
		{
			dr = cmd.ExecuteReader();
			bCache=Cache;
			if( bCache )
		...
		public UserModeDataReader(string sSQL,MySqlConnection conn)
		{
			if( conn==null )
			{
				conn = HADataConfiguration.GetMySqlConnection();
		...
		public UserModeDataReader(string sSQL,MySqlConnection conn,MySqlTransaction trans,bool Cache)
		{
			if( conn==null )
			{
				conn = HADataConfiguration.GetMySqlConnection();
	...
	public class UserModeTable : DataTable
	{
		...
		{
			MySqlDataAdapter sqlda = new MySqlDataAdapter();
			MySqlCommand LoadCommand = new MySqlCommand("SELECT PK_UserMode,Description,Define FROM UserMode", conn);
			LoadCommand.CommandType = CommandType.Text;
			if( trans!=null )
				LoadCommand.Transaction = trans;
