namespace HAData.DataAccess {
	...
	public class CommandData : MyDataSet {
		...
		public CommandData(MySqlConnection conn,MySqlTransaction trans) {
		...
		protected static void CreateParameters(MySqlParameterCollection Params, bool IsInsert) {
			Params.Add(new MySqlParameter(PK_COMMAND_PARM, MySqlDbType.Int32,4));
			Params.Add(new MySqlParameter(DESCRIPTION_PARM, MySqlDbType.VarChar, 50));
			Params.Add(new MySqlParameter(DEFINE_PARM, MySqlDbType.VarChar, 50));
			Params.Add(new MySqlParameter(FK_COMMANDCATEGORY_PARM, MySqlDbType.Int32,4));
		...
		protected static void CreateCommands(MySqlConnection Conn, MySqlTransaction Trans, ref MySqlCommand LoadCommand, ref MySqlCommand InsertCommand, ref MySqlCommand UpdateCommand, ref MySqlCommand DeleteCommand) {
			if(LoadCommand == null) {
				...
				LoadCommand = new MySqlCommand("sp_Select_Command", Conn);
				LoadCommand.CommandType = CommandType.StoredProcedure;
				LoadCommand.Transaction = Trans;

				LoadCommand.Parameters.Add(new MySqlParameter(PK_COMMAND_PARM, MySqlDbType.Int32,4));
			}

			if(InsertCommand == null) {
				// Create the command since it's null
				InsertCommand = new MySqlCommand("sp_Insert_Command", Conn);
			...
			if(UpdateCommand == null) {
				// Create the command since it's null
				UpdateCommand = new MySqlCommand("sp_Update_Command", Conn);
			...
			{
				DeleteCommand = new MySqlCommand("sp_Delete_Command", Conn);
				DeleteCommand.CommandType = CommandType.StoredProcedure;
				DeleteCommand.Transaction = Trans;

				DeleteCommand.Parameters.Add(PK_COMMAND_PARM, MySqlDbType.Int32,4, PK_COMMAND_FIELD);
		...
		protected static void CreateCommands(MySqlDataAdapter odbcda,MySqlConnection Conn, MySqlTransaction Trans, ref MySqlCommand LoadCommand, ref MySqlCommand InsertCommand, ref MySqlCommand UpdateCommand, ref MySqlCommand DeleteCommand) {
				LoadCommand = new MySqlCommand("SELECT PK_Command,Description,Define,FK_CommandCategory FROM Command", Conn);
				LoadCommand.Transaction = Trans;
		...
		public static DataRowCollection LoadCommandWithWhere(ref MyDataSet ds, MySqlConnection conn, MySqlTransaction trans, string WhereClause) // marker:2
		{
		...
		public static DataRow LoadNoCacheCommand(ref MyDataSet ds, MySqlConnection conn, MySqlTransaction trans, System.Int32 PK_Command)
		{
			DataRow dr = null;
			if( ds==null )
			{
		...
		public static DataRow LoadCommand(ref MyDataSet ds, MySqlConnection conn, MySqlTransaction trans, System.Int32 PK_Command)  // marker:3
		{
			DataRow dr = null;
			if( ds==null )
			{
		...
		public CommandData LoadAll() {
			...
			// Create the command since it's null
			m_DSCommand.SelectCommand = new MySqlCommand("SELECT * FROM Command", m_Connection);
		...
		public static DataRowCollection LoadAll(ref MyDataSet ds, MySqlConnection conn, MySqlTransaction trans) {
			...
			if( conn==null )
				conn = HADataConfiguration.GetMySqlConnection();
			MySqlDataAdapter sqlda = new MySqlDataAdapter();
			MySqlCommand LoadCommand = new MySqlCommand("SELECT * FROM Command", conn);
		...
		public static bool UpdateCommand(ref MyDataSet ds, int CurUserID,MySqlConnection OdbcConn,MySqlTransaction Trans)
		{
	...
	public class CommandDataReader
	{
		...
		public CommandDataReader(MySqlDataReader d)
		{
			dr=d;
		}
		public CommandDataReader(MySqlCommand cmd)
		{
			dr = cmd.ExecuteReader();
		}

		public CommandDataReader(MySqlCommand cmd,bool Cache)
		{
			dr = cmd.ExecuteReader();
			bCache=Cache;
			if( bCache )
		...
		public CommandDataReader(string sSQL,MySqlConnection conn)
		{
			if( conn==null )
			{
				conn = HADataConfiguration.GetMySqlConnection();
		...
		public CommandDataReader(string sSQL,MySqlConnection conn,MySqlTransaction trans,bool Cache)
		{
			if( conn==null )
			{
				conn = HADataConfiguration.GetMySqlConnection();
	...
	public class CommandTable : DataTable
	{
		...
		{
			MySqlDataAdapter sqlda = new MySqlDataAdapter();
			MySqlCommand LoadCommand = new MySqlCommand("SELECT PK_Command,Description,Define,FK_CommandCategory FROM Command", conn);
			LoadCommand.CommandType = CommandType.Text;
			if( trans!=null )
				LoadCommand.Transaction = trans;
