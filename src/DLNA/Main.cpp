...
#define  VERSION "2.0.0.45.12022425524"
const char *g_szCompile_Date="Fri Feb 24 14:08:35 2012";
...
extern "C" {
	...
	{ 
		...
		if( bIsRuntimePlugin )
			return DLNA::PK_DeviceTemplate_get_static();
		else
			return 0;
	}
}
...
extern "C" {
	class Command_Impl *RegisterAsPlugIn(class Router *pRouter,int PK_Device,Logger *pPlutoLogger)
	{
		...
		DLNA *pDLNA = new DLNA(PK_Device, "localhost",true,false,pRouter);
		if( pDLNA->m_bQuit_get()|| !pDLNA->GetConfig() )
		{
			delete pDLNA;
			return NULL;
		}
...
int main(int argc, char* argv[]) 
{
	...
	if (bError)
	{
		cout << "A Pluto DCE Device.  See www.plutohome.com/dce for details." << endl
			<< "Usage: DLNA [-r Router's IP] [-d My Device ID] [-l dcerouter|stdout|null|filename]" << endl
	...
	try
	{
		if( sLogger=="dcerouter" )
			LoggerWrapper::SetInstance(new ServerLogger(PK_Device, DLNA::PK_DeviceTemplate_get_static(), sRouter_IP));
	...
	try
	{
		...
		{
			bAppError = true;
			if( pDLNA->m_pEvent && pDLNA->m_pEvent->m_pClientSocket && pDLNA->m_pEvent->m_pClientSocket->m_eLastError==ClientSocket::cs_err_CannotConnect )
			{
				bAppError = false;
				bReload = false;
