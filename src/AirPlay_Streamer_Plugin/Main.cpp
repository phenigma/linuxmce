...
*/
//<-dceag-incl-b->
#include "AirPlay_Streamer_Plugin.h"
...
extern "C" {
	...
	{ 
		...
		if( bIsRuntimePlugin )
			return AirPlay_Streamer_Plugin::PK_DeviceTemplate_get_static();
		else
			return 0;
	}
}
...
extern "C" {
	class Command_Impl *RegisterAsPlugIn(class Router *pRouter,int PK_Device,Logger *pPlutoLogger)
	{
		...
		AirPlay_Streamer_Plugin *pAirPlay_Streamer_Plugin = new AirPlay_Streamer_Plugin(PK_Device, "localhost",true,false,pRouter);
		if( pAirPlay_Streamer_Plugin->m_bQuit_get()|| !pAirPlay_Streamer_Plugin->GetConfig() )
		{
			delete pAirPlay_Streamer_Plugin;
			return NULL;
		}
		else
		{
			g_pCommand_Impl=pAirPlay_Streamer_Plugin;
			g_pDeadlockHandler=Plugin_DeadlockHandler;
			g_pSocketCrashHandler=Plugin_SocketCrashHandler;
		}
		return pAirPlay_Streamer_Plugin;
	}
}
//<-dceag-plug-e->
...
int main(int argc, char* argv[]) 
{
	...
	cout << "AirPlay_Streamer_Plugin, v." << VERSION << endl
	...
	if (bError)
	{
		cout << "A Pluto DCE Device.  See www.plutohome.com/dce for details." << endl
			<< "Usage: AirPlay_Streamer_Plugin [-r Router's IP] [-d My Device ID] [-l dcerouter|stdout|null|filename]" << endl
	...
	try
	{
		if( sLogger=="dcerouter" )
			LoggerWrapper::SetInstance(new ServerLogger(PK_Device, AirPlay_Streamer_Plugin::PK_DeviceTemplate_get_static(), sRouter_IP));
	...
	try
	{
		AirPlay_Streamer_Plugin *pAirPlay_Streamer_Plugin = new AirPlay_Streamer_Plugin(PK_Device, sRouter_IP,true,bLocalMode);
		if ( pAirPlay_Streamer_Plugin->GetConfig() && pAirPlay_Streamer_Plugin->Connect(pAirPlay_Streamer_Plugin->PK_DeviceTemplate_get()) ) 
		{
			g_pCommand_Impl=pAirPlay_Streamer_Plugin;
			g_pDeadlockHandler=DeadlockHandler;
			g_pSocketCrashHandler=SocketCrashHandler;
			LoggerWrapper::GetInstance()->Write(LV_STATUS, "Connect OK");
			pAirPlay_Streamer_Plugin->CreateChildren();
			if( bLocalMode )
				pAirPlay_Streamer_Plugin->RunLocalMode();
			else
			{
				if(pAirPlay_Streamer_Plugin->m_RequestHandlerThread)
					pthread_join(pAirPlay_Streamer_Plugin->m_RequestHandlerThread, NULL);  // This function will return when the device is shutting down
			}
		...
		{
			bAppError = true;
			if( pAirPlay_Streamer_Plugin->m_pEvent && pAirPlay_Streamer_Plugin->m_pEvent->m_pClientSocket && pAirPlay_Streamer_Plugin->m_pEvent->m_pClientSocket->m_eLastError==ClientSocket::cs_err_CannotConnect )
			{
				bAppError = false;
				bReload = false;
		...
		if( pAirPlay_Streamer_Plugin->m_bReload )
			bReload=true;
