...
extern "C" {
	...
	{ 
		...
		if( bIsRuntimePlugin )
			return VLC_Plugin::PK_DeviceTemplate_get_static();
		else
			return 0;
	}
}
...
extern "C" {
	class Command_Impl *RegisterAsPlugIn(class Router *pRouter,int PK_Device,Logger *pPlutoLogger)
	{
		...
		VLC_Plugin *pVLC_Plugin = new VLC_Plugin(PK_Device, "localhost",true,false,pRouter);
		if( pVLC_Plugin->m_bQuit_get()|| !pVLC_Plugin->GetConfig() )
		{
			delete pVLC_Plugin;
			return NULL;
		}
...
int main(int argc, char* argv[]) 
{
	...
	cout << "VLC_Plugin, v." << VERSION << endl
	...
	if (bError)
	{
		cout << "A Pluto DCE Device.  See www.plutohome.com/dce for details." << endl
			<< "Usage: VLC_Plugin [-r Router's IP] [-d My Device ID] [-l dcerouter|stdout|null|filename]" << endl
	...
	try
	{
		if( sLogger=="dcerouter" )
			LoggerWrapper::SetInstance(new ServerLogger(PK_Device, VLC_Plugin::PK_DeviceTemplate_get_static(), sRouter_IP));
	...
	try
	{
		VLC_Plugin *pVLC_Plugin = new VLC_Plugin(PK_Device, sRouter_IP,true,bLocalMode);
		if ( pVLC_Plugin->GetConfig() && pVLC_Plugin->Connect(pVLC_Plugin->PK_DeviceTemplate_get()) ) 
		{
			g_pCommand_Impl=pVLC_Plugin;
			g_pDeadlockHandler=DeadlockHandler;
			g_pSocketCrashHandler=SocketCrashHandler;
			LoggerWrapper::GetInstance()->Write(LV_STATUS, "Connect OK");
			pVLC_Plugin->CreateChildren();
			if( bLocalMode )
				pVLC_Plugin->RunLocalMode();
			else
			{
				if(pVLC_Plugin->m_RequestHandlerThread)
					pthread_join(pVLC_Plugin->m_RequestHandlerThread, NULL);  // This function will return when the device is shutting down
			}
		...
		{
			bAppError = true;
			if( pVLC_Plugin->m_pEvent && pVLC_Plugin->m_pEvent->m_pClientSocket && pVLC_Plugin->m_pEvent->m_pClientSocket->m_eLastError==ClientSocket::cs_err_CannotConnect )
			{
				bAppError = false;
				bReload = false;
