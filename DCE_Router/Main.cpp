//<-dceag-incl-b->
#include "DCE_Router.h"
#include "DCE/Logger.h"
#include "ServerLogger.h"
#include "StringUtils.h"

#define  DCE_Router_VERSION "testing"
...
extern "C" {
	class Command_Impl *RegisterAsPlugIn(class Router *pRouter,int PK_Device,string sLogger)
	{
		...
		DCE_Router *pDCE_Router = new DCE_Router(PK_Device, "localhost",true,false,pRouter);
		if( pDCE_Router->m_bQuit )
		{
			delete pDCE_Router;
			return NULL;
		}
...
int main(int argc, char* argv[]) 
{
	...
	if (bError)
	{
		cout << "DCE_Router, v." << DCE_Router_VERSION << endl
			<< "A Pluto DCE Device.  See www.plutohome.com/dce for details." << endl
			<< "Usage: DCE_Router [-r Router's IP] [-d My Device ID] [-l dce_router|stdout|null|filename]" << endl
	...
	try
	{
		DCE_Router *pDCE_Router = new DCE_Router(PK_Device, sRouter_IP);	
		if ( pDCE_Router->Connect() ) 
		{
			g_pPlutoLogger->Write(LV_STATUS, "Connect OK");
			pDCE_Router->CreateChildren();
			pthread_join(pDCE_Router->m_RequestHandlerThread, NULL);
