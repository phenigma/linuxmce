...
{
	map<Table_UnknownDevices::Key, class Row_UnknownDevices*, Table_UnknownDevices::Key_Less>::iterator it;
...
void Row_UnknownDevices::Delete()
{
	...
	if (!is_deleted)
		if (is_added)	
		{	
			vector<Row_UnknownDevices*>::iterator i;	
		...
		{
			Table_UnknownDevices::Key key(this);					
			map<Table_UnknownDevices::Key, Row_UnknownDevices*, Table_UnknownDevices::Key_Less>::iterator i = table->cachedRows.find(key);
...
long int Row_UnknownDevices::PK_UnknownDevices_get(){PLUTO_SAFETY_LOCK(M, table->m_Mutex);

return m_PK_UnknownDevices;}
string Row_UnknownDevices::Description_get(){PLUTO_SAFETY_LOCK(M, table->m_Mutex);

return m_Description;}
string Row_UnknownDevices::MacAddress_get(){PLUTO_SAFETY_LOCK(M, table->m_Mutex);
...
return m_psc_id;}
long int Row_UnknownDevices::psc_batch_get(){PLUTO_SAFETY_LOCK(M, table->m_Mutex);

return m_psc_batch;}
long int Row_UnknownDevices::psc_user_get(){PLUTO_SAFETY_LOCK(M, table->m_Mutex);

return m_psc_user;}
short int Row_UnknownDevices::psc_frozen_get(){PLUTO_SAFETY_LOCK(M, table->m_Mutex);

return m_psc_frozen;}
string Row_UnknownDevices::psc_mod_get(){PLUTO_SAFETY_LOCK(M, table->m_Mutex);

return m_psc_mod;}

		
void Row_UnknownDevices::PK_UnknownDevices_set(long int val){PLUTO_SAFETY_LOCK(M, table->m_Mutex);

m_PK_UnknownDevices = val; is_modified=true; is_null[0]=false;}
void Row_UnknownDevices::Description_set(string val){PLUTO_SAFETY_LOCK(M, table->m_Mutex);

m_Description = val; is_modified=true; is_null[1]=false;}
void Row_UnknownDevices::MacAddress_set(string val){PLUTO_SAFETY_LOCK(M, table->m_Mutex);
...
void Row_UnknownDevices::psc_batch_set(long int val){PLUTO_SAFETY_LOCK(M, table->m_Mutex);

m_psc_batch = val; is_modified=true; is_null[5]=false;}
void Row_UnknownDevices::psc_user_set(long int val){PLUTO_SAFETY_LOCK(M, table->m_Mutex);

m_psc_user = val; is_modified=true; is_null[6]=false;}
void Row_UnknownDevices::psc_frozen_set(short int val){PLUTO_SAFETY_LOCK(M, table->m_Mutex);

m_psc_frozen = val; is_modified=true; is_null[7]=false;}
void Row_UnknownDevices::psc_mod_set(string val){PLUTO_SAFETY_LOCK(M, table->m_Mutex);

m_psc_mod = val; is_modified=true; is_null[8]=false;}

		
bool Row_UnknownDevices::Description_isNull() {PLUTO_SAFETY_LOCK(M, table->m_Mutex);

return is_null[1];}
bool Row_UnknownDevices::MacAddress_isNull() {PLUTO_SAFETY_LOCK(M, table->m_Mutex);

return is_null[2];}
bool Row_UnknownDevices::IPAddress_isNull() {PLUTO_SAFETY_LOCK(M, table->m_Mutex);

return is_null[3];}
bool Row_UnknownDevices::psc_id_isNull() {PLUTO_SAFETY_LOCK(M, table->m_Mutex);

return is_null[4];}
bool Row_UnknownDevices::psc_batch_isNull() {PLUTO_SAFETY_LOCK(M, table->m_Mutex);

return is_null[5];}
bool Row_UnknownDevices::psc_user_isNull() {PLUTO_SAFETY_LOCK(M, table->m_Mutex);

return is_null[6];}
bool Row_UnknownDevices::psc_frozen_isNull() {PLUTO_SAFETY_LOCK(M, table->m_Mutex);

return is_null[7];}

			
void Row_UnknownDevices::Description_setNull(bool val){PLUTO_SAFETY_LOCK(M, table->m_Mutex);

is_null[1]=val;}
void Row_UnknownDevices::MacAddress_setNull(bool val){PLUTO_SAFETY_LOCK(M, table->m_Mutex);

is_null[2]=val;}
void Row_UnknownDevices::IPAddress_setNull(bool val){PLUTO_SAFETY_LOCK(M, table->m_Mutex);

is_null[3]=val;}
void Row_UnknownDevices::psc_id_setNull(bool val){PLUTO_SAFETY_LOCK(M, table->m_Mutex);

is_null[4]=val;}
void Row_UnknownDevices::psc_batch_setNull(bool val){PLUTO_SAFETY_LOCK(M, table->m_Mutex);

is_null[5]=val;}
void Row_UnknownDevices::psc_user_setNull(bool val){PLUTO_SAFETY_LOCK(M, table->m_Mutex);

is_null[6]=val;}
void Row_UnknownDevices::psc_frozen_setNull(bool val){PLUTO_SAFETY_LOCK(M, table->m_Mutex);
...
void Table_UnknownDevices::Commit()
{
...
//insert added
	...
	{
		vector<Row_UnknownDevices*>::iterator i = addedRows.begin();
	
		Row_UnknownDevices *pRow = *i;
...
//update modified
	...
	for (map<Key, Row_UnknownDevices*, Key_Less>::iterator i = cachedRows.begin(); i!= cachedRows.end(); i++)
		if	(((*i).second)->is_modified)
	{
		Row_UnknownDevices* pRow = (*i).second;	
		Key key(pRow);	

		char tmp_PK_UnknownDevices[32];
sprintf(tmp_PK_UnknownDevices, "%li", key.pk_PK_UnknownDevices);
...
//delete deleted added
	...
	{	
		vector<Row_UnknownDevices*>::iterator i = deleted_addedRows.begin();
		delete (*i);
...
//delete deleted cached
	...
	{	
		map<Key, Row_UnknownDevices*, Key_Less>::iterator i = deleted_cachedRows.begin();
...
pRow->m_psc_mod = string(row[8],lengths[8]);
		...
		Key key(pRow);
                ...
                map<Table_UnknownDevices::Key, Row_UnknownDevices*, Table_UnknownDevices::Key_Less>::iterator i = cachedRows.find(key);
...
Row_UnknownDevices* Table_UnknownDevices::GetRow(long int in_PK_UnknownDevices)
{
	...
	Key row_key(in_PK_UnknownDevices);

	map<Key, Row_UnknownDevices*, Key_Less>::iterator i;
...
Row_UnknownDevices* Table_UnknownDevices::FetchRow(Table_UnknownDevices::Key &key)
{
	PLUTO_SAFETY_LOCK(M, m_Mutex);
