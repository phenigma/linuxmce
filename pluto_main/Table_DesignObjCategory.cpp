...
{
	map<Table_DesignObjCategory::Key, class Row_DesignObjCategory*, Table_DesignObjCategory::Key_Less>::iterator it;
...
void Row_DesignObjCategory::Delete()
{
	...
	if (!is_deleted)
		if (is_added)	
		{	
			vector<Row_DesignObjCategory*>::iterator i;	
		...
		{
			Table_DesignObjCategory::Key key(this);					
			map<Table_DesignObjCategory::Key, Row_DesignObjCategory*, Table_DesignObjCategory::Key_Less>::iterator i = table->cachedRows.find(key);
...
string Row_DesignObjCategory::Modification_RecordInfo_get(){PLUTO_SAFETY_LOCK(M, table->m_Mutex);

return m_Modification_RecordInfo;}
short int Row_DesignObjCategory::IsNew_RecordInfo_get(){PLUTO_SAFETY_LOCK(M, table->m_Mutex);

return m_IsNew_RecordInfo;}
short int Row_DesignObjCategory::IsDeleted_RecordInfo_get(){PLUTO_SAFETY_LOCK(M, table->m_Mutex);

return m_IsDeleted_RecordInfo;}
long int Row_DesignObjCategory::FK_Users_RecordInfo_get(){PLUTO_SAFETY_LOCK(M, table->m_Mutex);
...
void Row_DesignObjCategory::Modification_RecordInfo_set(string val){PLUTO_SAFETY_LOCK(M, table->m_Mutex);

m_Modification_RecordInfo = val; is_modified=true; is_null[3]=false;}
void Row_DesignObjCategory::IsNew_RecordInfo_set(short int val){PLUTO_SAFETY_LOCK(M, table->m_Mutex);

m_IsNew_RecordInfo = val; is_modified=true; is_null[4]=false;}
void Row_DesignObjCategory::IsDeleted_RecordInfo_set(short int val){PLUTO_SAFETY_LOCK(M, table->m_Mutex);

m_IsDeleted_RecordInfo = val; is_modified=true; is_null[5]=false;}
void Row_DesignObjCategory::FK_Users_RecordInfo_set(long int val){PLUTO_SAFETY_LOCK(M, table->m_Mutex);
...
return is_null[2];}
bool Row_DesignObjCategory::IsNew_RecordInfo_isNull() {PLUTO_SAFETY_LOCK(M, table->m_Mutex);

return is_null[4];}
bool Row_DesignObjCategory::IsDeleted_RecordInfo_isNull() {PLUTO_SAFETY_LOCK(M, table->m_Mutex);

return is_null[5];}
bool Row_DesignObjCategory::FK_Users_RecordInfo_isNull() {PLUTO_SAFETY_LOCK(M, table->m_Mutex);
...
is_null[2]=val;}
void Row_DesignObjCategory::IsNew_RecordInfo_setNull(bool val){PLUTO_SAFETY_LOCK(M, table->m_Mutex);

is_null[4]=val;}
void Row_DesignObjCategory::IsDeleted_RecordInfo_setNull(bool val){PLUTO_SAFETY_LOCK(M, table->m_Mutex);

is_null[5]=val;}
void Row_DesignObjCategory::FK_Users_RecordInfo_setNull(bool val){PLUTO_SAFETY_LOCK(M, table->m_Mutex);
...
string Row_DesignObjCategory::Modification_RecordInfo_asSQL()
{
PLUTO_SAFETY_LOCK(M, table->m_Mutex);
...
string Row_DesignObjCategory::IsNew_RecordInfo_asSQL()
{
PLUTO_SAFETY_LOCK(M, table->m_Mutex);
...
string Row_DesignObjCategory::IsDeleted_RecordInfo_asSQL()
{
PLUTO_SAFETY_LOCK(M, table->m_Mutex);
...
string Row_DesignObjCategory::FK_Users_RecordInfo_asSQL()
{
PLUTO_SAFETY_LOCK(M, table->m_Mutex);
...
void Table_DesignObjCategory::Commit()
{
...
//insert added
	...
	{
		vector<Row_DesignObjCategory*>::iterator i = addedRows.begin();
	
		Row_DesignObjCategory *pRow = *i;
	
		
string values_list_comma_separated;
values_list_comma_separated = values_list_comma_separated + pRow->PK_DesignObjCategory_asSQL()+", "+pRow->FK_DesignObjCategory_Parent_asSQL()+", "+pRow->Description_asSQL()+", "+pRow->Modification_RecordInfo_asSQL()+", "+pRow->IsNew_RecordInfo_asSQL()+", "+pRow->IsDeleted_RecordInfo_asSQL()+", "+pRow->FK_Users_RecordInfo_asSQL();

	
		string query = "insert into DesignObjCategory (PK_DesignObjCategory, FK_DesignObjCategory_Parent, Description, Modification_RecordInfo, IsNew_RecordInfo, IsDeleted_RecordInfo, FK_Users_RecordInfo) values ("+
...
//update modified
	...
	for (map<Key, Row_DesignObjCategory*, Key_Less>::iterator i = cachedRows.begin(); i!= cachedRows.end(); i++)
		if	(((*i).second)->is_modified)
	{
		Row_DesignObjCategory* pRow = (*i).second;	
		Key key(pRow);	

		char tmp_PK_DesignObjCategory[32];
sprintf(tmp_PK_DesignObjCategory, "%li", key.pk_PK_DesignObjCategory);
...
update_values_list = update_values_list + "PK_DesignObjCategory="+pRow->PK_DesignObjCategory_asSQL()+", FK_DesignObjCategory_Parent="+pRow->FK_DesignObjCategory_Parent_asSQL()+", Description="+pRow->Description_asSQL()+", Modification_RecordInfo="+pRow->Modification_RecordInfo_asSQL()+", IsNew_RecordInfo="+pRow->IsNew_RecordInfo_asSQL()+", IsDeleted_RecordInfo="+pRow->IsDeleted_RecordInfo_asSQL()+", FK_Users_RecordInfo="+pRow->FK_Users_RecordInfo_asSQL();
...
//delete deleted added
	...
	{	
		vector<Row_DesignObjCategory*>::iterator i = deleted_addedRows.begin();
		delete (*i);
...
//delete deleted cached
	...
	{	
		map<Key, Row_DesignObjCategory*, Key_Less>::iterator i = deleted_cachedRows.begin();
...
bool Table_DesignObjCategory::GetRows(string where_statement,vector<class Row_DesignObjCategory*> *rows)
{
	...
	string query = "select * from DesignObjCategory where " + where_statement;
...
sscanf(row[6], "%li", &(pRow->m_FK_Users_RecordInfo));
		...
		Key key(pRow);
                ...
                map<Table_DesignObjCategory::Key, Row_DesignObjCategory*, Table_DesignObjCategory::Key_Less>::iterator i = cachedRows.find(key);
...
Row_DesignObjCategory* Table_DesignObjCategory::GetRow(long int in_PK_DesignObjCategory)
{
	...
	Key row_key(in_PK_DesignObjCategory);

	map<Key, Row_DesignObjCategory*, Key_Less>::iterator i;
...
Row_DesignObjCategory* Table_DesignObjCategory::FetchRow(Table_DesignObjCategory::Key &key)
{
	PLUTO_SAFETY_LOCK(M, m_Mutex);
