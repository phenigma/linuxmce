...
{
	map<Table_Command::Key, class Row_Command*, Table_Command::Key_Less>::iterator it;
...
void Row_Command::Delete()
{
	...
	if (!is_deleted)
		...
		{
			Table_Command::Key key(this);					
			map<Table_Command::Key, Row_Command*, Table_Command::Key_Less>::iterator i = table->cachedRows.find(key);
...
return m_AVCommand;}
string Row_Command::Modification_RecordInfo_get(){PLUTO_SAFETY_LOCK(M, table->m_Mutex);

return m_Modification_RecordInfo;}
short int Row_Command::IsNew_RecordInfo_get(){PLUTO_SAFETY_LOCK(M, table->m_Mutex);

return m_IsNew_RecordInfo;}
short int Row_Command::IsDeleted_RecordInfo_get(){PLUTO_SAFETY_LOCK(M, table->m_Mutex);

return m_IsDeleted_RecordInfo;}
long int Row_Command::FK_Users_RecordInfo_get(){PLUTO_SAFETY_LOCK(M, table->m_Mutex);
...
void Row_Command::Modification_RecordInfo_set(string val){PLUTO_SAFETY_LOCK(M, table->m_Mutex);

m_Modification_RecordInfo = val; is_modified=true; is_null[5]=false;}
void Row_Command::IsNew_RecordInfo_set(short int val){PLUTO_SAFETY_LOCK(M, table->m_Mutex);

m_IsNew_RecordInfo = val; is_modified=true; is_null[6]=false;}
void Row_Command::IsDeleted_RecordInfo_set(short int val){PLUTO_SAFETY_LOCK(M, table->m_Mutex);

m_IsDeleted_RecordInfo = val; is_modified=true; is_null[7]=false;}
void Row_Command::FK_Users_RecordInfo_set(long int val){PLUTO_SAFETY_LOCK(M, table->m_Mutex);
...
return is_null[2];}
bool Row_Command::IsNew_RecordInfo_isNull() {PLUTO_SAFETY_LOCK(M, table->m_Mutex);

return is_null[6];}
bool Row_Command::IsDeleted_RecordInfo_isNull() {PLUTO_SAFETY_LOCK(M, table->m_Mutex);

return is_null[7];}
bool Row_Command::FK_Users_RecordInfo_isNull() {PLUTO_SAFETY_LOCK(M, table->m_Mutex);
...
is_null[2]=val;}
void Row_Command::IsNew_RecordInfo_setNull(bool val){PLUTO_SAFETY_LOCK(M, table->m_Mutex);

is_null[6]=val;}
void Row_Command::IsDeleted_RecordInfo_setNull(bool val){PLUTO_SAFETY_LOCK(M, table->m_Mutex);

is_null[7]=val;}
void Row_Command::FK_Users_RecordInfo_setNull(bool val){PLUTO_SAFETY_LOCK(M, table->m_Mutex);
...
string Row_Command::Modification_RecordInfo_asSQL()
{
PLUTO_SAFETY_LOCK(M, table->m_Mutex);
...
string Row_Command::IsNew_RecordInfo_asSQL()
{
PLUTO_SAFETY_LOCK(M, table->m_Mutex);
...
string Row_Command::IsDeleted_RecordInfo_asSQL()
{
PLUTO_SAFETY_LOCK(M, table->m_Mutex);
...
string Row_Command::FK_Users_RecordInfo_asSQL()
{
PLUTO_SAFETY_LOCK(M, table->m_Mutex);
...
//insert added
	...
	{
		vector<Row_Command*>::iterator i = addedRows.begin();
	
		Row_Command *pRow = *i;
	
		
string values_list_comma_separated;
values_list_comma_separated = values_list_comma_separated + pRow->PK_Command_asSQL()+", "+pRow->Description_asSQL()+", "+pRow->Define_asSQL()+", "+pRow->FK_CommandCategory_asSQL()+", "+pRow->AVCommand_asSQL()+", "+pRow->Modification_RecordInfo_asSQL()+", "+pRow->IsNew_RecordInfo_asSQL()+", "+pRow->IsDeleted_RecordInfo_asSQL()+", "+pRow->FK_Users_RecordInfo_asSQL();

	
		string query = "insert into Command (PK_Command, Description, Define, FK_CommandCategory, AVCommand, Modification_RecordInfo, IsNew_RecordInfo, IsDeleted_RecordInfo, FK_Users_RecordInfo) values ("+
...
//update modified
	...
	for (map<Key, Row_Command*, Key_Less>::iterator i = cachedRows.begin(); i!= cachedRows.end(); i++)
		if	(((*i).second)->is_modified)
	{
		Row_Command* pRow = (*i).second;	
		Key key(pRow);	

		char tmp_PK_Command[32];
sprintf(tmp_PK_Command, "%li", key.pk_PK_Command);
...
update_values_list = update_values_list + "PK_Command="+pRow->PK_Command_asSQL()+", Description="+pRow->Description_asSQL()+", Define="+pRow->Define_asSQL()+", FK_CommandCategory="+pRow->FK_CommandCategory_asSQL()+", AVCommand="+pRow->AVCommand_asSQL()+", Modification_RecordInfo="+pRow->Modification_RecordInfo_asSQL()+", IsNew_RecordInfo="+pRow->IsNew_RecordInfo_asSQL()+", IsDeleted_RecordInfo="+pRow->IsDeleted_RecordInfo_asSQL()+", FK_Users_RecordInfo="+pRow->FK_Users_RecordInfo_asSQL();
...
//delete deleted added
	...
	{	
		vector<Row_Command*>::iterator i = deleted_addedRows.begin();
		delete (*i);
...
//delete deleted cached
	...
	{	
		map<Key, Row_Command*, Key_Less>::iterator i = deleted_cachedRows.begin();
...
bool Table_Command::GetRows(string where_statement,vector<class Row_Command*> *rows)
{
	...
	string query = "select * from Command where " + where_statement;
...
sscanf(row[8], "%li", &(pRow->m_FK_Users_RecordInfo));
		...
		Key key(pRow);
                ...
                map<Table_Command::Key, Row_Command*, Table_Command::Key_Less>::iterator i = cachedRows.find(key);
...
Row_Command* Table_Command::GetRow(long int in_PK_Command)
{
	...
	Key row_key(in_PK_Command);

	map<Key, Row_Command*, Key_Less>::iterator i;
...
Row_Command* Table_Command::FetchRow(Table_Command::Key &key)
{
	PLUTO_SAFETY_LOCK(M, m_Mutex);
