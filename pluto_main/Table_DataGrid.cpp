...
{
	map<Table_DataGrid::Key, class Row_DataGrid*, Table_DataGrid::Key_Less>::iterator it;
...
void Row_DataGrid::Delete()
{
	...
	if (!is_deleted)
		if (is_added)	
		{	
			vector<Row_DataGrid*>::iterator i;	
		...
		{
			Table_DataGrid::Key key(this);					
			map<Table_DataGrid::Key, Row_DataGrid*, Table_DataGrid::Key_Less>::iterator i = table->cachedRows.find(key);
...
return m_Define;}
string Row_DataGrid::Modification_RecordInfo_get(){PLUTO_SAFETY_LOCK(M, table->m_Mutex);

return m_Modification_RecordInfo;}
short int Row_DataGrid::IsNew_RecordInfo_get(){PLUTO_SAFETY_LOCK(M, table->m_Mutex);

return m_IsNew_RecordInfo;}
short int Row_DataGrid::IsDeleted_RecordInfo_get(){PLUTO_SAFETY_LOCK(M, table->m_Mutex);

return m_IsDeleted_RecordInfo;}
long int Row_DataGrid::FK_Users_RecordInfo_get(){PLUTO_SAFETY_LOCK(M, table->m_Mutex);
...
void Row_DataGrid::Modification_RecordInfo_set(string val){PLUTO_SAFETY_LOCK(M, table->m_Mutex);

m_Modification_RecordInfo = val; is_modified=true; is_null[3]=false;}
void Row_DataGrid::IsNew_RecordInfo_set(short int val){PLUTO_SAFETY_LOCK(M, table->m_Mutex);

m_IsNew_RecordInfo = val; is_modified=true; is_null[4]=false;}
void Row_DataGrid::IsDeleted_RecordInfo_set(short int val){PLUTO_SAFETY_LOCK(M, table->m_Mutex);

m_IsDeleted_RecordInfo = val; is_modified=true; is_null[5]=false;}
void Row_DataGrid::FK_Users_RecordInfo_set(long int val){PLUTO_SAFETY_LOCK(M, table->m_Mutex);
...
return is_null[2];}
bool Row_DataGrid::IsNew_RecordInfo_isNull() {PLUTO_SAFETY_LOCK(M, table->m_Mutex);

return is_null[4];}
bool Row_DataGrid::IsDeleted_RecordInfo_isNull() {PLUTO_SAFETY_LOCK(M, table->m_Mutex);

return is_null[5];}
bool Row_DataGrid::FK_Users_RecordInfo_isNull() {PLUTO_SAFETY_LOCK(M, table->m_Mutex);
...
is_null[2]=val;}
void Row_DataGrid::IsNew_RecordInfo_setNull(bool val){PLUTO_SAFETY_LOCK(M, table->m_Mutex);

is_null[4]=val;}
void Row_DataGrid::IsDeleted_RecordInfo_setNull(bool val){PLUTO_SAFETY_LOCK(M, table->m_Mutex);

is_null[5]=val;}
void Row_DataGrid::FK_Users_RecordInfo_setNull(bool val){PLUTO_SAFETY_LOCK(M, table->m_Mutex);
...
string Row_DataGrid::Modification_RecordInfo_asSQL()
{
PLUTO_SAFETY_LOCK(M, table->m_Mutex);
...
string Row_DataGrid::IsNew_RecordInfo_asSQL()
{
PLUTO_SAFETY_LOCK(M, table->m_Mutex);
...
string Row_DataGrid::IsDeleted_RecordInfo_asSQL()
{
PLUTO_SAFETY_LOCK(M, table->m_Mutex);
...
string Row_DataGrid::FK_Users_RecordInfo_asSQL()
{
PLUTO_SAFETY_LOCK(M, table->m_Mutex);
...
//insert added
	...
	{
		vector<Row_DataGrid*>::iterator i = addedRows.begin();
	
		Row_DataGrid *pRow = *i;
	
		
string values_list_comma_separated;
values_list_comma_separated = values_list_comma_separated + pRow->PK_DataGrid_asSQL()+", "+pRow->Description_asSQL()+", "+pRow->Define_asSQL()+", "+pRow->Modification_RecordInfo_asSQL()+", "+pRow->IsNew_RecordInfo_asSQL()+", "+pRow->IsDeleted_RecordInfo_asSQL()+", "+pRow->FK_Users_RecordInfo_asSQL();

	
		string query = "insert into DataGrid (PK_DataGrid, Description, Define, Modification_RecordInfo, IsNew_RecordInfo, IsDeleted_RecordInfo, FK_Users_RecordInfo) values ("+
...
//update modified
	...
	for (map<Key, Row_DataGrid*, Key_Less>::iterator i = cachedRows.begin(); i!= cachedRows.end(); i++)
		if	(((*i).second)->is_modified)
	{
		Row_DataGrid* pRow = (*i).second;	
		Key key(pRow);	

		char tmp_PK_DataGrid[32];
sprintf(tmp_PK_DataGrid, "%li", key.pk_PK_DataGrid);
...
//delete deleted added
	...
	{	
		vector<Row_DataGrid*>::iterator i = deleted_addedRows.begin();
		delete (*i);
...
//delete deleted cached
	...
	{	
		map<Key, Row_DataGrid*, Key_Less>::iterator i = deleted_cachedRows.begin();
...
bool Table_DataGrid::GetRows(string where_statement,vector<class Row_DataGrid*> *rows)
{
	...
	string query = "select * from DataGrid where " + where_statement;
...
sscanf(row[6], "%li", &(pRow->m_FK_Users_RecordInfo));
		...
		Key key(pRow);
                ...
                map<Table_DataGrid::Key, Row_DataGrid*, Table_DataGrid::Key_Less>::iterator i = cachedRows.find(key);
...
Row_DataGrid* Table_DataGrid::GetRow(long int in_PK_DataGrid)
{
	...
	Key row_key(in_PK_DataGrid);

	map<Key, Row_DataGrid*, Key_Less>::iterator i;
...
Row_DataGrid* Table_DataGrid::FetchRow(Table_DataGrid::Key &key)
{
	PLUTO_SAFETY_LOCK(M, m_Mutex);
