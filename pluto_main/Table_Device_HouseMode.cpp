...
{
	map<Table_Device_HouseMode::Key, class Row_Device_HouseMode*, Table_Device_HouseMode::Key_Less>::iterator it;
...
void Row_Device_HouseMode::Delete()
{
	...
	if (!is_deleted)
		if (is_added)	
		{	
			vector<Row_Device_HouseMode*>::iterator i;	
		...
		{
			Table_Device_HouseMode::Key key(this);					
			map<Table_Device_HouseMode::Key, Row_Device_HouseMode*, Table_Device_HouseMode::Key_Less>::iterator i = table->cachedRows.find(key);
...
return m_FK_Event;}
string Row_Device_HouseMode::Modification_RecordInfo_get(){PLUTO_SAFETY_LOCK(M, table->m_Mutex);

return m_Modification_RecordInfo;}
short int Row_Device_HouseMode::IsNew_RecordInfo_get(){PLUTO_SAFETY_LOCK(M, table->m_Mutex);

return m_IsNew_RecordInfo;}
short int Row_Device_HouseMode::IsDeleted_RecordInfo_get(){PLUTO_SAFETY_LOCK(M, table->m_Mutex);

return m_IsDeleted_RecordInfo;}
long int Row_Device_HouseMode::FK_Users_RecordInfo_get(){PLUTO_SAFETY_LOCK(M, table->m_Mutex);
...
void Row_Device_HouseMode::Modification_RecordInfo_set(string val){PLUTO_SAFETY_LOCK(M, table->m_Mutex);

m_Modification_RecordInfo = val; is_modified=true; is_null[3]=false;}
void Row_Device_HouseMode::IsNew_RecordInfo_set(short int val){PLUTO_SAFETY_LOCK(M, table->m_Mutex);

m_IsNew_RecordInfo = val; is_modified=true; is_null[4]=false;}
void Row_Device_HouseMode::IsDeleted_RecordInfo_set(short int val){PLUTO_SAFETY_LOCK(M, table->m_Mutex);

m_IsDeleted_RecordInfo = val; is_modified=true; is_null[5]=false;}
void Row_Device_HouseMode::FK_Users_RecordInfo_set(long int val){PLUTO_SAFETY_LOCK(M, table->m_Mutex);

m_FK_Users_RecordInfo = val; is_modified=true; is_null[6]=false;}

		
bool Row_Device_HouseMode::IsNew_RecordInfo_isNull() {PLUTO_SAFETY_LOCK(M, table->m_Mutex);

return is_null[4];}
bool Row_Device_HouseMode::IsDeleted_RecordInfo_isNull() {PLUTO_SAFETY_LOCK(M, table->m_Mutex);

return is_null[5];}
bool Row_Device_HouseMode::FK_Users_RecordInfo_isNull() {PLUTO_SAFETY_LOCK(M, table->m_Mutex);

return is_null[6];}

			
void Row_Device_HouseMode::IsNew_RecordInfo_setNull(bool val){PLUTO_SAFETY_LOCK(M, table->m_Mutex);

is_null[4]=val;}
void Row_Device_HouseMode::IsDeleted_RecordInfo_setNull(bool val){PLUTO_SAFETY_LOCK(M, table->m_Mutex);

is_null[5]=val;}
void Row_Device_HouseMode::FK_Users_RecordInfo_setNull(bool val){PLUTO_SAFETY_LOCK(M, table->m_Mutex);
...
string Row_Device_HouseMode::Modification_RecordInfo_asSQL()
{
PLUTO_SAFETY_LOCK(M, table->m_Mutex);
...
string Row_Device_HouseMode::IsNew_RecordInfo_asSQL()
{
PLUTO_SAFETY_LOCK(M, table->m_Mutex);
...
string Row_Device_HouseMode::IsDeleted_RecordInfo_asSQL()
{
PLUTO_SAFETY_LOCK(M, table->m_Mutex);
...
string Row_Device_HouseMode::FK_Users_RecordInfo_asSQL()
{
PLUTO_SAFETY_LOCK(M, table->m_Mutex);
...
void Table_Device_HouseMode::Commit()
{
...
//insert added
	...
	{
		vector<Row_Device_HouseMode*>::iterator i = addedRows.begin();
	
		Row_Device_HouseMode *pRow = *i;
	
		
string values_list_comma_separated;
values_list_comma_separated = values_list_comma_separated + pRow->FK_Device_asSQL()+", "+pRow->FK_HouseMode_asSQL()+", "+pRow->FK_Event_asSQL()+", "+pRow->Modification_RecordInfo_asSQL()+", "+pRow->IsNew_RecordInfo_asSQL()+", "+pRow->IsDeleted_RecordInfo_asSQL()+", "+pRow->FK_Users_RecordInfo_asSQL();

	
		string query = "insert into Device_HouseMode (FK_Device, FK_HouseMode, FK_Event, Modification_RecordInfo, IsNew_RecordInfo, IsDeleted_RecordInfo, FK_Users_RecordInfo) values ("+
...
//update modified
	...
	for (map<Key, Row_Device_HouseMode*, Key_Less>::iterator i = cachedRows.begin(); i!= cachedRows.end(); i++)
		if	(((*i).second)->is_modified)
	{
		Row_Device_HouseMode* pRow = (*i).second;	
		Key key(pRow);	
...
sprintf(tmp_FK_HouseMode, "%li", key.pk_FK_HouseMode);
...
update_values_list = update_values_list + "FK_Device="+pRow->FK_Device_asSQL()+", FK_HouseMode="+pRow->FK_HouseMode_asSQL()+", FK_Event="+pRow->FK_Event_asSQL()+", Modification_RecordInfo="+pRow->Modification_RecordInfo_asSQL()+", IsNew_RecordInfo="+pRow->IsNew_RecordInfo_asSQL()+", IsDeleted_RecordInfo="+pRow->IsDeleted_RecordInfo_asSQL()+", FK_Users_RecordInfo="+pRow->FK_Users_RecordInfo_asSQL();
...
//delete deleted added
	...
	{	
		vector<Row_Device_HouseMode*>::iterator i = deleted_addedRows.begin();
		delete (*i);
...
//delete deleted cached
	...
	{	
		map<Key, Row_Device_HouseMode*, Key_Less>::iterator i = deleted_cachedRows.begin();
...
bool Table_Device_HouseMode::GetRows(string where_statement,vector<class Row_Device_HouseMode*> *rows)
{
	...
	string query = "select * from Device_HouseMode where " + where_statement;
...
sscanf(row[6], "%li", &(pRow->m_FK_Users_RecordInfo));
		...
		Key key(pRow);
                ...
                map<Table_Device_HouseMode::Key, Row_Device_HouseMode*, Table_Device_HouseMode::Key_Less>::iterator i = cachedRows.find(key);
...
Row_Device_HouseMode* Table_Device_HouseMode::GetRow(long int in_FK_Device, long int in_FK_HouseMode)
{
	...
	Key row_key(in_FK_Device, in_FK_HouseMode);

	map<Key, Row_Device_HouseMode*, Key_Less>::iterator i;
...
Row_Device_HouseMode* Table_Device_HouseMode::FetchRow(Table_Device_HouseMode::Key &key)
{
	PLUTO_SAFETY_LOCK(M, m_Mutex);
