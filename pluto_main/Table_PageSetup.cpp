...
{
	map<Table_PageSetup::Key, class Row_PageSetup*, Table_PageSetup::Key_Less>::iterator it;
...
void Row_PageSetup::Delete()
{
	...
	if (!is_deleted)
		if (is_added)	
		{	
			vector<Row_PageSetup*>::iterator i;	
		...
		{
			Table_PageSetup::Key key(this);					
			map<Table_PageSetup::Key, Row_PageSetup*, Table_PageSetup::Key_Less>::iterator i = table->cachedRows.find(key);
...
//insert added
	...
	{
		vector<Row_PageSetup*>::iterator i = addedRows.begin();
	
		Row_PageSetup *pRow = *i;
...
//update modified
	...
	for (map<Key, Row_PageSetup*, Key_Less>::iterator i = cachedRows.begin(); i!= cachedRows.end(); i++)
		if	(((*i).second)->is_modified)
	{
		Row_PageSetup* pRow = (*i).second;	
		Key key(pRow);	

		char tmp_PK_PageSetup[32];
sprintf(tmp_PK_PageSetup, "%li", key.pk_PK_PageSetup);
...
//delete deleted added
	...
	{	
		vector<Row_PageSetup*>::iterator i = deleted_addedRows.begin();
		delete (*i);
...
//delete deleted cached
	...
	{	
		map<Key, Row_PageSetup*, Key_Less>::iterator i = deleted_cachedRows.begin();
...
sscanf(row[10], "%li", &(pRow->m_FK_Users_RecordInfo));
		...
		Key key(pRow);
                ...
                map<Table_PageSetup::Key, Row_PageSetup*, Table_PageSetup::Key_Less>::iterator i = cachedRows.find(key);
...
Row_PageSetup* Table_PageSetup::GetRow(long int in_PK_PageSetup)
{
	...
	Key row_key(in_PK_PageSetup);

	map<Key, Row_PageSetup*, Key_Less>::iterator i;
...
Row_PageSetup* Table_PageSetup::FetchRow(Table_PageSetup::Key &key)
{
	PLUTO_SAFETY_LOCK(M, m_Mutex);
