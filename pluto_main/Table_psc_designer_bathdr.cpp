...
{
	map<Table_psc_designer_bathdr::Key, class Row_psc_designer_bathdr*, Table_psc_designer_bathdr::Key_Less>::iterator it;
...
void Row_psc_designer_bathdr::Delete()
{
	...
	if (!is_deleted)
		if (is_added)	
		{	
			vector<Row_psc_designer_bathdr*>::iterator i;	
		...
		{
			Table_psc_designer_bathdr::Key key(this);					
			map<Table_psc_designer_bathdr::Key, Row_psc_designer_bathdr*, Table_psc_designer_bathdr::Key_Less>::iterator i = table->cachedRows.find(key);
...
string Row_psc_designer_bathdr::Value_get(){PLUTO_SAFETY_LOCK(M, table->m_Mutex);
...
void Row_psc_designer_bathdr::Value_set(string val){PLUTO_SAFETY_LOCK(M, table->m_Mutex);
...
string Row_psc_designer_bathdr::Value_asSQL()
{
PLUTO_SAFETY_LOCK(M, table->m_Mutex);
...
void Table_psc_designer_bathdr::Commit()
{
...
//insert added
	...
	{
		vector<Row_psc_designer_bathdr*>::iterator i = addedRows.begin();
	
		Row_psc_designer_bathdr *pRow = *i;
	
		
string values_list_comma_separated;
values_list_comma_separated = values_list_comma_separated + pRow->PK_psc_designer_bathdr_asSQL()+", "+pRow->Value_asSQL();

	
		string query = "insert into psc_designer_bathdr (PK_psc_designer_bathdr, Value) values ("+
...
//update modified
	...
	for (map<Key, Row_psc_designer_bathdr*, Key_Less>::iterator i = cachedRows.begin(); i!= cachedRows.end(); i++)
		if	(((*i).second)->is_modified)
	{
		Row_psc_designer_bathdr* pRow = (*i).second;	
		Key key(pRow);	

		char tmp_PK_psc_designer_bathdr[32];
sprintf(tmp_PK_psc_designer_bathdr, "%li", key.pk_PK_psc_designer_bathdr);
...
update_values_list = update_values_list + "PK_psc_designer_bathdr="+pRow->PK_psc_designer_bathdr_asSQL()+", Value="+pRow->Value_asSQL();
...
//delete deleted added
	...
	{	
		vector<Row_psc_designer_bathdr*>::iterator i = deleted_addedRows.begin();
		delete (*i);
...
//delete deleted cached
	...
	{	
		map<Key, Row_psc_designer_bathdr*, Key_Less>::iterator i = deleted_cachedRows.begin();
...
pRow->m_Value = string(row[1],lengths[1]);
		...
		Key key(pRow);
                ...
                map<Table_psc_designer_bathdr::Key, Row_psc_designer_bathdr*, Table_psc_designer_bathdr::Key_Less>::iterator i = cachedRows.find(key);
...
Row_psc_designer_bathdr* Table_psc_designer_bathdr::GetRow(long int in_PK_psc_designer_bathdr)
{
	...
	Key row_key(in_PK_psc_designer_bathdr);

	map<Key, Row_psc_designer_bathdr*, Key_Less>::iterator i;
...
Row_psc_designer_bathdr* Table_psc_designer_bathdr::FetchRow(Table_psc_designer_bathdr::Key &key)
{
	PLUTO_SAFETY_LOCK(M, m_Mutex);
