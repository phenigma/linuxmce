...
class DLL_EXPORT Table_DesignObj
{
private:
...
private:		
	...
	map<Table_DesignObj::Key, class Row_DesignObj*, Table_DesignObj::Key_Less> cachedRows;
	map<Table_DesignObj::Key, class Row_DesignObj*, Table_DesignObj::Key_Less> deleted_cachedRows;
	vector<class Row_DesignObj*> addedRows;
	vector<class Row_DesignObj*> deleted_addedRows;	
...
private:	
	...
	class Row_DesignObj* FetchRow(Key &key);
...
void Orbiter_FK_DesignObj_ScreenSaver_getrows(vector <class Row_Orbiter*> *rows);
		...
		void SetupSerialization() {
			StartSerializeList() + m_PK_DesignObj+ m_Description+ m_Define+ m_FK_DesignObjType+ m_FK_DesignObjCategory+ m_FK_DesignObj_IncludeIfOtherIncluded+ m_Priority+ m_CantGoBack+ m_CommandsProcessedAtServer+ m_TimeoutSeconds+ m_Animate+ m_FK_StabilityStatus+ m_Modification_RecordInfo+ m_IsNew_RecordInfo+ m_IsDeleted_RecordInfo+ m_FK_Users_RecordInfo;
		}
	private:
