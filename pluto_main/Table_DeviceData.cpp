...
values_list_comma_separated = values_list_comma_separated + pRow->PK_DeviceData_asSQL()+", "+pRow->Description_asSQL()+", "+pRow->Define_asSQL()+", "+pRow->FK_ParameterType_asSQL()+", "+pRow->psc_id_asSQL()+", "+pRow->psc_batch_asSQL()+", "+pRow->psc_user_asSQL()+", "+pRow->psc_frozen_asSQL()+", "+pRow->psc_mod_asSQL();

	
		string query = "insert into DeviceData (PK_DeviceData, Description, Define, FK_ParameterType, psc_id, psc_batch, psc_user, psc_frozen, psc_mod) values ("+
...
bool Table_DeviceData::GetRows(string where_statement,vector<class Row_DeviceData*> *rows)
{
	...
	if( StringUtils::StartsWith(where_statement,"where ",true) || StringUtils::StartsWith(where_statement,"join ",true) )
		query = "select * from DeviceData " + where_statement;
	else if( StringUtils::StartsWith(where_statement,"select ",true) )
		query = where_statement;
	else
		query = "select * from DeviceData where " + where_statement;
