...
{
	map<Table_psc_shared_batdet::Key, class Row_psc_shared_batdet*, Table_psc_shared_batdet::Key_Less>::iterator it;
...
void Row_psc_shared_batdet::Delete()
{
	...
	if (!is_deleted)
		if (is_added)	
		{	
			vector<Row_psc_shared_batdet*>::iterator i;	
		...
		{
			Table_psc_shared_batdet::Key key(this);					
			map<Table_psc_shared_batdet::Key, Row_psc_shared_batdet*, Table_psc_shared_batdet::Key_Less>::iterator i = table->cachedRows.find(key);
...
//insert added
	...
	{
		vector<Row_psc_shared_batdet*>::iterator i = addedRows.begin();
	
		Row_psc_shared_batdet *pRow = *i;
...
//update modified
	...
	for (map<Key, Row_psc_shared_batdet*, Key_Less>::iterator i = cachedRows.begin(); i!= cachedRows.end(); i++)
		if	(((*i).second)->is_modified)
	{
		Row_psc_shared_batdet* pRow = (*i).second;	
		Key key(pRow);	

		char tmp_PK_psc_shared_batdet[32];
sprintf(tmp_PK_psc_shared_batdet, "%li", key.pk_PK_psc_shared_batdet);
...
//delete deleted added
	...
	{	
		vector<Row_psc_shared_batdet*>::iterator i = deleted_addedRows.begin();
		delete (*i);
...
//delete deleted cached
	...
	{	
		map<Key, Row_psc_shared_batdet*, Key_Less>::iterator i = deleted_cachedRows.begin();
...
pRow->m_Value = string(row[1],lengths[1]);
		...
		Key key(pRow);
                ...
                map<Table_psc_shared_batdet::Key, Row_psc_shared_batdet*, Table_psc_shared_batdet::Key_Less>::iterator i = cachedRows.find(key);
...
Row_psc_shared_batdet* Table_psc_shared_batdet::GetRow(long int in_PK_psc_shared_batdet)
{
	...
	Key row_key(in_PK_psc_shared_batdet);

	map<Key, Row_psc_shared_batdet*, Key_Less>::iterator i;
...
Row_psc_shared_batdet* Table_psc_shared_batdet::FetchRow(Table_psc_shared_batdet::Key &key)
{
	PLUTO_SAFETY_LOCK(M, m_Mutex);
