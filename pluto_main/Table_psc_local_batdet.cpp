...
{
	map<Table_psc_local_batdet::Key, class Row_psc_local_batdet*, Table_psc_local_batdet::Key_Less>::iterator it;
...
void Row_psc_local_batdet::Delete()
{
	...
	if (!is_deleted)
		if (is_added)	
		{	
			vector<Row_psc_local_batdet*>::iterator i;	
		...
		{
			Table_psc_local_batdet::Key key(this);					
			map<Table_psc_local_batdet::Key, Row_psc_local_batdet*, Table_psc_local_batdet::Key_Less>::iterator i = table->cachedRows.find(key);
...
string Row_psc_local_batdet::Value_get(){PLUTO_SAFETY_LOCK(M, table->m_Mutex);
...
void Row_psc_local_batdet::Value_set(string val){PLUTO_SAFETY_LOCK(M, table->m_Mutex);
...
string Row_psc_local_batdet::Value_asSQL()
{
PLUTO_SAFETY_LOCK(M, table->m_Mutex);
...
void Table_psc_local_batdet::Commit()
{
...
//insert added
	...
	{
		vector<Row_psc_local_batdet*>::iterator i = addedRows.begin();
	
		Row_psc_local_batdet *pRow = *i;
	
		
string values_list_comma_separated;
values_list_comma_separated = values_list_comma_separated + pRow->PK_psc_local_batdet_asSQL()+", "+pRow->Value_asSQL();

	
		string query = "insert into psc_local_batdet (PK_psc_local_batdet, Value) values ("+
...
//update modified
	...
	for (map<Key, Row_psc_local_batdet*, Key_Less>::iterator i = cachedRows.begin(); i!= cachedRows.end(); i++)
		if	(((*i).second)->is_modified)
	{
		Row_psc_local_batdet* pRow = (*i).second;	
		Key key(pRow);	

		char tmp_PK_psc_local_batdet[32];
sprintf(tmp_PK_psc_local_batdet, "%li", key.pk_PK_psc_local_batdet);
...
update_values_list = update_values_list + "PK_psc_local_batdet="+pRow->PK_psc_local_batdet_asSQL()+", Value="+pRow->Value_asSQL();
...
//delete deleted added
	...
	{	
		vector<Row_psc_local_batdet*>::iterator i = deleted_addedRows.begin();
		delete (*i);
...
//delete deleted cached
	...
	{	
		map<Key, Row_psc_local_batdet*, Key_Less>::iterator i = deleted_cachedRows.begin();
...
pRow->m_Value = string(row[1],lengths[1]);
		...
		Key key(pRow);
                ...
                map<Table_psc_local_batdet::Key, Row_psc_local_batdet*, Table_psc_local_batdet::Key_Less>::iterator i = cachedRows.find(key);
...
Row_psc_local_batdet* Table_psc_local_batdet::GetRow(long int in_PK_psc_local_batdet)
{
	...
	Key row_key(in_PK_psc_local_batdet);

	map<Key, Row_psc_local_batdet*, Key_Less>::iterator i;
...
Row_psc_local_batdet* Table_psc_local_batdet::FetchRow(Table_psc_local_batdet::Key &key)
{
	PLUTO_SAFETY_LOCK(M, m_Mutex);
