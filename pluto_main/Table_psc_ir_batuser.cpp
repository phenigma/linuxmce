...
long int Row_psc_ir_batuser::PK_psc_ir_batuser_get(){PLUTO_SAFETY_LOCK(M, table->m_Mutex);

return m_PK_psc_ir_batuser;}
long int Row_psc_ir_batuser::FK_psc_ir_bathdr_get(){PLUTO_SAFETY_LOCK(M, table->m_Mutex);

return m_FK_psc_ir_bathdr;}
long int Row_psc_ir_batuser::psc_user_get(){PLUTO_SAFETY_LOCK(M, table->m_Mutex);

return m_psc_user;}
short int Row_psc_ir_batuser::is_sup_get(){PLUTO_SAFETY_LOCK(M, table->m_Mutex);

return m_is_sup;}

		
void Row_psc_ir_batuser::PK_psc_ir_batuser_set(long int val){PLUTO_SAFETY_LOCK(M, table->m_Mutex);

m_PK_psc_ir_batuser = val; is_modified=true; is_null[0]=false;}
void Row_psc_ir_batuser::FK_psc_ir_bathdr_set(long int val){PLUTO_SAFETY_LOCK(M, table->m_Mutex);

m_FK_psc_ir_bathdr = val; is_modified=true; is_null[1]=false;}
void Row_psc_ir_batuser::psc_user_set(long int val){PLUTO_SAFETY_LOCK(M, table->m_Mutex);

m_psc_user = val; is_modified=true; is_null[2]=false;}
void Row_psc_ir_batuser::is_sup_set(short int val){PLUTO_SAFETY_LOCK(M, table->m_Mutex);
...
bool Table_psc_ir_batuser::Commit()
{
...
values_list_comma_separated = values_list_comma_separated + pRow->PK_psc_ir_batuser_asSQL()+", "+pRow->FK_psc_ir_bathdr_asSQL()+", "+pRow->psc_user_asSQL()+", "+pRow->is_sup_asSQL();

	
		string query = "insert into psc_ir_batuser (PK_psc_ir_batuser, FK_psc_ir_bathdr, psc_user, is_sup) values ("+
...
update_values_list = update_values_list + "PK_psc_ir_batuser="+pRow->PK_psc_ir_batuser_asSQL()+", FK_psc_ir_bathdr="+pRow->FK_psc_ir_bathdr_asSQL()+", psc_user="+pRow->psc_user_asSQL()+", is_sup="+pRow->is_sup_asSQL();
...
bool Table_psc_ir_batuser::GetRows(string where_statement,vector<class Row_psc_ir_batuser*> *rows)
{
	...
	if( StringUtils::StartsWith(where_statement,"where ",true) || StringUtils::StartsWith(where_statement,"join ",true) )
		query = "select * from psc_ir_batuser " + where_statement;
	else if( StringUtils::StartsWith(where_statement,"select ",true) )
		query = where_statement;
	else
		query = "select * from psc_ir_batuser where " + where_statement;
