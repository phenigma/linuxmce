...
{
	map<Table_psc_ir_tables::Key, class Row_psc_ir_tables*, Table_psc_ir_tables::Key_Less>::iterator it;
...
void Row_psc_ir_tables::Delete()
{
	...
	if (!is_deleted)
		if (is_added)	
		{	
			vector<Row_psc_ir_tables*>::iterator i;	
		...
		{
			Table_psc_ir_tables::Key key(this);					
			map<Table_psc_ir_tables::Key, Row_psc_ir_tables*, Table_psc_ir_tables::Key_Less>::iterator i = table->cachedRows.find(key);
...
return is_null[2];}
bool Row_psc_ir_tables::frozen_isNull() {PLUTO_SAFETY_LOCK(M, table->m_Mutex);

return is_null[3];}
bool Row_psc_ir_tables::last_psc_id_isNull() {PLUTO_SAFETY_LOCK(M, table->m_Mutex);

return is_null[4];}
bool Row_psc_ir_tables::last_psc_batch_isNull() {PLUTO_SAFETY_LOCK(M, table->m_Mutex);
...
is_null[2]=val;}
void Row_psc_ir_tables::frozen_setNull(bool val){PLUTO_SAFETY_LOCK(M, table->m_Mutex);

is_null[3]=val;}
void Row_psc_ir_tables::last_psc_id_setNull(bool val){PLUTO_SAFETY_LOCK(M, table->m_Mutex);

is_null[4]=val;}
void Row_psc_ir_tables::last_psc_batch_setNull(bool val){PLUTO_SAFETY_LOCK(M, table->m_Mutex);
...
void Table_psc_ir_tables::Commit()
{
...
//insert added
	...
	{
		vector<Row_psc_ir_tables*>::iterator i = addedRows.begin();
	
		Row_psc_ir_tables *pRow = *i;
	
		
string values_list_comma_separated;
values_list_comma_separated = values_list_comma_separated + pRow->PK_psc_ir_tables_asSQL()+", "+pRow->Tablename_asSQL()+", "+pRow->filter_asSQL()+", "+pRow->frozen_asSQL()+", "+pRow->last_psc_id_asSQL()+", "+pRow->last_psc_batch_asSQL();

	
		string query = "insert into psc_ir_tables (PK_psc_ir_tables, Tablename, filter, frozen, last_psc_id, last_psc_batch) values ("+
...
//update modified
	...
	for (map<Key, Row_psc_ir_tables*, Key_Less>::iterator i = cachedRows.begin(); i!= cachedRows.end(); i++)
		if	(((*i).second)->is_modified)
	{
		Row_psc_ir_tables* pRow = (*i).second;	
		Key key(pRow);	

		char tmp_PK_psc_ir_tables[32];
sprintf(tmp_PK_psc_ir_tables, "%li", key.pk_PK_psc_ir_tables);
...
update_values_list = update_values_list + "PK_psc_ir_tables="+pRow->PK_psc_ir_tables_asSQL()+", Tablename="+pRow->Tablename_asSQL()+", filter="+pRow->filter_asSQL()+", frozen="+pRow->frozen_asSQL()+", last_psc_id="+pRow->last_psc_id_asSQL()+", last_psc_batch="+pRow->last_psc_batch_asSQL();
...
//delete deleted added
	...
	{	
		vector<Row_psc_ir_tables*>::iterator i = deleted_addedRows.begin();
		delete (*i);
...
//delete deleted cached
	...
	{	
		map<Key, Row_psc_ir_tables*, Key_Less>::iterator i = deleted_cachedRows.begin();
...
bool Table_psc_ir_tables::GetRows(string where_statement,vector<class Row_psc_ir_tables*> *rows)
{
	...
	if( StringUtils::StartsWith(where_statement,"where ",true) || StringUtils::StartsWith(where_statement,"join ",true) )
		query = "select * from psc_ir_tables " + where_statement;
	else if( StringUtils::StartsWith(where_statement,"select ",true) )
		query = where_statement;
	else
		query = "select * from psc_ir_tables where " + where_statement;
...
sscanf(row[5], "%li", &(pRow->m_last_psc_batch));
		...
		Key key(pRow);
                ...
                map<Table_psc_ir_tables::Key, Row_psc_ir_tables*, Table_psc_ir_tables::Key_Less>::iterator i = cachedRows.find(key);
...
Row_psc_ir_tables* Table_psc_ir_tables::GetRow(long int in_PK_psc_ir_tables)
{
	...
	Key row_key(in_PK_psc_ir_tables);

	map<Key, Row_psc_ir_tables*, Key_Less>::iterator i;
...
Row_psc_ir_tables* Table_psc_ir_tables::FetchRow(Table_psc_ir_tables::Key &key)
{
	PLUTO_SAFETY_LOCK(M, m_Mutex);
