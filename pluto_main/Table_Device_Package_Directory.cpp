...
{
	map<Table_Device_Package_Directory::Key, class Row_Device_Package_Directory*, Table_Device_Package_Directory::Key_Less>::iterator it;
...
void Row_Device_Package_Directory::Delete()
{
	...
	if (!is_deleted)
		if (is_added)	
		{	
			vector<Row_Device_Package_Directory*>::iterator i;	
		...
		{
			Table_Device_Package_Directory::Key key(this);					
			map<Table_Device_Package_Directory::Key, Row_Device_Package_Directory*, Table_Device_Package_Directory::Key_Less>::iterator i = table->cachedRows.find(key);
...
//insert added
	...
	{
		vector<Row_Device_Package_Directory*>::iterator i = addedRows.begin();
	
		Row_Device_Package_Directory *pRow = *i;
...
//update modified
	...
	for (map<Key, Row_Device_Package_Directory*, Key_Less>::iterator i = cachedRows.begin(); i!= cachedRows.end(); i++)
		if	(((*i).second)->is_modified)
	{
		Row_Device_Package_Directory* pRow = (*i).second;	
		Key key(pRow);	
...
//delete deleted added
	...
	{	
		vector<Row_Device_Package_Directory*>::iterator i = deleted_addedRows.begin();
		delete (*i);
...
//delete deleted cached
	...
	{	
		map<Key, Row_Device_Package_Directory*, Key_Less>::iterator i = deleted_cachedRows.begin();
...
bool Table_Device_Package_Directory::GetRows(string where_statement,vector<class Row_Device_Package_Directory*> *rows)
{
	...
	string query = "select * from Device_Package_Directory where " + where_statement;
...
sscanf(row[3], "%hi", &(pRow->m_Verified));
		...
		Key key(pRow);
                ...
                map<Table_Device_Package_Directory::Key, Row_Device_Package_Directory*, Table_Device_Package_Directory::Key_Less>::iterator i = cachedRows.find(key);
...
Row_Device_Package_Directory* Table_Device_Package_Directory::GetRow(long int in_FK_Device, long int in_FK_Package_Directory)
{
	...
	Key row_key(in_FK_Device, in_FK_Package_Directory);

	map<Key, Row_Device_Package_Directory*, Key_Less>::iterator i;
...
Row_Device_Package_Directory* Table_Device_Package_Directory::FetchRow(Table_Device_Package_Directory::Key &key)
{
	PLUTO_SAFETY_LOCK(M, m_Mutex);
