...
class DLL_EXPORT Table_psc_website_tables : public TableBase , SingleLongKeyBase
{
private:
...
public:
	...
	Table_psc_website_tables(Database_pluto_main *pDatabase):database(pDatabase), m_Mutex("psc_website_tables")
	{
...
class DLL_EXPORT Row_psc_website_tables : public TableRow, public SerializeClass
	{
...
void last_psc_batch_setNull(bool val);
		...
		void SetupSerialization() {
			StartSerializeList() + m_PK_psc_website_tables+ m_Tablename+ m_filter+ m_frozen+ m_last_psc_id+ m_last_psc_batch;
		}
	private:
