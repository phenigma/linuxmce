...
{
	map<Table_VertAlignment::Key, class Row_VertAlignment*, Table_VertAlignment::Key_Less>::iterator it;
...
void Row_VertAlignment::Delete()
{
	...
	if (!is_deleted)
		if (is_added)	
		{	
			vector<Row_VertAlignment*>::iterator i;	
		...
		{
			Table_VertAlignment::Key key(this);					
			map<Table_VertAlignment::Key, Row_VertAlignment*, Table_VertAlignment::Key_Less>::iterator i = table->cachedRows.find(key);
...
return m_Define;}
string Row_VertAlignment::Modification_RecordInfo_get(){PLUTO_SAFETY_LOCK(M, table->m_Mutex);

return m_Modification_RecordInfo;}
short int Row_VertAlignment::IsNew_RecordInfo_get(){PLUTO_SAFETY_LOCK(M, table->m_Mutex);

return m_IsNew_RecordInfo;}
short int Row_VertAlignment::IsDeleted_RecordInfo_get(){PLUTO_SAFETY_LOCK(M, table->m_Mutex);

return m_IsDeleted_RecordInfo;}
long int Row_VertAlignment::FK_Users_RecordInfo_get(){PLUTO_SAFETY_LOCK(M, table->m_Mutex);
...
void Row_VertAlignment::Modification_RecordInfo_set(string val){PLUTO_SAFETY_LOCK(M, table->m_Mutex);

m_Modification_RecordInfo = val; is_modified=true; is_null[3]=false;}
void Row_VertAlignment::IsNew_RecordInfo_set(short int val){PLUTO_SAFETY_LOCK(M, table->m_Mutex);

m_IsNew_RecordInfo = val; is_modified=true; is_null[4]=false;}
void Row_VertAlignment::IsDeleted_RecordInfo_set(short int val){PLUTO_SAFETY_LOCK(M, table->m_Mutex);

m_IsDeleted_RecordInfo = val; is_modified=true; is_null[5]=false;}
void Row_VertAlignment::FK_Users_RecordInfo_set(long int val){PLUTO_SAFETY_LOCK(M, table->m_Mutex);

m_FK_Users_RecordInfo = val; is_modified=true; is_null[6]=false;}

		
bool Row_VertAlignment::IsNew_RecordInfo_isNull() {PLUTO_SAFETY_LOCK(M, table->m_Mutex);

return is_null[4];}
bool Row_VertAlignment::IsDeleted_RecordInfo_isNull() {PLUTO_SAFETY_LOCK(M, table->m_Mutex);

return is_null[5];}
bool Row_VertAlignment::FK_Users_RecordInfo_isNull() {PLUTO_SAFETY_LOCK(M, table->m_Mutex);

return is_null[6];}

			
void Row_VertAlignment::IsNew_RecordInfo_setNull(bool val){PLUTO_SAFETY_LOCK(M, table->m_Mutex);

is_null[4]=val;}
void Row_VertAlignment::IsDeleted_RecordInfo_setNull(bool val){PLUTO_SAFETY_LOCK(M, table->m_Mutex);

is_null[5]=val;}
void Row_VertAlignment::FK_Users_RecordInfo_setNull(bool val){PLUTO_SAFETY_LOCK(M, table->m_Mutex);
...
string Row_VertAlignment::Modification_RecordInfo_asSQL()
{
PLUTO_SAFETY_LOCK(M, table->m_Mutex);
...
string Row_VertAlignment::IsNew_RecordInfo_asSQL()
{
PLUTO_SAFETY_LOCK(M, table->m_Mutex);
...
string Row_VertAlignment::IsDeleted_RecordInfo_asSQL()
{
PLUTO_SAFETY_LOCK(M, table->m_Mutex);
...
string Row_VertAlignment::FK_Users_RecordInfo_asSQL()
{
PLUTO_SAFETY_LOCK(M, table->m_Mutex);
...
void Table_VertAlignment::Commit()
{
...
//insert added
	...
	{
		vector<Row_VertAlignment*>::iterator i = addedRows.begin();
	
		Row_VertAlignment *pRow = *i;
	
		
string values_list_comma_separated;
values_list_comma_separated = values_list_comma_separated + pRow->PK_VertAlignment_asSQL()+", "+pRow->Description_asSQL()+", "+pRow->Define_asSQL()+", "+pRow->Modification_RecordInfo_asSQL()+", "+pRow->IsNew_RecordInfo_asSQL()+", "+pRow->IsDeleted_RecordInfo_asSQL()+", "+pRow->FK_Users_RecordInfo_asSQL();

	
		string query = "insert into VertAlignment (PK_VertAlignment, Description, Define, Modification_RecordInfo, IsNew_RecordInfo, IsDeleted_RecordInfo, FK_Users_RecordInfo) values ("+
...
//update modified
	...
	for (map<Key, Row_VertAlignment*, Key_Less>::iterator i = cachedRows.begin(); i!= cachedRows.end(); i++)
		if	(((*i).second)->is_modified)
	{
		Row_VertAlignment* pRow = (*i).second;	
		Key key(pRow);	

		char tmp_PK_VertAlignment[32];
sprintf(tmp_PK_VertAlignment, "%li", key.pk_PK_VertAlignment);
...
//delete deleted added
	...
	{	
		vector<Row_VertAlignment*>::iterator i = deleted_addedRows.begin();
		delete (*i);
...
//delete deleted cached
	...
	{	
		map<Key, Row_VertAlignment*, Key_Less>::iterator i = deleted_cachedRows.begin();
...
bool Table_VertAlignment::GetRows(string where_statement,vector<class Row_VertAlignment*> *rows)
{
	...
	string query = "select * from VertAlignment where " + where_statement;
...
sscanf(row[6], "%li", &(pRow->m_FK_Users_RecordInfo));
		...
		Key key(pRow);
                ...
                map<Table_VertAlignment::Key, Row_VertAlignment*, Table_VertAlignment::Key_Less>::iterator i = cachedRows.find(key);
...
Row_VertAlignment* Table_VertAlignment::GetRow(long int in_PK_VertAlignment)
{
	...
	Key row_key(in_PK_VertAlignment);

	map<Key, Row_VertAlignment*, Key_Less>::iterator i;
...
Row_VertAlignment* Table_VertAlignment::FetchRow(Table_VertAlignment::Key &key)
{
	PLUTO_SAFETY_LOCK(M, m_Mutex);
