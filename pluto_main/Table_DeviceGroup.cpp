...
Table_DeviceGroup::~Table_DeviceGroup()
{
	...
	{
		Row_DeviceGroup *pRow = (Row_DeviceGroup *) (*it).second;
		delete pRow;
	}
...
void Row_DeviceGroup::Delete()
{
	...
	Row_DeviceGroup *pRow = this; // Needed so we will have only 1 version of get_primary_fields_assign_from_row
	
	if (!is_deleted)
		if (is_added)	
		{	
			vector<TableRow*>::iterator i;	
			for (i = table->addedRows.begin(); (i!=table->addedRows.end()) && ( (Row_DeviceGroup *) *i != this); i++);
		...
		{
			SingleLongKey key(pRow->m_PK_DeviceGroup);
...
long int Row_DeviceGroup::psc_id_get(){PLUTO_SAFETY_LOCK(M, table->m_Mutex);

return m_psc_id;}
long int Row_DeviceGroup::psc_batch_get(){PLUTO_SAFETY_LOCK(M, table->m_Mutex);

return m_psc_batch;}
long int Row_DeviceGroup::psc_user_get(){PLUTO_SAFETY_LOCK(M, table->m_Mutex);

return m_psc_user;}
short int Row_DeviceGroup::psc_frozen_get(){PLUTO_SAFETY_LOCK(M, table->m_Mutex);

return m_psc_frozen;}
string Row_DeviceGroup::psc_mod_get(){PLUTO_SAFETY_LOCK(M, table->m_Mutex);
...
void Row_DeviceGroup::psc_id_set(long int val){PLUTO_SAFETY_LOCK(M, table->m_Mutex);

m_psc_id = val; is_modified=true; is_null[4]=false;}
void Row_DeviceGroup::psc_batch_set(long int val){PLUTO_SAFETY_LOCK(M, table->m_Mutex);

m_psc_batch = val; is_modified=true; is_null[5]=false;}
void Row_DeviceGroup::psc_user_set(long int val){PLUTO_SAFETY_LOCK(M, table->m_Mutex);

m_psc_user = val; is_modified=true; is_null[6]=false;}
void Row_DeviceGroup::psc_frozen_set(short int val){PLUTO_SAFETY_LOCK(M, table->m_Mutex);

m_psc_frozen = val; is_modified=true; is_null[7]=false;}
void Row_DeviceGroup::psc_mod_set(string val){PLUTO_SAFETY_LOCK(M, table->m_Mutex);
...
return is_null[2];}
bool Row_DeviceGroup::psc_id_isNull() {PLUTO_SAFETY_LOCK(M, table->m_Mutex);

return is_null[4];}
bool Row_DeviceGroup::psc_batch_isNull() {PLUTO_SAFETY_LOCK(M, table->m_Mutex);

return is_null[5];}
bool Row_DeviceGroup::psc_user_isNull() {PLUTO_SAFETY_LOCK(M, table->m_Mutex);

return is_null[6];}
bool Row_DeviceGroup::psc_frozen_isNull() {PLUTO_SAFETY_LOCK(M, table->m_Mutex);
...
is_null[2]=val;}
void Row_DeviceGroup::psc_id_setNull(bool val){PLUTO_SAFETY_LOCK(M, table->m_Mutex);

is_null[4]=val;}
void Row_DeviceGroup::psc_batch_setNull(bool val){PLUTO_SAFETY_LOCK(M, table->m_Mutex);

is_null[5]=val;}
void Row_DeviceGroup::psc_user_setNull(bool val){PLUTO_SAFETY_LOCK(M, table->m_Mutex);

is_null[6]=val;}
void Row_DeviceGroup::psc_frozen_setNull(bool val){PLUTO_SAFETY_LOCK(M, table->m_Mutex);
...
string Row_DeviceGroup::psc_id_asSQL()
{
PLUTO_SAFETY_LOCK(M, table->m_Mutex);
...
string Row_DeviceGroup::psc_batch_asSQL()
{
PLUTO_SAFETY_LOCK(M, table->m_Mutex);
...
string Row_DeviceGroup::psc_user_asSQL()
{
PLUTO_SAFETY_LOCK(M, table->m_Mutex);
...
string Row_DeviceGroup::psc_frozen_asSQL()
{
PLUTO_SAFETY_LOCK(M, table->m_Mutex);
...
string Row_DeviceGroup::psc_mod_asSQL()
{
PLUTO_SAFETY_LOCK(M, table->m_Mutex);
...
//insert added
	while (!addedRows.empty())
	{
		...
		Row_DeviceGroup *pRow = (Row_DeviceGroup *)*i;
	
		
string values_list_comma_separated;
values_list_comma_separated = values_list_comma_separated + pRow->PK_DeviceGroup_asSQL()+", "+pRow->FK_DeviceGroup_Parent_asSQL()+", "+pRow->FK_Installation_asSQL()+", "+pRow->Description_asSQL()+", "+pRow->psc_id_asSQL()+", "+pRow->psc_batch_asSQL()+", "+pRow->psc_user_asSQL()+", "+pRow->psc_frozen_asSQL()+", "+pRow->psc_mod_asSQL();

	
		string query = "insert into DeviceGroup (PK_DeviceGroup, FK_DeviceGroup_Parent, FK_Installation, Description, psc_id, psc_batch, psc_user, psc_frozen, psc_mod) values ("+
...
//update modified
	...
	{
		Row_DeviceGroup* pRow = (Row_DeviceGroup*) (*i).second;	
		SingleLongKey key(pRow->m_PK_DeviceGroup);

		char tmp_PK_DeviceGroup[32];
sprintf(tmp_PK_DeviceGroup, "%li", key.pk);
...
Row_DeviceGroup* Table_DeviceGroup::GetRow(long int in_PK_DeviceGroup)
{
	...
	SingleLongKey row_key(in_PK_DeviceGroup);
	...
	//row is cached
	if (i!=cachedRows.end())
		return (Row_DeviceGroup*) (*i).second;
...
Row_DeviceGroup* Table_DeviceGroup::FetchRow(SingleLongKey &key)
{
	PLUTO_SAFETY_LOCK(M, m_Mutex);
