...
{
	map<Table_Package_Source_Compat::Key, class Row_Package_Source_Compat*, Table_Package_Source_Compat::Key_Less>::iterator it;
...
void Row_Package_Source_Compat::Delete()
{
	...
	if (!is_deleted)
		if (is_added)	
		{	
			vector<Row_Package_Source_Compat*>::iterator i;	
		...
		{
			Table_Package_Source_Compat::Key key(this);					
			map<Table_Package_Source_Compat::Key, Row_Package_Source_Compat*, Table_Package_Source_Compat::Key_Less>::iterator i = table->cachedRows.find(key);
...
//insert added
	...
	{
		vector<Row_Package_Source_Compat*>::iterator i = addedRows.begin();
	
		Row_Package_Source_Compat *pRow = *i;
...
//update modified
	...
	for (map<Key, Row_Package_Source_Compat*, Key_Less>::iterator i = cachedRows.begin(); i!= cachedRows.end(); i++)
		if	(((*i).second)->is_modified)
	{
		Row_Package_Source_Compat* pRow = (*i).second;	
		Key key(pRow);	

		char tmp_PK_Package_Source_Compat[32];
sprintf(tmp_PK_Package_Source_Compat, "%li", key.pk_PK_Package_Source_Compat);
...
//delete deleted added
	...
	{	
		vector<Row_Package_Source_Compat*>::iterator i = deleted_addedRows.begin();
		delete (*i);
...
//delete deleted cached
	...
	{	
		map<Key, Row_Package_Source_Compat*, Key_Less>::iterator i = deleted_cachedRows.begin();
...
pRow->m_Comments = string(row[5],lengths[5]);
		...
		Key key(pRow);
                ...
                map<Table_Package_Source_Compat::Key, Row_Package_Source_Compat*, Table_Package_Source_Compat::Key_Less>::iterator i = cachedRows.find(key);
...
Row_Package_Source_Compat* Table_Package_Source_Compat::GetRow(long int in_PK_Package_Source_Compat)
{
	...
	Key row_key(in_PK_Package_Source_Compat);

	map<Key, Row_Package_Source_Compat*, Key_Less>::iterator i;
...
Row_Package_Source_Compat* Table_Package_Source_Compat::FetchRow(Table_Package_Source_Compat::Key &key)
{
	PLUTO_SAFETY_LOCK(M, m_Mutex);
