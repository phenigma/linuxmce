...
class DLL_EXPORT Table_DeviceTemplate
{
private:
...
private:		
	...
	map<Table_DeviceTemplate::Key, class Row_DeviceTemplate*, Table_DeviceTemplate::Key_Less> cachedRows;
	map<Table_DeviceTemplate::Key, class Row_DeviceTemplate*, Table_DeviceTemplate::Key_Less> deleted_cachedRows;
	vector<class Row_DeviceTemplate*> addedRows;
	vector<class Row_DeviceTemplate*> deleted_addedRows;	
...
private:	
	...
	class Row_DeviceTemplate* FetchRow(Key &key);
...
void InfraredCode_FK_DeviceTemplate_getrows(vector <class Row_InfraredCode*> *rows);
		...
		void SetupSerialization() {
			StartSerializeList() + m_PK_DeviceTemplate+ m_Description+ m_Comments+ m_FK_DeviceCategory+ m_FK_Manufacturer+ m_Define+ m_ImplementsDCE+ m_IsEmbedded+ m_CommandLine+ m_RequiresGUI+ m_IsAVDevice+ m_IsPlugIn+ m_IRFrequency+ m_FK_StabilityStatus+ m_DestinationPackage+ m_DestinationDir+ m_FK_Users_Maintainer+ m_Modification_RecordInfo+ m_IsNew_RecordInfo+ m_IsDeleted_RecordInfo+ m_FK_Users_RecordInfo;
		}
	private:
