...
{
	map<Table_Room::Key, class Row_Room*, Table_Room::Key_Less>::iterator it;
...
void Row_Room::Delete()
{
	...
	if (!is_deleted)
		...
		{
			Table_Room::Key key(this);					
			map<Table_Room::Key, Row_Room*, Table_Room::Key_Less>::iterator i = table->cachedRows.find(key);
...
long int Row_Room::PK_Room_get(){PLUTO_SAFETY_LOCK(M, table->m_Mutex);

return m_PK_Room;}
long int Row_Room::FK_Installation_get(){PLUTO_SAFETY_LOCK(M, table->m_Mutex);

return m_FK_Installation;}
long int Row_Room::FK_RoomType_get(){PLUTO_SAFETY_LOCK(M, table->m_Mutex);

return m_FK_RoomType;}
string Row_Room::Description_get(){PLUTO_SAFETY_LOCK(M, table->m_Mutex);

return m_Description;}
long int Row_Room::FK_Icon_get(){PLUTO_SAFETY_LOCK(M, table->m_Mutex);
...
void Row_Room::FK_Installation_set(long int val){PLUTO_SAFETY_LOCK(M, table->m_Mutex);

m_FK_Installation = val; is_modified=true; is_null[1]=false;}
void Row_Room::FK_RoomType_set(long int val){PLUTO_SAFETY_LOCK(M, table->m_Mutex);

m_FK_RoomType = val; is_modified=true; is_null[2]=false;}
void Row_Room::Description_set(string val){PLUTO_SAFETY_LOCK(M, table->m_Mutex);

m_Description = val; is_modified=true; is_null[3]=false;}
void Row_Room::FK_Icon_set(long int val){PLUTO_SAFETY_LOCK(M, table->m_Mutex);
...
return is_null[2];}
bool Row_Room::FK_Icon_isNull() {PLUTO_SAFETY_LOCK(M, table->m_Mutex);
...
void Row_Room::FK_RoomType_setNull(bool val){PLUTO_SAFETY_LOCK(M, table->m_Mutex);

is_null[2]=val;}
void Row_Room::FK_Icon_setNull(bool val){PLUTO_SAFETY_LOCK(M, table->m_Mutex);
...
string Row_Room::FK_RoomMode_asSQL()
{
PLUTO_SAFETY_LOCK(M, table->m_Mutex);
...
//insert added
	...
	{
		vector<Row_Room*>::iterator i = addedRows.begin();
...
//update modified
	...
	for (map<Key, Row_Room*, Key_Less>::iterator i = cachedRows.begin(); i!= cachedRows.end(); i++)
		if	(((*i).second)->is_modified)
	{
		Row_Room* pRow = (*i).second;	
		Key key(pRow);	

		char tmp_PK_Room[32];
sprintf(tmp_PK_Room, "%li", key.pk_PK_Room);
...
//delete deleted added
	...
	{	
		vector<Row_Room*>::iterator i = deleted_addedRows.begin();
		delete (*i);
...
//delete deleted cached
	...
	{	
		map<Key, Row_Room*, Key_Less>::iterator i = deleted_cachedRows.begin();
...
bool Table_Room::GetRows(string where_statement,vector<class Row_Room*> *rows)
{
	...
	if( StringUtils::StartsWith(where_statement,"where ",true) || StringUtils::StartsWith(where_statement,"join ",true) )
		query = "select * from Room " + where_statement;
	else if( StringUtils::StartsWith(where_statement,"select ",true) )
		query = where_statement;
	else
		query = "select * from Room where " + where_statement;
...
{
pRow->is_null[5]=true;
pRow->m_FK_RoomMode = 0;
}
else
{
pRow->is_null[5]=false;
sscanf(row[5], "%li", &(pRow->m_FK_RoomMode));
}
...
pRow->m_psc_mod = string(row[10],lengths[10]);
		...
		Key key(pRow);
                ...
                map<Table_Room::Key, Row_Room*, Table_Room::Key_Less>::iterator i = cachedRows.find(key);
...
Row_Room* Table_Room::GetRow(long int in_PK_Room)
{
	...
	Key row_key(in_PK_Room);

	map<Key, Row_Room*, Key_Less>::iterator i;
...
Row_Room* Table_Room::FetchRow(Table_Room::Key &key)
{
	PLUTO_SAFETY_LOCK(M, m_Mutex);
