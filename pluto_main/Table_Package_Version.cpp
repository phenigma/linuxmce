...
{
	map<Table_Package_Version::Key, class Row_Package_Version*, Table_Package_Version::Key_Less>::iterator it;
...
void Row_Package_Version::Delete()
{
	...
	if (!is_deleted)
		if (is_added)	
		{	
			vector<Row_Package_Version*>::iterator i;	
		...
		{
			Table_Package_Version::Key key(this);					
			map<Table_Package_Version::Key, Row_Package_Version*, Table_Package_Version::Key_Less>::iterator i = table->cachedRows.find(key);
...
//insert added
	...
	{
		vector<Row_Package_Version*>::iterator i = addedRows.begin();
	
		Row_Package_Version *pRow = *i;
...
//update modified
	...
	for (map<Key, Row_Package_Version*, Key_Less>::iterator i = cachedRows.begin(); i!= cachedRows.end(); i++)
		if	(((*i).second)->is_modified)
	{
		Row_Package_Version* pRow = (*i).second;	
		Key key(pRow);	
...
sprintf(tmp_FK_Version, "%li", key.pk_FK_Version);
...
//delete deleted added
	...
	{	
		vector<Row_Package_Version*>::iterator i = deleted_addedRows.begin();
		delete (*i);
...
//delete deleted cached
	...
	{	
		map<Key, Row_Package_Version*, Key_Less>::iterator i = deleted_cachedRows.begin();
...
pRow->m_Comments = string(row[2],lengths[2]);
		...
		Key key(pRow);
                ...
                map<Table_Package_Version::Key, Row_Package_Version*, Table_Package_Version::Key_Less>::iterator i = cachedRows.find(key);
...
Row_Package_Version* Table_Package_Version::GetRow(long int in_FK_Package, long int in_FK_Version)
{
	...
	Key row_key(in_FK_Package, in_FK_Version);

	map<Key, Row_Package_Version*, Key_Less>::iterator i;
...
Row_Package_Version* Table_Package_Version::FetchRow(Table_Package_Version::Key &key)
{
	PLUTO_SAFETY_LOCK(M, m_Mutex);
