...
{
	map<Table_psc_ir_repset::Key, class Row_psc_ir_repset*, Table_psc_ir_repset::Key_Less>::iterator it;
...
void Row_psc_ir_repset::Delete()
{
	...
	if (!is_deleted)
		if (is_added)	
		{	
			vector<Row_psc_ir_repset*>::iterator i;	
		...
		{
			Table_psc_ir_repset::Key key(this);					
			map<Table_psc_ir_repset::Key, Row_psc_ir_repset*, Table_psc_ir_repset::Key_Less>::iterator i = table->cachedRows.find(key);
...
//insert added
	...
	{
		vector<Row_psc_ir_repset*>::iterator i = addedRows.begin();
	
		Row_psc_ir_repset *pRow = *i;
	
		
string values_list_comma_separated;
values_list_comma_separated = values_list_comma_separated + pRow->PK_psc_ir_repset_asSQL()+", "+pRow->Value_asSQL();

	
		string query = "insert into psc_ir_repset (PK_psc_ir_repset, Value) values ("+
...
//update modified
	...
	for (map<Key, Row_psc_ir_repset*, Key_Less>::iterator i = cachedRows.begin(); i!= cachedRows.end(); i++)
		if	(((*i).second)->is_modified)
	{
		Row_psc_ir_repset* pRow = (*i).second;	
		Key key(pRow);	

		char tmp_PK_psc_ir_repset[32];
sprintf(tmp_PK_psc_ir_repset, "%li", key.pk_PK_psc_ir_repset);
...
update_values_list = update_values_list + "PK_psc_ir_repset="+pRow->PK_psc_ir_repset_asSQL()+", Value="+pRow->Value_asSQL();
...
//delete deleted added
	...
	{	
		vector<Row_psc_ir_repset*>::iterator i = deleted_addedRows.begin();
		delete (*i);
...
//delete deleted cached
	...
	{	
		map<Key, Row_psc_ir_repset*, Key_Less>::iterator i = deleted_cachedRows.begin();
...
pRow->m_Value = string(row[1],lengths[1]);
		...
		Key key(pRow);
                ...
                map<Table_psc_ir_repset::Key, Row_psc_ir_repset*, Table_psc_ir_repset::Key_Less>::iterator i = cachedRows.find(key);
...
Row_psc_ir_repset* Table_psc_ir_repset::GetRow(long int in_PK_psc_ir_repset)
{
	...
	Key row_key(in_PK_psc_ir_repset);

	map<Key, Row_psc_ir_repset*, Key_Less>::iterator i;
...
Row_psc_ir_repset* Table_psc_ir_repset::FetchRow(Table_psc_ir_repset::Key &key)
{
	PLUTO_SAFETY_LOCK(M, m_Mutex);
