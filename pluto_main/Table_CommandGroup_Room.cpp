...
{
	map<Table_CommandGroup_Room::Key, class Row_CommandGroup_Room*, Table_CommandGroup_Room::Key_Less>::iterator it;
...
void Row_CommandGroup_Room::Delete()
{
	...
	if (!is_deleted)
		if (is_added)	
		{	
			vector<Row_CommandGroup_Room*>::iterator i;	
		...
		{
			Table_CommandGroup_Room::Key key(this);					
			map<Table_CommandGroup_Room::Key, Row_CommandGroup_Room*, Table_CommandGroup_Room::Key_Less>::iterator i = table->cachedRows.find(key);
...
return m_Sort;}
string Row_CommandGroup_Room::Modification_RecordInfo_get(){PLUTO_SAFETY_LOCK(M, table->m_Mutex);

return m_Modification_RecordInfo;}
short int Row_CommandGroup_Room::IsNew_RecordInfo_get(){PLUTO_SAFETY_LOCK(M, table->m_Mutex);

return m_IsNew_RecordInfo;}
short int Row_CommandGroup_Room::IsDeleted_RecordInfo_get(){PLUTO_SAFETY_LOCK(M, table->m_Mutex);

return m_IsDeleted_RecordInfo;}
long int Row_CommandGroup_Room::FK_Users_RecordInfo_get(){PLUTO_SAFETY_LOCK(M, table->m_Mutex);
...
void Row_CommandGroup_Room::Modification_RecordInfo_set(string val){PLUTO_SAFETY_LOCK(M, table->m_Mutex);

m_Modification_RecordInfo = val; is_modified=true; is_null[3]=false;}
void Row_CommandGroup_Room::IsNew_RecordInfo_set(short int val){PLUTO_SAFETY_LOCK(M, table->m_Mutex);

m_IsNew_RecordInfo = val; is_modified=true; is_null[4]=false;}
void Row_CommandGroup_Room::IsDeleted_RecordInfo_set(short int val){PLUTO_SAFETY_LOCK(M, table->m_Mutex);

m_IsDeleted_RecordInfo = val; is_modified=true; is_null[5]=false;}
void Row_CommandGroup_Room::FK_Users_RecordInfo_set(long int val){PLUTO_SAFETY_LOCK(M, table->m_Mutex);

m_FK_Users_RecordInfo = val; is_modified=true; is_null[6]=false;}

		
bool Row_CommandGroup_Room::IsNew_RecordInfo_isNull() {PLUTO_SAFETY_LOCK(M, table->m_Mutex);

return is_null[4];}
bool Row_CommandGroup_Room::IsDeleted_RecordInfo_isNull() {PLUTO_SAFETY_LOCK(M, table->m_Mutex);

return is_null[5];}
bool Row_CommandGroup_Room::FK_Users_RecordInfo_isNull() {PLUTO_SAFETY_LOCK(M, table->m_Mutex);

return is_null[6];}

			
void Row_CommandGroup_Room::IsNew_RecordInfo_setNull(bool val){PLUTO_SAFETY_LOCK(M, table->m_Mutex);

is_null[4]=val;}
void Row_CommandGroup_Room::IsDeleted_RecordInfo_setNull(bool val){PLUTO_SAFETY_LOCK(M, table->m_Mutex);

is_null[5]=val;}
void Row_CommandGroup_Room::FK_Users_RecordInfo_setNull(bool val){PLUTO_SAFETY_LOCK(M, table->m_Mutex);
...
string Row_CommandGroup_Room::Modification_RecordInfo_asSQL()
{
PLUTO_SAFETY_LOCK(M, table->m_Mutex);
...
string Row_CommandGroup_Room::IsNew_RecordInfo_asSQL()
{
PLUTO_SAFETY_LOCK(M, table->m_Mutex);
...
string Row_CommandGroup_Room::IsDeleted_RecordInfo_asSQL()
{
PLUTO_SAFETY_LOCK(M, table->m_Mutex);
...
string Row_CommandGroup_Room::FK_Users_RecordInfo_asSQL()
{
PLUTO_SAFETY_LOCK(M, table->m_Mutex);
...
void Table_CommandGroup_Room::Commit()
{
...
//insert added
	...
	{
		vector<Row_CommandGroup_Room*>::iterator i = addedRows.begin();
	
		Row_CommandGroup_Room *pRow = *i;
	
		
string values_list_comma_separated;
values_list_comma_separated = values_list_comma_separated + pRow->FK_CommandGroup_asSQL()+", "+pRow->FK_Room_asSQL()+", "+pRow->Sort_asSQL()+", "+pRow->Modification_RecordInfo_asSQL()+", "+pRow->IsNew_RecordInfo_asSQL()+", "+pRow->IsDeleted_RecordInfo_asSQL()+", "+pRow->FK_Users_RecordInfo_asSQL();

	
		string query = "insert into CommandGroup_Room (FK_CommandGroup, FK_Room, Sort, Modification_RecordInfo, IsNew_RecordInfo, IsDeleted_RecordInfo, FK_Users_RecordInfo) values ("+
...
//update modified
	...
	for (map<Key, Row_CommandGroup_Room*, Key_Less>::iterator i = cachedRows.begin(); i!= cachedRows.end(); i++)
		if	(((*i).second)->is_modified)
	{
		Row_CommandGroup_Room* pRow = (*i).second;	
		Key key(pRow);	
...
char tmp_FK_Room[32];
sprintf(tmp_FK_Room, "%li", key.pk_FK_Room);
...
//delete deleted added
	...
	{	
		vector<Row_CommandGroup_Room*>::iterator i = deleted_addedRows.begin();
		delete (*i);
...
//delete deleted cached
	...
	{	
		map<Key, Row_CommandGroup_Room*, Key_Less>::iterator i = deleted_cachedRows.begin();
...
bool Table_CommandGroup_Room::GetRows(string where_statement,vector<class Row_CommandGroup_Room*> *rows)
{
	...
	string query = "select * from CommandGroup_Room where " + where_statement;
...
sscanf(row[6], "%li", &(pRow->m_FK_Users_RecordInfo));
		...
		Key key(pRow);
                ...
                map<Table_CommandGroup_Room::Key, Row_CommandGroup_Room*, Table_CommandGroup_Room::Key_Less>::iterator i = cachedRows.find(key);
...
Row_CommandGroup_Room* Table_CommandGroup_Room::GetRow(long int in_FK_CommandGroup, long int in_FK_Room)
{
	...
	Key row_key(in_FK_CommandGroup, in_FK_Room);

	map<Key, Row_CommandGroup_Room*, Key_Less>::iterator i;
...
Row_CommandGroup_Room* Table_CommandGroup_Room::FetchRow(Table_CommandGroup_Room::Key &key)
{
	PLUTO_SAFETY_LOCK(M, m_Mutex);
