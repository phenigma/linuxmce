...
{
	map<Table_DeviceCategory_DeviceData::Key, class Row_DeviceCategory_DeviceData*, Table_DeviceCategory_DeviceData::Key_Less>::iterator it;
...
void Row_DeviceCategory_DeviceData::Delete()
{
	...
	if (!is_deleted)
		if (is_added)	
		{	
			vector<Row_DeviceCategory_DeviceData*>::iterator i;	
		...
		{
			Table_DeviceCategory_DeviceData::Key key(this);					
			map<Table_DeviceCategory_DeviceData::Key, Row_DeviceCategory_DeviceData*, Table_DeviceCategory_DeviceData::Key_Less>::iterator i = table->cachedRows.find(key);
...
string Row_DeviceCategory_DeviceData::DefaultValue_get(){PLUTO_SAFETY_LOCK(M, table->m_Mutex);
...
void Row_DeviceCategory_DeviceData::DefaultValue_set(string val){PLUTO_SAFETY_LOCK(M, table->m_Mutex);
...
void Row_DeviceCategory_DeviceData::DefaultValue_setNull(bool val){PLUTO_SAFETY_LOCK(M, table->m_Mutex);
...
string Row_DeviceCategory_DeviceData::DefaultValue_asSQL()
{
PLUTO_SAFETY_LOCK(M, table->m_Mutex);
...
void Table_DeviceCategory_DeviceData::Commit()
{
...
//insert added
	...
	{
		vector<Row_DeviceCategory_DeviceData*>::iterator i = addedRows.begin();
	
		Row_DeviceCategory_DeviceData *pRow = *i;
...
//update modified
	...
	for (map<Key, Row_DeviceCategory_DeviceData*, Key_Less>::iterator i = cachedRows.begin(); i!= cachedRows.end(); i++)
		if	(((*i).second)->is_modified)
	{
		Row_DeviceCategory_DeviceData* pRow = (*i).second;	
		Key key(pRow);	
...
//delete deleted added
	...
	{	
		vector<Row_DeviceCategory_DeviceData*>::iterator i = deleted_addedRows.begin();
		delete (*i);
...
//delete deleted cached
	...
	{	
		map<Key, Row_DeviceCategory_DeviceData*, Key_Less>::iterator i = deleted_cachedRows.begin();
...
bool Table_DeviceCategory_DeviceData::GetRows(string where_statement,vector<class Row_DeviceCategory_DeviceData*> *rows)
{
	...
	string query = "select * from DeviceCategory_DeviceData where " + where_statement;
...
pRow->m_psc_mod = string(row[11],lengths[11]);
		...
		Key key(pRow);
                ...
                map<Table_DeviceCategory_DeviceData::Key, Row_DeviceCategory_DeviceData*, Table_DeviceCategory_DeviceData::Key_Less>::iterator i = cachedRows.find(key);
...
Row_DeviceCategory_DeviceData* Table_DeviceCategory_DeviceData::GetRow(long int in_FK_DeviceCategory, long int in_FK_DeviceData)
{
	...
	Key row_key(in_FK_DeviceCategory, in_FK_DeviceData);

	map<Key, Row_DeviceCategory_DeviceData*, Key_Less>::iterator i;
...
Row_DeviceCategory_DeviceData* Table_DeviceCategory_DeviceData::FetchRow(Table_DeviceCategory_DeviceData::Key &key)
{
	PLUTO_SAFETY_LOCK(M, m_Mutex);
