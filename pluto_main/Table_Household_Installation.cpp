...
{
	map<Table_Household_Installation::Key, class Row_Household_Installation*, Table_Household_Installation::Key_Less>::iterator it;
...
void Row_Household_Installation::Delete()
{
	...
	if (!is_deleted)
		if (is_added)	
		{	
			vector<Row_Household_Installation*>::iterator i;	
		...
		{
			Table_Household_Installation::Key key(this);					
			map<Table_Household_Installation::Key, Row_Household_Installation*, Table_Household_Installation::Key_Less>::iterator i = table->cachedRows.find(key);
...
string Row_Household_Installation::Modification_RecordInfo_get(){PLUTO_SAFETY_LOCK(M, table->m_Mutex);

return m_Modification_RecordInfo;}
short int Row_Household_Installation::IsNew_RecordInfo_get(){PLUTO_SAFETY_LOCK(M, table->m_Mutex);

return m_IsNew_RecordInfo;}
short int Row_Household_Installation::IsDeleted_RecordInfo_get(){PLUTO_SAFETY_LOCK(M, table->m_Mutex);

return m_IsDeleted_RecordInfo;}
long int Row_Household_Installation::FK_Users_RecordInfo_get(){PLUTO_SAFETY_LOCK(M, table->m_Mutex);
...
void Row_Household_Installation::Modification_RecordInfo_set(string val){PLUTO_SAFETY_LOCK(M, table->m_Mutex);

m_Modification_RecordInfo = val; is_modified=true; is_null[2]=false;}
void Row_Household_Installation::IsNew_RecordInfo_set(short int val){PLUTO_SAFETY_LOCK(M, table->m_Mutex);

m_IsNew_RecordInfo = val; is_modified=true; is_null[3]=false;}
void Row_Household_Installation::IsDeleted_RecordInfo_set(short int val){PLUTO_SAFETY_LOCK(M, table->m_Mutex);

m_IsDeleted_RecordInfo = val; is_modified=true; is_null[4]=false;}
void Row_Household_Installation::FK_Users_RecordInfo_set(long int val){PLUTO_SAFETY_LOCK(M, table->m_Mutex);

m_FK_Users_RecordInfo = val; is_modified=true; is_null[5]=false;}

		
bool Row_Household_Installation::IsNew_RecordInfo_isNull() {PLUTO_SAFETY_LOCK(M, table->m_Mutex);

return is_null[3];}
bool Row_Household_Installation::IsDeleted_RecordInfo_isNull() {PLUTO_SAFETY_LOCK(M, table->m_Mutex);

return is_null[4];}
bool Row_Household_Installation::FK_Users_RecordInfo_isNull() {PLUTO_SAFETY_LOCK(M, table->m_Mutex);

return is_null[5];}

			
void Row_Household_Installation::IsNew_RecordInfo_setNull(bool val){PLUTO_SAFETY_LOCK(M, table->m_Mutex);

is_null[3]=val;}
void Row_Household_Installation::IsDeleted_RecordInfo_setNull(bool val){PLUTO_SAFETY_LOCK(M, table->m_Mutex);

is_null[4]=val;}
void Row_Household_Installation::FK_Users_RecordInfo_setNull(bool val){PLUTO_SAFETY_LOCK(M, table->m_Mutex);
...
string Row_Household_Installation::Modification_RecordInfo_asSQL()
{
PLUTO_SAFETY_LOCK(M, table->m_Mutex);
...
string Row_Household_Installation::IsNew_RecordInfo_asSQL()
{
PLUTO_SAFETY_LOCK(M, table->m_Mutex);
...
string Row_Household_Installation::IsDeleted_RecordInfo_asSQL()
{
PLUTO_SAFETY_LOCK(M, table->m_Mutex);
...
string Row_Household_Installation::FK_Users_RecordInfo_asSQL()
{
PLUTO_SAFETY_LOCK(M, table->m_Mutex);
...
void Table_Household_Installation::Commit()
{
...
//insert added
	...
	{
		vector<Row_Household_Installation*>::iterator i = addedRows.begin();
	
		Row_Household_Installation *pRow = *i;
	
		
string values_list_comma_separated;
values_list_comma_separated = values_list_comma_separated + pRow->FK_Household_asSQL()+", "+pRow->FK_Installation_asSQL()+", "+pRow->Modification_RecordInfo_asSQL()+", "+pRow->IsNew_RecordInfo_asSQL()+", "+pRow->IsDeleted_RecordInfo_asSQL()+", "+pRow->FK_Users_RecordInfo_asSQL();

	
		string query = "insert into Household_Installation (FK_Household, FK_Installation, Modification_RecordInfo, IsNew_RecordInfo, IsDeleted_RecordInfo, FK_Users_RecordInfo) values ("+
...
//update modified
	...
	for (map<Key, Row_Household_Installation*, Key_Less>::iterator i = cachedRows.begin(); i!= cachedRows.end(); i++)
		if	(((*i).second)->is_modified)
	{
		Row_Household_Installation* pRow = (*i).second;	
		Key key(pRow);	

		char tmp_FK_Household[32];
sprintf(tmp_FK_Household, "%li", key.pk_FK_Household);
...
update_values_list = update_values_list + "FK_Household="+pRow->FK_Household_asSQL()+", FK_Installation="+pRow->FK_Installation_asSQL()+", Modification_RecordInfo="+pRow->Modification_RecordInfo_asSQL()+", IsNew_RecordInfo="+pRow->IsNew_RecordInfo_asSQL()+", IsDeleted_RecordInfo="+pRow->IsDeleted_RecordInfo_asSQL()+", FK_Users_RecordInfo="+pRow->FK_Users_RecordInfo_asSQL();
...
//delete deleted added
	...
	{	
		vector<Row_Household_Installation*>::iterator i = deleted_addedRows.begin();
		delete (*i);
...
//delete deleted cached
	...
	{	
		map<Key, Row_Household_Installation*, Key_Less>::iterator i = deleted_cachedRows.begin();
...
bool Table_Household_Installation::GetRows(string where_statement,vector<class Row_Household_Installation*> *rows)
{
	...
	string query = "select * from Household_Installation where " + where_statement;
...
sscanf(row[5], "%li", &(pRow->m_FK_Users_RecordInfo));
		...
		Key key(pRow);
                ...
                map<Table_Household_Installation::Key, Row_Household_Installation*, Table_Household_Installation::Key_Less>::iterator i = cachedRows.find(key);
...
Row_Household_Installation* Table_Household_Installation::GetRow(long int in_FK_Household, long int in_FK_Installation)
{
	...
	Key row_key(in_FK_Household, in_FK_Installation);

	map<Key, Row_Household_Installation*, Key_Less>::iterator i;
...
Row_Household_Installation* Table_Household_Installation::FetchRow(Table_Household_Installation::Key &key)
{
	PLUTO_SAFETY_LOCK(M, m_Mutex);
