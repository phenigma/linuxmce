...
{
	map<Table_psc_local_repset::Key, class Row_psc_local_repset*, Table_psc_local_repset::Key_Less>::iterator it;
...
void Row_psc_local_repset::Delete()
{
	...
	if (!is_deleted)
		if (is_added)	
		{	
			vector<Row_psc_local_repset*>::iterator i;	
		...
		{
			Table_psc_local_repset::Key key(this);					
			map<Table_psc_local_repset::Key, Row_psc_local_repset*, Table_psc_local_repset::Key_Less>::iterator i = table->cachedRows.find(key);
...
//insert added
	...
	{
		vector<Row_psc_local_repset*>::iterator i = addedRows.begin();
	
		Row_psc_local_repset *pRow = *i;
...
//update modified
	...
	for (map<Key, Row_psc_local_repset*, Key_Less>::iterator i = cachedRows.begin(); i!= cachedRows.end(); i++)
		if	(((*i).second)->is_modified)
	{
		Row_psc_local_repset* pRow = (*i).second;	
		Key key(pRow);	

		char tmp_PK_psc_local_repset[32];
sprintf(tmp_PK_psc_local_repset, "%li", key.pk_PK_psc_local_repset);
...
//delete deleted added
	...
	{	
		vector<Row_psc_local_repset*>::iterator i = deleted_addedRows.begin();
		delete (*i);
...
//delete deleted cached
	...
	{	
		map<Key, Row_psc_local_repset*, Key_Less>::iterator i = deleted_cachedRows.begin();
...
pRow->m_Value = string(row[1],lengths[1]);
		...
		Key key(pRow);
                ...
                map<Table_psc_local_repset::Key, Row_psc_local_repset*, Table_psc_local_repset::Key_Less>::iterator i = cachedRows.find(key);
...
Row_psc_local_repset* Table_psc_local_repset::GetRow(long int in_PK_psc_local_repset)
{
	...
	Key row_key(in_PK_psc_local_repset);

	map<Key, Row_psc_local_repset*, Key_Less>::iterator i;
...
Row_psc_local_repset* Table_psc_local_repset::FetchRow(Table_psc_local_repset::Key &key)
{
	PLUTO_SAFETY_LOCK(M, m_Mutex);
