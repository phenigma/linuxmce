...
{
	map<Table_Device_EntertainArea::Key, class Row_Device_EntertainArea*, Table_Device_EntertainArea::Key_Less>::iterator it;
...
void Row_Device_EntertainArea::Delete()
{
	...
	if (!is_deleted)
		if (is_added)	
		{	
			vector<Row_Device_EntertainArea*>::iterator i;	
		...
		{
			Table_Device_EntertainArea::Key key(this);					
			map<Table_Device_EntertainArea::Key, Row_Device_EntertainArea*, Table_Device_EntertainArea::Key_Less>::iterator i = table->cachedRows.find(key);
...
string Row_Device_EntertainArea::Modification_RecordInfo_get(){PLUTO_SAFETY_LOCK(M, table->m_Mutex);

return m_Modification_RecordInfo;}
short int Row_Device_EntertainArea::IsNew_RecordInfo_get(){PLUTO_SAFETY_LOCK(M, table->m_Mutex);

return m_IsNew_RecordInfo;}
short int Row_Device_EntertainArea::IsDeleted_RecordInfo_get(){PLUTO_SAFETY_LOCK(M, table->m_Mutex);

return m_IsDeleted_RecordInfo;}
long int Row_Device_EntertainArea::FK_Users_RecordInfo_get(){PLUTO_SAFETY_LOCK(M, table->m_Mutex);
...
void Row_Device_EntertainArea::Modification_RecordInfo_set(string val){PLUTO_SAFETY_LOCK(M, table->m_Mutex);

m_Modification_RecordInfo = val; is_modified=true; is_null[2]=false;}
void Row_Device_EntertainArea::IsNew_RecordInfo_set(short int val){PLUTO_SAFETY_LOCK(M, table->m_Mutex);

m_IsNew_RecordInfo = val; is_modified=true; is_null[3]=false;}
void Row_Device_EntertainArea::IsDeleted_RecordInfo_set(short int val){PLUTO_SAFETY_LOCK(M, table->m_Mutex);

m_IsDeleted_RecordInfo = val; is_modified=true; is_null[4]=false;}
void Row_Device_EntertainArea::FK_Users_RecordInfo_set(long int val){PLUTO_SAFETY_LOCK(M, table->m_Mutex);

m_FK_Users_RecordInfo = val; is_modified=true; is_null[5]=false;}

		
bool Row_Device_EntertainArea::IsNew_RecordInfo_isNull() {PLUTO_SAFETY_LOCK(M, table->m_Mutex);

return is_null[3];}
bool Row_Device_EntertainArea::IsDeleted_RecordInfo_isNull() {PLUTO_SAFETY_LOCK(M, table->m_Mutex);

return is_null[4];}
bool Row_Device_EntertainArea::FK_Users_RecordInfo_isNull() {PLUTO_SAFETY_LOCK(M, table->m_Mutex);

return is_null[5];}

			
void Row_Device_EntertainArea::IsNew_RecordInfo_setNull(bool val){PLUTO_SAFETY_LOCK(M, table->m_Mutex);

is_null[3]=val;}
void Row_Device_EntertainArea::IsDeleted_RecordInfo_setNull(bool val){PLUTO_SAFETY_LOCK(M, table->m_Mutex);

is_null[4]=val;}
void Row_Device_EntertainArea::FK_Users_RecordInfo_setNull(bool val){PLUTO_SAFETY_LOCK(M, table->m_Mutex);
...
string Row_Device_EntertainArea::Modification_RecordInfo_asSQL()
{
PLUTO_SAFETY_LOCK(M, table->m_Mutex);
...
string Row_Device_EntertainArea::IsNew_RecordInfo_asSQL()
{
PLUTO_SAFETY_LOCK(M, table->m_Mutex);
...
string Row_Device_EntertainArea::IsDeleted_RecordInfo_asSQL()
{
PLUTO_SAFETY_LOCK(M, table->m_Mutex);
...
string Row_Device_EntertainArea::FK_Users_RecordInfo_asSQL()
{
PLUTO_SAFETY_LOCK(M, table->m_Mutex);
...
void Table_Device_EntertainArea::Commit()
{
...
//insert added
	...
	{
		vector<Row_Device_EntertainArea*>::iterator i = addedRows.begin();
	
		Row_Device_EntertainArea *pRow = *i;
	
		
string values_list_comma_separated;
values_list_comma_separated = values_list_comma_separated + pRow->FK_Device_asSQL()+", "+pRow->FK_EntertainArea_asSQL()+", "+pRow->Modification_RecordInfo_asSQL()+", "+pRow->IsNew_RecordInfo_asSQL()+", "+pRow->IsDeleted_RecordInfo_asSQL()+", "+pRow->FK_Users_RecordInfo_asSQL();

	
		string query = "insert into Device_EntertainArea (FK_Device, FK_EntertainArea, Modification_RecordInfo, IsNew_RecordInfo, IsDeleted_RecordInfo, FK_Users_RecordInfo) values ("+
...
//update modified
	...
	for (map<Key, Row_Device_EntertainArea*, Key_Less>::iterator i = cachedRows.begin(); i!= cachedRows.end(); i++)
		if	(((*i).second)->is_modified)
	{
		Row_Device_EntertainArea* pRow = (*i).second;	
		Key key(pRow);	
...
update_values_list = update_values_list + "FK_Device="+pRow->FK_Device_asSQL()+", FK_EntertainArea="+pRow->FK_EntertainArea_asSQL()+", Modification_RecordInfo="+pRow->Modification_RecordInfo_asSQL()+", IsNew_RecordInfo="+pRow->IsNew_RecordInfo_asSQL()+", IsDeleted_RecordInfo="+pRow->IsDeleted_RecordInfo_asSQL()+", FK_Users_RecordInfo="+pRow->FK_Users_RecordInfo_asSQL();
...
//delete deleted added
	...
	{	
		vector<Row_Device_EntertainArea*>::iterator i = deleted_addedRows.begin();
		delete (*i);
...
//delete deleted cached
	...
	{	
		map<Key, Row_Device_EntertainArea*, Key_Less>::iterator i = deleted_cachedRows.begin();
...
bool Table_Device_EntertainArea::GetRows(string where_statement,vector<class Row_Device_EntertainArea*> *rows)
{
	...
	string query = "select * from Device_EntertainArea where " + where_statement;
...
sscanf(row[5], "%li", &(pRow->m_FK_Users_RecordInfo));
		...
		Key key(pRow);
                ...
                map<Table_Device_EntertainArea::Key, Row_Device_EntertainArea*, Table_Device_EntertainArea::Key_Less>::iterator i = cachedRows.find(key);
...
Row_Device_EntertainArea* Table_Device_EntertainArea::GetRow(long int in_FK_Device, long int in_FK_EntertainArea)
{
	...
	Key row_key(in_FK_Device, in_FK_EntertainArea);

	map<Key, Row_Device_EntertainArea*, Key_Less>::iterator i;
...
Row_Device_EntertainArea* Table_Device_EntertainArea::FetchRow(Table_Device_EntertainArea::Key &key)
{
	PLUTO_SAFETY_LOCK(M, m_Mutex);
