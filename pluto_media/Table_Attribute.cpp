...
long int Row_Attribute::PK_Attribute_get(){PLUTO_SAFETY_LOCK(M, table->m_Mutex);

return m_PK_Attribute;}
long int Row_Attribute::FK_AttributeType_get(){PLUTO_SAFETY_LOCK(M, table->m_Mutex);

return m_FK_AttributeType;}
string Row_Attribute::Name_get(){PLUTO_SAFETY_LOCK(M, table->m_Mutex);
...
void Row_Attribute::FK_AttributeType_set(long int val){PLUTO_SAFETY_LOCK(M, table->m_Mutex);

m_FK_AttributeType = val; is_modified=true; is_null[1]=false;}
void Row_Attribute::Name_set(string val){PLUTO_SAFETY_LOCK(M, table->m_Mutex);
...
bool Table_Attribute::GetRows(string where_statement,vector<class Row_Attribute*> *rows)
{
	...
	if( StringUtils::StartsWith(where_statement,"where ",true) || StringUtils::StartsWith(where_statement,"join ",true) )
		query = "select * from Attribute " + where_statement;
	else if( StringUtils::StartsWith(where_statement,"select ",true) )
		query = where_statement;
	else
		query = "select * from Attribute where " + where_statement;
