#ifndef __Table_psc_media_batdet_H__
#define __Table_psc_media_batdet_H__
...
#include "Define_psc_media_batdet.h"
...
class DLL_EXPORT Table_psc_media_batdet : public TableBase , SingleLongKeyBase
{
private:
...
public:
	...
	Table_psc_media_batdet(Database_pluto_media *pDatabase):database(pDatabase), m_Mutex("psc_media_batdet")
	{
		pthread_mutexattr_init(&m_MutexAttr);
		pthread_mutexattr_settype(&m_MutexAttr, PTHREAD_MUTEX_RECURSIVE_NP);
		m_Mutex.Init(&m_MutexAttr); 
	};
	~Table_psc_media_batdet();

private:		
	friend class Row_psc_media_batdet;
	struct Key
	{
		friend class Row_psc_media_batdet;
		long int pk_PK_psc_media_batdet;

		
		Key(long int in_PK_psc_media_batdet);
	
		Key(class Row_psc_media_batdet *pRow);
	};
	struct Key_Less
	{			
		bool operator()(const Table_psc_media_batdet::Key &key1, const Table_psc_media_batdet::Key &key2) const;
	};	
...
public:				
	bool Commit();
	bool GetRows(string where_statement,vector<class Row_psc_media_batdet*> *rows);
	class Row_psc_media_batdet* AddRow();
	Database_pluto_media *Database_pluto_media_get() { return database; }
	
		
	class Row_psc_media_batdet* GetRow(long int in_PK_psc_media_batdet);
	

private:	
	
		
	class Row_psc_media_batdet* FetchRow(SingleLongKey &key);
		
			
};

class DLL_EXPORT Row_psc_media_batdet : public TableRow, public SerializeClass
	{
		friend struct Table_psc_media_batdet::Key;
		friend class Table_psc_media_batdet;
	private:
...
void FK_psc_media_bathdr_unauth_set(long int val);
		...
		Row_psc_media_batdet(Table_psc_media_batdet *pTable);
