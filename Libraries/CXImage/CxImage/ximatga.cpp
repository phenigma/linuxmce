...
bool CxImageTGA::Decode(CxFile *hFile)
{
  ...
  try
  {
	...
	if (tgaHead.PixelDepth==16 || tgaHead.PixelDepth==32) AlphaCreate(); // Image has alpha channel
...
bool CxImageTGA::Encode(CxFile * hFile)
{
    ...
    tgaHead.CmapLength=(head.biBitCount == 8) ? (BYTE)255 : (BYTE)0;   // Color Map Length
...
#if CXIMAGE_SUPPORT_ALPHA	// <vho>
					AlphaSet(ix+x,y,(BYTE)(pixel & 32786 >> 8));
#endif //CXIMAGE_SUPPORT_ALPHA
				}
				break;
					 }
...
#if CXIMAGE_SUPPORT_ALPHA	// <vho>
			AlphaSet(x+xoffset,y,(BYTE)(pixel & 32786 >> 8)); //alpha
#endif //CXIMAGE_SUPPORT_ALPHA
		}
		break;
			}
