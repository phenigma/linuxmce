...
{
	{ -1, TRUE, TRUE, "Supported files", "*.bmp;*.gif;*.jpg;*.jpeg;*.png;*.ico;*.tif;*.tiff;*.tga;*.pcx;*.wbmp;*.wmf;*.emf;*.j2k;*.jp2;*.jbg" },
	...
	DWORD dwFlags, BOOL bOpenFileDialog, int* pType)
{
	...
	int nDocType = (pType != NULL) ? *pType : CXIMAGE_FORMAT_JPG;
...
{
	CxImage* newImage = new CxImage();
	if (!newImage->LoadResource(FindResource(NULL,"IDR_JPG1","JPG"),CXIMAGE_FORMAT_JPG)){
		AfxMessageBox(newImage->GetLastError());
		delete newImage;
		newImage =NULL;
		return;
...
void CDemoApp::OnCximageDemosavemultipagetiff() 
{
	...
	if (!newImage3->LoadResource(FindResource(NULL,"IDR_JPG1","JPG"),CXIMAGE_FORMAT_JPG)){
