...
extern "C" {
	class Command_Impl *RegisterAsPlugIn(class Router *pRouter,int PK_Device,string sLogger)
	{
		...
		{
			g_pPlutoLogger = new ServerLogger(PK_Device, Text_To_Speech::PK_DeviceTemplate_get_static(), "localhost");
...
int main(int argc, char* argv[]) 
{
	...
	{
		if( sLogger=="dcerouter" )
			g_pPlutoLogger = new ServerLogger(PK_Device, Text_To_Speech::PK_DeviceTemplate_get_static(), sRouter_IP);
	...
	try
	{
		...
		if ( pText_To_Speech->Connect(pText_To_Speech->PK_DeviceTemplate_get()) ) 
		{
			...
			pthread_join(pText_To_Speech->m_RequestHandlerThread, NULL);
