...
error_reporting(0);
  ...
  if(isset($_GET['page'])) $cuv=$_GET['cuv'];
  else if($_POST['cuvant']!='') $cuv=$_POST['cuvant'];
        else $cuv="asdfseegreawewf32423423423";
 ...
 if($l>$nr_link&& $l!=0 && !isset($_GET['page'])) 
      {
       ...
       for($i=0;$i<=$nr_link;$i++){
         ...
         $out.='<p>'.$descriere[$i].'<p>';
           ...
           for($i=2;$i<=$max_page;$i++) $out.='<a href="index.php?section=search&page='.$i.'&l='.$l.'&cuv='.$cuv.'"> ' .$i. ' </a>';
           $out.='</td></tr>';
           $max=$max_page;
      }
     ...
     // $out.='<table width="100%" border="0" cellpadding="0" cellspacing="0" class="maintable"><td width="20"></td><td class="insidetable">';
      for($i=$nr_link*($j-1);$i<min($nr,$l);$i++) 
      {
       ...
       $out.='<tr><td></td><td align="center">';
           for($i=1;$i<$j;$i++) $out.='<a href="index.php?section=search&page='.$i.'&l='.$l.'&cuv='.$cuv.'"> ' .$i. ' </a>';
           $out.=$j;
           for($i=$j+1;$i<$max_page;$i++) $out.='<a href="index.php?section=search&page='.$i.'&l='.$l.'&cuv='.$cuv.'"> ' .$i. ' </a>';
           $out.='</td></tr>';
  }
