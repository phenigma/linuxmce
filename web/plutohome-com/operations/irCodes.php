...
function irCodes($output)
{
	...
	<p>Because you can add new devices and infrared codes, you must <a href="'.$PlutoHomeHost.'index.php?section=register">register</a> or <a href="'.$PlutoHomeHost.'index.php?section=login">login</a> to use this section.  That way when you add a device, or update infrared codes, you become the "owner" of that device.  As the owner, you can make any changes to your device, or overwrite your infrared codes.  But if another user makes a change to your device, those changes are isolated in a special place and given a "Batch number".  As the owner of the device, you will be sent an email that another user wants to make changes to your device.  That email will include the proposed changes, and you can click a link in the email to accept or reject the changes.<br><br>
	...
	<p>If you do not want to be notified when other users change your device, you can freeze it (meaning no changes are allowed), or make it anonymous (meaning anyone can change it).';
		...
		$out=pickDeviceTemplate($GLOBALS['rootAVEquipment'],1,0,0,0,1,'irCodes',$firstColLinks,$publicADO,1);
	}
