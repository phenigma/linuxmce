...
//-->
	...
	<div align="center" class="confirm"><B>'.@$_REQUEST['msg'].'</B></div>	
		<form action="index.php" method="POST" name="irGrouptest">
			<input type="hidden" name="section" value="irGrouptest">
...
<span class="normaltext">If you have question or comments, please contact us by <a href="mailto:support@plutohome.com?subject=IR Group '.$irgID.' x UserID '.$userID.'">email</a>.</span><br><br>				
	';	
		...
		if(!is_null($deviceID)){
			$newIRGroup=((int)@$_POST['irGrouptest']>0)?(int)$_POST['irGrouptest']:NULL;
		//	$oldIRGroup=(int)$_POST['oldIRGroup'];
			if($newIRGroup!=$oldIRGroup){
				...
				header("Location: index.php?section=irGrouptest&from=$from&dtID=$dtID&deviceID=$deviceID&infraredGroupID=$newIRGroup&msg=IR Group changed for selected device template.&label=".$GLOBALS['label']);
				exit();
			}
		}
		...
		if(isset($_POST['irgroup_command']) && (int)$_POST['irgroup_command']>0){
			...
			if($action!='delete' && $action1!='add' && $action1!='edit'){ 
				...
				header("Location: index.php?section=irGrouptest&from=$from&dtID=$dtID&deviceID=$deviceID&infraredGroupID=$irgID&msg=Custom code added.&label=".$GLOBALS['label']);
				exit();
				...
				header("Location: index.php?section=irGrouptest&from=$from&dtID=$dtID&deviceID=$deviceID&infraredGroupID=$irgID&irgID=$irgID&msg=Custom code deleted.&label=".$GLOBALS['label']);
				exit();
			}
      ...
      if($action1=='add'){  
          ...
          $publicADO->Execute('INSERT INTO InfraredGroup_Command (FK_InfraredGroup,FK_Command, FK_Device, FK_DeviceTemplate, FK_Users,IRData) VALUES("'.$irgID.'","'.$irg_c.'","'.$deviceID.'","'.$dtID.'","'.$_SESSION['userID'].'","'.$irData1.'")');
			$igcID=$publicADO->Insert_ID();
				...
				header("Location: index.php?section=irGrouptest&from=$from&dtID=$dtID&deviceID=$deviceID&infraredGroupID=$irgID&irgID=$irgID&msg=Custom code has been added.&label=".$GLOBALS['label']);
				exit();
         }
		   if($action1=='edit'){  
		...
		if(isset($_POST['update'])){
			...
			header("Location: index.php?section=irGrouptest&from=$from&deviceID=$deviceID&dtID=$dtID&infraredGroupID=$irgID&msg=IR codes updated.&label=".$GLOBALS['label']);
			exit();
		}

		header("Location: index.php?section=irGrouptest&from=$from&dtID=$dtID&deviceID=$deviceID&infraredGroupID=$irgID&lastAction=".@$lastAction);
	}
	...
	foreach ($commandsToShow AS $commandID){
		...
		while($rowStandardCode=$resStandardCode->FetchRow()){
			...
			if(is_null($deviceID)){
				...
				$textAreaElem='
                 ...
                 <input type="hidden" name="idCommand" value="'.$rowStandardCode['PK_Command'].'">';
			}
			$out.='
				<tr bgcolor="lightblue">
		...
		while($rowUserCode=$resUserCode->FetchRow()){
			...
			if($rowUserCode['FK_Users']==@$_SESSION['userID']){
				$textareaUserCode='<textarea name="irData_'.$rowUserCode['PK_InfraredGroup_Command'].'" rows="2" style="width:100%" >'.$rowUserCode['IRData'].'</textarea>';
            $textname="irData_".$rowUserCode['PK_InfraredGroup_Command'];
			}else{
				$textareaUserCode='
					<textarea name="ir_'.$rowUserCode['PK_InfraredGroup_Command'].'" rows="2" style="width:100%" READONLY  onClick="setPreferred(\'prefered_'.$rowUserCode['FK_Command'].'_'.(($irgID==0)?'':$irgID).'\','.$rowUserCode['PK_InfraredGroup_Command'].')">'.$rowUserCode['IRData'].'</textarea>
               ...
               $textname="ir_".$rowUserCode['PK_InfraredGroup_Command'];
			}
			...
			$out.='
				<tr bgcolor="'.$RowColor.'">
					...
					<td align="left">'.$textareaUserCode.'</td>
					<td align="left" width="100">';
					if(isset($_SESSION['userID']) && $rowUserCode['FK_Users']==@$_SESSION['userID']){
						$out.='<input type="button" name="edit" onclick="document.irGrouptest.action1.value=\'edit\';document.irGrouptest.irgroup_command.value='.$rowUserCode['PK_InfraredGroup_Command'].';document.irGrouptest.submit();"value="Edit Code"><br><input type="button" class="button" name="delCustomCode" value="Delete code" onClick="if(confirm(\'Are you sure you want to delete this code?\')){document.irGrouptest.action.value=\'delete\';document.irGrouptest.irgroup_command.value='.$rowUserCode['PK_InfraredGroup_Command'].';document.irGrouptest.submit();}"> ';
					}
					$out.='<input type="button" name="select" value="Copy To Cliboard" onClick="javascript:this.form.'.$textname.'.focus();this.form.'.$textname.'.select();"></a>
					</td>
				</tr>';
     ...
     if(isset($_SESSION['userID']))
     {
		 $out.='
				<tr bgcolor="lightblue">
					<td align="center" width="100"><B>'.$rowStandardCode['Description'].'</B><br> User: '.$_SESSION['userID'].'</td>
					<td width="25">&nbsp;</td>
					<td><textarea name="irData1_'.$commandID.'" rows="2" style="width:100%"></textarea></td>
					<td align="center" width="100"><input type="button" class="button" value="Add New Code" name="add" onclick="document.irGrouptest.action1.value=\'add\';document.irGrouptest.irgroup_command.value='.$commandID.';document.irGrouptest.submit();"></td>
				</tr>';
     }
		$out.='</table>';	
	}
