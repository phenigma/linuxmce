...
function newIRGCode($output)
{
	...
	if ($action=='form') {		
		...
		$resDevice=$publicADO->Execute($selectDevice,$dtID1);

		$rowDevice=$resDevice->FetchRow();
			
		$out.='
		<form action="index.php" method="post" name="newIRGCode">
		<input type="hidden" name="section" value="newIRGCode">
      ...
      if($dtID!='')
        ...
        { if($dtID=='0') $dtID='';
			...
			header("Location: index.php?section=newIRGCode&deviceID=$deviceID&dtID=$dtID&infraredGroupID=$infraredGroupID&commandID=".$commandID);
		}
