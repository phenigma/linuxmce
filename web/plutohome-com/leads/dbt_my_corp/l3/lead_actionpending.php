...
class DB3_ActionPending_Row extends DB_Row
{
	/**
	 * @return DB3_ActionPending_Row
	 * @param $PKID_ActionPending str
	 * @desc Will return FALSE if a matching row is not found
	 * PKID_ActionPending can also be an Array, an Object, or an ID-key
	 */
	function DB3_ActionPending_Row($PKID_ActionPending='')
	{
		$this->dbt_path='dbt_my_corp/';
		parent::DB_Row('DB_ActionPending_Table',$PKID_ActionPending);
		if ($PKID_ActionPending==='') parent::PRT_set_fields(array('FKID_Lead'=>0,'FKID_Action'=>0));
	}

	/**
	 * @return int
	 * @param $as_string bool
	 * @desc Get Value of PKID_ActionPending
	 */
	function GET_PKID_ActionPending($as_string=false)
	{
		return parent::PRT_get_field('PKID_ActionPending',$as_string);
	}
...
class DB3_ActionPending_Table extends DB_Table
{
	function DB3_ActionPending_Table(&$db)
	{
		parent::DB_Table($db,'ActionPending','PKID_ActionPending',true);

		$this->fields['PKID_ActionPending'] =& new DB_Field_Int('PKID_ActionPending',false);
	...
	/**
	 * @desc STATIC: get the tablename 'ActionPending' 
	 */
	...
	{
		$fields = array('PKID_ActionPending'=>1,'FKID_Lead'=>1,'FKID_Action'=>1,'FKID_Employee'=>1,'StartDate'=>1,'DueDate'=>1,'Comments'=>1);
	 ...
	 * @return arr
	 * @param $id_keys arr
	 * @desc Fetch PKID_ActionPending column, rows matching $id_keys or false=current, true=all
	 */
	function FETCH_PKID_ActionPending($id_keys=false)
	{
		return parent::PRT_fetch_col('PKID_ActionPending',$id_keys);
	}
	/**
	 * @return arr
	 ...
	 * @param $limit int
	 * @param $start int
	 * @desc Load Table where PKID_ActionPending in $values 
	 * returns $id_keys for use in FETCH_*() 
	 * NOTE: $start NOT supported by MS-SQL
	 */
	function LOADBY_PKID_ActionPending($values,$orderby='',$limit=-1,$start=0)
	{
		return parent::PRT_load_by_field('PKID_ActionPending',$values,$orderby,$limit,$start);
	}
	/**
	 * @return bool
	 * @desc Is PKID_ActionPending Required ?
	 */
	function ISREQ_PKID_ActionPending()
	{
		return true;
	}
