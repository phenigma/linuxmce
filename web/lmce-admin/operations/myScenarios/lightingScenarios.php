...
function lightingScenarios($output,$dbADO) {
	...
	include(APPROOT.'/languages/'.$GLOBALS['lang'].'/lightingScenarios.lang.php');
...
if($action=='form') {
	...
	if($resRooms->RecordCount()==0){
		$out.=$TEXT_LIGHTING_SCENARIOS_NO_ROOMS_CONST.' <a href="index.php?section=rooms">'.$TEXT_GOTO_ROOMS_CONST.'</a>';
	}
	...
	while($rowRooms=$resRooms->FetchRow()){
		...
		<tr bgcolor="#D1D9EA">
			...
			<td>&nbsp;</td>
			<td align="right"><input type="button" class="button" name="add" value="'.$TEXT_ADD_LIGHTING_SCENARIO_CONST.'" onClick="javascript:document.lightingScenarios.action.value=\'addToRoom\';document.lightingScenarios.roomID.value='.$rowRooms['PK_Room'].';document.lightingScenarios.roomName.value=\''.urlencode($rowRooms['RoomName']).'\';document.lightingScenarios.submit();"></td>
		</tr>
		<tr>
			<td colspan="3">'.autogeneratedScenarios($arrayID,0,$dbADO,0,'lightingScenarios',$rowRooms['PK_Room']).'</td>
		</tr>';
		$selectCommandGroups='
			SELECT * 
			FROM CommandGroup
		...
		$out.='
			<tr>
				<td colspan="3">'.$TEXT_NO_LIGHTING_SCENARIOS_CONST.'</td>
			</tr>';
		}
		$pos=0;
		$posInRoom=array();
		...
		while($rowCG=$resCommandGroups->FetchRow()){
			...
			<tr bgcolor="'.(($rowCG['PK_CommandGroup']==@$_REQUEST['lastAdded'])?'lightgreen':$color).'">
				...
				<td>
					...
					<a href="#" onClick="javascript:if(confirm(\''.$TEXT_LIGHTING_SCENARIOS_CONFIRM_DELETE_CONST.'\'))self.location=\'index.php?section=lightingScenarios&action=del&cgDelID='.$rowCG['PK_CommandGroup'].'\';">'.$TEXT_DELETE_CONST.'</a>
				</td>
			</tr>
			';
	...
	if(!$canModifyInstallation){
		Header('Location: index.php?section=lightingScenarios&error='.$TEXT_NOT_AUTHORISED_TO_MODIFY_INSTALLATION_CONST);
		exit();
	}
	if(isset($_GET['cgDelID']) && (int)$_GET['cgDelID']!=0){
		$cgToDelete=(int)$_GET['cgDelID'];
		deleteCommandGroup($cgToDelete,$dbADO);
		header("Location: index.php?section=lightingScenarios&msg=$TEXT_LIGHTING_SCENARIO_DELETED_CONST");
	}
	...
	if($action=='addToRoom'){
		...
		$msg=$TEXT_LIGHTING_SCENARIO_ADDED_CONST;
	}
	
	if($action=='testScenario'){
		$scenarioToTest=(int)$_REQUEST['editedCgID'];
		testScenario($scenarioToTest);
		header("Location: index.php?section=lightingScenarios&msg=$TEXT_LIGHTING_SCENARIO_TEST_CONST");
		exit();
	}
	...
	if(isset($_POST['updateCG']) || $action=='externalSubmit' || @(int)$_REQUEST['editedCgID']!=0 || $action=='addToRoom'){
		...
		*/
		...
		$msg=(isset($msg))?$msg:"$TEXT_LIGHTING_SCENARIO_UPDATED_CONST&lastAdded=".@$insertID;
	}
	...
	if(isset($_REQUEST['operation'])){
		...
		if($operation=='up'){
			...
			if(isset($offset[$fromPos+1])){
				...
				$msg=$TEXT_LIGHTING_SCENARIO_POSITION_UPDATED_CONST;
			}
		}else{
			$offset=array_keys($posInRoom);
			$fromPos=array_search($fromCgID,$offset);
	...
	$output->setPageTitle($TEXT_LIGHTING_SCENARIOS_CONST);
	$output->setNavigationMenu(array($TEXT_MY_SCENARIOS_CONST=>'index.php?section=myScenarios',$TEXT_LIGHTING_SCENARIOS_CONST=>'index.php?section=lightingScenarios'));
	$output->setScriptCalendar('null');
	$output->setBody($out);
	$output->setTitle(APPLICATION_NAME.' :: '.$TEXT_LIGHTING_SCENARIOS_CONST);
	$output->output();
