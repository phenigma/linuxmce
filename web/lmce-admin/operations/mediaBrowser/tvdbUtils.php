		...
		{ 
			...
			{ 	
				...
				$i++; 
			 	...
			 	$imgArray=array($processArray, $seasonArray);
 ...
 */
			...
			{	
			$xmlGrab = file_put_contents($xmlPath.'/'.$sID.'/'.$sID.'.'.$lang.'.zip' ,$dataFile) or die ('cant save!'); 
...
$ATotal = count($actorArray);
	...
	$i=0;
	$fileIdent = $_POST['fileID']; $attribType = 2;														//file id for reference
...
function addDirectors($directorArray, $fileID, $mediadbADO, $dbADO, $output, $con) 
{
	...
	$i=0;
	$fileIdent = $_POST['fileID']; $attribType = 1;
	while ($i < $dTotal)
	{	
...
{	//print_r($episodeData);
	...
	if ($chkCount < 1)
	{		
		$out.='inserting '.$epTitle.' into '.$_POST['filename'] ;
		$idQuery2="INSERT INTO File_Attribute  VALUES (\"$fileIdent\", \"$attrib\", 0, 0, NULL, NULL, NULL, 0, CURTIME(), NULL  )";
	mysql_query($idQuery2)  or die (mysql_error());
	} else
...
function addGenre($genreArray, $con ,$fileID, $mediadbADO, $dbADO, $output)
{
	...
	$i=0;
	$fileIdent = $_POST['fileID']; $attribType = 8;	
	while ($i < $gTotal)
	{	
...
function ids($episodeData, $con ,$attributeType, $mediadbADO, $dbADO, $output)
{//	print_r($episodeData);
	$idArray = array(array('TV Series ID'=>$episodeData['seriesID']), array( 'TV Program ID'=>$episodeData['epid']), array("Release Date" =>$episodeData['firstAir']), array("Episode" =>$episodeData['air_epNo']), array("Release Date" =>'Season '.$_POST['seasonNo']) );
...
ids($episodeData, $con ,$attributeType	, $mediadbADO, $dbADO, $output);
}
