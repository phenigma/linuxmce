<?php
function checkIMDB($output,$mediadbADO,$dbADO) {
	...
	if($action=='form'){
		...
		$out.='
			...
			<form action="index.php" method="POST" name="checkIMDB">
			<input type="hidden" name="section" value="checkIMDB">
			<input type="hidden" name="action" value="process">
			<input type="hidden" name="fileID" value="'.$fileID.'">
		
		'.IMDBSearch($fileData,$searchIndex,$mediadbADO).'
		</form>
	...
	// process area
		if(isset($_POST['doSearch'])){
			...
			header('Location: index.php?section=checkIMDB&fileID='.$fileID.'&scanned=1');
			exit();
		}
		...
		header('Location: index.php?section=checkIMDB&fileID='.$fileID);

	}
	
	$output->setReloadLeftFrame(false);
	$output->setMenuTitle($TEXT_FILES_AND_MEDIA_CONST.' |');
	$output->setPageTitle($TEXT_GET_INFO_ON_IMDB_CONST);	
	$output->setNavigationMenu(array($TEXT_EDIT_MEDIA_FILE_CONST=>'index.php?section=editMediaFile&fileID='.$fileID));
	$output->setScriptCalendar('null');
	$output->setBody($out);
	$output->setTitle(APPLICATION_NAME.' :: '.$TEXT_GET_INFO_ON_IMDB_CONST);
	$output->output();
}

function IMDBSearch($fileData,$searchIndex,$mediadbADO){
	if(isset($_REQUEST['scanned'])){
		...
		return IMDBSearchBox($fileData,$searchIndex,$mediadbADO);
	}
}

function IMDBSearchBox($fileData,$searchIndex,$mediadbADO){
...
function processDoSearch($fileData,$mediadbADO){	
	...
	$scanUrl='coverArtExtractor_IMDB.php?fileID='.$id.'&'.$searchParms;
...
function getScanResults($fileID,$mediadbADO){
	...
	if($res->RecordCount()==0){
		return 'No results, please use the filters and try again.<br><a href="index.php?section=checkIMDB&fileID='.$fileID.'">Back</a>';
	}
	$out='
	<table>
		<tr class="alternate_back">
	...
	while($row=$res->FetchRow()){
		...
		$out.='
		<tr class="alternate_back">
			<td colspan="2"><input type="radio" name="matched" value="'.$id.'"> <B>IMDB ID:</B> '.$row['ID'].'</td>
		</tr>
		<tr>
