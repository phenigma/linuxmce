...
#webfilterproxy Parental control - Alblasco1702
#Webfilter
if [[ -z $DisableWebfilter ]]; then
    if [ -z "$DisableWebfilter" -a "${DisableWebfilter+x}" = "x" ]; then
        echo "\$DisableWebfilter variable is set with empty value."
        sed -i "s/DisableWebfilter=/DisableWebfilter=1/" /etc/pluto.conf
    else
        echo "\$DisableWebfilter is not set set it now"
        echo 'DisableWebfilter=1' >> /etc/pluto.conf
    fi
	#Webfilter config
	mv /etc/dansguardian/dansguardian.conf /etc/dansguardian/dansguardian.conf.orig
	cp /etc/dansguardian/dansguardianf1.conf /etc/dansguardian/dansguardianf0.conf
	mkdir /etc/dansguardian/lists/f1/
	mkdir /etc/dansguardian/lists/f2/
	chown -R www-data.www-data /etc/dansguardian/
	chmod 755 /etc/dansguardian/lists/f1/
	chmod 755 /etc/dansguardian/lists/f2/
	
else
    echo "\$DisableWebfilter found and is set to \"$DisableWebfilter\"."
fi

#Proxy
if [[ -z $DisableProxy ]]; then
    if [ -z "$DisableProxy" -a "${DisableProxy+x}" = "x" ]; then
        echo "\$DisableProxy variable is set with empty value."
        sed -i "s/DisableProxy=/DisableProxy=1/" /etc/pluto.conf
    else
        echo "\$DisableProxy is not set set it now"
        echo 'DisableProxy=1' >> /etc/pluto.conf
    fi
	#Proxy config
    chown -R www-data.www-data /etc/squid
    mv /etc/squid/squid.conf /etc/squid/squid.conf.orig
else
    echo "\$DisableProxy found and is set to \"$DisableProxy\"."
fi