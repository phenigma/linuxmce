<?
$TEXT_NETWORK_SETTINGS_CONST='Network settings';
$TEXT_NO_DEVICEDATA_NETWORK_INTERFACES_CONST='No record in Device_DeviceData for Network Interfaces.';
$TEXT_OFFLINEMODE_CONST='OfflineMode';
$TEXT_CORE_IDENTIFICATION_CONST='Core identification on local network';
$TEXT_RESET_CONST='Reset';
$TEXT_COMPUTER_NAME_CONST='Computer name';
$TEXT_DOMAIN_CONST='Domain';
$TEXT_CORE_SEEN_FROM_INTERNET_CONST='Your core as seen from the Internet';
$TEXT_OUTSIDE_IP_CONST='Outside IP';
$TEXT_OUTSIDE_HOSTNAME_CONST='Outside hostname';
$TEXT_DHCP_SERVERS_CONST='DHCP and RA servers on local network';
$TEXT_IPV4_DHCP_ENABLED_CONST='IPv4 DHCP server enabled';
$TEXT_IPV4_DHCP_LMCE_RANGE_CONST='Range for LinuxMCE devices';
$TEXT_IPV4_DHCP_NONLMCE_ENABLED_CONST='Provide IP addresses for anonymous devices not in LinuxMCE\'s database.';
$TEXT_IPV4_DHCP_NONLMCE_RANGE_CONST='Range for non-LinuxMCE devices';
$TEXT_IPV6_RA_ENABLED_CONST='IPv6 Router Advertisement daemon enabled (radvd)';
$TEXT_IPV6_PUBLIC_PREFIX_CONST='IPv6 public prefix';
$TEXT_VPN_ENABLED_CONST='L2TP/IPSEC VPN server enabled';
$TEXT_VPN_PSK_CONST='Pre Shared Key (PSK)';
$TEXT_VPN_CLIENT_RANGE_CONST='Range for VPN clients';
$TEXT_NUMBER_OF_NIC_CONST='Number of core\'s network adapters';
$TEXT_EXTERNAL_NIC_CONST='External network card';
$TEXT_GET_IP_FROM_DHCP_CONST='Obtain an IP address from DHCP';
$TEXT_USE_STATIC_IP_CONST='Use a static IP address';
$TEXT_NETMASK_CONST='Subnet mask';
$TEXT_GATEWAY_CONST='Gateway';
$TEXT_NAMESERVER_CONST='Nameserver';
$TEXT_INTERNAL_NIC_CONST='Internal network card';
$TEXT_SWAP_INTERFACES_CONST='Swap Interfaces';
$TEXT_OPEN_FIREWALL_CONST='You may need to open up ports in the firewall for some programs that run on your internal computers, like video conferencing, file sharing, etc.  To do this, visit the Advanced, <a href="index.php?section=firewall">Firewall</a> page.'
?>