<?php
$TEXT_PARENT_CATEGORY_CONST='Parent Category';
$TEXT_EVENT_LIST_CATEGORY_NAME_CONST='Event List Category Name';
$TEXT_EVENT_CATEGORY_VALIDATION_CONST='Please enter a name for this Event List Category!';
$TEXT_PARENT_CATEGORY_VALIDATION_CONST='Please select a parent category!';
$TEXT_EVENT_LIST_CATEGORY_ADDED_CONST='Event List Category added!';
$TEXT_CREATE_EVENT_CATEGORY_CONST='Create event category';
?>