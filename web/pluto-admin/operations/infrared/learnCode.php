...
function learnCode($output,$dbADO) {
	...
	if ($action=='form') {	
		...
		if(isset($_REQUEST['cancel']) || $action=='cancel'){
			...
			$isOtherCustomCode=$dbADO->Execute('SELECT * FROM InfraredGroup_Command WHERE (FK_InfraredGroup=? OR FK_InfraredGroup IS NULL) AND FK_Command=? AND FK_Device=? AND FK_DeviceTemplate=? AND psc_user=?',array($infraredGroupID,$commandID, $deviceID,$dtID,$_SESSION['userID']));
			if($isOtherCustomCode->RecordCount()>0){
				$rowOther=$isOtherCustomCode->FetchRow();
				$dbADO->Execute('UPDATE InfraredGroup_Command SET IRData=? WHERE PK_InfraredGroup_Command=? AND psc_user=?',array($irData,$rowOther['PK_InfraredGroup_Command'],$_SESSION['userID']));
			}else
