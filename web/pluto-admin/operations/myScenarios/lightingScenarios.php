...
if($action=='form') {
	...
	$out.='
		...
		<form action="index.php" method="POST" name="lightingScenarios" onSubmit="return validateForm();">
			<input type="hidden" name="section" value="lightingScenarios">
			<input type="hidden" name="action" value="add">	
	...
	$out.='
		for(i=0;i<devicesNo;i++){
			eval("isDim=document.lightingScenarios.command_"+devicesArray[i]+"[3].checked");
			if(isDim==true){
				eval("dimVal=parseInt(document.lightingScenarios.dimValue_"+devicesArray[i]+".value)");
				if(dimVal<0 || dimVal>100 || isNaN(dimVal)){
					alert("Please enter a value between 0 and 100.");
					eval("document.lightingScenarios.dimValue_"+devicesArray[i]+".focus()");
					return false;
				}
			}
		}
