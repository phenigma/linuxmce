...
/*******************************************/
var GLOBAL_TARGET='_top';

function showHideLayers() { 
  var i,p,v,obj,args=showHideLayers.arguments;
  for (i=0; i<(args.length-2); i+=3) if ((obj=findMenuObj(args[i]))!=null) { v=args[i+2];
    if (obj.style) { obj=obj.style; v=(v=='show')?'visible':(v='hide')?'hidden':v; }
    obj.visibility=v; }
}
function findMenuObj(n, d) { 
  var p,i,x;  if(!d) d=document; if((p=n.indexOf("?"))>0&&parent.frames.length) {
    d=parent.frames[n.substring(p+1)].document; n=n.substring(0,p);}
  if(!(x=d[n])&&d.all) x=d.all[n]; for (i=0;!x&&i<d.forms.length;i++) x=d.forms[i][n];
  for(i=0;!x&&d.layers&&i<d.layers.length;i++) x=findMenuObj(n,d.layers[i].document);
  if(!x && document.getElementById) x=document.getElementById(n); return x;
}
function changeProp(objName,x,theProp,theValue) { 
  var obj = findMenuObj(objName);
...
function Build()
{
   ...
   for (var j=0;j<MaxMenuIndex;j++)
   {
     ...
     //document.write(" onClick=\""+Menu[j][5]+"\"");
     document.write(" >");     
     ...
     {   
     	if(Menu[j][0]==Menu[j][1])
        	LINK="<A href=\""+Menu[j][5]+"\" class=\"link_navbar\" target=\""+GLOBAL_TARGET+"\">";
        else
        	LINK="<A href=\""+Menu[j][5]+"\" class=\"link_small_navbar\" target=\""+GLOBAL_TARGET+"\">";
     }
     ...
     document.write("<table border='0' WIDTH='100%' height='100%' cellpadding='2' cellspacing='0'><tr>");     
     ...
     if(Menu[j][2]!="")
     {
       ...
       if(Menu[j][0]==Menu[j][1]){
       		document.write('style="background-image:url('+serverPath+'include/images/back_on.jpg);">');
       }
       	else{
       		document.write(">");
       	}
