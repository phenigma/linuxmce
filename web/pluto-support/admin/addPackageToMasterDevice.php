...
//	$publicADO->debug=true;
	...
	$rs = $publicADO->Execute($queryOperatingSystem);
	...
	$rs = $publicADO->_Execute($queryDistroOperatingSystem);
	while ($row = $rs->FetchRow()) {
	...
	$rs = $publicADO->_Execute($queryRepository);
	while ($row = $rs->FetchRow()) {
	...
	if ($action=='form') {
		...
		<input type="hidden" name="PK_Package" value="'.$PK_Package.'">
			<table>			
				<tr>
					...
					$querySelectPackage= '
						...
						LEFT JOIN  DeviceTemplate on FK_Package  = PK_Package
							WHERE PK_Package = ?';
				$rs = $publicADO->Execute($querySelectPackage,array($PK_Package));
				...
				$resSourceCode=$publicADO->execute($querySourceCode);
				...
				$resPackageType=$publicADO->execute($queryPackageType);
				...
				$out.='</select></td>
						...
						<tr>
							...
							<td align="left">'.generatePullDown('manufacturer','Manufacturer','PK_Manufacturer','Description',$rowPackage['FK_Manufacturer'],$publicADO).'</td>
						</tr>
				';
				if($PK_Package!=0){
					$out.='	<tr>
					...
					$resPackageCompat=$publicADO->Execute($queryPackageCompat,$PK_Package);
					...
					$resPackagesDependsOn=$publicADO->Execute($queryPackagesDependsOn,$PK_Package);
...
<!--									<td>'.$rowPackagesDependsOn['Description'].'</td> -->
					...
					$queryRemainingPackages='
						SELECT * 
							...
							ORDER BY Description ASC';
					$resRemainingPackages=$publicADO->Execute($queryRemainingPackages);
					...
					$resSources=$publicADO->execute($querySources,$PK_Package);
					...
					$resSourceCompatibility=$publicADO->Execute($querySourceCompatibility,$rowSource['PK_Package_Source']);
				...
				$resDirectoryPackage=$publicADO->Execute($queryDirectoryPackage,array($PK_Package));
				...
				while ($row = $resDirectoryPackage->FetchRow()) {
					...
					$resDirectoryFiles=$publicADO->Execute($queryDirectoryFiles,array($row['PK_Package_Directory']));
					$out.='<table width="550">';
					...
					$out.='</fieldset>';
						...
						$out.='
							...
							$rs = $publicADO->_Execute($queryDirectory);
							while ($row = $rs->FetchRow()) {
		...
		if ((int)$PK_Package!=0) {
			...
			$updatePackage='
				...
				WHERE PK_Package=?';
			$publicADO->Execute($updatePackage,array($description, $FK_Package_Sourcecode,$isSourceCode,$FK_PackageType,$FK_Manufacturer,$PK_Package));
			...
			foreach($displayedPackagesArray AS $value){
				...
				if(isset($_POST['del_'.$value])){
					...
					$publicADO->Execute($deletePackagePackage,array($PK_Package,$value));
				}else{
					$updatePackagePackage='UPDATE Package_Package SET OnlyToBuild=? WHERE FK_Package=? AND FK_Package_DependsOn=?';
					$publicADO->Execute($updatePackagePackage,array($OnlyToBuild,$PK_Package,$value));
				}
			}
			...
			if($newPackageDependsOn!=0){
				...
				$publicADO->Execute($insertPackagePackage,array($PK_Package,$newPackageDependsOn,0));
			}
			...
			if(isset($_POST['addSource'])){
				...
				$publicADO->Execute($insertPackageSource,$PK_Package);
			}
			...
			foreach($compatArray AS $value){
				...
				if(isset($_POST['deleteCompatibility_'.$value])){
					...
					$publicADO->Execute($deletePackageSourceCompat,$value);
				}else{
					...
					$publicADO->Execute($updatePackageSourceCompat,array($compatOperatingSystem,$compatDistro,$compatMustBuild,$compatComments,$value));
				}
			}
			...
			foreach($displayedSourcesArray AS $value){
				...
				$publicADO->Execute($updatePackageSource,array($sourceName,$sourceRepositorySource,$sourceRepository,$sourceVersion,$sourceParms,$value));
				...
				if(isset($_POST['deleteSource_'.$value])){
					...
					$publicADO->Execute($deletePackageSource,$value);
				}
				...
				if(isset($_POST['addCompatibility_'.$value])){
					...
					$publicADO->Execute($insertPackageSourceCompat,$value);
				}
			}
			...
			foreach($displayedPackageDirectoryArray as $elem){
				...
				$publicADO->Execute($updatePackageDirectory,array($pdOperatingSystem,$pdDistro,$pdPath,$pdInputPath,$pdFlipSource,$elem));
			}
			...
			foreach($displayedPackageDirectoryFilesArray as $elem){
				...
				$publicADO->Execute($updatePackageDirectoryFile,array($pdfOS,$pdfDistro,$pdfFile,$pdfMakeCommand,$elem));
			...
			foreach($PackageCompatArray AS $PK_Package_Compat){
				if(isset($_POST['deletePackageCompat_'.$PK_Package_Compat])){
					...
					$publicADO->Execute($deletePackageCompat,$PK_Package_Compat);
				}else{
					...
					$publicADO->Execute($updatePackageCompat,array($os,$distro,$PK_Package_Compat));
				}
			}
			...
			if(isset($_POST['addPackageCompatibility'])){
				...
				$publicADO->Execute($insertPackageCompat,array($PK_Package,NULL,NULL));
			}
			$out.="
			<script>
			...
			$publicADO->Execute($insertPackage,array($description,$FK_Package_Sourcecode,$isSourceCode,$FK_PackageType,$FK_Manufacturer));
			$PK_Package=$publicADO->Insert_ID();
			...
			if($deviceID!=0){		
				...
				$publicADO->Execute($updateDeviceTemplate,array($PK_Package,$deviceID));
			}
