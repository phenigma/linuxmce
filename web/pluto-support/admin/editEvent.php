<?php
	...
	if ($action=='form') {
		...
		$resSelectEvent = $publicADO->_Execute($querySelectEvent);
		...
		$resSelectEventCategory = $publicADO->_Execute($querySelectEventCategory);
		...
		if ($resSelectEventCategory) {
			while ($rowSelectEventCategory=$resSelectEventCategory->FetchRow()) {
				...
				$EventCategoryTxt.=getEventCategoryChildsArray($rowSelectEventCategory['PK_EventCategory'],$rowSelectEventCategory['Description'],$EventEventCategory,$publicADO);
			}
		}
		$out.='
		...
		";
		$resSelectEventParams = $publicADO->Execute($selectEventParams,array($selectedEvent));
		...
		$resSelRemainingEventParams = $publicADO->Execute($querySelRemainingEventParams);
		...
		foreach ($displayedEventParams as $param) {
			...
			$resOldValue = $publicADO->Execute($selectOldValue,array($selectedEvent,$param));
				if ($resOldValue && $resOldValue->RecordCount()==1)	{
					$rowOldValue = $resOldValue->FetchRow();
					 if ($rowOldValue['Description']!=$description) {
					 	...
					 	$resUpd = $publicADO->Execute($queryUpdateEventParameter,array($description,$selectedEvent,$param));
		...
		if ($addEventParameterToEvent!=0) {
			...
			$res = $publicADO->Execute($queryInsertEventParameter,array($selectedEvent,$addEventParameterToEvent));			
		...
		if ($category != 0 && $EventDescription!='' && $selectedEvent!=0) {
			...
			$publicADO->Execute($insertEvent,array($EventDescription,$category,$selectedEvent));
		} else {				
