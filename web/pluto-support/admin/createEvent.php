	...
	if ($action=='form') {
		...
		if ($resSelectEventCategory) {
			while ($rowSelectEventCategory=$resSelectEventCategory->FetchRow()) {
				...
				$EventCategoryTxt.=getEventCategoryChildsArray($rowSelectEventCategory['PK_EventCategory'],$rowSelectEventCategory['Description'],0,$publicADO);
			}
		}
		...
		if ($EventDescription!='' && $category!=0) {
			...
			$query = $publicADO->Execute($insertEvent,array($EventDescription,$category));
			$out.="
			<script>
