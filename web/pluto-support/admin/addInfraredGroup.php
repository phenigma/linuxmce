<?php
	...
	$deviceCategsTxt = '<option value="0">-please select-</option>';
		while ($row = $rs->FetchRow()) {
			...
			$deviceCategsTxt.=getDeviceCategoryChildsOptions($row['PK_DeviceCategory'],$row['Description'],0,'',$publicADO);
		}
	$rs->Close();
	...
	if ($action=='form') {		
		...
		if ($InfraredGroupDescription!='' && $deviceCategs!=0) {
			...
			$publicADO->Execute($queryInsertCmdGroup,array($FK_Manufacturer,$InfraredGroupDescription,$deviceCategs));
			$insertID = $publicADO->Insert_ID();
			
			$queryDevicesTemplates='SELECT * FROM DeviceTemplate WHERE FK_DeviceCategory=?';
			$resDeviceTemplates=$publicADO->Execute($queryDevicesTemplates,array($deviceCategs));
			while($row = $resDeviceTemplates->FetchRow()){
				$ins=$publicADO->Execute('INSERT INTO DeviceTemplate_InfraredGroup (FK_DeviceTemplate , FK_InfraredGroup) VALUES(?,?)',array($row['PK_DeviceTemplate'],$insertID));
			}
				$out.="
				<script>
