...
//	$publicADO->debug=true;
	...
	if ($action=='form') {
		...
		$rs = $publicADO->Execute($queryGetValues,array($infraredGroupID));
		if ($rs->RecordCount()==1) {
			$row=$rs->FetchRow();
		...
		if ($addNewCommandToThisCommandGroup!=0) {
			...
			$publicADO->Execute($query,array($infraredGroupID,$addNewCommandToThisCommandGroup));			
		...
		if ((int)$deviceCategory!=0 && (int)$deviceID!=0 && $description!='') {
			$insertObjToDevice = 'UPDATE InfraredGroup 
					...
					';			
			$query = $publicADO->Execute($insertObjToDevice,array($deviceCategory,stripslashes($description),$infraredGroupID));
			$out.="";
