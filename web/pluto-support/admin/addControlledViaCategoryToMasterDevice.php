	...
	if ($action=='form') {
					...
					<td>
						...
						$rs = $publicADO->_Execute($queryMasterDeviceCategories_parents);
							while ($row = $rs->FetchRow()) {
								...
								$out.=getDeviceCategoryChildsOptions($row['PK_DeviceCategory'],cleanString($row['Description']),0,$notIn,$publicADO);
							}
						$rs->Close();
		...
		if ((int)$controlledVia!=0 && (int)$deviceID!=0 ) {
			$insertObjToDevice = 'insert into DeviceTemplate_DeviceCategory_ControlledVia
					...
					values(?,?,?,?)';
			$query = $publicADO->Execute($insertObjToDevice,array($deviceID,$controlledVia,$reroute,$autocreate));
