<?php
	...
	if ($action=='form') {
		...
		/*$querySelectRemainingControlledViaForDevice = "
						...
						$rs = $publicADO->_Execute($querySelectRemainingControlledViaForDevice);
						while ($row = $rs->FetchRow()) {
		...
		while ($rowSelectedPipesUsed = $resSelectPipesUsed->FetchRow()) {	
			...
			$deviceTo=$rowSelectedPipesUsed['FK_To'];
				...
				$res=$publicADO->Execute($updateDevicePipe,array($input,$outputs,$pipe,$toChild,$objID));
		...
		if ((int)$controlledViaDeviceTemplateID!=0) {
			...
			where  PK_DeviceTemplate_DeviceTemplate_ControlledVia = ? 
					';
			$query = $publicADO->Execute($insertObjToDevice,array($_SESSION['editControlledViaToMasterDevice']['deviceTemplateID'],$reroute,$autocreate,$objID));
