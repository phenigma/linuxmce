<?php
	...
	if ($action=='form') {
		...
		$rs = $publicADO->Execute($queryGetValues,array($commandGroupID));
		if ($rs->RecordCount()==1) {
			$row=$rs->FetchRow();
					...
					<td>
						...
						$rs = $publicADO->_Execute($queryMasterDeviceCategories_parents);
							while ($row = $rs->FetchRow()) {
								...
								$out.=getDeviceCategoryChildsOptions($row['PK_DeviceCategory'],$row['Description'],$deviceCategoryID,'',$publicADO);
							}
						$rs->Close();
					$out.='
						</select>
				...
				<tr>
					<td colspan="2">		
						<fieldset>
							...
							$res = $publicADO->Execute($queryGetCommands);
		...
		foreach ($displayedCommands as $elem) {
			...
			if ($commandAssocDescription!='') {
				...
				$resD = $publicADO->Execute($query,array($commandGroupID,$elem));
				...
				$publicADO->Execute($query,array(stripslashes($commandAssocDescription),$commandGroupID,$elem));
			}
		}
		...
		if ($addNewCommandToThisCommandGroup!=0) {
			...
			$publicADO->Execute($query,array($commandGroupID,$addNewCommandToThisCommandGroup));			
		...
		if ((int)$deviceCategory!=0 && (int)$deviceID!=0 && $description!='') {
			$insertObjToDevice = 'UPDATE DeviceCommandGroup 
					...
					';			
			$query = $publicADO->Execute($insertObjToDevice,array($deviceCategory,stripslashes($description),$commandGroupID));
			$out.="";
