...
//the form	
	if ($action=='form') {
		...
		$rs = $publicADO->Execute($queryGetMasterDeviceDetails,array($deviceID));	
		...
		<form action="right.php" method="post" name="editMasterDevice" target="_self">
			<input type="hidden" name="section" value="operations/editMasterDevice">
		...
		$rs = $publicADO->_Execute($queryMasterDeviceCategories_parents);
							while ($row = $rs->FetchRow()) {
								...
								$out.=getDeviceCategoryChildsOptions($row['PK_DeviceCategory'],stripslashes($row['Description']),$deviceCategID,'',$publicADO);
							}
		$out.='			</select>
					</td>
				</tr>
		...
		$rs = $publicADO->_Execute($queryManufacturers);
							while ($row = $rs->FetchRow()) {								
		...
		$out.='		</select> <a href="javascript:void(0);" onClick="windowOpen(\'index.php?section=addManufacturer&from=editMasterDevice\',\'width=300,height=200,toolbars=true,resizable=1,scrollbars=1\');">Create Manufacturer</a>
				</td>
			</tr>
			<tr>
		...
		$rs = $publicADO->Execute($selectObjects,array($deviceID));
			while ($row = $rs->FetchRow()) {
		...
		$out.='		</td>
				...
				<tr>
					...
					$rs = $publicADO->Execute($selectControlled,array($deviceID));
						while ($row = $rs->FetchRow()) {
				...
				$rs = $publicADO->Execute($querySelectPackages);
				if($rs->RecordCount()==0)
					$out.='No records.<br>';
				while ($row = $rs->FetchRow()) {
				...
				<tr>
					...
					<td>
						<input type="hidden" value="'.$isAVDevice.'" name="old_isAVDevice"><input type="checkbox" name="isAVDevice" '.($isAVDevice==1?" checked='checked' ":"").' value="1" onClick="javascript:this.form.submit();"> 
						...
						'.(($isAVDevice==1)?"<input type=\"button\" class=\"button\" name=\"irCodes\" value=\"IR Codes\" onClick=\"windowOpen('index.php?section=irCodes&from=editMasterDevice&dtID={$deviceID}&infraredGroupID=0','status=0,resizable=1,width=800,height=600,toolbars=true,scrollbars=1,resizable=1');\">":'').' 
					</td>
				</tr>
				<tr>
				...
				$resDeviceData = $publicADO->_Execute($deviceData);
				$usedParams = array();
				$usedParams[] = 0;
				...
				$rs = $publicADO->_Execute($querySelectRemainingParameterForDevice);
				while ($row = $rs->FetchRow()) {
				...
				$resSelCheckedCommandGroups = $publicADO->Execute($querySelCheckedCommandGroups,$deviceID);
				...
				$query = "select  ".$publicADO->IfNull('FK_DeviceCategory_Parent','0')." as a from DeviceCategory 				
							WHERE 
								PK_DeviceCategory = ?
				";
				$deviceParent=0;
				$res = $publicADO->Execute($query,array($deviceCategID));
				if ($res) {
				...
				";
				$resCommands = $publicADO->_Execute($query);
		...
		$out.='		
				...
				<fieldset>
					...
					$resEventsSelected = $publicADO->Execute($queryEventsSelected,array($deviceID));
					...
					$resEventsList = $publicADO->_Execute($queryEventsList);
					if ($resEventsList) {					
				...
				<fieldset>
            		<legend>Plug & Play</legend>
						<table>';
					$dhcpArray=array();
					$resDHCP=$publicADO->Execute('SELECT * FROM DHCPDevice WHERE FK_DeviceTemplate=?',$deviceID);
					$out.='
							<tr>
			...
			$resDT_DO=$publicADO->Execute($queryDT_DO,$deviceID);
			$linksArray=array();
			...
			$out.='		
				...
				$querySelectCustomPages = '
					...
					';
				$resSelectCustomPages = $publicADO->Execute($querySelectCustomPages,array($deviceID));
		...
		if(isset($_REQUEST['doID'])){
			...
			$publicADO->Execute('DELETE FROM DeviceTemplate_DesignObj WHERE FK_DeviceTemplate=? AND FK_DesignObj=?',array($deviceID,$doID));
		...
		if($newMacFrom!='' && $newMacTo!=''){
			$publicADO->Execute('INSERT INTO DHCPDevice (FK_DeviceTemplate, Mac_Range_Low, Mac_Range_High,FK_Manufacturer,FK_DeviceCategory,Description) VALUES (?,?,?,?,?,?)',array($deviceID,$newMacFrom,$newMacTo,$newManufacturer,$newCategory,$newComment));
		...
		foreach ($dhcpArray AS $dhcpID){
			...
			$publicADO->Execute('UPDATE DHCPDevice SET Mac_Range_Low=?, Mac_Range_High=?,FK_Manufacturer=?,FK_DeviceCategory=?,Description=? WHERE PK_DHCPDevice=?',array($macFrom, $macTo,$newManufacturer,$newCategory,$newComment,$dhcpID));
		}
		
		if($_REQUEST['toDel']!=''){
			$dhcpID=(int)$_REQUEST['toDel'];
			if($dhcpID!=0){
				$publicADO->Execute('DELETE FROM DHCPDevice WHERE PK_DHCPDevice=?',$dhcpID);
			}
		}
		
		if($newScreen!=0){
			$newScreenExist=$publicADO->Execute('SELECT * FROM DesignObj WHERE PK_DesignObj=?',$newScreen);
			if($newScreenExist->RecordCount()>0){
				$screenDT=$publicADO->Execute('SELECT * FROM DeviceTemplate_DesignObj WHERE FK_DeviceTemplate=? AND FK_DesignObj=?',array($deviceID,$newScreen));
				if($screenDT->RecordCount()>0){
					header("Location: index.php?section=editMasterDevice&model=$deviceID&error=Screen already exist!");
					exit();
				}else{
					$publicADO->Execute('INSERT INTO DeviceTemplate_DesignObj (FK_DeviceTemplate,FK_DesignObj) VALUES (?,?)',array($deviceID,$newScreen));
				}
			}else{
		...
		foreach ($usedParamsArray as $param) {
			...
			if ($descriptionParam!='') {
				...
				$query = "update DeviceTemplate_DeviceData
						...
						";			
				$publicADO->Execute($query,array($defaultValueParam,$descriptionParam,$requiredParam,
			...
			$deviceCommandGroup = cleanInteger(@$_POST['DeviceCommandGroup_'.$param]);
				...
				if ($resOldValuesData && $resOldValuesData->RecordCount()==1) {					
					if ($deviceCommandGroup==0) {
						...
						$publicADO->Execute($queryForCommandGroup);
					...
					//insert record in database
						if ($deviceCommandGroup==1) {
								...
								$publicADO->Execute($queryForCommandGroup,array($param,$deviceID));
								//$locationGoTo = "DeviceCommandGroup_".$param;
								$locationGoTo = "#commands_link";								
						}
				}				
		...
		if ($newEventToMasterDevice!=0) {
			...
			$publicADO->Execute($query,array($deviceID,$newEventToMasterDevice));
		...
		$usedEventsArray = explode(",",$usedEvents);
		 foreach ($usedEventsArray as $elem) {
		 	...
		 	$resOldValue = $publicADO->Execute($selectOldValue,array($deviceID,$elem));
		 	$oldDescForEvent='';
		 		if ($resOldValue) {
		 	...
		 	if ($desc!='' && $desc!=$oldDescForEvent) {		 		
		 		...
		 		$publicADO->Execute($query,array($desc,$deviceID,$elem));
		...
		if ($newDeviceData!=0 && $deviceID!=0) {
			...
			$publicADO->Execute($query,array($deviceID,$newDeviceData));
			
			$publicADO->Execute('INSERT INTO Device_DeviceData (FK_DeviceData, FK_Device) SELECT '.$newDeviceData.',PK_Device FROM Device WHERE FK_DeviceTemplate=?',$deviceID);
		...
		if ($isAVDevice!=$old_isAVDevice) {			
			if($isAVDevice==1){
				...
				$publicADO->Execute('DELETE FROM DeviceTemplate_AV WHERE FK_DeviceTemplate=?',$deviceID);
				$publicADO->Execute('DELETE FROM DeviceTemplate_DeviceCategory_ControlledVia WHERE FK_DeviceTemplate=?',$deviceID);
			}
		}
		...
		$publicADO->Execute($updateQuery,array($description,$ImplementsDCE,$commandLine,$category,$manufacturer,$package,$isPlugIn,$isEmbedded,$inheritsMacFromPC,$isIPBased,$ConfigureScript,$comments,$manufacturerURL,$internalURLsufix,$deviceID));
		...
		if($isPlugIn==1 && $oldIsPlugIn==0){
			$insertControlledVia = '
				...
				VALUES(?,?)';
			$query = $publicADO->Execute($insertControlledVia,array($deviceID,$GLOBALS['rootDCERouter']));
		
		}	
			if (strstr($locationGoTo,"#")) {
				header("Location: right.php?section=operations/editMasterDevice&model=$deviceID&msg=Saved!".$locationGoTo);
			} else {
				header("Location: right.php?section=operations/editMasterDevice&model=$deviceID&msg=Saved!&lastAction=".$locationGoTo);
			}
		} else {
			header("Location: right.php?section=operations/editMasterDevice&model=$deviceID&msg=Missed parrameters!");
		}
