...
extern "C" {
	...
	{ 
		...
		if( bIsRuntimePlugin )
			return Security_Plugin::PK_DeviceTemplate_get_static();
		else
			return 0;
	}
}
...
int main(int argc, char* argv[]) 
{
	...
	cout << "Security_Plugin, v." << VERSION << endl
	...
	if (bError)
	{
		cout << "A Pluto DCE Device.  See www.plutohome.com/dce for details." << endl
			<< "Usage: Security_Plugin [-r Router's IP] [-d My Device ID] [-l dcerouter|stdout|null|filename]" << endl
	...
	{
		if( sLogger=="dcerouter" )
			g_pPlutoLogger = new ServerLogger(PK_Device, Security_Plugin::PK_DeviceTemplate_get_static(), sRouter_IP);
	...
	try
	{
		...
		if ( pSecurity_Plugin->Connect(pSecurity_Plugin->PK_DeviceTemplate_get()) ) 
		{
			...
			pSecurity_Plugin->CreateChildren();
			pthread_join(pSecurity_Plugin->m_RequestHandlerThread, NULL);
