 ...
 * used to append the contents of the 'bootargs' environment variable prior
 * to running 'localcmd'.
 *
 * Returns 1 on success or < 0 on error.
 */
...
static int label_boot(cmd_tbl_t *cmdtp, struct pxe_label *label)
{
	...
	if (label->append) {
		appends = malloc(strlen(label->append) + 1);
		if (!appends)
			return 1;
		appends[0] = '\0';
		strcpy(appends,label->append);
		len += strlen(appends);
	}
	...
	if (len) {
		...
		strcat(newbootargs, appends);
		strcat(newbootargs, ip_str);
		strcat(newbootargs, mac_str);
