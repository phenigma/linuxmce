...
static int label_localboot(struct pxe_label *label)
{
	...
	if (label->append)
		setenv("bootargs", strcat(bootargs,label->append));
...
printf("Start:Length - %s:%s\n", getenv("fileaddr"), getenv("filesize"));
		initrdaddr = strcat(initrdaddr,"0x");
		initrdaddr = strcat(initrdaddr,getenv("fileaddr"));
		initrdaddr = strcat(initrdaddr,":0x");
		initrdaddr = strcat(initrdaddr,getenv("filesize"));
		bootm_argv[2] = initrdaddr;
printf("again - %s\n", bootm_argv[2]);
	} else {
		bootm_argv[2] = "-";
	}
