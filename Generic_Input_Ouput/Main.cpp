...
extern "C" {
	class Command_Impl *RegisterAsPlugIn(class Router *pRouter,int PK_Device,string sLogger)
	{
		...
		Generic_Input_Ouput *pGeneric_Input_Ouput = new Generic_Input_Ouput(PK_Device, "localhost",true,false,pRouter);
		if( pGeneric_Input_Ouput->m_bQuit )
		{
			delete pGeneric_Input_Ouput;
			return NULL;
		}
		return pGeneric_Input_Ouput;
	}
}
//<-dceag-plug-e->
...
int main(int argc, char* argv[]) 
{
	...
	try
	{
		Generic_Input_Ouput *pGeneric_Input_Ouput = new Generic_Input_Ouput(PK_Device, sRouter_IP);	
		if ( pGeneric_Input_Ouput->Connect() ) 
		{
			g_pPlutoLogger->Write(LV_STATUS, "Connect OK");
			pGeneric_Input_Ouput->CreateChildren();
			pthread_join(pGeneric_Input_Ouput->m_RequestHandlerThread, NULL);
